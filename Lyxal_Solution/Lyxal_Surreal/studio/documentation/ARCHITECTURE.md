...
│                 (Système DB-Driven)                        │
├────────────────────────────────────────────────────────────┤
│  • studio_component (composants réutilisables)            │
│    ├── identity (code, value, slug)                        │
│    ├── structure (type, props, children, variants)        │
│    ├── config (props_schema, category, version)           │
│    └── Parser TypeScript modulaire                         │
│                                                            │
│  • StudioComponentRenderer (renderer principal)           │
│  • StructureRenderer (rendu récursif)                     │
│  • ContextManager (fusion des contextes)                  │
└────────────────┬───────────────────────────────────────────┘
                 │
                 ↓ Structure JSON → Composants React
┌────────────────────────────────────────────────────────────┐
...
## 🔧 Système de Composants DB-Driven

### Vue d'Ensemble

Lyxal Studio implémente un **système révolutionnaire de composants 100% pilotés par base de données**. Contrairement aux approches traditionnelles où les composants sont codés en dur, ici **tout est défini dans SurrealDB** et rendu dynamiquement.

### Architecture du Système

```
┌─────────────────────────────────────────────────────────────┐
│                SURREALDB (Définition Composants)            │
│                                                             │
│  studio_component:button                                    │
│    ├── identity.code: "button"                              │
│    ├── structure: { type: "button", props: {...} }          │
│    ├── config.props_schema: [...]                           │
│    └── status.is_active: true                               │
│                                                             │
│  studio_page:dashboard                                      │
│    └── content_structure: { type: "div", children: [...] }  │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ↓ Parser TypeScript Modulaire
┌─────────────────────────────────────────────────────────────┐
│                REACT RUNTIME (Interprétation)               │
│                                                             │
│  useStudioComponent("button")                               │
│    ↓                                                        │
│  parseComponent(structure, props, context)                  │
│    ↓                                                        │
│  <StudioComponentRenderer> → <button>Content</button>       │
└─────────────────────────────────────────────────────────────┘
```

### Table `studio_component` - Structure Complète

Chaque composant est défini avec **8 blocs fonctionnels** :
...
```json
{
  ...
  "structure": {
    ...
    "variants": { "primary": { "css_classes": ["bg-blue-500"] } }
  },
  "config": {
    "category": "atom",
    "version": "1.0.0",
...
Le système utilise un **parser modulaire** qui transforme la structure JSON en composants React :

```typescript
// lib/studio/parser/index.ts
export const parseComponent = (
  structure: StudioComponentStructure,
  props: Record<string, any>,
  context: TemplateContext
): React.ReactElement => {
  // Pipeline : resolveTemplate → resolveProps → createReactElement
  return createReactElement(structure, props, context);
};
```

**Modules du parser :**
- `resolveTemplate.ts` - Résout les `{{variables}}`
- `resolveProps.ts` - Fusionne props structure + props composant
- `resolveChildren.ts` - Parse récursivement les enfants
- `applyVariants.ts` - Applique les variants CSS
- `createReactElement.ts` - Crée l'élément React final

### Composants Runtime Principaux

#### `StudioComponentRenderer`
**Rôle** : Point d'entrée principal pour rendre un composant DB.

```tsx
<StudioComponentRenderer
  code="button"
  props={{ label: "Cliquez-moi", variant: "primary" }}
  context={{ user, tenant }}
/>
```

#### `StructureRenderer`
**Rôle** : Rend récursivement les structures complexes (pages, layouts).

#### `ContextManager`
**Rôle** : Fusionne tous les contextes (page, user, tenant, props, state).

### Hooks Utilisateur

```typescript
// Charger un composant depuis DB
const { component, loading, error } = useStudioComponent('button');

// Accès au state global
const globalState = useStudioState();

// Gestion des actions
const { handleAction } = useActionHandler();

// Charger une page
const { page } = useStudioPage('dashboard');
```

### Exemple Complet : Bouton DB-Driven
...
CREATE studio_component:button SET
  ...
  structure = {
    type = "button",
    props = { className = ["btn", "{{props.variant}}"] },
    children = [{ type = "text", content = "{{props.label}}" }]
  },
  config = {
    props_schema = [
...
<StudioComponentRenderer code="button" props={{ label: "Save" }} />
```

**Résultat rendu :** `<button class="btn primary">Save</button>`

### Avantages du Système

- ✅ **Sécurité** : Validation stricte, pas d'injection HTML
- ✅ **Performance** : Rendu React natif optimisé
- ✅ **Réutilisabilité** : Composants partagés entre tenants
- ✅ **Maintenance** : Modification UI sans redéploiement code
- ✅ **White-Label** : Interfaces complètement différentes par tenant
- ✅ **Type-Safe** : Validation TypeScript des props

### Intégration dans les Pages

Les pages utilisent le même système pour définir leur contenu complet :

```surql
CREATE studio_page:dashboard SET
  identity = { code = "dashboard" },
  content_structure = {
    type = "div",
    children = [
      { type = "component", component = "header", props = { title = "Dashboard" } },
      { type = "component", component = "stats_grid", props = { data = "{{page.stats}}" } }
    ]
  };
```

```tsx
<StudioPageRenderer pageCode="dashboard" />
```
...
### Documentation Runtime Complète

1. **[STUDIO_COMPONENT_SCHEMA.md](../runtime/STUDIO_COMPONENT_SCHEMA.md)** → Schéma complet `studio_component` (8 blocs)
2. **[STUDIO_PAGE_SCHEMA.md](../runtime/STUDIO_PAGE_SCHEMA.md)** → Schéma complet `studio_page` et `content_structure`
3. **[SYSTEME_RENDU.md](../runtime/SYSTEME_RENDU.md)** → Approche JSON unique (pas HTML)
4. **[ORDRE_IMPLEMENTATION.md](../runtime/ORDRE_IMPLEMENTATION.md)** → Guide de démarrage corrigé

### Documentation Architecture & Intégration

5. **[DATABASE.md](./DATABASE.md)** → Voir schémas avec champs mobile/web
6. **[FUNCTIONS.md](./FUNCTIONS.md)** → Code de toutes les fonctions
7. **[GUIDE.md](./GUIDE.md)** → Guide d'utilisation pratique
8. **[INTEGRATION.md](./INTEGRATION.md)** → Intégration React + React Native
9. **[MOBILE.md](./MOBILE.md)** → Guide complet React Native
10. **[DAISYUI.md](./DAISYUI.md)** → Guide complet DaisyUI

### Documentation Thèmes & Assets

11. **[ICONS_RUNTIME.md](../runtime/ICONS_RUNTIME.md)** → Système d'icônes DB-driven
12. **[THEMES_RUNTIME.md](../runtime/THEMES_RUNTIME.md)** → Système de thèmes DB-driven
13. **[TEMPLATES_PAGES.md](../runtime/TEMPLATES_PAGES.md)** → Système de templates (en réflexion)
