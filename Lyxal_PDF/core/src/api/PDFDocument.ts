...
import { RandomAccessReader } from 'src/core/io/RandomAccessReader';
...
export default class PDFDocument {
  static async loadStream(
    pdf: RandomAccessReader,
    options: LoadOptions = {},
  ): Promise<PDFDocument> {
    // Avoid circular dependency by lazy importing
    const { PDFObjectLoader } = require('src/core/io/PDFObjectLoader');
    
    // Create Context first
    const context = PDFContext.create();
    
    // Pass context to loader so parsed objects are bound to it
    const loader = new PDFObjectLoader(pdf, context);
    await loader.readXrefTable();
    
    const refs = loader.getAllObjectRefs();
    for (const ref of refs) {
        const obj = await loader.loadObject(ref);
        console.log(`DEBUG: Loaded ${ref.toString()} -> ${obj.constructor.name}`);
        context.assign(ref, obj);
    }
    
    context.trailerInfo = loader.getTrailerInfo();
    console.log('DEBUG: Trailer Info:', JSON.stringify(context.trailerInfo, null, 2));
    if (context.trailerInfo.Root) {
       console.log('DEBUG: Root Ref:', context.trailerInfo.Root.toString());
       console.log('DEBUG: Root Lookup:', context.lookup(context.trailerInfo.Root));
    }
