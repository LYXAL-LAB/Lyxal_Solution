...
import { calculateSHA384, calculateSHA512 } from "./calculate_sha_other";
import { Dict, isName, Name } from "./primitives";
import { calculateMD5 } from "./calculate_md5";
import { calculateSHA256 } from "./calculate_sha256";
...
export class AESBaseCipher {
  ...
  decryptBlock(data: Uint8Array, finalize: boolean, iv: Uint8Array | null = null): Uint8Array {
    const sourceLength = data.length;
  ...
  encrypt(data: Uint8Array, iv: Uint8Array | null = null) {
    const sourceLength = data.length;
    let buffer = this.buffer,
      bufferLength = this.bufferPosition;
    const result: Uint8Array[] = [];
...
class PDFBase {
  ...
  checkUserPassword(password: Uint8Array, userValidationSalt: Uint8Array, userPassword: Uint8Array) {
    ...
    const result = this._hash(password, hashData, new Uint8Array(0));
  ...
  getUserKey(password: Uint8Array, userKeySalt: Uint8Array, userEncryption: Uint8Array) {
    ...
    const key = this._hash(password, hashData, new Uint8Array(0));
    const cipher = new AES256Cipher(key);
...
export class PDF20 extends PDFBase {
  _hash(password: Uint8Array, input: Uint8Array, userBytes: Uint8Array): Uint8Array {
    ...
    while (i < 64 || e.at(-1)! > i - 32) {
      ...
      // @ts-ignore
      const remainder = e.slice(0, 16).reduce((a, b) => a + b, 0) % 3; // Simplified for TS, need verification if sumPrecise exists
      if (remainder === 0) {
        // @ts-ignore
...
export class CipherTransform {
  StringCipherConstructor: any;
  StreamCipherConstructor: any;
  ...
  createStream(stream: any, length: number) {
...
export class CipherTransformFactory {
  ...
  #prepareKeyData(
    ...
    let cipher: ARCFourCipher, checkData;
  ...
  #buildCipherConstructor(cf: Dict | null, name: Name | null, num: number, gen: number, key: Uint8Array) {
  ...
  constructor(dict: Dict, fileId: string, password: string | null) {
    ...
    if (algorithm !== 5) {
      ...
      encryptionKey = this.#createEncryptionKey20(
        revision,
        passwordBytes || new Uint8Array(0),
        ownerPassword,
        ownerValidationSalt,
        ownerKeySalt,
        uBytes,
