...
import { DrawOPS, info, OPS } from "../shared/util";
...
export function recoverGlyphName(name: string, glyphsUnicodeMap: any): string {
 ...
 */
export function type1FontGlyphMapping(properties: any, builtInEncoding: any, glyphNames: string[]): Record<number, number> {
  ...
  if (properties.isInternalFont) {
    ...
    for (const charCodeStr in builtInEncoding) {
      charCodeToGlyphId[parseInt(charCodeStr)] = builtInEncoding[charCodeStr];
    }
  } else {
  ...
  let glyphsUnicodeMap: any;
  if (differences) {
    for (const charCodeStr in differences) {
...
export const getVerticalPresentationForm = getLookupTableFactory((t: any) => {
...
export function compileType3Glyph({ data: img, width, height }: { data: Uint8Array, width: number, height: number }): any[] | null {
