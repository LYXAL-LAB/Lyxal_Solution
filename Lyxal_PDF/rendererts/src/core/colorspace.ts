...
function resizeRgbaImage(
  ...
  if (alpha01 === 1) {
    ...
    const src32 = new Uint32Array(src.buffer, src.byteOffset, src.byteLength >> 2);
    const dest32 = new Uint32Array(dest.buffer, dest.byteOffset, dest.byteLength >> 2);
...
export class ColorSpace {
   ...
   */
  getRgb(src: TypedArray, srcOffset: number, output: Uint8ClampedArray = new Uint8ClampedArray(3)): Uint8ClampedArray {
    this.getRgbItem(src, srcOffset, output, 0);
    return output;
  }

  getRgbHex(src: TypedArray, srcOffset: number): string {
   ...
   * array).
   */
  getRgbBuffer(src: TypedArray, srcOffset: number, count: number, dest: TypedArray, destOffset: number, bits: number, alpha01: number) {
   ...
   */
  isDefaultDecode(decodeMap: number[], bpc: number): boolean {
  ...
  fillRgb(
    ...
    let rgbBuf: TypedArray | null = null;
   ...
   */
  static isDefaultDecode(decode: number[], numComps: number) {
...
export class AlternateCS extends ColorSpace {
  ...
  constructor(numComps: number, base: ColorSpace, tintFn: any) {
...
export class PatternCS extends ColorSpace {
  ...
  isDefaultDecode(decodeMap: number[], bpc: number) {
...
export class CalGrayCS extends ColorSpace {
  ...
  constructor(whitePoint: number[], blackPoint: number[] | null, gamma: number) {
    super("CalGray", 1);
...
export class CalRGBCS extends ColorSpace {
  ...
  whitePoint: Float32Array | number[];
  blackPoint: Float32Array | number[];
  GR: number;
  GG: number;
  GB: number;
  MXA: number;
  ...
  constructor(whitePoint: Float32Array | number[], blackPoint: Float32Array | number[] | null, gamma: Float32Array | number[] | null, matrix: Float32Array | number[] | null) {
    super("CalRGB", 3);
  ...
  #matrixProduct(a: TypedArray, b: TypedArray, result: TypedArray) {
    result[0] = a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
    result[1] = a[3] * b[0] + a[4] * b[1] + a[5] * b[2];
    result[2] = a[6] * b[0] + a[7] * b[1] + a[8] * b[2];
  }

  #toFlat(sourceWhitePoint: TypedArray, LMS: TypedArray, result: TypedArray) {
    result[0] = (LMS[0] * 1) / sourceWhitePoint[0];
    result[1] = (LMS[1] * 1) / sourceWhitePoint[1];
    result[2] = (LMS[2] * 1) / sourceWhitePoint[2];
  }

  #toD65(sourceWhitePoint: TypedArray, LMS: TypedArray, result: TypedArray) {
    const D65X = 0.95047;
    const D65Y = 1;
    const D65Z = 1.08883;
  ...
  #compensateBlackPoint(sourceBlackPoint: TypedArray, XYZ_Flat: TypedArray, result: TypedArray) {
  ...
  #normalizeWhitePointToFlat(sourceWhitePoint: TypedArray, XYZ_In: TypedArray, result: TypedArray) {
  ...
  #normalizeWhitePointToD65(sourceWhitePoint: TypedArray, XYZ_In: TypedArray, result: TypedArray) {
    const LMS = result;
    this.#matrixProduct(CalRGBCS.#BRADFORD_SCALE_MATRIX, XYZ_In, LMS);
  ...
  #toRgb(src: TypedArray, srcOffset: number, dest: TypedArray, destOffset: number, scale: number) {
    ...
    this.#normalizeWhitePointToFlat(this.whitePoint as TypedArray, XYZ, XYZ_Flat);

    const XYZ_Black = CalRGBCS.#tempConvertMatrix1;
    this.#compensateBlackPoint(this.blackPoint as TypedArray, XYZ_Flat, XYZ_Black);
...
export class LabCS extends ColorSpace {
  ...
  constructor(whitePoint: number[], blackPoint: number[] | null, range: number[] | null) {
    super("Lab", 3);
  ...
  #toRgb(src: TypedArray, srcOffset: number, maxVal: number | false, dest: TypedArray, destOffset: number) {
