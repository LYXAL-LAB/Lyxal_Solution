...
class Root extends XFAObject {
  ...
  constructor(ids: Map<string, any>) {
    super(-1, "root");
    this.element = null;
    this[$ids] = ids;
  }
...
class Builder {
  ...
  _namespacePrefixes: Map<string, any[]>;
  _namespaces: Map<string, any>;
  _nextNsId: number;
  _currentNamespace: any;

  constructor(rootNameSpace: any = null) {
  ...
  buildRoot(ids: Map<string, any>) {
    return new Root(ids);
  }

  build({ nsPrefix, name, attributes, namespace, prefixes }: any) {
  ...
  _addNamespacePrefix(prefixes: any[]) {
