...
import { Lexer, TOKEN, Token } from "./formcalc_lexer.js";
...
const Operators: any = {
  ...
  // Unary operators.
  minus: { id: 4, nargs: 1, prec: 2, assoc: RTL, repr: "-", op: (x: any) => -x },
  plus: { id: 5, nargs: 1, prec: 2, assoc: RTL, repr: "+", op: (x: any) => +x },
  not: {
    id: 6,
    nargs: 1,
    prec: 2,
  ...
  mul: { id: 7, nargs: 2, prec: 3, assoc: LTR, repr: "*", op: (x: any, y: any) => x * y },
  div: { id: 8, nargs: 2, prec: 3, assoc: LTR, repr: "/", op: (x: any, y: any) => x / y },

  add: { id: 9, nargs: 2, prec: 4, assoc: LTR, repr: "+", op: (x: any, y: any) => x + y },
  sub: {
    id: 10,
    nargs: 2,
    prec: 4,
...
class SimpleExprParser {
  ...
  static checkPrecedence(left: any, right: any) {
    return (
...
class AstCall extends Leaf {
  ...
  constructor(callee: any, params: any[]) {
    super();
    this.callee = callee;
    this.params = params;
  }
...
class AstSubscript extends Leaf {
  ...
  constructor(operand: any, index: any) {
    super();
...
class AstBinaryOperator extends Leaf {
  ...
  constructor(id: number, left: any, right: any, repr: string) {
    super();
    this.id = id;
    this.left = left;
    this.right = right;
  ...
  static getOperatorOrValue(operator: any, left: any, right: any) {
...
class AstUnaryOperator extends Leaf {
  ...
  constructor(id: number, arg: any, repr: string) {
    super();
    this.id = id;
    this.arg = arg;
    this.repr = repr;
  ...
  static getOperatorOrValue(operator: any, arg: any) {
    if (!arg.isConstant()) {
...
class AstString extends Leaf {
  ...
  toNumber() {
    return !isNaN(parseFloat(this.str)) ? parseFloat(this.str) : 0;
  }
...
class VarDecl extends Leaf {
  ...
  constructor(id: string, expr: any) {
    super();
    this.id = id;
    this.expr = expr;
  }
...
class FuncDecl extends Leaf {
  ...
  constructor(id: string, params: any[], body: any) {
    super();
    this.id = id;
    this.params = params;
    this.body = body;
...
class IfDecl extends Leaf {
  ...
  constructor(condition: any, thenClause: any, elseIfClause: any, elseClause: any) {
    super();
  ...
  dump() {
    return {
      ...
      then: this.then.dump(),
      elseif: this.elseif ? this.elseif.map((x: any) => x.dump()) : null,
...
class ElseIfDecl extends Leaf {
  ...
  constructor(condition: any, thenClause: any) {
    super();
    this.condition = condition;
    this.then = thenClause;
  }
...
class WhileDecl extends Leaf {
  ...
  constructor(condition: any, whileClause: any) {
    super();
    this.condition = condition;
...
class ForDecl extends Leaf {
  ...
  constructor(assignment: any, upto: boolean, end: any, step: any, body: any) {
    super();
...
class Parser {
  ...
  parseAssigmentOrExpr(tok: any): [any, any] {
    const savedTok = tok;
  ...
  parseSimpleExpr(tok: any = null): [any, any] {
