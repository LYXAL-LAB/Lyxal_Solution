...
const converters: Record<string, (node: any, style: any) => void> = {
...
function setMinMaxDimensions(node: any, style: any) {
  const parent = node[$getSubformParent]();
...
function layoutText(text: any, xfaFont: any, margin: any, lineHeight: any, fontFinder: any, width: any) {
...
function layoutNode(node: any, availableSpace: any) {
  let height = null;
  let width = null;
  let isBroken = false;
...
function computeBbox(node: any, html: any, availableSpace: any) {
  let bbox;
  if (node.w !== "" && node.h !== "") {
...
function fixDimensions(node: any) {
  const parent = node[$getSubformParent]();
  if (parent.layout?.includes("row")) {
    const extra = parent[$extra];
    const colSpan = node.colSpan;
...
function toStyle(node: any, ...names: string[]) {
...
function createWrapper(node: any, html: any) {
...
function fixTextIndent(styles: any) {
...
function setAccess(node: any, classNames: string[]) {
  switch (node.access) {
    case "nonInteractive":
...
function getCurrentPara(node: any) {
  const stack = node[$getTemplateRoot]()[$extra].paraStack;
  return stack.length ? stack.at(-1) : null;
}

function setPara(node: any, nodeStyle: any, value: any) {
...
function setFontFamily(xfaFont: any, node: any, fontFinder: any, style: any) {
  if (!fontFinder) {
