...
// @ts-ignore
import { calculateMD5 } from "./calculate_md5";
...
export async function writeObject(
  ...
  obj: any,
  buffer: any[],
  { encrypt = null, encryptRef = null }: any
) {
...
export async function writeDict(dict: any, buffer: any[], transform: any) {
  buffer.push("<<");
...
async function writeStream(stream: any, buffer: any[], transform: any) {
...
async function writeArray(array: any[], buffer: any[], transform: any) {
  buffer.push("[");
...
export async function writeValue(value: any, buffer: any[], transform: any) {
...
function computeMD5(filesize: any, xrefInfo: any) {
  ...
  // @ts-ignore
  const md5BufferLen = Math.sumPrecise(md5Buffer.map((str: any) => str.length));
...
function writeXFADataForAcroform(str: string, changes: any) {
  ...
  for (const { xfa } of changes) {
    ...
    if (node) {
      node.childNodes = Array.isArray(value)
        ? value.map((val: any) => new SimpleDOMNode("value", val))
...
function updateXFA({ xfaData, xfaDatasetsRef, changes, xref }: any) {
  if (xfaData === null) {
...
async function getXRefTable(xrefInfo: any, baseOffset: any, newRefs: any, newXref: any, buffer: any) {
  buffer.push("xref\n");
...
function getIndexes(newRefs: any) {
  const indexes: any[] = [];
...
function computeIDs(baseOffset: any, xrefInfo: any, newXref: any) {
...
function getTrailerDict(xrefInfo: any, changes: any, useXrefStream: any) {
...
export async function writeChanges(changes: any, xref: any, buffer: any[] = []) {
  const newRefs = [];
