...
class XMLParserBase {
  ...
  onBeginElement(name: string, attributes: any[], isEmpty: boolean) {}
...
class SimpleDOMNode {
  ...
  childNodes: SimpleDOMNode[] | null = null;
  attributes: any[] | null = null;

  constructor(nodeName: string, nodeValue?: any) {
  ...
  hasChildNodes() {
    return (this.childNodes?.length ?? 0) > 0;
  }
   ...
   */
  searchNode(paths: any[], pos: number): SimpleDOMNode | null {
    if (pos >= paths.length) {
      return this;
    }
    ...
    const stack: any[] = [];
    let node: SimpleDOMNode | null = this;

    while (true) {
      if (node && component.name === node.nodeName) {
        if (component.pos === 0) {
      ...
      if (node?.childNodes && node.childNodes.length > 0) {
        ...
        while (stack.length !== 0) {
          ...
          if (parent.childNodes && newPos < parent.childNodes.length) {
  ...
  dump(buffer: string[]) {
    ...
    if (this.hasChildNodes() && this.childNodes) {
      buffer.push(">");
...
class SimpleXMLParser extends XMLParserBase {
  ...
  parseFromString(data: string) {
    ...
    if (!this._currentFragment || this._currentFragment.length === 0) {
        return undefined;
    }
