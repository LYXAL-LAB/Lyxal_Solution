...
export class MetadataParser {
      ...
      .replaceAll(/>\\376\\377([^<]+)/g, function (all, codes) {
        const bytes = codes
          .replaceAll(/\\([0-3])([0-7])([0-7])/g, function (code, d1, d2, d3) {
            return String.fromCharCode(parseInt(d1) * 64 + parseInt(d2) * 8 + parseInt(d3) * 1);
          })
  ...
  _getSequence(entry: any) {
    const name = entry.nodeName;
    if (name !== "rdf:bag" && name !== "rdf:seq" && name !== "rdf:alt") {
      return null;
    }
    return entry.childNodes.filter((node: any) => node.nodeName === "rdf:li");
  }

  _parseArray(entry: any) {
    ...
    this._metadataMap.set(
      entry.nodeName,
      sequence.map((node: any) => node.textContent.trim())
    );
  }

  _parse(xmlDocument: any) {
