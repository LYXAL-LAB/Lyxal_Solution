...
class DefaultAppearanceEvaluator extends EvaluatorPreprocessor {
  constructor(str: string) {
    super(new StringStream(str), null); // Missing xref?
  }
...
class AppearanceStreamEvaluator extends EvaluatorPreprocessor {
  ...
  constructor(stream: any, evaluatorOptions: any, xref: any, globalColorSpaceCache: any) {
    super(stream, xref);
    this.stream = stream;
...
export function getPdfColor(color: any, isFill: boolean) {
  if (color[0] === color[1] && color[1] === color[2]) {
    const gray = color[0] / 255;
    return `${numberToString(gray)} ${isFill ? "g" : "G"}`;
  }
  return (
    Array.from(color, (c: number) => numberToString(c / 255)).join(" ") +
    ` ${isFill ? "rg" : "RG"}`
  );
}

// Create default appearance string from some information.
export function createDefaultAppearance({ fontSize, fontName, fontColor }: any) {
...
export class FakeUnicodeFont {
  static _fontNameId: number;
  static _fontDescriptorRef: any;
  ...
  constructor(xref: any, fontFamily: string) {
    this.xref = xref;
    this.widths = null;
  ...
  static getFirstPositionInfo(rect: number[], rotation: number, fontSize: number) {
  ...
  createAppearance(text: string, rect: number[], rotation: number, fontSize: number, bgColor: any, strokeAlpha: number) {
    const ctx = this._createContext();
    const lines = [];
