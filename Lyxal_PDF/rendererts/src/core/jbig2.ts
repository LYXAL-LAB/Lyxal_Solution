...
class ContextCache {
  private contexts: { [id: string]: Int8Array } = {};

  getContexts(id: string): Int8Array {
    if (this.contexts[id]) {
      return this.contexts[id];
    }
    return (this.contexts[id] = new Int8Array(1 << 16));
  }
}
...
class DecodingContext {
  ...
  public start: number;
  public end: number;
  private _decoder: ArithmeticDecoder | null = null;
  private _contextCache: ContextCache | null = null;
  ...
  get decoder(): ArithmeticDecoder {
    if (!this._decoder) {
      this._decoder = new ArithmeticDecoder(this.data, this.start, this.end);
    }
...
function decodeInteger(
  ...
  if (signedValue !== undefined && signedValue >= MIN_INT_32 && signedValue <= MAX_INT_32) {
    return signedValue;
  }
  return null;
}
...
// 7.3 Segment types
const SegmentTypes: (string | null)[] = [
  "SymbolDictionary",
  null,
  null,
  null,
...
interface RefinementTemplate {
  coding: Point[];
  reference: Point[];
}

const RefinementTemplates: RefinementTemplate[] = [
  {
    coding: [
      { x: 0, y: -1 },
      { x: 1, y: -1 },
...
function decodeBitmapTemplate0(
  ...
  const bitmap: Bitmap = [];
  let contextLabel: number, i: number, j: number, pixel: number, row: Uint8Array, row1: Uint8Array, row2: Uint8Array;
...
function decodeSymbolDictionary(
  ...
  let tableB1, symbolWidths: number[] = [];
  if (huffman) {
...
function decodeTextRegion(
  ...
  while (i < numberOfSymbolInstances) {
    ...
    do {
      ...
      let s2: number, t2: number, symbolRow: Uint8Array;
      if (transposed) {
...
function decodeHalftoneRegion(
  ...
  // Prepare bitmap.
  const regionBitmap: Bitmap = [];
  let i: number, j: number, row: Uint8Array;
  ...
  const grayScaleBitPlanes: Bitmap[] = [];
  let mmrInput: Reader, bitmap: Bitmap;
  if (mmr) {
  ...
  let mg: number, ng: number, bit: number, patternIndex: number, patternBitmap: Bitmap, x: number, y: number, patternRow: Uint8Array, regionRow: Uint8Array;
...
function readSegmentHeader(data: Uint8Array, start: number) {
  const segmentHeader: any = {};
...
function readSegments(header: any, data: Uint8Array, start: number, end: number) {
  const segments = [];
  let position = start;
  while (position < end) {
...
function processSegment(segment: any, visitor: SimpleSegmentVisitor) {
  const header = segment.header;

  const data = segment.data,
    end = segment.end;
  let position = segment.start;
  let args: any[], at: Point[], i: number, atLength: number;
  switch (header.type) {
...
function processSegments(segments: any[], visitor: SimpleSegmentVisitor) {
  for (let i = 0, ii = segments.length; i < ii; i++) {
...
class SimpleSegmentVisitor {
  currentPageInfo: any = null;
  buffer: any = null;
  symbols: any = null;
  patterns: any = null;
  customTables: any = null;

  onPageInformation(info: any) {
  ...
  drawBitmap(regionInfo: any, bitmap: Bitmap) {
    ...
    let i: number, j: number, mask: number, offset: number;
    switch (combinationOperator) {
      case 0: // OR
        for (i = 0; i < height; i++) {
          mask = mask0;
  ...
  onImmediateGenericRegion(region: any, data: Uint8Array, start: number, end: number) {
  ...
  onImmediateTextRegion(region: any, referredSegments: number[], data: Uint8Array, start: number, end: number) {
    const regionInfo = region.info;
    let huffmanTables, huffmanInput;
  ...
  onPatternDictionary(dictionary: any, currentSegment: number, data: Uint8Array, start: number, end: number) {
  ...
  onImmediateHalftoneRegion(region: any, referredSegments: number[], data: Uint8Array, start: number, end: number) {
...
function getCustomHuffmanTable(index: number, referredTo: number[], customTables: any) {
...
export class Jbig2Image {
  ...
  parseChunks(chunks: { data: Uint8Array; start: number; end: number }[]): Uint8ClampedArray {
    return parseJbig2Chunks(chunks);
  }

  parse(data: Uint8Array): { imgData: Uint8ClampedArray; width: number; height: number } {
    const result = parseJbig2(data);
