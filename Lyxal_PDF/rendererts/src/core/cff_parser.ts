...
import { readInt16 } from "./core_utils";
...
interface CharstringValidation {
  id: string;
  min: number;
...
const CharstringValidationData: (CharstringValidation | null)[] = [
  /*  0 */ null,
...
const CharstringValidationData12: (CharstringValidation | null)[] = [
  null,
  null,
  null,
...
export class CFFParser {
  ...
  constructor(file: { getBytes: () => Uint8Array }, properties: any, seacAnalysisEnabled: boolean) {
  ...
  parse(): CFF {
    ...
    let charset: CFFCharset, encoding: CFFEncoding | null;
    if (cff.isCIDFont) {
  ...
  parseHeader(): { obj: CFFHeader; endPos: number } {
  ...
  parseIndex(pos: number): { obj: CFFIndex; endPos: number } {
  ...
  createDict(Type: any, dict: any[], strings: CFFStrings): any {
  ...
  parseCharString(state: any, data: Uint8Array, localSubrIndex: CFFIndex, globalSubrIndex: CFFIndex): boolean {
  ...
  parseCharStrings({
    ...
    fdArray: any[];
    privateDict: any;
  }): { charStrings: CFFIndex; seacs: any[]; widths: any[] } {
    const seacs: any[] = [];
    const widths: any[] = [];
  ...
  parseEncoding(pos: number, properties: any, strings: CFFStrings, charset: any[]): CFFEncoding {
    const encoding: Record<number, number> = Object.create(null);
    const bytes = this.bytes;
...
export class CFF {
  ...
  duplicateFirstGlyph(): void {
    ...
    if (this.isCIDFont && this.fdSelect) {
...
export class CFFIndex {
  ...
  set(index: number, data: Uint8Array): void {
...
class CFFDict {
  ...
  opcodes: Record<number, number[]>;
  order: number[];
  strings: CFFStrings;
  values: Record<number, any>;

  constructor(tables: any, strings: CFFStrings) {
  ...
  setByKey(key: number, value: any[]): boolean {
...
export class CFFTopDict extends CFFDict {
  ...
  privateDict: CFFPrivateDict | null = null;
...
export const CFFCharsetPredefinedTypes = {
  ISO_ADOBE: 0,
  EXPERT: 1,
  EXPERT_SUBSET: 2,
};
...
export class CFFCharset {
  ...
  constructor(predefined: boolean, format: number, charset: any[], raw: Uint8Array | null) {
...
export class CFFEncoding {
  ...
  constructor(predefined: boolean, format: number, encoding: Record<number, number>, raw: Uint8Array | null) {
    this.predefined = predefined;
    this.format = format;
...
class CFFOffsetTracker {
  ...
  setEntryLocation(key: string, values: number[], output: { data: any[] }): void {
    if (!(key in this.offsets)) {
...
export class CFFCompiler {
  ...
  compile(): any[] {
    const cff = this.cff;
    const output: { data: any[]; length: number; add: (data: any[]) => void } = {
      data: [],
      length: 0,
      add(data: any[]) {
        try {
  ...
  compileNameIndex(names: string[]): any[] {
    ...
    for (const name of names) {
      ...
      const sanitizedNameStr = sanitizedName.join("");

      if (sanitizedNameStr === "") {
        // sanitizedName = "Bad_Font_Name"; // Error in JS logic? sanitizedName is array here.
        // Assuming original logic meant string assignment.
        nameIndex.add(stringToBytes("Bad_Font_Name"));
      } else {
        nameIndex.add(stringToBytes(sanitizedNameStr));
      }
    }
    return this.compileIndex(nameIndex);
  }

  compileTopDicts(dicts: CFFTopDict[], length: number, removeCidKeys: boolean): { trackers: CFFOffsetTracker[]; output: any[] } {
  ...
  compilePrivateDicts(dicts: CFFTopDict[], trackers: CFFOffsetTracker[], output: { data: any[]; length: number; add: (data: any[]) => void }): void {
  ...
  compileDict(dict: CFFDict, offsetTracker: CFFOffsetTracker): any[] {
    const out: number[] = [];
  ...
  compileStringIndex(strings: string[]): any[] {
  ...
  compileCharStrings(charStrings: CFFIndex): any[] {
  ...
  compileCharset(charset: CFFCharset, numGlyphs: number, strings: CFFStrings, isCIDFont: boolean): any[] {
  ...
  compileEncoding(encoding: CFFEncoding): any[] {
    return this.compileTypedArray(encoding.raw!);
  }

  compileFDSelect(fdSelect: CFFFDSelect): any[] {
    ...
    switch (format) {
      ...
      case 3:
        ...
        break;
      default:
        // Case 3 is the only other handled case in original code, but we need to initialize out to satisfy TS
        out = new Uint8Array(0); // Should not happen given existing logic
        break;
    }
    return this.compileTypedArray(out);
  }

  compileTypedArray(data: Uint8Array): any[] {
    return Array.from(data);
  }

  compileIndex(index: CFFIndex, trackers: CFFOffsetTracker[] = []): any[] {
