...
function getFloat214(data: Uint8Array, offset: number): number {
  return readInt16(data, offset) / 16384;
}

function getSubroutineBias(subrs: any[]): number {
...
function parseCmap(data: Uint8Array, start: number, end: number): CmapRange[] {
  ...
  if (format === 4) {
    ...
    ranges = [];
    for (i = 0; i < groups; i++) {
      const startRange = readUint32(data, p);
      ranges.push({
        start: startRange,
        end: readUint32(data, p + 4),
        idDelta: readUint32(data, p + 8) - startRange,
      });
      p += 12;
    }
    return ranges;
...
function parseCff(data: Uint8Array, start: number, end: number, seacAnalysisEnabled: boolean) {
  ...
  const parser = new CFFParser(
    new Stream(data, start, end - start, null),
    properties,
    seacAnalysisEnabled
  );
...
function parseGlyfTable(glyf: Uint8Array, loca: Uint8Array, isGlyphLocationsLong: number) {
  ...
  if (isGlyphLocationsLong) {
    ...
    itemSize = 2;
    itemDecode = (data: Uint8Array, offset: number) => 2 * readUint16(data, offset);
  }
...
function lookupCmap(ranges: CmapRange[], unicode: string) {
...
function compileGlyf(code: Uint8Array, cmds: Commands, font: any) {
...
function compileCharString(charStringCode: Uint8Array, cmds: Commands, font: any, glyphId: number) {
  function moveTo(x: number, y: number) {
    if (firstPoint) {
      // Close the current subpath in adding a straight line to the first point.
      cmds.add(DrawOPS.lineTo, firstPoint);
  ...
  function parse(code: Uint8Array) {
    let i = 0;
    while (i < code.length) {
...
const NOOP: any[] | Float32Array | Float16Array = new Float32Array(0);
...
class Commands {
  ...
  add(cmd: number, args: number[] | null = null) {
    if (args) {
  ...
  getPath(): Float32Array | Float16Array {
...
class CompiledFont {
  fontMatrix: number[];
  compiledGlyphs: Record<number, any>;
  compiledCharCodeToGlyphId: Record<number, number>;
  cmap: any;
  glyphs: any;
  isCFFCIDFont: boolean = false;
  fdSelect: any;
  fdArray: any;

  constructor(fontMatrix: number[]) {
    if (
  ...
  compileGlyph(code: any, glyphId: number) {
  ...
  compileGlyphImpl(code: any, cmds: Commands, glyphId?: number) {
...
class TrueTypeCompiled extends CompiledFont {
  ...
  compileGlyphImpl(code: any, cmds: Commands) {
...
export class FontRendererFactory {
  static create(font: { data: any; fontMatrix: any }, seacAnalysisEnabled: boolean) {
