...
import { lookupNormalRect, stringToAsciiOrUTF16BE } from "./core_utils.ts";
...
class StructTreeRoot {
  ...
  structParentIds: RefSetCache | null = null;
  kidRefToPosition: Map<string, number> | null | undefined = undefined;
  parentTree: NumberTree | null = null;

  constructor(xref: XRef, rootDict: Dict, rootRef: any) {
    this.xref = xref;
    this.dict = rootDict;
  ...
  async canUpdateStructTree({ pdfManager, newAnnotationsByPage }: any) {
    if (!this.ref) {
  ...
  async updateStructureTree({ newAnnotationsByPage, pdfManager, changes }: any) {
  ...
  static async #writeKids({
    ...
    let nextKey = -1;
    let structTreePageObjs: Map<number, Map<number, Ref>>;
  ...
  static #writeProperties(
    tagDict: Dict,
    { type, title, lang, alt, expanded, actualText }: any
  ) {
  ...
  static #collectParents({ elements, xref, pageDict, numberTree }: any) {
    ...
    const updateElement = (kid: any, pageKid: any, kidRef: any) => {
...
class StructElementNode {
  ...
  parseKid(pageObjId: string | null, kid: any) {
...
class StructElement {
  ...
  dict: Dict | null;
  mcid: number | null;
  pageObjId: string | null;
  refObjId: string | null;
  parentNode: StructElementNode | null;
...
class StructTreePage {
  root: StructTreeRoot | null;
  xref: XRef | null;
  rootDict: Dict | null;
  pageDict: Dict;
  nodes: (StructElementNode | null)[];

  constructor(structTreeRoot: StructTreeRoot | null, pageDict: Dict) {
  ...
  addNode(dict: any, map: Map<any, any>, level = 0): StructElementNode | null {
    if (level > MAX_DEPTH) {
  ...
  get serializable(): any {
    function nodeToSerializable(node: StructElementNode, parent: any, level = 0) {
      if (level > MAX_DEPTH) {
...
export { StructTreePage, StructTreeRoot, StructElement, StructElementNode };