...
import { Dict, isDict, Ref, RefSetCache } from "./primitives.ts";
...
class WorkerMessageHandler {
  ...
  static setup(handler: any, port: any) {
    let testMessageProcessed = false;
    handler.on("test", (data: any) => {
    ...
    handler.on("configure", (data: any) => {
      setVerbosityLevel(data.verbosity);
    });

    handler.on("GetDocRequest", (data: any) => this.createDocumentHandler(data, port));
  }

  static createDocumentHandler(docParams: any, port: any) {
    // This context is actually holds references on pdfManager and handler,
    // until the latter is destroyed.
    let pdfManager: any;
    let terminated = false;
    let cancelXHRs: any = null;
    ...
    if (typeof PDFJSDev === "undefined" || PDFJSDev.test("GENERIC")) {
      ...
      const buildMsg = (type: any, prop: any) =>
    ...
    let handler: any = new MessageHandler(workerHandlerName, docId, port);
    ...
    async function getPdfManager({
      ...
      new Promise(function (resolve, reject) {
        const readChunk = function ({ value, done }: any) {
          try {
            ensureNotTerminated();
            if (done) {
              if (!newPdfManager) {
    ...
    function setupDoc(data: any) {
      function onSuccess(doc: any) {
        ensureNotTerminated();
      ...
      function pdfManagerReady() {
        ...
        loadDocument(false).then(onSuccess, function (reason: any) {
          ensureNotTerminated();
      ...
      getPdfManager(data)
        .then(function (newPdfManager: any) {
          if (terminated) {
          ...
          pdfManager.requestLoadedStream(/* noFetch = */ true).then((stream: any) => {
    ...
    handler.on("GetPage", function (data: any) {
      return pdfManager.getPage(data.pageIndex).then(function (page: any) {
        return Promise.all([
    ...
    handler.on("GetPageIndex", function (data: any) {
      const pageRef = Ref.get(data.num, data.gen);
      return pdfManager.ensureCatalog("getPageIndex", [pageRef]);
    });

    handler.on("GetDestinations", function (data: any) {
      return pdfManager.ensureCatalog("destinations");
    });

    handler.on("GetDestination", function (data: any) {
      return pdfManager.ensureCatalog("getDestination", [data.id]);
    });

    handler.on("GetPageLabels", function (data: any) {
      return pdfManager.ensureCatalog("pageLabels");
    });

    handler.on("GetPageLayout", function (data: any) {
      return pdfManager.ensureCatalog("pageLayout");
    });

    handler.on("GetPageMode", function (data: any) {
      return pdfManager.ensureCatalog("pageMode");
    });

    handler.on("GetViewerPreferences", function (data: any) {
      return pdfManager.ensureCatalog("viewerPreferences");
    });

    handler.on("GetOpenAction", function (data: any) {
      return pdfManager.ensureCatalog("openAction");
    });

    handler.on("GetAttachments", function (data: any) {
      return pdfManager.ensureCatalog("attachments");
    });

    handler.on("GetDocJSActions", function (data: any) {
      return pdfManager.ensureCatalog("jsActions");
    });

    handler.on("GetPageJSActions", function ({ pageIndex }: any) {
      return pdfManager
        .getPage(pageIndex)
        .then((page: any) => pdfManager.ensure(page, "jsActions"));
    });

    handler.on(
      "GetAnnotationsByType",
      async function ({ types, pageIndexesToSkip }: any) {
        ...
        try {
          for (let i = 0, ii = numPages; i < ii; i++) {
            ...
            pagePromises.push(
              pdfManager.getPage(i).then(async (page: any) => {
                if (!page) {
                  return [];
                }
                return (
    ...
    handler.on("GetOutline", function (data: any) {
      return pdfManager.ensureCatalog("documentOutline");
    });

    handler.on("GetOptionalContentConfig", function (data: any) {
      return pdfManager.ensureCatalog("optionalContentConfig");
    });

    handler.on("GetPermissions", function (data: any) {
      return pdfManager.ensureCatalog("permissions");
    });

    handler.on("GetMetadata", function (data: any) {
      return Promise.all([
    ...
    handler.on("GetMarkInfo", function (data: any) {
      return pdfManager.ensureCatalog("markInfo");
    });

    handler.on("GetData", function (data: any) {
      return pdfManager.requestLoadedStream().then((stream: any) => stream.bytes);
    });

    handler.on("GetAnnotations", function ({ pageIndex, intent }: any) {
      return pdfManager.getPage(pageIndex).then(function (page: any) {
    ...
    handler.on("GetFieldObjects", function (data: any) {
      return pdfManager
        .ensureDoc("fieldObjects")
        .then((fieldObjects: any) => fieldObjects?.allFields || null);
    });

    handler.on("HasJSActions", function (data: any) {
      return pdfManager.ensureDoc("hasJSActions");
    });

    handler.on("GetCalculationOrderIds", function (data: any) {
      return pdfManager.ensureDoc("calculationOrderIds");
    });

    handler.on("ExtractPages", async function ({ pageInfos }: any) {
      if (!pageInfos) {
    ...
    handler.on(
      "SaveDocument",
      async function ({ isPureXfa, numPages, annotationStorage, filename }: any) {
        ...
        if (newAnnotationsByPage) {
          ...
          for (const [pageIndex, annotations] of newAnnotationsByPage) {
            newAnnotationPromises.push(
              pdfManager.getPage(pageIndex).then((page: any) => {
        ...
        if (isPureXfa) {
          ...
          for (let pageIndex = 0; pageIndex < numPages; pageIndex++) {
            promises.push(
              pdfManager.getPage(pageIndex).then(function (page: any) {
        ...
        const needAppearances =
          ...
          changes.values().some((ref: any) => ref.needAppearances);
        ...
        let newXrefInfo: any = Object.create(null);
        if (xref.trailer) {
    ...
    handler.on("GetOperatorList", function (data: any, sink: any) {
    ...
    handler.on("GetTextContent", function (data: any, sink: any) {
    ...
    handler.on("GetStructTree", function (data: any) {
      return pdfManager
        .getPage(data.pageIndex)
        .then((page: any) => pdfManager.ensure(page, "getStructTree"));
    });

    handler.on("FontFallback", function (data: any) {
      return pdfManager.fontFallback(data.id, handler);
    });

    handler.on("Cleanup", function (data: any) {
      return pdfManager.cleanup(/* manuallyTriggered = */ true);
    });

    handler.on("Terminate", function (data: any) {
      terminated = true;
    ...
    handler.on("Ready", function (data: any) {
      setupDoc(docParams);
      docParams = null; // we don't need docParams anymore -- saving memory.
    });

    if (typeof PDFJSDev === "undefined" || PDFJSDev.test("TESTING")) {
      handler.on("GetXFADatasets", function (data: any) {
        return pdfManager.ensureDoc("xfaDatasets");
      });
      handler.on("GetStartXRefPos", function (data: any) {
        return pdfManager.ensureDoc("startXRef");
      });
      handler.on("GetAnnotArray", function (data: any) {
        return pdfManager
          .getPage(data.pageIndex)
          .then((page: any) => page.annotations.map((a: any) => a.toString()));
      });
    }
  ...
  static initializeFromPort(port: any) {
