...
import { AnnotationType, LinkAnnotation } from './src/core/annotation';
...
%%EOF`;
    ...
    try {
        ...
        doc.getPage(0).then(async page => {
            ...
            console.log(`Found ${annots.length} annotations`);
            
            if (annots.length > 0) {
                const a = annots[0];
                console.log("Annot Rect:", a.rect);
                if (a instanceof LinkAnnotation) {
                    console.log("Link URI:", a.url);
                } else {
                    console.log("Not a LinkAnnotation?", a.constructor.name);
                }
            }
