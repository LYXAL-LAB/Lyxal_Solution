...
export class I18nService {
  static resolveKey(keyOrId: unknown, _locale: string): string | undefined {
    if (keyOrId === null || keyOrId === undefined) return undefined;
    ...
    try {
      ...
      if (typeof keyOrId === 'string') {
        ...
        const obj = keyOrId as any;
        if (typeof obj.id === 'string') raw = obj.id; // ex: { id: 'base_i18n_key:menu.dashboard.name' }
        else if (typeof obj.$id === 'string') raw = obj.$id;
        else raw = String(obj);
      } else {
        raw = String(keyOrId);
      }
