...
export interface SubscriptionGuardCondition {
  plan?: string;        // Plan d'abonnement requis
  feature?: string;     // Fonctionnalité spécifique requise
  minLevel?: number;    // Niveau minimum d'abonnement
}

/**
 * Niveaux d'abonnement (du plus bas au plus élevé)
 */
const SUBSCRIPTION_LEVELS: Record<string, number> = {
  'free': 0,
  'basic': 1,
  'pro': 2,
  'premium': 3,
 ...
 */
export async function executeSubscriptionGuard(
  condition: SubscriptionGuardCondition,
