 ...
 */
export async function executeFeatureGuard(condition, context) {
    ...
    try {
        ...
        const hasRoleAccess = featureRoles.some(role => context.user?.roles?.includes(role));
        if (hasRoleAccess) {
 ...
 */
function getRolesForFeature(feature) {
    const featureRoles = {
 ...
 */
function getFeatureVersion(subscription, feature) {
 ...
 */
function isVersionCompatible(userVersion, requiredVersion) {
 ...
 */
export function hasFeatureAccess(user, feature, version) {
    // Super admin a tout
    if (user?.roles?.includes('super_admin'))
        return true;
    // Permissions explicites
    if (user?.permissions?.includes(`feature:${feature}`))
        return true;
    // Abonnement
