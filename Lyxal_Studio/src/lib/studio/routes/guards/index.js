 ...
 */
export async function executeGuard(guard, context) {
    ...
    try {
        switch (type) {
            ...
            case 'role':
                return await executeRoleGuard(condition || {}, context);
            case 'subscription':
                return await executeSubscriptionGuard(condition || {}, context);
            case 'feature':
                return await executeFeatureGuard(condition || {}, context);
            default:
 ...
 */
export function validateGuardConfig(guard) {
    const errors = [];
    // Vérifier le type
    if (!guard.type) {
 ...
 */
export function getAvailableGuardTypes() {
    return ['auth', 'role', 'subscription', 'feature'];
}
/**
 * Récupère la description d'un type de guard
 */
export function getGuardTypeDescription(type) {
    const descriptions = {
 ...
 */
export function createGuard(type, condition = {}) {
    const guard = { type, condition };
