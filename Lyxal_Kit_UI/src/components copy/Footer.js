 ...
 * @returns JSX.Element
 */
const Footer = memo(({ companyName, currentTheme = "light", onCopyrightClick }) => {
    ...
    const copyrightText = useMemo(() => `© ${platformName} ${anneeConstruction}`, [platformName, anneeConstruction]);
    ...
    const footerClasses = useMemo(() => "bg-base-200/80 backdrop-blur-sm border-t border-base-300 !px-4 sm:!px-6 lg:!px-8 !py-2 transition-colors duration-200", []);
    const contentClasses = useMemo(() => "flex items-center justify-center text-sm text-base-content/70 transition-colors duration-200", []);
    return (_jsx("footer", { id: "app-footer", className: footerClasses, role: "contentinfo", "aria-label": ariaLabels.footer, children: _jsx("div", { id: "footer-container", className: "w-full flex items-center justify-center", children: _jsx("div", { id: "footer-content", className: contentClasses, children: onCopyrightClick ? (_jsx("button", { id: "footer-copyright-btn", className: "text-sm text-base-content/70 hover:text-base-content transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-sm", onClick: handleCopyrightClick, "aria-label": ariaLabels.copyrightButton, role: "button", children: copyrightText })) : (_jsx("span", { id: "footer-copyright", "aria-label": ariaLabels.copyright, role: "text", children: copyrightText })) }) }) }));
});
