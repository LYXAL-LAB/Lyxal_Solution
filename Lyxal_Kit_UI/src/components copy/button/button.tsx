...
import { 
  Loader, Check, X, ArrowRight, Download, Upload, Trash2, Edit, 
  Plus, Minus, Search, Heart, Star, Share2, Send, Save, 
  Play, Pause, Volume2, Settings, User, Bell, ShoppingCart,
  Eye, EyeOff, Lock, Unlock, Mail, Phone, Home, Menu,
  ChevronRight, ChevronLeft, ChevronUp, ChevronDown, Copy,
  ExternalLink, Zap, Award, Gift, TrendingUp, AlertCircle
} from 'lucide-react';
...
export default function ButtonLibrary() {
  ...
  const bg = darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-100';
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className={`${card} rounded-xl p-8 shadow-lg`}>
          <div className="flex justify-between items-center">
            <div>
              <h1 className={`text-4xl font-bold ${txt} mb-2`}>Bibliothèque de Boutons</h1>
              <p className={txt2}>Collection complète de styles de boutons React avec Tailwind CSS</p>
            </div>
            <button
        ...
        <ButtonSection title="⚡ Boutons avec États">
          <ButtonDemo label="Loading">
            ...
            >
              {loading.btn1 ? <Loader className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
          ...
          <ButtonDemo label="Success State">
            ...
            >
              {loading.btn2 ? <Check className="w-5 h-5" /> : <Save className="w-5 h-5" />}
          ...
          <ButtonDemo label="Toggle Like">
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${
                liked.btn1 ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'
              }`}
            >
              <Heart className={`w-5 h-5 ${liked.btn1 ? 'fill-current' : ''}`} />
