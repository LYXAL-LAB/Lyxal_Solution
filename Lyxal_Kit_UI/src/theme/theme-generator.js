...
export class ThemeGenerator {
     ...
     */
    static generateDarkVariant(lightTheme) {
        ...
        Object.keys(lightTheme).forEach(key => {
            ...
            if (key.includes('base-100')) {
                ...
                const rgb = lightTheme[typedKey].split(' ').map(Number);
     ...
     */
    static generateAnalogousPalette(baseColor, count = 5) {
     ...
     * @returns Tableau de couleurs hex
     */
    static generateMonochromaticPalette(baseColor, count = 9) {
        const rgb = this.hexToRgb(baseColor);
        const hsl = this.rgbToHsl(...rgb);
        const palette = [];
        for (let i = 0; i < count; i++) {
     ...
     */
    static adjustBrightness(rgb, percent) {
     ...
     */
    static generateNeutral(rgb, lightness) {
        const hsl = this.rgbToHsl(...rgb);
     ...
     */
    static exportTheme(theme, format = 'css') {
        switch (format) {
            case 'css':
