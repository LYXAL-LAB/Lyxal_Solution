...
export class ThemeManager {
  ...
  setVariable(variableName: string, value: string): void {
    ...
    if (this.customThemes[this.currentTheme]) {
      this.customThemes[this.currentTheme][variableName as keyof ThemeVariables] = finalValue;
      this.saveCustomThemes();
    }
  }
  ...
  private validateColors(colors: CustomThemeColors): boolean {
    const required: (keyof CustomThemeColors)[] = ['primary', 'secondary', 'accent'];
    ...
    const allValid = Object.values(colors).every(color => 
      color === undefined || hexRegex.test(color)
    );
