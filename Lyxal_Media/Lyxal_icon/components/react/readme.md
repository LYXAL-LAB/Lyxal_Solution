...
Lyxal for React is not yet another icon component! There are many of them already.
...
For extended documentation visit [Lyxal for React documentation](https://Lyxal.design/docs/icon-components/react/).
...
```bash
npm install --save-dev @Lyxal/react
```

If you are using Yarn:

```bash
yarn add --dev @Lyxal/react
```

## Usage with API

Install `@Lyxal/react` and import `Icon` from it:

```typescript
import { Icon } from '@Lyxal/react';
```
...
See [Lyxal for React icon names documentation](https://Lyxal.design/docs/icon-components/react/icon-name.html) for more detailed explanation.
...
import { Icon, InlineIcon } from '@Lyxal/react';
...
See [icon transformations documentation](https://Lyxal.design/docs/icon-components/react/transform.html) for more details.
