...
Lyxal for Svelte is not yet another icon component! There are many of them already.
...
For extended documentation visit [Lyxal for Svelte documentation](https://Lyxal.design/docs/icon-components/svelte/).
...
```bash
npm install --save-dev @Lyxal/svelte
```

If you are using Yarn:

```bash
yarn add --dev @Lyxal/svelte
```

## Usage with API

Install `@Lyxal/svelte` and import `Icon` as default export from it:

```js
import Icon from '@Lyxal/svelte';
```
...
See [Lyxal for Svelte icon names documentation](https://Lyxal.design/docs/icon-components/svelte/icon-name.html) for more detailed explanation.
...
<script>
import Icon from '@Lyxal/svelte';
import alertIcon from '@Lyxal-icons/mdi/alert';
</script>
...
See [icon transformations documentation](https://Lyxal.design/docs/icon-components/svelte/transform.html) for more details.
...
<script>
	...
	onMount(async () => {
		const promises = [
			import('@Lyxal/svelte'), // Component
			import('@Lyxal-icons/bi/link-45deg'), // Icon #1
		];
