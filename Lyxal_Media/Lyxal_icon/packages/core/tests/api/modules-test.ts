import type { LyxalAPIConfig } from '../../lib/api/config';
...
describe('Testing API modules', () => {
	...
	const prepareQuery = (
		...
		const item: LyxalAPIIconsQueryParams = {
			type: 'icons',
			provider,
			prefix,
			icons,
	...
	it('Empty module', () => {
		...
		// Get config
		const config = getAPIConfig(provider) as LyxalAPIConfig;
		...
		// Check getAPIModule()
		const item = getAPIModule(provider) as LyxalAPIModule;
