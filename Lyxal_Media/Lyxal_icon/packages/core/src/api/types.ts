...
import { LyxalIcon, LyxalJSON } from '@Lyxal/types';

/**
 * Custom icons loader
 */
export type LyxalCustomIconsLoader = (
	icons: string[],
	prefix: string,
	provider: string
) => Promise<LyxalJSON | null> | LyxalJSON | null;

/**
 * Custom loader for one icon
 */
export type LyxalCustomIconLoader = (
	name: string,
	prefix: string,
	provider: string
) => Promise<LyxalIcon | null> | LyxalIcon | null;
...
export interface IconStorageWithAPI extends IconStorage {
	 ...
	 */
	loadIcons?: LyxalCustomIconsLoader;
	loadIcon?: LyxalCustomIconLoader;
