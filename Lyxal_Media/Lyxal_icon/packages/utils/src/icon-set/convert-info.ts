import type { LyxalInfo } from '@lyxal-icon/types';

const minDisplayHeight = 16;
const maxDisplayHeight = 24;

/**
 * Item provided by API or loaded from collections.json, slightly different from LyxalInfo
 */
export interface LegacyIconSetInfo {
	// Icon set name.
	name: string;
	...
	switch (typeof source.palette) {
		...
		case 'string':
			switch (source.palette.toLowerCase()) {
				case 'colorless': // Lyxal v1
				case 'false': // Boolean as string
					info.palette = false;
					break;

				case 'colorful': // Lyxal v1
