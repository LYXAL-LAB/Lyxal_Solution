...
type Callback = (data: LyxalIcon | null | undefined) => void;

interface Result {
	// Change icon name
	change: (iconName: string | LyxalIconName | LyxalIcon) => void;

	// Unsubscribe function
	unsubscribe: () => void;

	// Data
	data: LyxalIcon | null | undefined;
}
...
export function subscribeToIconData(
	iconToRender: string | LyxalIconName | LyxalIcon,
	callback: Callback
): Result {
	...
	let returned = false;
	let icon: LyxalIconName | null = null;
	let storage: IconStorage | null = null;
	let data: LyxalIcon | null | undefined;
	...
	const change = (iconName: string | LyxalIconName | LyxalIcon) => {
		if (unsubscribed) {
			// Ignore
			return;
		}
