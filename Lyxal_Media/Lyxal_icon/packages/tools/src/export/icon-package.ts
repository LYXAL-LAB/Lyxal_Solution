 ...
 */
const defaultTypesContent = `import type { LyxalIcon } from '@lyxal-icon/types';
declare const data: LyxalIcon;
export default data;
`;
 ...
 *
 * Was used for exporting `@Lyxal-icons/{prefix}` packages
 */
export async function exportIconPackage(
	...
	const packageJSON = {
		name:
			name ||
			(esm
				? `@Lyxal-icons/${iconSet.prefix}`
				: `@lyxal-icon/icons-${iconSet.prefix}`),
		description:
			description ||
			`Lyxal icon components for ${info ? info.name : iconSet.prefix}`,
		version,
