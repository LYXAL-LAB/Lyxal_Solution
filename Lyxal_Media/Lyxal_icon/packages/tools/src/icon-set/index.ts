...
import {
	...
	defaultIconProps,
} from '@lyxal-icon/utils/lib/icon/defaults';
...
import { minifyIconSet } from '@lyxal-icon/utils/lib/icon-set/minify';
import { convertIconSetInfo } from '@lyxal-icon/utils/lib/icon-set/convert-info';
...
import type {
	ParentIconsList,
	ParentIconsTree,
} from '@lyxal-icon/utils/lib/icon-set/tree';
import { mergeIconData, trimSVG } from '@lyxal-icon/utils';
...
export class IconSet {
	...
	// Information block
	public info: LyxalInfo | undefined;
	 ...
	 * Resolve icon
	 */
	resolve(name: string, full: false): ResolvedLyxalIcon | null;
	resolve(name: string): ResolvedLyxalIcon | null;
	resolve(name: string, full: true): Required<ResolvedLyxalIcon> | null;
	resolve(
		name: string,
		full = false
	): Required<ResolvedLyxalIcon> | ResolvedLyxalIcon | null {
		// Get parent icons tree
		const entries = this.entries;
		const item = entries[name];
		const tree =
		...
		let result = {} as ResolvedLyxalIcon;
		...
		function parse(name: string) {
			...
			result = mergeIconData(item.props, result) as ResolvedLyxalIcon;
			if (item.type === 'icon') {
	 ...
	 * Export icon set
	 */
	export(validate = true): LyxalJSON {
		const icons = Object.create(null) as LyxalIcons;
		const aliases = Object.create(null) as LyxalAliases;
		const tree = validate
			? this.getTree()
		...
		names.forEach((name) => {
			...
			switch (item.type) {
				...
				case 'variation': {
					...
					const alias: ExtendedLyxalAlias = {
						parent: item.parent,
						...props,
					};
					aliases[name] = alias;
		...
		// Add info
		let info: LyxalInfo | undefined;
		if (this.info) {
			// Update icons count and clone object
			this.info.total = this.count();
			info = JSON.parse(JSON.stringify(this.info)) as LyxalInfo;
		}
		...
		// Get categories
		const categories = Object.create(null) as LyxalCategories;
	 ...
	 * Add/update icon
	 */
	setIcon(name: string, icon: ResolvedLyxalIcon): boolean {
