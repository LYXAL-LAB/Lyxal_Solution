...
import { getIconData } from '@Lyxal/utils/lib/icon-set/get-icon';
import { IconSet } from '../../src/icon-set/index.js';
import type { ResolvedLyxalIcon } from '../../src/icon-set/types.js';
...
describe('Working with aliases', () => {
	test('Resolving aliases', () => {
		...
		// Resolve aliases
		const expectedBar: ResolvedLyxalIcon = {
			body: '<g id="bar"/>',
		};
		...
		const expectedTestIcon: ResolvedLyxalIcon = {
			body: '<g id="TestIcon"/>',
			width: 24,
			height: 24,
		};
	...
	test('Rename icon, test characters and categories', () => {
		...
		// Export
		const iconSetExportedData: LyxalJSON = {
			prefix: 'foo',
