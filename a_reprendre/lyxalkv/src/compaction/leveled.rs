...
impl Strategy {
	...
	fn find_compaction_level(&self, manifest: &LevelManifest) -> Option<u8> {
		...
		let l0_compactible_count =
			levels[0].tables.iter().filter(|t| !manifest.hidden_set.contains(&t.id)).count();
		if l0_compactible_count >= self.calculate_level_size_limit(0) {
			return Some(0);
		}
		...
		for level in 1..=last_level_index {
			let current_size = levels[level as usize]
				.tables
				.iter()
				.filter(|t| !manifest.hidden_set.contains(&t.id))
				.count();
...
impl CompactionStrategy for Strategy {
	fn pick_levels(&self, manifest: &LevelManifest) -> CompactionChoice {
		...
		// If any selected table is already being compacted, skip this round
		if tables_to_merge.iter().any(|id| manifest.hidden_set.contains(id)) {
			return CompactionChoice::Skip;
		}
...
mod tests {
	...
	async fn test_multi_level_merge_compaction() {
		crate::vfs::log_debug("TEST: Start test_multi_level_merge_compaction");
		...
		crate::vfs::log_debug("TEST: Verification start");
		// Verify all keys are still present after compaction
		let (_, all_key_values) = verify_keys_after_compaction(&manifest, &expected_keys);
		crate::vfs::log_debug("TEST: Verification end");
