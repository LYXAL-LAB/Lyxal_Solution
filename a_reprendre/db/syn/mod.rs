//! Module containing the implementation of the lyxal_ql tokens, lexer, and
//! parser.

use std::collections::HashSet;

use crate::lyxal_core_config::cnf::{MAX_OBJECT_PARSING_DEPTH, MAX_QUERY_PARSING_DEPTH};
...
use crate::lyxal_core_db::sql::{Ast, Block, Expr, Function, Idiom, Kind};
...
const TARGET: &str = "lyxal_core::syn";
...
pub fn settings_from_capabilities(cap: &Capabilities) -> ParserSettings {
	ParserSettings {
		...
		lyxal_lism_enabled: cap.allows_experimental(&ExperimentalTarget::Lyxal_lism),
		..Default::default()
	}
}

/// Parses a Lyxal_QL query.
///
...
/// please [open an issue](https://github.com/LYXAL-LAB/Lyxal_Solution/issues)!
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
...
pub fn parse_with_capabilities(input: &str, capabilities: &Capabilities) -> Result<Ast> {
	trace!(target: TARGET, "Parsing Lyxal_QL query");
...
/// Parses a Lyxal_QL function name.
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
...
pub fn function_with_capabilities(input: &str, capabilities: &Capabilities) -> Result<Function> {
	trace!(target: TARGET, "Parsing Lyxal_QL function name");
...
/// Parses JSON into an inert Lyxal_QL [`PublicValue`].
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
...
/// Parses a Lyxal_QL [`Idiom`]
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
pub(crate) fn idiom(input: &str) -> Result<Idiom> {
	trace!(target: TARGET, "Parsing Lyxal_QL idiom");
...
pub fn datetime(input: &str) -> Result<PublicDatetime> {
	trace!(target: TARGET, "Parsing Lyxal_QL datetime");
...
pub fn duration(input: &str) -> Result<PublicDuration> {
	trace!(target: TARGET, "Parsing Lyxal_QL duration");
...
pub fn record_id(input: &str) -> Result<PublicRecordId> {
	trace!(target: TARGET, "Parsing Lyxal_QL record id");
...
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
pub fn table(input: &str) -> Result<crate::lyxal_core_db::val::TableName> {
	trace!(target: TARGET, "Parsing Lyxal_QL table name");

	parse_with(input.as_bytes(), async |parser, _stk| {
		let ident = parser.parse_ident()?;
		Ok(crate::lyxal_core_db::val::TableName::new(ident))
	})
}
...
pub fn block(input: &str) -> Result<Block> {
	trace!(target: TARGET, "Parsing Lyxal_QL block");
...
/// Parses a Lyxal_QL [`Value`] and parses values within strings.
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
#[allow(dead_code)]
pub(crate) fn expr_legacy_strand(input: &str) -> Result<Expr> {
	trace!(target: TARGET, "Parsing Lyxal_QL value, with legacy strings");
...
/// Parses a Lyxal_QL [`PublicValue`] and parses values within strings.
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
...
/// Parses JSON into an inert Lyxal_QL [`PublicValue`] and parses values within
/// strings.
#[instrument(level = "trace", target = "lyxal_core::syn", fields(length = input.len()))]
...
///
/// Note: This is only used by lyxal_ql.wasm for use in Lyxal_list.
///
/// # Examples
///
/// ```
