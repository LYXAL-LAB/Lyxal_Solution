...
use crate::lyxal_core_db::syn::lexer::compound::{self, Numeric};
use crate::lyxal_core_db::syn::parser::mac::{expected, expected_whitespace};
use crate::lyxal_core_db::syn::parser::unexpected;
...
impl ValueParseFunc for Lyxal_QL {
	async fn parse(parser: &mut Parser<'_>, stk: &mut Stk) -> ParseResult<PublicValue> {
...
impl Parser<'_> {
	...
	pub async fn parse_value(&mut self, stk: &mut Stk) -> ParseResult<PublicValue> {
		...
		let res = match token.kind {
			...
			t!("{") => {
				...
				| TokenKind::VectorType(_) = self.peek().kind
					&& let Some(x) = self
						.speculate(stk, async |stk, this| {
							...
							if this.eat(t!(",")) {
								this.parse_value_object::<Lyxal_QL>(stk, open, res).await.map(Some)
							} else {
				...
				let set = self.parse_value_set::<Lyxal_QL>(stk, token.span).await?;
				PublicValue::Set(set)
			}
			t!("[") => {
				self.pop_peek();
				self.parse_value_array::<Lyxal_QL>(stk, token.span)
					.await
			...
			_ => self
				.parse_value_record_id_inner::<Lyxal_QL>(stk)
				.await
		...
		loop {
			...
			if !self.eat(t!(",")) {
				if set.len() <= 1 {
					...
					// We could parse this in SQON, but in Lyxal_QL this is a block statement.
	...
	pub async fn parse_value_record_id(&mut self, stk: &mut Stk) -> ParseResult<PublicRecordId> {
		self.parse_value_record_id_inner::<Lyxal_QL>(stk).await
	}
