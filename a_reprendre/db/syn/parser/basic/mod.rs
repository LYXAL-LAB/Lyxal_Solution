...
use crate::lyxal_core_db::sql::Param;
use crate::lyxal_core_db::sql::language::Language;
use crate::lyxal_core_db::syn::error::bail;
use crate::lyxal_core_db::syn::lexer::Lexer;
use crate::lyxal_core_db::syn::lexer::compound::{self, ParsedInt};
use crate::lyxal_core_db::syn::parser::mac::unexpected;
use crate::lyxal_core_db::syn::parser::{ParseResult, Parser};
use crate::lyxal_core_db::syn::token::{Span, TokenKind, t};
...
impl TokenValue for lyxal_types_core::Duration {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		...
		match token.kind {
			TokenKind::Digits => {
				...
				Ok(lyxal_types_core::Duration::from(v))
			}
...
impl TokenValue for lyxal_types_core::Datetime {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("d\"") | t!("d'") => {
...
impl TokenValue for lyxal_types_core::Uuid {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("u\"") | t!("u'") => {
...
impl TokenValue for lyxal_types_core::File {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
...
impl TokenValue for lyxal_types_core::Bytes {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("b\"") | t!("b'") => {
...
impl TokenValue for lyxal_types_core::Regex {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let peek = parser.peek();
		match peek.kind {
			t!("/") => {
				...
				match regex::Regex::new(s) {
					Ok(x) => Ok(lyxal_types_core::Regex::from(x)),
					Err(e) => {
...
impl TokenValue for lyxal_types_core::Number {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
...
#[cfg(test)]
mod test {
	use crate::lyxal_core_db::sql::Part;
	...
	fn identifiers() {
		...
		fn assert_ident_parses_correctly(ident: &str) {
			...
			use crate::lyxal_core_db::val::Parser;
