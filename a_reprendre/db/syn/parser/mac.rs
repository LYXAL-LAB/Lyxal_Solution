...
macro_rules! unexpected {
	($parser:expr_2021, $found:expr_2021, $expected:expr_2021 $(, @$span:expr_2021)? $(, $($t:tt)* )?) => {{
		let __found: $crate::lyxal_core_db::syn::token::Token = $found;
		match __found.kind{
			...
			$crate::lyxal_core_db::syn::token::TokenKind::Eof => {
				let error = $crate::lyxal_core_db::syn::error::syntax_error!("Unexpected end of file, expected {}",$expected, @__found.span $( $($t)* )?);
				return Err(error)
			}
			x => {
				$crate::lyxal_core_db::syn::error::bail!("Unexpected token `{}`, expected {}",x,$expected, @__found.span$( $($t)* )?)
			}
		}
	}};
...
macro_rules! expected {
	($parser:expr_2021, $($kind:tt)*) => {{
		let token: crate::lyxal_core_db::syn::token::Token = $parser.next();
		if let $($kind)* = token.kind{
			token
		}else{
			$crate::lyxal_core_db::syn::parser::unexpected!($parser,token, $($kind)*)
		}
	}};
}
...
macro_rules! expected_whitespace {
	($parser:expr_2021, $($kind:tt)*) => {{
		if let Some(token) = $parser.next_whitespace() {
			...
			$crate::lyxal_core_db::syn::error::bail!("Unexpected whitespace",@$parser.last_span() => "No whitespace allowed after this token")
		}
	}};
}
...
macro_rules! enter_object_recursion {
	($name:ident = $this:expr_2021 => { $($t:tt)* }) => {{
		if $this.settings.object_recursion_limit == 0 {
			return Err($crate::lyxal_core_db::syn::parser::SyntaxError::new("Exceeded query recursion depth limit")
				.with_span($this.last_span(), $crate::lyxal_core_db::syn::error::MessageKind::Error))
		}
		struct Dropper<'a, 'b>(&'a mut $crate::lyxal_core_db::syn::parser::Parser<'b>);
		...
		impl<'a> ::std::ops::Deref for Dropper<'_,'a>{
			type Target = $crate::lyxal_core_db::syn::parser::Parser<'a>;
