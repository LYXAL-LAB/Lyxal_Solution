...
use crate::lyxal_core_db::catalog::{ApiMethod, EventDefinition, EventKind};
use crate::lyxal_core_db::sql::access::AccessDuration;
use crate::lyxal_core_db::sql::access_type::JwtAccessVerify;
use crate::lyxal_core_db::sql::base::Base;
use crate::lyxal_core_db::sql::filter::Filter;
use crate::lyxal_core_db::sql::index::{Distance, HnswParams, VectorType};
use crate::lyxal_core_db::sql::kind::KindLiteral;
use crate::lyxal_core_db::sql::statements::define::connector::*;
use crate::lyxal_core_db::sql::statements::define::config::api::{ApiConfig, Middleware};
use crate::lyxal_core_db::sql::statements::define::config::defaults::DefaultConfig;
use crate::lyxal_core_db::sql::statements::define::config::graphql::{GraphQLConfig, TableConfig};
use crate::lyxal_core_db::sql::statements::define::config::{ConfigInner, graphql};
use crate::lyxal_core_db::sql::statements::define::user::PassType;
use crate::lyxal_core_db::sql::statements::define::{
	ApiAction, DefineAccessStatement, DefineAnalyzerStatement, DefineApiStatement,
	DefineBucketStatement, DefineConfigStatement,DefineConnectorStatement, DefineDatabaseStatement, DefineDefault,
...
use crate::lyxal_core_db::sql::{ModuleExecutable, SiloExecutable, Lyxal_lismExecutable};
use crate::lyxal_core_db::syn::error::bail;
use crate::lyxal_core_db::syn::parser::mac::{expected, unexpected};
use crate::lyxal_core_db::syn::parser::{ParseResult, Parser};
use crate::lyxal_core_db::syn::token::{Token, TokenKind, t};
...
impl Parser<'_> {
	pub(crate) async fn parse_define_stmt(
		...
		match next.kind {
			...
			t!("CONNECTOR") => self.parse_define_connector(stk).await.map(DefineStatement::Connector),			
	...
	/// DEFINE CONNECTOR ///
		pub(crate) async fn parse_define_connector(
    &mut self,
    stk: &mut Stk,
) -> ParseResult<DefineConnectorStatement> {
    let kind = if self.eat(t!("IF")) {
        expected!(self, t!("NOT"));
        expected!(self, t!("EXISTS"));
        DefineKind::IfNotExists
    ...
    let name = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
    expected!(self, t!("URL"));
    let base_url = stk.run(|ctx| self.parse_expr_field(ctx)).await?;

    let mut res = DefineConnectorStatement {
        kind,
        name,
        base_url,
        ..Default::default()
    ...
    loop {
        match self.peek_kind() {
            t!("HEADERS") => {
                self.pop_peek();
                res.headers = Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
            }
            t!("AUTH") => {
                self.pop_peek();
                res.auth = Some(self.parse_connector_auth(stk).await?);
            }
            t!("ENDPOINT") => {
                self.pop_peek();
                res.endpoints.push(self.parse_connector_endpoint(stk).await?);
            }
            t!("RETRY") => {
                self.pop_peek();
                let attempts = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
                expected!(self, t!("BACKOFF"));
                let backoff = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
                let mut on = Vec::new();
                if self.eat(t!("ON")) {
                    loop {
                        on.push(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
                        if !self.eat(t!(",")) {
                            break;
                        }
                    }
                }
                res.config.retry = Some(RetryConfig { attempts, backoff, on });
            }
            t!("LIMIT") => {
                self.pop_peek();
                let requests = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
                expected!(self, t!("PER"));
                let per = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
                res.config.rate_limit = Some(RateLimitConfig { requests, per });
            }
            t!("ON") => {
                self.pop_peek();
                expected!(self, t!("ERROR"));
                res.error_map.push(self.parse_connector_error_map(stk).await?);
            }
            t!("COMMENT") => {
                self.pop_peek();
	...
	async fn parse_connector_endpoint(
		&mut self,
		stk: &mut Stk,
	) -> ParseResult<ConnectorEndpoint> {
		let name = self.parse_ident()?;
		expected!(self, t!("METHOD"));
		let peek = self.peek();
		let method = match peek.kind {
			t!("GET")    => { self.pop_peek(); ApiMethod::Get }
			t!("POST")   => { self.pop_peek(); ApiMethod::Post }
			t!("PUT")    => { self.pop_peek(); ApiMethod::Put }
			t!("PATCH")  => { self.pop_peek(); ApiMethod::Patch }
			t!("DELETE") => { self.pop_peek(); ApiMethod::Delete }
			_ => unexpected!(self, peek, "`GET`, `POST`, `PUT`, `PATCH` or `DELETE`"),
		};
		expected!(self, t!("PATH"));
		let path = stk.run(|ctx| self.parse_expr_field(ctx)).await?;

		let mut endpoint = ConnectorEndpoint {
			name,
			method,
			path,
			query: None,
		...
		loop {
			match self.peek_kind() {
				t!("QUERY") => {
					self.pop_peek();
					endpoint.query = Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
				}
				t!("BODY") => {
					self.pop_peek();
					endpoint.body = Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
				}
				t!("TIMEOUT") => {
					self.pop_peek();
					endpoint.timeout = Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
				}
				_ => break,
			}
		}
	...
	async fn parse_connector_auth(
		&mut self,
		stk: &mut Stk,
	) -> ParseResult<ConnectorAuth> {
		let peek = self.peek();
		match peek.kind {
			t!("BEARER") => {
				self.pop_peek();
				let token = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
				Ok(ConnectorAuth::Bearer(token))
			}
			t!("BASIC") => {
				self.pop_peek();
				let user = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
				let pass = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
				Ok(ConnectorAuth::Basic(user, pass))
			}
			t!("APIKEY") => {
				self.pop_peek();
				let name = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
				...
				expected!(self, t!("IN"));
				let peek = self.peek();
				let location = match peek.kind {
					t!("HEADER") => { self.pop_peek(); AuthLocation::Header }
					t!("QUERY")  => { self.pop_peek(); AuthLocation::Query }
					_ => unexpected!(self, peek, "`HEADER` or `QUERY`"),
				};
				Ok(ConnectorAuth::ApiKey { name, value, location })
			}
			_ => unexpected!(self, peek, "`BEARER`, `BASIC` or `APIKEY`"),
		}
	}

	async fn parse_connector_error_map(
		&mut self,
		stk: &mut Stk,
	) -> ParseResult<ConnectorErrorMap> {
		expected!(self, t!("STATUS"));
		let status = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
		expected!(self, t!("MESSAGE"));
		let message = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
		let code = if self.eat(t!("CODE")) {
			Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?)
		} else {
			None
		};
		Ok(ConnectorErrorMap { status, message, code })
	}
		...
		loop {
			match self.peek_kind() {
				...
				t!("FULLTEXT") => {
					...
					res.index = Index::FullText(crate::lyxal_core_db::sql::index::FullTextParams {
   ...
   /// DEFINE MODULE ///
	...
	pub(crate) async fn parse_define_module(
		...
		bail!(
			"Lyxal_lism modules are not supported in WASM environments",
	...
	pub(crate) async fn parse_define_module(
		...
		if !self.settings.lyxal_lism_enabled {
			bail!(
				"Experimental capability `lyxal_lism` is not enabled",
		...
		let executable = match peek.kind {
			...
			t!("f\"") | t!("f'") => {
				...
				ModuleExecutable::Lyxal_lism(Lyxal_lismExecutable(file.into()))
			}
			_ => {
