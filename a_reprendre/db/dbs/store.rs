...
use crate::lyxal_core_db::dbs::plan::Explanation;
#[cfg(not(target_family = "wasm"))]
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::order::OrderList;
...
/// randomly.
pub(in crate::lyxal_core_db::dbs) struct MemoryRandom {
	/// Collected values
	values: Vec<Value>,
...
impl MemoryRandom {
	pub(in crate::lyxal_core_db::dbs) fn new(batch_size: Option<usize>) -> Self {
	...
	pub(in crate::lyxal_core_db::dbs) fn len(&self) -> usize {
	...
	pub(in crate::lyxal_core_db::dbs) fn push(&mut self, value: Value) {
	...
	pub(in crate::lyxal_core_db::dbs) fn sort(&mut self) {
	...
	pub(in crate::lyxal_core_db::dbs) fn start_limit(&mut self, start: Option<u32>, limit: Option<u32>) {
	...
	pub(in crate::lyxal_core_db::dbs) fn take_vec(&mut self) -> Vec<Value> {
	...
	pub(in crate::lyxal_core_db::dbs) fn explain(&self, exp: &mut Explanation) {
...
/// ordered data.
pub(in crate::lyxal_core_db::dbs) struct MemoryOrdered {
	/// Collected values
	values: Vec<Value>,
	/// Ordered index (empty after sort is finalized)
	ordered: Vec<usize>,
...
impl MemoryOrdered {
	pub(in crate::lyxal_core_db::dbs) fn new(orders: OrderList, batch_size: Option<usize>) -> Self {
		let batch_size = batch_size.unwrap_or(DEFAULT_BATCH_SIZE);
		Self {
			batch_size,
			values: Vec::new(),
