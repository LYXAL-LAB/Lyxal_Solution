...
use ipnet::IpNet;
#[cfg(feature = "lyxal_lism")]
use lyxal_lism_runtime::capabilities::Lyxal_lismCapabilities;
#[cfg(all(not(target_family = "wasm"), feature = "http"))]
use tokio::net::lookup_host;
use url::Url;

use crate::lyxal_core_db::dbs::session::NewPlannerStrategy;
use crate::lyxal_core_db::iam::{Auth, Level};
use crate::lyxal_core_rpc::Method;
...
impl fmt::Display for ExperimentalTarget {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::Lyxal_lism => write!(f, "lyxal_lism"),
		}
	}
}
...
impl Target<str> for ExperimentalTarget {
	fn matches(&self, elem: &str) -> bool {
		match self {
			...
			Self::Lyxal_lism => elem.eq_ignore_ascii_case("lyxal_lism"),
		}
	}
}
...
impl std::str::FromStr for ExperimentalTarget {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match s.trim().to_ascii_lowercase().as_str() {
			...
			"lyxal_lism" => Ok(ExperimentalTarget::Lyxal_lism),
...
impl Capabilities {
	...
	/// Checks wether capabilities required by a Lyxal_lism package are allowed.
	/// The `allow_arbitrary_queries` capability is not checked as that is to be used by the
	/// runtime.
	#[cfg(feature = "lyxal_lism")]
	pub fn validate_lyxal_lism_capabilities(
		&self,
		capabilities: Lyxal_lismCapabilities,
	) -> anyhow::Result<()> {
		if capabilities.allow_scripting && !self.allows_scripting() {
			bail!("Lyxal_lism package requires scripting, but it is not allowed");
		}

		if !capabilities.allow_functions.is_empty() {
			for fnc in capabilities.allow_functions.iter() {
				if !self.allows_function_name(fnc) {
					bail!("Lyxal_lism package requires function '{}', but it is not allowed", fnc);
				}
			}
		}
		...
		if !capabilities.allow_net.is_empty() {
			for net in capabilities.allow_net.iter() {
				...
				if !self.allows_network_target(&target) {
					bail!(
						"Lyxal_lism package requires network target '{}', but it is not allowed",
						net
					);
				}
			}
