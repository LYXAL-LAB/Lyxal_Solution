...
use lyxal_types_core::{Error as TypesError, QueryError, ToSql};
...
use crate::lyxal_core_db::catalog::providers::{CatalogProvider, NamespaceProvider, RootProvider};
use crate::lyxal_core_db::ctx::reason::Reason;
use crate::lyxal_core_db::ctx::{Context, FrozenContext};
use crate::lyxal_core_db::dbs::response::QueryResult;
use crate::lyxal_core_db::dbs::{Force, Options, QueryType};
use crate::lyxal_core_db::doc::DefaultBroker;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::exec::planner::try_plan_expr;
use crate::lyxal_core_db::expr::parameterize::expr_to_ident;
use crate::lyxal_core_db::expr::paths::{DB, NS};
use crate::lyxal_core_db::expr::plan::LogicalPlan;
use crate::lyxal_core_db::expr::statements::{OptionStatement, UseStatement};
use crate::lyxal_core_db::expr::{Base, ControlFlow, Expr, FlowResult, TopLevelExpr};
use crate::lyxal_core_db::iam::{Action, ResourceKind};
use crate::lyxal_core_kvs::slowlog::SlowLogVisit;
use crate::lyxal_core_kvs::{Datastore, LockType, Transaction, TransactionType};
use crate::lyxal_core_rpc::types_error_from_anyhow;
use crate::types::PublicNotification;
use crate::lyxal_core_db::val::{Array, Value, convert_value_to_public_value};
use crate::{lyxal_core_error, crate::lyxal_core_db::expr, crate::lyxal_core_db::sql};
use crate::map;
use crate::yield_now;

const TARGET: &str = "lyxal_db::dbs";
...
pub struct Executor {
	...
	cached_session: Option<Arc<crate::lyxal_core_db::exec::context::SessionInfo>>,
}
...
impl Executor {
	...
	fn get_session_info(&mut self) -> Option<Arc<crate::lyxal_core_db::exec::context::SessionInfo>> {
	...
	fn extract_session_info(&self) -> Option<std::sync::Arc<crate::lyxal_core_db::exec::context::SessionInfo>> {
		use crate::lyxal_core_db::exec::context::SessionInfo;
		use crate::lyxal_core_db::expr::paths::{AC, DB, ID, IP, NS, OR, RD, TK};
	...
	async fn execute_operator_plan(
		&mut self,
		plan: Arc<dyn crate::lyxal_core_db::exec::ExecOperator>,
		txn: Arc<Transaction>,
	) -> FlowResult<Value> {
		use tokio_util::sync::CancellationToken;

		use crate::lyxal_core_db::catalog::providers::{DatabaseProvider, NamespaceProvider};
		use crate::lyxal_core_db::exec::context::{
		...
		let exec_ctx = match required_level {
			crate::lyxal_core_db::exec::context::ContextLevel::Root => ExecutionContext::Root(root_ctx),
			crate::lyxal_core_db::exec::context::ContextLevel::Namespace => {
			...
			crate::lyxal_core_db::exec::context::ContextLevel::Database => {
		...
		let stream = match plan.execute(&exec_ctx) {
			Ok(s) => s,
			Err(crate::lyxal_core_db::expr::ControlFlow::Return(v)) => {
				// RETURN - propagate as control flow signal
				return Err(ControlFlow::Return(v));
			}
			Err(crate::lyxal_core_db::expr::ControlFlow::Break) => {
				return Err(ControlFlow::Break);
			}
			Err(crate::lyxal_core_db::expr::ControlFlow::Continue) => {
				return Err(ControlFlow::Continue);
			}
			Err(crate::lyxal_core_db::expr::ControlFlow::Err(e)) => {
	...
	/// transaction.
	#[instrument(level = "debug", name = "executor", target = "lyxal_db::dbs", skip_all)]
	async fn execute_plan_in_transaction(
		...
		let res = match plan {
			...
			TopLevelExpr::Kill(s) => {
				Arc::get_mut(&mut self.ctx)
					.ok_or_else(|| {
						crate::lyxal_core_error::Error::unreachable(
						...
						for res in &mut self.results[start_results..] {
							...
							res.result = Err(TypesError::query(
								format!(
									...
									crate::lyxal_core_db::val::Duration::from(timeout)
								),
		...
		for expr in plan.expressions {
			...
			let query_result = match result {
				Ok(value) | Err(ControlFlow::Return(value)) => QueryResult {
					time,
					result: crate::lyxal_core_db::val::convert_value_to_public_value(value)
...
mod tests {
	...
	use crate::lyxal_core_db::iam::{Level, Role};
