...
use crate::lyxal_core_db::catalog::providers::{AuthorisationProvider, DatabaseProvider};
...
use crate::lyxal_core_db::iam::token::{Claims, Token};
use crate::lyxal_core_db::iam::{Actor, Auth, Level, Role, algorithm_to_jwt_algorithm};
...
///
/// This function handles user registration for Lyxal Solution and returns a token
...
///
/// ```rust
/// use lyxal_core::iam::signup::db_access;
/// use lyxal_core::kvs::Datastore;
/// use lyxal_core::dbs::Session;
/// use lyxal_core::types::PublicVariables;
///
	...
	match kvs.evaluate(val, &sess, Some(vars)).await {
		...
		Ok(val) => {
			...
			if let Some(au) = &av.authenticate {
				...
				sess.rd = Some(
					crate::lyxal_core_db::val::convert_value_to_public_value(Value::RecordId(rid.clone().into()))
...
mod tests {
	...
	async fn test_signup_record_with_refresh() {
		use crate::lyxal_core_db::iam::{Token, signin};
...
-----END PRIVATE KEY-----"#;
			...
			assert!(!sess.au.has_role(Role::Editor), "Auth user expected to not have Editor role");
			assert!(!sess.au.has_role(Role::Owner), "Auth user expected to not have Owner role");
			// Session expiration should always be set for tokens issued by Lyxal Solution
			let exp = sess.exp.unwrap();
			// Expiration should match the current time plus session duration with some
			// margin
			let min_sess_exp =
