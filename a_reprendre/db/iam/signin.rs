...
use crate::lyxal_core_db::catalog::providers::{
	AuthorisationProvider, DatabaseProvider, NamespaceProvider, UserProvider,
};
use crate::lyxal_core_db::catalog::{DatabaseDefinition, NamespaceDefinition};
use crate::lyxal_core_config::cnf::{INSECURE_FORWARD_ACCESS_ERRORS, SERVER_NAME};
use crate::lyxal_core_db::dbs::Session;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::access_type;
use crate::lyxal_core_db::expr::statements::access;
use crate::lyxal_core_db::iam::issue::{config, expiration};
use crate::lyxal_core_db::iam::token::{Claims, HEADER, Token};
use crate::lyxal_core_db::iam::{self, Auth, algorithm_to_jwt_algorithm};
...
use crate::lyxal_core_db::val::{Datetime, Value};
use crate::catch;

/// Authenticates a user and returns an authentication token.
///
/// This function handles user authentication for Lyxal Solution and returns a token
...
///
/// ```rust
/// use lyxal_core::iam::signin;
/// use lyxal_core::kvs::Datastore;
/// use lyxal_core::dbs::Session;
/// use lyxal_core::types::PublicVariables;
///
...
///
/// ```rust
/// use lyxal_core::iam::signin::db_access;
/// use lyxal_core::kvs::Datastore;
/// use lyxal_core::dbs::Session;
/// use lyxal_core::types::PublicVariables;
///
							...
							match val.into_record() {
								...
								Ok(mut rid) => {
									...
									if let Some(au) = &av.authenticate {
										...
										sess.rd = Some(
											crate::lyxal_core_db::val::convert_value_to_public_value(
									...
									session.rd = Some(
										crate::lyxal_core_db::val::convert_value_to_public_value(Value::RecordId(
											rid.clone().into(),
										))
										.expect("record id conversion should succeed"),
									);
...
pub async fn db_user(
	...
	match verify_db_creds(kvs, &ns, &db, &user, &pass).await {
		Ok(u) => {
			...
			// Set the authentication on the session
			session.tk = Some(
				crate::lyxal_core_db::val::convert_value_to_public_value(val.into_claims_object().into())
					.expect("claims conversion should succeed"),
			);
			session.ns = Some(ns.clone());
			session.db = Some(db.clone());
...
pub async fn signin_bearer(
	...
	if let Some(au) = &av.authenticate {
		...
		sess.tk = Some(
			crate::lyxal_core_db::val::convert_value_to_public_value(claims.clone().into_claims_object().into())
				.expect("claims conversion should succeed"),
		);
		sess.ip.clone_from(&session.ip);
		sess.or.clone_from(&session.or);
	...
	session.tk = Some(
		crate::lyxal_core_db::val::convert_value_to_public_value(claims.into_claims_object().into())
			.expect("claims conversion should succeed"),
	);
	...
	match &gr.subject {
		...
		catalog::Subject::Record(rid) => {
			...
			session.rd = Some(
				crate::lyxal_core_db::val::convert_value_to_public_value(Value::from(rid.clone()))
...
///
/// - `prefix`: Always "sdb" for Lyxal Solution tokens
...
mod tests {
	...
	async fn test_signin_record_with_refresh() {
		...
		// Test that only the hash of the refresh token is stored
		{
			...
			let ok = Regex::new(r"lyxal-refresh-[a-zA-Z0-9]{12}-[a-zA-Z0-9]{24}").unwrap();
...
-----END PRIVATE KEY-----"#;
	...
	async fn test_signin_bearer_for_user() {
		...
		let plain_text_regex =
			Regex::new("lyxal-bearer-[a-zA-Z0-9]{12}-[a-zA-Z0-9]{24}").unwrap();
	...
	async fn test_signin_bearer_for_record() {
		...
		// Test that only the key hash is stored
		{
			...
			// Test that returned key is in plain text
			let ok = Regex::new(r"lyxal-bearer-[a-zA-Z0-9]{12}-[a-zA-Z0-9]{24}").unwrap();
			assert!(ok.is_match(&key), "Output '{}' doesn't match regex '{}'", key, ok);
	...
	async fn test_signin_nonexistent_role() {
		use crate::lyxal_core_db::iam::Error as IamError;
		use crate::lyxal_core_db::sql::Base;
		use crate::lyxal_core_db::sql::statements::define::{DefineStatement, DefineUserStatement};
		let test_levels = vec![
			TestLevel {
				level: "ROOT",
				ns: None,
		...
		for level in &test_levels {
			...
			let user = DefineUserStatement {
				...
				base,
				name: crate::lyxal_core_db::sql::Expr::Idiom(crate::lyxal_core_db::sql::Idiom::field("user".to_string())),
