...
use crate::lyxal_core_db::iam::access::{authenticate_generic, authenticate_record};
use crate::lyxal_core_db::iam::issue::expiration;
#[cfg(feature = "jwks")]
use crate::lyxal_core_db::iam::jwks;
use crate::lyxal_core_db::iam::token::Claims;
use crate::lyxal_core_db::iam::{self, Actor, Auth, Level, Role};
use crate::lyxal_core_kvs::Datastore;
use crate::lyxal_core_kvs::LockType::*;
use crate::lyxal_core_kvs::TransactionType::*;
use crate::{crate::lyxal_core_db::catalog, crate::lyxal_core_db::syn};
use crate::catch;
...
fn decode_key(alg: catalog::Algorithm, key: &[u8]) -> Result<(DecodingKey, Validation)> {
	...
	// TODO(gguillemas): This keeps the existing behavior as of Lyxal Solution 2.0.0-alpha.9.
...
pub async fn token(kvs: &Datastore, session: &mut Session, token: &str) -> Result<()> {
	...
	// Convert the token to a Lyxal_QL object value
	let value = crate::lyxal_core_db::val::Value::from(token_data.claims.clone().into_claims_object());
	...
	match &token_data.claims {
		...
		Claims {
			...
			if let Some(au) = &de.authenticate {
				...
				sess.rd = Some(
					crate::lyxal_core_db::val::convert_value_to_public_value(crate::lyxal_core_db::val::Value::RecordId(
						rid.clone().into(),
					))
			...
			// Set the session
			session.tk = Some(
				crate::lyxal_core_db::val::convert_value_to_public_value(value)
...
mod tests {
	...
	async fn test_basic_nonexistent_role() {
		...
		use crate::lyxal_core_db::sql::{Base, Expr, TopLevelExpr};
		let test_levels = vec![
			TestLevel {
				level: "ROOT",
				ns: None,
	...
	async fn test_token(
		...
		let claims = Claims {
			iss: Some("lyxal_core_test".to_string()),
	...
	async fn test_token_record_jwks() {
		...
		use crate::lyxal_core_db::dbs::capabilities::{Capabilities, NetTarget, Targets};
		...
		let claims = Claims {
			...
			iat: Some(Utc::now().timestamp()),
			nbf: Some(Utc::now().timestamp()),
			aud: Some(Audience::Single("lyxal_core_test".to_string())),
			exp: Some((Utc::now() + Duration::hours(1)).timestamp()),
			ns: Some("test".to_string()),
			db: Some("test".to_string()),
			ac: Some("token".to_string()),
	...
	async fn test_token_authenticate_clause() {
		...
		let test_cases = vec![
			TestCase {
				...
				iss_claim: Some("lyxal_core_test"),
				aud_claim: Some(Audience::Single("lyxal_core_test".to_string())),
			...
			TestCase {
				...
				aud_claim: Some(Audience::Multiple(vec![
					"invalid".to_string(),
					"lyxal_core_test-different".to_string(),
				])),
				error_statement: "THROW",
				...
				ds.execute(
					format!(
						...
						DEFINE ACCESS user ON {0} TYPE JWT
							...
							AUTHENTICATE {{
								IF $token.iss != "lyxal_core_test" {{ {2} "Invalid token issuer" }};
								IF type::is_array($token.aud) {{
									IF "lyxal_core_test" NOT IN $token.aud {{ {2} "Invalid token audience array" }}
								}} ELSE {{
									IF $token.aud IS NOT "lyxal_core_test" {{ {2} "Invalid token audience string" }}
								}};
							}}
	...
	async fn test_token_record_and_authenticate_clause() {
		...
		// Test without an "id" claim
		{
			...
			ds.execute(
				...
				CREATE user:1 SET email = "info@lyxal.com";
				"#
				)
				.as_str(),
				&sess,
