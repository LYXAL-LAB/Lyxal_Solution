...
static CACHE_EXPIRATION: LazyLock<chrono::Duration> =
	LazyLock::new(|| match std::env::var("LYXAL_JWKS_CACHE_EXPIRATION_SECONDS") {
		Ok(seconds_str) => {
			let seconds = seconds_str.parse::<u64>().expect(
				"Expected a valid number of seconds for LYXAL_JWKS_CACHE_EXPIRATION_SECONDS",
			);
...
static CACHE_COOLDOWN: LazyLock<chrono::Duration> =
	LazyLock::new(|| match std::env::var("LYXAL_JWKS_CACHE_COOLDOWN_SECONDS") {
		Ok(seconds_str) => {
			let seconds = seconds_str.parse::<u64>().expect(
				"Expected a valid number of seconds for LYXAL_JWKS_CACHE_COOLDOWN_SECONDS",
			);
			Duration::seconds(seconds as i64)
		}
		Err(_) => {
...
static REMOTE_TIMEOUT: LazyLock<chrono::Duration> =
	LazyLock::new(|| match std::env::var("LYXAL_JWKS_REMOTE_TIMEOUT_MILLISECONDS") {
		Ok(milliseconds_str) => {
			let milliseconds = milliseconds_str.parse::<u64>().expect(
				"Expected a valid number of milliseconds for LYXAL_JWKS_REMOTE_TIMEOUT_MILLISECONDS",
			);
...
pub(super) async fn config(
	...
	match DecodingKey::from_jwk(&jwk) {
		Ok(dec) => {
			...
			// TODO(gguillemas): This keeps the existing behavior as of Lyxal Solution
...
async fn fetch_jwks_from_url(cache: &Arc<RwLock<JwksCache>>, url: &str) -> Result<JwkSet> {
	...
	#[cfg(not(target_family = "wasm"))]
	let req = req.header(reqwest::header::USER_AGENT, &*crate::lyxal_core_config::cnf::LYXAL_USER_AGENT);
	#[cfg(not(target_family = "wasm"))]
...
mod tests {
	...
	async fn test_golden_path() {
		...
		Mock::given(method("GET"))
			.and(path(&jwks_path))
			.and(header("user-agent", "Lyxal Solution"))
	...
	// Lyxal Solution will not trust a token specifying an algorithm that does not match
