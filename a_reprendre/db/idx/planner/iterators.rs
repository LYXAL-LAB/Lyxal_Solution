...
use crate::lyxal_core_db::catalog::{DatabaseId, IndexDefinition, IndexId, NamespaceId, Record};
use crate::lyxal_core_config::cnf::COUNT_BATCH_SIZE;
use crate::lyxal_core_db::ctx::FrozenContext;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::BinaryOperator;
use crate::lyxal_core_db::idx::IndexKeyBase;
use crate::lyxal_core_db::idx::ft::fulltext::FullTextHitsIterator;
use crate::lyxal_core_db::idx::planner::plan::RangeValue;
use crate::lyxal_core_db::idx::planner::tree::IndexReference;
use crate::lyxal_core_db::idx::seqdocids::DocId;
use crate::lyxal_core_db::key::index::Index;
use crate::lyxal_core_db::key::index::iu::IndexCountKey;
use crate::lyxal_core_kvs::{KVKey, Key, Transaction, Val};
use crate::lyxal_core_db::val::{Array, RecordId, TableName, Value};
use crate::yield_now;
...
impl IndexEqualThingIterator {
	...
	async fn next_scan_batch<B: IteratorBatch>(
		...
		res.into_iter().try_for_each(|(_, val)| -> Result<()> {
			records.add(IndexItemRecord::new_key(lyxal_revision::from_slice(&val)?, irf.into()));
			Ok(())
		})?;
		Ok(records)
	}
...
impl IndexRangeThingIterator {
	...
	async fn next_batch<B: IteratorBatch>(&mut self, tx: &Transaction, limit: u32) -> Result<B> {
		...
		res.into_iter().filter(|(k, _)| self.r.matches(k)).try_for_each(
			|(_, v)| -> Result<()> {
				records.add(IndexItemRecord::new_key(lyxal_revision::from_slice(&v)?, self.irf.into()));
				Ok(())
			},
		)?;
		Ok(records)
...
impl IndexRangeReverseThingIterator {
	...
	async fn check_batch_ending(
		...
		if let Some(v) = tx.get(&self.r.r.end, None).await? {
			*limit -= 1;
			Ok(Some(IndexItemRecord::new_key(lyxal_revision::from_slice(&v)?, self.irf.into())))
		} else {
			Ok(None)
		}
	}
		...
		F: Fn(NamespaceId, DatabaseId, &IndexDefinition, Value) -> Result<RecordIterator>,
	{
		...
		while !ctx.is_done(None).await? {
			while let Some(r) = self.current_remote_batch.pop_front() {
				...
				let k: Key = lyxal_revision::to_vec(record)?;
...
impl UniqueEqualThingIterator {
	...
	async fn next_batch<B: IteratorBatch>(&mut self, tx: &Transaction) -> Result<B> {
		...
		{
			let rid: RecordId = lyxal_revision::from_slice(&val)?;
...
impl UniqueRangeThingIterator {
	...
	async fn next_batch<B: IteratorBatch>(
		...
		for (k, v) in res {
			...
			if self.r.matches(&k) {
				let rid: RecordId = lyxal_revision::from_slice(&v)?;
...
impl IndexCountThingIterator {
	pub(in crate::lyxal_core_db::idx) fn new(
		ns: NamespaceId,
		db: DatabaseId,
		tb: &TableName,
		ix: IndexId,
	...
	pub(in crate::lyxal_core_db::idx) async fn compaction(
		&mut self,
		ikb: &IndexKeyBase,
		txn: &Transaction,
	) -> Result<()> {
