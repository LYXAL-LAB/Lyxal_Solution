...
use crate::lyxal_core_db::catalog::{FullTextParams, Scoring};
...
use crate::lyxal_core_db::expr::Idiom;
use crate::lyxal_core_db::expr::operator::BooleanOperator;
use crate::lyxal_core_db::idx::IndexKeyBase;
use crate::lyxal_core_db::idx::ft::analyzer::Analyzer;
use crate::lyxal_core_db::idx::ft::analyzer::filter::FilteringStage;
use crate::lyxal_core_db::idx::ft::analyzer::tokenizer::Tokens;
use crate::lyxal_core_db::idx::ft::highlighter::{HighlightParams, Highlighter, Offseter};
use crate::lyxal_core_db::idx::ft::offset::Offset;
use crate::lyxal_core_db::idx::ft::{DocLength, Score, TermFrequency};
use crate::lyxal_core_db::idx::planner::iterators::MatchesHitsIterator;
use crate::lyxal_core_db::idx::seqdocids::{DocId, SeqDocIds};
use crate::lyxal_core_db::idx::trees::store::IndexStores;
use crate::lyxal_core_db::key::index::tt::Tt;
use crate::lyxal_core_kvs::{Transaction, impl_kv_value_revisioned};
...
impl QueryTerms {
	...
	pub(in crate::lyxal_core_db::idx::ft) fn matches_or(&self, tks: &[Tokens]) -> Result<bool> {
	...
	pub(in crate::lyxal_core_db::idx::ft) fn matches_and(&self, tks: &[Tokens]) -> Result<bool> {
		for t in self.tokens.list() {
			let t = self.tokens.get_token_string(t)?;
			let mut found = false;
			for tokens in tks {
...
pub(crate) struct Bm25Params {
	pub(in crate::lyxal_core_db::idx) k1: f32,
	pub(in crate::lyxal_core_db::idx) b: f32,
}
...
impl FullTextIndex {
	...
	pub(in crate::lyxal_core_db::idx) async fn matches_value(
		&self,
		stk: &mut Stk,
		ctx: &FrozenContext,
		opt: &Options,
	...
	async fn compute_doc_length_and_count(
		...
		for (_, v) in tx.getr(range.clone(), None).await? {
			let st: DocLengthAndCount = lyxal_revision::from_slice(&v)?;
	...
	async fn compact_doc_length_and_count(&self, tx: &Transaction) -> Result<bool> {
		...
		tx.set(&key, &lyxal_revision::to_vec(&dlc)?, None).await?;
		Ok(has_logs)
	}
...
mod tests {
	...
	use crate::lyxal_core_db::catalog::{DatabaseId, FullTextParams, IndexId, NamespaceId};
	use crate::lyxal_core_config::cnf::dynamic::DynamicConfiguration;
	use crate::lyxal_core_db::ctx::{Context, FrozenContext};
	use crate::lyxal_core_db::dbs::Options;
	use crate::lyxal_core_db::expr::statements::DefineAnalyzerStatement;
	use crate::lyxal_core_db::idx::IndexKeyBase;
	use crate::lyxal_core_db::idx::ft::offset::Offset;
	use crate::lyxal_core_db::idx::index::IndexOperation;
	use crate::lyxal_core_kvs::LockType::*;
	use crate::lyxal_core_kvs::{Datastore, Transaction, TransactionType};
	use crate::lyxal_core_db::sql::Expr;
	use crate::lyxal_core_db::sql::statements::DefineStatement;
	use crate::lyxal_core_db::syn;
	use crate::lyxal_core_db::val::{Array, RecordId, Value};
