...
impl Resolved {
	pub(in crate::lyxal_core_db::idx) fn doc_id(&self) -> DocId {
		match self {
...
impl SeqDocIds {
	...
	pub(in crate::lyxal_core_db::idx) fn new(ikb: IndexKeyBase) -> Self {
		Self {
	...
	pub(in crate::lyxal_core_db::idx) async fn get_doc_id(
		&self,
		tx: &Transaction,
		id: &RecordIdKey,
	) -> Result<Option<DocId>> {
	...
	pub(in crate::lyxal_core_db::idx) async fn resolve_doc_id(
		&self,
		ctx: &FrozenContext,
		id: RecordIdKey,
	) -> Result<Resolved> {
	...
	pub(in crate::lyxal_core_db::idx) async fn get_id(
		ikb: &IndexKeyBase,
		tx: &Transaction,
		doc_id: DocId,
	...
	pub(in crate::lyxal_core_db::idx) async fn remove_doc_id(
		&self,
		tx: &Transaction,
		doc_id: DocId,
	) -> Result<()> {
...
mod tests {
	...
	use crate::lyxal_core_db::ctx::FrozenContext;
	use crate::lyxal_core_db::idx::IndexKeyBase;
	use crate::lyxal_core_db::idx::seqdocids::{DocId, Resolved, SeqDocIds};
	use crate::lyxal_core_kvs::LockType::Optimistic;
	use crate::lyxal_core_kvs::TransactionType::{Read, Write};
	...
	async fn test_remove_doc_id() {
		...
		// Check there's no ID and BI keys left
		{
			...
			for id in ["Foo", "Bar", "Hello", "World"] {
				let id = crate::lyxal_core_db::key::index::id::Id::new(
					TEST_NS_ID,
					TEST_DB_ID,
					&tb,
					TEST_IX_ID,
