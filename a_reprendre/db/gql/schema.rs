...
//! - **Type mapping** -- [`kind_to_type`] converts Lyxal Solution `Kind` descriptors to GraphQL
//!   `TypeRef`s; [`gql_to_sql_kind`] does the reverse conversion for input values.
//! - **Value conversion** -- [`sql_value_to_gql_value`] converts Lyxal Solution runtime values to
...
use crate::lyxal_core_db::catalog::providers::{AuthorisationProvider, DatabaseProvider, TableProvider};
...
use crate::lyxal_core_db::expr::kind::{GeometryKind, KindLiteral};
use crate::lyxal_core_db::expr::{Expr, Kind, Literal};
use crate::lyxal_core_db::gql::error::{internal_error, schema_error, type_error};
use crate::lyxal_core_db::gql::functions::process_fns;
use crate::lyxal_core_db::gql::mutations::process_mutations;
use crate::lyxal_core_db::gql::relations::collect_relations;
use crate::lyxal_core_db::gql::tables::process_tbs;
use crate::lyxal_core_kvs::{Datastore, LockType, Transaction, TransactionType};
use crate::lyxal_core_db::val::{
...
/// Convert a Lyxal Solution runtime value to a GraphQL output value.
///
...
/// Convert a Lyxal Solution runtime value to GraphQL, using field kind metadata when available.
///
...
/// Map a Lyxal Solution [`Kind`] to a GraphQL [`TypeRef`].
///
/// This is the central type-mapping function: it translates Lyxal Solution's type
...
/// Map a Lyxal Solution [`Kind`] to a GraphQL [`TypeRef`], with optional enum naming scope.
///
...
fn convert_static_record_id_key(
	key: crate::lyxal_core_db::expr::RecordIdKeyLit,
...
/// Convert a GraphQL input value to a Lyxal Solution value, guided by the expected [`Kind`].
///
...
/// the appropriate Lyxal Solution value type.  The `kind` parameter tells the function
...
/// duration, uuid, then parsed as Lyxal_QL expressions.  For `Kind::Either`,
...
/// Convert a GraphQL coordinate value (nested arrays of numbers) to a Lyxal Solution
...
/// Convert a GraphQL geometry input Object (GeoJSON format) to a Lyxal Solution
/// `Geometry` value.
///
...
/// Convert a Lyxal Solution `Geometry` value to a `GqlValue::Object` in GeoJSON format.
///
