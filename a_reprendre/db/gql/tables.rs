...
use crate::lyxal_core_db::catalog::{FieldDefinition, TableDefinition};
use crate::lyxal_core_db::dbs::Session;
use crate::lyxal_core_db::expr::field::Selector;
use crate::lyxal_core_db::expr::order::{OrderList, Ordering};
use crate::lyxal_core_db::expr::part::Part;
use crate::lyxal_core_db::expr::statements::SelectStatement;
...
use crate::lyxal_core_db::gql::error::internal_error;
use crate::lyxal_core_db::gql::schema::{geometry_gql_type_name, kind_to_type_with_enum_prefix, unwrap_type};
use crate::lyxal_core_db::gql::utils::{GqlValueUtils, execute_plan};
use crate::lyxal_core_kvs::Datastore;
use crate::lyxal_core_db::val::{Array as SurArray, Datetime, Object as SurObject, RecordId, TableName, Value};
...
fn parse_version_arg(args: &IndexMap<Name, GqlValue>) -> Result<Option<Datetime>, GqlError> {
	match args.get("version") {
		Some(GqlValue::String(s)) => {
			let dt = crate::lyxal_core_db::val::datetime(s)
...
struct NestedSubField {
	...
	name: String,
	/// The field's Lyxal Solution kind, if defined.
	kind: Option<Kind>,
...
fn make_table_get_field(tb: &TableDefinition, kvs: Arc<Datastore>) -> Field {
	...
	Field::new(format!("_get_{}", tb.name), TypeRef::named(&tb_name_str), move |ctx| {
		...
		FieldFuture::new(async move {
			...
			let record_id: RecordId = match crate::lyxal_core_db::val::record_id(&rid_str) {
				Ok(x) => x.into(),
...
fn make_generic_get_field(kvs: Arc<Datastore>) -> Field {
	Field::new("_get", TypeRef::named("record"), move |ctx| {
		...
		FieldFuture::new(async move {
			...
			let record_id: RecordId = match crate::lyxal_core_db::val::record_id(&id) {
				Ok(x) => x.into(),
				Err(_) => {
...
/// Convert a GraphQL filter input object into a Lyxal_QL `WHERE` condition.
///
...
/// Operators that map directly to Lyxal Solution binary operators.
...
/// Operators that map to Lyxal Solution function calls.
...
/// Convert a single field's filter object to a Lyxal_QL expression.
///
