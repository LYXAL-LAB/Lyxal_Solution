...
use crate::lyxal_core_db::catalog::FunctionDefinition;
use crate::lyxal_core_db::dbs::Session;
use crate::lyxal_core_db::expr::{Expr, FunctionCall, Kind, LogicalPlan, TopLevelExpr};
use crate::lyxal_core_db::gql::schema::kind_to_type_with_enum_prefix;
...
/// - A resolver that converts GraphQL arguments to Lyxal_QL values, invokes the function via a
		...
		let mut field = Field::new(
			...
			move |ctx| {
				...
				FieldFuture::new(async move {
					...
					// Convert each GraphQL argument to its Lyxal_QL equivalent
					...
					let func_call = Expr::FunctionCall(Box::new(FunctionCall {
						receiver: crate::lyxal_core_db::expr::Function::Custom(fnd1.name.clone()),
						arguments: args,
					}));
					...
					// Convert the Lyxal_QL result to a GraphQL value
