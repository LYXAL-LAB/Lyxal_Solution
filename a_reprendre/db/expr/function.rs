...
use crate::lyxal_core_db::expr::{Expr, Idiom, Kind, Model, ModuleExecutable, Script, Value};
use crate::lyxal_core_functions;
use crate::lyxal_core_db::iam::AuthLimit;
...
impl Function {
	...
	pub fn read_only(&self) -> bool {
		match self {
			...
			Self::Normal(f) => f != "api::invoke" && f != "connector::call" && f != "connector::health" && f != "connector::batch",
			Self::Model(_) => true,
		}
	}
	...
	pub(crate) async fn compute(
		...
		match self {
			Function::Normal(s) => {
				...
				Ok(crate::lyxal_core_functions::run(stk, ctx, opt, doc, s, args).await?)
			}
...
impl ToSql for FunctionCall {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let fnc: crate::lyxal_core_db::sql::FunctionCall = self.clone().into();
		fnc.fmt_sql(f, fmt);
	}
}
