...
use crate::lyxal_core_db::expr::{Expr, FlowResult, RecordIdLit};
...
/// lyxal_ql rules regarding number equality are not observed, 1f != 1dec.
...
impl Literal {
	...
	pub(crate) async fn compute(
		...
		let res = match self {
			...
			Literal::Set(exprs) => {
				let mut set = crate::lyxal_core_db::val::Set::new();
				for e in exprs.iter() {
...
impl ToSql for Literal {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let lit: crate::lyxal_core_db::sql::Literal = self.clone().into();
		lit.fmt_sql(f, fmt);
	}
}
...
impl ToSql for ObjectEntry {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let entry: crate::lyxal_core_db::sql::literal::ObjectEntry = self.clone().into();
		entry.fmt_sql(f, fmt);
	}
}
