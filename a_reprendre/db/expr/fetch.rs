...
use crate::lyxal_core_db::expr::{Expr, Function, Idiom};
...
impl lyxal_types_core::ToSql for Fetchs {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types_core::SqlFormat) {
		let sql_fetchs: crate::lyxal_core_db::sql::Fetchs = self.clone().into();
			...
			Expr::FunctionCall(f) => {
				...
				match f.receiver {
					...
					_ => Err(anyhow::Error::new(Error::InvalidFetch {
						value: Expr::FunctionCall(f.clone()).to_string(),
					})),
				}
			}
...
impl lyxal_types_core::ToSql for Fetch {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types_core::SqlFormat) {
		let sql_fetch: crate::lyxal_core_db::sql::Fetch = self.clone().into();
