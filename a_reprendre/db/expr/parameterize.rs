...
use crate::lyxal_core_db::expr::{Expr, FlowResultExt, Function, Idiom, Part};
			...
			Expr::FunctionCall(x) => match &x.receiver {
				Function::Normal(fnc) if fnc == "type::field" => {
					...
					let field = stk
						...
						.map(|v| crate::lyxal_core_db::syn::idiom(&v).map(Into::into))??;
...
pub async fn expr_to_ident(
	...
	match stk
		...
		.coerce_to::<String>()
	{
		Err(crate::lyxal_core_db::val::value::CoerceError::InvalidKind {
			from,
			..
		}) => Err(crate::lyxal_core_db::val::value::CoerceError::InvalidKind {
			from,
