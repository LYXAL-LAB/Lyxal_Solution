...
use crate::lyxal_core_db::expr::{Base, Cond, ControlFlow, FlowResult, FlowResultExt as _, RecordIdLit};
use crate::lyxal_core_db::iam::{Action, ResourceKind};
use crate::lyxal_core_db::val::{Array, Datetime, Duration, Object, Value};
use crate::lyxal_core_db::{catalog, val};
...
pub fn new_grant_bearer(ty: catalog::BearerAccessType) -> catalog::GrantBearer {
	...
	let prefix = match ty {
		catalog::BearerAccessType::Bearer => "lyxal-bearer",
		catalog::BearerAccessType::Refresh => "lyxal-refresh",
	};
...
/// Returns the lyxal_ql object representation of the access grant
...
pub async fn create_grant(
	...
	match &ac.access_type {
		...
		catalog::AccessType::Record(at) => {
			...
			let res = match base {
				Base::Db => {
					...
					let key = crate::lyxal_core_db::key::database::access::gr::new(ns, db, &gr.ac, &gr.id);
			...
			match res {
				...
				Err(e) => {
					if matches!(
						e.downcast_ref(),
						Some(Error::Kvs(crate::lyxal_core_kvs::Error::TransactionKeyAlreadyExists))
					) {
						error!(
		...
		catalog::AccessType::Bearer(at) => {
			...
			let res = match base {
				Base::Root => {
					let key = crate::lyxal_core_db::key::root::access::gr::new(&gr.ac, &gr.id);
					txn.put(&key, &gr_store, None).await
				}
				Base::Ns => {
					let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
					let key =
						crate::lyxal_core_db::key::namespace::access::gr::new(ns.namespace_id, &gr.ac, &gr.id);
					txn.put(&key, &gr_store, None).await
				}
				Base::Db => {
					let (ns, db) = opt.ns_db()?;
					...
					let key = crate::lyxal_core_db::key::database::access::gr::new(
						db.namespace_id,
						db.database_id,
						&gr.ac,
						&gr.id,
...
pub async fn revoke_grant(
	...
	match &stmt.gr {
		Some(gr) => {
			...
			match base {
				Base::Root => {
					let key = crate::lyxal_core_db::key::root::access::gr::new(&stmt.ac, gr);
					txn.set(&key, &revoke, None).await?;
				}
				Base::Ns => {
					let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
					let key = crate::lyxal_core_db::key::namespace::access::gr::new(ns.namespace_id, &stmt.ac, gr);
					txn.set(&key, &revoke, None).await?;
				}
				Base::Db => {
					let (ns, db) = opt.ns_db()?;
		...
		None => {
			...
			for gr in grs.iter() {
				...
				match base {
					Base::Root => {
						let key = crate::lyxal_core_db::key::root::access::gr::new(&stmt.ac, &gr.id);
						txn.set(&key, &gr, None).await?;
					}
					Base::Ns => {
						let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
						let key = crate::lyxal_core_db::key::namespace::access::gr::new(
							ns.namespace_id,
							&stmt.ac,
							&gr.id,
						);
...
async fn compute_purge(
	...
	for gr in grs.iter() {
		...
		if purge_expired || purge_revoked {
			match base {
				Base::Root => txn.del(&crate::lyxal_core_db::key::root::access::gr::new(&stmt.ac, &gr.id)).await?,
				Base::Ns => {
					let ns = ctx.get_ns_id(opt).await?;
					txn.del(&crate::lyxal_core_db::key::namespace::access::gr::new(ns, &stmt.ac, &gr.id)).await?
				}
				Base::Db => {
					let (ns, db) = ctx.expect_ns_db_ids(opt).await?;
					txn.del(&crate::lyxal_core_db::key::database::access::gr::new(ns, db, &stmt.ac, &gr.id))
						.await?
				}
			};
...
impl ToSql for AccessStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let sql_stmt: crate::lyxal_core_db::sql::statements::AccessStatement = self.clone().into();
