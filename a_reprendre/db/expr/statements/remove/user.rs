...
use crate::lyxal_core_db::catalog::providers::UserProvider;
	...
	pub(crate) async fn compute(
		...
		match self.base {
			Base::Root => {
				...
				let key = crate::lyxal_core_db::key::root::us::new(&us.name);
				txn.del(&key).await?;
				// Clear the cache
				txn.clear_cache();
				// Ok all good
			...
			Base::Ns => {
				...
				let key = crate::lyxal_core_db::key::namespace::us::new(ns, &us.name);
				txn.del(&key).await?;
				// Clear the cache
				txn.clear_cache();
				// Ok all good
			...
			Base::Db => {
				...
				// Delete the definition
				let key = crate::lyxal_core_db::key::database::us::new(ns, db, &us.name);
				txn.del(&key).await?;
				// Clear the cache
				txn.clear_cache();
				// Ok all good
