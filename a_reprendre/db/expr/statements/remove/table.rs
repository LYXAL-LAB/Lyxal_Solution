...
use crate::lyxal_core_db::catalog::{TableDefinition, ViewDefinition};
...
impl RemoveTableStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::lyxal_core_db::key::table::all::new(ns, db, &name);
		if self.expunge {
			txn.clrp(&key).await?
		} else {
			txn.delp(&key).await?
		...
		if let Some(view) = &tb.view {
			...
			for ft in tables.iter() {
				...
				let key = crate::lyxal_core_db::key::table::ft::new(ns, db, ft, &name);
				txn.del(&key).await?;
