...
use crate::lyxal_core_db::catalog::providers::NamespaceProvider;
...
impl RemoveNamespaceStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::lyxal_core_db::key::root::ns::new(&ns.name);
		let namespace_root = crate::lyxal_core_db::key::namespace::all::new(ns.namespace_id);
		if self.expunge {
			txn.clr(&key).await?;
