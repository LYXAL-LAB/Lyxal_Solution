...
impl RemoveEventStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::lyxal_core_db::key::table::ev::new(ns, db, &ev.target_table, &ev.name);
		txn.del(&key).await?;
...
impl ToSql for RemoveEventStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::remove::RemoveEventStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
