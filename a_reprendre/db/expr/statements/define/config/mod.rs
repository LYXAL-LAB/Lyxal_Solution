...
use crate::lyxal_core_db::catalog::providers::{DatabaseProvider, RootProvider};
use crate::lyxal_core_db::catalog::{ConfigDefinition, GraphQLConfig};
...
use crate::lyxal_core_db::expr::Value;
use crate::lyxal_core_db::expr::statements::define::DefineKind;
use crate::lyxal_core_db::iam::{Action, ConfigKind, ResourceKind};
use crate::fail;
	...
	pub(crate) async fn compute(
		...
		match base {
			Base::Root => {
				...
				// Compute the config
				let key = crate::lyxal_core_db::key::root::root_config::new(cg);
			...
			Base::Db => {
				...
				// Compute the config
				let key = crate::lyxal_core_db::key::database::cg::new(ns, db, cg);
				let store = self.inner.compute(stk, ctx, opt, doc).await?;
				// Put the config
				txn.replace(&key, &store).await?;
				// Clear the cache
