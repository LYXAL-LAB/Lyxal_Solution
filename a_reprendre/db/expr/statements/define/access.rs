...
use crate::lyxal_core_db::catalog::providers::{AuthorisationProvider, NamespaceProvider};
use crate::lyxal_core_db::catalog::{self, AccessDefinition};
...
use crate::lyxal_core_db::expr::access::AccessDuration;
...
use crate::lyxal_core_db::val::{self, Duration, Value};
use crate::map_opt;
	...
	pub(crate) async fn compute(
		...
		match &self.base {
			Base::Root => {
				...
				let key = crate::lyxal_core_db::key::root::ac::new(&definition.name);
			...
			Base::Ns => {
				...
				// Process the statement
				let key = crate::lyxal_core_db::key::namespace::ac::new(ns, &definition.name);
			...
			Base::Db => {
				...
				// Process the statement
				let key = crate::lyxal_core_db::key::database::ac::new(ns, db, &definition.name);
				txn.set(&key, &definition, None).await?;
				// Clear the cache
				txn.clear_cache();
				// Ok all good
...
impl ToSql for DefineAccessStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::define::DefineAccessStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
