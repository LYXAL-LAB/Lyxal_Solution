...
use crate::lyxal_core_db::catalog::{MlModelDefinition, Permission};
...
use crate::lyxal_core_db::expr::{Base, Expr, FlowResultExt};
...
impl DefineModelStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::lyxal_core_db::key::database::ml::new(ns, db, &self.name, &self.version);
		txn.set(
			&key,
			&MlModelDefinition {
...
impl ToSql for DefineModelStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::define::DefineModelStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
