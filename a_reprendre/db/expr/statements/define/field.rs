...
use crate::lyxal_core_db::expr::parameterize::{expr_to_ident, expr_to_idiom};
use crate::lyxal_core_db::expr::reference::Reference;
use crate::lyxal_core_db::expr::{
	Base, Expr, FlowResultExt, Idiom, Kind, KindLiteral, Literal, Part, RecordIdKeyLit,
};
use crate::lyxal_core_db::iam::{Action, AuthLimit, ResourceKind};
...
impl DefineFieldStatement {
	pub(crate) async fn to_definition(
		...
		let computed_deps = self.computed.as_ref().map(|expr| {
			let deps = crate::lyxal_core_db::expr::computed_deps::extract_computed_deps(expr);
	...
	pub(crate) async fn process_recursive_definitions(
		...
		if let Some(mut cur_kind) = self.field_kind.as_ref().and_then(|x| x.inner_kind()) {
			...
			loop {
				...
				let key = crate::lyxal_core_db::key::table::fd::new(ns, db, &definition.table, &fd);
	...
	pub(crate) async fn validate_computed_cycles(
		...
		for fd in fields.iter() {
			...
			let deps = if let Some(ref cd) = fd.computed_deps {
				...
				crate::lyxal_core_db::expr::computed_deps::extract_computed_deps(expr).fields
			} else {
				Vec::new()
			};
