...
use crate::lyxal_core_db::catalog::aggregation::{
...
use crate::lyxal_core_db::doc::{self, CursorDoc, Document, DocumentContext, NsDbTbCtx};
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::changefeed::ChangeFeed;
...
use crate::lyxal_core_db::key;
use crate::lyxal_core_kvs::Transaction;
use crate::lyxal_core_db::val::{Array, Number, RecordId, RecordIdKey, TableName, Value};
use crate::fail;
use crate::yield_now;
...
impl DefineTableStatement {
	...
	pub(crate) async fn compute(
		...
		if let Some(view) = &tb_def.view {
			...
			let key = crate::lyxal_core_db::key::table::all::new(ns.namespace_id, db.database_id, &name);
			txn.delp(&key).await?;
			...
			for ft in tables.iter() {
				...
				let key = crate::lyxal_core_db::key::table::ft::new(ns.namespace_id, db.database_id, ft, &name);
	...
	pub async fn add_in_out_fields(
		...
		if let TableType::Relation(rel) = &tb.table_type {
			...
			{
				let key = crate::lyxal_core_db::key::table::fd::new(ns, db, &tb.name, "in");
				let val =
			...
			{
				let key = crate::lyxal_core_db::key::table::fd::new(ns, db, &tb.name, "out");
