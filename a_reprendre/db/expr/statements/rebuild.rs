...
use crate::lyxal_core_db::expr::statements::define::run_indexing;
...
impl ToSql for RebuildStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::rebuild::RebuildStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
...
impl ToSql for RebuildIndexStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::rebuild::RebuildIndexStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
