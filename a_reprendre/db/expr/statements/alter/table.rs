...
use crate::lyxal_core_db::catalog::{Permissions, TableType};
use crate::lyxal_core_db::ctx::FrozenContext;
use crate::lyxal_core_db::dbs::Options;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::statements::DefineTableStatement;
use crate::lyxal_core_db::expr::{Base, ChangeFeed};
...
impl AlterTableStatement {
	...
	pub(crate) async fn compute(&self, ctx: &FrozenContext, opt: &Options) -> Result<Value> {
		...
		if self.compact {
			let key = crate::lyxal_core_db::key::table::all::new(ns, db, &self.name);
...
impl ToSql for AlterTableStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::statements::alter::AlterTableStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
