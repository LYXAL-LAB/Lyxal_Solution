...
use crate::lyxal_core_db::catalog::{NodeLiveQuery, SubscriptionDefinition, SubscriptionFields};
use crate::lyxal_core_db::ctx::FrozenContext;
use crate::lyxal_core_db::dbs::{Options, ParameterCapturePass, Variables};
use crate::lyxal_core_db::doc::CursorDoc;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::visit::Visit;
use crate::lyxal_core_db::expr::{Cond, Expr, Fetchs, Fields, FlowResultExt as _};
	...
	pub(crate) async fn compute(
			...
			.catch_return()?
		{
			Value::Table(tb) => {
				...
				let key = crate::lyxal_core_db::key::node::lq::new(nid, live_query_id);
				txn.replace(
					&key,
					&NodeLiveQuery {
						ns,
				...
				let key = crate::lyxal_core_db::key::table::lq::new(ns, db, &tb, live_query_id);
		...
		// Return the query id
		Ok(crate::lyxal_core_db::val::Uuid(live_query_id).into())
	}
}
...
mod tests {
	...
	use crate::lyxal_core_db::catalog::providers::{CatalogProvider, TableProvider};
	use crate::lyxal_core_db::dbs::{Capabilities, Session};
	use crate::lyxal_core_kvs::Datastore;
	use crate::lyxal_core_kvs::LockType::Optimistic;
	use crate::lyxal_core_kvs::TransactionType::Write;
