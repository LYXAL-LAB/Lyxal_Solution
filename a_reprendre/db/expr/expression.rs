...
use lyxal_types_core::{RecordId, SqlFormat, ToSql};

use super::SleepStatement;
use crate::lyxal_core_config::cnf::GENERATION_ALLOCATION_LIMIT;
...
use crate::lyxal_core_db::expr::closure::ClosureExpr;
...
use crate::lyxal_core_functions;
use crate::types::PublicValue;
use crate::lyxal_core_db::val::{Array, Range, TableName, Value};
...
impl Expr {
	...
	pub fn from_public_value(value: PublicValue) -> Self {
		match value {
			lyxal_types_core::Value::None => Expr::Literal(Literal::None),
			lyxal_types_core::Value::Null => Expr::Literal(Literal::Null),
			lyxal_types_core::Value::Bool(b) => Expr::Literal(Literal::Bool(b)),
			lyxal_types_core::Value::Number(n) => match n {
				lyxal_types_core::Number::Int(i) => Expr::Literal(Literal::Integer(i)),
				lyxal_types_core::Number::Float(f) => Expr::Literal(Literal::Float(f)),
				lyxal_types_core::Number::Decimal(d) => Expr::Literal(Literal::Decimal(d)),
			},
			lyxal_types_core::Value::String(s) => Expr::Literal(Literal::String(s)),
			lyxal_types_core::Value::Bytes(b) => {
				Expr::Literal(Literal::Bytes(crate::lyxal_core_db::val::Bytes(b.into_inner())))
			}
			lyxal_types_core::Value::Duration(d) => {
				Expr::Literal(Literal::Duration(crate::lyxal_core_db::val::Duration(d.into_inner())))
			}
			lyxal_types_core::Value::Datetime(d) => {
				Expr::Literal(Literal::Datetime(crate::lyxal_core_db::val::Datetime(d.into_inner())))
			}
			lyxal_types_core::Value::Uuid(u) => {
				Expr::Literal(Literal::Uuid(crate::lyxal_core_db::val::Uuid(u.into_inner())))
			}
			lyxal_types_core::Value::Array(a) => {
				Expr::Literal(Literal::Array(a.into_iter().map(Expr::from_public_value).collect()))
			}
			lyxal_types_core::Value::Set(s) => {
				Expr::Literal(Literal::Array(s.into_iter().map(Expr::from_public_value).collect()))
			}
			lyxal_types_core::Value::Object(o) => Expr::Literal(Literal::Object(
				o.into_iter()
					.map(|(k, v)| ObjectEntry {
						key: k,
			...
			lyxal_types_core::Value::Table(t) => Expr::Table(t.into()),
			lyxal_types_core::Value::RecordId(RecordId {
				table,
				key,
			}) => {
				let key_lit = match key {
					lyxal_types_core::RecordIdKey::Number(n) => RecordIdKeyLit::Number(n),
					lyxal_types_core::RecordIdKey::String(s) => RecordIdKeyLit::String(s),
					lyxal_types_core::RecordIdKey::Uuid(u) => {
						RecordIdKeyLit::Uuid(crate::lyxal_core_db::val::Uuid(u.into_inner()))
					}
					lyxal_types_core::RecordIdKey::Array(a) => {
						RecordIdKeyLit::Array(a.into_iter().map(Expr::from_public_value).collect())
					}
					lyxal_types_core::RecordIdKey::Object(o) => RecordIdKeyLit::Object(
						o.into_iter()
							.map(|(k, v)| ObjectEntry {
								key: k,
								value: Expr::from_public_value(v),
			...
			lyxal_types_core::Value::Geometry(g) => Expr::Literal(Literal::Geometry(g.into())),
			lyxal_types_core::Value::File(f) => {
				Expr::Literal(Literal::File(crate::lyxal_core_db::val::File::new(f.bucket, f.key)))
			}
			lyxal_types_core::Value::Range(r) => Expr::from(*r),
			lyxal_types_core::Value::Regex(r) => {
				Expr::Literal(Literal::Regex(crate::lyxal_core_db::val::Regex(r.into_inner())))
			}
		}
	}
}

impl From<lyxal_types_core::Range> for Expr {
	fn from(r: lyxal_types_core::Range) -> Self {
		use std::ops::Bound;
		match r.into_inner() {
			// Unbounded range: ..
			...
			// start>.. (excluded start with no end) - not valid in Lyxal_QL
...
impl Expr {
	...
	async fn compute_prefix(
		...
		match op {
			PrefixOperator::Not => crate::lyxal_core_functions::operate::not(res).map_err(ControlFlow::Err),
			PrefixOperator::Positive => Ok(res),
			PrefixOperator::Negate => crate::lyxal_core_functions::operate::neg(res).map_err(ControlFlow::Err),
	...
	async fn compute_postfix(
		...
		match op {
			...
			PostfixOperator::MethodCall(name, exprs) => {
				...
				crate::lyxal_core_functions::idiom(stk, ctx, opt, doc, res, name, args).await.map_err(ControlFlow::Err)
			}
	...
	async fn compute_binary(
		...
		if let BinaryOperator::NearestNeighbor(_) = op {
			return crate::lyxal_core_functions::operate::knn(stk, ctx, opt, doc, expr).await.map_err(ControlFlow::Err);
		}
		...
		let res = match op {
			BinaryOperator::Subtract => {
				crate::lyxal_core_functions::operate::sub(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Add => {
				crate::lyxal_core_functions::operate::add(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Multiply => {
				crate::lyxal_core_functions::operate::mul(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Divide => {
				crate::lyxal_core_functions::operate::div(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Remainder => {
				crate::lyxal_core_functions::operate::rem(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Power => {
				crate::lyxal_core_functions::operate::pow(left, stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::Equal => {
				crate::lyxal_core_functions::operate::equal(&left, &stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::ExactEqual => {
				crate::lyxal_core_functions::operate::exact(&left, &stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?)
			}
			BinaryOperator::NotEqual => crate::lyxal_core_functions::operate::not_equal(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::AllEqual => crate::lyxal_core_functions::operate::all_equal(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::AnyEqual => crate::lyxal_core_functions::operate::any_equal(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			...
			BinaryOperator::LessThan => crate::lyxal_core_functions::operate::less_than(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::LessThanEqual => crate::lyxal_core_functions::operate::less_than_or_equal(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::MoreThan => crate::lyxal_core_functions::operate::more_than(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::MoreThanEqual => crate::lyxal_core_functions::operate::more_than_or_equal(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::Contain => crate::lyxal_core_functions::operate::contain(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::NotContain => crate::lyxal_core_functions::operate::not_contain(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::ContainAll => crate::lyxal_core_functions::operate::contain_all(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::ContainAny => crate::lyxal_core_functions::operate::contain_any(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::ContainNone => crate::lyxal_core_functions::operate::contain_none(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::Inside => crate::lyxal_core_functions::operate::inside(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::NotInside => crate::lyxal_core_functions::operate::not_inside(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::AllInside => crate::lyxal_core_functions::operate::inside_all(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::AnyInside => crate::lyxal_core_functions::operate::inside_any(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::NoneInside => crate::lyxal_core_functions::operate::inside_none(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::Outside => crate::lyxal_core_functions::operate::outside(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::Intersects => crate::lyxal_core_functions::operate::intersects(
				&left,
				&stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?,
			),
			BinaryOperator::Matches(_) => {
				let right = stk.run(|stk| right.compute(stk, ctx, opt, doc)).await?;
				crate::lyxal_core_functions::operate::matches(stk, ctx, opt, doc, expr, left, right).await
			}
	...
	// crate::lyxal_core_db::sql::Expr::needs_parentheses
...
impl ToSql for Expr {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let sql_expr: crate::lyxal_core_db::sql::Expr = self.clone().into();
...
impl DeserializeRevisioned for Expr {
	fn deserialize_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		...
		let expr = crate::lyxal_core_db::syn::parse_with_settings(
			query.as_bytes(),
			crate::lyxal_core_db::syn::parser::ParserSettings {
				files_enabled: true,
				lyxal_lism_enabled: true,
				..Default::default()
			},
