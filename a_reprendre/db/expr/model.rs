...
#[cfg(feature = "ml")]
use lyxal_ml_core::errors::error::LyxalError;
#[cfg(feature = "ml")]
use lyxal_ml_core::execution::compute::ModelComputation;
#[cfg(feature = "ml")]
use lyxal_ml_core::ndarray as mlNdarray;
#[cfg(feature = "ml")]
use lyxal_ml_core::storage::surml_file::SurMlFile;
...
use crate::lyxal_core_db::expr::{ControlFlow, FlowResult};
#[cfg(feature = "ml")]
...
impl ToSql for Model {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let stmt: crate::lyxal_core_db::sql::model::Model = self.clone().into();
...
impl Model {
	...
	pub(crate) async fn compute(
		...
		use crate::lyxal_core_db::val::{CoerceError, Number};
		...
		match argument {
			...
			Value::Object(v) => {
				...
				let bytes = crate::lyxal_core_telemetry::obs::get(&path).await?;
			...
			Value::Number(v) => {
				...
				// Get the model file as bytes
				let bytes = crate::lyxal_core_telemetry::obs::get(&path).await.map_err(ControlFlow::from)?;
