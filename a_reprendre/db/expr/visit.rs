...
use crate::lyxal_core_db::catalog::{GraphQLConfig, Permission, Permissions, Relation, TableType};
use crate::lyxal_core_db::expr::access_type::{BearerAccess, JwtAccessVerify};
use crate::lyxal_core_db::expr::data::Assignment;
use crate::lyxal_core_db::expr::field::Selector;
use crate::lyxal_core_db::expr::lookup::LookupSubject;
use crate::lyxal_core_db::expr::order::Ordering;
use crate::lyxal_core_db::expr::part::{DestructurePart, Recurse, RecurseInstruction};
use crate::lyxal_core_db::expr::reference::{Reference, ReferenceDeleteStrategy};
...
use crate::lyxal_core_db::expr::statements::alter::{
...
use crate::lyxal_core_db::expr::statements::define::config::ConfigInner;
use crate::lyxal_core_db::expr::statements::define::config::api::ApiConfig;
use crate::lyxal_core_db::expr::statements::define::config::defaults::DefaultConfig;
use crate::lyxal_core_db::expr::statements::define::{
	ApiAction, DefineBucketStatement, DefineConfigStatement, DefineDefault, DefineSequenceStatement,
};
use crate::lyxal_core_db::expr::statements::rebuild::RebuildStatement;
use crate::lyxal_core_db::expr::statements::remove::{
...
use crate::lyxal_core_db::expr::statements::{
	...
	UseStatement,DefineConnectorStatement,RemoveConnectorStatement,
};
...
implement_visitor! {
	...
	fn visit_define_connector(this, d: &DefineConnectorStatement) {
		this.visit_expr(&d.base_url)?;
		if let Some(h) = d.headers.as_ref() {
			this.visit_expr(h)?;
		}
		for ep in d.endpoints.iter() {
			this.visit_expr(&ep.path)?;
		}
		if let Some(comment) = d.comment.as_ref() {
			this.visit_expr(comment)?;
		}
		Ok(())
	}
	...
	fn visit_remove(this, r: &RemoveStatement){
		match r {
			...
			RemoveStatement::Connector(r) => {
				this.visit_remove_connector(r)?;
			},
	...
	fn visit_remove_connector(this, r: &RemoveConnectorStatement) {
...
implement_visitor_mut! {
	...
	fn visit_mut_remove(this, r: &mut RemoveStatement){
		match r {
			...
			RemoveStatement::Connector(r) => {
				this.visit_mut_remove_connector(r)?;
			},
			RemoveStatement::Database(r) => {
	...
	fn visit_mut_remove_connector(this, r: &mut RemoveConnectorStatement){
		this.visit_mut_expr(&mut r.name)?;
		Ok(())
	}
	...
	fn visit_mut_define(this, d: &mut DefineStatement){
		match d {
			...
			DefineStatement::Connector(d) => {
				this.visit_mut_define_connector(d)?;
			},
			DefineStatement::Database(d) => {
	...
	fn visit_mut_define_connector(this, d: &mut DefineConnectorStatement) {
    	this.visit_mut_expr(&mut d.base_url)?;
    	if let Some(h) = d.headers.as_mut() {
        	this.visit_mut_expr(h)?;
		}
		for ep in d.endpoints.iter_mut() {
			this.visit_mut_expr(&mut ep.path)?;
		}
		if let Some(comment) = d.comment.as_mut() {
			this.visit_mut_expr(comment)?;
		}
		Ok(())
	}
