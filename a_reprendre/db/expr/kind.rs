...
use crate::lyxal_core_db::expr::{Expr, Literal, Part, Value};
...
impl HasKind for crate::lyxal_core_db::val::Object {
	fn kind() -> Kind {
		Kind::Object
	}
}
...
impl ToSql for Kind {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let kind: crate::lyxal_core_db::sql::Kind = self.clone().into();
		kind.fmt_sql(f, fmt);
	}
}
	...
	fn from(v: crate::types::PublicKindLiteral) -> Self {
		match v {
			...
			crate::types::PublicKindLiteral::Duration(d) => {
				KindLiteral::Duration(crate::lyxal_core_db::val::Duration(*d))
			}
...
impl ToSql for KindLiteral {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let lit: crate::lyxal_core_db::sql::kind::KindLiteral = self.clone().into();
		lit.fmt_sql(f, fmt)
	}
}
