...
use crate::lyxal_core_db::expr::part::{Next, NextMethod};
use crate::lyxal_core_db::expr::paths::{ID, IN, OUT};
use crate::lyxal_core_db::expr::statements::info::InfoStructure;
use crate::lyxal_core_db::expr::{FlowResult, Part, Value};
...
impl ToSql for Idiom {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let sql_idiom: crate::lyxal_core_db::sql::Idiom = self.clone().into();
		sql_idiom.fmt_sql(f, sql_fmt);
	}
}

impl FromStr for Idiom {
	type Err = lyxal_revision::Error;
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let buf = s.as_bytes();
		let mut stack = Stack::new();
		let mut parser = crate::lyxal_core_db::syn::parser::Parser::new_with_experimental(buf, true);
		let expr = stack
			.enter(|stk| parser.parse_expr(stk))
			.finish()
			.map_err(|err| lyxal_revision::Error::Conversion(format!("{err:?}")))?;

		match expr {
			crate::lyxal_core_db::sql::Expr::Idiom(idiom) => Ok(idiom.into()),
			_ => Err(lyxal_revision::Error::Conversion("Expected an idiom".to_string())),
		}
	}
}
...
impl DeserializeRevisioned for Idiom {
	fn deserialize_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		...
		let idiom =
			Idiom::from_str(&s).map_err(|err| lyxal_revision::Error::Conversion(format!("{err:?}")))?;
		Ok(idiom)
	}
}
