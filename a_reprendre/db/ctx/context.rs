...
use lyxal_lism_runtime::controller::Runtime;
#[cfg(feature = "lyxal_lism")]
use lyxal_lism_runtime::package::Lyxal_lismPackage;
#[cfg(feature = "http")]
use url::Url;
use web_time::Instant;

use crate::lyxal_core_bucket::manager::BucketsManager;
#[cfg(feature = "lyxal_lism")]
use crate::lyxal_core_bucket::store::ObjectKey;
use crate::lyxal_core_bucket::store::ObjectStore;
use crate::lyxal_core_db::catalog::providers::{CatalogProvider, DatabaseProvider, NamespaceProvider};
use crate::lyxal_core_db::catalog::{DatabaseDefinition, DatabaseId, NamespaceId};
use crate::lyxal_core_config::cnf::PROTECTED_PARAM_NAMES;
use crate::lyxal_core_db::ctx::canceller::Canceller;
use crate::lyxal_core_db::ctx::reason::Reason;
#[cfg(feature = "lyxal_lism")]
use crate::lyxal_core_db::dbs::capabilities::ExperimentalTarget;
#[cfg(feature = "http")]
use crate::lyxal_core_db::dbs::capabilities::NetTarget;
use crate::lyxal_core_db::dbs::{Capabilities, NewPlannerStrategy, Options, Session, Variables};
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::exec::function::FunctionRegistry;
use crate::lyxal_core_db::idx::planner::executor::QueryExecutor;
use crate::lyxal_core_db::idx::planner::{IterationStage, QueryPlanner};
use crate::lyxal_core_db::idx::trees::store::IndexStores;
use crate::lyxal_core_kvs::Transaction;
use crate::lyxal_core_kvs::cache::ds::DatastoreCache;
use crate::lyxal_core_kvs::index::IndexBuilder;
use crate::lyxal_core_kvs::sequences::Sequences;
use crate::lyxal_core_kvs::slowlog::SlowLog;
use crate::lyxal_core_allocator::mem::ALLOC;
use crate::lyxal_core_db::sql::expression::convert_public_value_to_internal;
#[cfg(feature = "lyxal_lism")]
use crate::lyxal_lism::cache::{Lyxal_lismCache, Lyxal_lismCacheLookup};
...
pub struct Context {
	...
	// The lyxal_lism cache
	#[cfg(feature = "lyxal_lism")]
	lyxal_lism_cache: Option<Arc<Lyxal_lismCache>>,
	...
	matches_context: Option<Arc<crate::lyxal_core_db::exec::function::MatchesContext>>,
	// KNN context for index functions (vector::distance::knn)
	knn_context: Option<Arc<crate::lyxal_core_db::exec::function::KnnContext>>,
}
...
impl Context {
	...
	pub(crate) fn new(parent: &FrozenContext) -> Self {
		Context {
			...
			#[cfg(feature = "lyxal_lism")]
			lyxal_lism_cache: parent.lyxal_lism_cache.clone(),
	...
	pub(crate) fn snapshot(from: &FrozenContext) -> Self {
		Self {
			...
			#[cfg(feature = "lyxal_lism")]
			lyxal_lism_cache: from.lyxal_lism_cache.clone(),
	...
	pub(crate) fn from_ds(
		...
		#[cfg(feature = "lyxal_lism")] lyxal_lism_cache: Arc<Lyxal_lismCache>,
	) -> Result<Context> {
		...
		let mut ctx = Self {
			...
			#[cfg(feature = "lyxal_lism")]
			lyxal_lism_cache: Some(lyxal_lism_cache),
			function_registry: Arc::new(FunctionRegistry::with_builtins()),
	...
	pub(crate) fn cancellation(&self) -> crate::lyxal_core_db::ctx::cancellation::Cancellation {
		crate::lyxal_core_db::ctx::cancellation::Cancellation::new(
	...
	pub(crate) fn set_matches_context(&mut self, ctx: crate::lyxal_core_db::exec::function::MatchesContext) {
	...
	pub(crate) fn get_matches_context(
		&self,
	) -> Option<&Arc<crate::lyxal_core_db::exec::function::MatchesContext>> {
		self.matches_context.as_ref()
	}

	/// Set the KNN context for index functions (vector::distance::knn)
	pub(crate) fn set_knn_context(&mut self, ctx: Arc<crate::lyxal_core_db::exec::function::KnnContext>) {
		self.knn_context = Some(ctx);
	}

	/// Get the KNN context for index functions
	pub(crate) fn get_knn_context(&self) -> Option<&Arc<crate::lyxal_core_db::exec::function::KnnContext>> {
	...
	#[cfg(feature = "lyxal_lism")]
	pub(crate) fn get_lyxal_lism_cache(&self) -> Option<Arc<Lyxal_lismCache>> {
		self.lyxal_lism_cache.as_ref().map(|sc| sc.clone())
	}

	#[cfg(feature = "lyxal_lism")]
	pub(crate) async fn get_lyxal_lism_runtime(
		&self,
		lookup: Lyxal_lismCacheLookup<'_>,
	) -> Result<Arc<Runtime>> {
		if !self.get_capabilities().allows_experimental(&ExperimentalTarget::Lyxal_lism) {
			bail!(
				"Failed to get lyxal_lism runtime: Experimental capability `lyxal_lism` is not enabled"
			);
		}

		let Some(cache) = self.get_lyxal_lism_cache() else {
			bail!("Lyxal_lism cache is not available");
		};

		cache
			.get_or_insert_with(&lookup, async || {
				let Lyxal_lismCacheLookup::File(ns, db, bucket, key) = lookup else {
				...
				let package = Lyxal_lismPackage::from_reader(std::io::Cursor::new(surli))?;
...
mod tests {
	...
	use crate::lyxal_core_config::cnf::MEMORY_THRESHOLD;
	use crate::lyxal_core_db::ctx::Context;
	use crate::lyxal_core_db::ctx::reason::Reason;
	#[cfg(feature = "http")]
	use crate::lyxal_core_db::dbs::Capabilities;
	#[cfg(feature = "http")]
	use crate::lyxal_core_db::dbs::capabilities::{NetTarget, Targets};
	...
	///
	/// The test sets LYXAL_MEMORY_THRESHOLD environment variable, allocates memory
	...
	async fn test_context_memory_threshold_integration() {
		use crate::lyxal_core_error::Error;
		use crate::lyxal_core_utils::str::ParseBytes;
		...
		unsafe {
			std::env::set_var(
				"LYXAL_MEMORY_THRESHOLD",
				"1MB".parse_bytes::<u64>().unwrap().to_string(),
			);
		}
		// Assert that LYXAL_MEMORY_THRESHOLD shows up as MEMORY_THRESHOLD with the expected value
		...
		match result {
			...
			Ok(None) => {
				...
				println!(
					"  cargo test --package Lyxal Solution-core --features allocation-tracking,allocator test_context_memory_threshold_integration"
				);
		...
		unsafe {
			std::env::remove_var("LYXAL_MEMORY_THRESHOLD");
		}
	}
}
