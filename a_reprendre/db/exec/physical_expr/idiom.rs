...
use crate::lyxal_core_db::exec::{AccessMode, CombineAccessModes, ContextLevel, ExecOperator};
...
impl PhysicalExpr for IdiomExpr {
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> crate::lyxal_core_db::expr::FlowResult<Value> {
		...
		let value = if let Some(ref start) = self.start_expr {
			...
			// This matches legacy Lyxal_QL behavior where undefined identifiers
...
///
/// In Lyxal_QL, certain operations like field access, `.*`, and destructuring
...
/// over each element of the array. This implements Lyxal_QL's idiom continuity
