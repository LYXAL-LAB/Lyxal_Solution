...
use crate::lyxal_core_db::exec::plan_or_compute::{block_required_context, legacy_compute};
use crate::lyxal_core_db::exec::planner::expr_to_physical_expr;
use crate::lyxal_core_db::expr::{Block, ControlFlow, Expr, FlowResult};
...
fn create_planning_context(
	exec_ctx: &crate::lyxal_core_db::exec::ExecutionContext,
...
fn get_legacy_context<'a>(
	exec_ctx: &'a crate::lyxal_core_db::exec::ExecutionContext,
	cached_ctx: &mut Option<FrozenContext>,
) -> anyhow::Result<(&'a crate::lyxal_core_db::dbs::Options, FrozenContext)> {
...
impl BlockPhysicalExpr {
	...
	async fn evaluate_block_entry(
		...
		current_exec_ctx: &mut crate::lyxal_core_db::exec::ExecutionContext,
			...
			Expr::Let(set_stmt) => {
				...
				*current_exec_ctx =
				...
				if let Some(ctx) = legacy_ctx {
					let mut new_ctx = crate::lyxal_core_db::ctx::Context::new(ctx);
