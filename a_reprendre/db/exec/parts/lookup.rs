...
use crate::lyxal_core_db::exec::{AccessMode, ContextLevel, ExecOperator};
...
impl From<crate::lyxal_core_db::expr::Dir> for LookupDirection {
	fn from(dir: crate::lyxal_core_db::expr::Dir) -> Self {
		match dir {
			crate::lyxal_core_db::expr::Dir::Out => LookupDirection::Out,
			crate::lyxal_core_db::expr::Dir::In => LookupDirection::In,
			crate::lyxal_core_db::expr::Dir::Both => LookupDirection::Both,
		}
	}
}
...
async fn evaluate_lookup(
	...
	match value {
		...
		Value::Array(arr) => {
			...
			// This matches Lyxal Solution semantics: `->edge` on an array of records
...
async fn evaluate_lookup_for_value(
	...
	let stream = lookup.plan.execute(&bound_ctx).map_err(|e| match e {
		crate::lyxal_core_db::expr::ControlFlow::Err(e) => e,
		crate::lyxal_core_db::expr::ControlFlow::Return(v) => {
			anyhow::anyhow!("Unexpected return in lookup: {:?}", v)
		}
		crate::lyxal_core_db::expr::ControlFlow::Break => anyhow::anyhow!("Unexpected break in lookup"),
		crate::lyxal_core_db::expr::ControlFlow::Continue => anyhow::anyhow!("Unexpected continue in lookup"),
	})?;
	...
	while let Some(batch_result) = stream.next().await {
		let batch = batch_result.map_err(|e| match e {
			...
			crate::lyxal_core_db::expr::ControlFlow::Break => anyhow::anyhow!("Unexpected break in lookup"),
			crate::lyxal_core_db::expr::ControlFlow::Continue => {
