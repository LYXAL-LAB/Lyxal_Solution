...
use crate::lyxal_core_db::exec::function::MethodDescriptor;
use crate::lyxal_core_db::exec::physical_expr::function::validate_return;
use crate::lyxal_core_db::exec::physical_expr::{BlockPhysicalExpr, EvalContext, PhysicalExpr};
use crate::lyxal_core_db::exec::{AccessMode, CombineAccessModes, ContextLevel};
...
impl PhysicalExpr for MethodPart {
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		...
		match result {
			Ok(v) => Ok(v),
			Err(e) => {
				if let Some(crate::lyxal_core_error::Error::InvalidFunctionArguments {
					message,
					..
				}) = e.downcast_ref::<crate::lyxal_core_error::Error>()
				{
					Err(crate::lyxal_core_error::Error::InvalidMethodArguments {
		...
		match closure.as_ref() {
			Closure::Expr {
				...
				let block_expr = BlockPhysicalExpr {
					block: crate::lyxal_core_db::expr::Block(vec![body.clone()]),
				};
				...
				let result = match block_expr.evaluate(eval_ctx).await {
					Ok(v) => v,
					Err(crate::lyxal_core_db::expr::ControlFlow::Return(v)) => v,
