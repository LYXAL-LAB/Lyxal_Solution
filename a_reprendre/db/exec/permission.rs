...
//! at execution time. Since Lyxal_QL allows DDL and DML interleaving within transactions,
...
use crate::lyxal_core_db::exec::{DatabaseContext, EvalContext, ExecutionContext, PhysicalExpr};
use crate::lyxal_core_db::iam::{Action, Auth};
...
pub async fn convert_permission_to_physical(
	...
	match permission {
		...
		Permission::Specific(expr) => {
			...
			let physical_expr =
				crate::lyxal_core_db::exec::planner::expr_to_physical_expr(expr.clone(), ctx).await?;
