...
use crate::lyxal_core_db::exec::parts::is_final;
use crate::lyxal_core_db::exec::{BoxFut, ExecOperator, ExecutionContext, FlowResult};
...
pub(crate) fn collect_discovery_targets(
	v: Value,
	out: &mut Vec<Value>,
) -> crate::lyxal_core_db::exec::FlowResult<()> {
	match v {
		Value::Array(arr) => {
			for inner in arr.0 {
				if is_final(&inner) {
