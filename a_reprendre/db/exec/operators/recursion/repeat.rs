...
//! ```
//!
//! Example Lyxal_QL (repeat recurse builds a nested tree per record):
//!
//! ```surql
...
use crate::lyxal_core_db::exec::parts::recurse::value_hash;
use crate::lyxal_core_db::exec::parts::{clean_iteration, evaluate_physical_path, get_final, is_final};
use crate::lyxal_core_db::exec::physical_expr::{EvalContext, PhysicalExpr, RecursionCtx};
use crate::lyxal_core_db::exec::{ExecutionContext, FlowResult};
...
pub(crate) fn evaluate_repeat_recurse<'a>(
	value: &'a Value,
	ctx: EvalContext<'a>,
) -> crate::lyxal_core_db::exec::BoxFut<'a, FlowResult<Value>> {
	Box::pin(async move {
		let rec_ctx = match &ctx.recursion_ctx {
			Some(rc) => rc.clone(),
			None => {
				// RepeatRecurse outside recursion context is an error
				return Err(crate::lyxal_core_error::Error::UnsupportedRepeatRecurse.into());
			}
		};
				...
				Value::Array(arr) => {
					...
					for v in arr.iter() {
						...
						if !is_recursion_target(v) {
							return Err(crate::lyxal_core_error::Error::InvalidRecursionTarget {
								value: v.to_sql(),
							}
							.into());
						}
		...
		Err(crate::lyxal_core_error::Error::UnsupportedRepeatRecurse.into())
	})
}
...
pub(crate) async fn evaluate_recurse_iterative(
	...
	min_depth: u32,
	max_depth: u32,
	body: &Option<Arc<dyn crate::lyxal_core_db::exec::ExecOperator>>,
