...
//! ```
//!
//! Example Lyxal_QL (default instruction = no `+collect` / `+path` / `+shortest`):
//!
//! ```surql
...
use crate::lyxal_core_db::exec::FlowResult;
...
pub(crate) async fn evaluate_recurse_default(
	...
	if !user_specified_max && depth >= system_limit {
		return Err(crate::lyxal_core_error::Error::IdiomRecursionLimitExceeded {
			limit: system_limit,
		}
		.into());
	}
