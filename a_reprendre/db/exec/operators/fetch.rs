...
use crate::lyxal_core_db::expr::ControlFlowExt;
...
fn fetch_field_path<'a>(
	...
	value: &'a mut Value,
	path: &'a [Part],
) -> crate::lyxal_core_db::exec::BoxFut<'a, crate::lyxal_core_db::expr::FlowResult<()>> {
	Box::pin(async move {
...
) -> crate::lyxal_core_db::expr::FlowResult<()> {
	for item in values {
...
pub(crate) async fn fetch_raw_record(
	...
	rid: &RecordId,
	version: Option<u64>,
) -> crate::lyxal_core_db::expr::FlowResult<Option<Value>> {
...
pub(crate) async fn process_fetched_record(
	...
	rid: &RecordId,
	val: &mut Value,
) -> crate::lyxal_core_db::expr::FlowResult<bool> {
...
pub(crate) async fn batch_fetch_records(
	ctx: &ExecutionContext,
	rids: &[RecordId],
) -> crate::lyxal_core_db::expr::FlowResult<Vec<Value>> {
	if rids.is_empty() {
		return Ok(Vec::new());
	}
...
mod tests {
	use super::*;
	use crate::lyxal_core_db::exec::physical_expr::Literal;

	#[test]
	fn test_fetch_name() {
		use crate::lyxal_core_db::exec::operators::scan::DynamicScan;
		use crate::lyxal_core_db::expr::part::Part;
		...
		let scan = Arc::new(DynamicScan::new(
			Arc::new(Literal(Value::from("test"))) as Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>,
			None,
			None,
			None,
			None,
