...
use lyxal_revision::{DeserializeRevisioned, SerializeRevisioned};
use tempfile::{Builder, TempDir};
use tokio::task::spawn_blocking;

use super::common::{OrderByField, SortDirection, compare_keys};
use crate::lyxal_core_config::cnf::EXTERNAL_SORTING_BUFFER_LIMIT;
...
impl ExecOperator for ExternalSort {
	...
	fn output_ordering(&self) -> crate::lyxal_core_db::exec::OutputOrdering {
		...
		crate::lyxal_core_db::exec::OutputOrdering::Sorted(
			self.order_by
				.iter()
				.map(|f| {
					...
					let path = crate::lyxal_core_db::exec::field_path::FieldPath::field(sql);
					SortProperty {
						path,
		...
		let sorted_stream = futures::stream::once(async move {
			...
			while let Some(batch_result) = input_stream.next().await {
				...
				if ctx.cancellation().is_cancelled() {
					return Err(crate::lyxal_core_db::expr::ControlFlow::Err(anyhow::anyhow!(
