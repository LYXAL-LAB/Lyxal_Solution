...
use crate::lyxal_core_db::exec::permission::{PhysicalPermission, convert_permission_to_physical};
...
pub(crate) async fn resolve_table_context(
	...
	let field_state = build_field_state_raw(txn, ctx, ns_id, db_id, table_name, true)
		.await
		.map_err(|cf| match cf {
			crate::lyxal_core_db::expr::ControlFlow::Err(e) => Error::Internal(e.to_string()),
