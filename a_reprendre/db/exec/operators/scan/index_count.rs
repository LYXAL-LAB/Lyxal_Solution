...
use crate::lyxal_core_db::catalog::{DatabaseId, Index, NamespaceId, Permission};
...
use crate::lyxal_core_kvs::KVValue;
use crate::lyxal_core_db::val::{Number, Object, TableName, Value};
...
async fn sum_index_count_deltas(
	...
	db: DatabaseId,
	tb: &TableName,
	ix: crate::lyxal_core_db::catalog::IndexId,
	...
	let key_stream = txn.stream_keys(
		...
		None, // no limit
		0,    // no skip
		crate::lyxal_core_db::idx::planner::ScanDirection::Forward,
	);
...
async fn count_with_filter_fallback(
	...
	use crate::lyxal_core_db::exec::permission::PhysicalPermission;
	...
	while let Some(result) = kv_stream.next().await {
		...
		for (key, val) in entries {
			let decoded_key = crate::lyxal_core_db::key::record::RecordKey::decode_key(&key)
				.context("Failed to decode record key")?;
			let rid_val = crate::lyxal_core_db::val::RecordId {
				table: decoded_key.tb.into_owned(),
				key: decoded_key.id,
			};
			let mut record = crate::lyxal_core_db::catalog::Record::kv_decode_value(val)
