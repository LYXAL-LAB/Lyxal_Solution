...
use crate::lyxal_core_db::expr::{ControlFlow, Dir};
use crate::lyxal_core_db::idx::planner::ScanDirection;
use crate::lyxal_core_kvs::{CachePolicy, KVKey};
...
async fn compute_graph_ranges(
	...
	if edge_tables.is_empty() {
		...
		let beg = crate::lyxal_core_db::key::graph::egprefix(ns_id, db_id, &rid.table, &rid.key, dir)
			.context("Failed to create graph prefix")?;
		let end = crate::lyxal_core_db::key::graph::egsuffix(ns_id, db_id, &rid.table, &rid.key, dir)
...
async fn eval_graph_bound(
	...
	match bound {
		Bound::Unbounded => {
			if is_start {
				crate::lyxal_core_db::key::graph::ftprefix(ns_id, db_id, &rid.table, &rid.key, dir, edge_table)
					.context("Failed to create graph table prefix")
			} else {
				crate::lyxal_core_db::key::graph::ftsuffix(ns_id, db_id, &rid.table, &rid.key, dir, edge_table)
...
fn encode_graph_key(
	...
	dir: &Dir,
	edge_table: &TableName,
	fk: crate::lyxal_core_db::val::RecordIdKey,
) -> Result<Vec<u8>, ControlFlow> {
	crate::lyxal_core_db::key::graph::new(
		ns_id,
		db_id,
		&rid.table,
		&rid.key,
...
fn decode_graph_edge(key: &[u8]) -> Result<RecordId, ControlFlow> {
	let decoded =
		crate::lyxal_core_db::key::graph::Graph::decode_key(key).context("Failed to decode graph key")?;
	Ok(RecordId {
