...
use crate::lyxal_core_kvs::{KVKey, KVValue};
use crate::lyxal_core_db::val::{Number, Object, RecordIdKey, TableName, Value};
...
async fn count_with_perm_fallback(
	...
	rid: Option<&crate::lyxal_core_db::val::RecordId>,
	version: Option<u64>,
	...
	let (beg, end) = if let Some(rid) = rid {
		match &rid.key {
			...
			_ => {
				...
				let Some(value) =
					crate::lyxal_core_db::exec::operators::fetch::fetch_raw_record(ctx, rid, version).await?
				else {
					return Ok(0);
				};
