...
use crate::lyxal_core_db::exec::permission::{
	PhysicalPermission, check_permission_for_value, convert_permission_to_physical,
};
use crate::lyxal_core_db::exec::planner::expr_to_physical_expr;
use crate::lyxal_core_db::exec::{EvalContext, ExecutionContext, PhysicalExpr, ValueBatch, ValueBatchStream};
use crate::lyxal_core_db::expr::{ControlFlow, ControlFlowExt};
use crate::lyxal_core_db::idx::planner::ScanDirection;
use crate::lyxal_core_db::key::record;
use crate::lyxal_core_kvs::{KVKey, KVValue, Transaction};
use crate::lyxal_core_db::val::{RecordIdKey, TableName, Value};

/// A raw computed field entry before topological sorting:
/// `(field_name, physical_expr, optional_kind, dependency_field_names)`.
type RawComputedField = (String, Arc<dyn PhysicalExpr>, Option<crate::lyxal_core_db::expr::Kind>, Vec<String>);
...
pub(crate) fn determine_scan_direction(
	...
	use crate::lyxal_core_db::expr::order::Ordering as OrderingType;
...
pub(crate) fn kv_scan_stream(
	txn: Arc<Transaction>,
	beg: crate::lyxal_core_kvs::Key,
	end: crate::lyxal_core_kvs::Key,
	version: Option<u64>,
...
pub(crate) fn decode_record(key: &[u8], val: Vec<u8>) -> Result<Value, ControlFlow> {
	let decoded_key =
		crate::lyxal_core_db::key::record::RecordKey::decode_key(key).context("Failed to decode record key")?;

	let rid = crate::lyxal_core_db::val::RecordId {
		table: decoded_key.tb.into_owned(),
		key: decoded_key.id,
	};

	let mut record =
		crate::lyxal_core_db::catalog::Record::kv_decode_value(val).context("Failed to deserialize record")?;
...
) -> Result<crate::lyxal_core_kvs::Key, ControlFlow> {
	match bound {
		Bound::Unbounded => {
...
pub(crate) struct FieldState {
	...
	dep_map: Arc<HashMap<String, crate::lyxal_core_db::expr::computed_deps::ComputedDeps>>,
}
...
pub(crate) struct ComputedFieldDef {
	...
	kind: Option<crate::lyxal_core_db::expr::Kind>,
}
...
pub(crate) async fn build_field_state_raw(
	...
	ns_id: crate::lyxal_core_db::catalog::NamespaceId,
	db_id: crate::lyxal_core_db::catalog::DatabaseId,
	...
	let has_field_perms = check_perms
		&& field_defs
			.iter()
			.any(|fd| !matches!(fd.select_permission, crate::lyxal_core_db::catalog::Permission::Full));
	...
	let mut dep_map: HashMap<String, crate::lyxal_core_db::expr::computed_deps::ComputedDeps> = HashMap::new();
	...
	for fd in field_defs.iter() {
		if let Some(ref expr) = fd.computed {
			...
			let deps = if let Some(ref cd) = fd.computed_deps {
				crate::lyxal_core_db::expr::computed_deps::ComputedDeps {
					fields: cd.fields.clone(),
					is_complete: cd.is_complete,
				}
			} else {
				crate::lyxal_core_db::expr::computed_deps::extract_computed_deps(expr)
			};
	...
	let sorted_indices = crate::lyxal_core_db::expr::computed_deps::topological_sort_computed_fields(&topo_input);
...
pub(crate) fn filter_field_state_for_projection(
	...
	let required =
		crate::lyxal_core_db::expr::computed_deps::resolve_required_computed_fields(needed, &full_state.dep_map);
