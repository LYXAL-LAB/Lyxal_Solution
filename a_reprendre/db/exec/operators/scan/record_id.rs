...
use crate::lyxal_core_db::val::{RecordId, RecordIdKey, Value};
...
impl ExecOperator for RecordIdScan {
	...
	fn cardinality_hint(&self) -> crate::lyxal_core_db::exec::CardinalityHint {
		...
		crate::lyxal_core_db::exec::CardinalityHint::AtMostOne
	}
	...
	fn output_ordering(&self) -> OutputOrdering {
		...
		OutputOrdering::Sorted(vec![SortProperty {
			path: crate::lyxal_core_db::exec::field_path::FieldPath::field("id"),
...
pub(crate) async fn execute_record_lookup(
	...
	let (select_permission, field_state) = if let Some(res) = resolved {
		...
		if table_def.is_none() {
			return Err(ControlFlow::Err(anyhow::Error::new(crate::lyxal_core_error::Error::TbNotFound {
		...
		let perm = if check_perms {
			let catalog_perm = match &table_def {
				...
				None => crate::lyxal_core_db::catalog::Permission::None,
			};
