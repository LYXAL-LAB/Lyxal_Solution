...
use crate::lyxal_core_db::catalog::{DatabaseId, NamespaceId, Permission};
...
use crate::lyxal_core_db::exec::operators::scan::pipeline::ScanPipeline;
use crate::lyxal_core_db::exec::permission::{
	PhysicalPermission, convert_permission_to_physical, should_check_perms,
	validate_record_user_access,
};
use crate::lyxal_core_db::exec::planner::util::{resolve_condition_params, strip_knn_from_condition};
...
use crate::lyxal_core_db::expr::with::With;
use crate::lyxal_core_db::expr::{Cond, ControlFlow, ControlFlowExt};
...
pub struct DynamicScan {
	...
	pub(crate) knn_context: Option<Arc<crate::lyxal_core_db::exec::function::KnnContext>>,
}
		...
		let stream = async_stream::try_stream! {
			...
			Value::RecordId(rid) => {
				...
				let version: Option<u64> = match &version {
					Some(expr) => {
						...
						Some(
							v.cast_to::<crate::lyxal_core_db::val::Datetime>()
			...
			while let Some(batch_result) = source.next().await {
				...
				if ctx.cancellation().is_cancelled() {
					Err(ControlFlow::Err(
						anyhow::anyhow!(crate::lyxal_core_error::Error::QueryCancelled),
					))?;
				}
...
mod tests {
	...
	async fn create_test_scan(table_name: &str, with_index_hints: bool) -> DynamicScan {
		...
		let source = expr_to_physical_expr(
			crate::lyxal_core_db::expr::Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::String(
