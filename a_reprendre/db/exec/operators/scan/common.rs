...
use crate::lyxal_core_db::exec::{ControlFlowExt, EvalContext, ExecutionContext, PhysicalExpr};
use crate::lyxal_core_db::expr::ControlFlow;
use crate::lyxal_core_kvs::{CachePolicy, Transaction};
...
/// Lyxal_QL semantics where graph traversal on an object uses its `id`.
...
pub(crate) async fn fetch_and_filter_records_batch(
	...
	db_id: DatabaseId,
	rids: &[RecordId],
	select_permission: &crate::lyxal_core_db::exec::permission::PhysicalPermission,
	check_perms: bool,
	version: Option<u64>,
	cache_policy: CachePolicy,
) -> Result<Vec<Value>, ControlFlow> {
	...
	for record in records {
		...
		if check_perms {
			...
			let allowed = crate::lyxal_core_db::exec::permission::check_permission_for_value(
				select_permission,
				&record.data,
				ctx,
			)
