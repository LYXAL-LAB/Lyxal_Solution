...
use crate::lyxal_core_db::exec::index::access_path::{BTreeAccess, IndexRef};
use crate::lyxal_core_db::exec::index::iterator::{
...
pub struct IndexScan {
	...
	pub table_name: crate::lyxal_core_db::val::TableName,
...
impl ExecOperator for IndexScan {
	...
	fn output_ordering(&self) -> crate::lyxal_core_db::exec::OutputOrdering {
		...
		let cols: Vec<SortProperty> = self
			...
			.collect();
		if cols.is_empty() {
			crate::lyxal_core_db::exec::OutputOrdering::Unordered
		} else {
			crate::lyxal_core_db::exec::OutputOrdering::Sorted(cols)
		}
	}
	...
	fn execute(&self, ctx: &ExecutionContext) -> FlowResult<ValueBatchStream> {
		...
		let stream = async_stream::try_stream! {
			...
			let version: Option<u64> = match &version_expr {
				Some(expr) => {
					let eval_ctx = crate::lyxal_core_db::exec::EvalContext::from_exec_ctx(&ctx);
				...
				(BTreeAccess::Equality(value), true) => {
					if ctx.cancellation().is_cancelled() {
						Err(ControlFlow::Err(anyhow::anyhow!(
							crate::lyxal_core_error::Error::QueryCancelled
						)))?;
					}
