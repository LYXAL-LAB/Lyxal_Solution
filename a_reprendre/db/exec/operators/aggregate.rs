...
use crate::lyxal_core_db::exec::function::{Accumulator, AggregateFunction};
		...
		let aggregate_stream = async_stream::try_stream! {
			...
			while let Some(batch_result) = input_stream.next().await {
				...
				if ctx.cancellation().is_cancelled() {
					Err(crate::lyxal_core_db::expr::ControlFlow::Err(
			...
			if group_by_exprs.is_empty() && groups.is_empty() {
				let perms_active = ctx
					.should_check_perms(crate::lyxal_core_db::iam::Action::View)
					.unwrap_or(true);
				if !perms_active {
