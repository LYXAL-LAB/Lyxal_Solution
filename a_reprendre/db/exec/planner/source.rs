...
use crate::lyxal_core_db::exec::parts::LookupDirection;
use crate::lyxal_core_db::exec::planner::select::SelectPipelineConfig;
...
impl<'ctx> Planner<'ctx> {
	...
	pub(crate) async fn plan_index_function(
		...
		use crate::lyxal_core_db::exec::function::{IndexContext, IndexContextKind};
		use crate::lyxal_core_db::exec::physical_expr::function::IndexFunctionExec;
	...
	pub(crate) async fn convert_order_list(
		&self,
		order_list: crate::lyxal_core_db::expr::order::OrderList,
	) -> Result<Vec<OrderByField>, Error> {
		let mut fields = Vec::with_capacity(order_list.len());
		for order_field in order_list {
			let expr: Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr> =
	...
	) -> Result<std::ops::Bound<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>, Error> {
		match bound {
	...
	pub(crate) async fn plan_lookup(
		&self,
		lookup: crate::lyxal_core_db::expr::lookup::Lookup,
	...
	pub(crate) async fn plan_lookup_with_input(
		...
		crate::lyxal_core_db::expr::lookup::Lookup {
			kind,
			expr,
			what,
			cond,
		...
		let base_scan: Arc<dyn ExecOperator> = match &kind {
			crate::lyxal_core_db::expr::lookup::LookupKind::Graph(dir) => {
				let mut edge_tables: Vec<EdgeTableSpec> = Vec::with_capacity(what.len());
				for s in what {
					let spec = match s {
						crate::lyxal_core_db::expr::lookup::LookupSubject::Table {
							table,
							..
						} => EdgeTableSpec {
							table,
						...
						crate::lyxal_core_db::expr::lookup::LookupSubject::Range {
							table,
							range,
							..
						} => {
				...
				let scan =
					if cond.is_none() && split.is_none() && order.is_none() && group.is_none() {
						if let Some(crate::lyxal_core_db::expr::limit::Limit(crate::lyxal_core_db::expr::Expr::Literal(
							crate::lyxal_core_db::expr::Literal::Integer(n),
						))) = &limit
						{
							let offset = match &start {
								Some(crate::lyxal_core_db::expr::start::Start(crate::lyxal_core_db::expr::Expr::Literal(
									crate::lyxal_core_db::expr::Literal::Integer(s),
								))) => *s as usize,
								_ => 0,
							};
			...
			crate::lyxal_core_db::expr::lookup::LookupKind::Reference => {
		...
		if needs_full_pipeline {
			...
			let split_op: Arc<dyn ExecOperator> = if let Some(splits) = split {
				Arc::new(crate::lyxal_core_db::exec::operators::Split::new(
					filtered,
			...
			let sorted: Arc<dyn ExecOperator> = match order {
				Some(crate::lyxal_core_db::expr::order::Ordering::Order(order_list)) => {
					let order_by = self.convert_order_list(order_list).await?;
					Arc::new(crate::lyxal_core_db::exec::operators::Sort::new(split_op, order_by))
				}
				Some(crate::lyxal_core_db::expr::order::Ordering::Random) => {
