...
//!
//! Converts Lyxal_QL idiom AST nodes to physical part expressions for runtime evaluation.
...
use crate::lyxal_core_db::exec::operators::{CurrentValueSource, RecursionOp};
use crate::lyxal_core_db::exec::parts::{
...
use crate::lyxal_core_db::exec::physical_expr::IdiomExpr;
use crate::lyxal_core_db::exec::{ExecOperator, PhysicalExpr};
use crate::lyxal_core_db::expr::part::{DestructurePart as AstDestructurePart, Part, RecurseInstruction};
...
impl<'ctx> Planner<'ctx> {
	...
	pub(crate) async fn convert_idiom(
		&self,
		idiom: crate::lyxal_core_db::expr::idiom::Idiom,
	...
	pub(crate) fn convert_parts(
		&self,
		parts: Vec<Part>,
	) -> crate::lyxal_core_db::exec::BoxFut<'_, Result<Vec<Arc<dyn PhysicalExpr>>, Error>> {
		Box::pin(async move {
			...
			while let Some(part) = iter.next() {
				...
				if let Part::Recurse(recurse, None, instruction) = part {
					let system_limit = *crate::lyxal_core_config::cnf::IDIOM_RECURSION_LIMIT as u32;
					let (min_depth, max_depth) = match recurse {
						crate::lyxal_core_db::expr::part::Recurse::Fixed(n) => (n, Some(n)),
						crate::lyxal_core_db::expr::part::Recurse::Range(min, max) => (min.unwrap_or(1), max),
					};
		...
		match part {
			...
			Part::Lookup(lookup) => {
				...
				let direction = match &lookup.kind {
					crate::lyxal_core_db::expr::lookup::LookupKind::Graph(dir) => LookupDirection::from(dir),
					crate::lyxal_core_db::expr::lookup::LookupKind::Reference => LookupDirection::Reference,
				};
	...
	) -> crate::lyxal_core_db::exec::BoxFut<'_, Result<Vec<DestructureField>, Error>> {
		Box::pin(async move {
...
fn lookup_metadata(lookup: &crate::lyxal_core_db::expr::lookup::Lookup) -> (LookupDirection, bool) {
	let direction = match &lookup.kind {
		crate::lyxal_core_db::expr::lookup::LookupKind::Graph(dir) => LookupDirection::from(dir),
		crate::lyxal_core_db::expr::lookup::LookupKind::Reference => LookupDirection::Reference,
	};
