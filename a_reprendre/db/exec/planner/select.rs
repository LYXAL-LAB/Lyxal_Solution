...
use crate::lyxal_core_db::catalog::providers::{DatabaseProvider, NamespaceProvider, TableProvider};
use crate::lyxal_core_config::cnf::MAX_ORDER_LIMIT_PRIORITY_QUEUE_SIZE;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::exec::expression_registry::{ComputePoint, ExpressionRegistry, resolve_order_by_alias};
use crate::lyxal_core_db::exec::field_path::FieldPath;
use crate::lyxal_core_db::exec::index::access_path::{AccessPath, select_access_path};
use crate::lyxal_core_db::exec::index::analysis::IndexAnalyzer;
#[cfg(all(storage, not(target_family = "wasm")))]
use crate::lyxal_core_db::exec::operators::ExternalSort;
use crate::lyxal_core_db::exec::operators::scan::determine_scan_direction;
use crate::lyxal_core_db::exec::operators::scan::resolved::{ResolvedTableContext, resolve_table_context};
...
use crate::lyxal_core_db::exec::{ExecOperator, OperatorMetrics};
use crate::lyxal_core_db::expr::field::{Field, Fields};
use crate::lyxal_core_db::expr::{Cond, Expr, Idiom, Literal};
...
pub(crate) struct SelectPipelineConfig {
	pub cond: Option<crate::lyxal_core_db::expr::cond::Cond>,
	pub split: Option<crate::lyxal_core_db::expr::split::Splits>,
	pub group: Option<crate::lyxal_core_db::expr::group::Groups>,
	pub order: Option<crate::lyxal_core_db::expr::order::Ordering>,
	pub limit: Option<crate::lyxal_core_db::expr::limit::Limit>,
	pub start: Option<crate::lyxal_core_db::expr::start::Start>,
	pub omit: Vec<Expr>,
	...
	pub precompiled_predicate: Option<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>,
}
...
impl<'ctx> Planner<'ctx> {
	...
	fn can_eliminate_sort(
		...
		use crate::lyxal_core_db::exec::ordering::SortProperty;
	...
	fn register_and_push_projection(
		...
		registry: &mut ExpressionRegistry,
		expr_key: String,
		physical: Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>,
		output_name: String,
	) {
	...
	pub(crate) async fn plan_omit(
		&self,
		omit: Vec<Expr>,
	) -> Result<Vec<crate::lyxal_core_db::expr::idiom::Idiom>, Error> {
	...
	pub(crate) async fn resolve_field_idioms(
		...
		use crate::lyxal_core_db::expr::Function;

		match expr {
			Expr::Idiom(idiom) => Ok(vec![idiom]),
			Expr::Param(ref param) => {
				let value =
					self.ctx.value(param.as_str()).cloned().unwrap_or(crate::lyxal_core_db::val::Value::None);
				let s = value.clone().coerce_to::<String>().map_err(|_| Error::InvalidFetch {
					value: value.to_string(),
				})?;
				let idiom: Idiom = crate::lyxal_core_db::syn::idiom(&s)
	...
	pub(crate) fn extract_needed_fields(
		...
		use crate::lyxal_core_db::expr::Part;
		use crate::lyxal_core_db::expr::visit::{Visit, Visitor};
		...
		impl Visitor for NeededFieldExtractor {
			...
			fn visit_idiom(&mut self, idiom: &crate::lyxal_core_db::expr::Idiom) -> Result<(), Self::Error> {
		...
		if let Some(ordering) = order {
			match ordering {
				crate::lyxal_core_db::expr::order::Ordering::Random => {}
				crate::lyxal_core_db::expr::order::Ordering::Order(order_list) => {
	...
	pub(crate) async fn plan_select_statement(
		&self,
		mut select: crate::lyxal_core_db::expr::statements::SelectStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let explain = select.explain.take();
		let plan = Box::pin(self.plan_select_core(select)).await?;
		match explain {
			Some(crate::lyxal_core_db::expr::explain::Explain(full)) => {
				if full {
					Ok(Arc::new(AnalyzePlan {
						plan,
						format: crate::lyxal_core_db::expr::ExplainFormat::Json,
	...
	async fn plan_select_core(
		&self,
		select: crate::lyxal_core_db::expr::statements::SelectStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let crate::lyxal_core_db::expr::statements::SelectStatement {
			fields,
			omit,
			only,
			what,
		...
		if is_count_all_eligible(&fields, &group, &cond, &split, &order, &fetch, &omit, &what) {
			use crate::lyxal_core_db::exec::operators::CountScan;
			let table_expr = self
		...
		{
			use crate::lyxal_core_db::exec::operators::scan::index_count::IndexCountScan;
			let table_expr =
		...
		let primary_table = what
			...
			.or_else(|| {
				what.iter().find_map(|e| match e {
					Expr::Param(p) => {
						if let Some(crate::lyxal_core_db::val::Value::Table(t)) = self.ctx.value(p.as_str()) {
							Some(t.clone())
						} else {
							None
						}
		...
		let planning_ctx: std::borrow::Cow<'_, crate::lyxal_core_db::ctx::FrozenContext> =
			if let Some(ref c) = cond {
				let mc = extract_matches_context(c, Some(self.ctx));
				let hm = !mc.is_empty();
				if hm || has_knn_early {
					let mut child = crate::lyxal_core_db::ctx::Context::new(self.ctx);
					if hm {
						let mut mc = mc;
					...
					if has_knn_early {
						child.set_knn_context(std::sync::Arc::new(
							crate::lyxal_core_db::exec::function::KnnContext::new(),
						));
					}
		...
		let source_is_single_scan = what.len() == 1
			...
			|| (what.len() == 1
				&& matches!(&what[0], Expr::Param(p) if {
					matches!(self.ctx.value(p.as_str()), Some(crate::lyxal_core_db::val::Value::Table(_)))
				}));
	...
	pub(crate) async fn plan_sources(
		&self,
		what: Vec<Expr>,
		version: Option<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>,
		cond: Option<&Cond>,
		order: Option<&crate::lyxal_core_db::expr::order::Ordering>,
		with: Option<&crate::lyxal_core_db::expr::with::With>,
		needed_fields: Option<std::collections::HashSet<String>>,
		scan_predicate: Option<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>,
		scan_limit: Option<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>,
		scan_start: Option<Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>,
	...
	pub(crate) async fn plan_source(
		...
		use crate::lyxal_core_db::exec::operators::{FullTextScan, IndexScan, KnnScan};
		...
		match expr {
			...
			Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::RecordId(rid)) => {
				let record_id_expr = self
					.physical_expr(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::RecordId(rid)))
					.await?;
				Ok(PlannedSource {
			...
			Expr::Param(ref param) => match self.ctx.value(param.as_str()) {
				Some(crate::lyxal_core_db::val::Value::Table(_)) => {
					let filter_action = if scan_predicate.is_some() {
						FilterAction::FullyConsumed
					} else {
						FilterAction::UseOriginal
				...
				Some(crate::lyxal_core_db::val::Value::RecordId(_)) => {
	...
	async fn try_resolve_table_ctx(
		txn: &crate::lyxal_core_kvs::Transaction,
		ctx: &crate::lyxal_core_db::ctx::FrozenContext,
		ns: &str,
		db: &str,
		table_name: &crate::lyxal_core_db::val::TableName,
	) -> Option<crate::lyxal_core_db::exec::operators::scan::resolved::ResolvedTableContext> {
	...
	async fn has_matching_count_index(&self, what: &[Expr], cond: &Option<Cond>) -> bool {
		...
		indexes.iter().any(|ix| {
			if let crate::lyxal_core_db::catalog::Index::Count(ref idx_cond) = ix.index {
	...
	async fn resolve_access_path(
		...
		order: Option<&crate::lyxal_core_db::expr::order::Ordering>,
		with: Option<&crate::lyxal_core_db::expr::with::With>,
	) -> Result<Option<(AccessPath, crate::lyxal_core_db::idx::planner::ScanDirection)>, Error> {
		let direction = determine_scan_direction(&order.cloned());

		if matches!(with, Some(crate::lyxal_core_db::expr::with::With::NoIndex)) {
