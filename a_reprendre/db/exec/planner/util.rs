...
use crate::lyxal_core_db::catalog::Distance;
use crate::lyxal_core_db::catalog::providers::DatabaseProvider;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::exec::field_path::{FieldPath, FieldPathPart};
use crate::lyxal_core_db::expr::field::{Field, Fields};
use crate::lyxal_core_db::expr::operator::NearestNeighbor;
use crate::lyxal_core_db::expr::visit::{MutVisitor, Visit, VisitMut, Visitor};
use crate::lyxal_core_db::expr::{BinaryOperator, Cond, Expr, Idiom, Literal, Param};
use crate::lyxal_core_db::val::Number;
...
) -> Option<crate::lyxal_core_db::val::Value> {
	...
	match lit {
		...
		Literal::RecordId(rid) => {
			...
			let key = match &rid.key {
				RecordIdKeyLit::Number(n) => crate::lyxal_core_db::val::RecordIdKey::Number(*n),
				RecordIdKeyLit::String(s) => crate::lyxal_core_db::val::RecordIdKey::String(s.clone()),
				RecordIdKeyLit::Uuid(u) => crate::lyxal_core_db::val::RecordIdKey::Uuid(*u),
				_ => return None,
			};
			Some(Value::RecordId(crate::lyxal_core_db::val::RecordId::new(rid.table.clone(), key)))
		}
		Literal::Array(arr) => {
...
pub(crate) fn try_expr_to_value(expr: &Expr) -> Option<crate::lyxal_core_db::val::Value> {
	match expr {
...
pub(super) fn literal_to_value(
	lit: crate::lyxal_core_db::expr::literal::Literal,
) -> Result<crate::lyxal_core_db::val::Value, Error> {
	use crate::lyxal_core_db::expr::literal::Literal;
	use crate::lyxal_core_db::val::{Range, Value};
...
pub(super) fn key_lit_to_expr(lit: &crate::lyxal_core_db::expr::RecordIdKeyLit) -> Result<Expr, Error> {
	use crate::lyxal_core_db::expr::RecordIdKeyLit;
	match lit {
		RecordIdKeyLit::Number(n) => Ok(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::Integer(*n))),
		RecordIdKeyLit::String(s) => {
			Ok(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::String(s.clone())))
		}
		RecordIdKeyLit::Uuid(u) => Ok(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::Uuid(*u))),
		RecordIdKeyLit::Array(exprs) => {
			Ok(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::Array(exprs.clone())))
		}
		RecordIdKeyLit::Object(entries) => {
			Ok(Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::Object(entries.clone())))
		}
...
impl Visitor for KnnOperatorChecker {
	...
	fn visit_select(&mut self, _: &crate::lyxal_core_db::expr::SelectStatement) -> Result<(), Self::Error> {
		Ok(())
	}
}
...
pub(crate) fn strip_index_conditions(
	cond: &Cond,
	access: &crate::lyxal_core_db::exec::index::access_path::BTreeAccess,
	cols: &[Idiom],
) -> Option<Cond> {
	let mut expr = cond.0.clone();
...
struct IndexConditionStripper<'a> {
	...
	access: &'a crate::lyxal_core_db::exec::index::access_path::BTreeAccess,
}
...
impl IndexConditionStripper<'_> {
	...
	fn matches_access(&self, left: &Expr, op: &BinaryOperator, right: &Expr) -> bool {
		use crate::lyxal_core_db::exec::index::access_path::BTreeAccess;
...
impl MutVisitor for IndexConditionStripper<'_> {
	...
	fn visit_mut_select(
		&mut self,
		_: &mut crate::lyxal_core_db::expr::SelectStatement,
	) -> Result<(), Self::Error> {
		Ok(())
	}
}
...
struct ParamResolver<'a> {
	values: &'a std::collections::HashMap<String, crate::lyxal_core_db::val::Value>,
}
...
pub(crate) async fn resolve_condition_params(
	cond: &Cond,
	ctx: &crate::lyxal_core_db::ctx::FrozenContext,
	ns_db: Option<(crate::lyxal_core_db::catalog::NamespaceId, crate::lyxal_core_db::catalog::DatabaseId)>,
) -> Cond {
...
pub(super) fn extract_record_id_point_lookup(
	cond: &Cond,
	table_name: &crate::lyxal_core_db::val::TableName,
) -> Option<Expr> {
	find_id_equality_in_and_chain(&cond.0, table_name)
}

/// Walk the top-level AND chain looking for `id = <RecordId literal>`.
fn find_id_equality_in_and_chain(expr: &Expr, table_name: &crate::lyxal_core_db::val::TableName) -> Option<Expr> {
	match expr {
...
fn check_id_recordid_pair(
	...
	if let Expr::Idiom(idiom) = idiom_side
		...
		&& !matches!(rid.key, crate::lyxal_core_db::expr::RecordIdKeyLit::Range(_))
	{
		Some(lit_side.clone())
	} else {
		None
...
pub(super) fn extract_matches_context(
	cond: &Cond,
	ctx: Option<&crate::lyxal_core_db::ctx::FrozenContext>,
) -> crate::lyxal_core_db::exec::function::MatchesContext {
	let mut collector = MatchesCollector(crate::lyxal_core_db::exec::function::MatchesContext::new(), ctx);
	let _ = collector.visit_expr(&cond.0);
	collector.0
}
...
struct MatchesCollector<'a>(
	crate::lyxal_core_db::exec::function::MatchesContext,
	Option<&'a crate::lyxal_core_db::ctx::FrozenContext>,
);
	...
	fn visit_expr(&mut self, expr: &Expr) -> Result<(), Self::Error> {
		...
		} = expr && let Expr::Idiom(idiom) = left.as_ref()
		{
			...
			if let Some(query) = query_str {
				...
				self.0.insert(
					match_ref,
					crate::lyxal_core_db::exec::function::MatchInfo {
						idiom: idiom.clone(),
						query,
					},
				);
...
pub(super) fn extract_table_from_context(ctx: &crate::lyxal_core_db::ctx::FrozenContext) -> crate::lyxal_core_db::val::TableName {
	if let Some(mc) = ctx.get_matches_context()
		&& let Some(table) = mc.table()
	{
		return table.clone();
	}
	crate::lyxal_core_db::val::TableName::from("unknown".to_string())
}
...
) -> Result<Option<std::sync::Arc<dyn crate::lyxal_core_db::exec::PhysicalExpr>>, Error> {
	match version_expr {
...
pub(super) fn order_is_scan_compatible(order: &Option<crate::lyxal_core_db::expr::order::Ordering>) -> bool {
	use crate::lyxal_core_db::expr::order::Ordering;
	match order {
		None => true,
...
pub(super) fn index_covers_ordering(
	index_ref: &crate::lyxal_core_db::exec::index::access_path::IndexRef,
	direction: crate::lyxal_core_db::idx::planner::ScanDirection,
	order: &crate::lyxal_core_db::expr::order::Ordering,
) -> bool {
	use crate::lyxal_core_db::exec::operators::SortDirection;
	use crate::lyxal_core_db::exec::ordering::{OutputOrdering, SortProperty};
	use crate::lyxal_core_db::expr::order::Ordering;
	...
	let required: Vec<SortProperty> = order_list
		.iter()
		.filter_map(|field| {
			crate::lyxal_core_db::exec::field_path::FieldPath::try_from(&field.value).ok().map(|path| {
	...
	let dir = match direction {
		crate::lyxal_core_db::idx::planner::ScanDirection::Forward => SortDirection::Asc,
		crate::lyxal_core_db::idx::planner::ScanDirection::Backward => SortDirection::Desc,
	};
	...
	let cols: Vec<SortProperty> = ix_def
		...
		.iter()
		.filter_map(|idiom| {
			crate::lyxal_core_db::exec::field_path::FieldPath::try_from(idiom).ok().map(|path| SortProperty {
				path,
				direction: dir,
				collate: false,
				numeric: false,
...
pub(super) fn get_effective_limit_literal(
	start: &Option<crate::lyxal_core_db::expr::start::Start>,
	limit: &Option<crate::lyxal_core_db::expr::limit::Limit>,
) -> Option<usize> {
...
pub(super) fn derive_field_name(expr: &Expr) -> String {
	match expr {
		...
		Expr::FunctionCall(call) => {
			let idiom: crate::lyxal_core_db::expr::idiom::Idiom = call.receiver.to_idiom();
...
pub(super) fn idiom_to_field_name(idiom: &crate::lyxal_core_db::expr::idiom::Idiom) -> String {
	use lyxal_types_core::ToSql;
...
pub(super) fn idiom_to_field_path(idiom: &crate::lyxal_core_db::expr::idiom::Idiom) -> FieldPath {
	use lyxal_types_core::ToSql;
...
pub(super) fn is_indexed_count_eligible(
	fields: &Fields,
	group: &Option<crate::lyxal_core_db::expr::group::Groups>,
	cond: &Option<crate::lyxal_core_db::expr::cond::Cond>,
	split: &Option<crate::lyxal_core_db::expr::split::Splits>,
	order: &Option<crate::lyxal_core_db::expr::order::Ordering>,
	fetch: &Option<crate::lyxal_core_db::expr::fetch::Fetchs>,
	omit: &[Expr],
	what: &[Expr],
) -> bool {
...
pub(super) fn is_count_all_eligible(
	...
	matches!(
		&what[0],
		Expr::Table(_)
			| Expr::Literal(crate::lyxal_core_db::expr::literal::Literal::RecordId(_))
			| Expr::Param(_)
			| Expr::Postfix { .. }
	)
}
