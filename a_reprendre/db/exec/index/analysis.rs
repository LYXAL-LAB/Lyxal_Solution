...
use crate::lyxal_core_db::catalog::{Index, IndexDefinition};
use crate::lyxal_core_db::exec::planner::util::try_literal_to_value;
use crate::lyxal_core_db::expr::operator::{MatchesOperator, NearestNeighbor, PrefixOperator};
use crate::lyxal_core_db::expr::order::Ordering;
use crate::lyxal_core_db::expr::with::With;
use crate::lyxal_core_db::expr::{BinaryOperator, Cond, Expr, Idiom};
	...
	pub fn try_in_expansion(
		...
		for (idiom, values) in &in_exprs {
			...
			for (idx, ix_def) in self.indexes.iter().enumerate() {
				...
				if !matches!(ix_def.index, crate::lyxal_core_db::catalog::Index::Idx | crate::lyxal_core_db::catalog::Index::Uniq)
				{
					continue;
				}
...
// literal_to_value and expr_to_value are imported from crate::lyxal_core_db::exec::planner::util
