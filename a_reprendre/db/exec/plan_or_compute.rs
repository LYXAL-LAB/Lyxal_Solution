...
use crate::lyxal_core_db::exec::context::{ContextLevel, ExecutionContext};
use crate::lyxal_core_db::exec::planner::try_plan_expr;
use crate::lyxal_core_db::exec::{FlowResult, ValueBatchStream};
use crate::lyxal_core_db::expr::part::{Part, RecurseInstruction};
use crate::lyxal_core_db::expr::statements::InfoStatement;
use crate::lyxal_core_db::expr::{Base, Block, ControlFlow, ControlFlowExt, Expr, Literal};
...
) -> Result<(&crate::lyxal_core_db::dbs::Options, FrozenContext), Error> {
...
pub(crate) fn get_legacy_context_with_param<'a>(
	...
	param_name: &str,
	param_value: &Value,
) -> Result<(&'a crate::lyxal_core_db::dbs::Options, FrozenContext), Error> {
	let options = exec_ctx
		.options()
		.ok_or_else(|| Error::Thrown("Options not available for legacy compute fallback".into()))?;

	let mut ctx = crate::lyxal_core_db::ctx::Context::new(exec_ctx.ctx());
...
pub(crate) async fn legacy_compute(
	...
	opt: &crate::lyxal_core_db::dbs::Options,
	doc: Option<&crate::lyxal_core_db::doc::CursorDoc>,
) -> crate::lyxal_core_db::expr::FlowResult<Value> {
...
pub(crate) async fn collect_single_value(
	...
	if values.is_empty() {
		...
		Ok(Value::Array(crate::lyxal_core_db::val::Array(values)))
	}
}
...
fn record_id_key_required_context(key: &crate::lyxal_core_db::expr::RecordIdKeyLit) -> ContextLevel {
...
fn idiom_required_context(idiom: &crate::lyxal_core_db::expr::Idiom) -> ContextLevel {
