...
impl ScalarFunction for ValueDiff {
	...
	fn invoke_async<'a>(
		...
		Box::pin(async move {
			...
			crate::lyxal_core_functions::value::diff(args).await
		})
	}
}
...
impl ScalarFunction for ValuePatch {
	...
	fn invoke_async<'a>(
		...
		Box::pin(async move {
			...
			crate::lyxal_core_functions::value::patch(args).await
		})
	}
}
		...
		Box::pin(async move {
			...
			stack
				.enter(|stk| async move {
					crate::lyxal_core_functions::value::chain((stk, frozen, opt, doc), args).await
				})
				.finish()
				.await
		})
