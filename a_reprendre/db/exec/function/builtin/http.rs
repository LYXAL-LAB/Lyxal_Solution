...
#[cfg(feature = "http")]
use crate::lyxal_core_db::val::Object;
...
async fn http_disabled() -> Result<Value> {
	Err(anyhow::anyhow!(crate::lyxal_core_error::Error::HttpDisabled))
}
...
fn extract_uri(args: &[Value], fn_name: &str) -> Result<String> {
	match args.first() {
		...
		Some(v) => Err(anyhow::anyhow!(crate::lyxal_core_error::Error::InvalidFunctionArguments {
		...
		None => Err(anyhow::anyhow!(crate::lyxal_core_error::Error::InvalidFunctionArguments {
			name: fn_name.to_owned(),
...
async fn http_head_impl(ctx: &EvalContext<'_>, args: Vec<Value>) -> Result<Value> {
	...
	let url = url::Url::parse(&uri).map_err(|_| crate::lyxal_core_error::Error::InvalidUrl(uri.clone()))?;
...
async fn http_request(
	...
	use crate::lyxal_core_config::cnf::LYXAL_USER_AGENT;
	...
	let cli = {
		...
		let capabilities_clone: Arc<crate::lyxal_core_db::dbs::Capabilities> = Arc::clone(&capabilities);
		...
		let count = *crate::lyxal_core_config::cnf::MAX_HTTP_REDIRECTS;
		let policy =
		...
		reqwest::Client::builder()
			.pool_idle_timeout(Duration::from_secs(*crate::lyxal_core_config::cnf::HTTP_IDLE_TIMEOUT_SECS))
			.pool_max_idle_per_host(*crate::lyxal_core_config::cnf::MAX_HTTP_IDLE_CONNECTIONS_PER_HOST)
			.connect_timeout(Duration::from_secs(*crate::lyxal_core_config::cnf::HTTP_CONNECT_TIMEOUT_SECS))
			.tcp_keepalive(Some(Duration::from_secs(60)))
			.http2_keep_alive_interval(Some(Duration::from_secs(30)))
			.http2_keep_alive_timeout(Duration::from_secs(10))
			.redirect(policy)
			.dns_resolver(Arc::new(
				crate::lyxal_core_functions::http::resolver::FilteringResolver::from_capabilities(capabilities),
			))
			.build()?
	};
	...
	// Add body if present
	if let Some(b) = body {
		let public_body = crate::lyxal_core_db::val::convert_value_to_public_value(b)?;
