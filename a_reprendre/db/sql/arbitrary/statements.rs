...
use crate::lyxal_core_db::sql::access_type::{BearerAccess, BearerAccessSubject};
use crate::lyxal_core_db::sql::arbitrary::{
	self, arb_group, arb_opt, arb_order, arb_splits, arb_vec1, atleast_one, insert_data,
};
use crate::lyxal_core_db::sql::kind::KindLiteral;
use crate::lyxal_core_db::sql::statements::SetStatement;
...
impl<'a> Arbitrary<'a> for DefineAccessStatement {
	fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
		...
		let duration = crate::lyxal_core_db::sql::access::AccessDuration {
			grant: u.arbitrary()?,
			token: u.arbitrary()?,
			session: u.arbitrary()?,
		};
