...
impl From<UserDuration> for crate::lyxal_core_db::expr::user::UserDuration {
	fn from(v: UserDuration) -> Self {
		crate::lyxal_core_db::expr::user::UserDuration {
			token: v.token.into(),
			session: v.session.into(),
		}
	}
}
impl From<crate::lyxal_core_db::expr::user::UserDuration> for UserDuration {
	fn from(v: crate::lyxal_core_db::expr::user::UserDuration) -> Self {
		UserDuration {
			token: v.token.into(),
			session: v.session.into(),
		}
