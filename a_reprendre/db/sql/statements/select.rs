...
impl ToSql for SelectStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		...
		if !matches!(self.version, Expr::Literal(Literal::None)) {
			write_sql!(f, fmt, " VERSION {}", &self.version);
		}
...
impl From<SelectStatement> for crate::lyxal_core_db::expr::statements::SelectStatement {
...
impl From<crate::lyxal_core_db::expr::statements::SelectStatement> for SelectStatement {
	fn from(v: crate::lyxal_core_db::expr::statements::SelectStatement) -> Self {
		Self {
			fields: v.fields.into(),
			omit: v.omit.into_iter().map(Into::into).collect(),
			only: v.only,
