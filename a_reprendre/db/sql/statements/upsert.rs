...
impl From<UpsertStatement> for crate::lyxal_core_db::expr::statements::UpsertStatement {
...
impl From<crate::lyxal_core_db::expr::statements::UpsertStatement> for UpsertStatement {
	fn from(v: crate::lyxal_core_db::expr::statements::UpsertStatement) -> Self {
		Self {
			only: v.only,
			what: v.what.into_iter().map(From::from).collect(),
			with: v.with.map(Into::into),
