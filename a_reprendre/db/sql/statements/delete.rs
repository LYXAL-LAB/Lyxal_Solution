...
use crate::lyxal_core_db::sql::{Cond, Explain, Expr, Literal, Output, With};
...
impl From<DeleteStatement> for crate::lyxal_core_db::expr::statements::DeleteStatement {
	fn from(v: DeleteStatement) -> Self {
		crate::lyxal_core_db::expr::statements::DeleteStatement {
			only: v.only,
...
impl From<crate::lyxal_core_db::expr::statements::DeleteStatement> for DeleteStatement {
	fn from(v: crate::lyxal_core_db::expr::statements::DeleteStatement) -> Self {
		DeleteStatement {
			only: v.only,
			what: v.what.into_iter().map(From::from).collect(),
			with: v.with.map(Into::into),
