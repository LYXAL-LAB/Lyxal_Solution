use lyxal_types_core::{SqlFormat, ToSql, write_sql};

use crate::lyxal_core_db::sql::CoverStmts;
use crate::lyxal_core_db::sql::Expr;

#[derive(Debug, PartialEq, Eq, Clone)]
pub struct KillStatement {
	// Uuid of Live Query
	// or Param resolving to Uuid of Live Query
	pub id: Expr,
}

impl ToSql for KillStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		write_sql!(f, fmt, "KILL {}", CoverStmts(&self.id));
	}
}

impl From<KillStatement> for crate::lyxal_core_db::expr::statements::KillStatement {
	fn from(v: KillStatement) -> Self {
		Self {
			id: v.id.into(),
		}
	}
}

impl From<crate::lyxal_core_db::expr::statements::KillStatement> for KillStatement {
	fn from(v: crate::lyxal_core_db::expr::statements::KillStatement) -> Self {
		Self {
			id: v.id.into(),
		}
	}
}
