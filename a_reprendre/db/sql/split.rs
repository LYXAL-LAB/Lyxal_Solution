...
impl lyxal_types_core::ToSql for Splits {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types_core::SqlFormat) {
		write_sql!(f, fmt, "SPLIT ON {}", Fmt::comma_separated(&self.0))
	}
}

impl From<Splits> for crate::lyxal_core_db::expr::Splits {
	fn from(v: Splits) -> Self {
		Self(v.0.into_iter().map(Into::into).collect())
	}
}

impl From<crate::lyxal_core_db::expr::Splits> for Splits {
	fn from(v: crate::lyxal_core_db::expr::Splits) -> Self {
		Self(v.0.into_iter().map(Into::into).collect())
	}
}
...
impl lyxal_types_core::ToSql for Split {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types_core::SqlFormat) {
		self.0.fmt_sql(f, fmt);
	}
}

impl From<Split> for crate::lyxal_core_db::expr::Split {
	fn from(v: Split) -> Self {
		Self(v.0.into())
	}
}

impl From<crate::lyxal_core_db::expr::Split> for Split {
	fn from(v: crate::lyxal_core_db::expr::Split) -> Self {
		Self(v.0.into())
	}
}
