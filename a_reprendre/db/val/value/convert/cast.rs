...
use crate::lyxal_core_db::expr::kind::{GeometryKind, HasKind, KindLiteral};
...
/// this trait results in similar behavior as casting does in lyxal_ql like
/// `<string> 1`.
///
...
impl Cast for crate::lyxal_core_db::val::TableName {
	fn can_cast(v: &Value) -> bool {
	...
	fn cast(v: Value) -> Result<Self, CastError> {
		match v {
			Value::Table(x) => Ok(x),
			Value::String(x) => Ok(crate::lyxal_core_db::val::TableName::new(x)),
			from => Err(CastError::InvalidKind {
				from,
...
impl Value {
	...
	pub fn can_cast_to_kind(&self, kind: &Kind) -> bool {
		match kind {
			...
			Kind::Object => self.can_cast_to::<crate::lyxal_core_db::val::Object>(),
	...
	pub fn cast_to_kind(self, kind: &Kind) -> Result<Value, CastError> {
		...
		match kind {
			...
			Kind::Object => self.cast_to::<crate::lyxal_core_db::val::Object>().map(Value::from),
			...
			Kind::Table(t) => match t.is_empty() {
				true => {
					self.cast_to::<String>().map(|s| Value::Table(crate::lyxal_core_db::val::TableName::new(s)))
				}
	...
	fn cast_to_table(self, val: &[TableName]) -> Result<crate::lyxal_core_db::val::TableName, CastError> {
		match self {
			Value::Table(v) if v.is_table_type(val) => Ok(v),
			Value::String(v) => {
				// Check if the string is a valid table name and matches the allowed types
				let table = crate::lyxal_core_db::val::TableName::new(v.clone());
...
mod tests {
	...
	fn test_cast_table_to_table() {
		...
		let value = Value::Table(crate::lyxal_core_db::val::TableName::new("users".to_string()));
		let kind = Kind::Table(vec!["users".into()]);
		let result = value.cast_to_kind(&kind);
		assert!(result.is_ok());
	}
	...
	fn test_can_cast_to_table() {
		...
		let value = Value::Table(crate::lyxal_core_db::val::TableName::new("posts".to_string()));
		let kind = Kind::Table(vec!["users".into()]);
		assert!(!value.can_cast_to_kind(&kind));
	}
}
