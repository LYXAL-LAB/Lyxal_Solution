...
use crate::lyxal_core_db::dbs::{Options, Variables};
use crate::lyxal_core_db::doc::CursorDoc;
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::{ClosureExpr, Expr, FlowResultExt, Kind, Param};
use crate::lyxal_core_functions::args::Any;
...
impl SerializeRevisioned for Closure {
	fn serialize_revisioned<W: std::io::Write>(
		...
		Err(lyxal_revision::Error::Conversion("Closures cannot be stored on disk".to_string()))
	}
}

impl DeserializeRevisioned for Closure {
	fn deserialize_revisioned<R: std::io::Read>(_reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		Err(lyxal_revision::Error::Conversion("Closures cannot be deserialized from disk".to_string()))
	}
}
