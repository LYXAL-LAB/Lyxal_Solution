...
use crate::lyxal_core_db::val::{IndexFormat, Value};
...
impl TryFrom<Set> for lyxal_types_core::Set {
	type Error = anyhow::Error;

	fn try_from(s: Set) -> Result<Self, Self::Error> {
		Ok(lyxal_types_core::Set::from(
			s.0.into_iter().map(lyxal_types_core::Value::try_from).collect::<Result<Vec<_>, _>>()?,
		))
	}
}

impl From<lyxal_types_core::Set> for Set {
	fn from(s: lyxal_types_core::Set) -> Self {
