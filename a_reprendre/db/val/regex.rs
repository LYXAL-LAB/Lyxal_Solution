...
use crate::lyxal_core_config::cnf::{REGEX_CACHE_SIZE, REGEX_SIZE_LIMIT};

pub(crate) const REGEX_TOKEN: &str = "$lyxal_core::private::Regex";
...
impl From<lyxal_types_core::Regex> for Regex {
	fn from(v: lyxal_types_core::Regex) -> Self {
		Self(v.into_inner())
	}
}

impl From<Regex> for lyxal_types_core::Regex {
	fn from(x: Regex) -> Self {
		lyxal_types_core::Regex::from(x.0)
	}
}
