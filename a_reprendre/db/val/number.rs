//! Numeric value type used throughout Lyxal.
//!
...
use crate::lyxal_core_db::expr::decimal::DecimalLexEncoder;
use crate::lyxal_core_functions::util::math::ToFloat;
use crate::lyxal_core_db::val::{TryAdd, TryDiv, TryFloatDiv, TryMul, TryNeg, TryPow, TryRem, TrySub};
...
impl From<lyxal_types_core::Number> for Number {
	fn from(v: lyxal_types_core::Number) -> Self {
		match v {
			lyxal_types_core::Number::Int(i) => Self::Int(i),
			lyxal_types_core::Number::Float(f) => Self::Float(f),
			lyxal_types_core::Number::Decimal(d) => Self::Decimal(d),
		}
	}
}

impl From<Number> for lyxal_types_core::Number {
