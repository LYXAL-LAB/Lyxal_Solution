...
use crate::lyxal_core_db::doc::{CursorDoc, Document};
use crate::lyxal_core_error::Error;
use crate::lyxal_core_db::expr::data::Assignment;
use crate::lyxal_core_db::expr::dir::Dir;
use crate::lyxal_core_db::expr::lookup::LookupKind;
use crate::lyxal_core_db::expr::paths::{IN, OUT};
use crate::lyxal_core_db::expr::reference::ReferenceDeleteStrategy;
use crate::lyxal_core_db::expr::statements::{DeleteStatement, UpdateStatement};
use crate::lyxal_core_db::expr::{AssignOperator, Data, Expr, FlowResultExt as _, Idiom, Literal, Lookup, Part};
use crate::lyxal_core_db::idx::planner::ScanDirection;
use crate::lyxal_core_db::key::r#ref::Ref;
...
impl Document {
	...
	async fn purge_edges(
		...
		if self.initial.doc.is_edge() {
			...
			let key = crate::lyxal_core_db::key::graph::new(ns, db, &l.table, &l.key, o, rid);
			txn.del(&key).await?;
			// Purge the left inner edge
			let key = crate::lyxal_core_db::key::graph::new(ns, db, &rid.table, &rid.key, i, l);
			txn.del(&key).await?;
			// Purge the right inner edge
			let key = crate::lyxal_core_db::key::graph::new(ns, db, &rid.table, &rid.key, o, r);
			txn.del(&key).await?;
			// Purge the right pointer edge
			let key = crate::lyxal_core_db::key::graph::new(ns, db, &r.table, &r.key, i, rid);
			txn.del(&key).await?;
	...
	async fn purge_references(
		...
		let prefix = crate::lyxal_core_db::key::r#ref::prefix(ns, db, &rid.table, &rid.key)?;
		let suffix = crate::lyxal_core_db::key::r#ref::suffix(ns, db, &rid.table, &rid.key)?;
