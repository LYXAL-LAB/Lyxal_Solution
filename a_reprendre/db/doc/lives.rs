...
use crate::lyxal_core_db::catalog::{Permission, SubscriptionDefinition, SubscriptionFields};
use crate::lyxal_core_db::ctx::{Context, FrozenContext};
use crate::lyxal_core_db::dbs::{MessageBroker, Options, Statement};
...
use crate::lyxal_core_db::expr::paths::{AC, ID, RD, TK};
use crate::lyxal_core_kvs::Transaction;
use crate::types::{PublicAction, PublicNotification};
use crate::lyxal_core_db::val::{Value, convert_value_to_public_value};
...
impl Document {
	...
	async fn lq_compute(
		...
		let session_id = match sess.pick(ID.as_ref()) {
			...
			Value::String(s) => s.parse::<crate::lyxal_core_db::val::Uuid>().ok().map(|uuid| uuid.into()),
			_ => None,
		};
	...
	async fn lq_allow(
		...
		if opt.check_perms(crate::lyxal_core_db::iam::Action::View)? {
			// Get the table
