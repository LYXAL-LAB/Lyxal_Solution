...
use crate::lyxal_core_db::expr::statements::define::DefineConfigStatement;
use crate::lyxal_core_db::expr::statements::{
	...
	UpdateStatement, UpsertStatement, DefineConnectorStatement
};
use crate::lyxal_core_db::expr::visit::{MutVisitor, VisitMut};
use crate::lyxal_core_db::expr::{Expr, Field, Fields, Function, Groups, Idiom, Part, SelectStatement};
use crate::lyxal_core_db::val::{Array, Datetime, Number, Object, TryAdd as _, TryFloatDiv, TryMul, Value};
...
impl MutVisitor for AggregateExprCollector<'_> {
    ...
    /// DefineConnectorStatement ///
	fn visit_mut_define_connector(
		&mut self,
		d: &mut DefineConnectorStatement,
	) -> Result<(), Self::Error> {
		self.visit_mut_expr(&mut d.base_url)?;
		if let Some(comment) = d.comment.as_mut() {
			self.visit_mut_expr(comment)?;
		}
		Ok(())
	}
...
impl MutVisitor for ParentRewritor {
    ...
    /// DefineConnectorStatement ///
	fn visit_mut_define_connector(&mut self,d: &mut DefineConnectorStatement,) -> Result<(), Self::Error> {
		self.visit_mut_expr(&mut d.base_url)?;
		if let Some(comment) = d.comment.as_mut() {
			self.visit_mut_expr(comment)?;
		}
