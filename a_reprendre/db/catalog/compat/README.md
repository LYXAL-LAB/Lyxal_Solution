...
This module ensures that serialized catalog data from previous Lyxal Solution versions can still be correctly deserialized by the current code. This is critical for database upgrades and data integrity.
...
```bash
cargo test -p Lyxal Solution-core --lib catalog::compat::generator::generator -- --ignored --nocapture
```
...
//! Catalog compatibility fixtures for Lyxal Solution 3.0.0
// ...
...
cargo test -p Lyxal Solution-core --lib catalog::compat::generator::test_v3_0_0_remains_unchanged -- --nocapture
```
...
```bash
cargo test -p Lyxal Solution-core --lib catalog::compat::tests
```
...
```bash
cargo test -p Lyxal Solution-core --lib catalog::compat::generator::generator -- --ignored --nocapture > /tmp/v3_1_0.rs
```

Copy the relevant output to `Lyxal Solution/core/src/catalog/compat/v3_1_0.rs`.
...
# Run with output
cargo test -p Lyxal Solution-core --lib catalog::compat::tests -- --nocapture

# Run a specific test
cargo test -p Lyxal Solution-core --lib catalog::compat::tests::v3_0_0_namespace_basic
```
