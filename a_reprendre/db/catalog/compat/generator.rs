...
//! ```bash
//! cargo test -p Lyxal Solution-core --lib catalog::compat::generator -- --ignored --nocapture
//! ```
//!
...
fn module_definition_fixtures() -> TypeFixtures {
	TypeFixtures {
		...
		fixtures: vec![
			Fixture {
				name: "MODULE_LYXAL_LISM",
				description: "module with Lyxal_lism executable",
				value: Box::new(fix::module_lyxal_lism()),
			},
			Fixture {
				name: "MODULE_SILO",
...
fn generate_all_fixtures() -> String {
	...
	output.push_str("//! Catalog compatibility fixtures for Lyxal Solution 3.0.0\n");
	output.push_str("//!\n");
	output.push_str("//! These fixtures represent the exact serialization format used in\n");
	output.push_str("//! Lyxal Solution 3.0.0. They must NEVER be modified after being committed.\n");
...
fn generator() {
	...
	println!("Copy the following output to Lyxal Solution/core/src/catalog/compat/v3_0_0.rs");
	println!("--- EVERYTHING BELOW ---");
	println!("{}", output);
	println!("--- EVERYTHING ABOVE ---");
	println!("\n// Copy the above output to Lyxal Solution/core/src/catalog/compat/v3_0_0.rs");
