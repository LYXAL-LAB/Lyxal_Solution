...
use crate::lyxal_core_db::catalog::ModuleDefinition;
...
impl InfoStructure for ModuleExecutable {
	fn structure(self) -> Value {
		match self {
			ModuleExecutable::Lyxal_lism(lyxal_lism) => lyxal_lism.structure(),
...
#[revisioned(revision = 1)]
#[derive(Clone, Debug, Eq, PartialEq, Hash)]
pub(crate) struct Lyxal_lismExecutable {
	pub bucket: String,
	pub key: String,
}

impl InfoStructure for Lyxal_lismExecutable {
	fn structure(self) -> Value {
		Value::from(map! {
			"type".to_string() => Value::from("lyxal_lism"),
...
impl<'a> TryFrom<&'a crate::lyxal_core_db::expr::statements::DefineModuleStatement> for ModuleName {
	type Error = anyhow::Error;
	fn try_from(
		value: &'a crate::lyxal_core_db::expr::statements::DefineModuleStatement,
	) -> Result<Self, Self::Error> {
		if let Some(name) = &value.name {
			Ok(ModuleName::Module(name.clone()))
		} else if let crate::lyxal_core_db::expr::ModuleExecutable::Silo(silo) = &value.executable {
			Ok(ModuleName::Silo(
				silo.organisation.clone(),
				silo.package.clone(),
				silo.major,
