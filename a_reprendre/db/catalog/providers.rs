...
pub(crate) trait NamespaceProvider {
	...
	/// dynamic mode.
	#[instrument(level = "trace", target = "Lyxal Solution::core::kvs::tx", skip(self, ctx))]
...
/// Connector data access provider.
#[cfg_attr(target_family = "wasm", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_family = "wasm"), async_trait::async_trait)]
pub(crate) trait ConnectorProvider {
    async fn all_db_connectors(
        &self,
        ns: NamespaceId,
        db: DatabaseId,
    ) -> Result<Arc<[catalog::ConnectorDefinition]>>;

    async fn get_db_connector(
        &self,
        ns: NamespaceId,
        db: DatabaseId,
        cn: &str,
    ) -> Result<Option<Arc<catalog::ConnectorDefinition>>>;

    async fn put_db_connector(
        &self,
        ns: NamespaceId,
        db: DatabaseId,
        cn: &catalog::ConnectorDefinition,
    ) -> Result<()>;

    async fn del_db_connector(
        &self,
        ns: NamespaceId,
        db: DatabaseId,
        cn: &str,
