...
use crate::lyxal_core_db::catalog::{DatabaseId, NamespaceId, Permissions, ViewDefinition};
use crate::lyxal_core_db::expr::statements::info::InfoStructure;
use crate::lyxal_core_db::expr::{ChangeFeed, Kind};
...
use crate::lyxal_core_db::sql::statements::DefineTableStatement;
...
impl Relation {
	fn rev_convert_from(&mut self, _rev: u16, value: Option<Kind>) -> Result<(), lyxal_revision::Error> {
		if let Some(x) = value {
			let Kind::Record(x) = x else {
				return Err(lyxal_revision::Error::Conversion(format!(
	...
	fn rev_convert_to(&mut self, _rev: u16, value: Option<Kind>) -> Result<(), lyxal_revision::Error> {
		if let Some(x) = value {
			let Kind::Record(x) = x else {
				return Err(lyxal_revision::Error::Conversion(format!(
					"Invalid kind within table relation, should have been a record, found: {:#?}",
