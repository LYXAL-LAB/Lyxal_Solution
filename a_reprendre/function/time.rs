...
use crate::lyxal_core_db::val::{Datetime, Duration, Number, Value};
use crate::fail;
...
pub mod from {
	...
	use crate::lyxal_core_error::Error;
	use crate::lyxal_core_db::val::{Datetime, Uuid, Value};
		...
		match val.0.get_timestamp() {
			Some(v) => {
				...
				match Datetime::try_from((s as i64, ns)) {
					Ok(v) => Ok(v.into()),
					_ => Err(anyhow::anyhow!("Failed to convert UUID Timestamp to Datetime.")),
				}
			}
			None => Err(anyhow::Error::new(Error::InvalidFunctionArguments {
