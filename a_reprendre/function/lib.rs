                    ...
                    $function_name => {
                        $(
                            if !$ctx.get_capabilities().allows_experimental(&ExperimentalTarget::$exp_target) {
                                return Err(::anyhow::Error::new($crate::lyxal_core_error::Error::InvalidFunction {
...
pub async fn asynchronous(
	...
	fn cpu_intensive<R: Send + 'static>(
		...
		|| Box::pin(crate::lyxal_core_db::exe::spawn(function))
	}
	...
	dispatch!(
		...
		//
		"connector::call" => connector::call((stk, ctx, opt)).await,
		"connector::list" => connector::list((stk, ctx, opt)).await,
		"connector::info" => connector::info((stk, ctx, opt)).await,
		"connector::health" => connector::health((stk, ctx, opt)).await,
		"connector::batch" => connector::batch((stk, ctx, opt)).await,
		//
...
mod tests {
	...
	use crate::lyxal_core_db::sql::{Expr, Function};
	...
	async fn implementations_are_present() {
		...
		for line in fnc_no_idiom.lines() {
			...
			let res =
				crate::lyxal_core_db::val::expr_with_capabilities(&format!("{}()", name), &Capabilities::all());
			...
			#[cfg(all(feature = "scripting", feature = "kv-mem"))]
			{
				...
				let dbs = crate::lyxal_core_kvs::Datastore::new("memory")
					.await
					.unwrap()
					.with_capabilities(Capabilities::all());
				let ses = crate::lyxal_core_db::dbs::Session::owner().with_ns("test").with_db("test");
