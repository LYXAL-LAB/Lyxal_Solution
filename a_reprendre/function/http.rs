...
fn try_as_uri(fn_name: &str, value: Value) -> Result<String> {
	match value {
		// Pre-check URI.
		Value::String(uri) if crate::lyxal_core_functions::util::http::uri_is_valid(&uri) => Ok(uri),
...
fn try_as_opts(
	...
	error_message: &str,
	value: Option<Value>,
) -> Result<Option<crate::lyxal_core_db::val::Object>> {
	match value {
...
pub async fn head(
	...
	crate::lyxal_core_functions::util::http::head(ctx, uri, opts).await
}
...
pub async fn get(
	...
	crate::lyxal_core_functions::util::http::get(ctx, uri, opts).await
}
...
pub async fn put(
	...
	crate::lyxal_core_functions::util::http::put(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub async fn post(
	...
	crate::lyxal_core_functions::util::http::post(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub async fn patch(
	...
	crate::lyxal_core_functions::util::http::patch(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub async fn delete(
	...
	crate::lyxal_core_functions::util::http::delete(ctx, uri, opts).await
}
	...
	impl Resolve for FilteringResolver {
		fn resolve(&self, name: Name) -> Resolving {
			...
			Box::pin(async move {
				...
				if !name_is_allowed {
					return Err(Box::new(crate::lyxal_core_error::Error::NetTargetNotAllowed(
				...
				{
					return Err(
						Box::new(crate::lyxal_core_error::Error::NetTargetNotAllowed(denied.to_string()))
							as Box<dyn Error + Send + Sync>,
					);
				}
