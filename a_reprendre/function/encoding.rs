...
pub mod cbor {
	...
	use crate::lyxal_core_error::Error;
	use crate::lyxal_core_rpc::format::cbor;
	use crate::lyxal_core_db::val::{Bytes, Value};

	pub fn encode((arg,): (Value,)) -> Result<Value> {
		// Convert internal value to public, encode, then convert back
		let public_val = crate::lyxal_core_db::val::convert_value_to_public_value(arg)?;
	...
	pub fn decode((arg,): (Bytes,)) -> Result<Value> {
		...
		Ok(crate::lyxal_core_db::sql::expression::convert_public_value_to_internal(public_val))
	}
}
