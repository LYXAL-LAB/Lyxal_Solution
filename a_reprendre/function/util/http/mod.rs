...
use crate::lyxal_core_config::cnf::LYXALDB_USER_AGENT;
...
impl HttpClientManager {
	...
	async fn get_or_create_client(
		&self,
		capabilities: Arc<crate::lyxal_core_db::dbs::Capabilities>,
		redirect_checker: Option<
			impl Fn(&Url) -> Result<(), crate::lyxal_core_error::Error> + Send + Sync + 'static,
		>,
	) -> Result<Arc<Client>> {
	...
	fn hash_capabilities(&self, capabilities: &crate::lyxal_core_db::dbs::Capabilities) -> u64 {
...
async fn request(
	...
	let body = match body {
		Some(v) => Some(crate::lyxal_core_db::val::convert_value_to_public_value(v)?),
		None => None,
	};
	...
	let cli = {
		...
		let redirect_checker = move |url: &Url| -> Result<(), crate::lyxal_core_error::Error> {
			...
			let target = NetTarget::from_str(url.host_str().unwrap_or(""))
				.map_err(|e| crate::lyxal_core_error::Error::InvalidUrl(format!("Invalid host: {}", e)))?;

			if !capabilities_clone.matches_any_allow_net(&target)
				|| capabilities_clone.matches_any_deny_net(&target)
			{
				return Err(crate::lyxal_core_error::Error::NetTargetNotAllowed(url.to_string()));
			}
	...
	if cfg!(not(target_family = "wasm")) {
		req = req.header(reqwest::header::USER_AGENT, &*LYXALDB_USER_AGENT);
	}
