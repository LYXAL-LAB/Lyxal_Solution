...
use super::modules::lyxal::query::QueryContext;
use super::modules::{loader, resolver};
use super::{classes, fetch, globals, modules};
use crate::lyxal_core_config::cnf::{SCRIPTING_MAX_MEMORY_LIMIT, SCRIPTING_MAX_STACK_SIZE};
...
pub async fn run(
	...
	let time_limit = Duration::from_millis(*crate::lyxal_core_config::cnf::SCRIPTING_MAX_TIME_LIMIT as u64);
	...
	async_with!(ctx => |ctx| {
		let res = async {
			...
			fetch::register(&ctx)?;
			// Register the lyxal module as a global object
			let (module, promise) = Module::evaluate_def::<modules::lyxal::Package, _>(ctx.clone(), "lyxal")?;
			promise.finish::<()>()?;
			global.set("lyxal",
			...
			// Register the special Lyxal types as classes
			classes::init(&ctx)?;
