...
use super::classes;
use crate::lyxal_core_db::val::{Geometry, Number, Value};
				...
				if v.is_integer() {
					if let Some(v) = v.to_i64() {
						...
						Err(Exception::from_message(
							ctx.clone(),
							"Couldn't convert Lyxal_QL Decimal to JavaScript number",
						)?
						.throw())
					}
					...
					Err(Exception::from_message(
						ctx.clone(),
						"Couldn't convert Lyxal_QL Decimal to a JavaScript number",
					)?
					.throw())
				}
			}
