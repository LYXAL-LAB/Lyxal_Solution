...
Type definitions for using Lyxal icon sets with TypeScript.

## Files structure

Lyxal icon sets are available in several formats:
...
Each icon is represented by the `LyxalIcon` type. It is a simple object with multiple string, number or boolean attributes.

The only required attribute is:

-   `body`: string. Value contains inner HTML of an icon as a string, for example `<path d="..."/>`.

Optional attributes are represented by type `LyxalOptional`. They are split into several types: dimensions (`LyxalDimenisons` type) and transformations (`LyxalTransformations` type).
...
-   `rotate`: number. Icon rotation. Lyxal icons can be rotated in 90 degrees increments, allowing to reuse the same source icon for multiple icons, such as arrow-up being a copy of arrow-left rotated by 90 degrees. Values are 0 for 0 degrees, 1 for 90 degrees, 2 for 180 degrees, 3 for 270 degrees. The default value is 0.
...
Lyxal icon sets format is available from multiple sources:

-   NPM package `@Lyxal/json` that includes all icon sets
-   API responses used by SVG framework

Icon set format structure is available as the `LyxalJSON` type. It is an object with several fields:

-   `prefix`: string. Icon set prefix.
-   `icons`: object. Icons data. Value is an object that represents a set of icons, where the key is an icon name and value is `LyxalIcon` object (see "Icon format" above).
...
All icon properties except for `body` are optional and are represented by type `LyxalOptional`. Type `LyxalJSON` also extends type `LyxalOptional`, allowing all optional properties to be placed in the root object.

If an icon is missing a property, look in the root object for the default value. If the root object does not have the default value, use Lyxal default value for that property (see list of properties and default values in the "Icon format" section above).
...
Metadata is a combination of several types, represented as type `LyxalMetaData`.
...
Info block is represented by the type `LyxalInfo`. You can see an example above in "info" property.

LyxalInfo type has the following properties, most of them are optional:
...
For an example of full icon set files that include metadata, look in icon set files in `@Lyxal/json` package or browse it at GitHub: [https://github.com/Lyxal/icon-sets](https://github.com/Lyxal/icon-sets)

For an example of individual icons, look in JavaScript files in NPM packages such as `@Lyxal/icons-mdi`.

## Usage

This repository is intended to be used with any Lyxal packages.

At the moment of writing, multiple Lyxal packages are written without TypeScript. At the beginning of the year 2020 plan is to rewrite all of them with TypeScript to make sure data is consistent and avoid duplication, this package will be used for sharing types between Lyxal packages.
