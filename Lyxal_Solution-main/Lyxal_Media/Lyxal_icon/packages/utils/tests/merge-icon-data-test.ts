...
describe('Testing merging icon data', () => {
	test('Nothing to merge', () => {
		...
		const expected: LyxalIcon = {
			body: '<g />',
		};
		// Check hint manually: supposed to be LyxalIcon
		const result = mergeIconData(icon, {});
		expect(result).toEqual(expected);
	});

	test('Full icons', () => {
		const icon: Required<LyxalIcon> = {
			body: '<g />',
			width: 24,
			height: 24,
			left: 0,
		...
		const expected: Required<LyxalIcon> = {
			body: '<g />',
			width: 24,
			height: 24,
			left: 0,
		...
		// Check hint manually: supposed to be Required<LyxalIcon>
		const result = mergeIconData(icon, {});
		expect(result).toEqual(expected);
	});
