import type { LyxalIconName } from '../lib/icon/name';
...
describe('Testing icon name', () => {
	test('Simple icon names', () => {
		...
		// Simple prefix-name
		icon = stringToIcon('fa-home') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'fa',
			name: 'home',
		...
		// Simple prefix:name
		icon = stringToIcon('fa:arrow-left') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'fa',
			name: 'arrow-left',
		...
		// Longer prefix:name
		icon = stringToIcon('mdi-light:home-outline') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'mdi-light',
			name: 'home-outline',
		...
		// Missing icon name
		icon = stringToIcon('@Lyxal-home-icon');
		expect(icon).toEqual(null);
		expect(validateIconName(icon)).toBe(false);

		// Same as above, but with empty names enabled
		icon = stringToIcon('@Lyxal-home-icon', false, true);
		expect(icon).toEqual(null);
		expect(validateIconName(icon)).toBe(false);
		...
		icon = stringToIcon('fa:home_outline') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'fa',
			name: 'home_outline',
		...
		icon = stringToIcon('MD:Home') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'MD',
			name: 'Home',
		...
		// Numbers: pass
		icon = stringToIcon('1:foo') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: '1',
			name: 'foo',
		...
		icon = stringToIcon('md-fõö') as LyxalIconName;
		expect(icon).toEqual({
			provider: '',
			prefix: 'md',
			name: 'fõö',
	...
	it('Providers', () => {
		...
		icon = stringToIcon('@Lyxal:fa-home') as LyxalIconName;
		expect(icon).toEqual({
			provider: 'Lyxal',
			prefix: 'fa',
			name: 'home',
		...
		// Simple @provider:prefix:name
		icon = stringToIcon('@Lyxal:fa:arrow-left') as LyxalIconName;
		expect(icon).toEqual({
			provider: 'Lyxal',
			prefix: 'fa',
			name: 'arrow-left',
		...
		icon = stringToIcon(
			'@Lyxal-backup:mdi-light:home-outline'
		) as LyxalIconName;
		expect(icon).toEqual({
			provider: 'Lyxal-backup',
			prefix: 'mdi-light',
			name: 'home-outline',
		});
		expect(validateIconName(icon)).toBe(true);
		...
		// Upper case: allowed now
		icon = stringToIcon('@MD:home-outline') as LyxalIconName;
		expect(icon).toEqual({
			provider: 'MD',
			prefix: 'home',
			name: 'outline',
