...
const attribsRegex = /([\w:-]+)="([^"]*)"/g;

// Regex to find tag name
...
export function parseXMLContent(
	...
	do {
		...
		if (tagContent.startsWith('/')) {
			...
			const tagNameMatch = tagContent.slice(1).match(tagNameRegex);
		...
		// Parse tag name
		const tagNameMatch = tagContent.match(tagNameRegex);
		if (!tagNameMatch) {
			// Something is wrong
			return null;
		}
		...
		let match;
		attribsRegex.lastIndex = 0;
		while ((match = attribsRegex.exec(tagContent))) {
