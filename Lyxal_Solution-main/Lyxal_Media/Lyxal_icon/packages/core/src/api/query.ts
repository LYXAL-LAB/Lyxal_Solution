...
import { initRedundancy } from '@Lyxal/api-redundancy';
import {
	getAPIModule,
	type LyxalAPIQueryParams,
	type LyxalAPISendQuery,
} from './modules';
import {
	createAPIConfig,
	type LyxalAPIConfig,
	type PartialLyxalAPIConfig,
} from './config';
...
interface LyxalAPIInternalStorage {
	config: LyxalAPIConfig;
	redundancy: Redundancy;
}
const redundancyCache = Object.create(null) as Record<
	string,
	LyxalAPIInternalStorage
>;
...
function getRedundancyCache(
	provider: string
): LyxalAPIInternalStorage | undefined {
...
export function sendAPIQuery(
	...
	let send: LyxalAPISendQuery | undefined;
