...
import {
	...
	matchIconName,
	stringToIcon,
} from '@Lyxal/utils/lib/icon/name';
 ...
 */
export type LyxalIconLoaderAbort = () => void;
 ...
 */
export type LyxalIconLoaderCallback = (
	loaded: LyxalIconName[],
	missing: LyxalIconName[],
	pending: LyxalIconName[],
	unsubscribe: LyxalIconLoaderAbort
) => void;

/**
 * Function to load icons
 */
export type LyxalLoadIcons = (
 ...
 */
export type IsPending = (icon: LyxalIconName) => boolean;
...
function parseLoaderResponse(
	...
	if (data && typeof data === 'object') {
		...
		try {
			const parsed = addIconSet(storage, data as LyxalJSON);
			if (!parsed.length) {
				checkMissing();
				return;
			}
			...
			if (customIconLoader) {
				icons.forEach((name) => {
					...
					parsePossiblyAsyncResponse(response, (data) => {
						const iconSet: LyxalJSON | null = data
							? {
									prefix,
									icons: {
										[name]: data,
 ...
 */
export const isPending: IsPending = (icon: LyxalIconName): boolean => {
 ...
 * Load icons
 */
export const loadIcons: LyxalLoadIcons = (
	icons: (LyxalIconName | string)[],
	callback?: LyxalIconLoaderCallback
): LyxalIconLoaderAbort => {
