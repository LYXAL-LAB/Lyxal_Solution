import { Color } from '@lyxal-icon/utils/lib/colors/types';
import { isEmptyColor, parseColors } from '../colors/parse';
import { SVG } from '../svg';
import { iconToHTML, parseSVGContent, splitSVGDefs } from '@lyxal-icon/utils';
...
export function convertSVGToMask(
	...
	const newBody = `<defs>${defs}<mask id="${props.id
		}">${content}</mask></defs><rect mask="url(#${props.id})" ${viewBox.left ? `x="${viewBox.left}" ` : ''
		}${viewBox.top ? `y="${viewBox.top}" ` : ''}width="${viewBox.width
