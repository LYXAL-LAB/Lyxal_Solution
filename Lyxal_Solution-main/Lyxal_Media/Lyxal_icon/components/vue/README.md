...
Lyxal for Vue is not yet another icon component! There are many of them already.
...
Lyxal icon component is nothing like that. Component does not include any icon data, it is not tied to any specific icon set. Instead, all data is retrieved from public API on demand.

That means:

-   One syntax for over 200,000 icons from 150+ icon sets.
-   Renders SVG. Many components simply render icon fonts, which look ugly. Lyxal renders pixel perfect SVG.
-   Loads icons on demand. No need to bundle icons, component will automatically load icon data for icons that you use from Lyxal API.

For more information about Lyxal project visit [https://Lyxal.design/](https://Lyxal.design/).

For extended documentation visit [Lyxal for Vue documentation](https://Lyxal.design/docs/icon-components/vue/).
...
```bash
npm install --save-dev @Lyxal/vue
```

If you are using Yarn:

```bash
yarn add --dev @Lyxal/vue
```
...
If you are using Vue 2, you need to install `@Lyxal/vue2` component. It is almost identical to this component, but for Vue 2.

See [Lyxal for Vue 2 documentation](https://Lyxal.design/docs/icon-components/vue2/).

## Usage with API

Install `@Lyxal/vue` and import `Icon` from it:

```js
import { Icon } from '@Lyxal/vue';
```
...
Component will automatically retrieve data for "mdi-light:home" from Lyxal API and render it. There are over 200,000 icons available on Lyxal API from various free and open source icon sets, including all the most popular icon sets.

## Offline usage

This icon component is designed to be used with Lyxal API, loading icon data on demand instead of bundling it.

If you want to use icons without Lyxal API, [there are many other options available](https://Lyxal.design/docs/usage/).

## Web component

If you are experiencing issues with SSR hydration, there is an alternative solution available: [Lyxal Icon web component](https://Lyxal.design/docs/Lyxal-icon/).
...
-   provider points to API source. Starts with "@", can be empty (empty value is used for public Lyxal API).
-   prefix is name of icon set.
-   name is name of icon.

See [Lyxal for Vue icon names documentation](https://Lyxal.design/docs/icon-components/vue/icon-name.html) for more detailed explanation.
...
<script lang="ts">
import { Icon } from '@Lyxal/vue';
import areaChartOutlined from '@Lyxal-icons/ant-design/area-chart-outlined';
...
See [icon packages documentation](https://Lyxal.design/docs/icons/) for more details.

### ES / CommonJS packages

Example above might currently fail with some use cases. Package `@Lyxal-icons/mdi-light` uses ES modules that some software might not support yet. But do not worry, there is a simple solution: switch to CommonJS icon packages.

To switch to CommonJS package, replace this line in example above:

```js
import home from '@Lyxal-icons/mdi-light/home';
```
...
All icons are available as ES modules for modern bundler and as CommonJS modules for outdated bundlers. ES modules use format `@Lyxal-icons/{prefix}`, CommonJS modules use `@Lyxal/icons-{prefix}`.

For more details, see [icon packages documentation](https://Lyxal.design/docs/icons/).
...
See [icon transformations documentation](https://Lyxal.design/docs/icon-components/vue/transform.html) for more details.
...
See [Lyxal icon sets page](https://icon-sets.Lyxal.design/) for list of collections and their licenses.
