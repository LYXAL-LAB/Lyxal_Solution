...
import { mergeCustomisations } from '@lyxal-icon/utils/lib/customisations/merge';
import { flipFromString } from '@lyxal-icon/utils/lib/customisations/flip';
import { rotateFromString } from '@lyxal-icon/utils/lib/customisations/rotate';
 ...
 */
const inlineDefaults: Required<LyxalIconCustomisations> = {
...
/**
 * Render icon
 * @param icon Icon must be validated before calling this function
 * @param props Partial properties
...
export const render = (
	...
	// Check mode
	const mode: LyxalRenderMode = props.mode || 'svg';
	...
	if (name) {
		...
		if (iconName) {
			...
			for (const prop of props) {
				if (iconName[prop]) {
					classNames.push('Lyxal--' + iconName[prop]);
				}
			}
	...
	if (mode === 'svg') {
		...
		componentProps.dangerouslySetInnerHTML = {
			__html: cleanUpInnerHTML(replaceIDs(item.body)),
		};
