...
import type { LyxalIconProperties } from './attributes/types';
import { exportFunctions, LyxalExportedFunctions } from './functions';
 ...
 * Icon status
 */
export type LyxalIconStatus = 'rendered' | 'loading' | 'failed';

/**
 * Interface
 */
declare interface PartialLyxalIconHTMLElement extends HTMLElement {
	// Restart animation for animated icons
	restartAnimation: () => void;

	// Get status
	get status(): LyxalIconStatus;
}

// Add dynamically generated getters and setters
export declare interface LyxalIconHTMLElement
	extends PartialLyxalIconHTMLElement,
	// Functions added dynamically after class creation
	LyxalExportedFunctions,
	Required<LyxalIconProperties> { }

/**
 * Constructor
 */
interface PartialLyxalIconHTMLElementClass {
	new(): PartialLyxalIconHTMLElement;
	prototype: PartialLyxalIconHTMLElement;
}

export interface LyxalIconHTMLElementClass
	// Functions added dynamically as static methods and methods on instance
	extends LyxalExportedFunctions {
	new(): LyxalIconHTMLElement;
	prototype: LyxalIconHTMLElement;
}

/**
 * Register 'Lyxal-icon' component, if it does not exist
 */
export function defineLyxalIcon(
	name = 'Lyxal-icon'
): LyxalIconHTMLElementClass | undefined {
	...
	if (ConflictingClass) {
		return ConflictingClass as LyxalIconHTMLElementClass;
	}

	// All attributes
	const attributes: (keyof LyxalIconProperties)[] = [
		// Icon
		'icon',
		// Mode
		'mode',
	 ...
	 * Component class
	 */
	const LyxalIcon: PartialLyxalIconHTMLElementClass = class extends ParentClass {
		// Root
		...
		*/
		...
		set icon(value: string | LyxalIcon) {
	...
	attributes.forEach((attr) => {
		if (!(attr in LyxalIcon.prototype)) {
			Object.defineProperty(LyxalIcon.prototype, attr, {
				get: function () {
	...
	for (const key in functions) {
		LyxalIcon[key] = LyxalIcon.prototype[key] = functions[key];
	}

	// Define new component
	customElements.define(name, LyxalIcon);

	return LyxalIcon as LyxalIconHTMLElementClass;
}
