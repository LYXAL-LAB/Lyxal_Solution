...
import { defineLyxalIcon, LyxalIconHTMLElement } from '../src/component';
import type { IconState } from '../src/state';

export declare interface DebugLyxalIconHTMLElement
	extends LyxalIconHTMLElement {
...
describe('Testing icon component', () => {
	...
	it('Registering component', () => {
		...
		expect(window.customElements.get('Lyxal-icon')).toBeUndefined();

		// Define component
		const LyxalIcon = defineLyxalIcon();
		expect(LyxalIcon).toBeDefined();
		expect(window.customElements.get('Lyxal-icon')).toBeDefined();

		// Create element
		const node = document.createElement(
			'Lyxal-icon'
		) as DebugLyxalIconHTMLElement;
		expect(node instanceof LyxalIcon).toBe(true);
		expect(node.status).toBe('loading');

		// Define component again (should return previous class)
		const LyxalIcon2 = defineLyxalIcon();
		expect(LyxalIcon2).toBe(LyxalIcon);

		// Create another element
		const node2 = document.createElement(
			'Lyxal-icon'
		) as DebugLyxalIconHTMLElement;
		expect(node2 instanceof LyxalIcon).toBe(true);
	...
	it('Creating component instance, changing properties', async () => {
		...
		expect(typeof LyxalIcon.loadIcon).toBe('function');
	...
	it('Restarting animation', async () => {
		...
		// Define component
		expect(defineLyxalIcon()).toBeDefined();
		expect(window.customElements.get('Lyxal-icon')).toBeDefined();
