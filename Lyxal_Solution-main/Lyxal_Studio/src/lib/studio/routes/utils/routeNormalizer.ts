import { StudioRoute, RouteMetadata, Permission } from '../../types/route';

/**
 * Options de normalisation
 */
export interface RouteNormalizationOptions {
  generateCodes?: boolean;     // Générer automatiquement les codes si manquants
  generateSlugs?: boolean;     // Générer automatiquement les slugs si manquants
  validateReferences?: boolean; // Valider les références (pages, etc.)
  sanitizeMetadata?: boolean;   // Nettoyer les métadonnées
  enforceDefaults?: boolean;    // Appliquer les valeurs par défaut
}
...
export class RouteNormalizer {
  ...
  static normalizeRoute(
    ...
    options: RouteNormalizationOptions = {}
  ): RouteNormalizationResult {
    const result: RouteNormalizationResult = {
      route: route as StudioRoute,
      changes: [],
      warnings: [],
      errors: []
    };
    ...
    try {
      ...
      result.route.permissions = permissionsResult.permissions as Permission[];
  ...
  private static normalizeIdentity(
    identity?: Partial<StudioRoute['identity']>,
    options: RouteNormalizationOptions = {}
  ): {
    identity: StudioRoute['identity'];
    changes: string[];
    warnings: string[];
  } {
  ...
  private static normalizePermissions(permissions?: string[]): {
    ...
    const validPermissions = uniquePermissions.filter(p =>
      ['guest', 'authenticated', 'admin', 'manager'].includes(p)
    );
   ...
   */
  private static normalizeGuards(guards?: any[]): {
    guards: any[];
    changes: string[];
    warnings: string[];
  } {
   ...
   */
  private static normalizeMetadata(metadata: RouteMetadata): {
    metadata: RouteMetadata;
    changes: string[];
  } {
    const changes: string[] = [];
    const normalized: RouteMetadata = { ...metadata };
   ...
   */
  private static applyDefaults(route: StudioRoute): {
    route: StudioRoute;
    changes: string[];
  } {
    const changes: string[] = [];
   ...
   */
  private static validateRouteConsistency(route: StudioRoute): {
    warnings: string[];
    errors: string[];
  } {
    const warnings: string[] = [];
  ...
  private static generateSlugFromPath(path: string): string {
    return path
  ...
  private static generateCodeFromPath(path: string): string {
    return path
      .replace(/^\/+/, '') // Supprimer les slash initiaux
