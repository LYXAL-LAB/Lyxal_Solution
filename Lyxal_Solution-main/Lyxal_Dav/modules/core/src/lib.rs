...
impl DavContext {
    pub fn new(
        ...
        backend: std::sync::Arc<dyn crate::backend::DavBackend>,
        ...
        Self { method, path, body, headers, backend, principal, tenant }
    }
...
pub async fn process(ctx: DavContext) -> Result<DavResponse, DavError> {
    match ctx.method.as_str() {
        ...
        "PROPPATCH" => methods::proppatch::handle(ctx).await,
		...
		"MKCOL" => methods::mkcol::handle(ctx).await,
		"MOVE" => methods::r#move::handle(ctx).await,
		"COPY" => methods::copy::handle(ctx).await,
		"LOCK" => methods::lock::handle(ctx).await,
		"UNLOCK" => methods::unlock::handle(ctx).await,
        "OPTIONS" => {
            let mut resp = DavResponse::empty(StatusCode::OK);
			resp.headers.insert("Allow".into(), "OPTIONS, GET, PUT, DELETE, PROPFIND, REPORT, PROPPATCH, MKCALENDAR, MKCOL, MOVE, COPY, LOCK, UNLOCK".into());
