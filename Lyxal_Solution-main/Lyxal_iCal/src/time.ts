...
import type { weekDay, timeInit } from "./types";
...
export class Time {
    ...
    static fromDateTimeString(aValue: string, prop?: Property): Time {
        ...
        if (aValue.slice(-1) === 'Z') {
            ...
            zoneId = prop.getParameter('tzid') as string | undefined;

            if (prop.parent) {
                if ((prop.parent as any).name === 'standard' || (prop.parent as any).name === 'daylight') {
                    ...
                    zone = (prop.parent as any).getTimeZoneByID(zoneId);
                }
            }
        }

        const timeData: timeInit = {
            year: strictParseInt(aValue.slice(0, 4)),
            month: strictParseInt(aValue.slice(5, 7)),
            day: strictParseInt(aValue.slice(8, 10)),
     ...
     */
    static fromData(aData?: timeInit, aZone?: Timezone): Time {
        let t = new Time();
    ...
    private static _epochTime: Time | null = null;
    /**
     * January 1st, 1970 as an ICAL.Time.
     */
    static get epochTime(): Time {
        if (!this._epochTime) {
            this._epochTime = Time.fromData({
                year: 1970,
                month: 1,
                day: 1,
                hour: 0,
    ...
    static _cmp_attr(a: Time, b: Time, attr: keyof Time): number {
    ...
    static readonly SUNDAY: weekDay = 1;
    static readonly MONDAY: weekDay = 2;
    static readonly TUESDAY: weekDay = 3;
    static readonly WEDNESDAY: weekDay = 4;
    static readonly THURSDAY: weekDay = 5;
    static readonly FRIDAY: weekDay = 6;
    static readonly SATURDAY: weekDay = 7;

    /**
     * The default weekday for the WKST part.
     */
    static readonly DEFAULT_WEEK_START: weekDay = 2; // MONDAY
     ...
     */
    readonly icalclass: string = "icaltime";
    private _cachedUnixTime: number | null = null;
     ...
     * day to 33).
     */
    private _pendingNormalization: boolean = false;

    /**
     * Used by fromDayOfYear
     */
    ...
    private _getTimeAttr<K extends keyof Time['_time']>(attr: K): Time['_time'][K] {
    ...
    private _setTimeAttr<K extends keyof Time['_time']>(attr: K, val: Time['_time'][K]): void {
     ...
     */
    clone(): Time {
        return new Time(this._time, this.zone || undefined);
    }
     ...
     * @param useUTC            If true, the UTC values of the date will be used
     */
    fromJSDate(aDate: Date | null, useUTC?: boolean): Time {
        if (!aDate) {
            this.reset();
        } else {
            if (useUTC) {
     ...
     * @param aZone          Timezone this position occurs in
     */
    fromData(aData?: timeInit | Time, aZone?: Timezone): Time {
        if (aData) {
        ...
        if (aData && !("isDate" in aData)) {
            ...
            this.isDate = (aData as any).isDate;
        }

        if (aData && "timezone" in aData) {
            let zone = TimezoneService.get(
                (aData as any).timezone
            );
    ...
    dayOfWeek(aWeekStart?: weekDay): weekDay {
        ...
        if (dowCacheKey in Time._dowCache) {
            return Time._dowCache[dowCacheKey] as weekDay;
        }
     ...
     */
    nthWeekDay(aDayOfWeek: weekDay, aPos: number): number {
    ...
    weekNumber(aWeekStart?: weekDay): number {
        let wnCacheKey = (this.year << 12) + (this.month << 8) + (this.day << 3) + (aWeekStart || 0);
     ...
     */
    compare(other: Time | Period): number {
        if ('icalclass' in other && (other as any).icalclass === 'icalperiod') {
            return (-1 * other.compare(this)) || 0; // Avoid -0
        } else {
            let a = this.toUnixTime();
            let b = (other as Time).toUnixTime();
     ...
     * @param aTime            The time to adjust, defaults to the
     *                                    current instance.
     */
    adjust(aExtraDays: number, aExtraHours: number, aExtraMinutes: number, aExtraSeconds: number, aTime?: Time['_time']): Time {

        let minutesOverflow: number, hoursOverflow: number,
            daysOverflow = 0, yearsOverflow = 0;

        let second: number, minute: number, hour: number, day: number;
    ...
    toJSON(): object {
        ...
        let result: Record<string, any> = Object.create(null);
