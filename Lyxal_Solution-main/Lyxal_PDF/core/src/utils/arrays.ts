...
export const mergeIntoTypedArray = (...arrays: (string | Uint8Array)[]) => {
  ...
  const typedArrays: Uint8Array[] = new Array(arrayCount);
  let totalSize = 0;

  for (let idx = 0; idx < arrayCount; idx++) {
    const element = arrays[idx];
    const arr = element instanceof Uint8Array ? element : typedArrayFor(element);
    typedArrays[idx] = arr;
...
// Optimized reverse for modern runtimes (Bun/Node/Browser)
