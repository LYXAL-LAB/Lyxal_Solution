...
function findUnequal(arr: any[], start: number, value: any): number {
  let j, jj;
...
function reverseValues(arr: any[], start: number, end: number): void {
...
function createBidiText(str: string, isLTR: boolean, vertical: boolean = false): { str: string; dir: string } {
  let dir = "ltr";
  if (vertical) {
    dir = "ttb";
  } else if (!isLTR) {
...
export function bidi(str: string, startLevel: number = -1, vertical: boolean = false): { str: string; dir: string } {
  let isLTR = true;
