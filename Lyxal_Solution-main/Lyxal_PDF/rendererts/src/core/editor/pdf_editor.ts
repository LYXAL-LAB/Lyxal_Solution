...
import { Dict, isName, Name, Ref, RefSet, RefSetCache } from "../primitives.ts";
import { getModificationDate, stringToPDFString } from "../../shared/util.js";
import { incrementalUpdate, writeValue } from "../writer.ts";
...
import { PDFDocument, Page } from "../document.ts";
...
class PageData {
  ...
  pointingNamedDestinations: Set<string> | null;

  constructor(page: Page, documentData: DocumentData) {
    this.page = page;
...
class DocumentData {
  ...
  destinations: Map<string, any> | null;
  pageLabels: Map<number, any> | null;
  pagesMap: RefSetCache;
  oldRefMapping: RefSetCache;
  dedupNamedDestinations: Map<string, string>;
  usedNamedDestinations: Set<string>;
  postponedRefCopies: RefSetCache;
  usedStructParents: Set<number>;
  oldStructParentMapping: Map<number, number>;
  structTreeRoot: any | null;
  parentTree: Map<number, any> | null;
  idTree: Map<string, any> | null;
  roleMap: Dict | null;
  classMap: Dict | null;
  namespaces: any[] | null;
  structTreeAF: any[] | null;
  structTreePronunciationLexicon: any[];
...
class PDFEditor {
  ...
  pagesRef: Ref;
  pagesDict: Dict;
  namesRef: Ref | null = null;
  namesDict: Dict | null;
  useObjectStreams: boolean;
  objStreamRefs: Set<number> | null;
  version: string;
  title: string;
  author: string;
  pageLabels: any[] | null;
  namedDestinations: Map<string, any>;
  parentTree: Map<number, any>;
  structTreeKids: any[];
   ...
   * @returns {Promise<Ref>}
   */
  async #cloneObject(obj: any, xref: XRef) {
    const ref = this.newRef;
   ...
   * @returns {Promise<*>}
   */
  async #collectDependencies(obj: any, mustClone: boolean, xref: XRef): Promise<any> {
    ...
    if (Array.isArray(obj)) {
      ...
      for (let i = 0, ii = obj.length; i < ii; i++) {
        ...
        if (postponedActions) {
          ...
          postponedActions.push((ref: any) => (obj[i] = ref));
          continue;
        }
        promises.push(
    ...
    if (dict) {
      for (const [key, rawObj] of dict.getRawEntries()) {
        ...
        if (postponedActions) {
          // The object is a reference that needs to be copied later.
          postponedActions.push((ref: any) => dict!.set(key, ref));
          continue;
        }
        promises.push(
  ...
  async #cloneStructTreeNode(
    parentStructRef: Ref | null,
    node: Dict,
    xref: XRef,
    removedStructElements: RefSet,
    dedupIDs: Map<string, string>,
    dedupClasses: Map<string, string>,
    dedupRoles: Map<string, string>,
    visited = new RefSet()
  ): Promise<Ref | null> {
    const {
      // @ts-ignore
    ...
    const newKids: any[] = [];
    const structElemIndices: number[] = [];
    for (let kid of kids) {
    ...
    if (typeof id === "string") {
      const stringId = stringToPDFString(id);
   ...
   * @return {Promise<void>}
   */
  async extractPages(pageInfos: any[]) {
    const promises = [];
    let newIndex = 0;
    ...
    for (const { document, includePages, excludePages } of pageInfos) {
      ...
      let keptIndices: any, keptRanges: any, deletedIndices: any, deletedRanges: any;
      ...
      for (let i = 0, ii = document.numPages; i < ii; i++) {
        ...
        promises.push(
          document.getPage(i).then((page: Page) => {
    ...
    for (let i = 0, ii = this.oldPages.length; i < ii; i++) {
      this.newPages[i] = await this.#makePageCopy(i);
    }
   ...
   * @return {Promise<void>}
   */
  async #collectDocumentData(documentData: DocumentData) {
    const {
    ...
    await Promise.all([
      pdfManager
        ...
        .then((destinations: any) => (documentData.destinations = destinations)),
      pdfManager
        .ensureCatalog("rawPageLabels")
        .then((pageLabels: any) => (documentData.pageLabels = pageLabels)),
      pdfManager
        .ensureCatalog("structTreeRoot")
        .then((structTreeRoot: any) => (documentData.structTreeRoot = structTreeRoot)),
    ]);
   ...
   */
  async #postCollectPageData(pageData: PageData) {
    const {
    ...
    for (const annotationRef of annotations) {
      ...
      promises.push(
        xref.fetchIfRefAsync(annotationRef).then(async (annotationDict: Dict) => {
   ...
   */
  #setPostponedRefCopies(allDocumentData: DocumentData[]) {
   ...
   * @param {Array<DocumentData>} allDocumentData
   */
  #fixPostponedRefCopies(allDocumentData: DocumentData[]) {
  ...
  #visitObject(obj: any, callback: (dict: Dict) => void, visited = new RefSet()) {
    if (obj instanceof Ref) {
      if (!visited.has(obj)) {
        visited.put(obj);
  ...
  async #mergeStructTrees(allDocumentData: DocumentData[]) {
   ...
   * @param {Array<DocumentData>} allDocumentData
   */
  #collectValidDestinations(allDocumentData: DocumentData[]) {
   ...
   */
  #fixNamedDestinations(annotations: Ref[], dedupNamedDestinations: Map<string, string>) {
    if (dedupNamedDestinations.size === 0) {
      return;
    }
    const fixDestination = (dict: Dict, key: string, dest: string) => {
      if (typeof dest === "string") {
        dict.set(
          key,
          dedupNamedDestinations.get(
            stringToPDFString(dest)
          ) || dest
        );
      }
    };
  ...
  async #collectPageLabels() {
    ...
    let currentIndex = 0;
    const newPageLabels: any[] = (this.pageLabels = [[0, currentLabel]]);
   ...
   */
  async #makePageCopy(pageIndex: number) {
  ...
  #makePageTree() {
    ...
    const stack: any[] = [{ dict: pagesDict, kids: pages, parentRef: pagesRef }];
   ...
   * @returns {Ref}
   */
  #makeNameNumTree(map: any[], areNames: boolean) {
    ...
    const stack: any[] = [{ dict: treeDict, entries: allEntries }];
   ...
   */
  async #createChanges(): Promise<[RefSetCache, Ref]> {
   ...
   */
  async #createObjectStream(objStreamRef: Ref, objRefs: Ref[], changes: RefSetCache) {
    const streamBuffer = [""];
    const objOffsets = [];
    let offset = 0;
    const buffer: any[] = [];
