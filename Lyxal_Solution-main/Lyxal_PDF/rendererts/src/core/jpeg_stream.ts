...
import { FeatureTest, shadow, warn } from "../shared/util";
import { BaseStream } from "./base_stream";
import { DecodeStream } from "./decode_stream";
import { Dict } from "./primitives";
import { JpegImage } from "./jpg";

// Define ImageDecoder locally to avoid TS errors if not in lib
declare class ImageDecoder {
  constructor(options: { data: any; type: string; preferAnimation?: boolean });
  decode(options?: any): Promise<{ image: any }>;
  close(): void;
...
export class JpegStream extends DecodeStream {
  ...
  static get canUseImageDecoder(): Promise<boolean> {
    return shadow(
      this,
      "canUseImageDecoder",
  ...
  get jpegOptions(): { decodeTransform: Int32Array | undefined; colorTransform: number | undefined } {
    const jpegOptions: { decodeTransform: Int32Array | undefined; colorTransform: number | undefined } = {
  ...
  async getTransferableImage(): Promise<any | null> {
