...
const shortcuts = new Map<string, (root: any, current: any) => any>([
...
function parseExpression(expr: string, dotDotAllowed = true, noExpr = true) {
...
function searchNode(
  ...
  if (fn) {
    isQualified = true;
    rootArr = [fn(root, container)];
    i = 1;
  } else {
    isQualified = container === null;
    rootArr = [container || root];
  }
...
function createDataNode(root: any, container: any, expr: string) {
  ...
  for (let ii = parsed.length; i < ii; i++) {
    ...
    switch (operator) {
      ...
      case operators.dotHash:
        ...
        children = (children as any).isXFAObjectArray ? (children as any).children : [children];
        break;
      default:
        break;
    }
