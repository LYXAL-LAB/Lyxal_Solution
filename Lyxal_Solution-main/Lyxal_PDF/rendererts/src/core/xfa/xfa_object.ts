...
class XFAObject {
  ...
  [$text]() {
    ...
    return this[_children].map((c: any) => c[$text]()).join("");
  }
  ...
  *[_filteredChildrenGenerator](filter: any, include: any) {
  ...
  [$addHTML](html: any, bbox: any) {
    this[$extra].children.push(html);
  }

  [$getAvailableSpace]() {}

  [$childrenToHTML]({ filter = null, include = true }: any) {
  ...
  [$setSetAttributes](attributes: any) {
   ...
   */
  [_getUnsetAttributes](protoAttributes: any) {
    const allAttr = this[_attributeNames];
    const setAttr = this[_setAttributes];
    return [...protoAttributes].filter((x: any) => allAttr.has(x) && !setAttr.has(x));
  }
   ...
   */
  [$resolvePrototypes](ids: any, ancestors = new Set()) {
    for (const child of this[_children]) {
      child[_resolvePrototypesHelper](ids, ancestors);
    }
  }

  [_resolvePrototypesHelper](ids: any, ancestors: any) {
  ...
  [_getPrototype](ids: any, ancestors: any) {
  ...
  [_applyPrototype](proto: any, ids: any, ancestors: any) {
    if (ancestors.has(proto)) {
      // We've a cycle so break it.
      warn(`XFA - Cycle detected in prototypes use.`);
      return;
    ...
    for (const name of Object.getOwnPropertyNames(this)) {
      ...
      if (value instanceof XFAObjectArray) {
        ...
        for (
          let i = value.children.length, ii = protoValue.children.length;
          i < ii;
          i++
        ) {
          const child = protoValue.children[i][$clone]();
          if (value.push(child)) {
            child[_parent] = this;
            this[_children].push(child);
            child[_resolvePrototypesHelper](ids, ancestors);
  ...
  [$clone]() {
    ...
    for (const child of this[_children]) {
      ...
      if (clone[name] === null) {
        ...
        clone[name].children.push(clonedChild);
      }
    }
  ...
  *[$getChildrenByNameIt](name: string, allTransparent: boolean, first = true): Generator<any> {
    if (name === "parent") {
      yield this[_parent];
      return;
    }
...
class XFAObjectArray {
  ...
  dump() {
    return this[_children].length === 1
      ...
      : this[_children].map((x: any) => x[$dump]());
  }

  [$clone]() {
    const clone = new XFAObjectArray(this[_max]);
    clone[_children] = this[_children].map((c: any) => c[$clone]());
    return clone;
  }
...
class XFAAttribute {
  [key: symbol]: any;
  constructor(node: any, name: string, value: any) {
    this[_parent] = node;
    this[$nodeName] = name;
    this[$content] = value;
    this[$consumed] = false;
  ...
  [$isDescendent](parent: any) {
    return this[_parent] === parent || this[_parent][$isDescendent](parent);
  }
}

class XmlObject extends XFAObject {
  constructor(nsId: number, name: string, attributes: any = {}) {
    super(nsId, name);
    this[$content] = "";
  ...
  [$getChildren](name: string | null = null) {
    ...
    return this[_children].filter((c: any) => c[$nodeName] === name);
  }
  ...
  *[$getChildrenByNameIt](name: string, allTransparent: boolean): Generator<any> {
    const value = this[_attributes].get(name);
    if (value) {
      yield value;
    }
  ...
  *[$getAttributeIt](name: string, skipConsumed: boolean) {
    const value = this[_attributes].get(name);
  ...
  *[$getRealChildrenByNameIt](name: string, allTransparent: boolean, skipConsumed: boolean): Generator<any> {
    for (const child of this[_children]) {
  ...
  [$dump](hasNS = false) {
    const dumped: any = Object.create(null);
    if (hasNS) {
...
class OptionObject extends ContentObject {
  ...
  [$finalize]() {
    this[$content] = getKeyword({
      ...
      validate: (k: string) => this[_options].includes(k),
    });
  }
