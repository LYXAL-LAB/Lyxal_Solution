...
const getSpecialPUASymbols = getLookupTableFactory(function (t: any) {
...
export function getUnicodeForGlyph(name: string, glyphsUnicodeMap: Record<string, number>): number {
  ...
  if (name[0] === "u") {
    ...
    if (hexStr && hexStr === hexStr.toUpperCase()) {
...
export function getUnicodeRangeFor(value: number, lastPosition: number = -1): number {
...
export function getCharUnicodeCategory(char: string): { isWhitespace: boolean; isZeroWidthDiacritic: boolean; isInvisibleFormatMark: boolean; } {
