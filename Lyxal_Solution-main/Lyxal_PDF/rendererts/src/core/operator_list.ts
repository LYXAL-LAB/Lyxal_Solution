...
function addState(parentState: any, pattern: number[], checkFn: any, iterateFn: any, processFn: any) {
...
addState(
  ...
  null,
  function iterateInlineImageGroup(context: any, i: number) {
  ...
  function foundInlineImageGroup(context: any, i: number) {
...
addState(
  ...
  null,
  function iterateImageMaskGroup(context: any, i: number) {
    const fnArray = context.fnArray;
    const iFirstSave = context.iCurr - 3;
    const pos = (i - iFirstSave) % 4;
    switch (pos) {
  ...
  function foundImageMaskGroup(context: any, i: number) {
...
addState(
  ...
  function iterateImageGroup(context: any, i: number) {
    const fnArray = context.fnArray,
      argsArray = context.argsArray;
    const iFirstSave = context.iCurr - 3;
    const pos = (i - iFirstSave) % 4;
...
addState(
  ...
  null,
  function iterateShowTextGroup(context: any, i: number) {
    const fnArray = context.fnArray,
      argsArray = context.argsArray;
...
class QueueOptimizer extends NullOptimizer {
  ...
  _optimize() {
    ...
    while (i < ii) {
      ...
      context.iCurr = i;
      i++;
      if (state.checkFn && !state.checkFn(context)) {
...
export class OperatorList {
  ...
  fnArray: number[];
  argsArray: any[];
  optimizer: NullOptimizer | QueueOptimizer;
  dependencies: Set<any>;
  _totalLength: number;
  weight: number;
  _resolved: Promise<void> | null;

  constructor(intent: number = 0, streamSink: any) {
  ...
  static setOptions({ isOffscreenCanvasSupported }: { isOffscreenCanvasSupported: boolean }) {
  ...
  addImageOps(fn: number, args: any, optionalContent?: string, hasMask = false) {
    if (hasMask) {
      this.addOp(OPS.save, null);
  ...
  addDependencies(dependencies: any[]) {
  ...
  flush(lastChunk = false, separateAnnots: any = null) {
