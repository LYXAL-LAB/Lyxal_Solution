...
// Define DecompressionStream interface if not available in the environment types
...
export class FlateStream extends DecodeStream {
  ...
  async getImageData(length: number, _decoderOptions: any): Promise<Uint8Array | null> {
  ...
  async asyncGetBytes(): Promise<Uint8Array | null> {
    this.stream.reset();
    const bytes = this.stream.getBytes(undefined); // Explicitly pass undefined for full length
  ...
  #endsStreamOnError(err: string): void {
    info(err);
    this.eof = true;
  }
