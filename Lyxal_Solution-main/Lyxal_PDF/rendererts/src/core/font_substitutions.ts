...
const substitutionMap = new Map<string, SubstitutionInfo>([
  [
    "Times-Roman",
    {
      local: [
...
function getStyleToAppend(style: FontStyle | undefined): string {
  switch (style) {
    case BOLD:
      return "Bold";
    case ITALIC:
...
function getFamilyName(str: string): string {
...
function generateFont(
  { alias, local, path, fallback, style, ultimate }: SubstitutionInfo,
  src: string[],
  ...
  const result: GeneratedFontResult = {
    style: null,
    ultimate: null,
  };
  if (local) {
...
export function getFontSubstitution(
  systemFontCache: Map<string, FontSubstitution | null>,
  idFactory: any,
  localFontPath: string,
  baseFontName: string,
