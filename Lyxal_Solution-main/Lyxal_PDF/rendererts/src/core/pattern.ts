...
class BaseShading {
  ...
  getIR(): any[] {
    return unreachable("Abstract method `getIR` called.");
  }
}
...
class RadialAxialShading extends BaseShading {
  ...
  constructor(
    ...
    const colorStops: any[] = (this.colorStops = []);
    ...
    if (!extendEnd) {
      ...
      const lastStop = colorStops.at(-1);
      if (lastStop) {
        lastStop[0] -= BaseShading.SMALL_NUMBER;
      }
...
class MeshStreamReader {
  ...
  constructor(stream: BaseStream, context: any) {
    this.stream = stream;
  ...
  get hasData() {
    // Cast to any to access potentially missing 'end' on BaseStream if it's actually Stream
  ...
  readBits(n: number) {
    const stream = this.stream as any; // Cast to access stream methods if needed
...
class MeshShading extends BaseShading {
  ...
  bbox: number[] | null;
  background: any;
  coords: any; // Initially number[][], then Float32Array
  colors: any; // Initially any[], then Uint8Array
  figures: any[];
...
export function getTilingPatternIR(operatorList: any, dict: any, color: any) {
