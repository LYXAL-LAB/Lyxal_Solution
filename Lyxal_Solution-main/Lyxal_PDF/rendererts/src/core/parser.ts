...
function getInlineImageCacheKey(bytes: Uint8Array): string {
  const strBuf: number[] = [];
...
export class Parser {
  ...
  getObj(cipherTransform: any = null) {
    ...
    while ((ch = stream.getByte()) !== -1) {
      if (state === 0) {
        ...
        if (ch === SPACE || ch === LF || ch === CR) {
          ...
          const bytes = stream.peekBytes(5 * n);
          const tmpLexer = new Lexer(
            new Stream(bytes, 0, bytes.length, null),
            knownCommands
          );
  ...
  makeInlineImage(cipherTransform: any) {
    ...
    let dictLength = 0;
    while (!isCmd(this.buf1, "ID") && (this.buf1 as any) !== EOF) {
      if (!(this.buf1 instanceof Name)) {
  ...
  #findStreamLength(startPos: number) {
    ...
    while (stream.pos < (stream as any).end) {
  ...
  makeStream(dict: Dict, cipherTransform: any) {
    const lexer = this.lexer;
  ...
  makeFilter(stream: BaseStream, name: string, maybeLength: number, params: any) {
...
export class Lexer {
  ...
  strBuf: string[] = [];
  _hexStringNumWarn: number = 0;
  beginInlineImagePos: number = -1;

  constructor(stream: BaseStream, knownCommands: any = null) {
    this.stream = stream;
    this.nextChar();
...
export class Linearization {
  static create(stream: BaseStream) {
    function getInt(linDict: Dict, name: string, allowZeroValue = false) {
    ...
    function getHints(linDict: Dict) {
