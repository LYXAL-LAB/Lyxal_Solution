...
export class CMap {
  ...
  mapBfRangeToArray(low: number, high: number, array: any[]): void {
    if (high - low > MAX_MAP_RANGE) {
  ...
  mapOne(src: number, dst: any): void {
    this._map[src] = dst;
  }

  lookup(code: number): any {
  ...
  forEach(callback: (key: number, value: any) => void): void {
...
function expectString(obj: any): void {
  if (typeof obj !== "string") {
    throw new FormatError("Malformed CMap: expected string.");
  }
}

function expectInt(obj: any): void {
  if (!Number.isInteger(obj)) {
    throw new FormatError("Malformed CMap: expected int.");
  }
}

function parseBfChar(cMap: CMap, lexer: Lexer): void {
  while (true) {
...
function parseBfRange(cMap: CMap, lexer: Lexer): void {
  while (true) {
    let obj = lexer.getObj();
    if (obj === EOF) {
      break;
...
function parseCidChar(cMap: CMap, lexer: Lexer): void {
  while (true) {
    let obj = lexer.getObj();
    if (obj === EOF) {
      break;
...
function parseCidRange(cMap: CMap, lexer: Lexer): void {
  while (true) {
    let obj = lexer.getObj();
    if (obj === EOF) {
      break;
...
function parseCodespaceRange(cMap: CMap, lexer: Lexer): void {
  while (true) {
    let obj = lexer.getObj();
    if (obj === EOF) {
      break;
...
function parseWMode(cMap: CMap, lexer: Lexer): void {
...
function parseCMapName(cMap: CMap, lexer: Lexer): void {
  const obj = lexer.getObj();
...
async function parseCMap(cMap: CMap, lexer: Lexer, fetchBuiltInCMap: (name: string) => Promise<any>, useCMap: string | null): Promise<CMap> {
...
async function extendCMap(cMap: CMap, fetchBuiltInCMap: (name: string) => Promise<any>, useCMap: string): Promise<CMap> {
...
async function createBuiltInCMap(name: string, fetchBuiltInCMap: (name: string) => Promise<any>): Promise<CMap> {
  if (name === "Identity-H") {
...
export class CMapFactory {
  static async create({ encoding, fetchBuiltInCMap, useCMap }: { encoding: Name | BaseStream; fetchBuiltInCMap: (name: string) => Promise<any>; useCMap: string | null }): Promise<CMap> {
