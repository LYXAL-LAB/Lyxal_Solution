...
const COMMAND_MAP: Record<string, number[]> = {
  hstem: [1],
  vstem: [3],
  vmoveto: [4],
  rlineto: [5],
...
class Type1CharString {
  ...
  executeCommand(howManyArgs: number, command: number[], keepStack: boolean = false): boolean {
...
function decrypt(data: Uint8Array, key: number, discardNumber: number): Uint8Array {
...
function decryptAscii(data: Uint8Array, key: number, discardNumber: number): Uint8Array {
  const c1 = 52845,
    c2 = 22719;
  let r = key | 0;
...
export class Type1Parser {
  ...
  constructor(stream: Stream, encrypted: boolean, seacAnalysisEnabled: boolean) {
    if (encrypted) {
      ...
      stream = new Stream(buffer, 0, buffer.length, null);
    }
   ...
   */
  extractFontProgram(properties: any): any {
    const stream = this.stream;

    const subrs: Uint8Array[] = [];
    const charstrings: any[] = [];
    const privateData: any = Object.create(null);
    privateData.lenIV = 4;
    const program = {
      subrs: [] as Uint8Array[],
      charstrings: [] as any[],
    ...
    let token: string | null, length, data, lenIV;
    ...
    // Assign subrs to program.subrs after the loop to ensure correct type
    program.subrs = subrs;
      ...
      switch (token) {
        ...
        case "Encoding":
          ...
          if (!/^\d+$/.test(encodingArg || "")) {
            ...
            encoding = [];
            const size = parseInt(encodingArg || "0", 10) | 0;
            this.getToken(); // read in 'array'
