...
export class CFFFont {
  properties: any;
  cff: any; // Type 'CFF' is complex, keeping as any for now or import CFF type if needed
  seacs: any[] | null;
  data: any;
  ...
  getGlyphMapping(): Record<number, number> {
    ...
    let charCodeToGlyphId: Record<number, number>;
    let glyphId;

    if (properties.composite) {
      let invCidToGidMap: Record<number, number> | undefined;
