...
import {
  ...
  class Catalog {
    #actualNumPages: number | null = null;
    #catDict: Dict | null = null;
  ...
  #readStructTreeRoot() {
    ...
    const root = new StructTreeRoot(obj, this.xref);
    // @ts-ignore
    root.init();
    return root;
  }
  
    get toplevelPagesDict_() {
  ...
  #readDocumentOutline() {
    ...
    const queue: any[] = [{ obj, parent: root }];
    ...
    while (queue.length > 0) {
      ...
      resetForm?: any;
      attachmentDest?: any;
    } = { url: null, dest: null, action: null };
      // @ts-ignore
  ...
  get optionalContentConfig() {
    ...
    #readOptionalContentConfig(config: Dict, groupRefCache: RefSetCache) {
      ...
      const parseOrder = (refs: any[], nestedLevels = 0): any[] | null => {
        if (!Array.isArray(refs)) {
          return null;
        }
        const order: any[] = [];
        ...
        const hiddenGroups: any[] = [];
      for (const [groupRef] of groupRefCache.items()) {
        if (groupRef && parsedOrderRefs.has(groupRef)) {
          continue;
        }
        if (groupRef) {
      ...
      const parseNestedOrder = (ref: any, nestedLevels: number): any | null => {
      ...
      const parseRBGroups = (rbGroups: any[]) => {
    ...
    getDestination(id: string) {
      ...
      if (this.hasOwnProperty("destinations")) {
        return (this as any).destinations[id] ?? null;
      }
      ...
      if (rawDests.length) {
        const dest = (this as any).destinations[id];
        if (dest) {
          return dest;
        }
      }
  ...
  get pageLabels() {
    ...
    #readPageLabels() {
      ...
      const pageLabels: any[] = new Array(this.numPages);
    ...
    get openAction() {
      ...
      const openAction: any = Object.create(null);
      ...
      if (obj instanceof Dict) {
        ...
        const resultObj: any = { url: null, dest: null, action: null };
  ...
  get xfaImages() {
    const obj = this.#catDict!.get("Names");
    let xfaImages: Map<string, any> | null = null;
  ...
  #collectJavaScript(): Map<string, string> | null {
    const obj = this.#catDict!.get("Names");
    let javaScript: Map<string, string> | null = null;
  
      const appendIfJavaScriptDict = (name: string, jsDict: any) => {
    ...
    get jsActions() {
      ...
      let actions: any = collectActions(
        this.xref,
        this.#catDict!,
        DocumentActionEventType
      );
      ...
      if (javaScript) {
        ...
        for (const [key, val] of (javaScript as Map<string, string>)) {
          if (key in actions) {
    ...
    async cleanup(manuallyTriggered = false) {
      ...
      const fontCacheValues: any = await Promise.all(this.fontCache as any);
      for (const { dict } of fontCacheValues) {
    ...
    async getPageDict(pageIndex: number) {
      ...
      while (nodesToVisit.length) {
        ...
        if (objId && !pageKidsCountCache.has(objId as any)) {
          pageKidsCountCache.put(objId as any, count);
        }
    ...
    async getAllPageDicts(recoveryMode = false) {
      ...
      const addPageDict = (pageDict: Dict, pageRef: Ref | null) => {
        // Help improve performance of the `getPageIndex` method.
    ...
    getPageIndex(pageRef: Ref) {
      ...
      function pagesBeforeRef(kidRef: Ref): Promise<any[] | null> {
        let total = 0,
          parentRef;
        ...
        return xref
          ...
          .then(function (kids: any) {
            if (!kids) {
              return null;
            }
  
            const kidPromises: Promise<void>[] = [];
            let found = false;
            for (const kid of kids) {
              ...
              kidPromises.push(
                xref.fetchAsync(kid).then(function (obj: any) {
                  if (!(obj instanceof Dict)) {
      ...
      let total = 0;
      const next = (ref: Ref): Promise<any> =>
