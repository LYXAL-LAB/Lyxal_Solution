...
import { ColorSpaceUtils } from "./colorspace_utils";
 ...
 */
function resizeImageMask(src: any, bpc: number, w1: number, h1: number, w2: number, h2: number) {
  const length = w2 * h2;
  let dest;
  if (bpc <= 8) {
...
export class PDFImage {
  ...
  interpolate: boolean;
  imageMask: boolean;
  matte: boolean | any[]; // boolean or array
  bpc: number;
  colorSpace: ColorSpace | any; // ColorSpace | null in some contexts?
  numComps: number;
  decode: number[] | null;
  needsDecode: boolean;
  decodeCoefficients: number[] | undefined;
  decodeAddends: number[] | undefined;
  smask: PDFImage | null = null;
  mask: PDFImage | number[] | null = null;
  jpxDecoderOptions: any;
  ...
  static async createMask({ image, isOffscreenCanvasSupported = false }: { image: any; isOffscreenCanvasSupported?: boolean }) {
    const { dict } = image;
  ...
  get drawWidth() {
    return Math.max(this.width, this.smask?.width || 0, (this.mask as PDFImage)?.width || 0);
  }
  ...
  get drawHeight() {
    return Math.max(
      this.height,
      this.smask?.height || 0,
      (this.mask as PDFImage)?.height || 0
    );
  }

  decodeBuffer(buffer: any) {
    const bpc = this.bpc;
  ...
  async fillOpacity(rgbaBuf: Uint8ClampedArray, width: number, height: number, actualHeight: number, image: any) {
  ...
  undoPreblend(buffer: any, width: number, height: number) {
    if (typeof PDFJSDev === "undefined" || PDFJSDev.test("TESTING")) {
      assert(
  ...
  async fillGrayBuffer(buffer: Uint8ClampedArray) {
    if (typeof PDFJSDev === "undefined" || PDFJSDev.test("TESTING")) {
      assert(
        buffer instanceof Uint8ClampedArray,
  ...
  createBitmap(kind: number, width: number, height: number, src: any) {
    const canvas = new OffscreenCanvas(width, height);
    const ctx = canvas.getContext("2d");
    let imgData;
