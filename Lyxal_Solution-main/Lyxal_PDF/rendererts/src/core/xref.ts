...
// Helper interfaces for XRef table/stream state
...
export class XRef {
  ...
  _cacheMap: Map<number, any>;
  _pendingRefs: RefSet;
  _newPersistentRefNum: number | null;
  _newTemporaryRefNum: number | null;
  _persistentRefsCache: Map<number, any> | null;
  startXRefQueue: number[] = [];
  trailer: Dict | null = null;
  topDict: Dict | null = null;
  encrypt: any = null;
  root: Dict | null = null;
  _generationFallback: boolean = false;
  tableState?: TableState;
  streamState?: StreamState;

  constructor(stream: Stream, pdfManager: any) {
    this.stream = stream;
  ...
  parse(recoveryMode = false) {
    ...
    if (!(trailerDict instanceof Dict)) {
      throw new FormatError("Invalid trailer: must be a Dict");
    }
    ...
    if (!recoveryMode) {
      throw new XRefParseException("Failed to parse XRef");
    }
  ...
  indexObjects() {
    ...
    for (const dict of [...trailerDicts, "genFallback", ...trailerDicts]) {
      ...
      try {
        const rootDict = (dict as Dict).get("Root");
      ...
      if (
        validPagesDict &&
        (!isEncrypted || (dict as Dict).has("Encrypt")) &&
        (dict as Dict).has("ID")
      ) {
        return dict;
      }
  ...
  readXRef(recoveryMode = false) {
    ...
    throw new XRefParseException("Failed to read XRef");
  }
  ...
  fetchIfRef(obj: any, suppressEncryption = false) {
  ...
  fetchUncompressed(ref: Ref, xrefEntry: XRefEntry, suppressEncryption = false): any {
    const gen = ref.gen;
    let num = ref.num;
    ...
    // @ts-ignore
    let obj: any = this.encrypt && !suppressEncryption
  ...
  async fetchIfRefAsync(obj: any, suppressEncryption: boolean) {
    if (obj instanceof Ref) {
