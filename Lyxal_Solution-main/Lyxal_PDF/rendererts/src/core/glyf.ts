...
export class GlyfTable {
  ...
  constructor({ glyfTable, isGlyphLocationsLong, locaTable, numGlyphs }: { glyfTable: DataView; isGlyphLocationsLong: boolean; locaTable: DataView; numGlyphs: number }) {
    this.glyphs = [];
  ...
  getSize(): number {
    // Replaced Math.sumPrecise with reduce
    return this.glyphs.reduce((acc, g) => acc + ((g.getSize() + 3) & ~3), 0);
  }

  write(): { isLocationLong: boolean; loca: Uint8Array; glyf: Uint8Array } {
...
class Glyph {
  header: GlyphHeader | null;
  simple: SimpleGlyph | null;
  composites: CompositeGlyph[] | null;

  constructor({ header = null, simple = null, composites = null }: { header?: GlyphHeader | null; simple?: SimpleGlyph | null; composites?: CompositeGlyph[] | null }) {
    this.header = header;
    this.simple = simple;
  ...
  static parse(pos: number, glyf: DataView): Glyph {
    ...
    pos += read as number;
    const headerObj = header as GlyphHeader;

    if (headerObj.numberOfContours < 0) {
      // Composite glyph.
      const composites: CompositeGlyph[] = [];
      while (true) {
        const [n, composite] = CompositeGlyph.parse(pos, glyf);
        pos += n as number;
        const compositeObj = composite as CompositeGlyph;
        composites.push(compositeObj);
        if (!(compositeObj.flags & MORE_COMPONENTS)) {
          break;
        }
      }

      return new Glyph({ header: headerObj, composites });
    }

    const simple = SimpleGlyph.parse(pos, glyf, headerObj.numberOfContours);

    return new Glyph({ header: headerObj, simple });
  }
  ...
  getSize(): number {
    ...
    const size = this.simple
      ? this.simple.getSize()
      : this.composites!.reduce((acc, c) => acc + c.getSize(), 0); // Replaced Math.sumPrecise
...
class GlyphHeader {
  ...
  constructor({ numberOfContours, xMin, yMin, xMax, yMax }: { numberOfContours: number; xMin: number; yMin: number; xMax: number; yMax: number }) {
...
class SimpleGlyph {
  ...
  constructor({ contours, instructions }: { contours: Contour[]; instructions: Uint8Array }) {
  ...
  static parse(pos: number, glyf: DataView, numberOfContours: number): SimpleGlyph {
    ...
    pos += 2;
    // Uint8Array slice works on buffer, but here glyf is DataView.
    // Need to access buffer and offsets.
    const instructions = new Uint8Array(glyf.buffer, glyf.byteOffset + pos, instructionLength).slice(); // Copy to avoid referencing original buffer
...
class CompositeGlyph {
  ...
  static parse(pos: number, glyf: DataView): [number, CompositeGlyph] {
    ...
    if (flags & WE_HAVE_INSTRUCTIONS) {
      const instructionLength = glyf.getUint16(pos);
      pos += 2;
      instructions = new Uint8Array(glyf.buffer, glyf.byteOffset + pos, instructionLength).slice();
      pos += instructionLength;
    }
