...
function checkRateLimit(ip, rps, burst) {
    const now = Date.now();
...
function zodToToolInput(schema) {
...
export async function startMcpServer() {
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (request) => {
        ...
        return {
            messages: prompt.messages(request.params.arguments)
        };
    });
    ...
    if (port) {
        ...
        app.post("/mcp", express.json(), async (req, res) => {
            const sessionId = req.query.sessionId; // Client doit envoyer ?sessionId=... ou on peut utiliser un header
