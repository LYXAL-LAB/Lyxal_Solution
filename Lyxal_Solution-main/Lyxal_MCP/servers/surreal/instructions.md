...
*   Use specific record IDs when you know them for better performance
*   Use parameterized queries to prevent SQL injection when dealing with user input
*   Use the enhanced select tool which safely parameterizes table names and supports user parameters
*   Use the raw query tool for complex operations not covered by convenience functions
*   Use merge/patch modes when updating records to preserve existing data
*   Create relationships to model graph data and enable complex queries
*   Use table names in select/delete when you want to operate on all records
...
*   **Filter and sort users**: `select("person", Some("age > 25"), None, None, Some("name ASC"), Some("10"), None)`
*   **Group articles by author**: `select("article", Some("published = true"), Some("author"), Some("author"), Some("created_at DESC"), Some("20"), None)`
*   **Paginate results**: `select("person", Some("city = 'NYC'"), None, None, Some("age DESC"), Some("5"), Some("10"))`
*   **Parameterized queries**: `select("person", Some("age > $min_age AND name CONTAINS $name_filter"), None, None, None, Some("10"), None, Some({"min_age": 25, "name_filter": "John"}))`

### Updating user profiles

*   **Replace entire profile**: `update("person:john", {"name": "John", "age": 30}, None)`
*   **Add new fields**: `update("person:john", {"city": "NYC"}, Some("merge"))`
*   **Update specific field**: `update("person:john", [{"op": "replace", "path": "/age", "value": 31}], Some("patch"))`
...
*   Always use specific Record IDs when you know them for better performance
*   Use relationships to model graph data instead of foreign keys
*   Use MERGE and PATCH for updates to preserve existing data
*   Define schemas for data validation and consistency
*   Use indexes on frequently queried fields
*   Use LIVE queries for real-time updates when needed
