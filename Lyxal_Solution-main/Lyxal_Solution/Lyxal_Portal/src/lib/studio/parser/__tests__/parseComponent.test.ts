...
describe('parseComponent - test_button integration', () => {
  ...
  const testButtonStructure: StudioComponentStructure = {
    type: 'button',
    props: {
  ...
  it('should keep action object in onClick', () => {
    ...
    expect((element.props as any).onClick).toBeDefined();
    expect((element.props as any).onClick.type).toBe('action');
    expect((element.props as any).onClick.action).toBe('state_update');
    expect((element.props as any).onClick.target).toBe('button_clicked');
  });
  ...
  it('should apply className from structure', () => {
    ...
    expect((element.props as any).className).toContain('btn');
  ...
  it('should override className if provided in props', () => {
    ...
    expect((element.props as any).className).toBe('custom-class');
  });
  ...
  it('should handle empty props', () => {
    ...
    expect((element.props as any).disabled).toBe('{{props.disabled}}');
  });
});
...
describe('parseComponent - edge cases', () => {
  ...
  it('should handle structure without children', () => {
    ...
    expect((element.props as any).type).toBe('text');
  });
  ...
  it('should handle complex nested templates', () => {
    ...
    expect((element.props as any)['data-page']).toBe('Contact');
  });
  ...
  it('should preserve non-template values', () => {
    ...
    expect((element.props as any).type).toBe('submit');
    expect((element.props as any).className).toEqual(['btn']);
  });
});
