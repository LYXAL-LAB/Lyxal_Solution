...
const Header: React.FC<HeaderProps> = memo(({
  ...
  const systemMenuItems = useMemo(() => [
    ...
    { id: 'menu-security', label: 'Sécurité', ariaLabel: 'Accéder aux paramètres de sécurité', onClick: undefined, className: 'btn btn-sm btn-ghost justify-start !px-3 !mx-1 text-xs sm:text-sm transition-all durée-200 hover:scale-[1.01] hover:bg-base-300 focus:ring-2 focus:ring-primary' },
  ...
  return (
    <>
      <nav id="header-navbar" className="navbar bg-base-100 border-b border-base-300 !px-4 sm:!px-6 lg:!px-8 transition-colors duration-200" role="navigation" aria-label={ariaLabels.navbar}>
        <div id="header-content" className="w-full flex items-center justify-between">
          <div id="navbar-start" className="navbar-start flex items-center">
            <button id="mobile-menu-btn" className="btn btn-ghost btn-circle lg:hidden mr-2 sm:mr-3 transition-all duration-200 hover:scale-105 focus:ring-2 focus:ring-primary focus:ring-offset-2" onClick={handleSidebarToggle} aria-label={ariaLabels.mobileMenu} aria-expanded={isSidebarOpen} aria-controls="sidebar">
              <svg id="mobile-menu-icon" className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path id="mobile-menu-path" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
              </svg>
            </button>
            {!isSidebarOpen && (
              <div className="h-10 flex flex-col items-center justify-center"><h1 id="header-title" className="text-lg sm:text-xl font-bold text-primary !m-0 !mb-0">{platformName}</h1></div>
            )}
          </div>

          <div id="navbar-center" className="navbar-center hidden sm:block" data-testid="breadcrumb-container">
            {/* Breadcrumb ou titre de section */}
            <div id="breadcrumb" className="text-sm breadcrumbs">
              <ul id="breadcrumb-list">
                <li id="breadcrumb-lyxal"><a id="breadcrumb-lyxal-link">LYXAL</a></li>
                <li id="breadcrumb-level"><a id="breadcrumb-level-link">Niveau 0</a></li>
          ...
          <div id="navbar-end" className="navbar-end">
            <div id="navbar-actions" className="flex items-center gap-1 sm:gap-2">
              {/* Sélecteur de thème */}
              <div id="theme-selector" className="dropdown dropdown-end">
                ...
                >
                  ...
                  >
                    <h3 className="font-semibold text-sm mb-3 text-base-content">Choisir un thème</h3>
                      ...
                      {sortedThemes.map((theme) => (
                        <button
                          ...
                          className={`btn btn-sm justify-start gap-3 !px-3 !mx-1 transition-all duration-200 hover:scale-[1.01] focus:ring-2 focus:ring-primary focus:ring-offset-1 ${
                            currentTheme === theme ? 'btn-primary' : 'btn-ghost hover:bg-base-300'
                          }`}
                          style={{ borderRadius: 'var(--radius-field, 0.25rem)' }}
                          role="menuitem"
                          aria-label={`Sélectionner le thème ${theme}`}
                          aria-current={currentTheme === theme ? 'true' : 'false'}
                        >
                          <ThemeColorPreview theme={theme} />
                          <span className="capitalize text-left flex-1 text-xs sm:text-sm">{theme}</span>
                          {currentTheme === theme && (
                            <svg 
              ...
              <div id="system-menu" className="dropdown dropdown-end">
                <div 
                  ...
                  aria-expanded="false"
                >
                  <svg id="system-menu-icon" className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path id="system-menu-path-1" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                    <path id="system-menu-path-2" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                </div>
                <div 
