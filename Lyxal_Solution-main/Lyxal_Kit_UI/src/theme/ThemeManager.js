...
export class ThemeManager {
    ...
    createCustomTheme(name, colors) {
        ...
        const theme = {
            ...
            '--color-primary-focus': this.rgbFromHex(colors.primaryFocus || colors.primary),
            '--color-primary-content': this.rgbFromHex(colors.primaryContent || '#ffffff'),
            '--color-secondary': this.rgbFromHex(colors.secondary),
            '--color-secondary-focus': this.rgbFromHex(colors.secondaryFocus || colors.secondary),
            '--color-secondary-content': this.rgbFromHex(colors.secondaryContent || '#ffffff'),
            '--color-accent': this.rgbFromHex(colors.accent),
            '--color-accent-focus': this.rgbFromHex(colors.accentFocus || colors.accent),
            '--color-accent-content': this.rgbFromHex(colors.accentContent || '#ffffff'),
            '--color-neutral': this.rgbFromHex(colors.neutral || '#1f2937'),
            '--color-neutral-focus': this.rgbFromHex(colors.neutralFocus || '#111827'),
            '--color-neutral-content': this.rgbFromHex(colors.neutralContent || '#ffffff'),
    ...
    setVariable(variableName, value) {
        ...
        if (this.customThemes[this.currentTheme]) {
            this.customThemes[this.currentTheme][variableName] = finalValue;
            this.saveCustomThemes();
        }
    }
    /**
    ...
    validateColors(colors) {
        ...
        const allValid = Object.values(colors).every(color => color === undefined || hexRegex.test(color));
     ...
     */
    async importDaisyUIThemes() {
        const fs = require('fs').promises;
        const path = require('path');
        try {
            const themesDir = path.join(__dirname, '../../../daisyui-master/packages/daisyui/src/themes');
            ...
            console.log(`✅ ${Object.keys(this.customThemes).length} thèmes DaisyUI importés`);
        }
        catch (error) {
