...
import ContentWrapper, { ContentComponentId } from './ContentWrapper';
...
interface LayoutState {
  ...
  /** État d'ouverture de l'agent IA */
  isAgentIAOpen: boolean;
  /** ID du contenu sélectionné dans le menu */
...
const Layout: React.FC<LayoutProps> = memo(({
  ...
  const agentIAContainerClasses = useMemo(() => 
    `transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] h-full flex items-center ${
      layoutState.isAgentIAOpen ? 'w-80' : 'w-0 overflow-hidden'
    }`,
    [layoutState.isAgentIAOpen]
  );
  ...
  const handleContentChange = useCallback((contentId: ContentComponentId) => {
    ...
    >
      ...
      >
        <Sidebar 
          ...
          selectedContentId={layoutState.selectedContentId}
        />
      </aside>
      ...
      >
        ...
        <Header 
          ...
          isAgentIAOpen={layoutState.isAgentIAOpen}
          onAgentIAToggle={handleAgentIAToggle}
        />

        {/* Zone centrale avec Agent IA et Contenu */}
        <div id="layout-center-zone" className="flex-1 flex overflow-hidden bg-base-100">
          {/* Container Agent IA */}
          <div id="layout-agent-ia-container" className={agentIAContainerClasses}>
            {layoutState.isAgentIAOpen && (
              <aside 
                id="layout-agent-ia-panel"
                className="h-full bg-base-100 border-r border-base-300 flex flex-col"
                role="complementary"
                aria-label="Agent IA - Assistant conversationnel"
              >
                <AgentIA onClose={handleAgentIAToggle} />
              </aside>
            )}
          </div>
          ...
          >
            <ContentWrapper selectedContentId={layoutState.selectedContentId} />
          </div>
        </div>
