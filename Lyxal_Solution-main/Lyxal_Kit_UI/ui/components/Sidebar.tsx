...
const Sidebar: React.FC<SidebarProps> = memo(
  ({ isOpen, onToggle, onContentChange, selectedContentId = "default" }) => {
    ...
    const menuIdToContentId = useCallback(
      (menuId: string): ContentComponentId => {
        ...
        },
        {
          ...
          icon: (
            ...
            >
              ...
              <circle cx="16" cy="12" r="1" fill="currentColor" />
              <path
                d="M12 8L14 6M12 16L14 18M8 12L6 10M16 12L18 14"
                strokeWidth="2"
                strokeLinecap="round"
              />
            </svg>
        ...
        },
        {
          ...
          icon: (
            ...
            >
              <rect x="3" y="8" width="18" height="8" rx="2" strokeWidth="2" />
              <path
                d="M12 5v3M12 16v3M8 12h3M13 12h3"
                strokeWidth="2"
                strokeLinecap="round"
              />
            </svg>
    ...
    const handleNavigation = useCallback((item: NavigationItem) => {
      announceToScreenReader(`Navigation vers ${item.name}`, {
        priority: "polite",
      });
      item.onClick?.();
    }, []);
      ...
      (item: NavigationItem) => {
        ...
        return (
          ...
          >
            <div
              id={`${item.id}-wrapper`}
              className="transition-all duration-700 ease-in-out w-[90%] mx-auto"
            >
              <button
                id={item.id}
              ...
              >
                {item.icon}
                {isOpen && (
                  <span className="ml-3 transition-all duration-700 ease-in-out whitespace-nowrap">
                    {item.name}
                  </span>
                )}
              </button>
