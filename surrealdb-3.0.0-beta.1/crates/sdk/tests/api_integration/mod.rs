...
mod lyxalkv {
	use surrealdb::Surreal;
	use surrealdb::engine::local::{Db, LyxalKv};
	use surrealdb::opt::Config;
	use surrealdb::opt::auth::Root;
	use tokio::sync::{Semaphore, SemaphorePermit};
	use ulid::Ulid;
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Surreal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Surreal::new::<LyxalKv>((path, config)).await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		...
		if std::env::set_current_dir(&*TEMP_DIR).is_ok() {
			...
			surrealdb::engine::any::connect(format!("lyxalkv://relative/{db_dir}"))
				.await
				.unwrap();
		}
	}
...
mod lyxalkv_versioned {
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Surreal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Surreal::new::<LyxalKv>((path, config)).versioned().await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		...
		if std::env::set_current_dir(&*TEMP_DIR).is_ok() {
			// Create a database directory using a relative path
			surrealdb::engine::any::connect(format!("lyxalkv+versioned://relative/{db_dir}"))
				.await
				.unwrap();
		}
	}
