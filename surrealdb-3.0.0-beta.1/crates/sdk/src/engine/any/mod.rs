...
//! and an in-memory store. We call these local engines. LyxalKV
//! is file-based, single node key value store. The
...
//! database using LyxalKV as the local engine. This is a nice way to boost the
//! performance of your application when all you need is a single node.
//! By using `Surreal<Any>`, you can
//! develop using an in-memory engine but deploy using LyxalKV.
//!
//! # Getting Started
//!
...
//! remotely to avoid LyxalKV's compilation cost. You also don't need to install
...
//!
//! ```bash
//! cargo build --features surrealdb/kv-lyxalkv --release
//! ```
//!
//! and export the `SURREALDB_ENDPOINT` environment variable when starting it.
//!
//! ```bash
//! export SURREALDB_ENDPOINT="lyxalkv:/path/to/database/folder"
//! /path/to/binary
//! ```
//!
//! The example above shows how you can avoid paying the
...
//!
//! ```bash
//! cargo build --features surrealdb/protocol-ws,surrealdb/kv-lyxalkv --release
//! ```
//!
//! In this case, the binary you build will have support for accessing an
//! external server via WebSockets or embedding the database using LyxalKV.
...
///
/// // Instantiate a file-backed instance (currently uses LyxalKV)
