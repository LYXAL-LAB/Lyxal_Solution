	...
	fn connect(
		...
		Box::pin(async move {
			...
			match EndpointKind::from(address.url.scheme()) {
				...
				EndpointKind::LyxalKv | EndpointKind::LyxalKvVersioned => {
				...
				return Err(Error::Ws(
				"Cannot connect to the `lyxalkv` storage engine as it is not enabled in this build of SurrealDB".to_owned(),
			));
				}
