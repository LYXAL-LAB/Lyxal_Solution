...
pub async fn create_job(
    ...
    let job = Job::new(name, cron, action, max_retries, payload, timezone)?;
    store.add_job(job.clone()).await?;
    Ok(job)
}

pub async fn delete_job(store: &InMemoryStore, id: &str) -> Result<(), SchedulerError> {
    store.remove_job(id).await
}

pub async fn enable_job(store: &InMemoryStore, id: &str) -> Result<(), SchedulerError> {
...
pub async fn disable_job(store: &InMemoryStore, id: &str) -> Result<(), SchedulerError> {
    if let Some(mut job) = store.get_job(id).await {
        job.enabled = false;
        store.update_job(job).await
    } else {
