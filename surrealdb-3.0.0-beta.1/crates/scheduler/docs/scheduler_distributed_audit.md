...
Ce document analyse le comportement attendu du scheduler Lyxal (intégré à SurrealDB) sur un backend de stockage distribué transactionnel (LyxalKV Distribué, FoundationDB, etc.), en se concentrant sur les mécanismes de Haute Disponibilité (HA) implémentés via les leases.
...
### Analyse sur Backend Distribué (LyxalKV Distribué)

1.  **Atomicité (ACID)** :
    *   SurrealDB (via son layer KV transactionnel) garantit que l'instruction `UPDATE` est atomique.
    *   Sur LyxalKV Distribué, cela se traduit par une transaction distribuée (2PC ou optimisé).
...
2.  **Isolation** :
    *   Le niveau d'isolation par défaut de SurrealDB/LyxalKV Distribué est généralement Snapshot Isolation ou Serializable.
...
1.  **Index** :
    ...
    *   **Point critique** : Si l'optimiseur n'utilise pas l'index composite ou si les index sont dispersés sur plusieurs régions LyxalKV Distribué, la latence du `SELECT` peut augmenter.
...
3.  **Latence Réseau** :
    *   En cluster géodistribué, la latence entre le nœud compute (Scheduler) et le stockage (LyxalKV Distribué) s'ajoute à chaque tick.
...
Le scheduler Lyxal respecte les principes transactionnels nécessaires pour fonctionner sur un backend distribué comme LyxalKV Distribué. L'utilisation de `UPDATE` conditionnel atomique pour les leases est la bonne approche. Aucune modification structurelle n'est requise.
