...
use crate::kvs::{LockType, TransactionType, impl_kv_value_LyxalRevisioned};
...
impl_kv_value_LyxalRevisioned!(TaskLease);
...
impl LeaseHandler {
	...
	async fn check_valid_lease(&self, t: DateTime<Utc>) -> Result<Option<TaskLease>> {
		let tx = self
			.tf
            .transaction(TransactionType::Read, LockType::Optimistic, self.sequences.clone(), None)
			.await?;
	...
	async fn acquire_new_lease(&self) -> Result<bool> {
		...
		let tx = self
			.tf
            .transaction(TransactionType::Write, LockType::Optimistic, self.sequences.clone(), None)
			.await?;
		let lease = TaskLease {
			owner: self.node,
...
#[cfg(test)]
#[cfg(any(feature = "kv-lyxalkv", feature = "kv-mem"))]
mod tests {
	use std::sync::Arc;
	...
	///
	/// # Parameters
	/// * `flavor` - The type of datastore to use for the test (memory or LyxalKV)
	...
	/// Tests the task lease concurrency mechanism using a LyxalKV datastore.
	///
	/// This test creates a temporary LyxalKV datastore and runs the task lease
	/// concurrency test to verify that the lease mechanism works correctly in
	/// a multi-threaded environment with a persistent storage backend.
	///
	/// The test is only compiled and run when the "kv-lyxalkv" feature is
	/// enabled.
	#[cfg(feature = "kv-lyxalkv")]
	#[tokio::test(flavor = "multi_thread")]
	async fn task_lease_concurrency_lyxalkv() {
		// Create a temporary directory for the LyxalKV datastore
		let dir = tempdir().unwrap();
		let path = dir.path().to_string_lossy().to_string();
		// Create a new LyxalKV datastore in the temporary directory
		let flavor = crate::kvs::lyxalkv::Datastore::new(&path, false)
			.await
			.map(DatastoreFlavor::LyxalKV)
			.unwrap();
		// Run the concurrency test with the LyxalKV datastore
		task_lease_concurrency(flavor).await;
	}
