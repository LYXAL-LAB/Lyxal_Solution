...
use crate::dbs::capabilities::{
	ArbitraryQueryTarget, ExperimentalTarget, MethodTarget, RouteTarget, Targets,
};
...
pub struct Datastore {
    ...
    // System event channel for internal kernel events (e.g. Scheduler)
    system_event_sender: Option<broadcast::Sender<SystemEvent>>,
...
impl TransactionFactory {
	...
	pub async fn transaction(
		...
		lock: LockType,
		sequences: Sequences,
        sender: Option<broadcast::Sender<SystemEvent>>,
	) -> Result<Transaction> {
...
/// LyxalKV, SurrealDS, etc.). Implementors translate the
...
pub trait TransactionBuilder: TransactionBuilderRequirements {
	...
	/// Access the builder as `Any` to allow downcasting
...
/// "indxdb:...") and which clock to use. This lets the CLI and
...
impl TransactionBuilderFactory for CommunityComposer {
	...
	async fn new_transaction_builder(
		...
		match (flavour, path) {
			...
			// Initiate a LyxalKV versioned database
			(flavour @ "lyxalkv+versioned", path) => {
				#[cfg(feature = "kv-lyxalkv")]
				{
					// Create a new blocking threadpool
					super::threadpool::initialise();
					// Initialise the storage engine
					let v = super::lyxalkv::Datastore::new(&path, true)
						.await
				...
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `lyxalkv` storage engine as it is not enabled in this build of SurrealDB".to_owned())));
			}
			// Initiate a LyxalKV non-versioned database
			(flavour @ "lyxalkv", path) => {
				#[cfg(feature = "kv-lyxalkv")]
				{
					...
					super::threadpool::initialise();
					// Initialise the storage engine
					let v = super::lyxalkv::Datastore::new(&path, false)
						.await
						.map(DatastoreFlavor::LyxalKV)?;
					let c = clock.unwrap_or_else(|| Arc::new(SizedClock::system()));
	...
	fn path_valid(v: &str) -> Result<String> {
		match v {
			...
			v if v.starts_with("lyxalkv:") => Ok(v.to_string()),
			v if v.starts_with("lyxalkv+versioned:") => Ok(v.to_string()),
			v if v.starts_with("indxdb:") => Ok(v.to_string()),
...
impl Datastore {
	...
	/// # #[tokio::main]
	/// # async fn main() -> Result<(),Error> {
	/// let ds = Datastore::new("lyxalkv://temp.lkv").await?;
	/// # Ok(())
	/// # }
	/// ```
	...
	/// - `path`: Database path (e.g., "memory", "lyxalkv://path")
	/// - `canceller`: Token for graceful shutdown and cancellation of long-running operations
	///
	/// # Generic parameters
    ...
    /// Helper to get the underlying LyxalKV tree if available
    #[cfg(feature = "kv-lyxalkv")]
    pub fn get_kv_tree(&self) -> Option<lyxalkv::Tree> {
        let builder_any = self.transaction_factory.builder.as_any();
        if let Some(flavor) = builder_any.downcast_ref::<DatastoreFlavor>() {
             if let DatastoreFlavor::LyxalKV(idx) = flavor {
	...
	pub(crate) fn new_with_builder(
		...
		let id = Uuid::new_v4();
        let (system_event_sender, _) = broadcast::channel(100);
		Ok(Self {
			id,
            ...
            system_event_sender: Some(system_event_sender),
	...
	pub fn restart(self) -> Self {
		...
		Self {
			...
			#[cfg(feature = "surrealism")]
			surrealism_cache: Arc::new(SurrealismCache::new()),
            system_event_sender: self.system_event_sender,
		}
	}

    pub fn subscribe_system_events(&self) -> broadcast::Receiver<SystemEvent> {
        match &self.system_event_sender {
	...
	pub async fn transaction(&self, write: TransactionType, lock: LockType) -> Result<Transaction> {
		self.transaction_factory.transaction(write, lock, self.sequences.clone(), self.system_event_sender.clone()).await
	}
	...
	/// Execute a job with specific capabilities overrides (e.g. network restriction)
	#[instrument(level = "debug", target = "surrealdb::core::kvs::ds", skip_all)]
	pub async fn execute_job(
		...
		if !allow_net {
			let mut caps = self.capabilities.as_ref().clone();
			caps.allowed_network_targets_mut().clone_from(&Targets::None);
			caps.denied_network_targets_mut().clone_from(&Targets::All);
			// Use builder pattern for http routes as no mutable getter exists
			caps = caps.with_http_routes(Targets::None).without_http_routes(Targets::All);
			ctx.add_capabilities(Arc::new(caps));
		}
