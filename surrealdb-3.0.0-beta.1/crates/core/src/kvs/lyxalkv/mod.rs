...
use lyxalkv::{Durability, Mode, Transaction as Tx, Tree, TreeBuilder};
...
impl Datastore {
    /// Return the underlying LyxalKV tree
...
impl Datastore {
	...
	pub(crate) async fn new(path: &str, enable_versions: bool) -> Result<Datastore> {
		...
		info!(target: TARGET, "Enabling value log separation: {}", *cnf::LYXALKV_ENABLE_VLOG);
		let builder = builder.with_enable_vlog(*cnf::LYXALKV_ENABLE_VLOG);
