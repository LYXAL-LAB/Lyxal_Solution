...
use crate::kvs::{KVKey, LockType, Transaction, TransactionType, impl_kv_value_LyxalRevisioned};
...
impl_kv_value_LyxalRevisioned!(BatchValue);
...
impl_kv_value_LyxalRevisioned!(SequenceState);
...
impl Sequence {
	...
	async fn load(
		...
		let tx =
            sqs.tf.transaction(TransactionType::Read, LockType::Optimistic, sqs.clone(), None).await?;
		let mut st: SequenceState = if let Some(v) = tx.get(&state_key, None).await? {
			lyxal_revision::from_slice(&v)?
		} else {
			SequenceState {
				next: start,
			}
	...
	async fn next(
		...
		let tx =
            self.tf.transaction(TransactionType::Write, LockType::Optimistic, sqs.clone(), None).await?;

		// Execute operations and ensure transaction is cancelled on error
		match tx.set(&self.state_key, &lyxal_revision::to_vec(&self.st)?, None).await {
			Ok(_) => {
				tx.commit().await?;
				Ok(v)
			}
	...
	async fn check_batch_allocation(
		...
		let tx =
            sqs.tf.transaction(TransactionType::Write, LockType::Optimistic, sqs.clone(), None).await?;
		...
		let result = async {
			...
			for (key, val) in val.iter() {
				let ba: BatchValue = lyxal_revision::from_slice(val)?;
			...
			let bv = lyxal_revision::to_vec(&BatchValue {
				to: next_to,
				owner: sqs.nid,
			})?;
