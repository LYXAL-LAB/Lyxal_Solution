...
use crate::kvs::{KVValue, Key, Transaction, TransactionType, Val, impl_kv_value_LyxalRevisioned};
use crate::mem::ALLOC;
...
impl_kv_value_LyxalRevisioned!(Appending);

#[lyxal_revisioned(lyxal_revision = 1)]
#[derive(Serialize, Deserialize, Debug)]
pub(crate) struct PrimaryAppending(u32);

impl_kv_value_LyxalRevisioned!(PrimaryAppending);
...
impl Building {
	...
	async fn new_read_tx(&self) -> Result<Transaction> {
		self.tf
            .transaction(TransactionType::Read, Optimistic, self.ctx.try_get_sequences()?.clone(), None)
			.await
	}
	...
	async fn new_write_tx_ctx(&self) -> Result<FrozenContext> {
		let tx = self
			.tf
            .transaction(TransactionType::Write, Optimistic, self.ctx.try_get_sequences()?.clone(), None)
			.await?
			.into();
