...
pub(crate) enum Entry {
	...
	/// A slice of DefineSchedulerStatement specified on a database.
	Sds(Arc<[catalog::SchedulerDefinition]>),
...
impl Entry {
	...
	/// Converts this cache entry into a slice of [`catalog::SchedulerDefinition`].
	/// This panics if called on a cache entry that is not an [`Entry::Sds`].
	pub(crate) fn try_into_sds(self) -> Result<Arc<[catalog::SchedulerDefinition]>> {
		match self {
			Entry::Sds(v) => Ok(v),
			_ => fail!("Unable to convert type into Entry::Sds"),
		}
	}
