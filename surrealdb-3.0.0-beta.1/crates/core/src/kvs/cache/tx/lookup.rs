...
pub(crate) enum Lookup<'a> {
	...
	/// A cache key for schedulers (on a database)
	Sds(NamespaceId, DatabaseId),
	...
	/// A cache key for a credential (on a database)
	Cr(NamespaceId, DatabaseId, &'a str),
	/// A cache key for a sequence (on a database)
	Sq(NamespaceId, DatabaseId, &'a str),
	/// A cache key for a scheduler (on a database)
	Sd(NamespaceId, DatabaseId, &'a str),
...
impl Equivalent<Key> for Lookup<'_> {
	...
	fn equivalent(&self, key: &Key) -> bool {
		match (self, key) {
			...
			(Self::Sds(la, lb), Key::Sds(ka, kb)) => la == ka && lb == kb,
			...
			(Self::Cr(la, lb, lc), Key::Cr(ka, kb, kc)) => la == ka && lb == kb && lc == kc,
			(Self::Sq(la, lb, lc), Key::Sq(ka, kb, kc)) => la == ka && lb == kb && lc == kc,
			(Self::Sd(la, lb, lc), Key::Sd(ka, kb, kc)) => la == ka && lb == kb && lc == kc,
