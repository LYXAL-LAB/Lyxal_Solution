...
impl<'a> From<Lookup<'a>> for Key {
	...
	fn from(value: Lookup<'a>) -> Self {
		match value {
			...
			Lookup::Cr(a, b, c) => Key::Cr(a, b, c.to_string()),
			Lookup::Sq(a, b,c) => Key::Sq(a, b, c.to_string()),
			Lookup::Sd(a, b, c) => Key::Sd(a, b, c.to_string()),
