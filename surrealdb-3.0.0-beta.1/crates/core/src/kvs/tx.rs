...
pub struct Transaction {
	...
	cf: crate::cf::Writer,
    /// Buffered system events to be emitted on commit
    events: std::sync::Mutex<Vec<SystemEvent>>,
    /// Channel to emit system events
...
impl Transaction {
	...
	pub async fn commit(&self) -> Result<()> {
		...
		self.tr.commit().await.map_err(Error::from)?;
    ...
    pub fn record_system_event(&self, event: SystemEvent) {
        if let Ok(mut events) = self.events.lock() {
            events.push(event);
        }
    }
...
impl DatabaseProvider for Transaction {
	...
	/// Retrieve all sequence definitions for a specific database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	...
	/// Retrieve all scheduler definitions for a specific database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	async fn all_db_schedulers(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
	) -> Result<Arc<[catalog::SchedulerDefinition]>> {
		let qey = cache::tx::Lookup::Sds(ns, db);
		match self.cache.get(&qey) {
			Some(val) => val.try_into_sds(),
			None => {
				let beg = crate::key::database::sd::prefix(ns, db)?;
				let end = crate::key::database::sd::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
				let entry = cache::tx::Entry::Sds(val.clone());
				self.cache.insert(qey, entry);
				Ok(val)
			}
		}
	...
	/// Retrieve a specific scheduler definition from a database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	async fn get_db_scheduler(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
		sc: &str,
	) -> Result<Arc<catalog::SchedulerDefinition>> {
		let qey = cache::tx::Lookup::Sd(ns, db, sc);
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::key::database::sd::Sd::new(ns, db, sc);
				let val = self.get(&key, None).await?.ok_or_else(|| Error::Internal(format!("Scheduler {sc} not found")))?;
				let val = Arc::new(val);
				let entry = cache::tx::Entry::Any(val.clone());
				self.cache.insert(qey, entry);
				Ok(val)
	...
	/// Put a scheduler definition into a database.
	async fn put_db_scheduler(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
		sc: &catalog::SchedulerDefinition,
	) -> Result<()> {
		let key = crate::key::database::sd::Sd::new(ns, db, &sc.name);
		self.set(&key, sc, None).await?;
		Ok(())
	}
	...
	/// Retrieve a specific credential definition from a database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	async fn get_db_credential(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
		cr: &str,
	) -> Result<Arc<catalog::CredentialDefinition>> {
		let qey = cache::tx::Lookup::Cr(ns, db, cr);
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::key::database::cr::new(ns, db, cr);
				let val = self.get(&key, None).await?.ok_or_else(|| Error::CrNotFound {
					name: cr.to_owned(),
				})?;
				let val = Arc::new(val);
				let entry = cache::tx::Entry::Any(val.clone());
	...
	/// Put a credential definition into a database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	async fn put_db_credential(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
		cr: &catalog::CredentialDefinition,
	) -> Result<()> {
		let key = crate::key::database::cr::new(ns, db, &cr.name);
		self.set(&key, cr, None).await?;
		Ok(())
	}

	/// Delete a credential definition from a database.
	#[instrument(level = "trace", target = "surrealdb::core::kvs::tx", skip(self))]
	async fn del_db_credential(
		&self,
		ns: NamespaceId,
		db: DatabaseId,
		cr: &str,
