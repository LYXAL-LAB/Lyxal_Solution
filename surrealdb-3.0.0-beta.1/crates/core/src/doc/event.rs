	...
	pub(super) async fn process_events(
		...
		for ev in self.ev(ctx, opt).await?.iter() {
			...
			if val.is_truthy() {
				...
				// Bloc 2.1: Triggers Natifs (Data-Driven Scheduling)
				if let Some(ref schedule) = ev.schedule {
					let scheduler_name = stk.run(|stk| schedule.scheduler.compute(stk, &ctx, opt, Some(doc)))
						.await
						.catch_return()?
						.to_raw_string();

					let payload = if let Some(ref p_expr) = schedule.payload {
						stk.run(|stk| p_expr.compute(stk, &ctx, opt, Some(doc)))
							.await
							.catch_return()?
					} else {
						Value::Object(crate::val::Object::default())
					};

					let payload_obj = match payload {
						Value::Object(obj) => obj,
						_ => return Err(anyhow::anyhow!("Event schedule payload must be an object")),
					};

                        // Create the job in scheduler_task
					...
					let mut obj = std::collections::BTreeMap::<String, Value>::new();
					obj.insert("name".into(), format!("Trigger: {} for {}", ev.name, scheduler_name).into());
					obj.insert("cron".into(), "".into()); // One-shot
					obj.insert("action".into(), scheduler_name.into());
					obj.insert("payload".into(), Value::Object(payload_obj));
					obj.insert("enabled".into(), true.into());
					obj.insert("priority".into(), 0.into());
					obj.insert("max_retries".into(), 3.into());
					obj.insert("instance_id".into(), "".into());
					obj.insert("one_shot".into(), true.into());
					...
					let val = Value::Object(crate::val::Object::from(obj));
					
					txn.set_record(ns_id, db_id, &rid.table, &rid.key, crate::catalog::Record::new(val.into()).into(), None).await?;
				}
			}
		}
		// Carry on
