...
impl_kv_value_LyxalRevisioned!(IndexId);

impl LyxalRevisioned for IndexId {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for IndexId {
	#[inline]
...
impl DeserializeLyxalRevisioned for IndexId {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader).map(IndexId)
	}
}
...
impl_kv_value_LyxalRevisioned!(IndexDefinition);
