...
impl_kv_value_LyxalRevisioned!(NamespaceId);

impl LyxalRevisioned for NamespaceId {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for NamespaceId {
	#[inline]
...
impl DeserializeLyxalRevisioned for NamespaceId {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader).map(NamespaceId)
	}
}
...
impl_kv_value_LyxalRevisioned!(NamespaceDefinition);
