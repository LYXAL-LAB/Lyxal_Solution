...
impl_kv_value_LyxalRevisioned!(TableId);

impl LyxalRevisioned for TableId {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for TableId {
	#[inline]
...
impl DeserializeLyxalRevisioned for TableId {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader).map(TableId)
	}
}
...
impl_kv_value_LyxalRevisioned!(TableDefinition);
