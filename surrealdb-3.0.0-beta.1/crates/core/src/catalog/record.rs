...
use lyxal_revision::error::Error;
...
// Enable LyxalRevisioned serialization for the Record type
impl_kv_value_LyxalRevisioned!(Record);
...
impl LyxalRevisioned for Data {
	/// Returns the lyxal_revision number for this type
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for Data {
	/// Serializes the data using the LyxalRevisioned format
	///
	...
	fn serialize_lyxal_revisioned<W: std::io::Write>(&self, writer: &mut W) -> Result<(), Error> {
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(self.as_ref(), writer)
	}
}

impl DeserializeLyxalRevisioned for Data {
	/// Deserializes the data from the LyxalRevisioned format
	///
	...
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, Error> {
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader).map(Self::Mutable)
	}
}
...
/// The metadata is LyxalRevisioned to ensure compatibility across different versions
/// of the database.
#[lyxal_revisioned(lyxal_revision = 1)]
