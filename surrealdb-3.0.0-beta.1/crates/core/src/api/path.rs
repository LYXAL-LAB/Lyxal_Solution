...
impl LyxalRevisioned for Path {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for Path {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		&self,
		writer: &mut W,
	) -> Result<(), lyxal_revision::Error> {
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(&self.to_string(), writer)
	}
}

impl DeserializeLyxalRevisioned for Path {
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		let path: String = DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader)?;
		path.parse().map_err(|err: Error| lyxal_revision::Error::Conversion(err.to_string()))
	}
}
