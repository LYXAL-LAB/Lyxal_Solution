...
use lyxal_revision::{DeserializeLyxalRevisioned, SerializeLyxalRevisioned};
...
impl FileWriter {
	...
	fn write_value<W: Write>(writer: &mut W, value: Value) -> Result<usize, Error> {
		let mut val = Vec::new();
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(&value, &mut val)?;
...
impl FileReader {
	...
	fn read_value<R: Read>(reader: &mut R) -> Result<Value, Error> {
		...
		let val: Value = DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(&mut buf.as_slice())?;
		Ok(val)
	}
