...
pub(crate) use event::{DefineEventStatement, EventSchedule};
...
pub(crate) use scheduler::DefineSchedulerStatement;
pub(crate) use sequence::DefineSequenceStatement;
pub(crate) use webhook::{DefineWebhookStatement, WebhookVerify, WebhookContentType, WebhookHandler, WebhookAuth, WebhookOptions, WebhookResponseMode};
pub(crate) use credential::{DefineCredentialStatement, CredentialType, CredentialAlgorithm};
...
pub(crate) enum DefineStatement {
    ...
    Scheduler(DefineSchedulerStatement),
	#[cfg_attr(feature = "arbitrary", arbitrary(skip))]
	Module(DefineModuleStatement),
    Webhook(DefineWebhookStatement),
    Credential(DefineCredentialStatement),
}
...
impl ToSql for DefineStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		match self {
            ...
            Self::Scheduler(v) => v.fmt_sql(f, fmt),
			Self::Sequence(v) => v.fmt_sql(f, fmt),
			Self::Module(v) => v.fmt_sql(f, fmt),
            Self::Webhook(v) => v.fmt_sql(f, fmt),
            Self::Credential(v) => v.fmt_sql(f, fmt),
		}
	}
}
...
impl From<DefineStatement> for crate::expr::statements::DefineStatement {
	fn from(v: DefineStatement) -> Self {
		match v {
            ...
            DefineStatement::Scheduler(v) => Self::Scheduler(v.into()),
			DefineStatement::Module(v) => Self::Module(v.into()),
            DefineStatement::Webhook(v) => Self::Webhook(v.into()),
            DefineStatement::Credential(v) => Self::Credential(v.into()),
		}
	}
}
...
impl From<crate::expr::statements::DefineStatement> for DefineStatement {
	fn from(v: crate::expr::statements::DefineStatement) -> Self {
		match v {
            ...
            crate::expr::statements::DefineStatement::Scheduler(v) => Self::Scheduler(v.into()),
			crate::expr::statements::DefineStatement::Module(v) => Self::Module(v.into()),
            crate::expr::statements::DefineStatement::Webhook(v) => Self::Webhook(v.into()),
            crate::expr::statements::DefineStatement::Credential(v) => Self::Credential(v.into()),
		}
	}
}
