...
use super::types::{AddressBookQuery, CalendarQuery, Filter, Lock, PropFilter, TextMatch};
...
pub fn parse_propfind(body: &[u8]) -> Result<PropfindRequest, DavError> {
    ...
    let mut request = PropfindRequest {
        all_prop: false,
        prop_names: false,
        props: vec![],
    };
...
pub fn parse_calendar_query(body: &[u8]) -> Result<CalendarQuery, DavError> {
    let text = std::str::from_utf8(body).map_err(|e| DavError::Internal(e.to_string()))?;
    let doc = Document::parse(text)?;
    let root = doc.root_element();
...
pub fn parse_free_busy_query(body: &[u8]) -> Result<CalendarQuery, DavError> {
    let text = std::str::from_utf8(body).map_err(|e| DavError::Internal(e.to_string()))?;
    let doc = Document::parse(text)?;
    let root = doc.root_element();
...
pub fn parse_addressbook_query(body: &[u8]) -> Result<AddressBookQuery, DavError> {
    let text = std::str::from_utf8(body).map_err(|e| DavError::Internal(e.to_string()))?;
    let doc = Document::parse(text)?;
    let root = doc.root_element();
        ...
        for child in filter_node.children() {
            ...
            if child.tag_name().name() == "prop-filter" {
                ...
                if let Some(tm_node) = find_child(child, "text-match") {
                    ...
                    let negate_condition = tm_node
                        .attribute("negate-condition")
                        .map(|s| s == "yes")
                        .unwrap_or(false);
                    let collation = tm_node
                        .attribute("collation")
                        .unwrap_or("i;ascii-casemap")
                        .to_string();
                    let match_type = tm_node
                ...
                query.filter.prop_filters.push(PropFilter {
                    name,
                    text_match,
                    is_not_defined,
                });
...
pub fn parse_proppatch(body: &[u8]) -> Result<PropPatchRequest, DavError> {
    ...
    if root.tag_name().name() != "propertyupdate" {
        return Err(DavError::BadRequest(
            "Root element must be propertyupdate".into(),
        ));
    }
...
pub fn parse_sync_collection(body: &[u8]) -> Result<SyncCollectionRequest, DavError> {
    ...
    if root.tag_name().name() != "sync-collection" {
        return Err(DavError::Internal(
            "Root element must be sync-collection".into(),
        ));
    }
...
pub fn parse_lockinfo(body: &[u8]) -> Result<LockInfo, DavError> {
    ...
    if let Some(owner) = root
        .children()
        .find(|n| n.is_element() && n.tag_name().name() == "owner")
    {
...
pub fn generate_lockdiscovery(lock: &Lock) -> String {
