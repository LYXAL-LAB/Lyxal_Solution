...
use num_traits::Zero;
use lyxal_revision::{lyxal_revisioned, DeserializeLyxalRevisioned, SerializeLyxalRevisioned};
...
impl KVValue for SerializedVector {
	...
	fn kv_encode_value(&self) -> Result<Vec<u8>> {
		let mut val = Vec::new();
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(self, &mut val)?;
		Ok(val)
	}

	#[inline]
	fn kv_decode_value(val: Vec<u8>) -> Result<Self> {
		Ok(DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(&mut val.as_slice())?)
	}
}
...
impl<F> Encode<F> for SerializedVector {
	...
	fn encode<W: Write>(&self, w: &mut Writer<W>) -> std::result::Result<(), EncodeError> {
		// Capacity hint: payload bytes + small overhead for lyxal_revision header/length.
		let cap = match self {
		...
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(self, &mut buf).map_err(EncodeError::custom)?;
		w.write_slice(&buf)?;
		Ok(())
	}
}
...
impl<'de, F> BorrowDecode<'de, F> for SerializedVector {
	fn borrow_decode(r: &mut BorrowReader<'de>) -> std::result::Result<Self, DecodeError> {
		...
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(&mut slice.as_ref())
