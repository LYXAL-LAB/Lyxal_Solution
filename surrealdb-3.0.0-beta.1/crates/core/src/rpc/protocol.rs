...
pub trait RpcProtocol {
	...
	async fn execute(
		...
		if !self.kvs().allows_rpc_method(&MethodTarget {
			...
			tracing::warn!("Capabilities denied RPC method call attempt, target: '{method}'");
		...
		match method {
			...
			// --- RTC / SFU Methods (LYXAL CUSTOM) ---
			Method::RtcCreate => self.rtc_create(session, params).await,
			Method::RtcJoin => self.rtc_join(session, params).await,
			Method::RtcLeave => self.rtc_leave(session, params).await,
			Method::RtcSignal => self.rtc_signal(session, params).await,
			Method::RtcPoll => self.rtc_poll(session, params).await,
			Method::RtcList => self.rtc_list(session).await,
			Method::RtcInfo => self.rtc_info(session, params).await,
			Method::RtcClose => self.rtc_close(session, params).await,
			Method::SfuCreate => self.sfu_create(session, params).await,
			Method::SfuOffer => self.sfu_offer(session, params).await,
			Method::SfuAnswer => self.sfu_answer(session, params).await,
			Method::SfuLeave => self.sfu_leave(session, params).await,
			Method::SfuInfo => self.sfu_info(session, params).await,
			Method::SfuList => self.sfu_list(session).await,
			Method::SfuStats => self.sfu_stats(session).await,
			Method::SfuClose => self.sfu_close(session, params).await,
			Method::SfuWebinar => self.sfu_webinar(session, params).await,
			Method::SfuJoinRole => self.sfu_join_role(session, params).await,
			Method::SfuPromote => self.sfu_promote(session, params).await,
			Method::SfuDemote => self.sfu_demote(session, params).await,
			Method::SfuRaiseHand => self.sfu_raise_hand(session, params).await,
			Method::SfuLowerHand => self.sfu_lower_hand(session, params).await,
			Method::SfuRaisedHands => self.sfu_raised_hands(session, params).await,
			Method::SfuParticipants => self.sfu_participants(session, params).await,
			Method::SfuSpeakers => self.sfu_speakers(session, params).await,
	...
	async fn live(
		...
		// BETA 2 CHANGE: Logic for diff/fields split
	...
	async fn select(
		...
		let sql = SelectStatement {
			...
			start: None,
			fetch: None,
			version: None, // Beta 2 cleanup (no Expr::Literal)
			timeout: None, // Beta 2 cleanup
			parallel: false,
			explain: None,
			tempfiles: false,
		};
	...
	async fn rtc_list(&self, _session_id: Option<Uuid>) -> Result<DbResult, RpcError> {
		Err(RpcError::MethodNotFound)
	}
	...
	async fn sfu_list(&self, _session_id: Option<Uuid>) -> Result<DbResult, RpcError> {
		Err(RpcError::MethodNotFound)
	}

	async fn sfu_stats(&self, _session_id: Option<Uuid>) -> Result<DbResult, RpcError> {
		Err(RpcError::MethodNotFound)
	}
