...
use crate::sql::statements::define::{
    ...
    DefineBucketStatement, DefineConfigStatement, DefineCredentialStatement, CredentialType, CredentialAlgorithm,
    DefineDatabaseStatement, DefineDefault,
    DefineEventStatement, EventSchedule, DefineFieldStatement, DefineFunctionStatement, DefineIndexStatement,
    DefineKind, DefineNamespaceStatement, DefineParamStatement, DefineSchedulerStatement, DefineSequenceStatement,
    DefineStatement, DefineTableStatement, DefineUserStatement, DefineWebhookStatement,
    WebhookVerify, WebhookContentType, WebhookHandler, WebhookAuth, WebhookOptions, WebhookResponseMode,
};
...
impl Parser<'_> {
	pub(crate) async fn parse_define_stmt(
		...
		match next.kind {
            ...
            t!("SCHEDULER") => self.parse_define_scheduler(stk).await.map(DefineStatement::Scheduler),
			t!("MODULE") => self.parse_define_module(stk).await.map(DefineStatement::Module),
            t!("WEBHOOK") => self.parse_define_webhook(stk).await.map(DefineStatement::Webhook),
            t!("CREDENTIAL") => self.parse_define_credential(stk).await.map(DefineStatement::Credential),
	...
	pub(crate) async fn parse_define_event(
		...
		loop {
			match self.peek_kind() {
				...
				t!("THEN") => {
					self.pop_peek();
					if self.eat(t!("SCHEDULE")) {
						let scheduler = stk.run(|ctx| self.parse_expr_field(ctx)).await?;
						let mut payload = None;
						if self.eat(t!("PAYLOAD")) {
							payload = Some(stk.run(|ctx| self.parse_expr_field(ctx)).await?);
						}
						res.schedule = Some(EventSchedule {
							scheduler,
							payload,
						});
					} else {
	...
	pub(crate) async fn parse_define_scheduler(
		&mut self,
		stk: &mut Stk,
	) -> ParseResult<DefineSchedulerStatement> {
		let kind = if self.eat(t!("OVERWRITE")) {
			DefineKind::Overwrite
		} else if self.eat(t!("IF")) {
			expected!(self, t!("NOT"));
			expected!(self, t!("EXISTS"));
			DefineKind::IfNotExists
		} else {
		...
		expected!(self, t!("ON"));
		let base = self.parse_base()?;
		expected!(self, t!("ACTION"));
		let action = stk.run(|stk| self.parse_expr_field(stk)).await?;
		let enabled = if self.eat(t!("DISABLED")) {
			false
		} else {
			true
		};
		let mut comment = Expr::Literal(Literal::None);
		if self.eat(t!("COMMENT")) {
			comment = stk.run(|stk| self.parse_expr_field(stk)).await?;
		}
		Ok(DefineSchedulerStatement {
			name,
			kind,
			base,
			action,
    ...
    /// Parse DEFINE WEBHOOK statement
    pub(crate) async fn parse_define_webhook(
        &mut self,
        stk: &mut Stk,
    ) -> ParseResult<DefineWebhookStatement> {
        // Parse IF NOT EXISTS / OVERWRITE
        let kind = if self.eat(t!("OVERWRITE")) {
            DefineKind::Overwrite
        } else if self.eat(t!("IF")) {
            expected!(self, t!("NOT"));
        ...
        // Parse ON PATH (path is an expression, will be evaluated to string)
        expected!(self, t!("ON"));
        expected!(self, t!("PATH"));
        let path = stk.run(|stk| self.parse_expr_field(stk)).await?;

        // Parse METHOD (optional, default POST, support multiple)
        let mut methods = Vec::new();
        if self.eat(t!("METHOD")) {
            loop {
                let token = self.next();
                methods.push(match token.kind {
                    t!("GET") => "GET".to_string(),
                    t!("POST") => "POST".to_string(),
                    t!("PUT") => "PUT".to_string(),
                    t!("DELETE") => "DELETE".to_string(),
                    t!("PATCH") => "PATCH".to_string(),
                    t!("HEAD") => "HEAD".to_string(),
                    _ => unexpected!(self, token, "a valid HTTP method (GET, POST, PUT, DELETE, PATCH, HEAD)"),
                });
                if !self.eat(t!(",")) {
                    break;
                }
        ...
        // Parse AUTH (optional) - authentication for the endpoint itself
        let (auth, auth_credential) = if self.eat(t!("AUTH")) {
            let token = self.next();
            let auth_type = match token.kind {
                t!("NONE") => WebhookAuth::None,
                t!("BASIC") => WebhookAuth::Basic,
                t!("BEARER") => WebhookAuth::Bearer,
                t!("HEADER") => {
                    let header_name = self.parse_string_lit()?;
                    WebhookAuth::Header(header_name)
                }
                t!("JWT") => WebhookAuth::Jwt,
                _ => unexpected!(self, token, "an auth type (NONE, BASIC, BEARER, HEADER, JWT)"),
            };
            
            let cred = if self.eat(t!("CREDENTIAL")) {
                Some(stk.run(|stk| self.parse_expr_field(stk)).await?)
            } else {
                None
            };
        ...
        // Parse VERIFY (optional) - signature verification
        let verify = if self.eat(t!("VERIFY")) {
            let token = self.next();
            match token.kind {
                t!("NONE") => WebhookVerify::None,
                t!("HMAC") => WebhookVerify::Hmac,
                t!("STRIPE") => WebhookVerify::Stripe,
                t!("RSA") => WebhookVerify::Rsa,
                _ => unexpected!(self, token, "a verification mode (NONE, HMAC, STRIPE, RSA)"),
            }
        } else {
            WebhookVerify::None
        };

        // Parse SECRET (optional)
        let secret = if self.eat(t!("SECRET")) {
            Some(stk.run(|stk| self.parse_expr_field(stk)).await?)
        } else {
            None
        };
        ...
        let content_type = if self.eat(t!("CONTENT")) {
            expected!(self, t!("TYPE"));
            let token = self.next();
            match token.kind {
                t!("JSON") => WebhookContentType::Json,
                t!("RAW") => WebhookContentType::Raw,
                t!("FORM") => WebhookContentType::Form,
                t!("BINARY") => WebhookContentType::Binary,
                _ => unexpected!(self, token, "a content type (JSON, RAW, FORM, BINARY)"),
            }
        } else {
            WebhookContentType::Json
        };

        // Parse HANDLER (required)
        expected!(self, t!("HANDLER"));
        let handler = if self.eat(t!("EVENT")) {
            let event_expr = stk.run(|stk| self.parse_expr_field(stk)).await?;
            WebhookHandler::Event(event_expr)
        } else {
            let expr = stk.run(|stk| self.parse_expr_field(stk)).await?;
            WebhookHandler::Function(expr)
        };

        // Extended parsing
        let mut respond = WebhookResponseMode::Immediate;
        let mut response_code = None;
        let mut whitelist = None;
        let mut options = WebhookOptions::default();
        ...
        loop {
            match self.peek_kind() {
                t!("RESPOND") => {
                    self.pop_peek();
                    let token = self.next();
                    respond = match token.kind {
                        t!("IMMEDIATE") => WebhookResponseMode::Immediate,
                        t!("HANDLER") => WebhookResponseMode::Handler,
                        t!("STREAMING") => WebhookResponseMode::Streaming,
                        _ => unexpected!(self, token, "a response mode (IMMEDIATE, HANDLER, STREAMING)"),
                    };
                }
                t!("RESPONSE") => {
                    self.pop_peek();
                    ...
                    match token.kind {
                        t!("CODE") => {
                            response_code = Some(self.next_token_value::<u16>()?);
                        }
                        t!("HEADERS") => {
                            let open_span = expected!(self, t!("{")).span;
                        ...
                        _ => unexpected!(self, token, "CODE or HEADERS"),
                    }
                }
                t!("WHITELIST") => {
                    self.pop_peek();
                    // whitelist is a comma separated string list
                    let s = self.parse_string_lit()?;
                    let list = s.split(',').map(|s| s.trim().to_string()).collect();
                    whitelist = Some(list);
                }
                t!("OPTIONS") => {
                    self.pop_peek();
                    ...
                    while !self.eat(t!("}")) {
                        ...
                        match key.as_str() {
                            "ignore_bots" => {
                                let token = self.next();
                                options.ignore_bots = match token.kind {
                                    t!("true") => true,
                                    t!("false") => false,
                                    _ => unexpected!(self, token, "true or false"),
                                };
                            }
                            "raw_body" => {
                                let token = self.next();
                                options.raw_body = match token.kind {
                                    t!("true") => true,
                                    t!("false") => false,
                                    _ => unexpected!(self, token, "true or false"),
                                };
                            }
                            _ => unexpected!(self, self.peek(), "ignore_bots or raw_body"),
                        }
                        if !self.eat(t!(",")) {
                            self.expect_closing_delimiter(t!("}"), open_span)?;
                            break;
        ...
        // Parse ENABLED/DISABLED (optional, default ENABLED)
        let enabled = !self.eat(t!("DISABLED"));
    ...
    /// Parse DEFINE CREDENTIAL statement
    pub(crate) async fn parse_define_credential(
        &mut self,
        stk: &mut Stk,
    ) -> ParseResult<DefineCredentialStatement> {
        // Parse IF NOT EXISTS / OVERWRITE
        let kind = if self.eat(t!("OVERWRITE")) {
            DefineKind::Overwrite
        } else if self.eat(t!("IF")) {
        ...
        let credential_type = {
            let token = self.next();
            match token.kind {
                t!("API") => CredentialType::Api,
                t!("WEBHOOK") => CredentialType::Webhook,
                t!("OAUTH") => CredentialType::OAuth,
                t!("JWT") => CredentialType::Jwt,
                t!("CUSTOM") => CredentialType::Custom,
                _ => unexpected!(self, token, "a credential type (API, WEBHOOK, OAUTH, JWT, CUSTOM)"),
            }
        };
        ...
        loop {
            match self.peek_kind() {
                t!("ALGORITHM") => {
                    self.pop_peek();
                    let token = self.next();
                    algorithm = Some(match token.kind {
                        t!("HMAC_SHA256") => CredentialAlgorithm::HmacSha256,
                        t!("HMAC_SHA512") => CredentialAlgorithm::HmacSha512,
                        t!("RSA") => CredentialAlgorithm::Rsa,
                        t!("ED25519") => CredentialAlgorithm::Ed25519,
                        _ => unexpected!(self, token, "an algorithm (HMAC_SHA256, HMAC_SHA512, RSA, ED25519)"),
                    });
                }
                t!("HEADERS") => {
                    self.pop_peek();
                    header = Some(stk.run(|stk| self.parse_expr_field(stk)).await?);
                }
                t!("EXPIRES") => {
                    self.pop_peek();
                    expires = Some(stk.run(|stk| self.parse_expr_field(stk)).await?);
                }
                t!("REFRESH") => {
                    self.pop_peek();
                    refresh = Some(stk.run(|stk| self.parse_expr_field(stk)).await?);
                }
                t!("COMMENT") => {
                    self.pop_peek();
                    comment = Some(stk.run(|stk| self.parse_expr_field(stk)).await?);
                }
                _ => break,
            }
        }

        Ok(DefineCredentialStatement {
            kind,
            name,
            credential_type,
            value,
            ...
            refresh,
            comment,
            permissions: crate::catalog::Permission::default(),
        })
    }
}
