...
use crate::syn::token::{Glued, SystemKeyword, Span, TokenKind, t};
...
impl Parser<'_> {
	...
	fn parse_geometry_kind(&mut self) -> ParseResult<GeometryKind> {
		let next = self.next();
		match next.kind {
			TokenKind::System(keyword) => match keyword {
				SystemKeyword::Point => Ok(GeometryKind::Point),
				SystemKeyword::Line => Ok(GeometryKind::Line),
				SystemKeyword::Polygon => Ok(GeometryKind::Polygon),
				SystemKeyword::MultiPoint => Ok(GeometryKind::MultiPoint),
				SystemKeyword::MultiLine => Ok(GeometryKind::MultiLine),
				SystemKeyword::MultiPolygon => Ok(GeometryKind::MultiPolygon),
				SystemKeyword::Collection => Ok(GeometryKind::Collection),
