...
impl LyxalRevisioned for Idiom {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for Idiom {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		&self,
		writer: &mut W,
	) -> Result<(), lyxal_revision::Error> {
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(&self.to_raw_string(), writer)
	}
}

impl DeserializeLyxalRevisioned for Idiom {
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, lyxal_revision::Error> {
		let s: String = DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(reader)?;
		let idiom =
