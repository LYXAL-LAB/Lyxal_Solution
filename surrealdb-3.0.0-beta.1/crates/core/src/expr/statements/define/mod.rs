...
pub(crate) use webhook::{DefineWebhookStatement, WebhookVerify, WebhookContentType, WebhookHandler, WebhookAuth, WebhookResponseMode, WebhookOptions};
...
impl DefineStatement {
	...
	pub(crate) async fn compute(
		...
		match self {
            ...
            Self::Scheduler(v) => v.compute(stk, ctx, opt, doc).await,
			Self::Sequence(v) => v.compute(stk, ctx, opt, doc).await,
			Self::Module(v) => v.compute(stk, ctx, opt, doc).await,
            Self::Webhook(v) => v.compute(stk, ctx, opt, doc).await,
            Self::Credential(v) => v.compute(stk, ctx, opt, doc).await,
		}
	}
}
