...
pub(crate) enum InfoStatement {
	// lyxal_revision discriminant override accounting for previous behavior when adding variants and
			...
			InfoStatement::Db(structured, version) => {
				...
				let res = if *structured {
					let object = map! {
						...
						"schedulers".to_string() => process(txn.all_db_schedulers(ns, db).await?),
					};
					...
					let object = map! {
						...
						"schedulers".to_string() => {
							let mut out = Object::default();
							for v in txn.all_db_schedulers( ns, db).await?.iter() {
								out.insert(v.name.clone(), v.to_sql().into());
							}
							out.into()
						},
