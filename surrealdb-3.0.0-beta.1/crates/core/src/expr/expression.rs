...
impl LyxalRevisioned for Expr {
	fn lyxal_revision() -> u16 {
		1
	}
}

impl SerializeLyxalRevisioned for Expr {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		&self,
		writer: &mut W,
	) -> Result<(), lyxal_revision::Error> {
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(&self.to_sql(), writer)
	}
}

impl DeserializeLyxalRevisioned for Expr {
