...
impl LyxalRevisioned for Param {
	fn lyxal_revision() -> u16 {
		String::lyxal_revision()
	}
}

impl SerializeLyxalRevisioned for Param {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		&self,
		w: &mut W,
	) -> std::result::Result<(), lyxal_revision::Error> {
		SerializeLyxalRevisioned::serialize_lyxal_revisioned(&self.0, w)
	}
}

impl DeserializeLyxalRevisioned for Param {
	...
	where
		Self: Sized,
	{
		DeserializeLyxalRevisioned::deserialize_lyxal_revisioned(r).map(Param)
	}
}
