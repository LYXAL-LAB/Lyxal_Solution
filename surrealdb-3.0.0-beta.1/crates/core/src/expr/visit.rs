...
use crate::expr::statements::{
	...
	DefineModuleStatement, DefineNamespaceStatement, DefineParamStatement, DefineSchedulerStatement, DefineStatement,
	...
	OutputStatement, RelateStatement, RemoveAccessStatement, RemoveAnalyzerStatement, RemoveApiStatement, RemoveBucketStatement,
	...
	RemoveParamStatement, RemoveSchedulerStatement, RemoveSequenceStatement, RemoveStatement, RemoveTableStatement, RemoveUserStatement,
...
implement_visitor! {
	...
	fn visit_remove(this, r: &RemoveStatement){
		match r {
			...
			RemoveStatement::Scheduler(r) => {
				this.visit_remove_scheduler(r)?;
			},
			...
			RemoveStatement::Webhook(r) => {
				this.visit_remove_webhook(r)?;
			},
			RemoveStatement::Credential(_) => {
				// Credential removal has no expressions to visit
			},
		}
		Ok(())
	}

	fn visit_remove_webhook(this, r: &super::statements::RemoveWebhookStatement){
	...
	fn visit_remove_scheduler(this, r: &RemoveSchedulerStatement){
		this.visit_expr(&r.name)?;
		Ok(())
	}
	...
	fn visit_define(this, d: &DefineStatement){
		match d {
            ...
            DefineStatement::Scheduler(d) => {
                this.visit_define_scheduler(d)?;
            },
			DefineStatement::Module(d) => {
				this.visit_define_module(d)?;
			},
			DefineStatement::Webhook(d) => {
				this.visit_define_webhook(d)?;
			},
			DefineStatement::Credential(d) => {
				this.visit_define_credential(d)?;
			},
		}
		Ok(())
	}

	fn visit_define_credential(this, d: &super::statements::DefineCredentialStatement) {
	...
	fn visit_define_webhook(this, d: &super::statements::DefineWebhookStatement) {
		this.visit_expr(&d.name)?;
		...
		match &d.handler {
			super::statements::define::WebhookHandler::Function(e) => {
				this.visit_expr(e)?;
			},
			super::statements::define::WebhookHandler::Event(e) => {
				this.visit_expr(e)?;
			},
		}
		if let Some(c) = &d.comment {
	...
	fn visit_define_scheduler(this, d: &DefineSchedulerStatement) {
		// Visite du nom et de l'action (Correction appliquée)
		this.visit_expr(&d.name)?;
		this.visit_expr(&d.action)?;
		if let Some(c) = &d.comment {
			this.visit_expr(c)?;
		}
		Ok(())
...
implement_visitor_mut! {
	...
	fn visit_mut_remove(this, r: &mut RemoveStatement){
		match r {
			...
			RemoveStatement::Scheduler(r) => {
				this.visit_mut_remove_scheduler(r)?;
			},
			RemoveStatement::Access(r) => {
			...
			RemoveStatement::Webhook(r) => {
				this.visit_mut_remove_webhook(r)?;
			},
			RemoveStatement::Credential(_) => {
				// Credential removal has no expressions to visit
			},
	...
	fn visit_mut_remove_webhook(this, r: &mut super::statements::RemoveWebhookStatement){
	...
	fn visit_mut_remove_scheduler(this, r: &mut RemoveSchedulerStatement){
		this.visit_mut_expr(&mut r.name)?;
		Ok(())
	}
	...
	fn visit_mut_define(this, d: &mut DefineStatement){
		match d {
            ...
            DefineStatement::Scheduler(d) => {
                this.visit_mut_define_scheduler(d)?;
            },
			DefineStatement::Module(d) => {
				this.visit_mut_define_module(d)?;
			},
			DefineStatement::Webhook(d) => {
				this.visit_mut_define_webhook(d)?;
			},
			DefineStatement::Credential(_) => {
				// Credential definition has no expressions to visit
			},
		}
		Ok(())
	}

	fn visit_mut_define_webhook(this, d: &mut super::statements::DefineWebhookStatement) {
		this.visit_mut_expr(&mut d.name)?;
		this.visit_mut_expr(&mut d.path)?;
		if let Some(s) = &mut d.secret {
			this.visit_mut_expr(s)?;
		}
		match &mut d.handler {
			super::statements::define::WebhookHandler::Function(e) => {
				this.visit_mut_expr(e)?;
    ...
    fn visit_mut_define_scheduler(this, d: &mut DefineSchedulerStatement) {
        this.visit_mut_expr(&mut d.name)?;
        this.visit_mut_expr(&mut d.action)?;
        if let Some(c) = &mut d.comment {
            this.visit_mut_expr(c)?;
        }
        Ok(())
