...
export function useRouteMatcher(match: string[]) {
	...
	const [pathname] = useAbsoluteLocation();
	const search = useSearch();
	const location = pathname + search;

	return match.some((m) => {
		if (m.includes("?")) {
			const [pathPattern, queryStr] = m.split("?");
			const [isMatch] = matchRoute(parser, pathPattern, pathname);
			if (!isMatch) return false;
			const actualQuery = location.includes("?") ? location.split("?")[1] : "";
			...
			for (const [k, v] of targetParams.entries()) {
				const actualVal = actualParams.get(k);
				if (k === "tab" && !actualVal && v === "dashboard") {
					continue;
				}
				if (actualVal !== v) return false;
			}
			return true;
		}
		return matchRoute(parser, m, pathname)[0];
	});
}
