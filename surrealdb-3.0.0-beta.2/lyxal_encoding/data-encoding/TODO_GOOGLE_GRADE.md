...
- [x] **Résolution des Warnings (`cargo check`)**
    - [x] Supprimer ou corriger les imports inutilisés (`unused imports`) dans `arithmetic.rs` et `lib.rs`.
    - [x] Gérer les variables assignées mais jamais lues (prefixer par `_`).
    - [x] Supprimer le code mort (`dead_code`) ou l'exposer si nécessaire.
- [x] **Vérification des Features**
    - [x] Vérifier pourquoi `decode_hex_simd` et `decode_base64_simd` sont marqués comme inutilisés.
    - [x] S'assurer que la feature SIMD est correctement détectée et activée par défaut sur x86_64.
- [x] **Formatage**
    - [x] Appliquer `cargo fmt` pour un style de code uniforme.
    - [x] Appliquer `cargo clippy` et corriger les suggestions pertinentes.

## 🟡 Phase 3 : Performance & Métriques

- [ ] **Implémentation du Décodage SIMD**
    - [ ] Ajouter la logique d'appel à `decode_hex_simd` et `decode_base64_simd` dans `decode_wrap_mut`.
    - [ ] Valider que les tests passent avec cette nouvelle logique.
- [ ] **Mise en place de Benchmarks (`criterion`)**
    - [ ] Créer un dossier `benches/`.
    - [ ] Benchmark comparatif : `lyxal_encoding::BASE58` vs crate `bs58`.
    - [ ] Benchmark comparatif : `lyxal_encoding::BASE64` vs crate `base64` (Standard vs SIMD pour encode/decode).
