...
pub(crate) fn encode_to_buffer(
	...
	let base: u32 = alphabet.len().try_into().expect("Alphabet length exceeds u32");
...
pub(crate) fn decode_to_buffer(
	...
	for (i, &byte) in alphabet.iter().enumerate() {
		lookup[byte as usize] = i.try_into().expect("Alphabet too long");
	}
	...
	for byte in input.bytes() {
		match lookup[byte as usize] {
			INVALID_INDEX => {
				...
				let position = input.bytes().position(|b| b == byte).unwrap_or(0);
			...
			index => {
				if let Err(()) = big.mul_add(base, u32::from(index)) {
					return Err(DecodeError {
	...
	let written = big.copy_into_bytes_be(output).map_err(|()| DecodeError {
...
#[cfg(feature = "alloc")]
mod alloc_impl {
	use super::{BigUintView, DecodeError, DecodeKind, EncodeError, EncodeKind, MAX_BIGINT_BUFFER};
	...
	pub(crate) fn decode(alphabet: &[u8], input: &str) -> Result<Vec<u8>, DecodeError> {
		...
		for (position, byte) in input.bytes().enumerate() {
			match lookup[byte as usize] {
				...
				index => {
					big.mul_add(base, u32::from(index));
				}
			}
		}
