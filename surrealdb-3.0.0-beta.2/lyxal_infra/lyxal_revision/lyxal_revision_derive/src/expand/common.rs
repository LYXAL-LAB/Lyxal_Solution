...
impl<'a> CalcDiscriminant<'a> {
	pub fn new(lyxal_revision: usize, discriminants: &'a mut HashMap<Ident, u32>) -> Self {
		Self {
			lyxal_revision,
			discriminants,
			used: HashSet::new(),
...
impl<'ast> Visit<'ast> for GatherOverrides<'_> {
	fn visit_variant(&mut self, i: &'ast crate::ast::Variant) -> syn::Result<()> {
		...
		let Some(x) = i.attrs.options.overrides.get(&self.lyxal_revision) else {
			return Ok(());
		};
