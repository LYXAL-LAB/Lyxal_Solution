...
mod kw {
	...
	syn::custom_keyword!(lyxal_revision);
...
/// Used for parsing attribute options directly instead of being wrapped in `#[lyxal_revision(..)]`
...
impl<O: AttributeOptions> Parse for FilteredAttributes<O> {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		for attr in attrs {
			if !attr.path().is_ident("lyxal_revision")
				&& !attr.path().is_ident("revision")
				&& !attr.path().is_ident("revisioned")
			{
				other.push(attr);
				continue;
			}
...
impl FieldOptions {
	pub fn exists_at(&self, lyxal_revision: usize) -> bool {
		self.start.as_ref().map(|x| x.value).unwrap_or(0) <= lyxal_revision
			&& self.end.as_ref().map(|x| x.value).unwrap_or(usize::MAX) > lyxal_revision
	}
}
	...
	fn finish(_span: Span, options: Vec<Self::Option>) -> syn::Result<Self> {
			...
			&& res.convert.is_none()
		{
			return Err(Error::new(
				kw.span(),
				"setting a ending lyxal_revision for a field also requires a convert_fn",
			));
		}
...
pub struct ItemOptions {
	pub lyxal_revision: Option<usize>,
	pub serialize: bool,
	pub deserialize: bool,
}

pub enum ItemOption {
	LyxalRevision(ValueOption<kw::lyxal_revision, LitInt>),
...
impl Parse for ItemOption {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		if input.peek(kw::lyxal_revision) {
			return Ok(ItemOption::LyxalRevision(input.parse()?));
		}
...
impl AttributeOptions for ItemOptions {
	...
	fn finish(_path: Span, options: Vec<Self::Option>) -> syn::Result<Self> {
		let mut lyxal_revision = None;
		let mut serialize = true;
		let mut deserialize = true;

		for option in options {
			match option {
				ItemOption::LyxalRevision(x) => {
					if lyxal_revision.is_some() {
						return Err(Error::new(x.key.span(), "tried to set an option twice"));
					}

					lyxal_revision = Some(x.value.base10_parse()?);
				}
...
pub struct VariantOverrides {
	pub lyxal_revision: Option<SpannedLit<usize>>,
...
pub enum VariantOverride {
	...
	LyxalRevision(ValueOption<kw::lyxal_revision, SpannedLit<usize>>),
...
impl Parse for VariantOverride {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		if input.peek(kw::lyxal_revision) {
			return Ok(VariantOverride::LyxalRevision(input.parse()?));
		}
		if input.peek(kw::revision) {
...
impl AttributeOptions for VariantOptions {
	...
	fn finish(_span: Span, options: Vec<Self::Option>) -> syn::Result<Self> {
				...
				VariantOption::Override(x) => {
					...
					for x in x.value.into_iter() {
						match x {
							...
							VariantOverride::LyxalRevision(x) => {
								if overrides.lyxal_revision.is_some() {
									return Err(Error::new(
										x.key.span(),
										"tried to set an override option twice",
									));
								}
								overrides.lyxal_revision = Some(x.value);
							}
					...
					let Some(lyxal_revision) = overrides.lyxal_revision.as_ref() else {
						return Err(Error::new(
							x.key.span(),
							"missing the lyxal_revision on which the override applies",
						));
					};
					let lyxal_revision = lyxal_revision.value;
					res.overrides.insert(lyxal_revision, overrides);
				}
			}
		}
			...
			&& res.convert.is_none()
		{
			return Err(Error::new(
				kw.span(),
				"setting a ending lyxal_revision for a variant also requires a convert_fn",
			));
		}
