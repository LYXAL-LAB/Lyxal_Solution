...
impl SerializeLyxalRevisioned for Uuid {
	#[inline]
...
impl DeserializeLyxalRevisioned for Uuid {
	#[inline]
...
impl super::specialised::SerializeLyxalRevisionedSpecialised for Vec<Uuid> {
	#[inline]
...
#[cfg(feature = "specialised")]
impl super::specialised::DeserializeLyxalRevisionedSpecialised for Vec<Uuid> {
	#[inline]
...
mod tests {
	...
	fn test_uuid() {
		...
		let out =
			<Uuid as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_vec_uuid() {
		...
		let out = <Vec<Uuid> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
			.unwrap();
		assert_eq!(val, out);
	}
}
