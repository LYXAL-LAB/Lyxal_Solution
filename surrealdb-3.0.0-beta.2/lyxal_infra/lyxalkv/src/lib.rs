...
// Registry for sharing database handles within the same process.
// This prevents "Access Denied" (os error 33/5) on Windows when multiple parts of the
// binary (like the Kernel and SurrealDB) try to open the same database file.
static TREE_REGISTRY: OnceLock<Mutex<HashMap<PathBuf, Weak<RwLock<lsm::CoreInner>>>>> = OnceLock::new();

fn get_tree_registry() -> &'static Mutex<HashMap<PathBuf, Weak<RwLock<lsm::CoreInner>>>> {
	TREE_REGISTRY.get_or_init(|| Mutex::new(HashMap::new()))
}

pub(crate) fn register_tree_inner(path: &Path, inner: &Arc<RwLock<lsm::CoreInner>>) {
	let mut reg = get_tree_registry().lock();
	reg.insert(path.to_path_buf(), Arc::downgrade(inner));
}

pub(crate) fn get_tree_inner(path: &Path) -> Option<Arc<RwLock<lsm::CoreInner>>> {
	let reg = get_tree_registry().lock();
