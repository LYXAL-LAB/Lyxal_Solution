...
impl BlockCache {
	...
	pub(crate) fn get_data_block(&self, table_id: u64, offset: u64) -> Option<Arc<Block>> {
		...
		if item.is_some() {
			crate::metrics::EngineMetrics::get().block_cache_hits.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
		} else {
			crate::metrics::EngineMetrics::get().block_cache_misses.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
		}
