...
impl Drop for TaskManager {
	fn drop(&mut self) {
		...
		// Signal background tasks to wake up and check the stop flag
		...
		// Spawn memtable compaction task
		{
			...
			let handle = tokio::spawn(async move {
				loop {
					...
					let result = crate::CURRENT_IO_PRIORITY.scope(crate::IoPriority::Background, async {
						core.compact_memtable()
					}).await;
