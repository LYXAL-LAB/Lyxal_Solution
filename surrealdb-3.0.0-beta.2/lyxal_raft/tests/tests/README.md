# Lyxalraft integration tests

The integration tests for Lyxalraft are stored in this directory and rely on
`memstore` with `serde` enabled.
Certain tests in Lyxalraft require the `serde` feature to be disabled.
To avoid enabling `serde` for all tests in Lyxalraft, we must relocate the
