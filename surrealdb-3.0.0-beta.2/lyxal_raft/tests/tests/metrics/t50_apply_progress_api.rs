...
/// Test applied progress API: get() and wait_until_ge()
///
/// What does this test do?
///
/// - Creates a single-node cluster
/// - Gets watch_apply_progress() handle
/// - Verifies initial state with get()
/// - Writes client requests to advance last-applied log id
/// - Uses wait_until_ge() with concrete target value
/// - Verifies get() returns same value as wait_until_ge()
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn apply_progress_api() -> Result<()> {
    let config = Arc::new(
        Config {
            enable_tick: false,
            ..Default::default()
    ...
    tracing::info!(log_index, "--- get applied progress watcher");
    let n0 = router.get_raft_handle(&0)?;
    let progress = n0.watch_apply_progress();

    tracing::info!(log_index, "--- verify initial applied progress with get()");
    let got = progress.get();
    let want = Some(log_id(1, 0, log_index));
    assert_eq!(got, want);

    tracing::info!(log_index, "--- spawn task to wait for future applied progress");
    ...
    tracing::info!(log_index, "--- verify wait_until_ge returns after logs are applied");
