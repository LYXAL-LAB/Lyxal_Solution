...
async fn append_conflicts() -> Result<()> {
    ...
    let req = AppendEntriesRequest {
        vote: Vote::new_committed(1, 2),
        prev_log_id: None,
        entries: vec![blank_ent::<lyxal_raft_memstore::TypeConfig>(0, 0, 0)],
        leader_commit: Some(log_id(1, 0, 2)),
    };
    ...
    let req = || AppendEntriesRequest {
        ...
        entries: vec![
            ...
            blank_ent::<lyxal_raft_memstore::TypeConfig>(1, 0, 2),
    ...
    let req = AppendEntriesRequest {
        ...
        entries: vec![blank_ent::<lyxal_raft_memstore::TypeConfig>(1, 0, 2)],
        leader_commit: Some(log_id(1, 0, 2)),
    };
    ...
    let req = AppendEntriesRequest {
        ...
        entries: vec![blank_ent::<lyxal_raft_memstore::TypeConfig>(2, 0, 3)],
        // this set the last_applied to 2
        leader_commit: Some(log_id(1, 0, 2)),
    };
    ...
    let req = AppendEntriesRequest {
        ...
        prev_log_id: Some(log_id(1, 0, 2)),
        entries: vec![
            blank_ent::<lyxal_raft_memstore::TypeConfig>(2, 0, 3),
            blank_ent::<lyxal_raft_memstore::TypeConfig>(2, 0, 4),
            blank_ent::<lyxal_raft_memstore::TypeConfig>(2, 0, 5),
        ],
        leader_commit: Some(log_id(1, 0, 2)),
    };
    ...
    let req = AppendEntriesRequest {
        ...
        entries: vec![blank_ent::<lyxal_raft_memstore::TypeConfig>(3, 0, 4)],
        leader_commit: Some(log_id(1, 0, 2)),
    };
    ...
    LS: RaftLogStorage<C>,
{
    ...
    let skip = 0;
    let want: Vec<Entry<lyxal_raft_memstore::TypeConfig>> = terms
        .iter()
        .skip(skip)
        .enumerate()
        .map(|(i, term)| blank_ent::<lyxal_raft_memstore::TypeConfig>(*term, 0, (i + skip) as u64))
        .collect::<Vec<_>>();
