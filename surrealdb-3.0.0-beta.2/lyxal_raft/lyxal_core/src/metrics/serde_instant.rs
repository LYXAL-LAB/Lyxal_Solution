...
mod serde_impl {
    ...
    where I: Instant
    {
        ...
        where S: Serializer {
            ...
            let system_time = {
                ...
                if now >= self.inner {
                    let d = now.saturating_duration_since(self.inner);
                    sys_now.checked_sub(d).unwrap_or(sys_now)
                } else {
                    let d = self.inner.saturating_duration_since(now);
                    sys_now.checked_add(d).unwrap_or(sys_now)
                }
            };
            ...
            impl<II: Instant> Visitor<'_> for InstantVisitor<II> {
                ...
                where E: de::Error {
                    ...
                    let instant = if system_time > sys_now {
                        let d = system_time.duration_since(sys_now).unwrap_or_default();
                        now.checked_add(d).unwrap_or(now)
                    } else {
                        let d = sys_now.duration_since(system_time).unwrap_or_default();
    ...
    mod tests {
        ...
        fn test_serde_instant() {
            ...
            let diff = now.saturating_duration_since(*deserialized).max((*deserialized).saturating_duration_since(now));
            assert!(diff < Duration::from_millis(5));

            // Test serialization format

            let nano = "1769566190166498800";
