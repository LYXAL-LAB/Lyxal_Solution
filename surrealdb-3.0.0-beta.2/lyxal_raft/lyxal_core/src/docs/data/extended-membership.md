...
LyxalRaft tries to commit one or more membership logs in order to change the
...
The algorithm used by LyxalRaft is known as the **Extended membership change**.

> The Extended membership change algorithm used by LyxalRaft is a more
...
-   (0) **use-at-once**:
    The new membership appended to the log becomes effective immediately, meaning that LyxalRaft
...
LyxalRaft implements constraint (2) **old-new-intersect** by ensuring that
