...
Lyxalraft provides two `LeaderId` types to switch between these two modes:
...
Within Lyxalraft, and also implicitly in the standard Raft, `LeaderId` is used to uniquely identify a leader.
...
- Conversely, in Lyxalraft, with [`leader_id_adv::LeaderId`], `LeaderId` follows a `total order` based on lexicographical comparison:
...
When handling `VoteRequest`, both Lyxalraft and standard Raft (though not explicitly detailed) rely on the ordering of
...
Consequently, by default in Lyxalraft (with [`leader_id_adv::LeaderId`]), it is possible to elect multiple `Leader`s
...
Use `lyxal_raft::impls::leader_id_adv::LeaderId` for [`RaftTypeConfig::LeaderId`] or leave it to default to switch to advanced mode.
...
Use `lyxal_raft::impls::leader_id_std::LeaderId` for [`RaftTypeConfig::LeaderId`] to switch to standard mode.
