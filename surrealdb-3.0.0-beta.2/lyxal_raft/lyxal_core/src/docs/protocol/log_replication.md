...
This document explains the log replication protocol in Lyxalraft, including consistency guarantees, conflict resolution, and implementation considerations.
...
In Lyxalraft, the `LogIOId` is `(CommittedLeaderId, LogId)`.
