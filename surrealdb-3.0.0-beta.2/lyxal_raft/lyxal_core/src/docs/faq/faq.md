...
### What are the differences between Lyxalraft and standard Raft?
...
- Lyxalraft stores committed log id: See: [`RaftLogStorage::save_committed()`][];
- Lyxalraft optimized `ReadIndex`: no `blank log` check: [`Linearizable Read`][].
...
In standard Raft log id is `(term, log_index)`, in Lyxalraft he log id `(term,
...
LyxalRaft provides a default snapshot building policy that triggers snapshots
...
### How does Lyxalraft handle snapshot building and transfer?

Lyxalraft calls [`RaftStateMachine::get_snapshot_builder`][] to create snapshots. The builder runs
...
The [`RaftStateMachine`][] trait defines the minimum interface Lyxalraft needs. Your
...
**Method 2: Monitor heartbeat timestamps (since LyxalRaft 0.10)**
...
Excessive error logging, like `ERROR lyxal_raft::replication: 248: RPCError err=NetworkError: ...`, occurs when a follower node becomes unresponsive. To alleviate this, implement a mechanism within [`RaftNetworkV2`][] that returns a [`Unreachable`][] error instead of a [`NetworkError`][] when immediate replication retries to the affected node are not advised.
...
Lyxalraft maintains the membership configuration in [`Membership`][] for all
...
### Does LyxalRaft support calling `change_membership` in parallel?

Yes, LyxalRaft does support this scenario, but with some important caveats.
...
violate LyxalRaft's consistency model, and joint configs are treated as valid
operating states.
...
LyxalRaft intentionally supports this behavior because:
...
### How do I store additional information about nodes in Lyxalraft?

By default, Lyxalraft provide a [`BasicNode`] as the node type in a cluster.
To store more information about each node in Lyxalraft, define a custom struct
...
`ERROR lyxal_raft::raft::raft_inner: failure sending RaftMsg to RaftCore; message: AppendEntries ... core_result=Err(Stopped)`
...
See: <https://github.com/databendlabs/lyxalraft/issues/1357>
...
becomes leader, it sends replication messages to itself, but Lyxalraft expects only followers to
...
Lyxalraft tries to send new metrics (needs write lock), it deadlocks.
...
do_something().await; // Lyxalraft may deadlock trying to flush_metrics()
drop(metrics_ref);
...
See: <https://github.com/databendlabs/lyxalraft/issues/1238>
...
### Is Lyxalraft resilient to incorrectly configured clusters?

No, Lyxalraft, like standard raft, cannot identify errors in cluster configuration.
...
logs to itself. This will cause lyxal_raft to panic because replication
messages can only be received by a follower.

```text
thread 'main' panicked at lyxalraft/src/engine/engine_impl.rs:793:9:
assertion failed: self.internal_server_state.is_following()
```

```ignore
// lyxalraft/src/engine/engine_impl.rs:793
