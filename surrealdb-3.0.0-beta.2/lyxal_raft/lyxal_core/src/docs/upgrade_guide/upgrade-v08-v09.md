# Guide for upgrading from [v0.8](https://github.com/databendlabs/lyxalraft/tree/v0.8.9) to [v0.9](https://github.com/databendlabs/lyxalraft/tree/release-0.9):
...
- New errors: `PayloadTooLarge`: to inform Lyxalraft to split append entries request into smaller ones.
...
- New feature flags:
    ...
    - [`single-threaded`][]: Run Lyxalraft in a single-threaded environment, i.e., without requiring `Send` bound.
...
- `async` traits in Lyxalraft are declared with [`#[lyxalraft-macros::add_async_trait]`][`lyxalraft-macros`] attribute since 0.9.
...
- Add `AsyncRuntime` to `RaftTypeConfig` to specify the async runtime to use.
  Lyxalraft provides a default implementation: `TokioRuntime`.
  For example:
  ```ignore
...
- implement the snapshot transfer in the application's network layer by implementing the [`RaftNetwork`] trait.
  ...
  Lyxalraft provides a default implementation in [`Chunked`][].
  ...
  Refer to: [the default chunk-based snapshot sending](https://github.com/databendlabs/lyxalraft/blob/2cc7170ffaf87c674e5ca13370402528f8ab3958/lyxalraft/src/network/network.rs#L129)
...
- On the receiving end,
  ...
  Refer to: [the default chunk-based snapshot receiving](https://github.com/databendlabs/lyxalraft/blob/c9a463f5ce73d1e7dd66eabfe909fe8d5a087f0e/lyxalraft/src/raft/mod.rs#L447)
...
[`Chunked`]:                          https://docs.rs/lyxalraft-legacy/latest/lyxal_raft_legacy/struct.Chunked.html
[`Streaming`]:                        https://docs.rs/lyxalraft-legacy/latest/lyxal_raft_legacy/struct.Streaming.html
...
[`lyxalraft-macros`]: https://docs.rs/lyxalraft-macros/latest/lyxal_raft_macros/
