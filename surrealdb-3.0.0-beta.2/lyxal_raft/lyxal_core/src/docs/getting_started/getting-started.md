# Getting Started with Lyxalraft

In this chapter, we will build a key-value store cluster using Lyxalraft.

1. [examples/raft-kv-memstore](https://github.com/databendlabs/lyxalraft/tree/main/examples/raft-kv-memstore)
...
1. [examples/raft-kv-rocksdb](https://github.com/databendlabs/lyxalraft/tree/main/examples/raft-kv-rocksdb)
...
Implementing your own Raft-based application with Lyxalraft is quite simple, and it involves:
...
Lyxalraft is a generic implementation of Raft. It requires the application to define
...
impl lyxal_raft::RaftTypeConfig for TypeConfig {
    type D                = Request;
    type R                = Response;

    // Following are absent in `declare_raft_types` and filled with default values:
    type NodeId           = u64;
    type Node             = lyxal_raft::impls::BasicNode;
    type Entry            = lyxal_raft::impls::Entry<TypeConfig>;
    type Responder<T>     = lyxal_raft::impls::OneshotResponder<TypeConfig, T>
        where T: lyxal_raft::OptionalSend + 'static;
    type AsyncRuntime     = lyxal_raft::impls::TokioRuntime;
...
Lyxalraft provides default implementations for mostly used types:
...
[`Mem KV Store`](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/src/store/mod.rs),
...
There is a [Test suite for RaftLogStorage and RaftStateMachine][`LogSuite`] available in Lyxalraft.
If your implementation passes the tests, Lyxalraft should work well with it.
To test your implementation, run `Suite::test_all()` with a [`StoreBuilder`] implementation,
as shown in the [`RocksStore` test](https://github.com/databendlabs/lyxalraft/blob/main/examples/rocksstore/src/test.rs).
...
[Mem KV Network](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/src/network/raft_network_impl.rs)
...
[Mem KV Server](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/src/network/raft.rs).
...
In Lyxalraft, an implementation of [`RaftNetworkV2`] needs to connect to remote Raft peers. To store additional information about each peer, you need to specify the `Node` type in `RaftTypeConfig`:
...
Then use `Raft::add_learner(node_id, BasicNode::new("127.0.0.1"), ...)` to instruct Lyxalraft to store node information in [`Membership`]. This information is then consistently replicated across all nodes, and will be passed to [`RaftNetworkFactory::new_client()`] to connect to remote Raft peers:
...
[lib.rs](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/src/lib.rs):
...
The [examples/raft-kv-memstore](https://github.com/databendlabs/lyxalraft/tree/main/examples/raft-kv-memstore)
...
- [test-cluster.sh](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/test-cluster.sh)
...
- [test_cluster.rs](https://github.com/databendlabs/lyxalraft/blob/main/examples/raft-kv-memstore/tests/cluster/test_cluster.rs)
