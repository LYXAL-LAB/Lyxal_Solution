...
use futures::Stream;
use lyxal_raft_macros::add_async_trait;
...
where C: RaftTypeConfig
{
    ...
    ///   is lost and `applied_state()` returns `None` or an old value. Lyxalraft will automatically
    ...
    ///   - `true`: Implementation **must** return `Some(builder)`. LyxalRaft uses this when a
    ///     snapshot is required for replication (e.g., logs have been purged and a follower needs
    ///     to catch up via snapshot).
    ///   - `false`: Implementation **may** return `None` to defer snapshot creation. LyxalRaft uses
    ...
    ///
    /// Lyxalraft will use this handle to receive snapshot data.
    ///
