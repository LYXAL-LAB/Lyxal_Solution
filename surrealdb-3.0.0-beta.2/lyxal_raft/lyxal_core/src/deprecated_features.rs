...
compile_error!(
    "The feature flag `adapt-network-v1` is removed since `0.10.0`. \
     ...
     use the `lyxalraft-legacy` crate instead."
);
...
- either add `LeaderId = lyxal_raft::impls::leader_id_std::LeaderId` to `declare_raft_types!(YourTypeConfig)` statement,
- or add `type LeaderId: lyxal_raft::impls::leader_id_std::LeaderId` to the `RaftTypeConfig` implementation."#
);
