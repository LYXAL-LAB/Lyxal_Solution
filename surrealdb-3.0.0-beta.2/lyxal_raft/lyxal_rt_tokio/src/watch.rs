use lyxal_raft_rt::OptionalSend;
use lyxal_raft_rt::OptionalSync;
use lyxal_raft_rt::watch;
	...
	T: OptionalSend + OptionalSync,
{
	fn changed(
		&mut self,
	) -> impl std::future::Future<Output = Result<(), watch::RecvError>> + std::marker::Send {
		async move { self.0.changed().await.map_err(|_| watch::RecvError(())) }
	}
