...
impl SerializeRevisioned for TableName {
	fn serialize_lyxal_revisioned<W: Write>(&self, w: &mut W) -> Result<(), revision::Error> {
		<String as SerializeRevisioned>::serialize_lyxal_revisioned(&self.0, w)
	}
}

impl DeserializeRevisioned for TableName {
	fn deserialize_lyxal_revisioned<R: Read>(r: &mut R) -> Result<Self, revision::Error> {
		let s = <String as DeserializeRevisioned>::deserialize_lyxal_revisioned(r)?;
		Ok(TableName(s))
	}
}
