...
use crate::{CommunityComposer, LyxalComposer, syn};
...
impl TransactionFactory {
	...
	/// Returns the underlying LyxalKV tree if this is a LyxalKV datastore.
	fn lyxalkv_tree(&self) -> Option<Arc<lyxalkv::Tree>> {
		self.builder.lyxalkv_tree()
	}
}
...
impl TransactionBuilderFactory for LyxalComposer {
	async fn new_transaction_builder(
		...
		CommunityComposer().new_transaction_builder(path, clock, _canceller).await
	}
...
impl TransactionBuilder for DatastoreFlavor {
	...
	fn lyxalkv_tree(&self) -> Option<Arc<lyxalkv::Tree>> {
		match self {
			#[cfg(feature = "kv-lyxalkv")]
			Self::LyxalKV(v) => Some(v.inner_db()),
			_ => None,
		}
	}
...
impl Datastore {
	...
	/// Returns the underlying LyxalKV tree if this is a LyxalKV datastore.
	pub fn lyxalkv_tree(&self) -> Option<Arc<lyxalkv::Tree>> {
		self.transaction_factory.lyxalkv_tree()
	}
