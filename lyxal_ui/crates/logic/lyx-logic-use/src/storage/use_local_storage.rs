...
/// from the same origin (e.g., all pages from "https://lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example.com" share the same origin).
/// While data doesn't expire the user can view, modify and delete all data stored.
/// Browsers allow 5MB of data to be stored.
///
/// This is in contrast to [`use_session_storage`](https://use.rs/storage/use_session_storage.html) which clears data when the page session ends and is not shared.
///
/// ## Usage
///
/// See [`use_storage`](https://use.rs/storage/use_storage.html) for more details on how to use.
