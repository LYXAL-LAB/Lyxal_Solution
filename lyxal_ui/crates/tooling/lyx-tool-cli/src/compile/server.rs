...
pub async fn lyx-platform-lyx_platform_lyx-platform-lyx_platform_server(
proj: &Arc<Project>,
changes: &ChangeSet,
...
if !changes.need_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_build() {
return Ok(Outcome::Success(Product::None));
}

let (envs, line, process) = lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cargo_process("build", &proj)?;
...
.dot()?;
if changed {
log::debug!("Cargo lyx-platform-lyx_platform_lyx-platform-lyx_platform_server bin changed");
Ok(Outcome::Success(Product::Server))
} else {
log::debug!("Cargo lyx-platform-lyx_platform_lyx-platform-lyx_platform_server bin unchanged");
...
pub fn lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cargo_process(cmd: &str, proj: &Project) -> Result<(String, String, Child)> {
...
let (envs, line) = build_cargo_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cmd(cmd, proj, &mut command);
Ok((envs, line, command.spawn()?))
}

pub fn build_cargo_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cmd(
cmd: &str,
proj: &Project,
command: &mut Command,
) -> (String, String) {
...
let lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_is_wasm = match &proj.bin.target_triple {
Some(t) => t.contains("wasm"),
None => false,
};
if cmd != "test" && !lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_is_wasm {
args.push(format!("--bin={}", proj.bin.target))
} else if cmd != "test" && lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_is_wasm {
