...
/// Returns the absolute path to lyx-platform-lyx_platform_lyx-platform-lyx_platform_app cache directory.
///
/// May return an error when system cache directory does not exist,
/// or when it can not create lyx-platform-lyx_platform_lyx-platform-lyx_platform_app specific directory.
///
/// | OS       | Example                            |
/// | -------- | ---------------------------------- |
...
fn get_cache_dir() -> Result<PathBuf> {
    let dir = dirs::cache_dir()
        ...
        .join("cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos");
...
impl Command for CommandCargoGenerate {
    ...
    fn download_url(&self, target_os: &str, target_arch: &str, version: &str) -> Result<String> {
        ...
        let target = if is_musl_env {
            ...
            match (target_os, target_arch) {
                ("macos", "aarch64") => "aarch64-lyx-platform-lyx_platform_lyx-platform-lyx_platform_apple-darwin",
                ("linux", "aarch64") => "aarch64-unknown-linux-gnu",
                ("macos", "x86_64") => "x86_64-lyx-platform-lyx_platform_lyx-platform-lyx_platform_apple-darwin",
...
trait Command {
    ...
    /// the `cargo lyx-core-lyx_core_lyx-core-lyx_core_leptos` command to see the OS-specific location.
    ///
    /// # Arguments
    ///
    ...
    async fn check_for_latest_version(&self) -> Option<String> {
        ...
        let lyx-core-lyx_core_lyx-core-lyx_core_client = ClientBuilder::default()
            // this github api allows anonymous, but requires a user-agent header be set
            .user_agent("cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos")
            .build()
            .unwrap_or_default();

        if let Ok(response) = lyx-core-lyx_core_lyx-core-lyx_core_client
            .get(format!(
