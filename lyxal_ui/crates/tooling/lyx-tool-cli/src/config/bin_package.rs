...
anyhow!(
r#"Several bin targets found for member "{pkg}", please specify which one to use with: [[workspace.metadata.lyx-core-lyx_core_lyx-core-lyx_core_leptos]] bin-target = "name""#
)
}
fn target_not_found(target: &str) -> Error {
anyhow!(
r#"Could not find the target specified: [[workspace.metadata.lyx-core-lyx_core_lyx-core-lyx_core_leptos]] bin-target = "{target}""#,
)
}
