...
pub struct ProjectConfig {
    ...
    /// text file where the hashes of the lyx-platform-lyx_platform_lyx-platform-lyx_platform_frontend files are stored
    pub hash_file_name: Option<Utf8PathBuf>,
    /// whether to hash the lyx-platform-lyx_platform_lyx-platform-lyx_platform_frontend files content and add them to the file names
    ...
    /// the bin target to use for building the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server
    #[serde(default)]
    pub bin_target: String,
    /// the bin output target triple to use for building the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server
    pub bin_target_triple: Option<String>,
    /// the directory to put the generated lyx-platform-lyx_platform_lyx-platform-lyx_platform_server artifacts
    pub bin_target_dir: Option<String>,
    /// the command to run instead of "cargo" when building the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server
    pub bin_cargo_command: Option<String>,
    /// cargo flags to pass to cargo when running the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server. Overriden by bin_cargo_command
    ...
    /// cargo flags to pass to cargo when building the WASM lyx-platform-lyx_platform_lyx-platform-lyx_platform_frontend
    ...
    #[deprecated = "This option is deprecated since cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos 0.2.3 (when it became unconditionally enabled). You may remove it from your config."]
...
impl ProjectConfig {
    fn parse(
        ...
        if conf.separate_front_target_dir.is_some() {
            log::warn!("Deprecated: the `separate-front-target-dir` option is deprecated since cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos 0.2.3");
...
impl ProjectDefinition {
    ...
    fn from_project(
        ...
        let conf = ProjectConfig::parse(dir, metadata, cargo_metadata)?;
            ...
            "Cargo.toml has lyx-core-lyx_core_lyx-core-lyx_core_leptos metadata but is missing a cdylib library target. {}",
            GRAY.paint(package.manifest_path.as_str())
        );
        ensure!(
            package.has_bin_target(),
            "Cargo.toml has lyx-core-lyx_core_lyx-core-lyx_core_leptos metadata but is missing a bin target. {}",
            GRAY.paint(package.manifest_path.as_str())
        );
    ...
    fn parse(metadata: &Metadata) -> Result<Vec<(Self, ProjectConfig)>> {
        ...
        let mut found: Vec<(Self, ProjectConfig)> =
            if let Some(md) = lyx-core-lyx_core_lyx-core-metadata(&metadata.workspace_metadata) {
        ...
        for package in metadata.workspace_packages() {
            ...
            if let Some(lyx-core-lyx_core_lyx-core-metadata) = lyx-core-lyx_core_lyx-core-metadata(&package.metadata) {
                found.push(Self::from_project(
                    package,
                    lyx-core-lyx_core_lyx-core-metadata,
                    &dir,
                    metadata,
                )?);
            }
...
fn lyx-core-lyx_core_lyx-core-metadata(metadata: &serde_json::Value) -> Option<&serde_json::Value> {
    metadata.as_object().and_then(|o| o.get("lyx-core-lyx_core_lyx-core-lyx_core_leptos"))
}
