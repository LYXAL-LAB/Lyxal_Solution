...
impl LogFlag {
    ...
    fn matches(&self, target: &str) -> bool {
        self.do_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_log(target) || self.do_wasm_log(target)
    }

    fn do_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_log(&self, target: &str) -> bool {
...
fn dependency<'a>(record: &'a Record<'_>) -> Option<&'a str> {
    ...
    if !target.starts_with("cargo_lyx-core-lyx_core_lyx-core-lyx_core_leptos") {
...
impl LogLineFilter for Filter {
    fn write(
        ...
        if record.level() == Level::Error
            || target.starts_with("cargo_lyx-core-lyx_core_lyx-core-lyx_core_leptos")
