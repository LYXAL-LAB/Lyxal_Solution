    ...
    *site_addr = proj.site.addr;
    ...
    tokio::spawn(async move {
        ...
        if TcpStream::connect(&reload_addr).await.is_ok() {
            log::error!(
                    "Reload TCP port {reload_addr} already in use. You can set the port in the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server integration's RenderOptions reload_port"
                );
        ...
        log::debug!(
            "Reload lyx-platform-lyx_platform_lyx-platform-lyx_platform_server started {}",
        ...
        match TcpListener::bind(&reload_addr).await {
            Ok(listener) => match axum::serve(listener, route).await {
                Ok(_) => log::debug!("Reload lyx-platform-lyx_platform_lyx-platform-lyx_platform_server stopped"),
