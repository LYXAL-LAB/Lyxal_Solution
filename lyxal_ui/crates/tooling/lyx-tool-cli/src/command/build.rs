...
// lyx-platform-lyx_platform_lyx-platform-lyx_platform_server to make it possible to include them as assets into the binary itself
if proj.release && proj.precompress {
compress::compress_static_files(proj.site.root_dir.clone().into()).await?;
}

if !compile::lyx-platform-lyx_platform_lyx-platform-lyx_platform_server(proj, &changes).await.await??.is_success() {
return Ok(false);
}
