use crate::compile::{front_cargo_process, lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cargo_process};
...
pub async fn test_proj(proj: &Project) -> Result<bool> {
    let (envs, line, mut proc) = lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_cargo_process("test", proj).dot()?;

    let lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_exit_status = proc.wait().await.dot()?;
    log::debug!("Cargo envs: {}", GRAY.paint(envs));
    log::info!("Cargo lyx-platform-lyx_platform_lyx-platform-lyx_platform_server tests finished {}", GRAY.paint(line));
    ...
    Ok(lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_exit_status.success() && front_exit_status.success())
}
```
```
```
