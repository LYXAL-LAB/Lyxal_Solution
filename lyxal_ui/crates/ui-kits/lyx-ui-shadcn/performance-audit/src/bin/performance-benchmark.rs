...
println!("ðŸƒ Running performance benchmarks...");
...
println!("âœ… HTML report saved to {:?}", output_path);
} else {
println!("{}", html_output);
}
}
...
println!("âŒ Some benchmarks failed!");
std::process::exit(1);
}
}
...
println!("âš ï¸ Failed to load baseline: {}", e);
...
println!("ðŸš¨ CRITICAL: {} critical regressions detected!", critical_regressions);
std::process::exit(1);
} else if major_regressions > 0 {
println!("âš ï¸ WARNING: {} major regressions detected!", major_regressions);
std::process::exit(1);
} else {
println!("âœ… No significant regressions detected!");
}
...
println!("ðŸ“ˆ Starting automated performance monitoring...");
...
println!("âœ… Report generated: {:?}", output);
Ok(())
}
...
println!("ðŸ”§ Setting up performance baseline...");
...
println!("âœ… Performance baseline established: {:?}", output);
...
println!("  Meets target: {}", if result.meets_target { "âœ…" } else { "âŒ" });
}
...
if result.meets_target { "âœ… Pass" } else { "âŒ Fail" }
)
}).collect::<Vec<_>>().join("")
)
}
