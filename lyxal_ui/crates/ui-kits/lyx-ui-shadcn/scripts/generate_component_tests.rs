...
if component_name != "lyx-ui-shadcn" { // Skip the main package
...
println!("ðŸš€ Generating comprehensive tests for all {} components...\n", self.components.len());

for component in &self.components {
println!("ðŸ“ Generating tests for: {}", component.name);
...
use super::*;
use leptos::*;
use lyx_ui_test_utils::leptos_testing::{{LeptosTestUtils, ComponentTestBuilder, test_helpers}};
use lyx_ui_test_utils::{{TestResult, Framework, Theme}};
...
use super::*;
use leptos::*;
use lyx_ui_test_utils::leptos_testing::LeptosTestUtils;
...
// Component breakdown
report.push_str("ðŸŽ¯ Component Results:\n");
for (component_name, result) in &self.test_results {
let status = if result.is_successful() { "âœ…" } else { "âŒ" };
...
fn main() {
println!("ðŸš€ Automated Test Generation for Leptos shadcn/ui Components");
println!("ðŸ“… Generation Date: September 3rd, 2025\n");

let mut generator = ComponentTestGenerator::new(".");

// Discover components
match generator.discover_components() {
Ok(_) => println!("âœ… Discovered {} components", generator.components.len()),
Err(e) => {
eprintln!("âŒ Failed to discover components: {}", e);
std::process::exit(1);
}
}
...
Ok(_) => println!("âœ… Test generation completed"),
Err(e) => {
eprintln!("âŒ Failed to generate tests: {}", e);
std::process::exit(1);
}
}
...
println!("\nâš ï¸ Some components may need manual attention:");
