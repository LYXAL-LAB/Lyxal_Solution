...
println!("ðŸ§ª Testing performance for: {}", component);
let passed = self.test_component_performance(component);
if !passed {
all_passed = false;
println!("âŒ Performance test failed for {}", component);
} else {
println!("âœ… Performance test passed for {}", component);
}
}
...
println!("  ðŸ“Š Render Time: {:?} (threshold: {}ms)", render_time, render_threshold);
println!("  ðŸ’¾ Memory Usage: {:.2}KB (threshold: {}KB)", memory_usage, memory_threshold);
println!("  âš¡ Interaction Time: {:?} (threshold: {}ms)", interaction_time, interaction_threshold);
println!("  â±ï¸  Total Test Time: {:?}", total_time);
...
println!("ðŸ“¦ Total Components Tested: {}", total_components);
println!("â±ï¸  Average Render Time: {:?}", avg_render_time);
println!("ðŸ’¾ Average Memory Usage: {:.2}KB", avg_memory_usage);
println!("âš¡ Average Interaction Time: {:?}", avg_interaction_time);

println!("\nðŸ“‹ Component Performance Details:");
for metric in &self.metrics {
println!("  ðŸ“¦ {}:", metric.component_name);
println!("    â±ï¸  Render: {:?}", metric.render_time);
println!("    ðŸ’¾ Memory: {:.2}KB", metric.memory_usage);
println!("    âš¡ Interaction: {:?}", metric.interaction_time);
}
...
println!("  âš ï¸  {}: Consider optimizing render performance", metric.component_name);
}
if metric.memory_usage > 80.0 {
println!("  âš ï¸  {}: Consider reducing memory usage", metric.component_name);
}
if metric.interaction_time.as_millis() > 5 {
println!("  âš ï¸  {}: Consider optimizing interaction performance", metric.component_name);
}
}
}
}
...
println!("ðŸ“Š Large Dataset Performance Test");
println!("  ðŸ“¦ Items: {}", num_items);
...
println!("ðŸ’¾ Memory Usage Performance Test");
println!("  ðŸ“¦ Components Created: {}", num_components);
println!("  ðŸ’¾ Memory Increase: {:.2}KB", memory_increase);
println!("  ðŸ’¾ Per Component: {:.2}KB", memory_increase / num_components as f64);
