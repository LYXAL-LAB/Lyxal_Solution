...
if framework != "leptos" {
eprintln!("{}", style("âš ï¸  Only Leptos framework is currently supported").yellow());
return Ok(());
}
...
if completed {
println!("{}", style("âœ… Completed Components:").bold().green());
for component in &completed_components {
println!("  â€¢ {}", style(component).green());
}
println!();
} else if missing {
println!("{}", style("âŒ Missing Components:").bold().red());
for component in &missing_components {
println!("  â€¢ {}", style(component).red());
}
println!();
} else {
// Show summary
println!("{}", style("ðŸ“Š Component Status:").bold());
...
println!("{}", style("ðŸ’¡ Usage:").bold().yellow());
