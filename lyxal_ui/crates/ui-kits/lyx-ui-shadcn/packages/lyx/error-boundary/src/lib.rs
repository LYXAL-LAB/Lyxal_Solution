...
pub fn ErrorBoundary(
    ...
    let has_error_signal = signal(false);
    let error_info_signal = signal(None::<ErrorInfo>);

    let has_error = has_error_signal.0;
    let set_has_error = has_error_signal.1;
    let _error_info = error_info_signal.0;
    let set_error_info = error_info_signal.1;
...
pub fn RichErrorFallback(
    ...
    let show_details_signal = signal(show_technical.get().unwrap_or(false));
    let show_details = show_details_signal.0;
    let set_show_details = show_details_signal.1;
    ...
    view! {
        <div class="rich-error-fallback" data-severity=move || severity_class()>
            <div class="error-content">
                {children.map(|c| c())}
...
pub fn RichErrorBoundary(
    ...
    let has_error_signal = signal(false);
    let error_context_signal = signal(None::<ErrorContext>);

    let has_error = has_error_signal.0;
    let set_has_error = has_error_signal.1;
    let error_context = error_context_signal.0;
    let set_error_context = error_context_signal.1;
...
pub fn use_error_handler() -> (ReadSignal<bool>, WriteSignal<bool>, WriteSignal<Option<ErrorInfo>>) {
    ...
    (has_error_signal.0, has_error_signal.1, error_info_signal.1)
}
...
pub fn use_rich_error_handler() -> (
    ...
    (
        has_error_signal.0,
        has_error_signal.1,
        error_context_signal.0,
        error_context_signal.1,
    )
}
