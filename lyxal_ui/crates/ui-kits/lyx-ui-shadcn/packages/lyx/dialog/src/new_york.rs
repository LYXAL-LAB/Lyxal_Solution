...
pub fn DialogTrigger(
    ...
    if let Some(as_child) = as_child.as_ref() {
        ...
        view! {
            <button
                node_ref=child_props.node_ref
                class=move || child_props.class.get()
                id=move || child_props.id.get().unwrap_or_default()
                style=move || child_props.style.get().to_string()
                disabled=move || child_props.disabled.get()
                on:click=move |e| {
                    if let Some(ref onclick) = child_props.onclick {
                        onclick.run(e);
                    }
                }
            >
