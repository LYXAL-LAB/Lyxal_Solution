...
if let Some(loading) = loading.get() {
state.update(|s| s.loading = loading);
}
if let Some(error_opt) = error.get() {
state.update(|s| s.error = error_opt);
}
...
view! { <div class="data-table-error">{error}</div> }.into_any()
} else {
view! {
<table class="data-table-table">
<thead>
<tr>
{s.columns.iter().map(|column| {
...
let s = state.get();
let column_key = column.key.clone();
if s.sort_config.column_key == column_key {
match s.sort_config.direction {
