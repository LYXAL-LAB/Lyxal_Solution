...
println!("âœ… Bundle size {} KB is within {} KB limit", actual_size_kb, self.bundle_size_limit_kb);
Ok(())
}
...
println!("âœ… Render time {} ms is within {} ms limit", render_time_ms, self.render_time_limit_ms);
Ok(())
}
...
println!("âœ… WASM compatibility test (simulated for non-WASM target)");
Ok(())
}
...
println!("âš ï¸  Component does not support SSR");
}

if !contract.supports_hydration {
println!("âš ï¸  Component does not support hydration");
}
