...
match command {
"scan" => {
println!("ðŸ” Scanning workspace for packages needing TDD implementation...");

let packages_needing_tdd = manager.scan_workspace()?;

if packages_needing_tdd.is_empty() {
println!("âœ… All packages already have adequate TDD implementation!");
} else {
println!("ðŸ“‹ Found {} packages needing TDD implementation:", packages_needing_tdd.len());
for package in &packages_needing_tdd {
println!("  - {}", package);
}

println!("\nðŸ’¡ Run 'cargo run --package leptos-shadcn-contract-testing --bin tdd_expansion apply' to implement TDD for all packages");
}
}
...
println!("âœ… TDD implementation complete!");
}
...
println!("ðŸ§ª Applying TDD to package: {}", package_name);
...
println!("âœ… TDD implementation complete for {}", package_name);
}

"report" => {
println!("ðŸ“Š Generating TDD implementation report...");
...
println!("ðŸ“„ Report saved to: {}", report_path);
println!("\n{}", report);
}

"validate" => {
println!("âœ… Validating TDD implementation across workspace...");

// Scan workspace
let packages = manager.scan_workspace()?;

if packages.is_empty() {
println!("âœ… All packages have adequate TDD implementation!");
std::process::exit(0);
} else {
println!("âŒ {} packages still need TDD implementation:", packages.len());
...
_ => {
eprintln!("âŒ Unknown command: {}", command);
print_help();
std::process::exit(1);
}
}
