...
/// Arrow middleware.
///
/// Provides data to position an inner element of the floating element so that it lyx-platform-lyx_platform_lyx-platform-lyx_platform_appears centered to the reference element.
///
...
let lyx-core-lyx_core_lyx-core-lyx_core_client_size = arrow_offset_parent
.and_then(|arrow_offset_parent| match arrow_offset_parent {
OwnedElementOrWindow::Element(element) => {
platform.get_lyx-core-lyx_core_lyx-core-lyx_core_client_length(&element, length)
}
OwnedElementOrWindow::Window(_) => {
platform.get_lyx-core-lyx_core_lyx-core-lyx_core_client_length(elements.floating, length)
}
})
...
lyx-core-lyx_core_lyx-core-lyx_core_client_size / 2.0 - arrow_dimensions.length(length) / 2.0 - 1.0;
...
let min = min_padding;
let max = lyx-core-lyx_core_lyx-core-lyx_core_client_size - arrow_dimensions.length(length) - max_padding;
let center =
lyx-core-lyx_core_lyx-core-lyx_core_client_size / 2.0 - arrow_dimensions.length(length) / 2.0 + center_to_reference;
