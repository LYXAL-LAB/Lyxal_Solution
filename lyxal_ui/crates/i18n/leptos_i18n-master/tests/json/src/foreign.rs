...
assert_eq_rendered!(fr, "before Cliquez pour incr̩menter le compteur after");
}
...
assert_eq_rendered!(fr, format!("before Vous avez cliqu̩ {} fois after", count));
}
...
let fr = td!(Locale::fr, foreign_key_to_interpolation, count);
assert_eq_rendered!(fr, "before Vous avez cliqu̩ <p>even a view!</p> fois after");
}
...
assert_eq_rendered!(fr, "before Vous avez cliqu̩ 32 fois after");
}
...
assert_eq_rendered!(fr, "before Vous avez cliqu̩ 67 fois after");
}
...
assert_eq_rendered!(fr, "before Vous avez cliqu̩ subkey_1 fois after");
}
