...
assert_eq_rendered!(fr, format!("Vous avez cliqu̩ {} fois", count));
}
...
let fr = td!(Locale::fr, click_count, count);
assert_eq_rendered!(fr, "Vous avez cliqu̩ <p>even a view!</p> fois");
}
...
assert_eq!(fr, format!("Vous avez cliqu̩ {count} fois"));
}
...
assert_eq!(fr, "Vous avez cliqu̩ beaucoups de fois");
}
