...
{if edit { dimension_name_rs.get() } else { "Create Dimension".to_string() }}
</h1>
...
style_class="h-[200px] max-w-md w-full"
/>
}
}>
{move || {
...
<Suspense fallback=move || {
view! {
<Skeleton variant=SkeletonVariant::Block style_class="h-20 max-w-[936px]" />
}
}>
{move || {
