...
let builder = crate::protocol_serde::shape_delete_variable::ser_delete_variable_headers(input, builder)?;
...
/// Returns `true` if the error kind is `DeleteVariableError::ResourceNotFound`.
pub fn is_resource_not_found(&self) -> bool {
matches!(self, Self::ResourceNotFound(_))
}
/// Returns `true` if the error kind is `DeleteVariableError::InternalServerError`.
