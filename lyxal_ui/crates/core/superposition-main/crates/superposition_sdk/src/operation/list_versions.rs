...
::std::write!(output, "/config/versions").expect("formatting should succeed");
...
let builder = crate::protocol_serde::shape_list_versions::ser_list_versions_headers(input, builder)?;
...
/// Returns `true` if the error kind is `ListVersionsError::InternalServerError`.
