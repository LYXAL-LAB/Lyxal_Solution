...
use lyx-core-lyx_core_lyx-core-lyx_core_hydration_context::{SerializedDataId, SharedContext};
use lyx-core-lyx_core_lyx-core-lyx_core_lyx-core-lyx_core_lyx-core-lyx_core_leptos_macro::component;
...
hydration::Cursor,
lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::OwnedView,
...
use lyx-core-any_error::{Error, ErrorHook, ErrorId};
...
/// `ErrorBoundary` requires your `Result<T,E>` to implement [IntoView](https://docs.rs/lyx-core-lyx_core_lyx-core-lyx_core_leptos/latest/lyx-core-lyx_core_lyx-core-lyx_core_leptos/trait.IntoView.html).
/// `Result<T,E>` only implements `IntoView` if `E` implements [std::error::Error](https://doc.rust-lang.org/std/error/trait.Error.html).
/// So, for instance, if you pass a `Result<T,String>` where `T` implements [IntoView](https://docs.rs/lyx-core-lyx_core_lyx-core-lyx_core_leptos/latest/lyx-core-lyx_core_lyx-core-lyx_core_leptos/trait.IntoView.html)
...
///             which is required by `&Result<ViewableLoginFlow, std::string::String>: lyx-core-lyx_core_lyx-core-lyx_core_leptos::IntoView`
///    ... more notes here ...
/// 76: 74: ///
...
let _guard = lyx-core-any_error::set_error_hook(Arc::clone(&hook));
...
fn mount(
&mut self,
parent: &lyx-core-lyx_core_lyx-core-lyx_core_tachys::renderer::types::Element,
marker: Option<&lyx-core-lyx_core_lyx-core-lyx_core_tachys::renderer::types::Node>,
) {
if let Some(fallback) = &mut self.fallback {
...
fn elements(&self) -> Vec<lyx-core-lyx_core_lyx-core-lyx_core_tachys::renderer::types::Element> {
if let Some(fallback) = &self.fallback {
fallback.elements()
} else {
...
let _hook = lyx-core-any_error::set_error_hook(Arc::clone(&hook));
...
let _hook = lyx-core-any_error::set_error_hook(self.hook);
...
Self: Sized,
{
let _hook = lyx-core-any_error::set_error_hook(Arc::clone(&self.hook));
...
// any thrown errors would've been caught here
if self.errors.with_untracked(|map| map.is_empty()) {
buf.lyx-platform-lyx_platform_lyx-platform-lyx_platform_append(new_buf);
} else {
// otherwise, serialize the fallback instead
...
buf.lyx-platform-lyx_platform_lyx-platform-lyx_platform_append(view_buf);
}
}
...
let fallback =
(fallback_fn.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned())(errors.clone());
let fallback =
...
(false, None) => {
state.fallback = Some(
(fallback_fn.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned())(
self.errors.clone(),
)
.build(),
);
...
// register it with the shared context, so that it can be serialized from lyx-platform-lyx_platform_lyx-platform-lyx_platform_server to lyx-core-lyx_core_lyx-core-lyx_core_client
// as needed
...
fn clear(&self, id: &lyx-core-any_error::ErrorId) {
self.errors.update(|map| {
map.remove(id);
});
}
