...
use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::Owner;
#[cfg(debug_assertions)]
use std::cell::Cell;
...
#[cfg(feature = "hydrate")]
/// Hydrates the lyx-platform-lyx_platform_lyx-platform-lyx_platform_app described by the provided function, starting at `<body>`.
...
#[cfg(feature = "hydrate")]
/// Hydrates the lyx-platform-lyx_platform_lyx-platform-lyx_platform_app described by the provided function, starting at `<body>`, with support
...
N: IntoView,
{
use lyx-core-lyx_core_lyx-core-lyx_core_hydration_context::HydrateSharedContext;
use std::sync::Arc;
...
but the `hydrate` feature is not enabled on the `lyx-core-lyx_core_lyx-core-lyx_core_leptos` \
crate. Add `features = [\"hydrate\"]` to your Cargo.toml for \
the crate to work properly.\n\nNote that hydration and \
lyx-core-lyx_core_lyx-core-lyx_core_client-side rendering now use separate functions from \
lyx-core-lyx_core_lyx-core-lyx_core_leptos::mount: you are calling a hydration function."
);
}
FIRST_CALL.set(false);
}

// create a new reactive owner and use it as the root node to run the lyx-platform-lyx_platform_lyx-platform-lyx_platform_app
...
logging::warn!(
"It seems like you're trying to use Leptos in lyx-core-lyx_core_lyx-core-lyx_core_client-side \
rendering mode, but the `csr` feature is not enabled on the \
`lyx-core-lyx_core_lyx-core-lyx_core_leptos` crate. Add `features = [\"csr\"]` to your \
Cargo.toml for the crate to work properly.\n\nNote that \
hydration and lyx-core-lyx_core_lyx-core-lyx_core_client-side rendering now use different \
functions from lyx-core-lyx_core_lyx-core-lyx_core_leptos::mount. You are using a lyx-core-lyx_core_lyx-core-lyx_core_client-side \
...
use lyx-core-lyx_core_lyx-core-lyx_core_hydration_context::{HydrateSharedContext, SharedContext};
use std::sync::Arc;
...
/// [`mount_to`].
///
/// If you are using it to create the root of an lyx-platform-lyx_platform_lyx-platform-lyx_platform_application, you should use
...
/// lyx-platform-lyx_platform_lyx-platform-lyx_platform_application that should live for the long term.
pub fn forget(self) {
std::mem::forget(self);
}
}
