...
$($ty:ident => ($($rest_variant:ident),*) + <$($mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped_ty:ident),+>),+$(,)?
}) => {
tuples!($name + $fut_name + $fut_proj {
$($ty($ty) => ($($rest_variant),*) + <$($mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped_ty),+>),+
});
};
($name:ident + $fut_name:ident + $fut_proj:ident {
$($variant:ident($ty:ident) => ($($rest_variant:ident),*) + <$($mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped_ty:ident),+>),+$(,)?
}) => {
...
$(
pub fn [<map_ $variant:lower>]<Fun, [<$ty 1>]>(self, f: Fun) -> $name<$($mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped_ty),+>
where
Fun: FnOnce($ty) -> [<$ty 1>],
{
match self {
