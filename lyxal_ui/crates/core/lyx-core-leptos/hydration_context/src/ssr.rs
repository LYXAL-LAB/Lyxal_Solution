...
#[derive(Default)]
/// The shared context that should be used on the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server side.
...
/// Creates a new shared context for rendering HTML on the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server.
pub fn new() -> Self {
Self {
...
/// Creates a new shared context for rendering HTML on the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server in "islands" mode.
///
/// This defaults to a mode in which the lyx-platform-lyx_platform_lyx-platform-lyx_platform_app is not hydrated, but allows you to opt into
...
let sync_data = mem::take(&mut *self.sync_buf.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned());
let async_data = mem::take(&mut *self.async_buf.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned());
...
.field("async_buf", &self.async_buf.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().len())
.finish()
}
}
...
self.async_buf.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push((id, fut))
}
...
self.errors.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push((
error_boundary_id,
error_id,
error,
));
...
mem::take(&mut *self.errors.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned())
}
...
let sync_data = mem::take(&mut *self.sync_buf.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned());
let async_data = self.async_buf.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
for error in mem::take(&mut *self.errors.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned()) {
_ = write!(
initial_chunk,
"[{}, {}, {:?}],",
error.0 .0,
...
for chunk in mem::take(&mut *incomplete.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned()) {
...
self.deferred.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push(wait_for);
}

fn await_deferred(&self) -> Option<PinnedFuture<()>> {
let deferred = mem::take(&mut *self.deferred.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned());
...
self.incomplete.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push(id);
}
...
let mut async_buf = self.async_buf.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
let sealed = self.sealed_error_boundaries.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
for error in mem::take(&mut *self.errors.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned()) {
