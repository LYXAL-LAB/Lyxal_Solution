...
let mut effect_lock = self.effect.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
self.text_stack.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push((id, text));
}
...
let mut text_stack = self.text_stack.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
let mut formatter_stack = self.formatter_stack.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
let text_pos =
...
self.formatter_stack.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().last()
{
...
/// A function that is lyx-platform-lyx_platform_lyx-platform-lyx_platform_applied to the text value before setting `document.title`.
#[repr(transparent)]
...
/// A component to set the documentâ€™s title by creating an [`HTMLTitleElement`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLTitleElement).
///
/// The `title` and `formatter` can be set independently of one another. For lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example, you can create a root-level
...
///     view! {
///       <main>
///         <Title text="Page A"/> // sets title to "Page A â€” Leptos Online"
///       </main>
///     }
/// }
///
...
///     view! {
///       <main>
///         <Title text="Page B"/> // sets title to "Page B â€” Leptos Online"
///       </main>
///     }
/// }
/// 229: 227: #[component]
pub fn Title(
/// A function that will be lyx-platform-lyx_platform_lyx-platform-lyx_platform_applied to any text value before itâ€™s set as the title.
#[prop(optional, into)]
...
let meta = use_head();
let lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_ctx = use_context::<ServerMetaContext>();
let id = meta.title.next_id();
if let Some(cx) = lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_ctx {
// if we are lyx-platform-lyx_platform_lyx-platform-lyx_platform_server rendering, we will not actually use these values via RenderHtml
// instead, they'll be handled separately by the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server integration
...
// so that they no longer lyx-platform-lyx_platform_lyx-platform-lyx_platform_appear. it will also revalidate the whole title in case this one was active
