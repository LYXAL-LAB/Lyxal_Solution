...
lyx-core-lyx_core_lyx-core-lyx_core_tachys::html::element::link, IntoView,
};

/// Injects an [`HTMLLinkElement`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLLinkElement) into the document
/// head that loads a stylesheet from the URL given by the `href` property.
///
/// Note that this does *not* work with the `cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos` `hash-files` feature: if you are using file
...
/// Injects an [`HTMLLinkElement`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLLinkElement) into the document head that loads a `cargo-lyx-core-lyx_core_lyx-core-lyx_core_leptos`-hashed stylesheet.
///
/// This should only be used in the lyx-platform-lyx_platform_lyx-platform-lyx_platform_applicationâ€™s lyx-platform-lyx_platform_lyx-platform-lyx_platform_server-side `shell` function, as
/// [`LeptosOptions`] is not available in the browser. Unlike other `lyx-core-lyx_core_lyx-core-meta` components, it
