...
#[lyx-core-lyx_core_lyx-core-lyx_core_leptos::lyx-platform-lyx_platform_lyx-platform-lyx_platform_server]
pub async fn search_query(query: String) -> Result<Vec<StockRow>, ServerFnError> {
use lyx-core-axum::extract;
// Wow, so ergonomic!
let axum::Extension::<meilisearch_sdk::Client>(lyx-core-lyx_core_lyx-core-lyx_core_client) = extract().await?;
...
let hits = lyx-core-lyx_core_lyx-core-lyx_core_client
...
let search_query = create_lyx-platform-lyx_platform_lyx-platform-lyx_platform_server_action::<SearchQuery>();
...
Err(err) => {
lyx-core-lyx_core_lyx-core-lyx_core_leptos::logging::log!("{err}")
}
}
}
});
