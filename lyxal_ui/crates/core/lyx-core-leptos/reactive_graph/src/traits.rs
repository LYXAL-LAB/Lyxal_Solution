...
//! |-------------------|-------|---------------------------------------------------------------------------------------|
//! | [`Track`]         | â€”     | Tracks changes to this value, adding it as a source of the current reactive oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server. |
//! | [`Notify`]       | â€”      | Notifies subscribers that this value has changed.                                     |
...
//!
//! For lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example, if you have a struct for which you can implement [`ReadUntracked`] and [`Track`], then
//! [`WithUntracked`] and [`With`] will be implemented automatically (as will [`GetUntracked`] and
//! [`Get`] for `Clone` types). But if you cannot implement [`ReadUntracked`] (because, for lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example,
...
use crate::{
effect::Effect,
graph::{Oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server, Source, Subscriber, ToAnySource},
owner::Owner,
...
if let Some(subscriber) = Oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server::get() {
...
This might mean your lyx-platform-lyx_platform_lyx-platform-lyx_platform_app is not responding to \
changes in signal values in the way you \
expect.\n\nHereâ€™s how to fix it:\n\n1. If this is \
inside a `view!` macro, make sure you are passing a \
function, not a value.\n  âŒ NO  <p>{{x.get() * \
2}}</p>\n  âœ… YES <p>{{move || x.get() * \
2}}</p>\n\n2. If itâ€™s in the body of a component, \
try wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apping this access in a closure: \n  âŒ NO  \
let y = x.get() * 2\n  âœ… YES let y = move || \
x.get() * 2.\n\n3. If youâ€™re *trying* to access the \
...
///
/// If your type contains a [`Signal`](crate::wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_appers::read::Signal),
...
/// Give read-only access to a signal's value by reference through a guard type,
/// and subscribes the active reactive oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server (an effect or computed) to changes in its value.
...
//
// This is a problem for e.g. the [`Signal`](crate::wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_appers::read::Signal) type,
...
/// Subscribes to the signal, lyx-platform-lyx_platform_lyx-platform-lyx_platform_applies the closure to the value, and returns the result,
/// or `None` if the signal has already been disposed.
#[track_caller]
...
/// Updates the value of a signal by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a function that updates it in place,
/// without notifying subscribers.
pub trait UpdateUntracked: DefinedAt {
/// The type of the value contained in the signal.
type Value;

/// Updates the value by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a function, returning the value returned by that function.
