...
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::effect::ImmediateEffect;
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::ArenaItem;
/// # let owner = lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::Owner::new(); owner.set();
/// let a = RwSignal::new(0);
/// let b = RwSignal::new(0);
///
/// // âœ… use effects to interact between reactive state and the outside world
/// let _drop_guard = ImmediateEffect::new(move || {
///   // on the next â€œtickâ€ prints "Value: 0" and subscribes to `a`
///   println!("Value: {}", a.get());
/// });
///
...
/// a.set(1);
/// # assert_eq!(a.get(), 1);
/// // âœ… because it's subscribed to `a`, the effect reruns and prints "Value: 1"
/// 50: 48: /// ## Notes
///
...
///    this with a web framework, this generally means that effects **do not run on the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server**.
///    and you can call browser-specific APIs within the effect function without causing issues.
///    If you need an effect to run on the lyx-platform-lyx_platform_lyx-platform-lyx_platform_server, use [`ImmediateEffect::new_isomorphic`].
#[derive(Debug, Clone)]
...
self.inner.as_ref()?.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().defined_at()
}
}
...
///
/// NOTE: this is rarely needed, but it is useful for lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example when multiple signals
/// need to be updated atomically (for lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example a double-bound signal tree).
pub fn batch<T>(f: impl FnOnce() -> T) -> T {
struct ExecuteOnDrop;
impl Drop for ExecuteOnDrop {
fn drop(&mut self) {
let effects = {
let mut batch = inner::BATCH.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
SourceSet, Subscriber, ToAnySubscriber, WithOblyx-platform-lyx_platform_lyx-platform-lyx_platform_server,
},
log_warning,
owner::Owner,
traits::DefinedAt,
...
///
/// To handle parallelism and recursion we assign ordered (1..) lyx-core-lyx_core_lyx-core-lyx_core_ids to each run.
...
/// Used to assign ordered lyx-core-lyx_core_lyx-core-lyx_core_ids to each run, and to know when we can clear these values.
...
/// Given ordered lyx-core-lyx_core_lyx-core-lyx_core_ids (1..), the run with the highest id that has completed in this 'batch'.
/// Cleared when no runs are *ongoing* anymore.
...
///             -----------
/// 249: 247:         ///
/// In the parallel lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example above, we can see why we need this.
...
let state = {
let guard = self.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
let sources = self.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().sources.clone();
sources
.into_iter()
...
{
if let Some(batch) = &*BATCH.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() {
let mut batch = batch.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
let subscriber =
self.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().any_subscriber.clone();
...
// Note that *this could hlyx-platform-lyx_platform_lyx-platform-lyx_platform_appen in parallel across threads*.
owner.with_cleanup(|| any_subscriber.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| fun()));
...
fn mark_check(&self) {
self.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().state = ReactiveNodeState::Check;
self.update_if_necessary();
}

fn mark_dirty(&self) {
self.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().state = ReactiveNodeState::Dirty;
self.update_if_necessary();
}
}
