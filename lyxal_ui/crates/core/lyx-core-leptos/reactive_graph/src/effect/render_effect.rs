...
/// 1. Its first run takes place immediately and synchronously: for lyx-ui-foundations-lyx_ui_foundations_lyx-ui-foundations-lyx_ui_foundations_example, if it is being used to
///    drive a user interface, it will run during rendering, not on the next tick after rendering.
///    (Hence â€œrender effect.â€)
...
let wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped = send_wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apper::SendWrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apper::new(move || {
fun.upgrade()
.map(|n| n.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().ptr_address())
});
// it's not redundant, it's due to the SendWrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apper deref
#[allow(clippy::redundant_closure)]
Box::new(move || wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped())
},
move |prev| fun.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().call((prev,)),
)
};
...
{
let (oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server, rx) = channel();
let owner = Owner::new();
...
HOT_RELOAD_SUBSCRIBERS.lock().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().retain(
...
*value.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() = Some(
owner.with(|| subscriber.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| fun(initial_value))),
);

lyx-core-lyx_core_lyx-core-lyx_core_any_spawner::Executor::spawn_local({
...
subscriber.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| fun(old_value))
});
...
.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| ScopedFuture::new(initial_value))
.await;
*value.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() = Some(initial);
...
self.value.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().as_mut().map(fun)
}

/// Takes the current value, replacing it with `None`.
pub fn take_value(&self) -> Option<T> {
self.value.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().take()
}
}
...
let (oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server, mut rx) = channel();
let value = Arc::new(RwLock::new(None::<T>));
let owner = Owner::new();
let inner = Arc::new(RwLock::new(EffectInner {
dirty: false,
...
.with(|| inner.to_any_subscriber().with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| fun(None)));
*value.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() = Some(initial_value);
