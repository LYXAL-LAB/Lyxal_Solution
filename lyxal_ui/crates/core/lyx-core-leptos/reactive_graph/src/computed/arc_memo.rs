...
signal::{
guards::{Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped, Plain, ReadGuard},
...
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::computed::*;
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::signal::signal;
/// # fn really_expensive_computation(value: i32) -> i32 { value };
/// let (value, set_value) = signal(0);
///
/// // ðŸ†— we could create a derived signal with a simple function
...
/// // ðŸ†— run #1: calls `really_expensive_computation` the first time
/// println!("expensive = {}", expensive());
/// // âŒ run #2: this calls `really_expensive_computation` a second time!
/// let some_value = expensive();
///
/// // instead, we create a memo
/// // ðŸ†— run #1: the calculation runs once immediately
/// let memoized = ArcMemo::new(move |_| really_expensive_computation(value.get()));
/// // ðŸ†— reads the current value of the memo
/// //    can be `memoized()` on nightly
/// println!("memoized = {}", memoized.get());
/// // âœ… reads the current value **without re-running the calculation**
...
/// - [`.with()`](crate::traits::With) allows you to reactively access the memoâ€™s
...
///     the memoâ€™s value by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a callback function without reactively
///     tracking it.
...
S: Storage<T>,
{
type Value = ReadGuard<T, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped<Plain<Option<S::Wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped>>, T>>;

fn try_read_untracked(&self) -> Option<Self::Value> {
self.update_if_necessary();

Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped::try_new(Arc::clone(&self.inner.value), |t| {
