...
///
/// **You probably donâ€™t need this,** but it can be a very useful optimization
/// in certain situations (e.g., â€œset the class `selected` if `selected() == this_row_index`)
...
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::StoredValue; let owner = lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::Owner::new(); owner.set();
...
*v.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() = Some(next_value.clone());
if prev.as_ref() != Some(&next_value) {
for (key, signal) in &*subs.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned() {
if f(key, &next_value)
|| (prev.is_some()
...
let read = {
let sub = self.subs.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().get(key).cloned();
sub.unwrap_or_else(|| {
self.subs
.write()
...
read.track();
(self.f)(key, self.v.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().as_ref().unwrap())
}

/// Removes the listener for the given key.
pub fn remove(&self, key: &T) {
let mut subs = self.subs.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
subs.remove(key);
}
