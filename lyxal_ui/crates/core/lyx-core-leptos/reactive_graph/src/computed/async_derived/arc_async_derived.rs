...
SubscriberSet, ToAnySource, ToAnySubscriber, WithOblyx-platform-lyx_platform_lyx-platform-lyx_platform_server,
},
owner::{use_context, Owner},
send_wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apper_ext::SendOption,
signal::{
guards::{AsyncPlain, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_appedMut, ReadGuard, WriteGuard},
ArcTrigger,
},
traits::{
...
/// # let _guard = lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::diagnostics::SpecialNonReactiveZone::enter();
///
...
///   value without cloning by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a callback function.
///   - [`.with_untracked()`](crate::traits::WithUntracked) allows you to access
///     the value by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a callback function without reactively
///     tracking it.
...
owner.with_cleanup(|| {
any_subscriber
.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| ScopedFuture::new($fun()))
})
} else {
owner.with_cleanup(|| {
any_subscriber
.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server_untracked(|| ScopedFuture::new($fun()))
})
};
#[cfg(feature = "sandboxed-arenas")]
...
match initial {
None => {
inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().notifier.notify();
(false, Some(initial_fut))
}
Some(orig_value) => {
let mut guard = this.inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
any_subscriber.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| source.track());
}
...
any_subscriber
.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server(|| any_subscriber.update_if_necessary())
} else {
any_subscriber
.with_oblyx-platform-lyx_platform_lyx-platform-lyx_platform_server_untracked(|| any_subscriber.update_if_necessary())
};
...
// generate new Future
let owner = inner.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().owner.clone();
...
let this_version = {
let mut guard = inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
guard.version += 1;
let version = guard.version;
let suspense_lyx-core-lyx_core_lyx-core-lyx_core_ids = mem::take(&mut guard.suspenses)
.into_iter()
.map(|sc| sc.task_id())
.collect::<Vec<_>>();
guard.pending_suspenses.extend(suspense_lyx-core-lyx_core_lyx-core-lyx_core_ids);
version
};
...
&mut inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().state,
...
for sub in (&inner.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().subscribers).into_iter() {
sub.mark_dirty();
}

// notify async .awaiters
for waker in mem::take(&mut *wakers.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned()) {
waker.wake();
}
...
inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().state = prev_state;
}
}
...
type Value =
ReadGuard<Option<T>, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped<AsyncPlain<SendOption<T>>, Option<T>>>;
...
self.inner
.write()
.lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned()
.suspenses
.push(suspense_context);
}
AsyncPlain::try_new(&self.value).map(|plain| {
ReadGuard::new(Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped::new_with_guard(plain, |v| v.deref()))
})
}
}
...
// new value
let mut guard = self.inner.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
guard.version += 1;

// tell any suspenses to stop waiting for this
drop(mem::take(&mut guard.pending_suspenses));

Some(Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_appedMut::new(
