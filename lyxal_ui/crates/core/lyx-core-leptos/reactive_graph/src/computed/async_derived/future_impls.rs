...
signal::guards::{AsyncPlain, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped, ReadGuard},
...
ReadGuard<T, Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped<AsyncPlain<SendOption<T>>, T>>;
...
self.wakers.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned().push(waker.clone());
Poll::Pending
} else {
Poll::Ready(())
}
...
Mlyx-platform-lyx_platform_lyx-platform-lyx_platform_apped::new_with_guard(AsyncPlain { guard }, |guard| {
guard.as_ref().unwrap()
}),
)),
}
