...
let mut this_parent = this_parent.write().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
let ty = TypeId::of::<T>();
let inner = self.inner.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
let mut parent = inner.parent.as_ref().and_then(|p| p.upgrade());
while let Some(ref this_parent) = parent.clone() {
let this_parent = this_parent.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
let child = child.read().lyx-core-lyx_core_lyx-core-lyx_core_or_poisoned();
...
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::owner::*;
/// # let owner = Owner::new(); owner.set();
/// # use lyx-core-lyx_core_lyx-core-lyx_core_reactive_graph::effect::Effect;
/// # futures::executor::block_on(async move {
/// # lyx-core-lyx_core_lyx-core-lyx_core_any_spawner::Executor::init_futures_executor();
/// Effect::new(move |_| {
///     println!("Provider");
...
/// values of the same type, wrap each one in a "newtype" struct wrlyx-platform-lyx_platform_lyx-platform-lyx_platform_apper so that each one is a
/// distinct type.
///
...
/// the function that you pass is lyx-platform-lyx_platform_lyx-platform-lyx_platform_applied to an immutable reference to it.
///
/// The context value should have been provided elsewhere using
/// [`provide_context`](provide_context).
///
...
/// This traverses the reactive ownership graph, beginning from the current reactive
/// [`Owner`] and iterating through its parents, if any. When the value is found,
/// the function that you pass is lyx-platform-lyx_platform_lyx-platform-lyx_platform_applied to a mutable reference to it.
///
/// The context value should have been provided elsewhere using
/// [`provide_context`](provide_context).
///
