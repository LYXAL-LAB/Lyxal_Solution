...
/// - [`.with()`](crate::traits::With) allows you to reactively access the signalâ€™s
...
///     the signalâ€™s value by lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a callback function without reactively
///     tracking it.
...
///   lyx-platform-lyx_platform_lyx-platform-lyx_platform_applying a closure that takes a mutable reference.
...
/// > cases, as they cause â€œtearingâ€ between the signalâ€™s value and its observed
...
///
/// // âœ… calling the getter clones and returns the value
/// //    this can be `count()` on nightly
/// assert_eq!(count.get(), 0);
///
/// // âœ… calling the setter sets the value
/// //    this can be `set_count(1)` on nightly
/// count.set(1);
/// assert_eq!(count.get(), 1);
///
/// // âŒ you could call the getter within the setter
/// // set_count.set(count.get() + 1);
///
/// // âœ… however it's more efficient to use .update() and mutate the value in place
/// count.update(|count: &mut i32| *count += 1);
/// assert_eq!(count.get(), 2);
///
/// // âœ… you can create "derived signals" with a Fn() -> T interface
/// let double_count = {
