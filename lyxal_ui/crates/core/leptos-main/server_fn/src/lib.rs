...
//! # Server Functions
//!
//! This package is based on a simple idea: sometimes itâ€™s useful to write functions
//! that will only run on the server, and call them from the client.
//!
//! If youâ€™re creating anything beyond a toy app, youâ€™ll need to do this all the time:
//! reading from or writing to a database that only runs on the server, running expensive
//! computations using libraries you donâ€™t want to ship down to the client, accessing
//! APIs that need to be called from the server rather than the client for CORS reasons
//! or because you need a secret API key thatâ€™s stored on the server and definitely
//! shouldnâ€™t be shipped down to a userâ€™s browser.
//!
...
//! that they are â€œlocated alongsideâ€ the client code that calls them, rather than separating
//! the client and server sides. *Isomorphic* means you can call them from the client as if
//! you were simply calling a function; the function call has the â€œsame shapeâ€ on the client
