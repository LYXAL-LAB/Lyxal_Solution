...
::std::write!(output, "/function/{function_name}/publish", function_name = function_name).expect("formatting should succeed");
...
let body = ::aws_smithy_types::body::SdkBody::from(crate::protocol_serde::shape_publish::ser_publish_input(&input)?);
...
/// Returns `true` if the error kind is `PublishError::ResourceNotFound`.
pub fn is_resource_not_found(&self) -> bool {
matches!(self, Self::ResourceNotFound(_))
}
/// Returns `true` if the error kind is `PublishError::InternalServerError`.
