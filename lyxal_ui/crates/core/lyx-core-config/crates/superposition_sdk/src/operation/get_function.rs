...
::std::write!(output, "/function/{function_name}", function_name = function_name).expect("formatting should succeed");
...
let builder = crate::protocol_serde::shape_get_function::ser_get_function_headers(input, builder)?;
...
/// Returns `true` if the error kind is `GetFunctionError::ResourceNotFound`.
pub fn is_resource_not_found(&self) -> bool {
matches!(self, Self::ResourceNotFound(_))
}
/// Returns `true` if the error kind is `GetFunctionError::InternalServerError`.
