...
fn main() -> anyhow::Result<()> {
    ...
    lyxal_proxy_lib::metrics::setup(
        &SocketAddress::new_v4(127, 0, 0, 1, 8125).into(),
        "main",
        false,
        None,
    );
    gauge!("lyxal_proxy.TEST", 42);
    ...
    let jg = thread::spawn(move || {
        ...
        lyxal_proxy_lib::http::testing::start_http_worker(
            http_listener,
            channel,
            max_buffers,
            buffer_size,
    ...
    let jg2 = thread::spawn(move || {
        let max_buffers = 500;
        let buffer_size = 16384;
        lyxal_proxy_lib::https::testing::start_https_worker(
            https_listener,
            channel2,
            max_buffers,
            buffer_size,
