...
use rusty_ulid::Ulid;
use lyxal_proxy_command_lib::logging::LogContext;
...
pub struct HttpContext {
    ...
    #[cfg(feature = "opentelemetry")]
    pub otel: Option<lyxal_proxy_command::logging::OpenTelemetry>,

    // ========== Read only
    /// signals wether Kawa should write a "Connection" header with a "close" value (request and response)
    pub closing: bool,
    /// the value of the custom header, named "Lyxal_proxy-Id", that Kawa should write (request and response)
    pub id: Ulid,
...
impl HttpContext {
    ...
    ///
    /// - edit headers (connection, forwarded, sticky cookie, lyxal_proxy-id)
    /// - save information:
    ///   - method
    ///   - authority
    ///   - path
    ...
    fn on_request_headers(&mut self, request: &mut GenericHttpStream) {
        ...
        let (otel, has_traceparent) = {
            let mut otel = lyxal_proxy_command_lib::logging::OpenTelemetry::default();
            let tp = traceparent
                .as_ref()
        ...
        // Create a custom "Lyxal_proxy-Id" header
        request.push_block(kawa::Block::Header(kawa::Pair {
            key: kawa::Store::Static(b"Lyxal_proxy-Id"),
    ...
    ///
    /// - edit headers (connection, set-cookie, lyxal_proxy-id)
    /// - save information:
    ///   - status code
    ///   - reason
    ///   - back keep-alive
