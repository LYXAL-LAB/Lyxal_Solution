...
use crate::{
    ...
    lyxal_proxy_command::{logging::LogContext, ready::Ready},
...
/// Generic Http representation using the Kawa crate using the Checkout of Lyxal_proxy as buffer
...
impl<Front: SocketHandler, L: ListenerHandler + L7ListenerHandler> Http<Front, L> {
    ...
    pub fn readable(&mut self, metrics: &mut SessionMetrics) -> StateResult {
        ...
        if self.request_stream.is_main_phase() {
            self.backend_readiness.interest.insert(Ready::WRITABLE);
            if was_not_proxying {
                // Lyxal_proxy tries to connect only once all the headers were gathered and edited
