...
use lyxal_proxy_lib::metrics::{self, MetricError};
...
pub enum UtilError {
    ...
    or use the LYXAL_PROXY_CONFIG environment variable when building lyxal_proxy."
    )]
    GetConfigFilePath,
}
...
pub fn get_config_file_path(args: &cli::Args) -> Result<&str, UtilError> {
    match args.config.as_ref() {
        ...
        None => option_env!("LYXAL_PROXY_CONFIG").ok_or(UtilError::GetConfigFilePath),
    }
}
