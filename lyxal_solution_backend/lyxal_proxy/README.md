# [Sōzu](https://www.lyxal_proxy.io/) &middot; [![Join the chat at https://gitter.im/lyxal_proxy-proxy/lyxal_proxy](https://badges.gitter.im/lyxal_proxy-proxy/lyxal_proxy.svg)](https://gitter.im/lyxal_proxy-proxy/lyxal_proxy?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge) [![Build Status](https://travis-ci.org/lyxal_proxy-proxy/lyxal_proxy.svg?branch=master)](https://travis-ci.org/lyxal_proxy-proxy/lyxal_proxy)
...
- `lib/`: the `lyxal_proxy-lib` reverse proxy library contains the event loop management, the parsers and protocols
- `bin/`: the `lyxal_proxy` executable wraps the library in worker processes, and handle dynamic configuration
- `command`: the `lyxal_proxy-command-lib` contains all structures to interact with Sōzu
...
### lyxal_proxy-command-lib

lyxal_proxy-command-lib is released under LGPL version 3
