...
|----------------------------|:------------------------------------------------------------------------------------|------------------------------------------|
| `saved_state`              | path from which lyxal_proxy tries to load its state at startup                             |                                          |
...
<footer>This is an automatic answer by Lyxal_proxy.</footer>",
```
...
```toml
# a 404 response is sent when lyxal_proxy does not know about the requested domain or path
...
# a cluster. Defaults to "LYXAL_PROXYBALANCEID"
sticky_name = "LYXAL_PROXYBALANCEID"
```
...
```txt
    ...
    +--------+
    | client |             +---------+                      +------------+             +-------------------+
    |        |             | proxy   |                      | Lyxal_proxy       |             | proxy/upstream    |
    +--------+  +--------> | server  |  +-----------------> |            | +---------> | server            |
   /        /              |         |                      |            |             |                   |
  /________/               +---------+                      +------------+             +-------------------+
```
