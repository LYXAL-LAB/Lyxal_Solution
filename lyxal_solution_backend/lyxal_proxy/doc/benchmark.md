...
```bash
mv target/release/lyxal_proxy $HOME/.cargo/bin/lyxal_proxy-0.15.14
```
...
```bash
cd bin
lyxal_proxy-0.15.14 --config my-config.toml start
```
...
```bash
ps -aux | grep lyxal_proxy
user   13368  0.7  0.1 188132 40344 pts/2    Sl+  11:35   0:00 /path/to/lyxal_proxy --config my-config.toml start
user   14157  0.0  0.0  79908 29628 pts/2    S+   11:35   0:00 /path/to/lyxal_proxy worker --id 0 --fd 5 --scm 7 --configuration-state-fd 3 --command-buffer-size 16384 --max-command-buffer-size 16384
user   14205  0.0  0.0   6556  2412 pts/3    S+   11:35   0:00 grep --color=auto lyxal_proxy
```
...
connect(12, {sa_family=AF_INET, sin_port=htons(1052), sin_addr=inet_addr("127.0.0.1")}, 16) = -1 EINPROGRESS (Operation now in progress)
 ...
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(mio::sys::unix::tcp::connect+0x84) [0x17b2ec4]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(mio::net::tcp::stream::TcpStream::connect+0x127) [0x17ad9e7]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::backends::Backend::try_connect+0x88) [0xb03898]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::backends::BackendMap::backend_from_cluster_id+0x465) [0xb04145]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::protocol::kawa_h1::Http<Front,L>::get_backend_for_sticky_session+0x5ab) [0xca8beb]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::protocol::kawa_h1::Http<Front,L>::backend_from_request+0x159) [0xca69b9]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::protocol::kawa_h1::Http<Front,L>::connect_to_backend+0xb7a) [0xcaafba]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::protocol::kawa_h1::Http<Front,L>::ready_inner+0x423) [0xcaf3a3]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(<lyxal_proxy_lib::protocol::kawa_h1::Http<Front,L> as lyxal_proxy_lib::protocol::SessionState>::ready+0x2d) [0xcb040d]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(<lyxal_proxy_lib::http::HttpStateMachine as lyxal_proxy_lib::protocol::SessionState>::ready+0xf2) [0xdbb9b2]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(<lyxal_proxy_lib::http::HttpSession as lyxal_proxy_lib::ProxySession>::ready+0x114) [0xdb4b54]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::server::Server::ready+0x746) [0xc95846]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy_lib::server::Server::run+0x66d) [0xc8869d]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy::worker::begin_worker_process+0x3060) [0x32e480]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(lyxal_proxy::main+0x42e) [0x556afe]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(core::ops::function::FnOnce::call_once+0xb) [0x1e453b]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(std::sys_common::backtrace::__rust_begin_short_backtrace+0xe) [0x287e6e]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(std::rt::lang_start::{{closure}}+0x11) [0x2ff2e1]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(std::rt::lang_start_internal+0x42e) [0x18217fe]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(std::rt::lang_start+0x3a) [0x2ff2ba]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(main+0x1e) [0x556f6e]
 > /usr/lib/libc.so.6(__libc_init_first+0x90) [0x27cd0]
 > /usr/lib/libc.so.6(__libc_start_main+0x8a) [0x27d8a]
 > /path/to/lyxal_proxy/target/debug/lyxal_proxy(_start+0x25) [0x17bca5]
```