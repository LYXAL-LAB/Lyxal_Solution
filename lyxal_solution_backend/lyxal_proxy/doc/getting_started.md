...
`lyxal_proxy` is published on [crates.io][cr].

To install them, you only have to do `cargo install lyxal_proxy`.

They will be built and available in the `~/.cargo/bin` folder.

### Build from source

Build the lyxal_proxy executable and command line:

`cd bin && cargo build --release --locked`

> The `--release` parameter inform cargo to compile lyxal_proxy with optimizations turned on.
