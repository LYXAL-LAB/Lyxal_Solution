# Spécification Technique : Mapping des Fonctions proxy::* (SurrealQL -> Sōzu)

Ce document définit l'interface exacte (API Surface) que le binaire Lyxal doit exposer via SurrealQL pour piloter le moteur Sōzu. Chaque fonction doit être implémentée dans `lyxal_db/core/src/fnc/proxy.rs`.

## 1. Module proxy::ingress (Gestion des points d'entrée)

| Fonction SurrealQL | Arguments | Action Rust (Sōzu lib) | Retour |
| :--- | :--- | :--- | :--- |
...
1. **Type Checking** : Chaque fonction doit valider ses types via le système `Value` de SurrealDB avant d'envoyer la commande à Sōzu.
2. **Atomicité** : Si une commande `proxy::*` échoue dans Sōzu, la transaction SurrealDB doit pouvoir être annulée (ou l'erreur doit être propagée clairement au MCP).
