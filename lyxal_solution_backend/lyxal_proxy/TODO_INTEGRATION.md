# TODO: Intégration Native Sōzu (lyxal_proxy) dans SurrealDB
...
## 2. Implémentation du module de fonctions SurrealQL (`fnc/proxy.rs`)
- [ ] Créer le fichier `lyxal_db/core/src/fnc/proxy.rs` sur le modèle de `http.rs`.
- [ ] Définir la syntaxe conforme SurrealDB pour les fonctions :
...
- [ ] Implémenter le "Casting" des types SurrealDB (`Value`) vers les types Sōzu (`WorkerRequest`).

## 3. Enregistrement dans le moteur SurrealDB
...
- [ ] Valider via Surrealist que l'ajout d'une route en SQL modifie bien le trafic en temps réel.
