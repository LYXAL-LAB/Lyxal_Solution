...
mod database_upgrade {
	...
	use lyxal::engine::any::{Any, connect};
	use lyxal::opt::auth::Root;
	use lyxal::{Connection, Lyxal, Value};
	use test_log::test;
	...
	const DEAL_STORE_DATASET: &str = "tests/data/lyxal-deal-store-mini.lyxql";
	...
	const DATA_IDX: [&str; 4] = [
		...
		"CREATE people:tobie SET name = 'Tobie', org='Lyxal'",
		"CREATE people:jaime SET name = 'Jaime', org='Lyxal'",
	];
	...
	const CHECK_IDX: [Check; 2] = [
		...
		(
			"SELECT name FROM people WITH INDEX idx_org WHERE org = 'Lyxal'",
	...
	async fn start_docker(docker_version: &str) -> (String, DockerContainer, Lyxal<Any>) {
		use lyxal::opt::WaitFor::Connection;
	...
	async fn create_data_on_docker(client: &Lyxal<Any>, info: &str, data: &[&str]) {
		info!("Create {info} data on Docker's instance");
		for l in data {
			info!("Run `{l}`");
			let data = lyxal::opt::Raw::from(l.to_string());
			client.query(data).await.expect(l).check().expect(l);
		}
	}

	async fn import_data_on_docker(client: &Lyxal<Any>, info: &str, path: &str) {
		info!("Import {info} data on Docker's instance: {path}");
		client.import(path).await.expect(info);
	}

	async fn check_data_on_docker(client: &Lyxal<Any>, info: &str, queries: &[Check]) {
		info!("Check {info} data on Docker's instance");
		for (query, expected) in queries {
			info!("Run `{query}`");
			match expected {
				Some(expected) => {
					let data = lyxal::opt::Raw::from(query.to_string());
					let response: Value =
	...
	async fn get_info_ns_db(client: &Lyxal<Any>, info: &str) -> (Value, Value) {
	...
	async fn get_info_tables(
		...
		for n in table_names {
			let data = lyxal::opt::Raw::from(format!("INFO FOR TABLE `{n}`"));
	...
	async fn get_table_rows(
		...
		for n in table_names {
			...
			info!("{q}");
			let rows: Value =
				client.query(lyxal::opt::Raw::from(q)).await.unwrap().take(0).unwrap();
	...
	async fn check_migrated_data(db: &Lyxal<Any>, info: &str, queries: &[Check]) {
	...
	async fn checked_query<C>(db: &Lyxal<C>, q: &str, expected: Option<&str>)
	where
		C: Connection,
	{
		info!("Run `{q}`");
		let data = lyxal::opt::Raw::from(q.to_string());
	...
	async fn new_local_instance(file_path: &String) -> Lyxal<Any> {
