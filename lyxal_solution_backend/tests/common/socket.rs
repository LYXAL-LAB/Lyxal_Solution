	...
	fn to_msg(format: Format, message: &serde_json::Value) -> Result<Message> {
		match format {
			...
			Format::Cbor => {
				...
				// to a LyxalQL value, so that record ids, uuids,
				// datetimes, and durations are stored properly.
				// First of all we convert the JSON type to a string.
				let json = message.to_string();
				// Then we parse the JSON in to LyxalQL.
				let lyxalql = lyxal_core::syn::value_legacy_strand(&json)?;
				// Then we convert the LyxalQL in to CBOR.
				let cbor = lyxal_core::rpc::format::cbor::encode(lyxalql)?;
			...
			Message::Binary(msg) => {
				...
				match format {
					Format::Cbor => {
						...
						// Then we convert it to a LyxalQL Value.
						let msg = lyxal_core::rpc::format::cbor::decode(msg.as_ref())?;
						// Then we convert the LyxalQL to JSON.
