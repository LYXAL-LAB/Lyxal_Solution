...
mod http_integration {
	...
	use reqwest::Client;
	use serde_json::json;
	use lyxal::headers::{AUTH_DB, AUTH_NS};
	use test_log::test;
	use ulid::Ulid;
	...
	async fn basic_auth() -> Result<(), Box<dyn std::error::Error>> {
		...
		headers.insert("lyxal-ns", ns.parse()?);
		headers.insert("lyxal-db", db.parse()?);
	...
	async fn session_id() {
		...
		// Request without header, gives a randomly generated session identifier
		{
			...
			let ns = Ulid::new().to_string();
			let db = Ulid::new().to_string();
			headers.insert("lyxal-ns", ns.parse().unwrap());
			headers.insert("lyxal-db", db.parse().unwrap());
	...
	async fn no_server_id_headers() -> Result<(), Box<dyn std::error::Error>> {
		// default server has the id headers
		{
			...
			assert!(res.headers().contains_key("lyxal-version"));
		}
	...
	async fn rpc_endpoint() -> Result<(), Box<dyn std::error::Error>> {
		...
		// Prepare HTTP client
		let mut headers = reqwest::header::HeaderMap::new();
		headers.insert("lyxal-ns", Ulid::new().to_string().parse()?);
		headers.insert("lyxal-db", Ulid::new().to_string().parse()?);
		headers.insert(header::ACCEPT, "application/json".parse()?);
		let client = reqwest::Client::builder()
			.connect_timeout(Duration::from_millis(10))
			.default_headers(headers)
	...
	async fn sql_endpoint() -> Result<(), Box<dyn std::error::Error>> {
		...
		// Creating a record with Accept Lyxal encoding is allowed
		{
			let res = client
				.post(url)
				.basic_auth(USER, Some(PASS))
				.header(header::ACCEPT, lyxal_core::api::format::FLATBUFFERS)
				.body("CREATE foo")
				.send()
				.await?;
			assert_eq!(res.status(), 200);
	...
	async fn version_endpoint() -> Result<(), Box<dyn std::error::Error>> {
		...
		assert_eq!(res.status(), 200, "response: {res:#?}");
		let body = res.text().await?;
		assert!(body.starts_with("lyxal-"), "body: {body}");
	...
	async fn ensure_namespace_and_database(
		...
		ns_headers.insert("lyxal-ns", ns.parse()?);
	...
	async fn signup_mal() -> Result<(), Box<dyn std::error::Error>> {
		...
		headers.insert("lyxal-ns", ns.parse()?);
		headers.insert("lyxal-db", db.parse()?);
		headers.insert(header::ACCEPT, lyxal_core::api::format::FLATBUFFERS.parse()?);
		headers.insert(header::CONTENT_TYPE, lyxal_core::api::format::FLATBUFFERS.parse()?);
		let client = reqwest::Client::builder()
			.connect_timeout(Duration::from_millis(10))
			.default_headers(headers)
			.build()?;
