...
mod cli_integration {
	...
	async fn all_commands() {
		...
		info!("* Export to file");
		let exported = {
			let exported = common::tmp_file("exported.lyxql");
			let args =
	...
	async fn with_anon_auth() {
		...
		{
			let tmp_file = common::tmp_file("exported.lyxql");
	...
	async fn with_import_file() {
		...
		info!("* Import and reimport root-level file");
		{
			...
			let query = r#"DEFINE ACCESS OVERWRITE admin ON ROOT TYPE JWT URL "https://www.lyxal.com/jwks.json";"#;
			let import_file = common::tmp_file("import.lyxql");
			...
			assert!(output.contains(
				r#"DEFINE ACCESS admin ON ROOT TYPE JWT URL 'https://www.lyxal.com/jwks.json'"#
			));
		...
		{
			// Start the server with an import file which contains invalid LyxalQL.
			let query = r#"
			...
			match res {
				Ok(_) => panic!("import file should contain parseable LyxalQL"),
				Err(e) => {
	...
	#[test]
	fn validate_succeed_for_valid_lyxql_files() {
		let temp_dir = assert_fs::TempDir::new().unwrap();

		let statement_file = temp_dir.child("statement.lyxql");
	...
	#[test]
	fn validate_failed_due_to_invalid_lyxql_files_syntax() {
		let temp_dir = assert_fs::TempDir::new().unwrap();
	...
	async fn test_capabilities() {
		...
		info!("* When all capabilities are allowed");
		{
			...
			let query = format!("RETURN http::get('http://{addr}/version');\n\n");
			let output = common::run(&cmd).input(&query).output().unwrap();
			assert!(output.contains("['lyxal-"), "unexpected output: {output:?}");
		...
		info!("* When capabilities are allowed globally, but functions are denied generally");
		{
			...
			let query = "RETURN string::lowercase('LYXAL');\n\n".to_string();
			let output = common::run(&cmd).input(&query).output().unwrap();
			assert!(
	...
	async fn test_temporary_directory() {
		...
		{
			let path = format!("lyxalkv:{}", tempfile::tempdir().unwrap().path().display());
