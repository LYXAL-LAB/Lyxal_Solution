...
#[async_trait]
impl Connector for WechatConnector {
    fn id(&self) -> &str { "wechat" }
    fn connector_type(&self) -> ConnectorType { ConnectorType::Social }
    fn metadata(&self) -> ConnectorMetadata {
        ConnectorMetadata {
            id: "wechat".to_string(),
            target: "wechat".to_string(),
            connector_type: ConnectorType::Social,
            name: json!({"en": "WECHAT"}),
            description: json!({"en": "wechat login"}),
            logo: "/logos/wechat.svg".to_string(),
            logo_dark: None,
            readme: "Login with wechat".to_string(),
...
#[async_trait]
impl SocialConnector for WechatConnector {
    async fn get_authorization_url(&self, state: &str, redirect_uri: &str) -> Result<String> {
        Ok(format!("https://auth.wechat.com/authorize?state={}&redirect={}", state, redirect_uri))
    }
