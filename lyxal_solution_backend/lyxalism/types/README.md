...
A language-agnostic serialization framework for WebAssembly (WASM) guest-host communication in Lyxal's Lyxalism ecosystem.

## Overview

`lyxalism-types` provides a custom binary serialization protocol designed to enable Lyxal client modules to be built in any language that compiles to WebAssembly. By defining a well-specified wire format and memory transfer protocol, this crate allows different programming languages to communicate seamlessly across WASM boundaries.

## Why Custom Serialization?

Instead of using standard serialization frameworks like Serde with JSON or bincode, `lyxalism-types` implements a custom protocol for several key reasons:
...
#### `lyxal_types::Value` and `Kind`
These types use **FlatBuffers** serialization via the `lyxal-protocol` crate:
```
[FlatBuffers-encoded data]
```

This leverages the existing Lyxal binary protocol for complex database types like records, geometries, and durations.
...
```toml
[dependencies]
lyxalism-types = { version = "*", features = ["host"] }
```
...
```rust
use lyxalism_types::{Serializable, Transfer, MemoryController};
...
```rust
use lyxalism_types::{Serializable, AsyncTransfer, AsyncMemoryController};
...
```rust
use lyxalism_types::{Args, args::Args};
...
- **lyxalism-runtime**: Host-side WASM runtime implementation
- **lyxalism**: Main API surface for building WASM modules
- **lyxalism-macros**: Procedural macros for deriving traits
- **lyxal-protocol**: FlatBuffers schema for Lyxal types
- **lyxal-types**: Core Lyxal type system

## License

See the main Lyxal repository for license information.
