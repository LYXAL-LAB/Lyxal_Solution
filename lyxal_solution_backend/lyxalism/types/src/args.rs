//! Function argument marshalling for Lyxal values.
//!
//! This module provides the [`Args`](crate::args::Args) trait for converting between typed tuples
//! and vectors of [`lyxal_types::Value`]. This enables type-safe function signatures
...
//! # Supported Patterns
//!
//! - Tuples from 1 to 10 elements (each implementing [`LyxalValue`])
//! - `Vec<T>` for variadic arguments
//! - `()` for zero arguments
//!
//! [`LyxalValue`]: lyxal_types::LyxalValue

use lyxal_types::{Error, LyxalValue};

/// Trait for marshalling function arguments to and from [`lyxal_types::Value`] vectors.
///
/// This trait enables type-safe function signatures while maintaining a language-agnostic
/// representation. It's implemented for tuples of types that implement [`LyxalValue`],
...
pub trait Args: Sized {
	/// Convert this argument tuple into a vector of [`lyxal_types::Value`].
	///
	/// This method consumes the arguments and produces a vector where each element
	/// corresponds to one argument converted via [`LyxalValue::into_value`].
	///
	/// # Example
	///
	/// ```rust,ignore
	...
	/// let values = args.to_values();
	/// // values = [ Value::String("hello"), Value::Number(42)]
	/// ```
	fn to_values(self) -> Vec<lyxal_types::Value>;

	/// Reconstruct typed arguments from a vector of [`lyxal_types::Value`].
	///
	...
	/// ```
	fn from_values(values: Vec<lyxal_types::Value>) -> std::result::Result<Self, Error>;

	/// Get the expected types for each argument position.
	///
	/// This returns a vector of [`lyxal_types::Kind`] describing the type of
