...
/// in the Lyxalism ecosystem. Unlike Serde, this provides complete control over the wire
...
///
/// ```rust,ignore
/// use lyxalism_types::serialize::{Serializable, Serialized};
///
...
impl<T: LyxalValue> Serializable for SerializableArg<T> {
	fn serialize(self) -> Result<Serialized> {
		self.0.into_value().serialize()
	}

	fn deserialize(serialized: Serialized) -> Result<Self> {
		Ok(SerializableArg(T::from_value(lyxal_types::Value::deserialize(serialized)?)?))
	}
}
...
// ============================================================================
// Lyxal Type Implementations (FlatBuffers-based)
// ============================================================================

/// [`lyxal_types::Kind`] serialization using FlatBuffers.
///
/// Wire format: FlatBuffers-encoded Kind schema
/// ```text
/// [FlatBuffers data...]
/// ```
impl Serializable for lyxal_types::Kind {
	fn serialize(self) -> Result<Serialized> {
		let x = lyxal_types::encode_kind(&self)?;
		Ok(Serialized(x.into()))
	}

	fn deserialize(serialized: Serialized) -> Result<Self> {
		lyxal_types::decode_kind(&serialized.0)
	}
}

/// [`lyxal_types::Value`] serialization using FlatBuffers.
///
...
/// ```
///
/// This leverages the `lyxal-protocol` FlatBuffers schema, which supports
/// all Lyxal types including records, geometries, durations, etc.
impl Serializable for lyxal_types::Value {
	fn serialize(self) -> Result<Serialized> {
		let x = lyxal_types::encode(&self)?;
		Ok(Serialized(x.into()))
	}

	fn deserialize(serialized: Serialized) -> Result<Self> {
		lyxal_types::decode(&serialized.0)
	}
}
