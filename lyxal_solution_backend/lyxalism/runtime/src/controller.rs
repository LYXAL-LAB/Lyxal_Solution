...
//! ```no_run
//! use std::sync::Arc;
//! use lyxalism_runtime::{controller::Runtime, package::LyxalismPackage};
//!
...
impl Controller {
	...
	pub async fn invoke<A: Args>(
		...
		let result: Result<lyxal_types::Value, String> =
			AsyncTransfer::receive(ptr_u32.into(), self).await?;
		result.map_err(|e| anyhow::anyhow!("WASM function returned error: {}", e))
	}

	pub async fn args(&mut self, name: Option<String>) -> Result<Vec<lyxal_types::Kind>> {
	...
	pub async fn returns(&mut self, name: Option<String>) -> Result<lyxal_types::Kind> {
...
#[async_trait]
impl lyxalism_types::controller::AsyncMemoryController for Controller {
