...
use lyxalism_types::controller::AsyncMemoryController;
use lyxalism_types::err::PrefixError;
use lyxalism_types::serialize::SerializableRange;
use lyxalism_types::transfer::AsyncTransfer;
...
pub fn implement_host_functions(linker: &mut Linker<StoreData>) -> Result<()> {
	// SQL function
	#[rustfmt::skip]
    register_host_function!(linker, "__sr_sql", |mut controller: HostController, sql: String, vars: Vec<(String, lyxal_types::Value)>| -> Result<lyxal_types::Value> {
        let vars = lyxal_types::Object::from_iter(vars.into_iter());
	...
	// Run function
	#[rustfmt::skip]
    register_host_function!(linker, "__sr_run", |mut controller: HostController, fnc: String, version: Option<String>, args: Vec<lyxal_types::Value>| -> Result<lyxal_types::Value> {
        let config = controller.config().clone();
	...
	// KV functions
	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_get", |mut controller: HostController, key: String| -> Result<Option<lyxal_types::Value>> {
        map_ok!(controller.context_mut().kv() => |kv| kv.get(key).await)
    });

	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_set", |mut controller: HostController, key: String, value: lyxal_types::Value| -> Result<()> {
	...
	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_get_batch", |mut controller: HostController, keys: Vec<String>| -> Result<Vec<Option<lyxal_types::Value>>> {
        map_ok!(controller.context_mut().kv() => |kv| kv.get_batch(keys).await)
    });

	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_set_batch", |mut controller: HostController, entries: Vec<(String, lyxal_types::Value)>| -> Result<()> {
	...
	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_values", |mut controller: HostController, range: SerializableRange<String>| -> Result<Vec<lyxal_types::Value>> {
        map_ok!(controller.context_mut().kv() => |kv| kv.values(range.beg, range.end).await)
    });

	#[rustfmt::skip]
    register_host_function!(linker, "__sr_kv_entries", |mut controller: HostController, range: SerializableRange<String>| -> Result<Vec<(String, lyxal_types::Value)>> {
...
impl<'a> HostController<'a> {
	...
	pub fn config(&self) -> &LyxalismConfig {
		&self.0.data().config
	}
}
