...
pub trait KVStore: Send + Sync {
	async fn get(&self, key: String) -> Result<Option<lyxal_types::Value>>;
	async fn set(&self, key: String, value: lyxal_types::Value) -> Result<()>;
	...
	async fn get_batch(&self, keys: Vec<String>) -> Result<Vec<Option<lyxal_types::Value>>>;
	async fn set_batch(&self, entries: Vec<(String, lyxal_types::Value)>) -> Result<()>;
	...
	) -> Result<Vec<lyxal_types::Value>>;
	async fn entries(
		&self,
		start: Bound<String>,
		end: Bound<String>,
	) -> Result<Vec<(String, lyxal_types::Value)>>;
...
pub struct BTreeMapStore {
	inner: RwLock<BTreeMap<String, lyxal_types::Value>>,
}
...
impl KVStore for BTreeMapStore {
	...
	async fn values(
		...
		let values: Vec<lyxal_types::Value> = map
			.iter()
	...
	async fn entries(
		...
		let entries: Vec<(String, lyxal_types::Value)> = map
			.iter()
			.filter(|(key, _)| self.in_range(key, &start, &end))
