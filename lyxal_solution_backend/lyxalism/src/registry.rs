...
use lyxalism_types::controller::MemoryController;
use lyxalism_types::transfer::{Ptr, Transfer};

/// Represents a wrapped function in the Lyxalism framework.
///
...
pub struct LyxalismFunction<A, R, F>
where
	A: 'static + Send + Sync + Args + Debug,
	R: 'static + Send + Sync + LyxalValue + Debug,
...
impl<A, R, F> LyxalismFunction<A, R, F>
where
	A: 'static + Send + Sync + Args + Debug,
	R: 'static + Send + Sync + LyxalValue + Debug,
	F: 'static + Send + Sync + Fn(A) -> Result<R, String>,
{
	/// Creates a new `LyxalismFunction` from the given callable.
	///
	/// # Parameters
	/// - `function`: The function or closure to wrap.
	///
	/// # Returns
	/// A new instance of `LyxalismFunction`.
	...
	pub fn args(&self) -> Vec<lyxal_types::Kind> {
		A::kinds()
	}
	...
	pub fn returns(&self) -> lyxal_types::Kind {
		R::kind_of()
	}
	...
	pub fn invoke_raw(&self, controller: &mut dyn MemoryController, args: Ptr) -> Result<Ptr> {
		let args = A::from_values(Vec::<lyxal_types::Value>::receive(args, controller)?)
