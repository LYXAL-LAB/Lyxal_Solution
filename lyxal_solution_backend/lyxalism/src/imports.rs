...
use lyxalism_types::transfer::Transfer;
...
/// Executes a Lyxal SQL query without variables.
///
...
where
	...
	R: LyxalValue,
{
	sql_with_vars(sql, lyxal_types::Variables::new())
}

/// Executes a Lyxal SQL query with optional variables.
///
...
where
	S: Into<String>,
	V: IntoIterator<Item = (String, lyxal_types::Value)>,
	R: LyxalValue,
{
	let sql = sql.into();
...
/// Runs a named function in the Lyxal runtime with optional version and arguments.
///
...
pub mod kv {
	...
	/// - If the FFI call or result reception encounters an issue.
	/// - If deserializing the result into `R` fails.
	pub fn get<K: Into<String>, R: LyxalValue>(key: K) -> Result<Option<R>> {
		let mut controller = Controller {};
	...
	/// - If transferring data fails.
	/// - If the FFI call or result reception encounters an issue.
	pub fn set<K: Into<String>, V: LyxalValue>(key: K, value: V) -> Result<()> {
		let mut controller = Controller {};
		let key = key.into().transfer(&mut controller)?;
	...
	pub fn values<R: RangeBounds<String>, T: LyxalValue>(range: R) -> Result<Vec<T>> {
		let mut controller = Controller {};
		let range = SerializableRange::from_range_bounds(range)?.transfer(&mut controller)?;
	...
	/// - If the FFI call or result reception encounters an issue.
	/// - If deserializing any result into `T` fails.
	pub fn entries<R: RangeBounds<String>, T: LyxalValue>(range: R) -> Result<Vec<(String, T)>> {
		let mut controller = Controller {};
		let range = SerializableRange::from_range_bounds(range)?.transfer(&mut controller)?;
