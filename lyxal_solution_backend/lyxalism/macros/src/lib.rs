...
pub fn lyxalism(attr: TokenStream, item: TokenStream) -> TokenStream {
	...
	for meta in args.iter() {
		match meta {
			Meta::NameValue(MetaNameValue {
				...
				}) = value
				{
					...
					if !val.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
						panic!(
							"#[lyxalism(name = \"...\")] must use only ASCII letters, digits, and underscores"
						);
					}
			...
			_ => panic!(
				"Unsupported attribute: expected #[lyxalism], #[lyxalism(default)], #[lyxalism(init)], or #[lyxalism(name = \"...\")]"
			),
		}
	}
	...
	for arg in &fn_sig.inputs {
		match arg {
			...
			FnArg::Receiver(_) => panic!("`self` is not supported in #[lyxalism] functions"),
		}
	}
	...
	let expanded = if is_init {
		...
		quote! {
			...
			pub extern "C" fn #export_ident(ptr: u32) -> i32 {
				use lyxalism::types::transfer::Transfer;
				let mut controller = lyxalism::Controller {};
				let f = lyxalism::LyxalismFunction::<#tuple_type, #result_type, _>::from(
