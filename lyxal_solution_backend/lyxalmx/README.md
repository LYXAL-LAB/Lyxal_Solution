...
<p align="center">
    <a href="https://lyxal.com#gh-dark-mode-only" target="_blank">
        <img width="200" src="/img/white/logo.svg" alt="lyxalmx Logo">
    </a>
    <a href="https://lyxal.com#gh-light-mode-only" target="_blank">
        <img width="200" src="/img/black/logo.svg" alt="lyxalmx Logo">
    </a>
</p>
...
<p align="center">
	<a href="https://github.com/lyxal/lyxalmx"><img src="https://img.shields.io/badge/status-stable-ff00bb.svg?style=flat-square"></a>
	&nbsp;
	<a href="https://docs.rs/lyxalmx/"><img src="https://img.shields.io/docsrs/lyxalmx?style=flat-square"></a>
	&nbsp;
	<a href="https://crates.io/crates/lyxalmx"><img src="https://img.shields.io/crates/v/lyxalmx?style=flat-square"></a>
	&nbsp;
	<a href="https://github.com/lyxal/lyxalmx"><img src="https://img.shields.io/badge/license-Apache_License_2.0-00bfff.svg?style=flat-square"></a>
</p>
...
```rust
use lyxalmx::{Database, DatabaseOptions};
...
lyxalmx supports optional persistence with two modes:
...
```rust
use lyxalmx::{Database, DatabaseOptions, PersistenceOptions, AolMode, SnapshotMode};
...
```rust
use lyxalmx::{Database, DatabaseOptions, PersistenceOptions, AolMode, SnapshotMode, FsyncMode, CompressionMode};
use std::time::Duration;
...
lyxalmx provides different levels of durability (the "D" in ACID) depending on the persistence configuration:

##### In-memory only mode (No persistence)

When persistence is disabled (the default), lyxalmx provides **no durability guarantees**. All data is lost when the process terminates, crashes, or the system shuts down. This mode is ideal for:
...
```rust
use lyxalmx::{Database, DatabaseOptions, PersistenceOptions, AolMode, FsyncMode};
...
lyxalmx's MVCC (Multi-Version Concurrency Control) design allows you to read data as it existed at any point in time. This enables powerful use cases like:
...
lyxalmx supports two isolation levels to balance between performance and consistency guarantees:
...
When a transaction is aborted due to a conflict (`KeyReadConflict` or `KeyWriteConflict`), lyxalmx can log the conflicting key for debugging purposes. These logs are emitted at the `DEBUG` level using the [`tracing`](https://docs.rs/tracing) crate, and are only included in debug builds (`#[cfg(debug_assertions)]`).

All conflict logs are emitted under a dedicated tracing target, exposed as the public constant [`LOG_TARGET_CONFLICTS`]:

```rust
use lyxalmx::LOG_TARGET_CONFLICTS;
// Value: "lyxalmx::conflicts"
```
...
tracing_subscriber::fmt()
    .with_env_filter(
        EnvFilter::builder()
            .parse("info,lyxalmx::conflicts=debug")
            .unwrap(),
    )
    .init();
```
...
```sh
RUST_LOG="info,lyxalmx::conflicts=debug" cargo run
```

#### Range operations

lyxalmx provides powerful range-based operations for scanning, counting, and iterating over keys. All range operations support:
...
**Note:** This project was originally developed under the name `memodb`. It has been renamed to `lyxalmx` to better reflect its evolution and alignment with the Lyxal ecosystem.
