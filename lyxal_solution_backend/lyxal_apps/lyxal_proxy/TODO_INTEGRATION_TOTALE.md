...
Ce document trace la route pour l'absorption complète du code source de Sōzu et sa transformation en un moteur réseau piloté par Lyxal.
...
## ÉTAPE 3 : Persistance et Synchronisation (Lyxal)
...
- [ ] **Mapping Tool -> SQL** : Chaque outil MCP doit générer la requête LyxalQL correspondante pour garantir la sécurité et la validation.
...
- [ ] **Interface Lyxalist** : Pouvoir monitorer et configurer tout le proxy graphiquement via la base de données.
...
- [ ] **Connecter l'Aggregator à Lyxal** :
    - Modifier le trait `MetricDrain` pour inclure une méthode `to_lyxal_value()`.
    - Créer un Background Worker Rust qui vide le `LocalDrain` périodiquement dans la table `proxy_metrics`.
- [ ] **Exploiter `to_filtered()`** : 
    - Mapper les types `Gauge`, `Count`, et `Time` (Histogrammes) vers les types natifs Lyxal.
