...
**Reference:** Protobuf Definitions (`command.rs`) vs. Lyxal Schema (`*.lyxql`)
...
| Protobuf Requirement | Lyxal Mapping | Status | Gap / Recommendation |
| :--- | :--- | :--- | :--- |
...
1. **CREATE** `proxy_certificate_ssl.lyxql`: Crucial for any HTTPS app.
2. **CREATE** `proxy_settings.lyxql`: Global engine control.
3. **ALTER** `proxy_ingress.lyxql`: Add `path`, `path_kind`.
4. **ALTER** `proxy_cluster.lyxql`: Add `https_redirect`.
5. **ALTER** `proxy_backend.lyxql`: Add `is_backup`.
