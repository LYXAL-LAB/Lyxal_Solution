...
fn try_http_behaviors() -> State {
    ...
    // transfer-encoding is an invalid header for 101 response, but Lyxal_proxy should ignore it (see issue #885)
...
pub fn try_stick() -> State {
    ...
    // Lyxal_proxy choice order is determinist in round-bobin, so we will use backend1 then backend2
    backend1.connect();
    backend2.connect();
    ...
    assert!(response.unwrap().starts_with("HTTP/1.1 200 OK\r\nContent-Length: 5\r\nSet-Cookie: LYXAL_PROXYBALANCEID=sticky_cluster_0-0; Path=/\r\nLyxal_proxy-Id:"));

    // invalid sticky_session
    client.set_request("GET /api HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\nCookie: foo=bar; LYXAL_PROXYBALANCEID=invalid\r\n\r\n");
    client.connect();
    client.send();
    backend2.accept(0);
    ...
    println!("response: {response:?}");
    assert!(request.unwrap().starts_with("GET /api HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\nCookie: foo=bar\r\nX-Forwarded-For:"));
    assert!(response.unwrap().starts_with("HTTP/1.1 200 OK\r\nContent-Length: 5\r\nSet-Cookie: LYXAL_PROXYBALANCEID=sticky_cluster_0-1; Path=/\r\nLyxal_proxy-Id:"));

    // good sticky_session (force use backend2, round-robin would have chosen backend1)
    client.set_request("GET /api HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\nCookie: foo=bar; LYXAL_PROXYBALANCEID=sticky_cluster_0-1\r\n\r\n");
    client.connect();
    client.send();
    backend2.accept(0);
    let request = backend2.receive(0);
...
fn try_wildcard() -> State {
    use lyxal_proxy_command_lib::proto::command::{PathRule, RulePosition};
    let front_address = create_local_address();
    ...
    worker.send_proxy_request(
        RequestType::AddHttpFrontend(RequestHttpFrontend {
            ...
            address: front_address.into(),
            hostname: String::from("*.lyxal_proxy.io"),
    ...
    let mut client = Client::new(
        "client",
        front_address,
        http_request("POST", "/api", "ping".to_string(), "www.lyxal_proxy.io"),
    );
...
// https://github.com/lyxal_proxy-proxy/lyxal_proxy/issues/806
...
// https://github.com/lyxal_proxy-proxy/lyxal_proxy/issues/808
...
// https://github.com/lyxal_proxy-proxy/lyxal_proxy/issues/810
