...
- [ ] Créer la table `proxy_certificate_ssl.lyxql` (Certificats/Clés).
- [ ] Créer la table `proxy_settings.lyxql` (Configuration globale du binaire).
- [ ] Ajouter `path` et `path_kind` dans `proxy_ingress.lyxql`.
- [ ] Ajouter `https_redirect` et `load_metric` dans `proxy_cluster.lyxql`.
- [ ] Ajouter `is_backup` dans `proxy_backend.lyxql`.
