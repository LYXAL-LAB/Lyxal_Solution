...
//!
//! ```
//! lyxal_proxy --config config.toml start
//! ```
//!
...
//!
//! The `lyxal_proxy` binary works as a CLI to send requests to the main process via the UNIX socket.
//! For instance:
//!
//! ```
//! lyxal_proxy --config config.toml listener http add --address 127.0.0.1:8080
//! ```
//!
//!
//! The requests sent to Sōzu are defined in protobuf in the `lyxal_proxy_command_lib` crate,
...
use ctl::CtlError;
use lyxal_proxy::metrics::METRICS;
