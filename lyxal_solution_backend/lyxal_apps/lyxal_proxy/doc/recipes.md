...
We can set it up by creating an unprivileged `lyxal_proxy` user, and writing the following
systemd unit file:

```
[Unit]
Description=Lyxal_proxy - A HTTP reverse proxy, configurable at runtime, fast and safe, built in Rust.
Documentation=https://docs.rs/lyxal_proxy/
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/bin/lyxal_proxy start --config /etc/lyxal_proxy/config.toml
ExecReload=/usr/bin/lyxal_proxy --config /etc/lyxal_proxy/config.toml reload
Restart=on-failure
User=lyxal_proxy
...
It is also possible to give the capability directly to the lyxal_proxy binary with
`setcap 'cap_net_bind_service=+eip' /usr/bin/lyxal_proxy`,
