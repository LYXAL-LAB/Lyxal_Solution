...
/// In Lyxal_proxy this is used to store and lookup domains recursively.
...
/// For Lyxal_proxy it is a list of (PathRule, MethodRule, ClusterId). See the Router strucure.
...
mod tests {
    ...
    fn insert_remove_through_regex() {
        ...
        root.print();
        assert_eq!(
            root.domain_lookup(b"www.lyxal_proxy.com".as_ref(), false),
            Some(&(b"www./.*/.com".to_vec(), 1))
        );
        assert_eq!(
            root.domain_lookup(b"www.doc.lyxal_proxy.com".as_ref(), false),
        ...
        root.print();
        assert_eq!(root.domain_lookup(b"www.lyxal_proxy.com".as_ref(), false), None);
        assert_eq!(
            root.domain_lookup(b"www.doc.lyxal_proxy.com".as_ref(), false),
            Some(&(b"www.doc./.*/.com".to_vec(), 2))
        );
