...
//!
//! ## Difference with the crate `lyxal_proxy`
//!
//! To create several workers and manage them all at once (which is the most common way to
//! use Sōzu), the crate `lyxal_proxy` is more indicated than using the lib directly.
//!
//! The crate `lyxal_proxy` provides a binary called the main process.
//! The main process uses `lyxal_proxy_lib` to start and manage workers.
...
//! We need the `lyxal_proxy_command_lib` to build a listener.
//!
//! ```
//! use lyxal_proxy_command_lib::{config::ListenerBuilder, proto::command::SocketAddress};
//!
...
//!     lyxal_proxy_lib::http::testing::start_http_worker(http_listener, proxy_channel, max_buffers, buffer_size);
//! });
//! ```
//!
//! ### Send orders
...
//!
//! ```
//! use lyxal_proxy_command_lib::proto::command::{Cluster, LoadBalancingAlgorithms};
//!
//! let cluster = Cluster {
...
//!
//!  use lyxal_proxy_command_lib::proto::command::{PathRule, RequestHttpFrontend, RulePosition, SocketAddress};
//!
...
//!
//! ```
//! use lyxal_proxy_command_lib::proto::command::{AddBackend, LoadBalancingParams, SocketAddress};
//!
...
#[macro_use]
extern crate lyxal_proxy_command_lib as lyxal_proxy_command;
#[cfg(test)]
#[macro_use]
...
/// unused for now but may be usefull for bypassing lyxal_proxy on a low level
#[cfg(feature = "splice")]
mod splice;
