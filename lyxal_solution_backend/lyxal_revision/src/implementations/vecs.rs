...
where
	W: Write,
{
	v.len().serialize_lyxal_revisioned(writer)?;
	writer.write_all(v).map_err(Error::Io)
}

impl<T> SerializeLyxalRevisioned for Vec<T>
where
	T: SerializeLyxalRevisioned + 'static,
{
	#[inline]
			...
			macro_rules! try_specialized {
				($ty:ty) => {
					if TypeId::of::<T>() == TypeId::of::<$ty>() {
						use crate::implementations::specialised::SerializeLyxalRevisionedSpecialised;
						let specialized = unsafe { &*(self as *const Vec<T> as *const Vec<$ty>) };
						return SerializeLyxalRevisionedSpecialised::serialize_lyxal_revisioned_specialised(
							specialized,
							writer,
						);
					}
...
impl<T> DeserializeLyxalRevisioned for Vec<T>
where
	T: DeserializeLyxalRevisioned + 'static,
{
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, Error> {
		...
		#[cfg(feature = "specialised")]
		{
			macro_rules! try_specialized {
				($ty:ty) => {
					if TypeId::of::<T>() == TypeId::of::<$ty>() {
						use crate::implementations::specialised::DeserializeLyxalRevisionedSpecialised;
						return Vec::<$ty>::deserialize_lyxal_revisioned_specialised(reader)
...
impl<T> LyxalRevisioned for Vec<T> {
	#[inline]
...
mod tests {
	...
	fn test_vec() {
		...
		let out =
			<Vec<String> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_vec_bool() {
		...
		let mut mem: Vec<u8> = vec![];
		val.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out = <Vec<bool> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
			.unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_vec_empty() {
		...
		// Test bool
		let mut mem: Vec<u8> = vec![];
		empty_bool.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_bool =
			<Vec<bool> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
		assert_eq!(empty_bool, out_bool);

		// Test String
		mem.clear();
		empty_string.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_string =
			<Vec<String> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
	...
	fn test_vec_large() {
		...
		let mut mem: Vec<u8> = vec![];
		large_bool.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_bool =
			<Vec<bool> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
	...
	fn test_vec_edge_cases() {
		...
		let mut mem: Vec<u8> = vec![];
		all_true.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_true =
			<Vec<bool> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
		...
		mem.clear();
		all_false.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_false =
			<Vec<bool> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
