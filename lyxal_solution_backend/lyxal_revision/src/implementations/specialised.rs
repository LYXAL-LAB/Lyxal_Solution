...
mod tests {
	...
	fn test_vec_edge_cases() {
		...
		let mut mem: Vec<u8> = vec![];
		single_u8.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out = <Vec<u8> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
			.unwrap();
		...
		mem.clear();
		float_specials.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_floats =
			<Vec<f64> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
	...
	fn test_vec_f32_special_values() {
		...
		float_specials.serialize_lyxal_revisioned(&mut mem).unwrap();
		let out_floats =
			<Vec<f32> as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice())
				.unwrap();
		assert_eq!(out_floats.len(), float_specials.len());
		for (i, (&expected, &actual)) in float_specials.iter().zip(out_floats.iter()).enumerate() {
			if expected.is_nan() {
