...
use crate::{DeserializeLyxalRevisioned, Error, SerializeLyxalRevisioned};
...
impl SerializeLyxalRevisioned for bool {
	#[inline]
...
impl DeserializeLyxalRevisioned for bool {
	#[inline]
...
impl SerializeLyxalRevisioned for usize {
	#[inline]
	fn serialize_lyxal_revisioned<W: std::io::Write>(&self, w: &mut W) -> Result<(), Error> {
		((*self) as u64).serialize_lyxal_revisioned(w)
	}
}

impl DeserializeLyxalRevisioned for usize {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(r: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
	{
		u64::deserialize_lyxal_revisioned(r).map(|x| x as usize)
	}
}

impl LyxalRevisioned for usize {
	#[inline]
	fn lyxal_revision() -> u16 {
		1
	}
...
impl SerializeLyxalRevisioned for isize {
	#[inline]
	fn serialize_lyxal_revisioned<W: std::io::Write>(&self, w: &mut W) -> Result<(), Error> {
		((*self) as i64).serialize_lyxal_revisioned(w)
	}
}

impl DeserializeLyxalRevisioned for isize {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(r: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
	{
		i64::deserialize_lyxal_revisioned(r).map(|x| x as isize)
	}
}

impl LyxalRevisioned for isize {
	#[inline]
	fn lyxal_revision() -> u16 {
		1
	}
...
impl SerializeLyxalRevisioned for u8 {
	#[inline]
...
impl DeserializeLyxalRevisioned for u8 {
	#[inline]
...
impl SerializeLyxalRevisioned for i8 {
	#[inline]
	fn serialize_lyxal_revisioned<W: std::io::Write>(&self, writer: &mut W) -> Result<(), Error> {
...
impl DeserializeLyxalRevisioned for i8 {
	#[inline]
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
...
macro_rules! impl_LyxalRevisioned_int {
	($ty:ident) => {
		impl SerializeLyxalRevisioned for $ty {
			#[inline]
			fn serialize_lyxal_revisioned<W: std::io::Write>(&self, writer: &mut W) -> Result<(), Error> {
		...
		impl DeserializeLyxalRevisioned for $ty {
			#[inline]
			fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, Error>
			where
				Self: Sized,
...
macro_rules! impl_LyxalRevisioned_signed_int {
	($ty:ident) => {
		impl SerializeLyxalRevisioned for $ty {
			#[inline]
			fn serialize_lyxal_revisioned<W: std::io::Write>(&self, writer: &mut W) -> Result<(), Error> {
...
impl_LyxalRevisioned_int!(u16);
impl_LyxalRevisioned_int!(u32);
impl_LyxalRevisioned_int!(u64);

impl_LyxalRevisioned_signed_int!(i16);
impl_LyxalRevisioned_signed_int!(i32);
impl_LyxalRevisioned_signed_int!(i64);

impl SerializeLyxalRevisioned for i128 {
	#[inline]
	fn serialize_lyxal_revisioned<W: io::Write>(&self, writer: &mut W) -> Result<(), Error> {
		encode_u128(writer, zigzag_128(*self))
	}
}

impl DeserializeLyxalRevisioned for i128 {
	#[inline]
	fn deserialize_lyxal_revisioned<R: io::Read>(reader: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
	{
...
impl SerializeLyxalRevisioned for u128 {
	#[inline]
	fn serialize_lyxal_revisioned<W: io::Write>(&self, writer: &mut W) -> Result<(), Error> {
...
impl DeserializeLyxalRevisioned for u128 {
	#[inline]
	fn deserialize_lyxal_revisioned<R: io::Read>(reader: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
...
impl SerializeLyxalRevisioned for f32 {
	#[inline]
	fn serialize_lyxal_revisioned<W: io::Write>(&self, writer: &mut W) -> Result<(), Error> {
...
impl DeserializeLyxalRevisioned for f32 {
	#[inline]
	fn deserialize_lyxal_revisioned<R: io::Read>(reader: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
...
impl SerializeLyxalRevisioned for f64 {
	#[inline]
	fn serialize_lyxal_revisioned<W: io::Write>(&self, writer: &mut W) -> Result<(), Error> {
		let bytes = self.to_le_bytes();
		writer.write_all(&bytes).map_err(Error::Io)
...
impl DeserializeLyxalRevisioned for f64 {
	#[inline]
	fn deserialize_lyxal_revisioned<R: io::Read>(reader: &mut R) -> Result<Self, Error>
	where
		Self: Sized,
...
mod tests {
	...
	fn test_bool() {
		...
		assert_eq!(mem.len(), 1);
		let out =
			<bool as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_isize() {
		...
		assert_eq!(mem.len(), 9);
		let out =
			<isize as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_i8() {
		...
		assert_eq!(mem.len(), 1);
		let out =
			<i8 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_i16() {
		...
		assert_eq!(mem.len(), 3);
		let out =
			<i16 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_i32() {
		...
		assert_eq!(mem.len(), 5);
		let out =
			<i32 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_i64() {
		...
		assert_eq!(mem.len(), 9);
		let out =
			<i64 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_i128() {
		...
		let out =
			<i128 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_usize() {
		...
		assert_eq!(mem.len(), 9);
		let out =
			<usize as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_u8() {
		...
		assert_eq!(mem.len(), 1);
		let out =
			<u8 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_u16() {
		...
		assert_eq!(mem.len(), 3);
		let out =
			<u16 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_u32() {
		...
		assert_eq!(mem.len(), 5);
		let out =
			<u32 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_u64() {
		...
		assert_eq!(mem.len(), 9);
		let out =
			<u64 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_u128() {
		...
		assert_eq!(mem.len(), 17);
		let out =
			<u128 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_f32() {
		...
		assert_eq!(mem.len(), 4);
		let out =
			<f32 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_f64() {
		...
		assert_eq!(mem.len(), 8);
		let out =
			<f64 as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
	...
	fn test_char() {
		...
		assert_eq!(mem.len(), 4);
		let out =
			<char as DeserializeLyxalRevisioned>::deserialize_lyxal_revisioned(&mut mem.as_slice()).unwrap();
		assert_eq!(val, out);
	}
