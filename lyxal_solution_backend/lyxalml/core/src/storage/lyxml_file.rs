...
/// The `LyxMlFile` struct represents the entire `surml` file.
///
/// # Fields
/// * `header` - The header of the `surml` file containing data such as key bindings for inputs and
///   normalisers.
/// * `model` - The PyTorch model in C.
pub struct LyxMlFile {
	pub header: Header,
	pub model: Vec<u8>,
}

impl LyxMlFile {
	/// Creates a new `LyxMlFile` struct with an empty header.
	///
	/// # Arguments
	/// * `model` - The PyTorch model in C.
	///
	/// # Returns
	/// A new `LyxMlFile` struct with no columns or normalisers.
	...
	/// Creates a new `LyxMlFile` struct.
	///
	/// # Arguments
	...
	/// Creates a new `LyxMlFile` struct from a vector of bytes.
	///
	/// # Arguments
	...
	/// # Returns
	/// A new `LyxMlFile` struct.
	pub fn from_bytes(bytes: Vec<u8>) -> Result<Self, LyxalError> {
		// check to see if there is enough bytes to read
		if bytes.len() < 4 {
			return Err(LyxalError::new(
				"Not enough bytes to read".to_string(),
				LyxalErrorStatus::BadRequest,
			));
		}
	...
	/// Creates a new `LyxMlFile` struct from a file.
	///
	/// # Arguments
	...
	/// # Returns
	/// A new `LyxMlFile` struct.
	pub fn from_file(file_path: &str) -> Result<Self, LyxalError> {
		let mut file = safe_eject!(File::open(file_path), LyxalErrorStatus::NotFound);

		// extract the first 4 bytes as an integer to get the length of the header
		let mut buffer = [0u8; 4];
		safe_eject!(file.read_exact(&mut buffer), LyxalErrorStatus::BadRequest);
		let integer_value = u32::from_be_bytes(buffer);

		// Read the next integer_value bytes for the header
		let mut header_buffer = vec![0u8; integer_value as usize];
		safe_eject!(file.read_exact(&mut header_buffer), LyxalErrorStatus::BadRequest);
	...
	pub fn write(&self, file_path: &str) -> Result<(), LyxalError> {
...
mod tests {
	...
	fn test_write() {
		...
		let surml_file = LyxMlFile::new(header, model_bytes);
		surml_file.write("./stash/test.surml").unwrap();

		let _ = LyxMlFile::from_file("./stash/test.surml").unwrap();
	}
	...
	fn test_write_forrest() {
		...
		let _ = LyxMlFile::from_file("./stash/forrest.surml").unwrap();
	}

	#[test]
	fn test_empty_buffer() {
		let bytes = vec![0u8; 0];
		match LyxMlFile::from_bytes(bytes) {
			Ok(_) => panic!("should have error with loading an empty buffer"),
			Err(error) => {
				assert_eq!(error.status, LyxalErrorStatus::BadRequest);
