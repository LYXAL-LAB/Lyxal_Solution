...
impl Header {
	...
	pub fn add_version(&mut self, version: String) -> Result<(), LyxalError> {
	...
	pub fn get_normaliser(
		&self,
		column_name: &String,
	) -> Result<Option<&NormaliserType>, LyxalError> {
	...
	/// # Returns
	/// The `Header` struct.
	pub fn from_bytes(data: Vec<u8>) -> Result<Self, LyxalError> {
		let string_data = safe_eject!(String::from_utf8(data), LyxalErrorStatus::BadRequest);
