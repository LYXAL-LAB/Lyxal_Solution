...
/// * `Lyxal` - The model was created in the lyxal database.
...
impl fmt::Display for OriginValue {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			OriginValue::Local(val) | OriginValue::Lyxal(val) | OriginValue::None(val) => {
				write!(f, "{}", val)
			}
		}
	}
...
impl Origin {
	...
	///
	/// # Arguments
	pub fn add_origin(&mut self, origin: String) -> Result<(), LyxalError> {
