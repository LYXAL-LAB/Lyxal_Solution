...
impl Version {
	...
	pub fn from_string(version: String) -> Result<Self, LyxalError> {
		if version == *"" {
		...
		Ok(Version {
			one: safe_eject!(one_str.parse::<u8>(), LyxalErrorStatus::BadRequest),
			two: safe_eject!(two_str.parse::<u8>(), LyxalErrorStatus::BadRequest),
			three: safe_eject!(three_str.parse::<u8>(), LyxalErrorStatus::BadRequest),
		})
	}
