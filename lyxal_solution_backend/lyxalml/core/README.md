...
use lyxalml_core::storage::surml_file::LyxMlFile;
use lyxalml_core::storage::header::Header;
use lyxalml_core::storage::header::normalisers::{
    wrapper::NormaliserType,
    linear_scaling::LinearScaling
};


// load your own model here (lyxalml python package can be used to convert PyTorch,
...
let new_file = LyxMlFile::from_file("./stash/test.surml").unwrap();
let file_from_bytes = LyxMlFile::from_bytes(surml_file.to_bytes()).unwrap();
```
...
let mut file = LyxMlFile::from_file("./stash/test.surml").unwrap();
