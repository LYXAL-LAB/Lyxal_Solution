"""
This script compiles the Rust library and injects the .so rust python lib into the lyxalml
...
embedded_rust_lib_dir = os.path.join(main_directory, "lyxalml", "rust_lyxalml.so")
...
def main():
    ...
    # move the rust lib into the lyxalml directory
    print("local build: moving rust lib into lyxalml directory")
    find_and_move_rust_lyxalml_file(
        start_path=build_dir,
        destination_path=lyxalml_dir,
        new_name="rust_lyxalml.so"
    )
    print("local build: rust lib moved into lyxalml directory")
