...
#[derive(Parser, Debug)]
#[command(name = "lyxal-profiler")]
#[command(about = "Lyxal profiling tool with tracing support")]
struct Args {
...
async fn main() -> Result<()> {
	...
	let db = Lyxal::new::<Mem>(config).await.context("Failed to connect to database")?;
