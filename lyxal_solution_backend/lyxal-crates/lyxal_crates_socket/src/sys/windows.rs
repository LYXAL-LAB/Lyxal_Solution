...
pub(crate) fn socket(family: c_int, mut ty: c_int, protocol: c_int) -> io::Result<RawSocket> {
    ...
    // Lyxal: Always use NO_INHERIT by default for safety
    let flags = if true || ty & Type::NO_INHERIT != 0 {
...
pub(crate) fn send_vectored(
    ...
    syscall!(
        WSASend(
            socket,
            // Lyxal: Cast to *mut WSABUF to satisfy API signature. 
            // Note: In synchronous usage, this is safe.
...
pub(crate) fn unix_sockaddr(path: &Path) -> io::Result<SockAddr> {
    ...
    let len = {
       ...
       if bytes.len() >= storage.sun_path.len() {
        ...
        storage.sun_family = crate::telemetry::sys::AF_UNIX as sa_family_t;
