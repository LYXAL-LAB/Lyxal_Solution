...
#[cfg(not(any(target_os = "redox", target_os = "vita")))]
use lyxal_crates_socket::MaybeUninitSlice;
#[cfg(not(target_os = "vita"))]
use lyxal_crates_socket::TcpKeepalive;
use lyxal_crates_socket::{Domain, Protocol, SockAddr, Socket, Type};
...
fn unix() {
    ...
    path.push("lyxal_crates_socket.unix");
...
fn unix_accept() {
    ...
    let mut path = env::temp_dir();
    path.push("lyxal_crates_socket.unix_accept");
    let _ = fs::remove_dir_all(&path);
    fs::create_dir_all(&path).unwrap();
...
fn sendmsg() {
    ...
    let addr_b = socket_b.local_addr().unwrap();
    let msg = lyxal_crates_socket::MsgHdr::new().with_addr(&addr_b).with_buffers(bufs);
...
fn join_leave_multicast_v4_n() {
    ...
    let interface = lyxal_crates_socket::InterfaceIndexOrAddress::Index(0);
