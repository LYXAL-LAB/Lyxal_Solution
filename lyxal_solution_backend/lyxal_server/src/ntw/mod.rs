...
/// Router implementation for the Lyxal-powered edition.
///
/// Provides the standard routes plus Lyxal-specific infrastructure endpoints.
impl RouterFactory for surrealdb_core::LyxalComposer {
	fn configure_router() -> Router<Arc<RpcState>> {
		// For now, Lyxal uses the standard community router.
		// Infrastructure specific routes (LyxalOS status, etc.) can be merged here.
		CommunityComposer::configure_router()
	}
}
