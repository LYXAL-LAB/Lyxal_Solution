...
use crate::{Kind, LyxalValue, Value};
...
pub fn decode<T: LyxalValue>(value: &[u8]) -> anyhow::Result<T> {
	let value_fb = flatbuffers::root::<lyxal_protocol::fb::v1::Value>(value)
...
pub fn decode_kind(value: &[u8]) -> anyhow::Result<Kind> {
	let value_fb = flatbuffers::root::<lyxal_protocol::fb::v1::Kind>(value)
