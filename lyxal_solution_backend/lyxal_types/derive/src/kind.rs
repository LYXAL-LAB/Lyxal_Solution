...
/// `lyxal_types::Kind` variant at compile time.
fn generate_kind_code(expr: &KindExpr) -> TokenStream2 {
	match expr {
		...
		KindExpr::Union(variants) => {
			...
			quote! {
				lyxal_types::Kind::Either(vec![#(#variant_codes),*])
			}
		}
		KindExpr::Literal(lit) => generate_literal_kind(lit),
		KindExpr::KindPrefix(path) => {
			// Convert Kind::Something to lyxal_types::Kind::Something
			quote! { lyxal_types::Kind::#path }
		}
		KindExpr::LiteralPrefix(expr) => {
			quote! {
				lyxal_types::Kind::Literal(lyxal_types::KindLiteral::#expr)
			}
		}
		...
		KindExpr::Object(fields) => {
			...
			quote! {
				lyxal_types::Kind::Literal(
					lyxal_types::KindLiteral::Object(
		...
		KindExpr::Array(elements) => {
			...
			quote! {
				lyxal_types::Kind::Literal(
					lyxal_types::KindLiteral::Array(vec![#(#element_codes),*])
				)
			}
		}
	}
...
fn generate_literal_kind(lit: &syn::Lit) -> TokenStream2 {
	match lit {
		syn::Lit::Bool(lit_bool) => {
			...
			quote! {
				lyxal_types::Kind::Literal(lyxal_types::KindLiteral::Bool(#value))
			}
		}
		syn::Lit::Int(lit_int) => {
			if let Ok(value) = lit_int.base10_parse::<i64>() {
				quote! {
					lyxal_types::Kind::Literal(lyxal_types::KindLiteral::Integer(#value))
				}
			} else {
		...
		syn::Lit::Float(lit_float) => {
			if let Ok(value) = lit_float.base10_parse::<f64>() {
				quote! {
					lyxal_types::Kind::Literal(lyxal_types::KindLiteral::Float(#value))
				}
			} else {
		...
		syn::Lit::Str(lit_str) => {
			...
			quote! {
				lyxal_types::Kind::Literal(lyxal_types::KindLiteral::String(#value.to_string()))
			}
		}
...
fn generate_simple_kind(ident: &Ident) -> TokenStream2 {
	...
	match name.as_str() {
		"any" => quote! { lyxal_types::Kind::Any },
		"none" => quote! { lyxal_types::Kind::None },
		"null" => quote! { lyxal_types::Kind::Null },
		"bool" => quote! { lyxal_types::Kind::Bool },
		"bytes" => quote! { lyxal_types::Kind::Bytes },
		"datetime" => quote! { lyxal_types::Kind::Datetime },
		"decimal" => quote! { lyxal_types::Kind::Decimal },
		"duration" => quote! { lyxal_types::Kind::Duration },
		"float" => quote! { lyxal_types::Kind::Float },
		"int" => quote! { lyxal_types::Kind::Int },
		"number" => quote! { lyxal_types::Kind::Number },
		"object" => quote! { lyxal_types::Kind::Object },
		"string" => quote! { lyxal_types::Kind::String },
		"uuid" => quote! { lyxal_types::Kind::Uuid },
		"regex" => quote! { lyxal_types::Kind::Regex },
		"range" => quote! { lyxal_types::Kind::Range },
		"table" => quote! { lyxal_types::Kind::Table(vec![]) },
		"record" => quote! { lyxal_types::Kind::Record(vec![]) },
		"geometry" => quote! { lyxal_types::Kind::Geometry(vec![]) },
		"set" => quote! { lyxal_types::Kind::Set(Box::new(lyxal_types::Kind::Any), None) },
		"array" => {
			quote! { lyxal_types::Kind::Array(Box::new(lyxal_types::Kind::Any), None) }
		}
		"file" => quote! { lyxal_types::Kind::File(vec![]) },
		"function" => quote! { lyxal_types::Kind::Function(None, None) },
...
fn generate_parameterized_kind(name: &Ident, params: &[KindParam]) -> TokenStream2 {
	...
	match name_str.as_str() {
		"table" => {
			...
			quote! {
				lyxal_types::Kind::Table(vec![#(#tables.into()),*])
			}
		}
		"record" => {
			let mut tables = Vec::new();
			...
			quote! {
				lyxal_types::Kind::Record(vec![#(#tables.into()),*])
			}
		}
		"array" => {
			if params.is_empty() {
				...
				quote! {
					lyxal_types::Kind::Array(Box::new(#inner_kind), #size)
				}
			}
		}
		"set" => {
			if params.is_empty() {
				quote! { lyxal_types::Kind::Set(Box::new(lyxal_types::Kind::Any), None) }
			} else {
				let inner_kind = generate_param_code(&params[0]);
				let size = if params.len() > 1 {
					if let KindParam::Literal(syn::Lit::Int(int_lit)) = &params[1] {
				...
				quote! {
					lyxal_types::Kind::Set(Box::new(#inner_kind), #size)
				}
			}
		}
		"geometry" => {
				...
				match param {
					KindParam::Ident(ident) => {
						...
						match geom_name.as_str() {
							"point" => {
								geom_kinds.push(quote! { lyxal_types::GeometryKind::Point })
							}
							"line" => {
								geom_kinds.push(quote! { lyxal_types::GeometryKind::Line })
							}
							"polygon" => {
								geom_kinds.push(quote! { lyxal_types::GeometryKind::Polygon })
							}
							"multipoint" => geom_kinds
								.push(quote! { lyxal_types::GeometryKind::MultiPoint }),
							"multiline" => {
								geom_kinds.push(quote! { lyxal_types::GeometryKind::MultiLine })
							}
							"multipolygon" => geom_kinds
								.push(quote! { lyxal_types::GeometryKind::MultiPolygon }),
							"collection" => geom_kinds
								.push(quote! { lyxal_types::GeometryKind::Collection }),
							_ => {}
						}
					}
					KindParam::Kind(KindExpr::Union(variants)) => {
						...
						for variant in variants {
							if let KindExpr::Ident(ident) = variant {
								...
								match geom_name.as_str() {
									"point" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::Point }),
									"line" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::Line }),
									"polygon" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::Polygon }),
									"multipoint" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::MultiPoint }),
									"multiline" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::MultiLine }),
									"multipolygon" => geom_kinds.push(
										quote! { lyxal_types::GeometryKind::MultiPolygon },
									),
									"collection" => geom_kinds
										.push(quote! { lyxal_types::GeometryKind::Collection }),
									_ => {}
			...
			quote! {
				lyxal_types::Kind::Geometry(vec![#(#geom_kinds),*])
			}
		}
		"file" => {
			...
			quote! {
				lyxal_types::Kind::File(vec![#(#buckets.to_string()),*])
			}
		}
