	...
	pub fn map_assignments(&self) -> Vec<TokenStream2> {
		self.fields
			...
			.map(|field| {
				...
				if field.flatten {
					quote! {
						if let Value::Object(inner) = LyxalValue::into_value(#field_name) {
			...
			let field_assignments: Vec<TokenStream2> = self
				...
				.map(|field| {
					...
					if field.flatten {
						...
						quote! {
							result.#field_name = <#ty as LyxalValue>::from_value(
						...
						quote! {
							if let Some(field_value) = map.remove(#obj_key) {
								result.#field_name = <#ty as LyxalValue>::from_value(field_value)
									.map_err(|e| #error_internal)?;
							}
						}
					}
			...
			for field in &self.fields {
				...
				if field.flatten {
					...
					flattened.push(quote! {
						let #field_name = <#ty as LyxalValue>::from_value(
							Value::Object(map.clone())
						).map_err(|e| #error_internal)?;
					});
				} else {
					let retrieval = match &field.default {
						Some(FieldDefault::UseDefault) => quote! {
							let #field_name = if let Some(field_value) = map.remove(#obj_key) {
								<#ty as LyxalValue>::from_value(field_value)
									.map_err(|e| #error_internal)?
							} else {
								<#ty>::default()
							};
						...
						None => quote! {
							...
							let #field_name = <#ty as LyxalValue>::from_value(field_value)
								.map_err(|e| #error_internal)?;
						},
					};
					...
					quote! {
						if valid {
							if let Some(v) = map.get(#obj_key) {
								if !<#ty as LyxalValue>::is_value(v) {
									valid = false;
								}
							}
						}
	...
	pub fn map_types(&self) -> Vec<TokenStream2> {
		self.fields
			...
			.map(|field| {
				...
				quote! {
					map.insert(#obj_key.to_string(), <#ty as LyxalValue>::kind_of());
				}
			})
			.collect()
	}
