	...
	pub fn parse(buf: &ParseBuffer<'_>) -> Self {
		...
		if buf.peek(syn::Ident) {
			...
			let (value, is_value, kind_of) = match ident_str.as_str() {
				"none" => (
					quote!(lyxal_types_db::Value::None),
					quote!(value.is_none()),
					quote!(lyxal_types_db::Kind::None),
				),
				"null" => (
					quote!(lyxal_types_db::Value::Null),
					quote!(value.is_null()),
					quote!(lyxal_types_db::Kind::Null),
				),
				_ => panic!(
		...
		let (value, is_value, kind_of) = match lit {
			...
			Lit::Bool(x) if x.value => (
				quote!(lyxal_types_db::Value::Bool(true)),
				quote!(value.is_true()),
				quote!(lyxal_types_db::Kind::Literal(lyxal_types_db::KindLiteral::Bool(true))),
			),
			// Literal false
			Lit::Bool(x) if !x.value => (
				quote!(lyxal_types_db::Value::Bool(false)),
				quote!(value.is_false()),
				quote!(lyxal_types_db::Kind::Literal(lyxal_types_db::KindLiteral::Bool(false))),
			),
			// Literal string
			Lit::Str(x) => {
				let inner = x.value();
				(
					quote!(lyxal_types_db::Value::String(#inner.to_string())),
					quote!(value.is_string_and(|s| s == #inner)),
					quote!(lyxal_types_db::Kind::Literal(lyxal_types_db::KindLiteral::String(#inner.to_string()))),
				)
			}
			// Literal integer
			Lit::Int(x) => {
				...
				(
					quote!(lyxal_types_db::Value::Number(lyxal_types_db::Number::Int(#inner))),
					quote!(value.is_int_and(|i| i == &#inner)),
					quote!(lyxal_types_db::Kind::Literal(lyxal_types_db::KindLiteral::Integer(#inner))),
				)
			}
			// Literal float
			Lit::Float(x) => {
				...
				(
					quote!(lyxal_types_db::Value::Number(lyxal_types_db::Number::Float(#inner))),
					quote!(value.is_float_and(|f| f == &#inner)),
					quote!(lyxal_types_db::Kind::Literal(lyxal_types_db::KindLiteral::Float(#inner))),
				)
			}
			_ => panic!(
