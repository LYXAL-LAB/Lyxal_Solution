...
impl CratePath {
	...
	///
	/// Looks for `#[lyxal(crate = "path::to::crate")]` attribute.
	/// If not specified, defaults to `::lyxal_types_db::types` when the
	/// `sdk-path` feature is enabled, otherwise `::lyxal_types_db`.
...
impl Default for CratePath {
	fn default() -> Self {
		let path = if cfg!(feature = "sdk-path") {
			quote! { ::lyxal_types_db::types }
		} else {
			quote! { ::lyxal_types_db }
		};
		Self {
			path,
		}
