use lyxal_types_db::{
	...
	OutOfRangeError, QueryError, SerializationError, LyxalValue, TypeError, ValidationError,
	Value,
};
...
fn test_error_deserialize_without_kind_defaults_to_internal() {
	...
	// when deserialising via LyxalValue::from_value (#[lyxal(default)]).
	let mut obj = Object::new();
...
fn test_error_wire_details_with_unknown_inner_kind() {
	...
	// LyxalValue impl handles this by falling back to Internal when the
