use lyxal_types_db::{GeometryKind, Kind, kind};
...
fn test_kind_proc_macro() {
	...
	cases! {
		...
		// Literals
		(true) => Kind::Literal(lyxal_types_db::KindLiteral::Bool(true)),
		(false) => Kind::Literal(lyxal_types_db::KindLiteral::Bool(false)),
		(42) => Kind::Literal(lyxal_types_db::KindLiteral::Integer(42)),
		("hello") => Kind::Literal(lyxal_types_db::KindLiteral::String("hello".to_string())),
		...
		// Literal:: prefix
		(Literal::Bool(true)) => Kind::Literal(lyxal_types_db::KindLiteral::Bool(true)),
		(Literal::Integer(42)) => Kind::Literal(lyxal_types_db::KindLiteral::Integer(42)),

		// Parenthesized expressions (escape hatch)
		((lyxal_types_db::Kind::String)) => Kind::String,
		(array<(lyxal_types_db::Kind::String)>) => Kind::Array(Box::new(Kind::String), None),
		(set<(lyxal_types_db::Kind::Int), 5>) => Kind::Set(Box::new(Kind::Int), Some(5)),

		// Object literals
		({ status: string }) => Kind::Literal(lyxal_types_db::KindLiteral::Object(
			std::collections::BTreeMap::from([
				("status".to_string(), Kind::String)
			])
		)),
		({ status: string, "user-id": int }) => Kind::Literal(lyxal_types_db::KindLiteral::Object(
			std::collections::BTreeMap::from([
				("status".to_string(), Kind::String),
		...
		({ status: Literal::String("OK".to_string()), result: any } | { status: Literal::String("ERR".to_string()), result: string }) => Kind::Either(vec![
			Kind::Literal(lyxal_types_db::KindLiteral::Object(
				std::collections::BTreeMap::from([
					("status".to_string(), Kind::Literal(lyxal_types_db::KindLiteral::String("OK".to_string()))),
					("result".to_string(), Kind::Any)
				])
			)),
			Kind::Literal(lyxal_types_db::KindLiteral::Object(
				std::collections::BTreeMap::from([
					("status".to_string(), Kind::Literal(lyxal_types_db::KindLiteral::String("ERR".to_string()))),
		...
		// Array literals
		([]) => Kind::Literal(lyxal_types_db::KindLiteral::Array(vec![])),
		([string]) => Kind::Literal(lyxal_types_db::KindLiteral::Array(vec![Kind::String])),
		([string, int, bool]) => Kind::Literal(lyxal_types_db::KindLiteral::Array(vec![
			Kind::String,
			Kind::Int,
			Kind::Bool
		])),
