...
use crate::{
	...
	Set, LyxalNone, LyxalNull, Table, Uuid, Value, kind,
};

/// Trait for converting between Lyxal values and Rust types
///
/// This trait provides a type-safe way to convert between Lyxal `Value` types
...
pub trait LyxalValue {
	...
	/// Converts this type into a Lyxal value
	fn into_value(self) -> Value;
	/// Attempts to convert a Lyxal value into this type
	fn from_value(value: Value) -> Result<Self, Error>
	where
		Self: Sized;
}

impl<T: LyxalValue> LyxalValue for Box<T> {
	fn kind_of() -> Kind {
		T::kind_of()
	}
...
impl<T: LyxalValue + Clone> LyxalValue for Arc<T> {
	fn kind_of() -> Kind {
		T::kind_of()
	}
...
impl<T: LyxalValue + Clone> LyxalValue for Rc<T> {
	fn kind_of() -> Kind {
		T::kind_of()
	}
...
macro_rules! impl_lyxal_value {
    ...
    (
        ...
        impl<$($generic: LyxalValue),*> LyxalValue for $type {
            fn kind_of() -> Kind {
                $kind
            }
        ...
        $(
            impl Value {
                ...
                pub fn $is_fnc<$($is_generic: LyxalValue),*>(&self) -> bool {
                    <$type>::is_value(self)
                }
            }
        )?
        ...
        $(
            impl Value {
                ...
                pub fn $from_fnc<$($from_generic: LyxalValue),*>(value: $type) -> Value {
        ...
        $(
            impl Value {
                /// Attempts to convert a Lyxal value into the given type
                ///
                /// Returns `Ok(T)` if the conversion is successful, `Err(Error)` otherwise.
                pub fn $into_fnc<$($into_generic: LyxalValue),*>(self) -> Result<$type, Error> {
		...
		$(
			impl Value {
				...
				pub fn $as_fnc<$($as_generic: LyxalValue),*>(&$self_as) -> Option<&$type> {
					$as_expr
				}
			}
        )?
		...
		$(
			impl Value {
				...
				///
				/// Returns `true` if the value can be converted to the type, `false` otherwise.
				pub fn $is_and_fnc<$($is_and_generic: LyxalValue),*>(&$self_is_and, $is_and_callback: impl FnOnce(&$type) -> bool) -> bool {
					$is_and
				}
			}
		)?
...
impl LyxalValue for Cow<'static, str> {
	fn kind_of() -> Kind {
		kind!(string)
	}
...
impl LyxalValue for &'static str {
	fn kind_of() -> Kind {
		kind!(string)
	}
...
impl_lyxal_value!(
	...
	from_vec<T>(self) => Value::Array(Array(self.into_iter().map(LyxalValue::into_value).collect())),
	into_vec<T>(value) => {
...
macro_rules! impl_tuples {
    ($($n:expr => ($($t:ident),+)),+ $(,)?) => {
        $(
            impl<$($t: LyxalValue),+> LyxalValue for ($($t,)+) {
                fn kind_of() -> Kind {
					kind!([$(($t::kind_of())),+])
                }
...
impl_numeric! {
	int:
		...
		// i64 is implemented with the impl_lyxal_value! macro
...
impl_numeric! {
	float:
		...
		// f64 is implemented with the impl_lyxal_value! macro
}

impl LyxalValue for serde_json::Value {
	fn kind_of() -> Kind {
		kind!(any)
	}
...
macro_rules! impl_slice {
	($($n:expr),+ $(,)?) => {
		$(
			impl<T: LyxalValue> LyxalValue for [T; $n] {
				fn kind_of() -> Kind {
...
impl LyxalValue for http::HeaderMap {
	fn kind_of() -> Kind {
		kind!(object)
	}
...
impl LyxalValue for http::StatusCode {
	fn kind_of() -> Kind {
		kind!(number)
	}
...
impl<T: LyxalValue> LyxalValue for LinkedList<T> {
	fn kind_of() -> Kind {
	...
	fn into_value(self) -> Value {
		Value::Array(Array(self.into_iter().map(LyxalValue::into_value).collect()))
	}
...
impl<T: LyxalValue + Hash + Eq> LyxalValue for HashSet<T> {
	fn kind_of() -> Kind {
		kind!(array<(T::kind_of())>)
	}
