...
This crate provides the shared public value type system for Lyxal. It serves as a foundational layer that defines all the data types that can be stored and manipulated in Lyxal.

## Purpose

The `lyxal_types_db` crate acts as a shared public value type system that:

- **Provides type safety**: Offers strongly-typed representations of all Lyxal data types
- **Enables serialization**: Supports serialization/deserialization of values
- **Facilitates type conversion**: Provides traits and methods for converting between Rust types and Lyxal values

## Core Types

### Value Types

The main `Value` enum represents all possible data types in Lyxal:

```rust
use lyxal_types_db::{Array, Datetime, Number, Object, Value};
...
The `LyxalValue` trait provides type-safe conversion between Rust types and Lyxal values:

```rust
use lyxal_types_db::{LyxalValue, Value};

// Convert from Rust type to Lyxal value
...
// Convert from Lyxal value to Rust type
...
```rust
use lyxal_types_db::{Geometry, Value};
use geo::Point;
...
Type-safe representation of Lyxal record identifiers:

```rust
use lyxal_types_db::{RecordId, RecordIdKey, Value};
...
```rust
use lyxal_types_db::{Value, Number, Array, Object};
use std::collections::BTreeMap;
...
This library provides two macros to easily create object and array values. All values you pass to the macro must implement the `LyxalValue` trait.

```rust
use lyxal_types_db::{object, array};
...
### Deriving the `LyxalValue` trait

#### Requirements

- All fields must implement the `LyxalValue` trait

```rust
use lyxal_types_db::LyxalValue;
...
```rust
use lyxal_types_db::{Value, LyxalValue};
...
- `lyxal_types_db_derive`: For deriving the `LyxalValue` trait
- `lyxal_protocol`: For the Lyxal protocol
...
This crate is part of Lyxal and follows the same licensing terms.
