...
use crate::data::{DataRef, MmapHandle};
...
impl<W: Write> TableWriter<W> {
	...
	pub(crate) fn finish(mut self) -> Result<usize> {
		...
		if let Some(fblock) = self.filter_block.take() {
			...
			if !fblock_data.is_empty() {
				let fblock_handle = self.write_compressed_block(
					DataRef::from_heap(fblock_data.into()),
	...
	fn write_compressed_block(
		...
		let compressed_block = compress_block(block_data.clone(), compression_type)?;
...
pub(crate) fn write_block_at_offset<W: Write>(
	...
	let bytes_to_write = block.len() + BLOCK_CKSUM_LEN + BLOCK_COMPRESS_LEN;
	
	writer.write_all(&block)?;
	writer.write_all(&[compression_type as u8; BLOCK_COMPRESS_LEN])?;
	writer.write_fixedint(mask(cksum.finalize()))?;

	crate::metrics::EngineMetrics::get().sstable_bytes_written.fetch_add(bytes_to_write as u64, std::sync::atomic::Ordering::Relaxed);

	let handle = BlockHandle::new(offset, block.len());
	let new_offset = offset + bytes_to_write;
...
pub(crate) fn compress_block(
	...
	match compression {
		CompressionType::SnappyCompression => {
			...
			match enc.compress(raw_block.as_slice(), buffer.as_mut_slice()) {
			...
			Ok(DataRef::from_heap(buffer.into()))
		}
		CompressionType::ZstdCompression => {
			let start = std::time::Instant::now();
			// Level 3 is the default for ZSTD, a good balance of speed/ratio
			let compressed = zstd::bulk::compress(raw_block.as_slice(), 3)
			...
			Ok(DataRef::from_heap(compressed.into()))
		}
...
pub(crate) fn decompress_block(
	...
	match compression {
		CompressionType::SnappyCompression => {
			...
			let res = dec.decompress_vec(compressed_block).map_err(|e| Error::Decompression(e.to_string()));
		...
		CompressionType::ZstdCompression => {
			let start = std::time::Instant::now();
			// We use a generous upper bound for decompression or let zstd handle it.
			// Standard SSTable blocks are usually around 4KB-64KB.
			let res = zstd::bulk::decompress(compressed_block, 1024 * 1024) // 1MB limit for safety
...
/// Reads a block of data as a DataRef, using mmap if available.
fn read_data_ref(
	f: Arc<dyn File>,
	#[allow(unused_variables)] mmap: Option<&Arc<MmapHandle>>,
	location: &BlockHandle,
) -> Result<DataRef> {
	#[cfg(feature = "mmap_read")]
	if let Some(handle) = mmap {
...
pub(crate) fn read_table_block(
	...
	let buf = read_data_ref(Arc::clone(&f), mmap, location)?;
	let compress_bytes = read_bytes(
		Arc::clone(&f),
		&BlockHandle::new(location.offset() + location.size(), BLOCK_COMPRESS_LEN),
	)?;
	let cksum_bytes = read_bytes(
		Arc::clone(&f),
		&BlockHandle::new(
	...
	let compress_type = compress_bytes[0];
	let cksum = unmask(u32::decode_fixed(&cksum_bytes).ok_or(SSTableError::CorruptedBlockHandle)?);

	if !verify_table_block(buf.as_slice(), compress_type, cksum) {
		return Err(Error::from(SSTableError::ChecksumVerificationFailed {
			block_offset: location.offset() as u64,
		}));
	}

	let compression = CompressionType::try_from(compress_type)?;
	let block_data = if compression == CompressionType::None {
		buf
	} else {
		let decompressed = decompress_block(buf.as_slice(), compression)?;
		DataRef::from_heap(decompressed.into())
	};

	Ok(Block::new(block_data, comparator))
}
...
pub(crate) struct Table {
	...
	#[cfg(feature = "mmap_read")]
	pub(crate) mmap: Option<Arc<MmapHandle>>,
}
...
impl Table {
	pub(crate) fn new(
		...
		#[cfg(feature = "mmap_read")]
		let mmap = {
			// Only attempt mmap for SysFile. 
			// We need the actual file path for deferred delete.
			file.map_read(_path).ok()
		};
	...
	fn read_block(&self, location: &BlockHandle) -> Result<Arc<Block>> {
		...
		crate::metrics::EngineMetrics::get().sstable_bytes_read.fetch_add(location.size() as u64, std::sync::atomic::Ordering::Relaxed);
		crate::metrics::EngineMetrics::get().record_read_latency(start.elapsed().as_nanos() as u64);
...
mod tests {
	...
	fn test_table_seek() {
		...
		let table = Arc::new(Table::new(1, opts, wrap_buffer(src), size as u64, None).unwrap());
	...
	fn test_many_items() {
		...
		let table = Table::new(1, opts, wrap_buffer(buffer), size as u64, None).unwrap();
	...
	fn test_iter_items() {
		...
		// Create a table reader
		let table = Arc::new(Table::new(1, opts, wrap_buffer(buffer), size as u64, None).unwrap());
	...
	fn test_table_key_range_persistence() {
		...
		// Load the table back
		let table = Table::new(1, opts, wrap_buffer(src), size as u64, None).unwrap();
	...
	fn test_get_nonexistent_key_returns_none() {
		...
		let size = writer.finish().unwrap();
		let table = Table::new(0, opts, wrap_buffer(buffer), size as u64, None).unwrap();
	...
	fn test_table_iterator_seek_nonexistent_key() {
		...
		let size = writer.finish().unwrap();
		let table = Arc::new(Table::new(0, opts, wrap_buffer(buffer), size as u64, None).unwrap());
	...
	fn test_iterator_trait_prev_multi_partition_bug() {
		...
		let size = writer.finish().unwrap();
		let table_arc = Arc::new(Table::new(0, opts, wrap_buffer(buffer), size as u64, None).unwrap());
	...
	fn test_range_at_exact_block_boundary() {
		...
		let (src, size) = build_table(data);
		let table = Arc::new(Table::new(1, opts.clone(), wrap_buffer(src), size as u64, None).unwrap());
