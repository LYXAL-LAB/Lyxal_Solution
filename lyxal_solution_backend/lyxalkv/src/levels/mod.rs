...
impl LevelManifest {
	...
	fn load_table(table_id: u64, opts: Arc<Options>) -> Result<Arc<Table>> {
		...
		let table = Arc::new(Table::new(table_id, opts, file, file_size, Some(table_file_path))?);
		Ok(table)
	}
...
pub(crate) fn replace_file_content<P: AsRef<Path>>(
	...
	// Create and write to the temporary file
	{
		let mut temp_file = {
			let mut attempts = 0;
			loop {
				match SysFile::create(&temp_path) {
					Ok(f) => break f,
					Err(e) => {
						attempts += 1;
						if attempts > 20 {
	...
	if let Some(permissions) = original_permissions {
		let mut attempts = 0;
		loop {
			match std::fs::set_permissions(&temp_path, permissions.clone()) {
				Ok(_) => {
					break;
				}
				Err(e) => {
					...
					if attempts > 20 {
						...
						let _ = crate::remove_file(&temp_path);
						return Err(e);
					}
					std::thread::sleep(std::time::Duration::from_millis(20 * attempts as u64));
				}
				...
				Err(e) => {
					...
					if attempts > 20 {
						...
						match std::fs::copy(&temp_path, target_path) {
							...
							Err(_) => {
								// Return original rename error
								let _ = crate::remove_file(&temp_path);
								return Err(e);
							}
						}
					}
					// Wait a bit before retrying (exponential backoff or constant)
					std::thread::sleep(std::time::Duration::from_millis(20 * attempts as u64));
				}
			}
		}
	...
	let updated_file = {
		let mut attempts = 0;
		loop {
			match std::fs::OpenOptions::new().write(true).open(target_path) {
				Ok(f) => {
					break f;
				}
				Err(e) => {
	...
	updated_file.sync_all().map_err(|e| e)?;
