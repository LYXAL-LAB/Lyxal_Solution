...
fn test_compression_size_reduction() {
	...
	let table_compressed = Arc::new(
		Table::new(2, opts_compressed, wrap_buffer(buffer_compressed), size_compressed as u64, None)
			.unwrap(),
	);
...
fn test_compression_checksum_verification() {
	...
	let table =
		Arc::new(Table::new(1, opts.clone(), wrap_buffer(buffer.clone()), size as u64, None).unwrap());
	let mut iter = table.iter(false, None);
	iter.seek_to_first().unwrap();
	...
	let corrupted_table_result = Table::new(2, opts, wrap_buffer(buffer), size as u64, None);
...
fn test_table_writer_with_level_compression() {
	...
	let table = Arc::new(Table::new(1, opts, wrap_buffer(buffer), buffer_len, None).unwrap());
