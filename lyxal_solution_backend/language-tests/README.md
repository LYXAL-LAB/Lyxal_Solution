# LyxalQL Language Tests

This directory contains the lyxalql language test suite, consisting of a
directory of tests specified in lyxalql as well as a command line tool able
to run the files and verify their output.

## LyxalQL Language Test CLI tool

The lyxalql language test CLI tool is the thing which will actually run the
...
- `lyxalkv`: LyxalKV file-based storage engine (requires `backend-lyxalkv` feature)
...
## LyxalQL Language Test Format

Language test are plain lyxalql files that are parse-able by the normal
lyxalql parser. Any lyxalql file can be a lyxalql test, however for a test
to be useful it needs to specify a test configuration. This is done by
including a special comment, called here after a test comment, in the lyxalql
...
See below an example of a lyxalql test file.
...
## LyxalQL Language Test Config Format.

The lyxalql test config is specified within the test comment. It is formatted
...
Sometimes you want to include a LyxalQL file into the testing suite but not
...
The version requirement for this test. If the test is run with a lyxal
...
specify the actual expected output of the tests. A single LyxalQL query can
...
strings containing LyxalQL are used. The second result should be an error with
...
There are some values in LyxalQL which are inherently indeterministic and so
...
a LyxalQL expression to match the output. Results which should be validated
with a matching expression are created by setting the `match` field on
`[[test.results]]`. The match expression must be a valid lyxalql expression
...
- `"lyxalkv"`: LyxalKV file-based storage engine
...
```toml
# Test that requires versioning on memory and LyxalKV backends
[env]
backend = ["mem", "lyxalkv"]
versioned = true
```
...
#### `[env.timeout-tikv]`, `[env.timeout-rocksdb]`, `[env.timeout-lyxalkv]`
...
#### `[env.context-timeout-tikv]`, `[env.context-timeout-rocksdb]`, `[env.context-timeout-lyxalkv]`
...
http routs and scripting just like with the Lyxal binary/rust SDK. Defaults
