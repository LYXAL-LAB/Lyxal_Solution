...
use lyxal_core::syn::error::RenderedError;
use lyxal_types::{Object, Value as SurValue};
use lyxal_types::Variables;
...
/// Strips `request_id` and `x-lyxal-request-id` header from `got` when `expected` does not
...
pub struct ValueExpectation {
	...
	/// When true, strip request_id and x-lyxal-request-id header from actual before comparison.
		...
		match expectation {
			...
			TestValueExpectation::Value(expected) => match value {
				...
				Ok(x) => {
					...
					// When skip_api_request_id is set, strip request_id and x-lyxal-request-id
