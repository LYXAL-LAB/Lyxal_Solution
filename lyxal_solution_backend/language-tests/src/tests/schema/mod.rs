...
use lyxal_core::dbs::capabilities::{
	ExperimentalTarget, FuncTarget, MethodTarget, NetTarget, RouteTarget,
};
use lyxal_core::syn::parser::ParserSettings;
use lyxal_core::syn::{self};
use lyxal_types::{Object, RecordId, ToSql, Value};
...
pub struct TestEnv {
	...
	pub signup: Option<LyxalObject>,
	pub signin: Option<LyxalObject>,
	...
	pub timeout_lyxalkv: Option<BoolOr<u64>>,
	...
	pub context_timeout_lyxalkv: Option<BoolOr<u64>>,
	...
	/// Valid values: "mem", "rocksdb", "lyxalkv", "tikv"
	#[serde(default)]
...
impl TestEnv {
	...
	pub fn timeout(&self, backend: Option<&str>) -> Option<u64> {
		...
		let override_timeout = match backend {
			...
			Some("lyxalkv") => self.timeout_lyxalkv,
			_ => None,
		};
	...
	pub fn context_timeout(&self, backend: Option<&str>) -> Option<u64> {
		...
		let override_timeout = match backend {
			...
			Some("lyxalkv") => self.context_timeout_lyxalkv,
			_ => None,
		};
...
///
/// Maps to `lyxal_core::dbs::NewPlannerStrategy` but uses shorter
...
impl<'de> Deserialize<'de> for TestExpectation {
		...
		D: de::Deserializer<'de>,
	{
		...
		if v.is_str() {
			LyxalConfigValue::deserialize(v).map_err(to_deser_error).map(TestExpectation::Plain)
...
/// A wrapper around the `Value` type for Lyxal in order to support parsing from toml.
#[derive(Clone, Debug)]
pub struct LyxalConfigValue(pub Value);

impl Serialize for LyxalConfigValue {
...
impl<'de> Deserialize<'de> for LyxalConfigValue {
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
	where
		D: serde::Deserializer<'de>,
	{
		...
		Ok(LyxalConfigValue(v))
	}
}

#[derive(Clone, Debug)]
pub struct LyxalExpr(pub String);

impl Serialize for LyxalExpr {
	fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
	where
		S: serde::Serializer,
	{
...
impl<'de> Deserialize<'de> for LyxalExpr {
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
	where
		D: serde::Deserializer<'de>,
	{
...
#[derive(Clone, Debug)]
pub struct LyxalRecordId(pub RecordId);

impl Serialize for LyxalRecordId {
	fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
	where
		S: serde::Serializer,
	{
...
impl<'de> Deserialize<'de> for LyxalRecordId {
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
	where
		D: serde::Deserializer<'de>,
	{
...
#[derive(Clone, Debug)]
pub struct LyxalObject(pub Object);

impl Serialize for LyxalObject {
	fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
	where
		S: serde::Serializer,
	{
...
impl<'de> Deserialize<'de> for LyxalObject {
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
	where
		D: serde::Deserializer<'de>,
	{
		...
		v.into_object().map(LyxalObject).or_else(|err| {
			Err(<D::Error as serde::de::Error>::custom(format_args!(
