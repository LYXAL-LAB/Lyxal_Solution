...
use lyxal_core::kvs::{Datastore, LockType, TransactionType};
...
impl CreateInfo {
	pub async fn new(backend: Backend) -> Result<Self> {
		...
		let mut dir = temp_dir.join(format!("lyxal_lang_tests_{rand}"));

		while tokio::fs::metadata(&dir).await.is_ok() {
			let rand = xorshift(&mut state);
			dir = temp_dir.join(format!("lyxal_lang_tests_{rand}"));
		}
	...
	pub async fn produce_ds(&self, versioned: bool) -> Result<(Datastore, Option<String>)> {
		...
		let ds = match self.backend {
			...
			Backend::LyxalKv => {
				let p = self.produce_path();
				let ds = if versioned {
					Datastore::new(&format!("lyxalkv://{p}?versioned=true")).await?
				} else {
					Datastore::new(&format!("lyxalkv://{p}")).await?
				};
				path = Some(p);
				ds
			}
