...
use lyxal_core::dbs::capabilities::ExperimentalTarget;
use lyxal_core::env::VERSION;
...
pub async fn run(color: ColorMode, matches: &ArgMatches) -> Result<()> {
	...
	match backend {
		...
		Backend::LyxalKv => bail!("LyxalKV backend feature is not enabled"),
...
async fn run_test_with_dbs(
	...
	if let Some(signup_vars) = config.env.as_ref().and_then(|x| x.signup.as_ref()) {
		if let Err(e) =
			lyxal_core::iam::signup::signup(dbs, &mut session, signup_vars.0.clone().into())
				.await
		{
	...
	if let Some(signin_vars) = config.env.as_ref().and_then(|x| x.signin.as_ref()) {
		if let Err(e) =
			lyxal_core::iam::signin::signin(dbs, &mut session, signin_vars.0.clone().into())
				.await
		{
