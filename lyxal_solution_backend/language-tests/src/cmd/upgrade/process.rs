...
use lyxal_core::rpc::{DbResponse, DbResult};
use lyxal_types::Error as TypesError;
use lyxal_types::Value;
...
impl LyxalProcess {
	pub async fn new(
		...
		tmp_dir: &str,
		port: u16,
	) -> Result<LyxalProcess, anyhow::Error> {
		let mut command = match version {
			DsVersion::Path(x) => {
				let path = Path::new(x).join("target").join("debug").join("lyxal");
				Command::new(path)
			}
			DsVersion::Version(x) => {
				let path = Path::new(".binary_cache").join(format!("lyxal-v{x}"));
				Command::new(path)
			}
		};
		...
		let endpoint = match config.backend {
			...
			UpgradeBackend::LyxalKv => format!("lyxalkv://{tmp_dir}/ds"),
		};
		...
		command
			...
			.env("LYXAL_RUNTIME_WORKER_THREADS", "1")
			.stdin(Stdio::null())
			.stdout(Stdio::piped())
			.stderr(Stdio::piped())
			.kill_on_drop(true);
	...
	pub async fn open_connection(&mut self) -> anyhow::Result<LyxalConnection> {
		let port = self.port;
		self.assert_running_while(async {
			let request = tokio_tungstenite::tungstenite::handshake::client::Request::builder()
				.uri(format!("ws://127.0.0.1:{}/rpc", port))
				.header(SEC_WEBSOCKET_PROTOCOL, lyxal_core::api::format::FLATBUFFERS)
...
pub struct LyxalConnection {
	id: i64,
	socket: WebSocketStream<MaybeTlsStream<TcpStream>>,
}

impl LyxalConnection {
	pub async fn request(&mut self, mut object: ProxyObject) -> Result<DbResponse, TypesError> {
		...
		let message = lyxal_core::rpc::format::flatbuffers::encode(&value)
		...
		loop {
			...
			let message = message.map_err(|e| {
				TypesError::internal(format!("Lyxal connection error: {e}"))
			})?;
			...
			let response: DbResponse = lyxal_core::rpc::format::flatbuffers::decode(&data)
				.map_err(|e| {
			...
			if response.id != Some(Value::Number(lyxal_types::Number::Int(id))) {
				continue;
			}
