...
pub async fn prepare_version(version: Version, download_permission: bool) -> anyhow::Result<()> {
	if Path::new(".binary_cache").join(format!("lyxal-v{version}")).exists() {
		return Ok(());
	}
	...
	bail!(
		"Could not run wget or curl, please install either curl or wget to facilitate downloading lyxal binaries"
	)
}
...
cfg_if::cfg_if! {
	if #[cfg(target_os = "windows")] {
		fn download_path(version: &Version) -> String{
			format!(".binary_cache/lyxal-v{version}.exe")
		}
	}else{
		fn download_path(version: &Version) -> String{
			format!(".binary_cache/lyxal-v{version}.tgz")
		}
	}
}
...
fn binary_url(version: &Version) -> String {
	format!(
		"https://github.com/lyxal/lyxal/releases/download/v{version}/lyxal-v{version}.{}",
		platform_name()
	)
}
...
async fn unzip(version: Version) -> anyhow::Result<()> {
	let mut command = Command::new("tar")
		...
		.args(["--transform", &format!("s/lyxal/lyxal-v{version}/g")])
		.arg("-xvf")
...
pub async fn retrieve_version_from_path(path: &str) -> anyhow::Result<Version> {
	let output = Command::new("cargo")
		...
		.await
		.with_context(|| {
			format!("failed to 'cargo metadata', could not find lyxal version for '{path}'")
		})?;
	...
	let version = json
		...
		.and_then(|x| {
			x.iter().find(|x| x.get("name").and_then(|x| x.as_str()) == Some("lyxal"))
		})
		.and_then(|x| x.get("version"))
		.and_then(|x| x.as_str())
		.ok_or_else(|| anyhow::anyhow!("Could not find 'lyxal' package in rust workspace"))?;
