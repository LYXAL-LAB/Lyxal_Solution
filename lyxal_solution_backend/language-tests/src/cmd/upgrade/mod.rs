...
pub async fn run(color: ColorMode, matches: &ArgMatches) -> Result<()> {
	...
	match config.backend {
		...
		#[cfg(feature = "backend-lyxalkv")]
		UpgradeBackend::LyxalKv => {}
		#[cfg(not(feature = "backend-lyxalkv"))]
		UpgradeBackend::LyxalKv => bail!("LyxalKV backend feature is not enabled"),
	}

	if UpgradeBackend::LyxalKv == config.backend {
		if let Some(DsVersion::Version(v)) =
			all_versions.iter().find(|x| **x < DsVersion::Version(Version::new(2, 0, 0)))
		{
			bail!(
				"Cannot run with backend lyxalkv and version {v}, lyxalkv was not yet available on this version"
			)
		}
	}
	...
	println!("Preparing used versions of lyxal");
	...
	let temp_dir = TempDir::new("lyxal_upgrade_tests")
		.await
	...
	let mut session = lyxal_core::dbs::Session::default();
...
async fn run_task_inner(
	...
	// run imports
	let mut process = process::LyxalProcess::new(&config, &task.from, dir, task.port).await?;
	...
	let result = loop {
		let process = process::LyxalProcess::new(&config, &task.to, dir, task.port).await?;
