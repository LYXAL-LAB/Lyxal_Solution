...
impl ValueEnum for Backend {
	fn value_variants<'a>() -> &'a [Self] {
		&[Backend::Memory, Backend::RocksDb, Backend::LyxalKv, Backend::TikV]
	}
	...
	fn to_possible_value(&self) -> Option<PossibleValue> {
		match self {
			...
			Backend::LyxalKv => Some(PossibleValue::new("lyxalkv")),
...
impl Display for Backend {
	fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::LyxalKv => f.write_str("lyxalkv"),
...
impl ValueEnum for UpgradeBackend {
	fn value_variants<'a>() -> &'a [Self] {
		&[UpgradeBackend::RocksDb, UpgradeBackend::LyxalKv]
	}

	fn to_possible_value(&self) -> Option<PossibleValue> {
		match self {
			UpgradeBackend::RocksDb => Some(PossibleValue::new("rocksdb")),
			UpgradeBackend::LyxalKv => Some(PossibleValue::new("lyxalkv")),
		}
	}
}
		...
		.subcommand(
			...
			.arg(
				arg!(--backend <BACKEND> "Specify the storage backend to use for the upgrade test")
					.value_parser(EnumValueParser::<UpgradeBackend>::new()).default_value("lyxalkv")
			)
			.arg(
				arg!(-f --from <VERSIONS> "The version to upgrade from. This can be either a version number or a path to the lyxal codebase.").required(true).value_delimiter(',').value_parser(DsVersion::from_str)
			)
			.arg(
				arg!(-t --to <VERSIONS> "The version to upgrade to. This can be either a version number or a path to the lyxal codebase.").required(true).value_delimiter(',').value_parser(DsVersion::from_str)
			)
			.arg(
