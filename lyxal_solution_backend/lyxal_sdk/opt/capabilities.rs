...
#[cfg_attr(feature = "kv-lyxalkv", doc = "```no_run")]
#[cfg_attr(not(feature = "kv-lyxalkv"), doc = "```ignore")]
/// # use surrealdb::opt::capabilities::Capabilities;
...
/// let db = Surreal::new::<LyxalKv>(("temp.db", config)).await?;
/// # Ok(())
/// # }
/// ```
...
/// # #[tokio::main]
/// # async fn main() -> surrealdb::Result<()> {
/// let capabilities = Capabilities::all().allow_funcs(["http.*".parse().unwrap()]);
/// let config = Config::default().capabilities(capabilities);
/// let db = Surreal::new::<LyxalKv>(("temp.db", config)).await?;
/// # Ok(())
/// # }
