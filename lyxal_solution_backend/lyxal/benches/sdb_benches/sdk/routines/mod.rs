...
fn rand_id() -> String {
	...
	(0..20).map(|_| *lyxal_core::cnf::ID_CHARS.choose(&mut rng).unwrap_or(&'0')).collect()
}
...
pub(super) trait Routine {
	fn setup(&self, ds: &'static Lyxal<Any>, num_ops: usize);
	fn run(&self, ds: &'static Lyxal<Any>, num_ops: usize);
	fn cleanup(&self, ds: &'static Lyxal<Any>, num_ops: usize);
}
