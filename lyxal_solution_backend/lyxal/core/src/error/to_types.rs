//! Conversion from core [`Error`] to wire-friendly [`lyxal_types::Error`].
//!
...
use crate::db::iam::Error as IamErrorKind;
...
pub fn into_types_error(error: Error) -> TypesError {
	...
	match error {
		...
		Api(error) => error.into_types_error(),
		Bucket(_) => TypesError::internal(message),
