...
use crate::config::cnf::LYXAL_USER_AGENT;
...
impl HttpClientManager {
	...
	async fn get_or_create_client(
		&self,
		capabilities: Arc<crate::db::dbs::Capabilities>,
		redirect_checker: Option<
			impl Fn(&Url) -> Result<(), crate::error::Error> + Send + Sync + 'static,
		>,
	) -> Result<Arc<Client>> {
		...
		match self.clients.entry(capabilities_hash) {
			...
			dashmap::mapref::entry::Entry::Vacant(entry) => {
				...
				let mut builder = Client::builder()
					.pool_idle_timeout(Duration::from_secs(*crate::config::cnf::HTTP_IDLE_TIMEOUT_SECS))
					.pool_max_idle_per_host(*crate::config::cnf::MAX_HTTP_IDLE_CONNECTIONS_PER_HOST)
					.connect_timeout(Duration::from_secs(*crate::config::cnf::HTTP_CONNECT_TIMEOUT_SECS))
					.tcp_keepalive(Some(Duration::from_secs(60)))
					.http2_keep_alive_interval(Some(Duration::from_secs(30)))
					.http2_keep_alive_timeout(Duration::from_secs(10));

				if let Some(checker) = redirect_checker {
					let count = *crate::config::cnf::MAX_HTTP_REDIRECTS;
				...
				builder = builder.dns_resolver(Arc::new(
					crate::function::http::resolver::FilteringResolver::from_capabilities(capabilities),
				));
	...
	fn hash_capabilities(&self, capabilities: &crate::db::dbs::Capabilities) -> u64 {
...
async fn request(
	...
	let body = match body {
		Some(v) => Some(crate::db::val::convert_value_to_public_value(v)?),
		None => None,
	};
	...
	let cli = {
		...
		let redirect_checker = move |url: &Url| -> Result<(), crate::error::Error> {
			...
			let target = NetTarget::from_str(url.host_str().unwrap_or(""))
				.map_err(|e| crate::error::Error::InvalidUrl(format!("Invalid host: {}", e)))?;

			if !capabilities_clone.matches_any_allow_net(&target)
				|| capabilities_clone.matches_any_deny_net(&target)
			{
				return Err(crate::error::Error::NetTargetNotAllowed(url.to_string()));
			}
	...
	if cfg!(not(target_family = "wasm")) {
		req = req.header(reqwest::header::USER_AGENT, &*LYXAL_USER_AGENT);
	}
...
pub(crate) async fn head(ctx: &FrozenContext, uri: String, opts: impl Into<Object>) -> Result<Value> {
	request(ctx, Method::HEAD, uri, None, opts).await
}

pub(crate) async fn get(ctx: &FrozenContext, uri: String, opts: impl Into<Object>) -> Result<Value> {
...
pub(crate) async fn delete(ctx: &FrozenContext, uri: String, opts: impl Into<Object>) -> Result<Value> {
