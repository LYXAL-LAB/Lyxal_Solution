...
use crate::db::val::value::{Cast as CastTrait, Coerce};
...
impl Arity {
	...
	/// arity.
	pub(crate) fn combine(self, other: Self) -> Arity {
		Arity {
...
impl Args {
	pub(crate) fn from_vec(args: Vec<Value>) -> Self {
		Args {
			next: None,
			count: 1,
			iter: args.into_iter(),
	...
	pub(crate) fn next(&mut self) -> Option<(usize, Value)> {
...
impl<T: LyxalValue> FromArg for FromPublic<T> {
	fn arity() -> Arity {
		Arity {
			lower: 1,
			upper: Some(1),
...
#[repr(transparent)]
pub(crate) struct Any(pub(crate) Vec<Value>);
