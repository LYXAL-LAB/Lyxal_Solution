...
use crate::config::cnf::ID_CHARS;
use crate::error::Error;
use crate::db::val::{Datetime, Duration, Number, Uuid, Value};

pub(crate) fn rand(_: ()) -> Result<Value> {
	Ok(rand::random::<f64>().into())
}

pub(crate) fn bool(_: ()) -> Result<Value> {
	Ok(rand::random::<bool>().into())
}

pub(crate) fn r#enum(Any(mut args): Any) -> Result<Value> {
	Ok(match args.len() {
		0 => Value::None,
...
// upperbound.
pub(crate) fn float((NoneOrRange(range),): (NoneOrRange<f64>,)) -> Result<Value> {
...
pub(crate) fn id((Optional(arg1), Optional(arg2)): (Optional<i64>, Optional<i64>)) -> Result<Value> {
...
pub(crate) fn int((NoneOrRange(range),): (NoneOrRange<i64>,)) -> Result<Value> {
...
pub(crate) fn string((Optional(arg1), Optional(arg2)): (Optional<i64>, Optional<i64>)) -> Result<Value> {
	// Set a reasonable maximum length
...
pub(crate) fn duration((dur1, dur2): (Duration, Duration)) -> Result<Value> {
...
pub(crate) fn time((NoneOrRange(range),): (NoneOrRange<Value>,)) -> Result<Value> {
...
pub(crate) fn ulid((Optional(timestamp),): (Optional<Datetime>,)) -> Result<Value> {
...
pub(crate) fn uuid((Optional(timestamp),): (Optional<Datetime>,)) -> Result<Value> {
...
pub mod uuid {
	...
	pub(crate) fn v4(_: ()) -> Result<Value> {
		Ok(Uuid::new_v4().into())
	}

	pub(crate) fn v7((Optional(timestamp),): (Optional<Datetime>,)) -> Result<Value> {
		let uuid = match timestamp {
			Some(timestamp) => {
				#[cfg(target_family = "wasm")]
				anyhow::ensure!(
					timestamp.0 >= chrono::DateTime::UNIX_EPOCH,
					crate::error::Error::InvalidFunctionArguments {
						name: String::from("rand::ulid"),
						message: format!(
							"To generate a ULID from a datetime, it must be a time beyond UNIX epoch."
						),
