...
pub(crate) async fn run(
	...
	if name.eq("sleep")
		...
		|| name.eq("file::list")
		|| name.eq("crate::function::record::exists")
		|| name.eq("crate::db::key::record::is_edge")
		|| name.eq("set::all")
		|| name.eq("set::any")
		|| name.eq("set::filter")
		|| name.eq("set::find")
                    ...
                    $function_name => {
                        $(
                            if !$ctx.get_capabilities().allows_experimental(&ExperimentalTarget::$exp_target) {
                                return Err(::anyhow::Error::new($crate::error::Error::InvalidFunction {
...
pub(crate) fn synchronous(
	...
	dispatch!(
		...
		"duration::from::string" => duration::from::string,
		...
		//
		"list::dedupe" => list::dedupe,
		"list::aggregate" => list::aggregate,
		"list::split_out" => list::split_out,
		"list::diff" => list::diff,
		//
		...
		//
		"meta::id" => crate::function::record::id,
		"meta::tb" => crate::function::record::tb,
		//
		"not" => not::not,
		//
		...
		//
		"parse::datetime::from_str" => parse::datetime::from_str,
		"parse::datetime::iso" => parse::datetime::iso,
		...
		"rand::uuid" => rand::uuid,
		//
		"crate::function::record::id" => crate::function::record::id,
		"crate::db::key::record::table" => crate::function::record::tb,
		"crate::function::record::tb" => crate::function::record::tb,
		//
		...
		//
		"time::add" => time::add,
		"time::add_business_days" => time::add_business_days,
		"time::ceil" => time::ceil,
		"time::day" => time::day,
		"time::diff" => time::diff,
		...
		"time::sub" => time::sub,
		"time::with_timezone" => time::with_timezone,
		//
...
pub(crate) async fn asynchronous(
	...
	fn cpu_intensive<R: Send + 'static>(
		...
		|| Box::pin(crate::db::exe::spawn(function))
	}
	...
	dispatch!(
		...
		//
		// list:: functions are registered in the synchronous dispatch
		//
		"bridge::call" => bridge::call((stk, ctx, opt, doc)).await,
		"bridge::list" => bridge::list((ctx, opt)).await,
		"bridge::info" => bridge::info((ctx, opt)).await,
		"bridge::health" => bridge::health(ctx).await,
		"bridge::batch" => bridge::batch((stk, ctx, opt, doc)).await,
		//
		"crate::function::record::exists" => crate::function::record::exists((stk, ctx, Some(opt), doc)).await,
		"crate::db::key::record::is_edge" => crate::function::record::is::edge((stk, ctx, Some(opt), doc)).await,
		//
	...
	match value {
		...
		Value::RecordId(t) => {
			...
			dispatch!(
				...
				//
				"exists" => crate::function::record::exists((stk, ctx, Some(opt), doc)).await,
				"is_edge" => crate::function::record::is::edge((stk, ctx, Some(opt), doc)).await,
				"id" => crate::function::record::id,
				"table" => crate::function::record::tb,
				"tb" => crate::function::record::tb,
...
mod tests {
	...
	use crate::db::sql::{Expr, Function};
	...
	async fn implementations_are_present() {
		...
		let re = Regex::new(r"(?ms)pub(crate) async fn idiom\(.*}").unwrap();
		...
		for line in fnc_no_idiom.lines() {
			...
			let res =
				crate::db::syn::expr_with_capabilities(&format!("{}()", name), &Capabilities::all());
			...
			#[cfg(all(feature = "scripting", feature = "kv-mem"))]
			{
				...
				let sql =
					format!("RETURN function() {{ return typeof lyxal.functions.{name}; }}");
				let dbs = crate::kvs::Datastore::new("memory")
					.await
					.unwrap()
					.with_capabilities(Capabilities::all());
				let ses = crate::db::dbs::Session::owner().with_ns("test").with_db("test");
		...
		if !problems.is_empty() {
			...
			panic!(
				"ensure functions can be parsed in core/src/sql/function.rs and are exported to JS in core/src/fnc/script/modules/lyxal"
			);
		}
	}
}
