...
pub(crate) async fn head(_: &FrozenContext, (_, _): (Value, Optional<Value>)) -> Result<Value> {
	anyhow::bail!(Error::HttpDisabled)
}

#[cfg(not(feature = "http"))]
pub(crate) async fn get(_: &FrozenContext, (_, _): (Value, Optional<Value>)) -> Result<Value> {
	anyhow::bail!(Error::HttpDisabled)
}
...
#[cfg(not(feature = "http"))]
pub(crate) async fn delete(_: &FrozenContext, (_, _): (Value, Optional<Value>)) -> Result<Value> {
	anyhow::bail!(Error::HttpDisabled)
}
...
fn try_as_uri(fn_name: &str, value: Value) -> Result<String> {
	match value {
		// Pre-check URI.
		Value::String(uri) if crate::function::util::http::uri_is_valid(&uri) => Ok(uri),
...
fn try_as_opts(
	...
	error_message: &str,
	value: Option<Value>,
) -> Result<Option<crate::db::val::Object>> {
	match value {
...
pub(crate) async fn head(
	...
	crate::function::util::http::head(ctx, uri, opts).await
}
...
pub(crate) async fn get(
	...
	crate::function::util::http::get(ctx, uri, opts).await
}
...
pub(crate) async fn put(
	...
	crate::function::util::http::put(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub(crate) async fn post(
	...
	crate::function::util::http::post(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub(crate) async fn patch(
	...
	crate::function::util::http::patch(ctx, uri, body.unwrap_or(Value::Null), opts).await
}
...
pub(crate) async fn delete(
	...
	crate::function::util::http::delete(ctx, uri, opts).await
}
...
pub mod resolver {
	...
	impl FilteringResolver {
		pub(crate) fn from_capabilities(cap: Arc<Capabilities>) -> Self {
			FilteringResolver {
				cap,
			}
		}
	...
	impl Resolve for FilteringResolver {
		fn resolve(&self, name: Name) -> Resolving {
			...
			Box::pin(async move {
				...
				if !name_is_allowed {
					return Err(Box::new(crate::error::Error::NetTargetNotAllowed(
				...
				{
					return Err(
						Box::new(crate::error::Error::NetTargetNotAllowed(denied.to_string()))
							as Box<dyn Error + Send + Sync>,
					);
				}
