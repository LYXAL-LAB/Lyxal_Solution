pub mod base64 {
	...
	pub(crate) fn encode((arg, Optional(padded)): (Bytes, Optional<bool>)) -> Result<Value> {
	...
	pub(crate) fn decode((arg,): (String,)) -> Result<Value> {
...
pub mod cbor {
	...
	pub(crate) fn encode((arg,): (Value,)) -> Result<Value> {
		// Convert internal value to public, encode, then convert back
		let public_val = crate::db::val::convert_value_to_public_value(arg)?;
	...
	pub(crate) fn decode((arg,): (Bytes,)) -> Result<Value> {
		let public_val =
		...
		Ok(crate::db::sql::expression::convert_public_value_to_internal(public_val))
	}
}
