...
use crate::config::cnf::{GENERATION_ALLOCATION_LIMIT, STRING_SIMILARITY_LIMIT};
use crate::error::Error;
use crate::function::util::string;
use crate::db::val::range::TypedRange;
use crate::db::val::{Regex, Value};
...
pub(crate) fn capitalize((string,): (String,)) -> Result<Value> {
	if string.is_empty() {
...
pub(crate) fn concat(Any(args): Any) -> Result<Value> {
...
pub(crate) fn contains((val, check): (String, String)) -> Result<Value> {
	Ok(val.contains(&check).into())
}

pub(crate) fn ends_with((val, chr): (String, String)) -> Result<Value> {
	Ok(val.ends_with(&chr).into())
}

pub(crate) fn join(Any(args): Any) -> Result<Value> {
...
pub(crate) fn len((string,): (String,)) -> Result<Value> {
	let num = string.chars().count() as i64;
	Ok(num.into())
}

pub(crate) fn lowercase((string,): (String,)) -> Result<Value> {
	Ok(string.to_lowercase().into())
}

pub(crate) fn repeat((val, num): (String, i64)) -> Result<Value> {
...
pub(crate) fn matches((val, Cast(regex)): (String, Cast<Regex>)) -> Result<Value> {
	Ok(regex.0.is_match(&val).into())
}

pub(crate) fn replace((val, search, replace): (String, Value, String)) -> Result<Value> {
	match search {
		Value::String(search) => {
...
pub(crate) fn reverse((string,): (String,)) -> Result<Value> {
...
pub(crate) fn slug((string,): (String,)) -> Result<Value> {
	Ok(string::slug::slug(string).into())
}

pub(crate) fn split((val, chr): (String, String)) -> Result<Value> {
	Ok(val.split(&chr).map(|x| Value::from(x.to_owned())).collect::<Vec<_>>().into())
}

pub(crate) fn starts_with((val, chr): (String, String)) -> Result<Value> {
	Ok(val.starts_with(&chr).into())
}

pub(crate) fn trim((string,): (String,)) -> Result<Value> {
	Ok(string.trim().into())
}

pub(crate) fn uppercase((string,): (String,)) -> Result<Value> {
	Ok(string.to_uppercase().into())
}

pub(crate) fn words((string,): (String,)) -> Result<Value> {
...
pub mod distance {
	...
	pub(crate) fn damerau_levenshtein((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn normalized_damerau_levenshtein((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn hamming((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn levenshtein((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn normalized_levenshtein((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn osa_distance((a, b): (String, String)) -> Result<Value> {
...
pub mod html {
	...
	pub(crate) fn encode((arg,): (String,)) -> Result<Value> {
		Ok(ammonia::clean_text(&arg).into())
	}

	pub(crate) fn sanitize((arg,): (String,)) -> Result<Value> {
...
pub mod is {
	...
	pub(crate) fn alphanum((arg,): (String,)) -> Result<Value> {
		if arg.is_empty() {
			Ok(Value::Bool(false))
		} else {
	...
	pub(crate) fn alpha((arg,): (String,)) -> Result<Value> {
		if arg.is_empty() {
			Ok(Value::Bool(false))
		} else {
	...
	pub(crate) fn ascii((arg,): (String,)) -> Result<Value> {
		if arg.is_empty() {
			Ok(Value::Bool(false))
		} else {
	...
	pub(crate) fn datetime((arg, Optional(fmt)): (String, Optional<String>)) -> Result<Value> {
		Ok(match fmt {
	...
	pub(crate) fn domain((arg,): (String,)) -> Result<Value> {
		Ok(addr::parse_domain_name(arg.as_str()).is_ok().into())
	}

	pub(crate) fn email((arg,): (String,)) -> Result<Value> {
		Ok(addr::parse_email_address(arg.as_str()).is_ok().into())
	}

	pub(crate) fn hexadecimal((arg,): (String,)) -> Result<Value> {
		if arg.is_empty() {
			Ok(Value::Bool(false))
		} else {
	...
	pub(crate) fn ip((arg,): (String,)) -> Result<Value> {
		Ok(arg.parse::<IpAddr>().is_ok().into())
	}

	pub(crate) fn ipv4((arg,): (String,)) -> Result<Value> {
		Ok(arg.parse::<Ipv4Addr>().is_ok().into())
	}

	pub(crate) fn ipv6((arg,): (String,)) -> Result<Value> {
		Ok(arg.parse::<Ipv6Addr>().is_ok().into())
	}

	pub(crate) fn latitude((arg,): (String,)) -> Result<Value> {
		Ok(LATITUDE_RE.is_match(arg.as_str()).into())
	}

	pub(crate) fn longitude((arg,): (String,)) -> Result<Value> {
		Ok(LONGITUDE_RE.is_match(arg.as_str()).into())
	}

	pub(crate) fn numeric((arg,): (String,)) -> Result<Value> {
		if arg.is_empty() {
			Ok(Value::Bool(false))
		} else {
	...
	pub(crate) fn semver((arg,): (String,)) -> Result<Value> {
		Ok(Version::parse(arg.as_str()).is_ok().into())
	}

	pub(crate) fn url((arg,): (String,)) -> Result<Value> {
		Ok(Url::parse(&arg).is_ok().into())
	}

	pub(crate) fn uuid((arg,): (String,)) -> Result<Value> {
		Ok(Uuid::parse_str(arg.as_ref()).is_ok().into())
	}

	pub(crate) fn ulid((arg,): (String,)) -> Result<Value> {
		Ok(Ulid::from_string(arg.as_ref()).is_ok().into())
	}

	pub(crate) fn record((arg, Optional(tb)): (String, Optional<Value>)) -> Result<Value> {
...
pub mod similarity {
	...
	pub(crate) fn fuzzy(arg: (String, String)) -> Result<Value> {
		smithwaterman(arg)
	}
	...
	///
	/// Uses [`strsim::jaro`].
	pub(crate) fn jaro((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn jaro_winkler((a, b): (String, String)) -> Result<Value> {
	...
	///
	/// Returns a score where higher values indicate greater similarity.
	pub(crate) fn smithwaterman((a, b): (String, String)) -> Result<Value> {
	...
	pub(crate) fn sorensen_dice((a, b): (String, String)) -> Result<Value> {
...
pub mod semver {
	...
	pub(crate) fn compare((left, right): (String, String)) -> Result<Value> {
	...
	pub(crate) fn major((version,): (String,)) -> Result<Value> {
		parse_version(&version, "string::semver::major", "Invalid semantic version")
			.map(|v| v.major.into())
	}

	pub(crate) fn minor((version,): (String,)) -> Result<Value> {
		parse_version(&version, "string::semver::minor", "Invalid semantic version")
			.map(|v| v.minor.into())
	}

	pub(crate) fn patch((version,): (String,)) -> Result<Value> {
	...
	pub mod inc {
		...
		use crate::function::string::semver::parse_version;
		use crate::db::val::Value;
	...
	pub mod set {
		...
		pub(crate) fn major((version, value): (String, i64)) -> Result<Value> {
		...
		pub(crate) fn minor((version, value): (String, i64)) -> Result<Value> {
			// TODO: Deal with negative trunc:
			let value = value as u64;
		...
		pub(crate) fn patch((version, value): (String, i64)) -> Result<Value> {
			// TODO: Deal with negative trunc:
			let value = value as u64;
...
mod tests {
	...
	use crate::function::args::{Cast, Optional};
	use crate::db::val::Value;
	...
	fn html_encode() {
		...
		let value = super::html::encode((String::from("Lyxal"),)).unwrap();
		assert_eq!(value, Value::String("Lyxal".into()));
	}
	...
	fn similarity_distance_length_limit() {
		use crate::config::cnf::STRING_SIMILARITY_LIMIT;
