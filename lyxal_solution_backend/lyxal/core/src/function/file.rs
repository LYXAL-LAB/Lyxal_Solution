...
pub(crate) fn bucket((file,): (File,)) -> Result<Value> {
	Ok(file.bucket.into())
}

pub(crate) fn key((file,): (File,)) -> Result<Value> {
	Ok(file.key.into())
}
...
impl DestinationFile {
	...
	pub(crate) fn bucket_matches_source(&self, other: &str) -> bool {
