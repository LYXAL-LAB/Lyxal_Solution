...
pub(crate) fn add((mut array, value): (Array, Value)) -> Result<Value> {
	match value {
		Value::Array(value) => {
			for v in value.0 {
...
pub(crate) fn append((mut array, value): (Array, Value)) -> Result<Value> {
	array.push(value);
	Ok(array.into())
}

pub(crate) fn at((array, i): (Array, i64)) -> Result<Value> {
...
pub(crate) fn boolean_not((mut array,): (Array,)) -> Result<Value> {
	array.iter_mut().for_each(|v| *v = (!v.is_truthy()).into());
	Ok(array.into())
}

pub(crate) fn boolean_or((lh, rh): (Array, Array)) -> Result<Value> {
...
pub(crate) fn boolean_and((lh, rh): (Array, Array)) -> Result<Value> {
	let (mut res, comp) = if lh.len() < rh.len() {
		(rh, lh)
	} else {
		(lh, rh)
...
pub(crate) fn boolean_xor((lh, rh): (Array, Array)) -> Result<Value> {
	let (mut res, comp) = if lh.len() < rh.len() {
		(rh, lh)
	} else {
		(lh, rh)
...
pub(crate) fn clump((array, clump_size): (Array, i64)) -> Result<Value> {
	let clump_size = clump_size.max(0) as usize;
	Ok(array.clump(clump_size)?.into())
}

pub(crate) fn combine((array, other): (Array, Array)) -> Result<Value> {
	Ok(array.combine(other).into())
}

pub(crate) fn complement((array, other): (Array, Array)) -> Result<Value> {
	Ok(array.complement(other).into())
}

pub(crate) fn concat(Rest(arrays): Rest<Array>) -> Result<Value> {
...
pub(crate) fn difference((array, other): (Array, Array)) -> Result<Value> {
	Ok(array.difference(other).into())
}

pub(crate) fn distinct((array,): (Array,)) -> Result<Value> {
	Ok(array.uniq().into())
}
...
pub(crate) async fn filter_index(
	(stk, ctx, opt, doc): (&mut Stk, &FrozenContext, Option<&Options>, Option<&CursorDoc>),
...
pub(crate) fn first((array,): (Array,)) -> Result<Value> {
...
pub(crate) fn flatten((array,): (Array,)) -> Result<Value> {
...
pub(crate) fn group((array,): (Array,)) -> Result<Value> {
	Ok(array.flatten().uniq().into())
}

pub(crate) fn insert((mut array, value, Optional(index)): (Array, Value, Optional<i64>)) -> Result<Value> {
	match index {
		Some(mut index) => {
...
pub(crate) fn intersect((array, other): (Array, Array)) -> Result<Value> {
	Ok(array.intersect(other).into())
}

pub(crate) fn is_empty((array,): (Array,)) -> Result<Value> {
	Ok(array.is_empty().into())
}

pub(crate) fn join((arr, sep): (Array, String)) -> Result<Value> {
	Ok(arr.into_iter().map(Value::into_raw_string).collect::<Vec<_>>().join(&sep).into())
}

pub(crate) fn last((array,): (Array,)) -> Result<Value> {
	if let [.., last] = &array[0..] {
		Ok(last.to_owned())
	} else {
		Ok(Value::None)
...
pub(crate) fn len((array,): (Array,)) -> Result<Value> {
	Ok(array.len().into())
}

pub(crate) fn logical_and((mut lh, mut rh): (Array, Array)) -> Result<Value> {
	if lh.len() < rh.len() {
		let lh_len = lh.len();
...
pub(crate) fn logical_or((mut lh, mut rh): (Array, Array)) -> Result<Value> {
	if lh.len() < rh.len() {
		for (idx, b) in lh.into_iter().enumerate() {
			if b.is_truthy() {
				rh[idx] = b;
...
pub(crate) fn logical_xor((mut lh, mut rh): (Array, Array)) -> Result<Value> {
	if lh.len() < rh.len() {
		let lh_len = lh.len();
		for (idx, b) in lh.into_iter().enumerate() {
...
pub(crate) fn matches((array, compare_val): (Array, Value)) -> Result<Value> {
	Ok(array.matches(compare_val).into())
}

pub(crate) fn max((array,): (Array,)) -> Result<Value> {
	Ok(array.into_iter().max().unwrap_or_default())
}

pub(crate) fn min((array,): (Array,)) -> Result<Value> {
	Ok(array.into_iter().min().unwrap_or_default())
}

pub(crate) fn pop((mut array,): (Array,)) -> Result<Value> {
	Ok(array.pop().unwrap_or(Value::None))
}

pub(crate) fn prepend((mut array, value): (Array, Value)) -> Result<Value> {
	array.insert(0, value);
	Ok(array.into())
}

pub(crate) fn push((mut array, value): (Array, Value)) -> Result<Value> {
	array.push(value);
	Ok(array.into())
}

pub(crate) fn range((start_range, Optional(end)): (Value, Optional<i64>)) -> Result<Value> {
	let range = if let Some(end) = end {
		let start =
...
pub(crate) fn sequence((offset_len, Optional(len)): (i64, Optional<i64>)) -> Result<Value> {
...
pub(crate) fn remove((mut array, mut index): (Array, i64)) -> Result<Value> {
	// Negative index means start from the back
	if index < 0 {
		index += array.len() as i64;
	}
...
pub(crate) fn repeat((value, count): (Value, i64)) -> Result<Value> {
	ensure!(
		count >= 0,
...
pub(crate) fn reverse((mut array,): (Array,)) -> Result<Value> {
	array.reverse();
	Ok(array.into())
}

pub(crate) fn shuffle((mut array,): (Array,)) -> Result<Value> {
...
pub(crate) fn sort((mut array, Optional(order)): (Array, Optional<Value>)) -> Result<Value> {
...
pub(crate) fn sort_natural((mut array, Optional(order)): (Array, Optional<Value>)) -> Result<Value> {
	if sort_as_asc(&order) {
...
pub(crate) fn sort_lexical((mut array, Optional(order)): (Array, Optional<Value>)) -> Result<Value> {
	if sort_as_asc(&order) {
...
pub(crate) fn sort_natural_lexical(
...
pub(crate) fn swap((mut array, from, to): (Array, i64, i64)) -> Result<Value> {
	let min = 0;
	let max = array.len();
...
pub(crate) fn transpose((array,): (Array,)) -> Result<Value> {
	Ok(array.transpose().into())
}

pub(crate) fn union((array, other): (Array, Array)) -> Result<Value> {
	Ok(array.union(other).into())
}

pub(crate) fn windows((array, window_size): (Array, i64)) -> Result<Value> {
...
pub mod sort {
	...
	pub(crate) fn asc((mut array,): (Array,)) -> Result<Value> {
		array.sort_unstable();
		Ok(array.into())
	}

	pub(crate) fn desc((mut array,): (Array,)) -> Result<Value> {
		array.sort_unstable_by(|a, b| b.cmp(a));
		Ok(array.into())
	}
}
