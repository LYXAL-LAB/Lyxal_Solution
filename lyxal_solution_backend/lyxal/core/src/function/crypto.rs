...
pub(crate) fn blake3((arg,): (String,)) -> Result<Value> {
	Ok(blake3::hash(arg.as_bytes()).to_string().into())
}

pub(crate) fn joaat((arg,): (String,)) -> Result<Value> {
	Ok(joaat::hash_bytes(arg.as_bytes()).into())
}

pub(crate) fn md5((arg,): (String,)) -> Result<Value> {
...
pub(crate) fn sha1((arg,): (String,)) -> Result<Value> {
...
pub(crate) fn sha256((arg,): (String,)) -> Result<Value> {
...
pub(crate) fn sha512((arg,): (String,)) -> Result<Value> {
...
pub mod argon2 {
	...
	pub(crate) fn cmp((hash, pass): (String, String)) -> Result<Value> {
	...
	pub(crate) fn r#gen((pass,): (String,)) -> Result<Value> {
...
pub mod bcrypt {
	...
	use crate::function::crypto::COST_ALLOWANCE;
	use crate::db::val::Value;
