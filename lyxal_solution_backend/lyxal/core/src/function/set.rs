...
use crate::db::val::{Closure, Set, Value};

/// Add value(s) to a set
pub(crate) fn add((mut set, value): (Set, Value)) -> Result<Value> {
	match value {
		Value::Array(arr) => {
			for v in arr.0 {
				set.insert(v);
...
pub(crate) fn remove((mut set, value): (Set, Value)) -> Result<Value> {
	match value {
		Value::Array(arr) => {
			for v in arr.0 {
				set.remove(&v);
...
pub(crate) fn union((set1, set2): (Set, Set)) -> Result<Value> {
	Ok(set1.union(set2).into())
}

/// Return the intersection of two sets (A ∩ B)
pub(crate) fn intersect((set1, set2): (Set, Set)) -> Result<Value> {
	Ok(set1.intersection(&set2).into())
}

/// Return the symmetric difference of two sets (A △ B)
pub(crate) fn difference((set1, set2): (Set, Set)) -> Result<Value> {
	Ok(set1.symmetric_difference(set2).into())
}

/// Return the relative complement (A \ B)
pub(crate) fn complement((set1, set2): (Set, Set)) -> Result<Value> {
	Ok(set1.complement(set2).into())
}

/// Get the number of elements in the set
pub(crate) fn len((set,): (Set,)) -> Result<Value> {
	Ok(set.len().into())
}

/// Check if the set is empty
pub(crate) fn is_empty((set,): (Set,)) -> Result<Value> {
	Ok(set.is_empty().into())
}

/// Check if the set contains a value
pub(crate) fn contains((set, value): (Set, Value)) -> Result<Value> {
...
pub(crate) fn at((set, i): (Set, i64)) -> Result<Value> {
	let mut idx = i;
	if idx < 0 {
		idx += set.len() as i64;
	}
...
pub(crate) fn join((set, sep): (Set, String)) -> Result<Value> {
...
pub(crate) fn last((set,): (Set,)) -> Result<Value> {
...
pub(crate) fn max((set,): (Set,)) -> Result<Value> {
	Ok(set.into_iter().max().unwrap_or(Value::None))
}

/// Get the minimum value in the set
pub(crate) fn min((set,): (Set,)) -> Result<Value> {
