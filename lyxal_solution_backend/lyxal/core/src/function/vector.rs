...
use crate::function::util::math::vector::{
	Add, Angle, CrossProduct, Divide, DotProduct, Magnitude, Multiply, Normalize, Project, Scale,
	Subtract,
};
use crate::db::val::{Number, Value};

pub(crate) fn add((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.add(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn angle((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.angle(&b)?.into())
}

pub(crate) fn divide((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.divide(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn cross((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.cross(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn dot((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.dot(&b)?.into())
}

pub(crate) fn magnitude((a,): (Vec<Number>,)) -> Result<Value> {
	Ok(a.magnitude().into())
}

pub(crate) fn multiply((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.multiply(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn normalize((a,): (Vec<Number>,)) -> Result<Value> {
	Ok(a.normalize().into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn project((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.project(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn subtract((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	Ok(a.subtract(&b)?.into_iter().map(Value::from).collect::<Vec<_>>().into())
}

pub(crate) fn scale((a, b): (Vec<Number>, Number)) -> Result<Value> {
...
pub mod distance {
	...
	use crate::function::get_execution_context;
	use crate::function::util::math::vector::{
		ChebyshevDistance, EuclideanDistance, HammingDistance, ManhattanDistance, MinkowskiDistance,
	};
	use crate::db::idx::planner::IterationStage;
	use crate::db::val::{Number, Value};

	pub(crate) fn chebyshev((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.chebyshev_distance(&b)?.into())
	}

	pub(crate) fn euclidean((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.euclidean_distance(&b)?.into())
	}

	pub(crate) fn hamming((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
	...
	pub(crate) fn mahalanobis((_, _): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Err(anyhow::Error::new(Error::Unimplemented(
			"vector::distance::mahalanobis() function".to_string(),
		)))
	}

	pub(crate) fn manhattan((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.manhattan_distance(&b)?.into())
	}

	pub(crate) fn minkowski((a, b, o): (Vec<Number>, Vec<Number>, Number)) -> Result<Value> {
...
pub mod similarity {
	...
	use crate::error::Error;
	use crate::function::util::math::vector::{CosineSimilarity, JaccardSimilarity, PearsonSimilarity};
	use crate::db::val::{Number, Value};

	pub(crate) fn cosine((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.cosine_similarity(&b)?.into())
	}

	pub(crate) fn jaccard((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.jaccard_similarity(&b)?.into())
	}

	pub(crate) fn pearson((a, b): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Ok(a.pearson_similarity(&b)?.into())
	}

	pub(crate) fn spearman((_, _): (Vec<Number>, Vec<Number>)) -> Result<Value> {
		Err(anyhow::Error::new(Error::Unimplemented(
