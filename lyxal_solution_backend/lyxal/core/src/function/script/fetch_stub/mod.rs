...
fn fetch<'js>(ctx: Ctx<'js>) -> Result<()> {
	Err(Exception::throw_internal(
		&ctx,
		"The 'fetch' function is not available in this build of Lyxal. In order to use 'fetch', enable the 'http' feature.",
	))
}
						...
						fn new(ctx: Ctx<'_>) -> Result<()> {
							Err(Exception::throw_internal(
								...
								concat!(
									"The '",
									stringify!($name),
									"' class is not available in this build of Lyxal. In order to use '",
									stringify!($name),
