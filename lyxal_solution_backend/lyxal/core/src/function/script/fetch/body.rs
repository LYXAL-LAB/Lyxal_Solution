...
use crate::function::script::fetch::stream::ReadableStream;
...
impl Body {
	...
	pub(crate) fn buffer<B>(kind: BodyKind, buffer: B) -> Self
	where
		B: Into<Bytes>,
	{
	...
	pub(crate) fn stream<S>(kind: BodyKind, stream: S) -> Self
	where
	...
	/// consumed.
	pub(crate) async fn to_buffer(&self) -> StdResult<Option<Bytes>, RequestError> {
		match self.data.replace(BodyData::Used) {
	...
	pub(crate) fn clone_js(&self, ctx: Ctx<'_>) -> Self {
