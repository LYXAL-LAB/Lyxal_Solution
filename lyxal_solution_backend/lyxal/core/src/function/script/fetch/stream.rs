...
impl<R> ReadableStream<R> {
	pub(crate) fn new<S: Stream<Item = R> + Send + Sync + 'static>(stream: S) -> Self {
		ReadableStream::new_box(Box::pin(stream))
	}

	pub(crate) fn new_box(stream: Pin<Box<dyn Stream<Item = R> + Send + Sync>>) -> Self {
...
impl<R: Clone + 'static + Send + Sync> ReadableStream<R> {
	...
	pub(crate) fn tee(&mut self) -> (ReadableStream<R>, impl Future<Output = ()> + use<R>) {
