...
use crate::function::script::fetch::classes::Blob;
...
impl<'js> FormData<'js> {
	...
	#[qjs(constructor)]
	pub(crate) fn new(ctx: Ctx<'js>, args: Rest<()>) -> Result<Self> {
		if !args.is_empty() {
	...
	pub(crate) fn has(&self, name: Coerced<StdString>) -> bool {
		self.values.contains_key(&name.0)
	}

	pub(crate) fn delete(&mut self, name: Coerced<StdString>) {
