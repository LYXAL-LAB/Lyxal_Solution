...
impl Headers {
	...
	#[qjs(constructor)]
	pub(crate) fn new<'js>(ctx: Ctx<'js>, init: Value<'js>) -> Result<Self> {
	...
	pub(crate) fn append(&mut self, ctx: Ctx<'_>, key: String, val: String) -> Result<()> {
		self.append_inner(&ctx, &key, &val)
	}

	// Deletes a header from the header set
	pub(crate) fn delete(&mut self, ctx: Ctx<'_>, key: String) -> Result<()> {
	...
	pub(crate) fn entries(&self) -> Vec<List<(String, String)>> {
	...
	pub(crate) fn get(&self, ctx: Ctx<'_>, key: String) -> Result<Option<String>> {
		// Process and check the header name is valid
		let key =
			HeaderName::from_str(&key).map_err(|e| Exception::throw_type(&ctx, &format!("{e}")))?;
	...
	pub(crate) fn has(&self, ctx: Ctx<'_>, key: String) -> Result<bool> {
		// Process and check the header name is valid
		let key =
			HeaderName::from_str(&key).map_err(|e| Exception::throw_type(&ctx, &format!("{e}")))?;
	...
	pub(crate) fn set(&mut self, ctx: Ctx<'_>, key: String, val: String) -> Result<()> {
		// Process and check the header name is valid
...
impl Headers {
	...
	pub(crate) fn new_inner<'js>(ctx: &Ctx<'js>, val: Value<'js>) -> Result<Self> {
