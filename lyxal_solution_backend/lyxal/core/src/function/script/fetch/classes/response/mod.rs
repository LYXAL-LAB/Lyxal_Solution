...
use crate::function::script::fetch::body::{Body, BodyKind};
use crate::function::script::fetch::{RequestError, util};
...
impl<'js> Response<'js> {
	...
	pub(crate) fn body_used(&self) -> bool {
		self.body.used()
	}
	...
	#[qjs(get)]
	pub(crate) fn headers(&self) -> Class<'js, Headers> {
	...
	#[qjs(rename = "clone")]
	pub(crate) fn clone_js(&self, ctx: Ctx<'js>) -> Self {
		Response {
	...
	pub(crate) async fn blob(&self, ctx: Ctx<'js>) -> Result<Blob> {
	...
	#[qjs(rename = "formData")]
	pub(crate) async fn form_data(&self, ctx: Ctx<'js>) -> Result<Value<'js>> {
	...
	pub(crate) async fn array_buffer(&self, ctx: Ctx<'js>) -> Result<ArrayBuffer<'js>> {
		let data = self.take_buffer(&ctx).await?;
	...
	#[qjs(static)]
	pub(crate) fn error(ctx: Ctx<'js>) -> Result<Self> {
	...
	#[qjs(static)]
	pub(crate) fn redirect(ctx: Ctx<'_>, url: String, status: Opt<u32>) -> Result<Response<'_>> {
		let url = url
			.parse::<Url>()
...
mod test {
	...
	use crate::function::script::fetch::test::create_test_context;
