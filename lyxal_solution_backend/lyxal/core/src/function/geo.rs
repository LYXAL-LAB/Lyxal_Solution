...
use crate::db::val::{Geometry, Value};

pub(crate) fn area((arg,): (Geometry,)) -> Result<Value> {
	match arg {
...
pub(crate) fn bearing((v, w): (Geometry, Geometry)) -> Result<Value> {
	Ok(match (v, w) {
...
pub(crate) fn centroid((arg,): (Geometry,)) -> Result<Value> {
...
pub(crate) fn distance((v, w): (Geometry, Geometry)) -> Result<Value> {
	Ok(match (v, w) {
...
pub mod hash {
	...
	use crate::function::util::geo;
	use crate::db::val::{Geometry, Value};

	pub(crate) fn encode((arg, Optional(len)): (Geometry, Optional<i64>)) -> Result<Value> {
		let len = match len {
	...
	pub(crate) fn decode((arg,): (Value,)) -> Result<Value> {
...
pub mod is {
	...
	pub(crate) fn valid((arg,): (Geometry,)) -> Result<Value> {
