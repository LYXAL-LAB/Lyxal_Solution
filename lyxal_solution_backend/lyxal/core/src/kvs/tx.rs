...
use crate::db::catalog::providers::{
...
use crate::db::dbs::node::Node;
use crate::db::doc::CursorRecord;
use crate::error::Error;
use crate::db::idx::planner::ScanDirection;
use crate::db::key::database::sq::Sq;
...
use crate::db::val::{RecordId, RecordIdKey, TableName};
...
impl Transaction {
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::tx", skip_all)]
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::tx", skip(self))]
	pub(crate) async fn getm_records(
		...
		if version.is_some() {
			let keys: Vec<crate::db::key::record::RecordKey<'_>> = rids
				.iter()
				.map(|rid| crate::db::key::record::new(ns, db, &rid.table, &rid.key))
				.collect();
		...
		let keys: Vec<crate::db::key::record::RecordKey<'_>> = uncached_rids
			.iter()
			.map(|(_, rid)| crate::db::key::record::new(ns, db, &rid.table, &rid.key))
			.collect();
	...
	pub(crate) async fn store_changes(&self) -> Result<()> {
		...
		let futures = changes.into_iter().map(|(ns, db, tb, value)| async move {
			...
			let key = crate::db::key::change::new(ns, db, ts, &tb).encode_key()?;
...
impl NodeProvider for Transaction {
	...
	async fn all_nodes(&self) -> Result<Arc<[Node]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::root::nd::prefix();
				let end = crate::db::key::root::nd::suffix();
	...
	async fn get_node(&self, id: Uuid) -> Result<Arc<Node>> {
		...
		match self.cache.get(&qey) {
			Some(val) => val,
			None => {
				let key = crate::db::key::root::nd::new(id);
...
impl RootProvider for Transaction {
	async fn get_default_config(&self) -> Result<Option<Arc<DefaultConfig>>> {
		...
		match self.cache.get(&qey) {
			Some(val) => val,
			None => {
				let key = crate::db::key::root::root_config::new("default");
	...
	async fn get_root_config(&self, cg: &str) -> Result<Option<Arc<ConfigDefinition>>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let key = crate::db::key::root::root_config::new(cg);
...
impl NamespaceProvider for Transaction {
	...
	async fn all_ns(&self) -> Result<Arc<[NamespaceDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::root::ns::prefix();
				let end = crate::db::key::root::ns::suffix();
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_ns_by_name(&self, ns: &str) -> Result<Option<Arc<NamespaceDefinition>>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let key = crate::db::key::root::ns::new(ns);
	...
	async fn put_ns(&self, ns: NamespaceDefinition) -> Result<Arc<NamespaceDefinition>> {
		let key = crate::db::key::root::ns::new(&ns.name);
...
impl DatabaseProvider for Transaction {
	...
	async fn all_db(&self, ns: NamespaceId) -> Result<Arc<[DatabaseDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::namespace::db::prefix(ns)?;
				let end = crate::db::key::namespace::db::suffix(ns)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_db_by_name(&self, ns: &str, db: &str) -> Result<Option<Arc<DatabaseDefinition>>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				...
				let key = crate::db::key::namespace::db::new(ns.namespace_id, db);
	...
	/// dynamic mode.
	#[instrument(level = "trace", target = "lyxal::core::kvs::tx", skip(self, ctx))]
	...
	async fn put_db(&self, ns: &str, db: DatabaseDefinition) -> Result<Arc<DatabaseDefinition>> {
		let key = crate::db::key::namespace::db::new(db.namespace_id, &db.name);
	...
	async fn del_db(&self, ns: &str, db: &str, expunge: bool) -> Result<Option<()>> {
		...
		let key = crate::db::key::namespace::db::new(db.namespace_id, &db.name);
		let database_root = crate::db::key::database::all::new(db.namespace_id, db.database_id);
		if expunge {
			self.clr(&key).await?;
	...
	async fn all_db_analyzers(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::az::prefix(ns, db)?;
				let end = crate::db::key::database::az::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_sequences(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::sq::prefix(ns, db)?;
				let end = crate::db::key::database::sq::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_functions(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::fc::prefix(ns, db)?;
				let end = crate::db::key::database::fc::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_modules(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::md::prefix(ns, db)?;
				let end = crate::db::key::database::md::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_params(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::pa::prefix(ns, db)?;
				let end = crate::db::key::database::pa::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_models(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::ml::prefix(ns, db)?;
				let end = crate::db::key::database::ml::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_configs(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::cg::prefix(ns, db)?;
				let end = crate::db::key::database::cg::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_db_model(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::ml::new(ns, db, ml, vn);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_db_analyzer(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let key = crate::db::key::database::az::new(ns, db, az);
	...
	async fn get_db_function(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::db::key::database::fc::new(ns, db, fc);
	...
	async fn get_db_module(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::db::key::database::md::new(ns, db, md);
	...
	async fn get_db_param(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::db::key::database::pa::new(ns, db, pa);
	...
	async fn get_db_config(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Option::Some),
			None => {
				let key = crate::db::key::database::cg::new(ns, db, cg);
				if let Some(val) = self.get(&key, None).await? {
					let val = Arc::new(val);
					let entr = cache::tx::Entry::Any(val.clone());
					self.cache.insert(qey, entr);
	...
	async fn put_db_function(
		...
		let key = crate::db::key::database::fc::new(ns, db, &fc.name);
	...
	async fn put_db_module(
		...
		let key = crate::db::key::database::md::new(ns, db, &name);
	...
	async fn put_db_param(
		...
		let key = crate::db::key::database::pa::new(ns, db, &pa.name);
...
impl TableProvider for Transaction {
	...
	async fn all_tb(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::tb::prefix(ns, db)?;
				let end = crate::db::key::database::tb::suffix(ns, db)?;
	...
	async fn all_tb_views(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::table::ft::prefix(ns, db, tb)?;
				let end = crate::db::key::table::ft::suffix(ns, db, tb)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_or_add_tb(
		...
		match self.cache.get(&qey) {
			...
			None => {
				...
				let table_key =
					crate::db::key::database::tb::new(db_def.namespace_id, db_def.database_id, tb);
	...
	async fn get_tb_by_name(
		...
		match self.cache.get(&qey) {
			...
			None => {
				...
				let key = crate::db::key::database::tb::new(db.namespace_id, db.database_id, tb);
	...
	async fn put_tb(
		...
		let key = crate::db::key::database::tb::new(tb.namespace_id, tb.database_id, &tb.name);
	...
	async fn all_tb_events(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::table::ev::prefix(ns, db, tb)?;
				let end = crate::db::key::table::ev::suffix(ns, db, tb)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_tb_fields(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::table::fd::prefix(ns, db, tb)?;
				let end = crate::db::key::table::fd::suffix(ns, db, tb)?;
				let val = self.getr(beg..end, version).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_tb_indexes(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::table::ix::prefix(ns, db, tb)?;
				let end = crate::db::key::table::ix::suffix(ns, db, tb)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_tb_lives(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::table::lq::prefix(ns, db, tb)?;
				let end = crate::db::key::table::lq::suffix(ns, db, tb)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_tb(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::tb::new(ns, db, tb);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_tb_event(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type(),
			None => {
				let key = crate::db::key::table::ev::new(ns, db, tb, ev);
	...
	async fn get_tb_field(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::table::fd::new(ns, db, tb, fd);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn put_tb_field(
		...
		let key = crate::db::key::table::fd::new(ns, db, tb, &name);
	...
	async fn get_tb_index(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::table::ix::new(ns, db, tb, ix);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_tb_index_by_id(
		...
		let key = crate::db::key::table::ix::IndexNameLookupKey::new(ns, db, tb, ix);
	...
	async fn put_tb_index(
		...
		let key = crate::db::key::table::ix::new(ns, db, tb, &ix.name);
		self.set(&key, ix, None).await?;

		let name_lookup_key =
			crate::db::key::table::ix::IndexNameLookupKey::new(ns, db, tb, ix.index_id);
	...
	async fn del_tb_index(
		...
		let key = crate::db::key::index::all::new(ns, db, tb, ix.index_id);
		self.delp(&key).await?;
	...
	async fn get_record(
		...
		if version.is_some() {
			...
			let key = crate::db::key::record::new(ns, db, tb, id);
...
impl UserProvider for Transaction {
	...
	async fn all_root_users(&self) -> Result<Arc<[catalog::UserDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::root::us::prefix();
				let end = crate::db::key::root::us::suffix();
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_ns_users(&self, ns: NamespaceId) -> Result<Arc<[catalog::UserDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::namespace::us::prefix(ns)?;
				let end = crate::db::key::namespace::us::suffix(ns)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_users(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::us::prefix(ns, db)?;
				let end = crate::db::key::database::us::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_root_user(&self, us: &str) -> Result<Option<Arc<catalog::UserDefinition>>> {
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::root::us::new(us);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_ns_user(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::namespace::us::new(ns, us);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
	...
	async fn get_db_user(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::us::new(ns, db, us);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
	...
	async fn put_root_user(&self, us: &catalog::UserDefinition) -> Result<()> {
		let key = crate::db::key::root::us::new(&us.name);
	...
	async fn put_ns_user(&self, ns: NamespaceId, us: &catalog::UserDefinition) -> Result<()> {
		let key = crate::db::key::namespace::us::new(ns, &us.name);
		self.set(&key, us, None).await?;
	...
	async fn put_db_user(
		...
		let key = crate::db::key::database::us::new(ns, db, &us.name);
		self.set(&key, us, None).await?;
...
impl AuthorisationProvider for Transaction {
	...
	async fn all_root_accesses(&self) -> Result<Arc<[catalog::AccessDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::root::ac::prefix();
				let end = crate::db::key::root::ac::suffix();
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_root_access_grants(&self, ra: &str) -> Result<Arc<[catalog::AccessGrant]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::root::access::gr::prefix(ra)?;
				let end = crate::db::key::root::access::gr::suffix(ra)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_ns_accesses(&self, ns: NamespaceId) -> Result<Arc<[catalog::AccessDefinition]>> {
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::namespace::ac::prefix(ns)?;
				let end = crate::db::key::namespace::ac::suffix(ns)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_ns_access_grants(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::namespace::access::gr::prefix(ns, na)?;
				let end = crate::db::key::namespace::access::gr::suffix(ns, na)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_accesses(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::ac::prefix(ns, db)?;
				let end = crate::db::key::database::ac::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn all_db_access_grants(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::access::gr::prefix(ns, db, da)?;
				let end = crate::db::key::database::access::gr::suffix(ns, db, da)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_root_access(&self, ra: &str) -> Result<Option<Arc<catalog::AccessDefinition>>> {
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::root::ac::new(ra);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_root_access_grant(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::root::access::gr::new(ac, gr);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_ns_access(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::namespace::ac::new(ns, na);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_ns_access_grant(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::namespace::access::gr::new(ns, ac, gr);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_db_access(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::ac::new(ns, db, da);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn get_db_access_grant(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::access::gr::new(ns, db, ac, gr);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
	...
	async fn del_root_access(&self, ra: &str) -> Result<()> {
		...
		let key = crate::db::key::root::access::all::new(ra);
		self.delp(&key).await?;
	...
	async fn del_ns_access(&self, ns: NamespaceId, na: &str) -> Result<()> {
		...
		self.del(&key).await?;
		// Delete any associated data including access grants.
		let key = crate::db::key::namespace::access::all::new(ns, na);
		self.delp(&key).await?;
	...
	async fn del_db_access(&self, ns: NamespaceId, db: DatabaseId, da: &str) -> Result<()> {
		...
		self.del(&key).await?;
		// Delete any associated data including access grants.
		let key = crate::db::key::database::access::all::new(ns, db, da);
		self.delp(&key).await?;
...
impl ApiProvider for Transaction {
	...
	async fn all_db_apis(&self, ns: NamespaceId, db: DatabaseId) -> Result<Arc<[ApiDefinition]>> {
		...
		match self.cache.get(&qey) {
			Some(val) => val,
			None => {
				let beg = crate::db::key::database::ap::prefix(ns, db)?;
				let end = crate::db::key::database::ap::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_db_api(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::ap::new(ns, db, ap);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
				let val = Arc::new(val);
...
impl BucketProvider for Transaction {
	...
	async fn all_db_buckets(
		...
		match self.cache.get(&qey) {
			...
			None => {
				let beg = crate::db::key::database::bu::prefix(ns, db)?;
				let end = crate::db::key::database::bu::suffix(ns, db)?;
				let val = self.getr(beg..end, None).await?;
				let val = util::deserialize_cache(val.iter().map(|x| x.1.as_slice()))?;
	...
	async fn get_db_bucket(
		...
		match self.cache.get(&qey) {
			Some(val) => val.try_into_type().map(Some),
			None => {
				let key = crate::db::key::database::bu::new(ns, db, bu);
				let Some(val) = self.get(&key, None).await? else {
					return Ok(None);
				};
