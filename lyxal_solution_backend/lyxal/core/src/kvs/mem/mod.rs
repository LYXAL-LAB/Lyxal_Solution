...
use lyxalmx::{Database, DatabaseOptions, KeyIterator, ScanIterator, Transaction as Tx};
...
impl Datastore {
	...
	pub(crate) async fn new(config: MemoryConfig) -> Result<Datastore> {
		...
		let db = if let Some(ref persist_path) = config.persist_path {
			...
			let mut persistence_opts = lyxalmx::PersistenceOptions::new(persist_path);
			// Map AOL mode
			persistence_opts.aol_mode = match config.aol_mode {
				AolMode::Never => lyxalmx::AolMode::Never,
				AolMode::Sync => lyxalmx::AolMode::SynchronousOnCommit,
				AolMode::Async => lyxalmx::AolMode::AsynchronousAfterCommit,
			};
			// Map snapshot mode
			persistence_opts.snapshot_mode = match config.snapshot_mode {
				SnapshotMode::Never => lyxalmx::SnapshotMode::Never,
				SnapshotMode::Interval(interval) => lyxalmx::SnapshotMode::Interval(interval),
			};
			// Map sync mode to fsync mode
			persistence_opts.fsync_mode = match config.sync_mode {
				SyncMode::Never => lyxalmx::FsyncMode::Never,
				SyncMode::Every => lyxalmx::FsyncMode::EveryAppend,
				SyncMode::Interval(d) => lyxalmx::FsyncMode::Interval(d),
			};
...
impl Transactable for Transaction {
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::api", skip(self), fields(keys = keys.sprint()))]
	...
	async fn set(&self, key: Key, val: Val, version: Option<u64>) -> Result<()> {
		// LyxalMX does not support versioned set queries.
		if version.is_some() {
	...
	async fn put(&self, key: Key, val: Val, version: Option<u64>) -> Result<()> {
		// LyxalMX does not support versioned put queries.
		if version.is_some() {
			return Err(Error::UnsupportedVersionedQueries);
		}
		// Check to see if transaction is closed
	...
	async fn clr(&self, key: Key) -> Result<()> {
		...
		// Load the inner transaction
		let mut inner = self.inner.write().await;
		// Remove the key (use del since delete doesn't exist in LyxalMX)
		inner.del(key)?;
		// Return result
		Ok(())
	}
