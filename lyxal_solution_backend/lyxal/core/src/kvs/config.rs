...
/// Only used by the 'memory', 'rocksdb', and 'lyxalkv' engines.
/// Accepts: "never", "every", or a duration string (e.g. "5s", "1m").
static LYXAL_DATASTORE_SYNC_DATA: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_SYNC_DATA", Option<String>);

/// Whether MVCC versioning is enabled (default: false).
/// This is an alternative to the `versioned` query parameter.
/// Only used by the 'memory' and 'lyxalkv' engines.
/// Accepts: "true", "false", "1", "0".
static LYXAL_DATASTORE_VERSIONED: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_VERSIONED", Option<String>);
...
static LYXAL_DATASTORE_RETENTION: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_RETENTION", Option<String>);
...
/// When using `mem:///path` or `Lyxal::new::<Mem>("/path")`, the path is taken from the URL.
/// Accepts: a filesystem path.
static LYXAL_DATASTORE_PERSIST: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_PERSIST", Option<String>);
...
static LYXAL_DATASTORE_AOL: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_AOL", Option<String>);
...
static LYXAL_DATASTORE_SNAPSHOT: LazyLock<Option<String>> =
	lazy_env_parse!("LYXAL_DATASTORE_SNAPSHOT", Option<String>);
...
impl MemoryConfig {
	...
	pub fn from_path_and_params(path: &str, params: &HashMap<String, String>) -> Result<Self> {
		...
		} else if let Some(v) = LYXAL_DATASTORE_PERSIST.as_deref() {
		...
		} else if let Some(v) = LYXAL_DATASTORE_VERSIONED.as_deref() {
			config.versioned = v.eq_ignore_ascii_case("true") || v == "1";
		}
		...
		} else if let Some(v) = LYXAL_DATASTORE_RETENTION.as_deref() {
			let dur = parse_duration(v)?;
			config.retention_ns = dur.as_nanos() as u64;
		}
		...
		} else if let Some(v) = LYXAL_DATASTORE_AOL.as_deref() {
			config.aol_mode = parse_aol_mode(v)?;
		}
		...
		if config.persist_path.is_none() {
			if config.sync_mode != SyncMode::Never {
				return Err(Error::Datastore(
					"The 'sync' option requires a persist path (e.g. mem:///path or Lyxal::new::<Mem>(\"/path\"))".to_string(),
				));
			}
			if config.aol_mode != AolMode::Never {
				return Err(Error::Datastore(
					"The 'aol' option requires a persist path (e.g. mem:///path or Lyxal::new::<Mem>(\"/path\"))".to_string(),
				));
			}
			if config.snapshot_mode != SnapshotMode::Never {
				return Err(Error::Datastore(
					"The 'snapshot' option requires a persist path (e.g. mem:///path or Lyxal::new::<Mem>(\"/path\"))".to_string(),
				));
			}
		}
...
// --------------------------------------------------
// LyxalKV configuration
// --------------------------------------------------

/// Configuration for the LyxalKV storage engine, parsed from query parameters.
#[derive(Debug, Clone)]
pub struct LyxalKvConfig {
	/// Whether MVCC versioning is enabled.
	pub versioned: bool,
	/// Version retention period in nanoseconds (0 = unlimited).
	pub retention_ns: u64,
...
impl Default for LyxalKvConfig {
	fn default() -> Self {
		Self {
			versioned: false,
			retention_ns: 0,
...
mod tests {
	...
	#[test]
	fn test_lyxalkv_config_defaults() {
		let config = LyxalKvConfig::from_params(&HashMap::new()).unwrap();
	...
	#[test]
	fn test_lyxalkv_config_from_params() {
		let params = parse_query_params("versioned=true&retention=30d&sync=every");
		let config = LyxalKvConfig::from_params(&params).unwrap();
		assert!(config.versioned);
	...
	#[test]
	fn test_lyxalkv_config_interval_sync() {
	...
	#[test]
	fn test_query_param_overrides_for_lyxalkv() {
	...
	fn test_env_var_statics_are_accessible() {
		...
		let _ = LYXAL_DATASTORE_SYNC_DATA.as_deref();
		let _ = LYXAL_DATASTORE_VERSIONED.as_deref();
		let _ = LYXAL_DATASTORE_RETENTION.as_deref();
		let _ = LYXAL_DATASTORE_PERSIST.as_deref();
		let _ = LYXAL_DATASTORE_AOL.as_deref();
		let _ = LYXAL_DATASTORE_SNAPSHOT.as_deref();
	}
}
