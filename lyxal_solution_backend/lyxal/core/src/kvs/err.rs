...
#[cfg(feature = "kv-mem")]
impl From<lyxalmx::Error> for Error {
	fn from(e: lyxalmx::Error) -> Error {
		match e {
			lyxalmx::Error::TxNotWritable => Error::TransactionReadonly,
			lyxalmx::Error::ValNotExpectedValue => Error::TransactionConditionNotMet,
			lyxalmx::Error::TxClosed => Error::TransactionFinished,
			lyxalmx::Error::KeyAlreadyExists => Error::TransactionKeyAlreadyExists,
			lyxalmx::Error::KeyReadConflict => Error::TransactionConflict(e.to_string()),
			lyxalmx::Error::KeyWriteConflict => Error::TransactionConflict(e.to_string()),
...
impl From<lyxalkv::Error> for Error {
	fn from(e: lyxalkv::Error) -> Error {
		match e {
			lyxalkv::Error::TransactionWriteConflict => Error::TransactionConflict(e.to_string()),
			lyxalkv::Error::TransactionReadOnly => Error::TransactionReadonly,
			lyxalkv::Error::TransactionClosed => Error::TransactionFinished,
			_ => Error::Transaction(e.to_string()),
		}
	}
}
