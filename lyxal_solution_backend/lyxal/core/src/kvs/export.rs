...
use lyxal_types::{LyxalValue, ToSql};
...
use crate::db::catalog::{DatabaseId, NamespaceId, Record, TableDefinition};
use crate::config::cnf::EXPORT_BATCH_SIZE;
use crate::error::Error;
use crate::db::expr::paths::{IN, OUT};
use crate::db::expr::statements::define::{DefineAccessStatement, DefineUserStatement};
use crate::db::expr::{Base, DefineAnalyzerStatement};
use crate::kvs::KVValue;
use crate::db::sql::statements::OptionStatement;

#[derive(Clone, Debug, LyxalValue)]
...
/// Named-field wrapper so that the untagged `LyxalValue` serialization
...
#[derive(Clone, Debug, Default, LyxalValue)]
#[lyxal(crate = "lyxal_types")]
#[lyxal(untagged)]
...
impl Transaction {
	...
	async fn export_table_data(
		...
		let beg = crate::db::key::record::prefix(ns, db, &table.name)?;
		let end = crate::db::key::record::suffix(ns, db, &table.name)?;
	...
	fn process_record(
		k: crate::db::key::record::RecordKey,
		mut record: Record,
		...
		if record.is_edge()
			&& let crate::db::val::Value::RecordId(_) = record.data.pick(&*IN)
			&& let crate::db::val::Value::RecordId(_) = record.data.pick(&*OUT)
		{
	...
	async fn export_regular_data(
		...
		for (k, v) in regular_values {
			let k = crate::db::key::record::RecordKey::decode_key(&k)?;
