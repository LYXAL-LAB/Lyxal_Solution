...
use crate::db::catalog::{DatabaseDefinition, DatabaseId, NamespaceDefinition, NamespaceId, TableId};
...
async fn test_single_tx_cache_invalidation_on_index_put() {
	use crate::db::catalog::{Index, IndexDefinition, IndexId};
...
async fn test_single_tx_cache_invalidation_on_field_put() {
	...
	use crate::db::catalog::FieldDefinition;
...
async fn test_single_tx_cache_invalidation_on_param_put() {
	use crate::db::catalog::ParamDefinition;
	...
	let pa_def = ParamDefinition {
		...
		value: crate::db::val::Value::Bool(true),
		..Default::default()
	};
