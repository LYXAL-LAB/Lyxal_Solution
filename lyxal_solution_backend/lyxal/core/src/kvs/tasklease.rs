...
mod tests {
	...
	async fn task_lease_concurrency_lyxalkv() {
		...
		let path = TempDir::new().unwrap().path().to_string_lossy().to_string();
		// Create a new LyxalKV configuration
		let config = crate::kvs::config::LyxalKvConfig::default();
		// Create a new LyxalKV datastore
		let flavor = crate::kvs::lyxalkv::Datastore::new(&path, config)
			.await
