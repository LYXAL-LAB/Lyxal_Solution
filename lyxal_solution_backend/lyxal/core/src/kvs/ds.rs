...
use lyxal_types::{AuthError, Error as TypesError, LyxalValue, object};
#[cfg(not(target_family = "wasm"))]
use tokio::spawn;
...
use crate::bucket::BucketStoreProvider;
use crate::bucket::manager::BucketsManager;
use crate::db::catalog::providers::{
	ApiProvider, CatalogProvider, DatabaseProvider, NamespaceProvider, NodeProvider, TableProvider,
	UserProvider,
};
use crate::db::catalog::{ApiDefinition, Index, NodeLiveQuery, SubscriptionDefinition};
use crate::config::cnf::NORMAL_FETCH_SIZE;
use crate::config::cnf::dynamic::DynamicConfiguration;
use crate::db::ctx::Context;
#[cfg(feature = "jwks")]
use crate::db::dbs::capabilities::NetTarget;
use crate::db::dbs::capabilities::{
	ArbitraryQueryTarget, ExperimentalTarget, MethodTarget, RouteTarget,
};
use crate::db::dbs::node::{Node, Timestamp};
use crate::db::dbs::{Capabilities, Executor, Options, QueryResult, QueryResultBuilder, Session};
use crate::db::doc::AsyncEventRecord;
use crate::error::Error;
use crate::db::expr::model::get_model_path;
use crate::db::expr::statements::{DefineModelStatement, DefineStatement, DefineUserStatement};
use crate::db::expr::{Base, Expr, FlowResultExt as _, Literal, LogicalPlan, TopLevelExpr};
#[cfg(feature = "jwks")]
use crate::db::iam::jwks::JwksCache;
use crate::db::iam::{Action, Auth, Error as IamError, Resource, ResourceKind, Role};
use crate::db::idx::IndexKeyBase;
use crate::db::idx::index::IndexOperation;
use crate::db::idx::trees::store::IndexStores;
use crate::db::key::root::ic::IndexCompactionKey;
...
use crate::db::sql::Ast;
#[cfg(feature = "lyxalism")]
use crate::lyxalism::cache::LyxalismCache;
use crate::db::syn::parser::{ParserSettings, StatementStream};
use crate::types::{PublicNotification, PublicValue, PublicVariables};
use crate::db::val::convert_value_to_public_value;
use crate::{LyxalComposer, db::syn};

const TARGET: &str = "lyxal::core::kvs::ds";
...
pub struct Datastore {
	...
	// The lyxalism cache
	#[cfg(feature = "lyxalism")]
	lyxalism_cache: Arc<LyxalismCache>,
...
pub struct Metrics {
	/// The name of the metrics group (e.g., "lyxal.rocksdb").
	pub name: &'static str,
...
/// RocksDB, TiKV, LyxalKV, LyxalDS, etc.). Implementors translate the
...
pub enum DatastoreFlavor {
	...
	LyxalKV(super::lyxalkv::Datastore),
}

impl TransactionBuilderFactoryRequirements for LyxalComposer {}

#[cfg_attr(target_family = "wasm", async_trait::async_trait(?Send))]
#[cfg_attr(not(target_family = "wasm"), async_trait::async_trait)]
impl TransactionBuilderFactory for LyxalComposer {
		...
		match (flavour, path) {
			...
			(flavour @ "memory", path) => {
				#[cfg(feature = "kv-mem")]
				{
					...
					// Parse LyxalMX configuration from URL path and query parameters
				...
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `memory` storage engine as it is not enabled in this build of Lyxal".to_owned())));
			}
			...
			(flavour @ "rocksdb", path) => {
				...
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `rocksdb` storage engine as it is not enabled in this build of Lyxal".to_owned())));
			}
			// Initiate a LyxalKV database
			(flavour @ "lyxalkv", path) => {
				#[cfg(feature = "kv-lyxalkv")]
				{
					// Create a new blocking threadpool
					super::threadpool::initialise();
					// Parse LyxalKV-specific configuration from query parameters
					let config =
						super::config::LyxalKvConfig::from_params(&params).map_err(Error::Kvs)?;
					// Initialise the storage engine
					let v = super::lyxalkv::Datastore::new(&path, config)
						.await
						.map(DatastoreFlavor::LyxalKV)?;
					info!(target: TARGET, "Started {flavour} kvs store");
					Ok(Box::<DatastoreFlavor>::new(v))
				}
				#[cfg(not(feature = "kv-lyxalkv"))]
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `lyxalkv` storage engine as it is not enabled in this build of Lyxal".to_owned())));
			}
			...
			(flavour @ "indxdb", path) => {
				...
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `indxdb` storage engine as it is not enabled in this build of Lyxal".to_owned())));
			}
			...
			(flavour @ "tikv", path) => {
				...
				bail!(Error::Kvs(crate::kvs::Error::Datastore("Cannot connect to the `tikv` storage engine as it is not enabled in this build of Lyxal".to_owned())));
			}
	...
	fn path_valid(v: &str) -> Result<String> {
		...
		match scheme_part {
			...
			v_s if v_s.starts_with("lyxalkv:") => Ok(v.to_string()),
...
impl TransactionBuilder for DatastoreFlavor {
	...
	async fn shutdown(&self) -> Result<()> {
		match self {
			...
			#[cfg(feature = "kv-lyxalkv")]
			Self::LyxalKV(v) => Ok(v.shutdown().await?),
			#[allow(unreachable_patterns)]
			_ => unreachable!(),
		}
	}
...
impl Display for DatastoreFlavor {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		match self {
			...
			#[cfg(feature = "kv-lyxalkv")]
			Self::LyxalKV(_) => write!(f, "lyxalkv"),
			#[allow(unreachable_patterns)]
			_ => unreachable!(),
		}
	}
...
impl Datastore {
	...
	/// # #[tokio::main]
	/// # async fn main() -> Result<(),Error> {
	/// let ds = Datastore::new("lyxalkv://temp.skv").await?;
	/// # Ok(())
	/// # }
	/// ```
	///
	...
	pub async fn new(path: &str) -> Result<Self> {
		Self::new_with_factory(LyxalComposer(), path, CancellationToken::new()).await
	}
	...
	/// - `path`: Database path (e.g., "memory", "lyxalkv://path", "tikv://host:port")
	/// - `canceller`: Token for graceful shutdown and cancellation of long-running operations
	///
	/// # Generic parameters
	...
	pub(crate) fn new_with_builder(
		...
		Ok(Self {
			...
			#[cfg(feature = "lyxalism")]
			lyxalism_cache: Arc::new(LyxalismCache::new()),
			async_event_trigger,
		})
	}
	...
	#[instrument(err, level = "trace", target = "lyxal::core::kvs::ds", skip_all)]
	...
	pub async fn get_version(&self) -> Result<(MajorVersion, bool)> {
		...
		let key = crate::db::key::version::new();
		...
		let val = match catch!(txn, txn.get(&key, None).await) {
			...
			None => {
				...
				let rng = crate::db::key::version::proceeding();
				...
				let version = if keys.is_empty() {
					...
					warn!(
						...
						"No version key found but existing data detected in storage. \
						 This storage contains data from a previous Lyxal version. \
	...
	pub async fn initialise_defaults(&self, namespace: &str, database: &str) -> Result<()> {
		info!(target: TARGET, "This is a new Lyxal instance. Initialising default namespace '{namespace}' and database '{database}'");
		// Create the SQL statement
		let sql = r"
			DEFINE NAMESPACE $namespace COMMENT 'Default namespace generated by Lyxal';
			USE NS $namespace;
			DEFINE DATABASE $database COMMENT 'Default database generated by Lyxal';
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::ds", skip_all)]
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::ds", skip(self))]
	...
	#[instrument(err, level = "trace", target = "lyxal::core::kvs::ds", skip(self))]
	pub async fn insert_node(&self) -> Result<()> {
		...
		crate::telemetry::sys::refresh().await;
		// Open transaction and set node data
		let txn = self.transaction(Write, Optimistic).await?;
		let key = crate::db::key::root::nd::Nd::new(self.id);
	...
	pub async fn update_node(&self) -> Result<()> {
		...
		// Open transaction and set node data
		let txn = self.transaction(Write, Optimistic).await?;
		let key = crate::db::key::root::nd::new(self.id);
		let now = self.clock_now();
		let node = Node::new(self.id, now, false);
	...
	pub async fn expire_nodes(&self) -> Result<()> {
		...
		if !inactive.is_empty() {
			...
			for nd in inactive.iter() {
				...
				let key = crate::db::key::root::nd::new(nd.id);
	...
	pub async fn remove_nodes(&self) -> Result<()> {
		...
		for id in archived.iter() {
			// Open a writeable transaction
			let beg = crate::db::key::node::lq::prefix(*id)?;
			let end = crate::db::key::node::lq::suffix(*id)?;
			...
			let txn = self.transaction(Write, Optimistic).await?;
			{
				...
				while let Some(rng) = next {
					...
					for (k, v) in res.result.iter() {
						...
						let nlq = catch!(txn, crate::db::key::node::lq::Lq::decode_key(k.clone()));
						// Check that the node for this query is archived
						if archived.contains(&nlq.nd) {
							// Get the key for this table live query
							let tlq = crate::db::key::table::lq::new(val.ns, val.db, &val.tb, nlq.lq);
		...
		for ns in nss.iter() {
			...
			for db in dbs.iter() {
				...
				for tb in tbs.iter() {
					...
					let beg =
						crate::db::key::table::lq::prefix(db.namespace_id, db.database_id, &tb.name)?;
					let end =
						crate::db::key::table::lq::suffix(db.namespace_id, db.database_id, &tb.name)?;
					let mut next = Some(beg..end);
					let txn = self.transaction(Write, Optimistic).await?;
					while let Some(rng) = next {
						// Fetch the next batch of keys and values
						...
						for (k, v) in res.result.iter() {
							...
							if archived.contains(&stm.node) {
								// Get the key for this node live query
								let tlq = catch!(txn, crate::db::key::table::lq::Lq::decode_key(k));
								// Get the key for this table live query
								let nlq = crate::db::key::node::lq::new(nid, lid);
								// Delete the node live query
								catch!(txn, txn.clr(&nlq).await);
								// Delete the table live query
								catch!(txn, txn.clr(&tlq).await);
	...
	pub async fn delete_queries(&self, ids: Vec<uuid::Uuid>) -> Result<()> {
		...
		for id in ids {
			// Get the key for this node live query
			let nlq = crate::db::key::node::lq::new(self.id(), id);
			// Fetch the LIVE meta data node entry
			if let Some(lq) = catch!(txn, txn.get(&nlq, None).await) {
				// Get the key for this node live query
				let nlq = crate::db::key::node::lq::new(self.id(), id);
				// Get the key for this table live query
				let tlq = crate::db::key::table::lq::new(lq.ns, lq.db, &lq.tb, id);
				// Delete the table live query
				catch!(txn, txn.clr(&tlq).await);
				// Delete the node live query
				catch!(txn, txn.clr(&nlq).await);
	...
	pub async fn changefeed_process(&self, interval: &Duration) -> Result<()> {
		...
		catch!(txn, crate::db::cf::gc_all_at(&lh, &txn).await);
		// Commit the changes
		catch!(txn, txn.commit().await);
		// Everything ok
		Ok(())
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::ds", skip(dbs))]
	...
	///
	/// ```rust,no_run
	/// use lyxal_core::kvs::{Datastore, TransactionType::*, LockType::*};
	/// use anyhow::Error;
	///
	/// #[tokio::main]
	/// async fn main() -> Result<(),Error> {
	...
	/// }
	/// ```
	#[instrument(level = "debug", target = "lyxal::core::kvs::ds", skip_all)]
	...
	pub async fn process_with_transaction(
		...
		let mut ctx = self.setup_ctx().map_err(|e| {
			e.downcast::<Error>()
				.map(crate::error::into_types_error)
				.unwrap_or_else(|e| TypesError::internal(e.to_string()))
		})?;

		// Store the query variables
		if let Some(vars) = vars {
			ctx.attach_variables(vars.into()).map_err(crate::error::into_types_error)?;
		}
		...
		S: Stream<Item = Result<Bytes>>,
	{
		...
		let parser_settings = ParserSettings {
			...
			lyxalism_enabled: ctx
				.get_capabilities()
				.allows_experimental(&ExperimentalTarget::Lyxalism),
			..Default::default()
		};
	...
	pub(crate) async fn process_plan(
		...
		// Start an execution context
		ctx.attach_session(sess).map_err(crate::error::into_types_error)?;
	...
	pub async fn export_with_config(
		...
		let (ns, db) = crate::db::iam::check::check_ns_db(sess)?;
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::ds", skip(self, sess))]
	...
	///
	/// TODO: This should not be public, but it is used in `lyxal/src/api/engine/local/mod.rs`.
	pub async fn get_db_model(
		...
		model_name: &str,
		model_version: &str,
	) -> Result<Option<Arc<crate::db::catalog::MlModelDefinition>>> {
		let tx = self.transaction(Read, Optimistic).await?;
...
mod test {
	use super::*;
	use crate::db::iam::verify::verify_root_creds;
	...
	pub async fn very_deep_query() -> Result<()> {
		...
		use crate::db::expr::{BinaryOperator, Expr, Literal};
		use crate::kvs::Datastore;
		use crate::db::val::{Number, Value};
