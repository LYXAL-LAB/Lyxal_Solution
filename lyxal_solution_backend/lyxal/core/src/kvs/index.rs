...
use crate::config::cnf::INDEXING_BATCH_SIZE;
...
use crate::db::val::{Object, RecordId, RecordIdKey, TableName, Value};
...
impl Building {
	...
	async fn run(&self) -> Result<()> {
		...
		// Remove existing index data.
		{
			...
			let key = crate::db::key::index::all::new(
				self.ix_key.ns,
				self.ix_key.db,
				&self.ix_key.tb,
				self.ix_key.ix,
		...
		let beg = crate::db::key::record::prefix(self.ix_key.ns, self.ix_key.db, self.ikb.table())?;
		let end = crate::db::key::record::suffix(self.ix_key.ns, self.ix_key.db, self.ikb.table())?;
	...
	async fn index_initial_batch(
		...
		for (k, v) in values {
			...
			let key = crate::db::key::record::RecordKey::decode_key(&k)?;
			// Parse the value.
