...
pub(super) static LYXALKV_ENABLE_VLOG: LazyLock<bool> =
	lazy_env_parse!("LYXAL_LYXALKV_ENABLE_VLOG", bool, true);

/// Whether to enable versioned index (default: false, only applies when versioning is enabled)
pub(super) static LYXALKV_VERSIONED_INDEX: LazyLock<bool> =
	lazy_env_parse!("LYXAL_LYXALKV_VERSIONED_INDEX", bool, false);

/// The block size in bytes (default: 64 KiB)
pub(super) static LYXALKV_BLOCK_SIZE: LazyLock<usize> =
	lazy_env_parse!(bytes, "LYXAL_LYXALKV_BLOCK_SIZE", usize, 64 * 1024);

/// The maximum value log file size in bytes (default: dynamic from 64 MiB to 512 MiB)
pub(super) static LYXALKV_VLOG_MAX_FILE_SIZE: LazyLock<u64> =
	lazy_env_parse!(bytes, "LYXAL_LYXALKV_VLOG_MAX_FILE_SIZE", u64, || {
...
/// 4 KiB)
pub(super) static LYXALKV_VLOG_THRESHOLD: LazyLock<usize> =
	lazy_env_parse!(bytes, "LYXAL_LYXALKV_VLOG_THRESHOLD", usize, 4 * 1024);

/// The block cache capacity in bytes (default: dynamic based on memory)
pub(super) static LYXALKV_BLOCK_CACHE_CAPACITY: LazyLock<u64> =
	lazy_env_parse!(bytes, "LYXAL_LYXALKV_BLOCK_CACHE_CAPACITY", u64, || {
		// Load the system attributes
		let mut system = System::new_all();
		// Refresh the system memory
		system.refresh_memory();
...
pub(super) static LYXALKV_GROUPED_COMMIT_TIMEOUT: LazyLock<u64> =
	lazy_env_parse!(duration, "LYXAL_LYXALKV_GROUPED_COMMIT_TIMEOUT", u64, || {
...
/// LYXALKV_GROUPED_COMMIT_MAX_BATCH_SIZE), then the coordinator will wait up to
/// LYXALKV_GROUPED_COMMIT_TIMEOUT to collect more transactions. Smaller batches are flushed
/// immediately to preserve low latency.
pub(super) static LYXALKV_GROUPED_COMMIT_WAIT_THRESHOLD: LazyLock<usize> =
	lazy_env_parse!("LYXAL_LYXALKV_GROUPED_COMMIT_WAIT_THRESHOLD", usize, 12);
...
pub(super) static LYXALKV_GROUPED_COMMIT_MAX_BATCH_SIZE: LazyLock<usize> =
	lazy_env_parse!("LYXAL_LYXALKV_GROUPED_COMMIT_MAX_BATCH_SIZE", usize, 4096);
