...
use lyxalkv::{
	Durability, Mode, Transaction as Tx, Tree, TreeBuilder, HistoryOptions, LSMIterator,
};
...
use super::config::{LyxalKvConfig, SyncMode};
use super::err::{Error, Result};
use crate::db::key::debug::Sprintable;
use crate::kvs::api::Transactable;
use crate::kvs::{Key, Val};

const TARGET: &str = "lyxal::core::kvs::lyxalkv";
...
impl Datastore {
	/// Open a new database
	pub(crate) async fn new(path: &str, config: LyxalKvConfig) -> Result<Datastore> {
		...
		let enable_vlog = *cnf::LYXALKV_ENABLE_VLOG || config.versioned;
		...
		info!(target: TARGET, "Setting value log max file size: {}", *cnf::LYXALKV_VLOG_MAX_FILE_SIZE);
		let builder = builder.with_vlog_max_file_size(*cnf::LYXALKV_VLOG_MAX_FILE_SIZE);

		// Configure value log threshold
		info!(target: TARGET, "Setting value log threshold: {}", *cnf::LYXALKV_VLOG_THRESHOLD);
		let builder = builder.with_vlog_value_threshold(*cnf::LYXALKV_VLOG_THRESHOLD);
		...
		let versioned_index = config.versioned && *cnf::LYXALKV_VERSIONED_INDEX;
		...
		info!(target: TARGET, "Setting block cache capacity: {}", *cnf::LYXALKV_BLOCK_CACHE_CAPACITY);
		let builder = builder.with_block_cache_capacity(*cnf::LYXALKV_BLOCK_CACHE_CAPACITY);
		// Set the block size
		info!(target: TARGET, "Setting block size: {}", *cnf::LYXALKV_BLOCK_SIZE);
		let builder = builder.with_block_size(*cnf::LYXALKV_BLOCK_SIZE);
...
impl Transactable for Transaction {
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::api", skip(self))]
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::api", skip(self), fields(key = key.sprint()))]
	...
	#[instrument(level = "trace", target = "lyxal::core::kvs::api", skip(self), fields(rng = rng.sprint()))]
		...
		let res = affinitypool::spawn_local(move || -> Result<_> {
			...
			match version {
				Some(ts) => {
					...
					let mut iter = inner.history_with_options(beg, end, opts.clone())?;
