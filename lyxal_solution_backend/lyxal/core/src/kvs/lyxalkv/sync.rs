...
/// LyxalKV's Write-Ahead Log to persistent storage at configurable intervals. It provides
...
/// - `lyxalkv:///path?sync=200ms` -- flush every 200 milliseconds
/// - `lyxalkv:///path?sync=1s` -- flush every second
///
/// ## Durability
///
...
impl BackgroundFlusher {
	...
	pub fn new(db: Tree, interval: Duration) -> Result<Self> {
		...
		let handle = thread::Builder::new()
			.name("lyxalkv-background-flusher".to_string())
			.spawn(move || {
				loop {
			...
			.map_err(|_| {
				Error::Datastore("failed to spawn LyxalKV background flush thread".to_string())
			})?;
		// Create a new background flusher
		Ok(Self {
			shutdown,
...
/// 1. Commit their LyxalKV transaction on the caller thread
...
///
/// This coordinator allows multiple threads to commit their LyxalKV transactions in parallel
...
/// - `lyxalkv:///path?sync=every` -- groups transaction commits together and wait for sync
///
/// Batching behavior is controlled by environment variables:
/// - `LYXAL_LYXALKV_GROUPED_COMMIT_TIMEOUT`: Maximum wait time for collecting a batch
///   (nanoseconds)
/// - `LYXAL_LYXALKV_GROUPED_COMMIT_WAIT_THRESHOLD`: Waiter count to trigger waiting
/// - `LYXAL_LYXALKV_GROUPED_COMMIT_MAX_BATCH_SIZE`: Maximum waiters per batch
///
/// # Design Philosophy
///
/// Unlike traditional grouped commit implementations that serialize all commit operations,
/// this coordinator:
/// - Allows **parallel commits**: Each thread commits its LyxalKV transaction independently
...
impl CommitCoordinator {
	...
	pub fn new(db: Tree) -> Result<Self> {
		...
		let timeout = *super::cnf::LYXALKV_GROUPED_COMMIT_TIMEOUT;
		let wait_threshold = *super::cnf::LYXALKV_GROUPED_COMMIT_WAIT_THRESHOLD;
		let max_batch_size = *super::cnf::LYXALKV_GROUPED_COMMIT_MAX_BATCH_SIZE;
		...
		let handle = thread::Builder::new()
			.name("lyxalkv-commit-coordinator".to_string())
			.spawn(move || {
				batcher.run();
			})
			.map_err(|_| {
				Error::Datastore("failed to spawn LyxalKV commit coordinator thread".to_string())
			})?;
		// Create a new commit coordinator
		Ok(Self {
			shared,
...
/// LyxalKV transaction before calling `wait_for_sync()`. This allows transaction commits
