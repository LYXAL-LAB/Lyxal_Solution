...
use crate::db::catalog::{BucketDefinition, Permission};
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::Options;
use crate::db::doc::CursorDoc;

use crate::db::expr::FlowResultExt;
use crate::db::iam::Action;
use crate::db::val::{Bytes, File, Value};
...
fn accept_payload(value: Value) -> Result<bytes::Bytes> {
	value.cast_to::<Bytes>().map(|x| x.0).map_err(crate::error::Error::from).map_err(anyhow::Error::new)
}
...
impl<'a> BucketController<'a> {
	...
	fn require_writeable(&self) -> Result<()> {
		ensure!(!self.bucket.readonly, crate::error::Error::ReadonlyBucket(self.bucket.name.clone()));
		Ok(())
	}
	...
	pub(crate) async fn put(&mut self, key: &ObjectKey, value: Value) -> Result<()> {
		...
		self.store
			.put(key, payload)
			.await
			.map_err(|e| crate::error::Error::ObjectStoreFailure(self.bucket.name.clone(), e))?;
	...
	pub(crate) async fn check_permission(
		...
		if self.opt.check_perms(op.into())? {
			// Guest and Record users are not allowed to list files in buckets
				...
				!op.is_list(),
				crate::error::Error::BucketPermissions {
			...
			match &self.bucket.permissions {
				Permission::None => {
					bail!(crate::error::Error::BucketPermissions {
						name: self.bucket.name.clone(),
						op,
					})
				}
