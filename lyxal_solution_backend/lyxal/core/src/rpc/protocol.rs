...
use anyhow::Result;
use lyxal_types::{HashMap, object};
use tokio::sync::RwLock;
use uuid::Uuid;

use crate::db::catalog::providers::{CatalogProvider, NamespaceProvider, RootProvider};
use crate::db::dbs::capabilities::{ExperimentalTarget, MethodTarget};
use crate::db::dbs::{QueryResult, QueryType, Session};
use crate::db::iam::token::Token;
...
use crate::db::sql::statements::live::LiveFields;
use crate::db::sql::{
...
use crate::types::{
	PublicArray, PublicRecordIdKey, PublicUuid, PublicValue, PublicVariables, LyxalValue,
};
...
pub trait RpcProtocol {
	...
	async fn attach(&self, session_id: Option<Uuid>) -> Result<DbResult, lyxal_types::Error> {
	...
	async fn detach(&self, session_id: Option<Uuid>) -> Result<DbResult, lyxal_types::Error> {
		match session_id {
			Some(id) => {
	...
	fn get_session(
		&self,
		id: &Option<Uuid>,
	) -> Result<Arc<RwLock<Session>>, lyxal_types::Error> {
	...
	async fn sessions(&self) -> Result<DbResult, lyxal_types::Error> {
		let array = self
			.session_map()
			.to_vec()
			.into_iter()
	...
	async fn get_tx(
		&self,
		_id: Uuid,
	) -> Result<Arc<crate::kvs::Transaction>, lyxal_types::Error> {
	...
	async fn execute(
		...
		method: Method,
		params: PublicArray,
	) -> Result<DbResult, lyxal_types::Error> {
	...
	async fn signup(
		...
		let out: Result<PublicValue> =
			crate::db::iam::signup::signup(self.kvs(), &mut session, params.into())
				.await
				.map(LyxalValue::into_value);
	...
	async fn signin(
		...
		let out: Result<PublicValue> =
			crate::db::iam::signin::signin(self.kvs(), &mut session, params.into())
				.await
				.map(LyxalValue::into_value);
	...
	async fn authenticate(
		...
		let out: Result<PublicValue> =
			crate::db::iam::verify::token(self.kvs(), &mut session, token.as_str())
				.await
	...
	async fn invalidate(
		...
		crate::db::iam::clear::clear(&mut session).map_err(types_error_from_anyhow)?;
		// Return nothing on success
		Ok(DbResult::Other(PublicValue::None))
	}
	...
	async fn revoke(&self, params: PublicArray) -> Result<DbResult, lyxal_types::Error> {
		// Process the method arguments
		let unexpected = || invalid_params("Expected (token:Token)".to_string());
		let Some(value) = extract_args(params.into_vec()) else {
			return Err(unexpected());
	...
	async fn reset(&self, session_id: Option<Uuid>) -> Result<DbResult, lyxal_types::Error> {
		// Get a write lock on the session
		let session_lock = self.get_session(&session_id)?;
		let mut session = session_lock.write().await;
		// Reset the current session
		crate::db::iam::reset::reset(&mut session);
		// Cleanup live queries
			...
			Some(&"mod") => {
					...
					.allows_experimental(&ExperimentalTarget::Lyxalism)
				{
					return Err(invalid_params(
						"Experimental capability `lyxalism` is not enabled".to_string(),
					));
				}
