...
//!
//! All RPC failures are represented as [`lyxal_types::Error`]. This module provides
//! constructor functions for the same cases that were previously [`RpcError`] variants.
//! Wire codes are set by the [`lyxal_types::Error`] constructors.
...
use crate::error::into_types_error;
...
/// 3. `crate::error::Error` (core database error) — convert via `into_types_error()`.
