...
use lyxal_types::{Error as TypesError, kind, object};
use uuid::Uuid;

use crate::db::dbs;
use crate::db::dbs::{QueryResult, QueryType};
use crate::rpc::request::SESSION_ID;
use crate::types::{
	PublicArray, PublicKind, PublicNotification, PublicObject, PublicValue, LyxalValue,
};
...
pub struct DbResultStats {
	...
	///
	/// Note: This comes from the `time` field of the [`crate::db::dbs::QueryResult`] struct.
			...
			PublicValue::Object(obj) => {
				...
				if obj.get("id").is_some() && obj.get("action").is_some() {
					...
					let session = match obj.remove(SESSION_ID) {
						Some(session) => LyxalValue::from_value(session)?,
						None => None,
					};
...
impl LyxalValue for DbResponse {
	fn kind_of() -> PublicKind {
		PublicKind::Object
	}
	...
	fn into_value(self) -> PublicValue {
		let mut value = match self.result {
			...
			Err(err) => map! {
				"error".to_string() => LyxalValue::into_value(err),
			},
		};
	...
	fn from_value(value: PublicValue) -> Result<Self, TypesError> {
		...
		let session_id = LyxalValue::from_value(obj.remove(SESSION_ID).unwrap_or_default())?;
