...
static STORE: LazyLock<Arc<dyn ObjectStore>> =
	LazyLock::new(|| initialize_store("LYXAL_OBJECT_STORE", "store"));

static CACHE: LazyLock<Arc<dyn ObjectStore>> =
	LazyLock::new(|| initialize_store("LYXAL_CACHE_STORE", "cache"));
...
mod tests {
	...
	fn test_initialize_store_env_var() {
		...
		unsafe { env::set_var("LYXAL_OBJECT_STORE", url) };
		let store = initialize_store("LYXAL_OBJECT_STORE", "store");
		// Assert the store is initialized with the correct URL
		assert!(store.to_string().contains("store"));

		unsafe { env::remove_var("LYXAL_OBJECT_STORE") };
		assert!(env::var("LYXAL_OBJECT_STORE").is_err());
		let store = initialize_store("LYXAL_OBJECT_STORE", "store");
		debug!("{store:?}");
