	...
	(duration, $key:expr_2021, $t:ty, || $default:expr_2021) => {
		std::sync::LazyLock::new(|| {
			std::env::var($key)
				.ok()
				.and_then(|s| {
					use $crate::utils::str::ParseDuration;
...
macro_rules! fail {
	($($arg:tt)+) => {
		return Err(::anyhow::Error::new($crate::error::Error::unreachable(format_args!($($arg)*))))
	};
}
...
mod test {
	...
	fn fail_literal() {
		...
		assert_eq!(
			format!("lyxal/core/src/mac/mod.rs:{}: Reached unreachable code", line + 1),
			msg
		);
	}
	...
	fn fail_arguments() {
		...
		assert_eq!(
			format!("lyxal/core/src/mac/mod.rs:{}: Found test but expected other", line + 1),
			msg
		);
	}
}
