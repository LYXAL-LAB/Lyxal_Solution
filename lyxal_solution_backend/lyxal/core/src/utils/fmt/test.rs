...
pub fn ensure_formats(s: &str) {
	let parsed = crate::db::syn::parse(s).unwrap();
	let parsed_formated = crate::db::syn::parse(&parsed.to_sql()).unwrap();

	let plan: crate::db::expr::LogicalPlan = parsed.clone().into();
	let parsed_formated_plan = crate::db::syn::parse(&plan.to_sql()).unwrap();
