...
use crate::db::expr::idiom::Idioms as ExprIdioms;
use crate::utils::fmt::{EscapeIdent, Fmt};
use crate::db::sql::{Expr, Literal, Part};
...
impl From<Idiom> for crate::db::expr::Idiom {
	fn from(v: Idiom) -> Self {
		crate::db::expr::Idiom(v.0.into_iter().map(Into::into).collect())
	}
}

impl From<crate::db::expr::Idiom> for Idiom {
	fn from(v: crate::db::expr::Idiom) -> Self {
		Idiom(v.0.into_iter().map(Into::into).collect())
	}
}

impl lyxal_types::ToSql for Idiom {
