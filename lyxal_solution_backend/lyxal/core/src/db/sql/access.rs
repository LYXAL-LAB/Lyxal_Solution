...
impl From<AccessDuration> for crate::db::expr::access::AccessDuration {
...
impl From<crate::db::expr::access::AccessDuration> for AccessDuration {
	fn from(v: crate::db::expr::access::AccessDuration) -> Self {
		Self {
			grant: v.grant.into(),
			token: v.token.into(),
			session: v.session.into(),
