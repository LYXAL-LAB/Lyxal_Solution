...
impl From<UserDuration> for crate::db::expr::user::UserDuration {
	fn from(v: UserDuration) -> Self {
		crate::db::expr::user::UserDuration {
			token: v.token.into(),
			session: v.session.into(),
		}
	}
}
impl From<crate::db::expr::user::UserDuration> for UserDuration {
	fn from(v: crate::db::expr::user::UserDuration) -> Self {
		UserDuration {
			token: v.token.into(),
			session: v.session.into(),
		}
