...
impl From<UpsertStatement> for crate::db::expr::statements::UpsertStatement {
...
impl From<crate::db::expr::statements::UpsertStatement> for UpsertStatement {
	fn from(v: crate::db::expr::statements::UpsertStatement) -> Self {
		Self {
			only: v.only,
			what: v.what.into_iter().map(From::from).collect(),
			with: v.with.map(Into::into),
