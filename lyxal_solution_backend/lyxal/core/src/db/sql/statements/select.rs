...
impl From<SelectStatement> for crate::db::expr::statements::SelectStatement {
...
impl From<crate::db::expr::statements::SelectStatement> for SelectStatement {
	fn from(v: crate::db::expr::statements::SelectStatement) -> Self {
		Self {
			fields: v.fields.into(),
			omit: v.omit.into_iter().map(Into::into).collect(),
			only: v.only,
