...
use crate::utils::fmt::{CoverStmts, Fmt, fmt_separated_by};
...
impl From<IfelseStatement> for crate::db::expr::statements::IfelseStatement {
	fn from(v: IfelseStatement) -> Self {
		crate::db::expr::statements::IfelseStatement {
...
impl From<crate::db::expr::statements::IfelseStatement> for IfelseStatement {
	fn from(v: crate::db::expr::statements::IfelseStatement) -> Self {
		IfelseStatement {
			exprs: v.exprs.into_iter().map(|(a, b)| (From::from(a), From::from(b))).collect(),
			close: v.close.map(Into::into),
		}
