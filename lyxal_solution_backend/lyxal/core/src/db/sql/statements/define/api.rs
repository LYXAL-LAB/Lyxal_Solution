...
impl From<DefineApiStatement> for crate::db::expr::statements::DefineApiStatement {
	fn from(v: DefineApiStatement) -> Self {
		crate::db::expr::statements::DefineApiStatement {
			kind: v.kind.into(),
			path: v.path.into(),
...
impl From<crate::db::expr::statements::DefineApiStatement> for DefineApiStatement {
	fn from(v: crate::db::expr::statements::DefineApiStatement) -> Self {
		DefineApiStatement {
			kind: v.kind.into(),
			path: v.path.into(),
			actions: v.actions.into_iter().map(Into::into).collect(),
...
impl From<ApiAction> for crate::db::expr::statements::define::ApiAction {
	fn from(v: ApiAction) -> Self {
		crate::db::expr::statements::define::ApiAction {
			methods: v.methods,
...
impl From<crate::db::expr::statements::define::ApiAction> for ApiAction {
	fn from(v: crate::db::expr::statements::define::ApiAction) -> Self {
		ApiAction {
			methods: v.methods,
			action: v.action.into(),
			config: v.config.into(),
