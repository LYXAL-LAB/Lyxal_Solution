...
use crate::db::sql::{Block, Expr, Kind, Literal, Permission};
...
impl From<DefineFunctionStatement> for crate::db::expr::statements::DefineFunctionStatement {
...
impl From<crate::db::expr::statements::DefineFunctionStatement> for DefineFunctionStatement {
	fn from(v: crate::db::expr::statements::DefineFunctionStatement) -> Self {
		Self {
			kind: v.kind.into(),
			name: v.name,
			args: v.args.into_iter().map(|(i, k)| (i, k.into())).collect(),
