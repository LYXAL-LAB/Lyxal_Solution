...
use crate::db::sql::{Cond, Data, Explain, Expr, Literal, Output, With};
...
impl From<UpdateStatement> for crate::db::expr::statements::UpdateStatement {
	fn from(v: UpdateStatement) -> Self {
		crate::db::expr::statements::UpdateStatement {
			only: v.only,
...
impl From<crate::db::expr::statements::UpdateStatement> for UpdateStatement {
	fn from(v: crate::db::expr::statements::UpdateStatement) -> Self {
		UpdateStatement {
			only: v.only,
			what: v.what.into_iter().map(From::from).collect(),
			with: v.with.map(Into::into),
