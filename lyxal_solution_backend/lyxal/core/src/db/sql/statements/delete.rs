...
use crate::db::sql::{Cond, Explain, Expr, Literal, Output, With};
...
impl From<DeleteStatement> for crate::db::expr::statements::DeleteStatement {
	fn from(v: DeleteStatement) -> Self {
		crate::db::expr::statements::DeleteStatement {
			only: v.only,
...
impl From<crate::db::expr::statements::DeleteStatement> for DeleteStatement {
	fn from(v: crate::db::expr::statements::DeleteStatement) -> Self {
		DeleteStatement {
			only: v.only,
			what: v.what.into_iter().map(From::from).collect(),
			with: v.with.map(Into::into),
