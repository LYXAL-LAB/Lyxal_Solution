...
use crate::db::sql::{BinaryOperator, Expr, Literal};

#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
pub struct Block(pub(crate) Vec<Expr>);

impl From<Block> for crate::db::expr::Block {
	fn from(v: Block) -> Self {
		crate::db::expr::Block(v.0.into_iter().map(Into::into).collect())
	}
}
impl From<crate::db::expr::Block> for Block {
	fn from(v: crate::db::expr::Block) -> Self {
