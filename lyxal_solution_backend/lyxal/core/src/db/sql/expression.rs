...
use crate::utils::fmt::{CoverStmts, EscapeIdent};
use crate::db::sql::ast::ExplainFormat;
use crate::db::sql::literal::ObjectEntry;
use crate::db::sql::lookup::LookupKind;
use crate::db::sql::operator::BindingPower;
...
impl Expr {
	...
	// crate::db::expr::Expr::needs_parentheses
...
fn convert_public_geometry_to_internal(geom: lyxal_types::Geometry) -> crate::db::val::Geometry {
	match geom {
		lyxal_types::Geometry::Point(p) => crate::db::val::Geometry::Point(p),
		lyxal_types::Geometry::Line(l) => crate::db::val::Geometry::Line(l),
		lyxal_types::Geometry::Polygon(p) => crate::db::val::Geometry::Polygon(p),
		lyxal_types::Geometry::MultiPoint(mp) => crate::db::val::Geometry::MultiPoint(mp),
		lyxal_types::Geometry::MultiLine(ml) => crate::db::val::Geometry::MultiLine(ml),
		lyxal_types::Geometry::MultiPolygon(mp) => crate::db::val::Geometry::MultiPolygon(mp),
		lyxal_types::Geometry::Collection(c) => crate::db::val::Geometry::Collection(
			c.into_iter().map(convert_public_geometry_to_internal).collect(),
		),
	}
}

fn convert_public_range_to_literal(range: lyxal_types::Range) -> Expr {
	use crate::db::sql::literal::Literal;
	use crate::db::sql::operator::BinaryOperator;
...
pub(crate) fn convert_public_value_to_internal(value: lyxal_types::Value) -> crate::db::val::Value {
	match value {
		lyxal_types::Value::None => crate::db::val::Value::None,
		lyxal_types::Value::Null => crate::db::val::Value::Null,
		lyxal_types::Value::Bool(b) => crate::db::val::Value::Bool(b),
		lyxal_types::Value::Number(n) => match n {
			lyxal_types::Number::Int(i) => {
				crate::db::val::Value::Number(crate::db::val::Number::Int(i))
			}
			lyxal_types::Number::Float(f) => {
				crate::db::val::Value::Number(crate::db::val::Number::Float(f))
			}
			lyxal_types::Number::Decimal(d) => {
				crate::db::val::Value::Number(crate::db::val::Number::Decimal(d))
			}
		},
		lyxal_types::Value::String(s) => crate::db::val::Value::String(s),
		lyxal_types::Value::Duration(d) => {
			crate::db::val::Value::Duration(crate::db::val::Duration(d.into_inner()))
		}
		lyxal_types::Value::Datetime(dt) => {
			crate::db::val::Value::Datetime(crate::db::val::Datetime(dt.into_inner()))
		}
		lyxal_types::Value::Uuid(u) => {
			crate::db::val::Value::Uuid(crate::db::val::Uuid(u.into_inner()))
		}
		lyxal_types::Value::Array(a) => crate::db::val::Value::Array(crate::db::val::Array::from(
			a.into_iter().map(convert_public_value_to_internal).collect::<Vec<_>>(),
		)),
		lyxal_types::Value::Set(s) => crate::db::val::Value::Set(crate::db::val::Set::from(
			s.into_iter()
				.map(convert_public_value_to_internal)
				.collect::<std::collections::BTreeSet<_>>(),
		)),
		lyxal_types::Value::Object(o) => crate::db::val::Value::Object(crate::db::val::Object::from(
			o.into_iter()
				.map(|(k, v)| (k, convert_public_value_to_internal(v)))
				.collect::<std::collections::BTreeMap<_, _>>(),
		)),
		lyxal_types::Value::Geometry(g) => {
			crate::db::val::Value::Geometry(convert_public_geometry_to_internal(g))
		}
		lyxal_types::Value::Bytes(b) => {
			crate::db::val::Value::Bytes(crate::db::val::Bytes(b.into_inner()))
		}
		lyxal_types::Value::Table(t) => crate::db::val::Value::Table(t.into()),
		lyxal_types::Value::RecordId(PublicRecordId {
			table,
			key,
		}) => {
			let key = convert_public_record_id_key_to_internal(key);
			crate::db::val::Value::RecordId(crate::db::val::RecordId {
				table: table.into(),
				key,
			})
		}
		lyxal_types::Value::File(f) => crate::db::val::Value::File(crate::db::val::File {
			bucket: f.bucket,
			key: f.key,
		}),
		lyxal_types::Value::Range(r) => crate::db::val::Value::Range(Box::new(crate::db::val::Range {
			start: match r.start {
		...
		lyxal_types::Value::Regex(r) => {
			crate::db::val::Value::Regex(crate::db::val::Regex(r.into_inner()))
		}
	}
}

fn convert_public_record_id_key_to_internal(
	key: lyxal_types::RecordIdKey,
) -> crate::db::val::RecordIdKey {
	match key {
		lyxal_types::RecordIdKey::Number(n) => crate::db::val::RecordIdKey::Number(n),
		lyxal_types::RecordIdKey::String(s) => crate::db::val::RecordIdKey::String(s),
		lyxal_types::RecordIdKey::Uuid(u) => {
			crate::db::val::RecordIdKey::Uuid(crate::db::val::Uuid(u.into_inner()))
		}
		lyxal_types::RecordIdKey::Array(a) => crate::db::val::RecordIdKey::Array(
			crate::db::val::Array(a.into_iter().map(convert_public_value_to_internal).collect()),
		),
		lyxal_types::RecordIdKey::Object(o) => {
			crate::db::val::RecordIdKey::Object(crate::db::val::Object(
				o.into_iter().map(|(k, v)| (k, convert_public_value_to_internal(v))).collect(),
			))
		}
		lyxal_types::RecordIdKey::Range(r) => {
			crate::db::val::RecordIdKey::Range(Box::new(crate::db::val::RecordIdKeyRange {
				start: match r.start {
					Bound::Included(k) => {
...
impl From<Expr> for crate::db::expr::Expr {
	fn from(v: Expr) -> Self {
		match v {
			Expr::Literal(l) => crate::db::expr::Expr::Literal(l.into()),
			Expr::Param(p) => crate::db::expr::Expr::Param(p.into()),
			Expr::Idiom(i) => crate::db::expr::Expr::Idiom(i.into()),
			Expr::Table(t) => crate::db::expr::Expr::Table(t.into()),
			Expr::Mock(m) => crate::db::expr::Expr::Mock(m.into()),
			Expr::Block(b) => crate::db::expr::Expr::Block(Box::new((*b).into())),
			Expr::Constant(c) => crate::db::expr::Expr::Constant(c.into()),
			Expr::Prefix {
				op,
				expr,
			} => crate::db::expr::Expr::Prefix {
				op: op.into(),
			...
			Expr::Binary {
				...
				op,
				right,
			} => crate::db::expr::Expr::Binary {
				left: Box::new((*left).into()),
				op: op.into(),
				right: Box::new((*right).into()),
			},
			Expr::FunctionCall(f) => crate::db::expr::Expr::FunctionCall(Box::new((*f).into())),
			Expr::Closure(s) => crate::db::expr::Expr::Closure(Box::new((*s).into())),
			Expr::Break => crate::db::expr::Expr::Break,
			Expr::Continue => crate::db::expr::Expr::Continue,
			Expr::Return(e) => crate::db::expr::Expr::Return(Box::new((*e).into())),
			Expr::Throw(e) => crate::db::expr::Expr::Throw(Box::new((*e).into())),
			Expr::IfElse(s) => crate::db::expr::Expr::IfElse(Box::new((*s).into())),
			Expr::Select(s) => crate::db::expr::Expr::Select(Box::new((*s).into())),
			Expr::Create(s) => crate::db::expr::Expr::Create(Box::new((*s).into())),
			Expr::Update(s) => crate::db::expr::Expr::Update(Box::new((*s).into())),
			Expr::Delete(s) => crate::db::expr::Expr::Delete(Box::new((*s).into())),
			Expr::Relate(s) => crate::db::expr::Expr::Relate(Box::new((*s).into())),
			Expr::Insert(s) => crate::db::expr::Expr::Insert(Box::new((*s).into())),
			Expr::Define(s) => crate::db::expr::Expr::Define(Box::new((*s).into())),
			Expr::Remove(s) => crate::db::expr::Expr::Remove(Box::new((*s).into())),
			Expr::Rebuild(s) => crate::db::expr::Expr::Rebuild(Box::new((*s).into())),
			Expr::Upsert(s) => crate::db::expr::Expr::Upsert(Box::new((*s).into())),
			Expr::Alter(s) => crate::db::expr::Expr::Alter(Box::new((*s).into())),
			Expr::Info(s) => crate::db::expr::Expr::Info(Box::new((*s).into())),
			Expr::Foreach(s) => crate::db::expr::Expr::Foreach(Box::new((*s).into())),
			Expr::Let(s) => crate::db::expr::Expr::Let(Box::new((*s).into())),
			Expr::Sleep(s) => crate::db::expr::Expr::Sleep(Box::new((*s).into())),
			Expr::Explain {
				format,
				analyze,
				statement,
			} => crate::db::expr::Expr::Explain {
				format: format.into(),
				analyze,
...
impl From<crate::db::expr::Expr> for Expr {
	fn from(v: crate::db::expr::Expr) -> Self {
		match v {
			crate::db::expr::Expr::Literal(l) => Expr::Literal(l.into()),
			crate::db::expr::Expr::Param(p) => Expr::Param(p.into()),
			crate::db::expr::Expr::Idiom(i) => Expr::Idiom(i.into()),
			crate::db::expr::Expr::Table(t) => Expr::Table(t.into_string()),
			crate::db::expr::Expr::Mock(m) => Expr::Mock(m.into()),
			crate::db::expr::Expr::Block(b) => Expr::Block(Box::new((*b).into())),
			crate::db::expr::Expr::Constant(c) => Expr::Constant(c.into()),
			crate::db::expr::Expr::Prefix {
				op,
				expr,
			} => Expr::Prefix {
			...
			crate::db::expr::Expr::FunctionCall(f) => Expr::FunctionCall(Box::new((*f).into())),
			crate::db::expr::Expr::Closure(s) => Expr::Closure(Box::new((*s).into())),
			crate::db::expr::Expr::Break => Expr::Break,
			crate::db::expr::Expr::Continue => Expr::Continue,
			crate::db::expr::Expr::Return(e) => Expr::Return(Box::new((*e).into())),
			crate::db::expr::Expr::Throw(e) => Expr::Throw(Box::new((*e).into())),
			crate::db::expr::Expr::IfElse(s) => Expr::IfElse(Box::new((*s).into())),
			crate::db::expr::Expr::Select(s) => Expr::Select(Box::new((*s).into())),
			crate::db::expr::Expr::Create(s) => Expr::Create(Box::new((*s).into())),
			crate::db::expr::Expr::Update(s) => Expr::Update(Box::new((*s).into())),
			crate::db::expr::Expr::Delete(s) => Expr::Delete(Box::new((*s).into())),
			crate::db::expr::Expr::Relate(s) => Expr::Relate(Box::new((*s).into())),
			crate::db::expr::Expr::Insert(s) => Expr::Insert(Box::new((*s).into())),
			crate::db::expr::Expr::Define(s) => Expr::Define(Box::new((*s).into())),
			crate::db::expr::Expr::Remove(s) => Expr::Remove(Box::new((*s).into())),
			crate::db::expr::Expr::Rebuild(s) => Expr::Rebuild(Box::new((*s).into())),
			crate::db::expr::Expr::Upsert(s) => Expr::Upsert(Box::new((*s).into())),
			crate::db::expr::Expr::Alter(s) => Expr::Alter(Box::new((*s).into())),
			crate::db::expr::Expr::Info(s) => Expr::Info(Box::new((*s).into())),
			crate::db::expr::Expr::Foreach(s) => Expr::Foreach(Box::new((*s).into())),
			crate::db::expr::Expr::Let(s) => Expr::Let(Box::new((*s).into())),
			crate::db::expr::Expr::Sleep(s) => Expr::Sleep(Box::new((*s).into())),
			crate::db::expr::Expr::Explain {
				format,
				analyze,
				statement,
