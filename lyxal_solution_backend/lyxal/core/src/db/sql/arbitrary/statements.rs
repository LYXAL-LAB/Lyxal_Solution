...
use crate::db::sql::access_type::{BearerAccess, BearerAccessSubject};
use crate::db::sql::arbitrary::{
	self, arb_group, arb_opt, arb_order, arb_splits, arb_vec1, atleast_one, insert_data,
};
use crate::db::sql::kind::KindLiteral;
use crate::db::sql::statements::SetStatement;
...
impl<'a> Arbitrary<'a> for DefineAccessStatement {
	fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
		...
		let duration = crate::db::sql::access::AccessDuration {
			grant: u.arbitrary()?,
			token: u.arbitrary()?,
			session: u.arbitrary()?,
		};
