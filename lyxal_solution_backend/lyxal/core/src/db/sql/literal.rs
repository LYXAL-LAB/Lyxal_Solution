...
use crate::utils::fmt::{CoverStmts, EscapeObjectKey, Float, QuoteStr};
use crate::db::sql::{Expr, RecordIdLit};
use crate::types::{
	PublicBytes, PublicDatetime, PublicDuration, PublicFile, PublicGeometry, PublicRegex,
	PublicUuid,
};
use crate::db::val::Geometry;
...
pub enum Literal {
	...
	Integer(i64),
	Decimal(
		#[cfg_attr(feature = "arbitrary", arbitrary(with = crate::db::sql::arbitrary::arb_decimal))]
		Decimal,
	),
...
impl From<Literal> for crate::db::expr::Literal {
	fn from(value: Literal) -> Self {
		match value {
			Literal::None => crate::db::expr::Literal::None,
			Literal::Null => crate::db::expr::Literal::Null,
			Literal::UnboundedRange => crate::db::expr::Literal::UnboundedRange,
			Literal::Bool(x) => crate::db::expr::Literal::Bool(x),
			Literal::Float(x) => crate::db::expr::Literal::Float(x),
			Literal::Integer(x) => crate::db::expr::Literal::Integer(x),
			Literal::Decimal(decimal) => crate::db::expr::Literal::Decimal(decimal),
			Literal::Duration(duration) => crate::db::expr::Literal::Duration(duration.into()),
			Literal::String(strand) => crate::db::expr::Literal::String(strand),
			Literal::RecordId(record_id_lit) => {
				crate::db::expr::Literal::RecordId(record_id_lit.into())
			}
			Literal::Datetime(datetime) => crate::db::expr::Literal::Datetime(datetime.into()),
			Literal::Uuid(uuid) => crate::db::expr::Literal::Uuid(uuid.into()),
			Literal::Regex(regex) => crate::db::expr::Literal::Regex(regex.into()),
			Literal::Array(exprs) => {
				crate::db::expr::Literal::Array(exprs.into_iter().map(From::from).collect())
			}
			Literal::Set(exprs) => {
				crate::db::expr::Literal::Set(exprs.into_iter().map(From::from).collect())
			}
			Literal::Object(items) => convert_geometry(items),
			Literal::Geometry(geometry) => crate::db::expr::Literal::Geometry(geometry.into()),
			Literal::File(file) => crate::db::expr::Literal::File(file.into()),
			Literal::Bytes(bytes) => crate::db::expr::Literal::Bytes(bytes.into()),
		}
	}
}

impl From<crate::db::expr::Literal> for Literal {
	fn from(value: crate::db::expr::Literal) -> Self {
		match value {
			crate::db::expr::Literal::None => Literal::None,
			crate::db::expr::Literal::Null => Literal::Null,
			crate::db::expr::Literal::UnboundedRange => Literal::UnboundedRange,
			crate::db::expr::Literal::Bool(x) => Literal::Bool(x),
			crate::db::expr::Literal::Float(x) => Literal::Float(x),
			crate::db::expr::Literal::Integer(x) => Literal::Integer(x),
			crate::db::expr::Literal::Decimal(decimal) => Literal::Decimal(decimal),
			crate::db::expr::Literal::Duration(duration) => Literal::Duration(duration.into()),
			crate::db::expr::Literal::String(strand) => Literal::String(strand),
			crate::db::expr::Literal::RecordId(record_id_lit) => {
				Literal::RecordId(record_id_lit.into())
			}
			crate::db::expr::Literal::Datetime(datetime) => Literal::Datetime(datetime.into()),
			crate::db::expr::Literal::Uuid(uuid) => Literal::Uuid(uuid.into()),
			crate::db::expr::Literal::Regex(regex) => Literal::Regex(regex.into()),
			crate::db::expr::Literal::Array(exprs) => {
				Literal::Array(exprs.into_iter().map(From::from).collect())
			}
			crate::db::expr::Literal::Set(exprs) => {
				Literal::Set(exprs.into_iter().map(From::from).collect())
			}
			crate::db::expr::Literal::Object(items) => {
				Literal::Object(items.into_iter().map(From::from).collect())
			}
			crate::db::expr::Literal::Geometry(geometry) => Literal::Geometry(geometry.into()),
			crate::db::expr::Literal::File(file) => Literal::File(file.into()),
			crate::db::expr::Literal::Bytes(bytes) => Literal::Bytes(bytes.into()),
		}
	}
}
...
fn convert_geometry(map: Vec<ObjectEntry>) -> crate::db::expr::Literal {
	if let Some(geom) = collect_geometry(&map) {
		crate::db::expr::Literal::Geometry(geom)
	} else {
		crate::db::expr::Literal::Object(map.into_iter().map(From::from).collect())
	}
}
...
impl From<ObjectEntry> for crate::db::expr::literal::ObjectEntry {
	fn from(value: ObjectEntry) -> Self {
		crate::db::expr::literal::ObjectEntry {
			key: value.key,
...
impl From<crate::db::expr::literal::ObjectEntry> for ObjectEntry {
	fn from(value: crate::db::expr::literal::ObjectEntry) -> Self {
		ObjectEntry {
			key: value.key,
			value: value.value.into(),
		}
