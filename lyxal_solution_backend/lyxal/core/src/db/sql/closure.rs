...
use crate::db::sql::{Expr, Kind, Param};
...
impl From<Closure> for crate::db::expr::ClosureExpr {
...
impl From<crate::db::expr::ClosureExpr> for Closure {
	fn from(v: crate::db::expr::ClosureExpr) -> Self {
		Self {
			args: v.args.into_iter().map(|(i, k)| (i.into(), k.into())).collect(),
			returns: v.returns.map(Into::into),
			body: v.body.into(),
