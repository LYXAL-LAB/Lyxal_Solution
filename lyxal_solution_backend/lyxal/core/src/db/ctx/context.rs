...
#[cfg(feature = "lyxalism")]
use crate::bucket::store::ObjectKey;
use crate::bucket::store::ObjectStore;
use crate::db::catalog::providers::{CatalogProvider, DatabaseProvider, NamespaceProvider};
use crate::db::catalog::{DatabaseDefinition, DatabaseId, NamespaceId};
use crate::config::cnf::PROTECTED_PARAM_NAMES;
use crate::db::ctx::canceller::Canceller;
use crate::db::ctx::reason::Reason;
#[cfg(feature = "lyxalism")]
use crate::db::dbs::capabilities::ExperimentalTarget;
#[cfg(feature = "http")]
use crate::db::dbs::capabilities::NetTarget;
use crate::db::dbs::{Capabilities, NewPlannerStrategy, Options, Session, Variables};
use crate::error::Error;
use crate::db::exec::function::FunctionRegistry;
use crate::db::idx::planner::executor::QueryExecutor;
use crate::db::idx::planner::{IterationStage, QueryPlanner};
...
#[cfg(feature = "lyxalism")]
use crate::lyxalism::cache::{LyxalismCache, LyxalismCacheLookup};
...
pub struct Context {
	...
	// The lyxalism cache
	#[cfg(feature = "lyxalism")]
	lyxalism_cache: Option<Arc<LyxalismCache>>,
	...
	matches_context: Option<Arc<crate::db::exec::function::MatchesContext>>,
	// KNN context for index functions (vector::distance::knn)
	knn_context: Option<Arc<crate::db::exec::function::KnnContext>>,
}
...
impl Context {
	...
	pub(crate) fn new(parent: &FrozenContext) -> Self {
		Context {
			...
			#[cfg(feature = "lyxalism")]
			lyxalism_cache: parent.lyxalism_cache.clone(),
	...
	pub(crate) fn snapshot(from: &FrozenContext) -> Self {
		Self {
			...
			#[cfg(feature = "lyxalism")]
			lyxalism_cache: from.lyxalism_cache.clone(),
	...
	pub(crate) fn from_ds(
		...
		#[cfg(feature = "lyxalism")] lyxalism_cache: Arc<LyxalismCache>,
	) -> Result<Context> {
		...
		let mut ctx = Self {
			...
			#[cfg(feature = "lyxalism")]
			lyxalism_cache: Some(lyxalism_cache),
			function_registry: Arc::new(FunctionRegistry::with_builtins()),
	...
	pub(crate) fn cancellation(&self) -> crate::db::ctx::cancellation::Cancellation {
		crate::db::ctx::cancellation::Cancellation::new(
	...
	pub(crate) fn set_matches_context(&mut self, ctx: crate::db::exec::function::MatchesContext) {
	...
	pub(crate) fn get_matches_context(
		&self,
	) -> Option<&Arc<crate::db::exec::function::MatchesContext>> {
		self.matches_context.as_ref()
	}

	/// Set the KNN context for index functions (vector::distance::knn)
	pub(crate) fn set_knn_context(&mut self, ctx: Arc<crate::db::exec::function::KnnContext>) {
		self.knn_context = Some(ctx);
	}

	/// Get the KNN context for index functions
	pub(crate) fn get_knn_context(&self) -> Option<&Arc<crate::db::exec::function::KnnContext>> {
	...
	#[cfg(feature = "lyxalism")]
	pub(crate) fn get_lyxalism_cache(&self) -> Option<Arc<LyxalismCache>> {
		self.lyxalism_cache.as_ref().map(|sc| sc.clone())
	}

	#[cfg(feature = "lyxalism")]
	pub(crate) async fn get_lyxalism_runtime(
		&self,
		lookup: LyxalismCacheLookup<'_>,
	) -> Result<Arc<Runtime>> {
		if !self.get_capabilities().allows_experimental(&ExperimentalTarget::Lyxalism) {
			bail!(
				"Failed to get lyxalism runtime: Experimental capability `lyxalism` is not enabled"
			);
		}

		let Some(cache) = self.get_lyxalism_cache() else {
			bail!("Lyxalism cache is not available");
		};

		cache
			.get_or_insert_with(&lookup, async || {
				let LyxalismCacheLookup::File(ns, db, bucket, key) = lookup else {
				...
				let package = LyxalismPackage::from_reader(std::io::Cursor::new(surli))?;
...
mod tests {
	...
	use crate::config::cnf::MEMORY_THRESHOLD;
	use crate::db::ctx::Context;
	use crate::db::ctx::reason::Reason;
	#[cfg(feature = "http")]
	use crate::db::dbs::Capabilities;
	#[cfg(feature = "http")]
	use crate::db::dbs::capabilities::{NetTarget, Targets};
	...
	async fn test_context_memory_threshold_integration() {
		use crate::error::Error;
		use crate::utils::str::ParseBytes;
		...
		match result {
			...
			Ok(None) => {
				...
				println!(
					"  cargo test --package lyxal-core --features allocation-tracking,allocator test_context_memory_threshold_integration"
				);
