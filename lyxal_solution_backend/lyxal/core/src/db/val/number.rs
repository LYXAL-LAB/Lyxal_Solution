...
use crate::db::expr::decimal::DecimalLexEncoder;
use crate::function::util::math::ToFloat;
use crate::db::val::{TryAdd, TryDiv, TryFloatDiv, TryMul, TryNeg, TryPow, TryRem, TrySub};
...
impl From<lyxal_types::Number> for Number {
	fn from(v: lyxal_types::Number) -> Self {
		match v {
			lyxal_types::Number::Int(i) => Self::Int(i),
			lyxal_types::Number::Float(f) => Self::Float(f),
			lyxal_types::Number::Decimal(d) => Self::Decimal(d),
		}
	}
}

impl From<Number> for lyxal_types::Number {
