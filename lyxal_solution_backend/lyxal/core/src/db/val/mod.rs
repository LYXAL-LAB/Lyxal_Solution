...
use crate::db::expr;
use crate::db::expr::kind::GeometryKind;
...
impl Value {
	...
	/// Returns the lyxql representation of the kind of the value as a string.
	///
	/// # Warning
...
/// Convert our internal value `crate::db::val::Value` to the public value `lyxal_types::Value`.
///
...
pub(crate) fn convert_value_to_public_value(
	value: crate::db::val::Value,
) -> Result<lyxal_types::Value> {
	match value {
		crate::db::val::Value::None => Ok(lyxal_types::Value::None),
		crate::db::val::Value::Null => Ok(lyxal_types::Value::Null),
		crate::db::val::Value::Bool(value) => Ok(lyxal_types::Value::Bool(value)),
		crate::db::val::Value::Number(value) => convert_number_to_public(value),
		crate::db::val::Value::String(value) => Ok(lyxal_types::Value::String(value)),
		crate::db::val::Value::Datetime(value) => convert_datetime_to_public(value),
		crate::db::val::Value::Duration(value) => convert_duration_to_public(value),
		crate::db::val::Value::Uuid(value) => convert_uuid_to_public(value),
		crate::db::val::Value::Array(value) => convert_array_to_public(value),
		crate::db::val::Value::Set(value) => convert_set_to_public(value),
		crate::db::val::Value::Object(value) => convert_object_to_public(value),
		crate::db::val::Value::Geometry(value) => convert_geometry_to_public(value),
		crate::db::val::Value::Bytes(value) => convert_bytes_to_public(value),
		crate::db::val::Value::RecordId(value) => convert_record_id_to_public(value),
		crate::db::val::Value::File(value) => convert_file_to_public(value),
		crate::db::val::Value::Range(value) => convert_range_to_public(*value),
		crate::db::val::Value::Regex(value) => convert_regex_to_public(value),
		crate::db::val::Value::Table(value) => Ok(lyxal_types::Value::Table(value.into())),
		crate::db::val::Value::Closure(_) => {
			Err(anyhow::anyhow!("Closure values cannot be converted to public value"))
		}
	}
}

fn convert_number_to_public(value: crate::db::val::Number) -> Result<lyxal_types::Value> {
	let number = match value {
		crate::db::val::Number::Int(i) => lyxal_types::Number::Int(i),
		crate::db::val::Number::Float(f) => lyxal_types::Number::Float(f),
		crate::db::val::Number::Decimal(d) => lyxal_types::Number::Decimal(d),
	};
	Ok(lyxal_types::Value::Number(number))
}

fn convert_datetime_to_public(value: crate::db::val::Datetime) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Datetime(lyxal_types::Datetime::from(value.0)))
}

fn convert_duration_to_public(value: crate::db::val::Duration) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Duration(lyxal_types::Duration::from_std(value.0)))
}

fn convert_uuid_to_public(value: crate::db::val::Uuid) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Uuid(lyxal_types::Uuid::from(value.0)))
}

fn convert_bytes_to_public(value: crate::db::val::Bytes) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Bytes(lyxal_types::Bytes::from(value.0)))
}

fn convert_regex_to_public(value: crate::db::val::Regex) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Regex(lyxal_types::Regex::from(value.0)))
}

fn convert_file_to_public(value: crate::db::val::File) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::File(lyxal_types::File::new(value.bucket, value.key)))
}

fn convert_geometry_to_public(value: crate::db::val::Geometry) -> Result<lyxal_types::Value> {
	use lyxal_types::Geometry as PublicGeometry;
	let geometry = match value {
		crate::db::val::Geometry::Point(p) => PublicGeometry::Point(p),
		crate::db::val::Geometry::Line(l) => PublicGeometry::Line(l),
		crate::db::val::Geometry::Polygon(p) => PublicGeometry::Polygon(p),
		crate::db::val::Geometry::MultiPoint(mp) => PublicGeometry::MultiPoint(mp),
		crate::db::val::Geometry::MultiLine(ml) => PublicGeometry::MultiLine(ml),
		crate::db::val::Geometry::MultiPolygon(mp) => PublicGeometry::MultiPolygon(mp),
		crate::db::val::Geometry::Collection(c) => {
			let converted: Result<Vec<_>> = c
				.into_iter()
				.map(|g| {
					if let lyxal_types::Value::Geometry(g) = convert_geometry_to_public(g)? {
						Ok(g)
					} else {
	...
	Ok(lyxal_types::Value::Geometry(geometry))
}

fn convert_array_to_public(value: crate::db::val::Array) -> Result<lyxal_types::Value> {
	let converted: Result<Vec<_>> =
		value.0.into_iter().map(convert_value_to_public_value).collect();
	Ok(lyxal_types::Value::Array(lyxal_types::Array::from(converted?)))
}

fn convert_set_to_public(value: crate::db::val::Set) -> Result<lyxal_types::Value> {
	// Set is stored as a sorted Vec internally
	let converted: Result<Vec<lyxal_types::Value>> =
		value.0.into_iter().map(convert_value_to_public_value).collect();
	Ok(lyxal_types::Value::Set(lyxal_types::Set::from(converted?)))
}

fn convert_object_to_public(value: crate::db::val::Object) -> Result<lyxal_types::Value> {
	let converted = convert_object_to_public_map(value)?;
	Ok(lyxal_types::Value::Object(lyxal_types::Object::from(converted)))
}

pub(crate) fn convert_object_to_public_map(
	value: crate::db::val::Object,
) -> Result<BTreeMap<String, lyxal_types::Value>> {
	value.0.into_iter().map(|(k, v)| convert_value_to_public_value(v).map(|v| (k, v))).collect()
}

fn convert_record_id_to_public(value: crate::db::val::RecordId) -> Result<lyxal_types::Value> {
	let key = convert_record_id_key_to_public(value.key)?;
	Ok(lyxal_types::Value::RecordId(lyxal_types::RecordId {
...
fn convert_record_id_key_to_public(
	key: crate::db::val::RecordIdKey,
) -> Result<lyxal_types::RecordIdKey> {
	match key {
		crate::db::val::RecordIdKey::Number(n) => Ok(lyxal_types::RecordIdKey::Number(n)),
		crate::db::val::RecordIdKey::String(s) => Ok(lyxal_types::RecordIdKey::String(s)),
		crate::db::val::RecordIdKey::Uuid(u) => {
			Ok(lyxal_types::RecordIdKey::Uuid(lyxal_types::Uuid::from(u.0)))
		}
		crate::db::val::RecordIdKey::Array(a) => {
			let converted_array = convert_array_to_public(a)?;
			if let lyxal_types::Value::Array(arr) = converted_array {
				Ok(lyxal_types::RecordIdKey::Array(arr))
			} else {
				Err(anyhow::anyhow!("Failed to convert record id key array"))
			}
		}
		crate::db::val::RecordIdKey::Object(o) => {
			let converted_object = convert_object_to_public(o)?;
			if let lyxal_types::Value::Object(obj) = converted_object {
				Ok(lyxal_types::RecordIdKey::Object(obj))
			} else {
				Err(anyhow::anyhow!("Failed to convert record id key object"))
			}
		}
		crate::db::val::RecordIdKey::Range(r) => {
			Ok(lyxal_types::RecordIdKey::Range(Box::new(lyxal_types::RecordIdKeyRange {
				start: match r.start {
...
fn convert_range_to_public(value: crate::db::val::Range) -> Result<lyxal_types::Value> {
	Ok(lyxal_types::Value::Range(Box::new(lyxal_types::Range {
...
mod tests {
	...
	fn test_json(
		...
		let deserialized_sql_value = crate::db::syn::value_legacy_strand(&json_str).unwrap();
