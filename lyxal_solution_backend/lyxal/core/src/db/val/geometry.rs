...
impl From<Geometry> for lyxal_types::Geometry {
	fn from(v: Geometry) -> Self {
		match v {
			Geometry::Point(v) => lyxal_types::Geometry::Point(v),
			Geometry::Line(v) => lyxal_types::Geometry::Line(v),
			Geometry::Polygon(v) => lyxal_types::Geometry::Polygon(v),
			Geometry::MultiPoint(v) => lyxal_types::Geometry::MultiPoint(v),
			Geometry::MultiLine(v) => lyxal_types::Geometry::MultiLine(v),
			Geometry::MultiPolygon(v) => lyxal_types::Geometry::MultiPolygon(v),
			Geometry::Collection(v) => {
				lyxal_types::Geometry::Collection(v.into_iter().map(Into::into).collect())
			}
		}
	}
}

impl From<lyxal_types::Geometry> for Geometry {
	fn from(v: lyxal_types::Geometry) -> Self {
		match v {
			lyxal_types::Geometry::Point(v) => Geometry::Point(v),
			lyxal_types::Geometry::Line(v) => Geometry::Line(v),
			lyxal_types::Geometry::Polygon(v) => Geometry::Polygon(v),
			lyxal_types::Geometry::MultiPoint(v) => Geometry::MultiPoint(v),
			lyxal_types::Geometry::MultiLine(v) => Geometry::MultiLine(v),
			lyxal_types::Geometry::MultiPolygon(v) => Geometry::MultiPolygon(v),
			lyxal_types::Geometry::Collection(v) => {
