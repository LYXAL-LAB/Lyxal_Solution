...
use crate::db::val::{IndexFormat, Value};
...
impl TryFrom<Set> for lyxal_types::Set {
	type Error = anyhow::Error;

	fn try_from(s: Set) -> Result<Self, Self::Error> {
		Ok(lyxal_types::Set::from(
			s.0.into_iter().map(lyxal_types::Value::try_from).collect::<Result<Vec<_>, _>>()?,
		))
	}
}

impl From<lyxal_types::Set> for Set {
	fn from(s: lyxal_types::Set) -> Self {
