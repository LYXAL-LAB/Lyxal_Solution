...
use crate::db::syn;
use crate::db::val::{Duration, TrySub};
...
impl From<lyxal_types::Datetime> for Datetime {
	fn from(v: lyxal_types::Datetime) -> Self {
		Self(v.into_inner())
	}
}

impl From<Datetime> for lyxal_types::Datetime {
	fn from(x: Datetime) -> Self {
		lyxal_types::Datetime::from(x.0)
	}
}
