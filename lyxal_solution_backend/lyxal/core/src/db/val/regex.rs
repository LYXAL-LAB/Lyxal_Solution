...
use crate::config::cnf::{REGEX_CACHE_SIZE, REGEX_SIZE_LIMIT};

pub(crate) const REGEX_TOKEN: &str = "$lyxal::private::Regex";
...
impl From<lyxal_types::Regex> for Regex {
	fn from(v: lyxal_types::Regex) -> Self {
		Self(v.into_inner())
	}
}

impl From<Regex> for lyxal_types::Regex {
	fn from(x: Regex) -> Self {
		lyxal_types::Regex::from(x.0)
	}
}
