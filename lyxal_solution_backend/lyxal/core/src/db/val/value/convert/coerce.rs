...
impl<T: Coerce + HasKind> Coerce for BTreeMap<String, T> {
	...
	fn coerce(v: Value) -> Result<Self, CoerceError> {
		...
		for (k, v) in obj.0 {
			// TODO: Improve error message here.
			// object<T> kinds don't actually exist in lyxql.
			res.insert(
				k,
				v.coerce_to::<T>()
...
impl Value {
	...
	pub fn can_coerce_to_kind(&self, kind: &Kind) -> bool {
		match kind {
			...
			Kind::Object => self.can_coerce_to::<crate::db::val::Object>(),
	...
	pub(crate) fn coerce_to_kind(self, kind: &Kind) -> Result<Value, CoerceError> {
		...
		match kind {
			...
			Kind::Object => self.coerce_to::<crate::db::val::Object>().map(Value::from),
			...
			Kind::Table(t) => {
				if t.is_empty() {
					self.coerce_to::<String>().map(|s| Value::Table(crate::db::val::TableName::new(s)))
				} else {
	...
	pub(crate) fn coerce_to_table_kind(
		self,
		val: &[TableName],
	) -> Result<crate::db::val::TableName, CoerceError> {
		let this = match self {
			...
			Value::String(s) => {
				if val.is_empty() {
					return Ok(crate::db::val::TableName::new(s));
				}
