...
//!
//! The `Id` key stores the mapping between Lyxal record IDs (`Thing`) and
...
use crate::db::key::category::{Categorise, Category};
use crate::db::val::{IndexFormat, RecordIdKey, TableName};
...
impl crate::kvs::KVKey for Id<'_> {
	...
	fn encode_key(&self) -> anyhow::Result<Vec<u8>> {
		Ok(storekey::encode_vec_format::<IndexFormat, _>(self)
			.map_err(|_| crate::error::Error::Unencodable)?)
	}
}
