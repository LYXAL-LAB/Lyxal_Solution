...
use crate::db::catalog::{ApiMethod, EventDefinition, EventKind};
use crate::db::sql::access::AccessDuration;
use crate::db::sql::access_type::JwtAccessVerify;
use crate::db::sql::base::Base;
use crate::db::sql::filter::Filter;
use crate::db::sql::index::{Distance, HnswParams, VectorType};
use crate::db::sql::kind::KindLiteral;
use crate::db::sql::statements::define::config::api::{ApiConfig, Middleware};
use crate::db::sql::statements::define::config::defaults::DefaultConfig;
use crate::db::sql::statements::define::config::graphql::{GraphQLConfig, TableConfig};
use crate::db::sql::statements::define::config::{ConfigInner, graphql};
use crate::db::sql::statements::define::user::PassType;
use crate::db::sql::statements::define::{
...
use crate::db::sql::tokenizer::Tokenizer;
use crate::db::sql::{
...
use crate::db::sql::{ModuleExecutable, SiloExecutable, LyxalismExecutable};
use crate::db::syn::error::bail;
use crate::db::syn::parser::mac::{expected, unexpected};
use crate::db::syn::parser::{ParseResult, Parser};
use crate::db::syn::token::{Token, TokenKind, t};
...
impl Parser<'_> {
	...
	pub(crate) async fn parse_define_module(
		...
		bail!(
			"Lyxalism modules are not supported in WASM environments",
	...
	pub(crate) async fn parse_define_module(
		...
		if !self.settings.lyxalism_enabled {
			bail!(
				"Experimental capability `lyxalism` is not enabled",
		...
		let executable = match peek.kind {
			...
			t!("f\"") | t!("f'") => {
				...
				ModuleExecutable::Lyxalism(LyxalismExecutable(file.into()))
			}
			_ => {
		...
		loop {
			match self.peek_kind() {
				...
				t!("FULLTEXT") => {
					...
					res.index = Index::FullText(crate::db::sql::index::FullTextParams {
