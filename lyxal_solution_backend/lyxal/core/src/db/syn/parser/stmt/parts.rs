...
use crate::db::sql::index::{Distance, VectorType};
use crate::db::sql::reference::{Reference, ReferenceDeleteStrategy};
use crate::db::sql::{
...
use crate::db::syn::token::{DistanceKind, Span, TokenKind, VectorTypeKind, t};
			...
			TokenKind::Distance(k) => {
				let dist = match k {
					...
					DistanceKind::Minkowski => {
						let distance: lyxal_types::Number = self.next_token_value()?;
