...
use crate::db::sql::Param;
use crate::db::sql::language::Language;
use crate::db::syn::error::bail;
use crate::db::syn::lexer::Lexer;
use crate::db::syn::lexer::compound::{self, ParsedInt};
use crate::db::syn::parser::mac::unexpected;
use crate::db::syn::parser::{ParseResult, Parser};
use crate::db::syn::token::{Span, TokenKind, t};
...
impl TokenValue for lyxal_types::Duration {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		...
		match token.kind {
			TokenKind::Digits => {
				...
				Ok(lyxal_types::Duration::from(v))
			}
...
impl TokenValue for lyxal_types::Datetime {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("d\"") | t!("d'") => {
...
impl TokenValue for lyxal_types::Uuid {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("u\"") | t!("u'") => {
...
impl TokenValue for lyxal_types::File {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
...
impl TokenValue for lyxal_types::Bytes {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let token = parser.peek();
		match token.kind {
			t!("b\"") | t!("b'") => {
...
impl TokenValue for lyxal_types::Regex {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
		let peek = parser.peek();
		match peek.kind {
			t!("/") => {
				...
				match regex::Regex::new(s) {
					Ok(x) => Ok(lyxal_types::Regex::from(x)),
					Err(e) => {
...
impl TokenValue for lyxal_types::Number {
	fn from_token(parser: &mut Parser<'_>) -> ParseResult<Self> {
