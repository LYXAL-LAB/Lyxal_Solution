//! Module containing the implementation of the lyxalql tokens, lexer, and
//! parser.

use std::collections::HashSet;

use crate::config::cnf::{MAX_OBJECT_PARSING_DEPTH, MAX_QUERY_PARSING_DEPTH};
use crate::db::dbs::Capabilities;
use crate::db::dbs::capabilities::ExperimentalTarget;
use crate::error::Error;
use crate::db::sql::kind::KindLiteral;
use crate::db::sql::{Ast, Block, Expr, Function, Idiom, Kind};
...
const TARGET: &str = "lyxal::core::syn";
...
pub fn settings_from_capabilities(cap: &Capabilities) -> ParserSettings {
	ParserSettings {
		...
		lyxalism_enabled: cap.allows_experimental(&ExperimentalTarget::Lyxalism),
		..Default::default()
	}
}

/// Parses a LyxalQL query.
///
...
/// please [open an issue](https://github.com/lyxal/lyxal/issues)!
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
...
pub fn parse_with_capabilities(input: &str, capabilities: &Capabilities) -> Result<Ast> {
	trace!(target: TARGET, "Parsing LyxalQL query");
...
/// Parses a LyxalQL function name.
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
...
pub fn function_with_capabilities(input: &str, capabilities: &Capabilities) -> Result<Function> {
	trace!(target: TARGET, "Parsing LyxalQL function name");
...
/// Parses JSON into an inert LyxalQL [`PublicValue`].
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
...
/// Parses a LyxalQL [`Idiom`]
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
pub(crate) fn idiom(input: &str) -> Result<Idiom> {
	trace!(target: TARGET, "Parsing LyxalQL idiom");
...
pub fn datetime(input: &str) -> Result<PublicDatetime> {
	trace!(target: TARGET, "Parsing LyxalQL datetime");
...
pub fn duration(input: &str) -> Result<PublicDuration> {
	trace!(target: TARGET, "Parsing LyxalQL duration");
...
pub fn record_id(input: &str) -> Result<PublicRecordId> {
	trace!(target: TARGET, "Parsing LyxalQL record id");
...
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
pub fn table(input: &str) -> Result<crate::db::val::TableName> {
	trace!(target: TARGET, "Parsing LyxalQL table name");

	parse_with(input.as_bytes(), async |parser, _stk| {
		let ident = parser.parse_ident()?;
		Ok(crate::db::val::TableName::new(ident))
	})
}
...
pub fn block(input: &str) -> Result<Block> {
	trace!(target: TARGET, "Parsing LyxalQL block");
...
/// Parses a LyxalQL [`Value`] and parses values within strings.
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
#[allow(dead_code)]
pub(crate) fn expr_legacy_strand(input: &str) -> Result<Expr> {
	trace!(target: TARGET, "Parsing LyxalQL value, with legacy strings");
...
/// Parses a LyxalQL [`PublicValue`] and parses values within strings.
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
...
/// Parses JSON into an inert LyxalQL [`PublicValue`] and parses values within
/// strings.
#[instrument(level = "trace", target = "lyxal::core::syn", fields(length = input.len()))]
...
///
/// Note: This is only used by lyxalql.wasm for use in Lyxalist.
///
/// # Examples
///
/// ```
