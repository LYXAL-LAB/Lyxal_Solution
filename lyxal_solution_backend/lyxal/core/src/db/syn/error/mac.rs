...
macro_rules! syntax_error {
	($format:literal $(, $expr:expr_2021)*
		...
		let __error: $crate::db::syn::error::SyntaxError = $crate::db::syn::error::SyntaxError::new(format_args!($format $(, $expr)*));
		$(
			$crate::db::syn::error::syntax_error!(#label __error, $span $(=> $label_format$(, $label_expr)*  )?);
		)*
		__error
	}};

	(#label $name:ident, $span:expr_2021 => $label_format:literal $(, $label_expr:expr_2021)* ) => {
		let $name = $name.with_labeled_span($span,$crate::db::syn::error::MessageKind::Error, format_args!($label_format $(, $label_expr)*));
	};

	(#label $name:ident, $span:expr_2021 ) => {
	    let $name = $crate::db::syn::error::SyntaxError::with_span($name,$span, $crate::db::syn::error::MessageKind::Error);
	};
}
...
macro_rules! bail {
	($($t:tt)*) => {{
		let __error = $crate::db::syn::error::syntax_error!($($t)*);
		return Err(__error)
	}};
}
