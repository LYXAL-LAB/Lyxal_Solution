...
//! at execution time. Since LyxalQL allows DDL and DML interleaving within transactions,
...
use crate::db::exec::{DatabaseContext, EvalContext, ExecutionContext, PhysicalExpr};
use crate::db::iam::{Action, Auth};
...
pub async fn convert_permission_to_physical(
	...
	match permission {
		...
		Permission::Specific(expr) => {
			...
			let physical_expr =
				crate::db::exec::planner::expr_to_physical_expr(expr.clone(), ctx).await?;
