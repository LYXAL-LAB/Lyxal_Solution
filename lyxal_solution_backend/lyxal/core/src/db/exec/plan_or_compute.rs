...
use crate::db::exec::context::{ContextLevel, ExecutionContext};
use crate::db::exec::planner::try_plan_expr;
use crate::db::exec::{FlowResult, ValueBatchStream};
use crate::db::expr::part::{Part, RecurseInstruction};
use crate::db::expr::statements::InfoStatement;
use crate::db::expr::{Base, Block, ControlFlow, ControlFlowExt, Expr, Literal};
...
) -> Result<(&crate::db::dbs::Options, FrozenContext), Error> {
...
pub(crate) fn get_legacy_context_with_param<'a>(
	...
	param_name: &str,
	param_value: &Value,
) -> Result<(&'a crate::db::dbs::Options, FrozenContext), Error> {
	let options = exec_ctx
		.options()
		.ok_or_else(|| Error::Thrown("Options not available for legacy compute fallback".into()))?;

	let mut ctx = crate::db::ctx::Context::new(exec_ctx.ctx());
...
pub(crate) async fn legacy_compute(
	...
	opt: &crate::db::dbs::Options,
	doc: Option<&crate::db::doc::CursorDoc>,
) -> crate::db::expr::FlowResult<Value> {
...
pub(crate) async fn collect_single_value(
	...
	if values.is_empty() {
		...
		Ok(Value::Array(crate::db::val::Array(values)))
	}
}
...
fn record_id_key_required_context(key: &crate::db::expr::RecordIdKeyLit) -> ContextLevel {
...
fn idiom_required_context(idiom: &crate::db::expr::Idiom) -> ContextLevel {
