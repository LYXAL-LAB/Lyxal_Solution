...
use crate::db::catalog::{Index, IndexDefinition};
use crate::db::exec::planner::util::try_literal_to_value;
use crate::db::expr::operator::{MatchesOperator, NearestNeighbor, PrefixOperator};
use crate::db::expr::order::Ordering;
use crate::db::expr::with::With;
use crate::db::expr::{BinaryOperator, Cond, Expr, Idiom};
	...
	pub fn try_in_expansion(
		...
		for (idiom, values) in &in_exprs {
			...
			for (idx, ix_def) in self.indexes.iter().enumerate() {
				...
				if !matches!(ix_def.index, crate::db::catalog::Index::Idx | crate::db::catalog::Index::Uniq)
				{
					continue;
				}
...
// literal_to_value and expr_to_value are imported from crate::db::exec::planner::util
