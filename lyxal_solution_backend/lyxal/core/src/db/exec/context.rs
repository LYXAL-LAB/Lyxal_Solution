...
use crate::db::dbs::{Capabilities, Options};
use crate::error::Error;
use crate::db::exec::function::FunctionRegistry;
use crate::db::expr::Base;
use crate::db::iam::{Action, Auth, ResourceKind};
...
/// Shared cache of [`FieldState`](crate::db::exec::operators::scan::pipeline::FieldState)
/// keyed by `(table_name, check_perms)`.
pub(crate) type FieldStateCache = Arc<
	tokio::sync::RwLock<
		HashMap<
			(crate::db::val::TableName, bool),
			Arc<crate::db::exec::operators::scan::pipeline::FieldState>,
		>,
	>,
>;
...
pub struct DatabaseContext {
	...
	pub(crate) table_def_cache: Arc<
		tokio::sync::RwLock<
			HashMap<crate::db::val::TableName, Option<Arc<crate::db::catalog::TableDefinition>>>,
		>,
	>,
	...
	pub(crate) index_def_cache: Arc<
		tokio::sync::RwLock<HashMap<crate::db::val::TableName, Arc<[crate::db::catalog::IndexDefinition]>>>,
	>,
}
...
impl DatabaseContext {
	...
	pub(crate) async fn get_table_def(
		&self,
		table: &crate::db::val::TableName,
	) -> anyhow::Result<Option<Arc<crate::db::catalog::TableDefinition>>> {
	...
	pub(crate) async fn get_table_indexes(
		&self,
		table: &crate::db::val::TableName,
	) -> anyhow::Result<Arc<[crate::db::catalog::IndexDefinition]>> {
		use crate::db::catalog::providers::TableProvider;
		// Check execution-level cache (read lock — concurrent reads allowed)
		{
...
impl ExecutionContext {
	...
	pub fn with_limited_auth(&self, limit: &crate::db::iam::AuthLimit) -> Self {
		let mut new = self.clone();
		let root = match &mut new {
			Self::Root(r) => r,
			Self::Namespace(n) => &mut n.root,
