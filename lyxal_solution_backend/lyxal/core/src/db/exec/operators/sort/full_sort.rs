...
impl ExecOperator for Sort {
	...
	fn output_ordering(&self) -> crate::db::exec::OutputOrdering {
		...
		crate::db::exec::OutputOrdering::Sorted(
			self.order_by
				.iter()
				.map(|f| {
					...
					let path = crate::db::exec::field_path::FieldPath::field(sql);
					SortProperty {
						path,
		...
		let sorted_stream = futures::stream::once(async move {
			...
			while let Some(batch_result) = input_stream.next().await {
				...
				if ctx.cancellation().is_cancelled() {
					return Err(crate::db::expr::ControlFlow::Err(anyhow::anyhow!(
...
) -> Result<Vec<Value>, crate::db::expr::ControlFlow> {
	spawn_blocking(move || {
