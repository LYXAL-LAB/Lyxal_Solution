...
use crate::db::catalog::providers::{AuthorisationProvider, DatabaseProvider, UserProvider};
	...
	fn execute(&self, ctx: &ExecutionContext) -> FlowResult<ValueBatchStream> {
		...
		Ok(Box::pin(stream::once(async move {
			match execute_namespace_info(&ctx, structured).await {
				...
				Err(e) => Err(crate::db::expr::ControlFlow::Err(e)),
			}
		})))
	}
...
async fn execute_namespace_info(ctx: &ExecutionContext, structured: bool) -> Result<Value> {
	...
	// Allowed to run?
	opt.is_allowed(Action::View, ResourceKind::Any, &crate::db::expr::Base::Ns)?;
