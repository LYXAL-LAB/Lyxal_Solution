...
use crate::db::expr::part::{DestructurePart, Part};
...
) -> Result<(), crate::db::expr::ControlFlow> {
	if field.expr.is_projection_function()
...
fn omit_nested_field(value: &mut Value, idiom: &Idiom, depth: usize) {
	...
	match part {
		...
		Part::Value(expr) => {
			...
			if let crate::db::expr::Expr::Literal(crate::db::expr::Literal::Integer(idx)) = expr
...
) -> Result<Value, crate::db::expr::ControlFlow> {
