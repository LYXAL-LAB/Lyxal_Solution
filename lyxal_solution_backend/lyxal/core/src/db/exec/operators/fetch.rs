...
use crate::db::expr::ControlFlowExt;
...
fn fetch_field_path<'a>(
	...
	value: &'a mut Value,
	path: &'a [Part],
) -> crate::db::exec::BoxFut<'a, crate::db::expr::FlowResult<()>> {
	Box::pin(async move {
...
pub(crate) async fn fetch_raw_record(
	...
	rid: &RecordId,
	version: Option<u64>,
) -> crate::db::expr::FlowResult<Option<Value>> {
...
pub(crate) async fn batch_fetch_records(
	ctx: &ExecutionContext,
	rids: &[RecordId],
) -> crate::db::expr::FlowResult<Vec<Value>> {
	if rids.is_empty() {
		return Ok(Vec::new());
	}
...
mod tests {
	use super::*;
	use crate::db::exec::physical_expr::Literal;

	#[test]
	fn test_fetch_name() {
		use crate::db::exec::operators::scan::DynamicScan;
		use crate::db::expr::part::Part;
		...
		let scan = Arc::new(DynamicScan::new(
			Arc::new(Literal(Value::from("test"))) as Arc<dyn crate::db::exec::PhysicalExpr>,
			None,
			None,
			None,
			None,
