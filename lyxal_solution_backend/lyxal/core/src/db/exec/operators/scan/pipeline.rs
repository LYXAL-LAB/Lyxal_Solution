...
use crate::db::exec::permission::{
	PhysicalPermission, check_permission_for_value, convert_permission_to_physical,
};
use crate::db::exec::planner::expr_to_physical_expr;
use crate::db::exec::{EvalContext, ExecutionContext, PhysicalExpr, ValueBatch, ValueBatchStream};
use crate::db::expr::{ControlFlow, ControlFlowExt};
use crate::db::idx::planner::ScanDirection;
use crate::kvs::{KVKey, KVValue, Transaction};
use crate::db::val::{RecordIdKey, TableName, Value};

/// A raw computed field entry before topological sorting:
/// `(field_name, physical_expr, optional_kind, dependency_field_names)`.
type RawComputedField = (String, Arc<dyn PhysicalExpr>, Option<crate::db::expr::Kind>, Vec<String>);
...
pub(crate) fn determine_scan_direction(
	...
	use crate::db::expr::order::Ordering as OrderingType;
...
pub(crate) fn decode_record(key: &[u8], val: Vec<u8>) -> Result<Value, ControlFlow> {
	let decoded_key =
		crate::db::key::record::RecordKey::decode_key(key).context("Failed to decode record key")?;

	let rid = crate::db::val::RecordId {
		table: decoded_key.tb.into_owned(),
		key: decoded_key.id,
	};

	let mut record =
		crate::db::catalog::Record::kv_decode_value(val).context("Failed to deserialize record")?;
...
pub(crate) fn range_start_key(
	...
	match bound {
		Bound::Unbounded => {
			crate::db::key::record::prefix(ns_id, db_id, table).context("Failed to create prefix key")
		}
		Bound::Included(v) => {
			crate::db::key::record::new(ns_id, db_id, table, v).encode_key().context("Failed to create begin key")
		}
		Bound::Excluded(v) => {
			let mut key = crate::db::key::record::new(ns_id, db_id, table, v)
				.encode_key()
...
pub(crate) fn range_end_key(
	...
	match bound {
		Bound::Unbounded => {
			crate::db::key::record::suffix(ns_id, db_id, table).context("Failed to create suffix key")
		}
		Bound::Excluded(v) => {
			crate::db::key::record::new(ns_id, db_id, table, v).encode_key().context("Failed to create end key")
		}
		Bound::Included(v) => {
			let mut key = crate::db::key::record::new(ns_id, db_id, table, v)
				.encode_key()
...
pub(crate) struct FieldState {
	...
	dep_map: Arc<HashMap<String, crate::db::expr::computed_deps::ComputedDeps>>,
}
...
pub(crate) struct ComputedFieldDef {
	...
	kind: Option<crate::db::expr::Kind>,
}
...
pub(crate) async fn build_field_state_raw(
	...
	ns_id: crate::db::catalog::NamespaceId,
	db_id: crate::db::catalog::DatabaseId,
	...
	let has_field_perms = check_perms
		&& field_defs
			.iter()
			.any(|fd| !matches!(fd.select_permission, crate::db::catalog::Permission::Full));
	...
	let mut dep_map: HashMap<String, crate::db::expr::computed_deps::ComputedDeps> = HashMap::new();
	...
	for fd in field_defs.iter() {
		if let Some(ref expr) = fd.computed {
			...
			let deps = if let Some(ref cd) = fd.computed_deps {
				crate::db::expr::computed_deps::ComputedDeps {
					fields: cd.fields.clone(),
					is_complete: cd.is_complete,
				}
			} else {
				crate::db::expr::computed_deps::extract_computed_deps(expr)
			};
	...
	let sorted_indices = crate::db::expr::computed_deps::topological_sort_computed_fields(&topo_input);
...
pub(crate) fn filter_field_state_for_projection(
	...
	let required =
		crate::db::expr::computed_deps::resolve_required_computed_fields(needed, &full_state.dep_map);
