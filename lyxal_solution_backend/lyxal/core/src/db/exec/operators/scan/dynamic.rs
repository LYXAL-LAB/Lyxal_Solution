...
use crate::db::catalog::{DatabaseId, NamespaceId, Permission};
use crate::error::Error;
use crate::db::exec::index::access_path::{AccessPath, select_access_path};
use crate::db::exec::index::analysis::IndexAnalyzer;
use crate::db::exec::operators::scan::pipeline::ScanPipeline;
use crate::db::exec::permission::{
	PhysicalPermission, convert_permission_to_physical, should_check_perms,
	validate_record_user_access,
};
use crate::db::exec::planner::util::{resolve_condition_params, strip_knn_from_condition};
use crate::db::exec::{
	AccessMode, ContextLevel, EvalContext, ExecOperator, ExecutionContext, FlowResult,
	OperatorMetrics, PhysicalExpr, ValueBatch, ValueBatchStream, monitor_stream,
};
use crate::db::expr::order::Ordering;
use crate::db::expr::with::With;
use crate::db::expr::{Cond, ControlFlow, ControlFlowExt};
use crate::db::iam::Action;
use crate::db::idx::planner::ScanDirection;
use crate::db::val::{TableName, Value};
...
pub struct DynamicScan {
	...
	pub(crate) knn_context: Option<Arc<crate::db::exec::function::KnnContext>>,
}
		...
		let stream = async_stream::try_stream! {
			...
			while let Some(batch_result) = source.next().await {
				...
				if ctx.cancellation().is_cancelled() {
					Err(ControlFlow::Err(
						anyhow::anyhow!(crate::error::Error::QueryCancelled),
					))?;
				}
...
async fn resolve_table_scan_stream(
	...
	match access_path {
		...
		_ => {
			let beg = crate::db::key::record::prefix(cfg.ns_id, cfg.db_id, &cfg.table_name)?;
			let end = crate::db::key::record::suffix(cfg.ns_id, cfg.db_id, &cfg.table_name)?;
...
mod tests {
	...
	async fn create_test_scan(table_name: &str, with_index_hints: bool) -> DynamicScan {
		...
		let source = expr_to_physical_expr(
			crate::db::expr::Expr::Literal(crate::db::expr::literal::Literal::String(
