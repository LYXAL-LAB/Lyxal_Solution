...
use crate::db::catalog::{DatabaseId, Index, NamespaceId, Permission};
use crate::error::Error;
use crate::db::exec::operators::scan::index_count::sum_index_count_deltas;
...
use crate::db::iam::Action;
use crate::kvs::{KVKey, KVValue};
use crate::db::val::{Number, Object, RecordIdKey, TableName, Value};
		...
		let stream = async_stream::try_stream! {
			...
			let version: Option<u64> = match &version {
				Some(expr) => {
					...
					Some(
						v.cast_to::<crate::db::val::Datetime>()
			...
			let count = if let Some(ref rid) = rid {
				...
				if let Some(count) = count_from_index {
					...
					let beg = crate::db::key::record::prefix(ns.namespace_id, db.database_id, &table_name)?;
					let end = crate::db::key::record::suffix(ns.namespace_id, db.database_id, &table_name)?;
...
async fn count_range(
	...
	match key {
		...
		_ => {
			...
			let record_key = crate::db::key::record::new(ns_id, db_id, table, key);
			let exists = txn
...
async fn count_with_perm_fallback(
	...
	rid: Option<&crate::db::val::RecordId>,
	version: Option<u64>,
	...
	let (beg, end) = if let Some(rid) = rid {
		match &rid.key {
			...
			_ => {
				...
				let Some(value) =
					crate::db::exec::operators::fetch::fetch_raw_record(ctx, rid, version).await?
				else {
					return Ok(0);
				};
		...
		let beg = crate::db::key::record::prefix(ns_id, db_id, table_name)?;
		let end = crate::db::key::record::suffix(ns_id, db_id, table_name)?;
		(beg, end)
	};
	...
	let kv_stream = txn.stream_keys_vals(
		...
		None, // no limit
		0,    // no skip
		crate::db::idx::planner::ScanDirection::Forward,
	...
	while let Some(result) = kv_stream.next().await {
		...
		for (key, val) in entries {
			let decoded_key = crate::db::key::record::RecordKey::decode_key(&key)
				.context("Failed to decode record key")?;
			let rid_val = crate::db::val::RecordId {
				table: decoded_key.tb.into_owned(),
				key: decoded_key.id,
			};
			let mut record = crate::db::catalog::Record::kv_decode_value(val)
