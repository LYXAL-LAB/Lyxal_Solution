...
use crate::db::val::{RecordId, RecordIdKey, Value};
...
impl ExecOperator for RecordIdScan {
	...
	fn cardinality_hint(&self) -> crate::db::exec::CardinalityHint {
		...
		crate::db::exec::CardinalityHint::AtMostOne
	}
...
pub(crate) async fn execute_record_lookup(
	...
	let (select_permission, field_state) = if let Some(res) = resolved {
		...
		if table_def.is_none() {
			return Err(ControlFlow::Err(anyhow::Error::new(crate::error::Error::TbNotFound {
