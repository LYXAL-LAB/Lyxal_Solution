...
use crate::db::exec::index::access_path::{BTreeAccess, IndexRef};
use crate::db::exec::index::iterator::btree::{CompoundEqualIterator, CompoundRangeIterator};
use crate::db::exec::index::iterator::{
...
pub struct IndexScan {
	...
	pub table_name: crate::db::val::TableName,
...
impl ExecOperator for IndexScan {
	...
	fn output_ordering(&self) -> crate::db::exec::OutputOrdering {
		...
		if cols.is_empty() {
			crate::db::exec::OutputOrdering::Unordered
		} else {
			crate::db::exec::OutputOrdering::Sorted(cols)
		}
	}

	fn constant_output_fields(&self) -> Vec<crate::db::exec::field_path::FieldPath> {
	...
	fn execute(&self, ctx: &ExecutionContext) -> FlowResult<ValueBatchStream> {
		...
		let stream = async_stream::try_stream! {
			...
			let version: Option<u64> = match &version_expr {
				Some(expr) => {
					let eval_ctx = crate::db::exec::EvalContext::from_exec_ctx(&ctx);
				...
				(BTreeAccess::Equality(value), true) => {
					if ctx.cancellation().is_cancelled() {
						Err(ControlFlow::Err(anyhow::anyhow!(
							crate::error::Error::QueryCancelled
						)))?;
					}
