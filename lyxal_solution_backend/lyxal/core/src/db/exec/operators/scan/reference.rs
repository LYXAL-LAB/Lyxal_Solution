...
use crate::db::val::{RecordId, TableName};
			...
			while let Some(batch_result) = input_stream.next().await {
				...
				for rid in &target_rids {
					...
					while let Some(result) = kv_stream.next().await {
						...
						for key in keys {
							let decoded = crate::db::key::r#ref::Ref::decode_key(&key)
...
async fn compute_ref_key_range(
	...
	if has_range {
		...
		if let Some(field) = referencing_field {
			// Field-specific scan
			let beg = crate::db::key::r#ref::ffprefix(ns_id, db_id, &rid.table, &rid.key, table, field)
				.context("Failed to create field prefix")?;
			let end = crate::db::key::r#ref::ffsuffix(ns_id, db_id, &rid.table, &rid.key, table, field)
				.context("Failed to create field suffix")?;
			Ok((beg, end))
		} else {
			// Table-only scan (all fields)
			let beg = crate::db::key::r#ref::ftprefix(ns_id, db_id, &rid.table, &rid.key, table)
				.context("Failed to create table prefix")?;
			let end = crate::db::key::r#ref::ftsuffix(ns_id, db_id, &rid.table, &rid.key, table)
				.context("Failed to create table suffix")?;
			Ok((beg, end))
		}
	} else {
		// Wildcard scan: all references for this record (any table, any field)
		let beg = crate::db::key::r#ref::prefix(ns_id, db_id, &rid.table, &rid.key)
			.context("Failed to create wildcard prefix")?;
		let end = crate::db::key::r#ref::suffix(ns_id, db_id, &rid.table, &rid.key)
...
async fn eval_ref_bound(
	...
	match bound {
		Bound::Unbounded => {
			if is_start {
				crate::db::key::r#ref::ffprefix(ns_id, db_id, &rid.table, &rid.key, table, field)
					.context("Failed to create field prefix")
			} else {
				crate::db::key::r#ref::ffsuffix(ns_id, db_id, &rid.table, &rid.key, table, field)
					.context("Failed to create field suffix")
			}
		}
		Bound::Included(expr) => {
			...
			if is_start {
				crate::db::key::r#ref::refprefix(ns_id, db_id, &rid.table, &rid.key, table, field, &fk)
					.context("Failed to create range key")
			} else {
				crate::db::key::r#ref::refsuffix(ns_id, db_id, &rid.table, &rid.key, table, field, &fk)
					.context("Failed to create range key")
			}
		}
		Bound::Excluded(expr) => {
...
mod tests {
	use super::*;
	use crate::db::exec::operators::CurrentValueSource;
