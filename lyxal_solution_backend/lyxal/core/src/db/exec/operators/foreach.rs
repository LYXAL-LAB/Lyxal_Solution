...
use crate::db::exec::plan_or_compute::{
...
use crate::db::expr::{Block, ControlFlow, ControlFlowExt, Expr, Param};
use crate::db::val::Value;
use crate::db::val::range::IntegerRangeIter;
...
async fn execute_foreach(
...
) -> crate::db::expr::FlowResult<ValueBatch> {
	...
	for v in iter {
		...
		if ctx.cancellation().is_cancelled() {
			return Err(ControlFlow::Err(anyhow::anyhow!(crate::error::Error::QueryCancelled)));
		}
