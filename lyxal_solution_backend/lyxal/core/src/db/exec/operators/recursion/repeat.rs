...
//! ```
//!
//! Example LyxalQL (repeat recurse builds a nested tree per record):
//!
//! ```lyxql
...
use crate::db::exec::parts::{clean_iteration, evaluate_physical_path, get_final, is_final};
use crate::db::exec::physical_expr::{EvalContext, PhysicalExpr, RecursionCtx};
use crate::db::exec::{ExecutionContext, FlowResult};
...
pub(crate) fn evaluate_repeat_recurse<'a>(
	value: &'a Value,
	ctx: EvalContext<'a>,
) -> crate::db::exec::BoxFut<'a, FlowResult<Value>> {
	Box::pin(async move {
		let rec_ctx = match &ctx.recursion_ctx {
			Some(rc) => rc.clone(),
			None => {
				// RepeatRecurse outside recursion context is an error
				return Err(crate::error::Error::UnsupportedRepeatRecurse.into());
			}
		};
				...
				Value::Array(arr) => {
					...
					for v in arr.iter() {
						...
						if !is_recursion_target(v) {
							return Err(crate::error::Error::InvalidRecursionTarget {
								value: v.to_sql(),
							}
							.into());
						}
		...
		Err(crate::error::Error::UnsupportedRepeatRecurse.into())
	})
}
...
pub(crate) async fn evaluate_recurse_iterative(
	...
	min_depth: u32,
	max_depth: u32,
	body: &Option<Arc<dyn crate::db::exec::ExecOperator>>,
