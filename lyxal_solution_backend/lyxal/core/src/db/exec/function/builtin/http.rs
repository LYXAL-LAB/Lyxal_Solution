...
async fn http_disabled() -> Result<Value> {
	Err(anyhow::anyhow!(crate::error::Error::HttpDisabled))
}
...
fn extract_uri(args: &[Value], fn_name: &str) -> Result<String> {
	match args.first() {
		...
		Some(v) => Err(anyhow::anyhow!(crate::error::Error::InvalidFunctionArguments {
		...
		None => Err(anyhow::anyhow!(crate::error::Error::InvalidFunctionArguments {
			name: fn_name.to_owned(),
...
async fn http_head_impl(ctx: &EvalContext<'_>, args: Vec<Value>) -> Result<Value> {
	...
	let url = url::Url::parse(&uri).map_err(|_| crate::error::Error::InvalidUrl(uri.clone()))?;
...
async fn http_request(
	...
	let cli = {
		...
		let capabilities_clone: Arc<crate::db::dbs::Capabilities> = Arc::clone(&capabilities);
	...
	// Add body if present
	if let Some(b) = body {
		let public_body = crate::db::val::convert_value_to_public_value(b)?;
