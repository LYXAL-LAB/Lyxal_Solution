...
//!
//! This module converts LyxalQL AST expressions (`Expr`) into physical execution
...
use crate::config::cnf::MAX_COMPUTATION_DEPTH;
use crate::db::ctx::FrozenContext;
use crate::db::dbs::NewPlannerStrategy;
use crate::error::Error;
use crate::db::exec::ExecOperator;
...
use crate::db::exec::physical_expr::{
	...
	RecordIdExpr, ScalarSubquery, SetLiteral, SiloModuleExec, LyxalismModuleExec, UnaryOp,
	UserDefinedFunctionExec,
};
use crate::db::expr::statements::IfelseStatement;
use crate::db::expr::{Expr, Function, FunctionCall};
...
impl<'ctx> Planner<'ctx> {
	...
	pub async fn physical_expr(
		&self,
		expr: Expr,
	) -> Result<Arc<dyn crate::db::exec::PhysicalExpr>, Error> {
		match expr {
			...
			Expr::Table(t) => Ok(Arc::new(PhysicalLiteral(crate::db::val::Value::Table(t)))),
	...
	async fn physical_binary_expr(
		&self,
		left: Expr,
		op: crate::db::expr::operator::BinaryOperator,
		right: Expr,
	) -> Result<Arc<dyn crate::db::exec::PhysicalExpr>, Error> {
		// For MATCHES operators with idiom left and string-literal or
		// bind-parameter right, create a MatchesOp that evaluates via
		// the full-text index.
		if let crate::db::expr::operator::BinaryOperator::Matches(ref matches_op) = op
			&& let Expr::Idiom(idiom) = left
		{
			let resolved_query = match &right {
				Expr::Literal(crate::db::expr::literal::Literal::String(s)) => Some(s.clone()),
			...
			if let Some(query) = resolved_query {
				...
				let right_phys = Box::pin(
					self.physical_expr(Expr::Literal(crate::db::expr::literal::Literal::String(query))),
				)
				.await?;
				return Ok(Arc::new(crate::db::exec::physical_expr::MatchesOp::new(
					left_phys,
					right_phys,
					matches_op.clone(),
					idiom_clone,
		...
		if is_simple_binary_eligible(&op) {
			...
			{
				return Ok(Arc::new(crate::db::exec::physical_expr::SimpleBinaryOp {
	...
	async fn physical_prefix_expr(
		&self,
		op: crate::db::expr::operator::PrefixOperator,
		expr: Expr,
	) -> Result<Arc<dyn crate::db::exec::PhysicalExpr>, Error> {
	...
	async fn physical_postfix_expr(
		&self,
		op: crate::db::expr::operator::PostfixOperator,
		expr: Expr,
	) -> Result<Arc<dyn crate::db::exec::PhysicalExpr>, Error> {
		use crate::db::expr::operator::PostfixOperator;
			...
			Function::Normal(name) => {
				...
				if registry.is_projection(&name) {
					let func_ctx = registry
						...
						.unwrap_or(crate::db::exec::ContextLevel::Database);
					...
					let func_ctx = registry
						.get(&name)
						.map(|f| f.required_context())
						.unwrap_or(crate::db::exec::ContextLevel::Root);
	...
	async fn physical_args(
		&self,
		args: Vec<Expr>,
	) -> Result<Vec<Arc<dyn crate::db::exec::PhysicalExpr>>, Error> {
	...
	pub async fn physical_expr_as_name(
		...
		use crate::db::exec::physical_expr::Literal as PhysicalLiteral;
		...
		{
			return Ok(Arc::new(PhysicalLiteral(crate::db::val::Value::String(name.clone()))));
		}

		if let Expr::Table(name) = expr {
			return Ok(Arc::new(PhysicalLiteral(crate::db::val::Value::String(name.to_string()))));
		}
	...
	fn convert_record_key_to_physical<'a>(
		&'a self,
		key: &'a crate::db::expr::RecordIdKeyLit,
	) -> crate::db::exec::BoxFut<
		'a,
		Result<crate::db::exec::physical_expr::record_id::PhysicalRecordIdKey, Error>,
	> {
		Box::pin(async move {
			use crate::db::exec::physical_expr::record_id::PhysicalRecordIdKey;
	...
	async fn convert_bound_to_physical(
		&self,
		bound: &std::ops::Bound<crate::db::expr::RecordIdKeyLit>,
	) -> Result<
		std::ops::Bound<Box<crate::db::exec::physical_expr::record_id::PhysicalRecordIdKey>>,
		Error,
	> {
		match bound {
	...
	fn plan_expr(
		&self,
		expr: Expr,
	) -> crate::db::exec::BoxFut<'_, Result<Arc<dyn ExecOperator>, Error>> {
		Box::pin(async move {
			match expr {
	...
	async fn plan_block(&self, block: crate::db::expr::Block) -> Result<Arc<dyn ExecOperator>, Error> {
		if block.0.is_empty() {
			use crate::db::exec::physical_expr::Literal as PhysicalLiteral;
			Ok(Arc::new(ExprPlan::new(Arc::new(PhysicalLiteral(crate::db::val::Value::None))))
	...
	async fn plan_return_statement(
		&self,
		output_stmt: crate::db::expr::statements::OutputStatement,
	...
	async fn plan_explain_statement(
		&self,
		format: crate::db::expr::ExplainFormat,
		analyze: bool,
		statement: Expr,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let inner_plan = self.plan_expr(statement).await?;
	...
	async fn plan_let_statement(
		&self,
		let_stmt: crate::db::expr::statements::SetStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let crate::db::expr::statements::SetStatement {
			name,
			what,
			kind: _,
		} = let_stmt;
		...
		Ok(Arc::new(crate::db::exec::operators::LetPlan::new(name, value)))
	}

	async fn plan_info_statement(
		&self,
		info: crate::db::expr::statements::info::InfoStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		use crate::db::expr::statements::info::InfoStatement;
		match info {
	...
	fn plan_foreach_statement(
		&self,
		stmt: crate::db::expr::statements::ForeachStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let crate::db::expr::statements::ForeachStatement {
			param,
			range,
			block,
		} = stmt;
	...
	fn plan_sleep_statement(
		&self,
		sleep_stmt: crate::db::expr::statements::SleepStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
...
macro_rules! try_plan_expr {
	($expr:expr, $ctx:expr, $txn:expr) => {{
		let __expr: &$crate::db::expr::Expr = $expr;
		if matches!(
			__expr,
			$crate::db::expr::Expr::Create(_)
				| $crate::db::expr::Expr::Update(_)
				| $crate::db::expr::Expr::Upsert(_)
				| $crate::db::expr::Expr::Delete(_)
				| $crate::db::expr::Expr::Insert(_)
				| $crate::db::expr::Expr::Relate(_)
				| $crate::db::expr::Expr::Define(_)
				| $crate::db::expr::Expr::Remove(_)
				| $crate::db::expr::Expr::Rebuild(_)
				| $crate::db::expr::Expr::Alter(_)
		) {
			Err($crate::error::Error::PlannerUnsupported(String::new()))
		} else if *$ctx.new_planner_strategy() == $crate::db::dbs::NewPlannerStrategy::ComputeOnly {
			Err($crate::error::Error::PlannerUnsupported(String::new()))
		} else {
			$crate::db::exec::planner::plan_expr_inner(__expr, $ctx, $txn).await
		}
	}};
}
...
pub(crate) async fn plan_expr_inner(
	...
	let ns =
		ctx.value("session").and_then(|v| v.as_object()).and_then(|o| o.get("ns")).and_then(|v| {
			match v {
				crate::db::val::Value::String(s) => Some(s.clone()),
				_ => None,
			}
		});
	let db =
...
fn is_simple_binary_eligible(op: &crate::db::expr::operator::BinaryOperator) -> bool {
	use crate::db::expr::operator::BinaryOperator;
	matches!(
		op,
...
mod planner_tests {
	...
	async fn test_planner_creates_let_operator() {
		let expr = Expr::Let(Box::new(crate::db::expr::statements::SetStatement {
			name: "x".to_string(),
			what: Expr::Literal(crate::db::expr::literal::Literal::Integer(42)),
			kind: None,
		}));
	...
	async fn test_planner_creates_scalar_plan() {
		let expr = Expr::Literal(crate::db::expr::literal::Literal::Integer(42));
