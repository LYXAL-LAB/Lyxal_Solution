...
use crate::db::exec::parts::LookupDirection;
use crate::db::exec::planner::select::SelectPipelineConfig;
...
impl<'ctx> Planner<'ctx> {
	...
	pub(crate) async fn plan_index_function(
		...
		use crate::db::exec::function::{IndexContext, IndexContextKind};
		use crate::db::exec::physical_expr::function::IndexFunctionExec;
	...
	) -> Result<std::ops::Bound<Arc<dyn crate::db::exec::PhysicalExpr>>, Error> {
		match bound {
	...
	pub(crate) async fn plan_lookup(
		&self,
		lookup: crate::db::expr::lookup::Lookup,
	...
	pub(crate) async fn plan_lookup_with_input(
		...
		let base_scan: Arc<dyn ExecOperator> = match &kind {
			crate::db::expr::lookup::LookupKind::Graph(dir) => {
				let mut edge_tables: Vec<EdgeTableSpec> = Vec::with_capacity(what.len());
				for s in what {
					let spec = match s {
						crate::db::expr::lookup::LookupSubject::Table {
							table,
							..
						} => EdgeTableSpec {
							table,
						...
						crate::db::expr::lookup::LookupSubject::Range {
							table,
							range,
							..
						} => {
				...
				let scan =
					if cond.is_none() && split.is_none() && order.is_none() && group.is_none() {
						if let Some(crate::db::expr::limit::Limit(crate::db::expr::Expr::Literal(
							crate::db::expr::Literal::Integer(n),
						))) = &limit
						{
							let offset = match &start {
								Some(crate::db::expr::start::Start(crate::db::expr::Expr::Literal(
									crate::db::expr::Literal::Integer(s),
								))) => *s as usize,
								_ => 0,
							};
			...
			crate::db::expr::lookup::LookupKind::Reference => {
		...
		if needs_full_pipeline {
			...
			let split_op: Arc<dyn ExecOperator> = if let Some(splits) = split {
				Arc::new(crate::db::exec::operators::Split::new(
					filtered,
			...
			let sorted: Arc<dyn ExecOperator> = match order {
				Some(crate::db::expr::order::Ordering::Order(order_list)) => {
					let order_by = self.convert_order_list(order_list).await?;
					Arc::new(crate::db::exec::operators::Sort::new(split_op, order_by))
				}
				Some(crate::db::expr::order::Ordering::Random) => {
