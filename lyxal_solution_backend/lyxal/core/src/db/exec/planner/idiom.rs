...
//!
//! Converts LyxalQL idiom AST nodes to physical part expressions for runtime evaluation.
...
use crate::db::exec::operators::{CurrentValueSource, RecursionOp};
use crate::db::exec::parts::{
...
use crate::db::exec::physical_expr::IdiomExpr;
use crate::db::exec::{ExecOperator, PhysicalExpr};
use crate::db::expr::part::{DestructurePart as AstDestructurePart, Part, RecurseInstruction};
...
impl<'ctx> Planner<'ctx> {
	...
	pub(crate) async fn convert_idiom(
		&self,
		idiom: crate::db::expr::idiom::Idiom,
	...
	pub(crate) fn convert_parts(
		&self,
		parts: Vec<Part>,
	) -> crate::db::exec::BoxFut<'_, Result<Vec<Arc<dyn PhysicalExpr>>, Error>> {
		Box::pin(async move {
			...
			while let Some(part) = iter.next() {
				...
				if let Part::Recurse(recurse, None, instruction) = part {
					let system_limit = *crate::config::cnf::IDIOM_RECURSION_LIMIT as u32;
					let (min_depth, max_depth) = match recurse {
						crate::db::expr::part::Recurse::Fixed(n) => (n, Some(n)),
						crate::db::expr::part::Recurse::Range(min, max) => (min.unwrap_or(1), max),
					};
		...
		match part {
			...
			Part::Lookup(lookup) => {
				...
				let direction = match &lookup.kind {
					crate::db::expr::lookup::LookupKind::Graph(dir) => LookupDirection::from(dir),
					crate::db::expr::lookup::LookupKind::Reference => LookupDirection::Reference,
				};
	...
	) -> crate::db::exec::BoxFut<'_, Result<Vec<DestructureField>, Error>> {
		Box::pin(async move {
...
fn lookup_metadata(lookup: &crate::db::expr::lookup::Lookup) -> (LookupDirection, bool) {
	let direction = match &lookup.kind {
		crate::db::expr::lookup::LookupKind::Graph(dir) => LookupDirection::from(dir),
		crate::db::expr::lookup::LookupKind::Reference => LookupDirection::Reference,
	};
