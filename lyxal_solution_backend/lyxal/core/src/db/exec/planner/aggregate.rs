...
use crate::db::expr::visit::{MutVisitor, VisitMut};
use crate::db::expr::{Expr, Function, FunctionCall, Idiom, Literal};
...
impl<'ctx> Planner<'ctx> {
	...
	pub(crate) async fn plan_aggregation(
		&self,
		fields: &Fields,
		group_by: &[crate::db::expr::idiom::Idiom],
	) -> Result<(Vec<AggregateField>, Vec<Arc<dyn crate::db::exec::PhysicalExpr>>), Error> {
	...
	pub(crate) async fn extract_aggregate_info(
		&self,
		mut expr: Expr,
	) -> Result<(Option<AggregateExprInfo>, Option<Arc<dyn crate::db::exec::PhysicalExpr>>), Error> {
...
struct AggregateExtractor<'a> {
	registry: &'a crate::db::exec::function::FunctionRegistry,
...
impl<'a> AggregateExtractor<'a> {
	fn new(registry: &'a crate::db::exec::function::FunctionRegistry) -> Self {
		Self {
			registry,
			aggregates: Vec::new(),
			aggregate_count: 0,
...
impl MutVisitor for AggregateExtractor<'_> {
	...
	fn visit_mut_select(
		&mut self,
		_s: &mut crate::db::expr::statements::SelectStatement,
	) -> Result<(), Self::Error> {
		Ok(())
	}
	...
	fn visit_mut_create(
		&mut self,
		_s: &mut crate::db::expr::statements::CreateStatement,
	) -> Result<(), Self::Error> {
		Ok(())
	}
	...
	fn visit_mut_update(
		&mut self,
		_s: &mut crate::db::expr::statements::UpdateStatement,
	) -> Result<(), Self::Error> {
		Ok(())
	}
	...
	fn visit_mut_delete(
		&mut self,
		_s: &mut crate::db::expr::statements::DeleteStatement,
	) -> Result<(), Self::Error> {
		Ok(())
	}
}
