...
use crate::db::catalog::providers::{DatabaseProvider, NamespaceProvider, TableProvider};
use crate::config::cnf::MAX_ORDER_LIMIT_PRIORITY_QUEUE_SIZE;
use crate::error::Error;
use crate::db::exec::expression_registry::{ComputePoint, ExpressionRegistry, resolve_order_by_alias};
use crate::db::exec::field_path::FieldPath;
use crate::db::exec::index::access_path::{AccessPath, BTreeAccess, select_access_path};
use crate::db::exec::index::analysis::IndexAnalyzer;
#[cfg(all(storage, not(target_family = "wasm")))]
use crate::db::exec::operators::ExternalSort;
use crate::db::exec::operators::scan::determine_scan_direction;
use crate::db::exec::operators::scan::resolved::{ResolvedTableContext, resolve_table_context};
use crate::db::exec::operators::{
...
use crate::db::exec::{ExecOperator, OperatorMetrics};
use crate::db::expr::field::{Field, Fields};
use crate::db::expr::{Cond, Expr, Idiom, Literal};
...
pub(crate) struct SelectPipelineConfig {
	pub cond: Option<crate::db::expr::cond::Cond>,
	pub split: Option<crate::db::expr::split::Splits>,
	pub group: Option<crate::db::expr::group::Groups>,
	pub order: Option<crate::db::expr::order::Ordering>,
	pub limit: Option<crate::db::expr::limit::Limit>,
	pub start: Option<crate::db::expr::start::Start>,
	pub omit: Vec<Expr>,
	...
	pub precompiled_predicate: Option<Arc<dyn crate::db::exec::PhysicalExpr>>,
}
...
impl<'ctx> Planner<'ctx> {
	...
	fn can_eliminate_sort(
		...
		order: &crate::db::expr::order::Ordering,
	) -> bool {
		use crate::db::exec::operators::SortDirection;
		use crate::db::exec::ordering::SortProperty;
		use crate::db::expr::order::Ordering;
		...
		let required: Vec<SortProperty> = order_list
			...
			.filter_map(|field| {
				...
				crate::db::exec::field_path::FieldPath::try_from(&field.value).ok().map(|path| {
	...
	fn register_and_push_projection(
		...
		registry: &mut ExpressionRegistry,
		expr_key: String,
		physical: Arc<dyn crate::db::exec::PhysicalExpr>,
		output_name: String,
	) {
	...
	pub(crate) async fn plan_omit(
		&self,
		omit: Vec<Expr>,
	) -> Result<Vec<crate::db::expr::idiom::Idiom>, Error> {
	...
	pub(crate) async fn plan_fetch(
		&self,
		fetch: Option<crate::db::expr::fetch::Fetchs>,
		input: Arc<dyn ExecOperator>,
	) -> Result<Arc<dyn ExecOperator>, Error> {
	...
	pub(crate) async fn plan_sort(
		...
		input: Arc<dyn ExecOperator>,
		order: crate::db::expr::order::Ordering,
		start: &Option<crate::db::expr::start::Start>,
		limit: &Option<crate::db::expr::limit::Limit>,
	...
	pub(crate) fn extract_needed_fields(
		...
		omit: &[Expr],
		cond: Option<&Cond>,
		order: Option<&crate::db::expr::order::Ordering>,
		group: Option<&crate::db::expr::group::Groups>,
		split: Option<&crate::db::expr::split::Splits>,
	) -> Option<std::collections::HashSet<String>> {
		use crate::db::expr::Part;
		use crate::db::expr::visit::{Visit, Visitor};
		...
		impl Visitor for NeededFieldExtractor {
			...
			fn visit_idiom(&mut self, idiom: &crate::db::expr::Idiom) -> Result<(), Self::Error> {
		...
		if let Some(ordering) = order {
			match ordering {
				crate::db::expr::order::Ordering::Random => {}
				crate::db::expr::order::Ordering::Order(order_list) => {
	...
	pub(crate) async fn plan_select_statement(
		&self,
		mut select: crate::db::expr::statements::SelectStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let explain = select.explain.take();
		let plan = Box::pin(self.plan_select_core(select)).await?;
		match explain {
			Some(crate::db::expr::explain::Explain(full)) => {
				if full {
					Ok(Arc::new(AnalyzePlan {
						plan,
						format: crate::db::expr::ExplainFormat::Json,
	...
	async fn plan_select_core(
		&self,
		select: crate::db::expr::statements::SelectStatement,
	) -> Result<Arc<dyn ExecOperator>, Error> {
		let crate::db::expr::statements::SelectStatement {
			fields,
			omit,
			only,
			what,
		...
		if is_count_all_eligible(&fields, &group, &cond, &split, &order, &fetch, &omit, &what) {
			use crate::db::exec::operators::CountScan;
			let table_expr = self
		...
		if is_indexed_count_eligible(&fields, &group, &cond, &split, &order, &fetch, &omit, &what)
			&& !matches!(with, Some(crate::db::expr::with::With::NoIndex))
		{
			...
			if has_count_idx || btree_access.is_some() {
				use crate::db::exec::operators::scan::index_count::IndexCountScan;
				let table_expr = self
		...
		let primary_table = what
			...
			.or_else(|| {
				what.iter().find_map(|e| match e {
					Expr::Param(p) => {
						if let Some(crate::db::val::Value::Table(t)) = self.ctx.value(p.as_str()) {
							Some(t.clone())
						} else {
							None
						}
		...
		let planning_ctx: std::borrow::Cow<'_, crate::db::ctx::FrozenContext> =
			if let Some(ref c) = cond {
				let mc = extract_matches_context(c, Some(self.ctx));
				let hm = !mc.is_empty();
				if hm || has_knn_early {
					let mut child = crate::db::ctx::Context::new(self.ctx);
					if hm {
						let mut mc = mc;
					...
					if has_knn_early {
						child.set_knn_context(std::sync::Arc::new(
							crate::db::exec::function::KnnContext::new(),
						));
					}
		...
		let source_is_single_scan = what.len() == 1
			...
			|| (what.len() == 1
				&& matches!(&what[0], Expr::Param(p) if {
					matches!(self.ctx.value(p.as_str()), Some(crate::db::val::Value::Table(_)))
				}));
	...
	pub(crate) async fn plan_sources(
		&self,
		what: Vec<Expr>,
		version: Option<Arc<dyn crate::db::exec::PhysicalExpr>>,
		cond: Option<&Cond>,
		order: Option<&crate::db::expr::order::Ordering>,
		with: Option<&crate::db::expr::with::With>,
		needed_fields: Option<std::collections::HashSet<String>>,
		scan_predicate: Option<Arc<dyn crate::db::exec::PhysicalExpr>>,
		scan_limit: Option<Arc<dyn crate::db::exec::PhysicalExpr>>,
		scan_start: Option<Arc<dyn crate::db::exec::PhysicalExpr>>,
	...
	pub(crate) async fn plan_source(
		...
		use crate::db::exec::operators::{FullTextScan, IndexScan, KnnScan};
		...
		match expr {
			...
			Expr::Literal(crate::db::expr::literal::Literal::RecordId(rid)) => {
				let record_id_expr = self
					.physical_expr(Expr::Literal(crate::db::expr::literal::Literal::RecordId(rid)))
					.await?;
				Ok(PlannedSource {
			...
			Expr::Param(ref param) => match self.ctx.value(param.as_str()) {
				Some(crate::db::val::Value::Table(_)) => {
					let filter_action = if scan_predicate.is_some() {
						FilterAction::FullyConsumed
					} else {
						FilterAction::UseOriginal
				...
				Some(crate::db::val::Value::RecordId(_)) => {
	...
	async fn try_resolve_table_ctx(
		...
		ctx: &crate::db::ctx::FrozenContext,
		ns: &str,
		db: &str,
		table_name: &crate::db::val::TableName,
	) -> Option<crate::db::exec::operators::scan::resolved::ResolvedTableContext> {
	...
	async fn has_matching_count_index(&self, what: &[Expr], cond: &Option<Cond>) -> bool {
		...
		indexes.iter().any(|ix| {
			if let crate::db::catalog::Index::Count(ref idx_cond) = ix.index {
	...
	async fn resolve_count_btree_access(
		...
		crate::db::exec::index::access_path::IndexRef,
		crate::db::exec::index::access_path::BTreeAccess,
	)> {
	...
	async fn resolve_access_path(
		...
		order: Option<&crate::db::expr::order::Ordering>,
		with: Option<&crate::db::expr::with::With>,
	) -> Result<Option<(AccessPath, crate::db::idx::planner::ScanDirection)>, Error> {
		let direction = determine_scan_direction(&order.cloned());

		if matches!(with, Some(crate::db::expr::with::With::NoIndex)) {
...
fn adjust_direction_for_order(
	path: AccessPath,
	order: Option<&crate::db::expr::order::Ordering>,
	default_direction: crate::db::idx::planner::ScanDirection,
) -> (AccessPath, crate::db::idx::planner::ScanDirection) {
	use crate::db::exec::field_path::FieldPath;
	use crate::db::exec::index::access_path::BTreeAccess;
	use crate::db::expr::order::Ordering;
...
fn detect_order_by_id_only(order: Option<&crate::db::expr::order::Ordering>) -> Option<SortDirection> {
	use crate::db::expr::order::Ordering;
