...
use crate::db::catalog::Distance;
use crate::db::catalog::providers::DatabaseProvider;
use crate::error::Error;
use crate::db::exec::field_path::{FieldPath, FieldPathPart};
use crate::db::exec::function::FunctionRegistry;
use crate::db::expr::field::{Field, Fields};
use crate::db::expr::operator::NearestNeighbor;
use crate::db::expr::visit::{MutVisitor, Visit, VisitMut, Visitor};
use crate::db::expr::{BinaryOperator, Cond, Expr, Idiom, Literal, Param};
use crate::db::val::Number;
...
pub(crate) fn try_literal_to_value(
	lit: &crate::db::expr::literal::Literal,
) -> Option<crate::db::val::Value> {
	use crate::db::expr::literal::Literal;
	...
	match lit {
		...
		Literal::RecordId(rid) => {
			...
			use crate::db::expr::RecordIdKeyLit;
			let key = match &rid.key {
				RecordIdKeyLit::Number(n) => crate::db::val::RecordIdKey::Number(*n),
				RecordIdKeyLit::String(s) => crate::db::val::RecordIdKey::String(s.clone()),
				RecordIdKeyLit::Uuid(u) => crate::db::val::RecordIdKey::Uuid(*u),
				_ => return None,
			};
			Some(Value::RecordId(crate::db::val::RecordId::new(rid.table.clone(), key)))
		}
		Literal::Array(arr) => {
...
pub(crate) fn try_expr_to_value(expr: &Expr) -> Option<crate::db::val::Value> {
	match expr {
...
impl MutVisitor for ExpressionFolder<'_> {
	...
	fn visit_mut_select(
		&mut self,
		_: &mut crate::db::expr::SelectStatement,
	) -> Result<(), Self::Error> {
...
fn try_eval_binary(
	op: &BinaryOperator,
	left: crate::db::val::Value,
	right: crate::db::val::Value,
) -> Option<crate::db::val::Value> {
	use crate::db::val::{TryAdd, TrySub};
...
pub(super) fn literal_to_value(
	lit: crate::db::expr::literal::Literal,
) -> Result<crate::db::val::Value, Error> {
	use crate::db::expr::literal::Literal;
	use crate::db::val::{Range, Value};
...
pub(super) fn key_lit_to_expr(lit: &crate::db::expr::RecordIdKeyLit) -> Result<Expr, Error> {
	use crate::db::expr::RecordIdKeyLit;
	match lit {
		RecordIdKeyLit::Number(n) => Ok(Expr::Literal(crate::db::expr::literal::Literal::Integer(*n))),
		RecordIdKeyLit::String(s) => {
			Ok(Expr::Literal(crate::db::expr::literal::Literal::String(s.clone())))
		}
		RecordIdKeyLit::Uuid(u) => Ok(Expr::Literal(crate::db::expr::literal::Literal::Uuid(*u))),
		RecordIdKeyLit::Array(exprs) => {
			Ok(Expr::Literal(crate::db::expr::literal::Literal::Array(exprs.clone())))
		}
		RecordIdKeyLit::Object(entries) => {
			Ok(Expr::Literal(crate::db::expr::literal::Literal::Object(entries.clone())))
		}
...
impl Visitor for KnnOperatorChecker {
	...
	fn visit_select(&mut self, _: &crate::db::expr::SelectStatement) -> Result<(), Self::Error> {
		Ok(())
	}
}
...
pub(crate) fn strip_index_conditions(
	cond: &Cond,
	access: &crate::db::exec::index::access_path::BTreeAccess,
	cols: &[Idiom],
) -> Option<Cond> {
	let mut expr = cond.0.clone();
...
struct IndexConditionStripper<'a> {
	...
	access: &'a crate::db::exec::index::access_path::BTreeAccess,
}
...
struct ParamResolver<'a> {
	values: &'a std::collections::HashMap<String, crate::db::val::Value>,
}
...
pub(crate) async fn resolve_condition_params(
	...
	ns_db: Option<(crate::db::catalog::NamespaceId, crate::db::catalog::DatabaseId)>,
) -> Cond {
...
fn find_id_equality_in_and_chain(expr: &Expr, table_name: &crate::db::val::TableName) -> Option<Expr> {
	match expr {
...
fn check_id_recordid_pair(
	...
	if let Expr::Idiom(idiom) = idiom_side
		...
		&& !matches!(rid.key, crate::db::expr::RecordIdKeyLit::Range(_))
	{
		Some(lit_side.clone())
	} else {
		None
...
pub(super) fn extract_matches_context(
	cond: &Cond,
	ctx: Option<&crate::db::ctx::FrozenContext>,
) -> crate::db::exec::function::MatchesContext {
	let mut collector = MatchesCollector(crate::db::exec::function::MatchesContext::new(), ctx);
	let _ = collector.visit_expr(&cond.0);
	collector.0
}
...
struct MatchesCollector<'a>(
	crate::db::exec::function::MatchesContext,
	Option<&'a crate::db::ctx::FrozenContext>,
);
	...
	fn visit_expr(&mut self, expr: &Expr) -> Result<(), Self::Error> {
		...
		} = expr && let Expr::Idiom(idiom) = left.as_ref()
		{
				...
				Expr::Param(param) => {
					...
					self.1.and_then(|ctx| {
						ctx.value(param.as_str()).and_then(|v| {
							if let crate::db::val::Value::String(s) = v {
								Some(s.clone())
							} else {
								None
							}
			...
			if let Some(query) = query_str {
				...
				self.0.insert(
					match_ref,
					crate::db::exec::function::MatchInfo {
						idiom: idiom.clone(),
						query,
					},
				);
...
pub(super) fn extract_table_from_context(ctx: &crate::db::ctx::FrozenContext) -> crate::db::val::TableName {
	if let Some(mc) = ctx.get_matches_context()
		&& let Some(table) = mc.table()
	{
		return table.clone();
	}
	crate::db::val::TableName::from("unknown".to_string())
}
...
) -> Result<Option<std::sync::Arc<dyn crate::db::exec::PhysicalExpr>>, Error> {
	match version_expr {
...
pub(super) fn order_is_scan_compatible(order: &Option<crate::db::expr::order::Ordering>) -> bool {
...
pub(super) fn index_covers_ordering(
	index_ref: &crate::db::exec::index::access_path::IndexRef,
	access: &crate::db::exec::index::access_path::BTreeAccess,
	direction: crate::db::idx::planner::ScanDirection,
	order: &crate::db::expr::order::Ordering,
) -> bool {
	use crate::db::exec::index::access_path::BTreeAccess;
	use crate::db::exec::operators::SortDirection;
	use crate::db::exec::ordering::{OutputOrdering, SortProperty};
	use crate::db::expr::order::Ordering;
	...
	let (skip_cols, equality_field_paths) = match access {
		BTreeAccess::Compound {
			...
			let paths: Vec<_> = ix_def
				...
				.iter()
				.take(prefix.len())
				.filter_map(|idiom| crate::db::exec::field_path::FieldPath::try_from(idiom).ok())
				.collect();
			(prefix.len(), paths)
		}
	...
	let dir = match direction {
		crate::db::idx::planner::ScanDirection::Forward => SortDirection::Asc,
		crate::db::idx::planner::ScanDirection::Backward => SortDirection::Desc,
	};
	let mut cols: Vec<SortProperty> = ix_def
		...
		.skip(skip_cols)
		.filter_map(|idiom| {
			crate::db::exec::field_path::FieldPath::try_from(idiom).ok().map(|path| SortProperty {
				path,
				direction: dir,
				collate: false,
				numeric: false,
	...
	if !index_ref.is_unique() && !ix_def.cols.is_empty() {
		cols.push(SortProperty {
			path: crate::db::exec::field_path::FieldPath::field("id"),
			direction: dir,
			collate: false,
			numeric: false,
		});
...
pub(super) fn derive_field_name(expr: &Expr) -> String {
	match expr {
		...
		Expr::FunctionCall(call) => {
			let idiom: crate::db::expr::idiom::Idiom = call.receiver.to_idiom();
...
pub(super) fn idiom_to_field_name(idiom: &crate::db::expr::idiom::Idiom) -> String {
	use lyxal_types::ToSql;
...
pub(super) fn idiom_to_field_path(idiom: &crate::db::expr::idiom::Idiom) -> FieldPath {
	use lyxal_types::ToSql;
...
pub(super) fn is_indexed_count_eligible(
	...
	cond: &Option<crate::db::expr::cond::Cond>,
...
pub(super) fn is_count_all_eligible(
	...
	matches!(
		&what[0],
		Expr::Table(_)
			| Expr::Literal(crate::db::expr::literal::Literal::RecordId(_))
			| Expr::Param(_)
			| Expr::Postfix { .. }
	)
}
