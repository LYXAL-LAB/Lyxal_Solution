...
use crate::db::exec::function::MethodDescriptor;
use crate::db::exec::physical_expr::function::validate_return;
use crate::db::exec::physical_expr::{BlockPhysicalExpr, EvalContext, PhysicalExpr};
use crate::db::exec::{AccessMode, CombineAccessModes, ContextLevel};
...
impl PhysicalExpr for MethodPart {
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		...
		match result {
			Ok(v) => Ok(v),
			Err(e) => {
				if let Some(crate::error::Error::InvalidFunctionArguments {
					message,
					..
				}) = e.downcast_ref::<crate::error::Error>()
				{
					Err(crate::error::Error::InvalidMethodArguments {
		...
		match closure.as_ref() {
			Closure::Expr {
				...
				let block_expr = BlockPhysicalExpr {
					block: crate::db::expr::Block(vec![body.clone()]),
				};
				...
				let result = match block_expr.evaluate(eval_ctx).await {
					Ok(v) => v,
					Err(crate::db::expr::ControlFlow::Return(v)) => v,
