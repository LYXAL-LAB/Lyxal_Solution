...
use crate::db::exec::{AccessMode, ContextLevel, ExecOperator};
use crate::db::expr::FlowResult;
...
impl From<crate::db::expr::Dir> for LookupDirection {
	fn from(dir: crate::db::expr::Dir) -> Self {
		match dir {
			crate::db::expr::Dir::Out => LookupDirection::Out,
			crate::db::expr::Dir::In => LookupDirection::In,
			crate::db::expr::Dir::Both => LookupDirection::Both,
		}
	}
}
...
async fn evaluate_lookup(
	...
	match value {
		...
		Value::Array(arr) => {
			...
			// This matches Lyxal semantics: `->edge` on an array of records
...
async fn evaluate_lookup_for_value(
	...
	let stream = lookup.plan.execute(&bound_ctx).map_err(|e| match e {
		crate::db::expr::ControlFlow::Err(e) => e,
		crate::db::expr::ControlFlow::Return(v) => {
			anyhow::anyhow!("Unexpected return in lookup: {:?}", v)
		}
		crate::db::expr::ControlFlow::Break => anyhow::anyhow!("Unexpected break in lookup"),
		crate::db::expr::ControlFlow::Continue => anyhow::anyhow!("Unexpected continue in lookup"),
	})?;
	...
	while let Some(batch_result) = stream.next().await {
		let batch = batch_result.map_err(|e| match e {
			...
			crate::db::expr::ControlFlow::Break => anyhow::anyhow!("Unexpected break in lookup"),
			crate::db::expr::ControlFlow::Continue => {
