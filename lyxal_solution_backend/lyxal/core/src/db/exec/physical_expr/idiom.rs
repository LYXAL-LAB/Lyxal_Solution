...
use crate::db::exec::{AccessMode, CombineAccessModes, ContextLevel, ExecOperator};
...
impl PhysicalExpr for IdiomExpr {
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> crate::db::expr::FlowResult<Value> {
		...
		let value = if let Some(ref start) = self.start_expr {
			...
			// This matches legacy LyxalQL behavior where undefined identifiers
...
///
/// In LyxalQL, certain operations like field access, `.*`, and destructuring
...
/// over each element of the array. This implements LyxalQL's idiom continuity
