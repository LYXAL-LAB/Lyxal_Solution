...
use crate::db::exec::{AccessMode, ContextLevel};
use crate::db::expr::FlowResult;
use crate::db::expr::idiom::Idiom;
use crate::db::expr::operator::MatchesOperator;
use crate::db::idx::ft::fulltext::{FullTextIndex, QueryTerms};
		...
		self.ft_cache
			.get_or_try_init(|| async {
				...
				let index_def = indexes.iter().find(|idx| {
					matches!(&idx.index, crate::db::catalog::Index::FullText(_))
				...
				let ft_params = match &index_def.index {
					crate::db::catalog::Index::FullText(params) => params,
					_ => unreachable!("Already checked for FullText above"),
				};

				let ikb =
					crate::db::idx::IndexKeyBase::new(ns_id, db_id, table_name, index_def.index_id);
...
fn extract_record_id(value: Option<&Value>) -> Result<crate::db::val::RecordId, anyhow::Error> {
	let current =
...
fn extract_table_from_value(value: &Value) -> Option<crate::db::val::TableName> {
	match value {
		Value::Object(obj) => match obj.get("id") {
