...
use crate::db::exec::{AccessMode, ExecOperator};
...
pub struct BinaryOp {
	...
	pub(crate) op: crate::db::expr::operator::BinaryOperator,
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		...
		Ok(match &self.op {
			...
			BinaryOperator::Range => {
				...
				Value::Range(Box::new(crate::db::val::Range {
...
impl PhysicalExpr for SimpleBinaryOp {
	...
	fn required_context(&self) -> crate::db::exec::ContextLevel {
		...
		crate::db::exec::ContextLevel::Database
	}
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		...
		let field_val: &Value = if let Value::Object(obj) = current {
			...
			_owned =
				crate::db::exec::parts::field::evaluate_field(current, &self.field_name, ctx).await?;
			&_owned
		};
...
#[derive(Debug, Clone)]
pub struct UnaryOp {
	pub(crate) op: crate::db::expr::operator::PrefixOperator,
...
impl PhysicalExpr for UnaryOp {
	...
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		use crate::db::expr::operator::PrefixOperator;
		use crate::function::operate;
...
pub struct PostfixOp {
	pub(crate) op: crate::db::expr::operator::PostfixOperator,
	pub(crate) expr: Arc<dyn PhysicalExpr>,
}
