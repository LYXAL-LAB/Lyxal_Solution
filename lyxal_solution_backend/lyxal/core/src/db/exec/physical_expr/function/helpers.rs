...
use crate::db::exec::{AccessMode, CombineAccessModes};
use crate::db::expr::{FlowResult, Kind};
...
pub(crate) fn args_required_context(args: &[Arc<dyn PhysicalExpr>]) -> crate::db::exec::ContextLevel {
	args.iter().map(|e| e.required_context()).max().unwrap_or(crate::db::exec::ContextLevel::Root)
}
