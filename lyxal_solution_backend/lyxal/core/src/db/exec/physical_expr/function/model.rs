...
use crate::db::exec::AccessMode;
use crate::db::exec::physical_expr::{EvalContext, PhysicalExpr};
use crate::db::expr::{FlowResult, Model};
...
impl PhysicalExpr for ModelFunctionExec {
	...
	fn required_context(&self) -> crate::db::exec::ContextLevel {
		// ML models are stored in the database, and arguments
		// may have their own context requirements
		args_required_context(&self.arguments).max(crate::db::exec::ContextLevel::Database)
	}

	#[cfg(feature = "ml")]
	async fn evaluate(&self, ctx: EvalContext<'_>) -> FlowResult<Value> {
		use lyxalml_core::errors::error::LyxalError;
		use lyxalml_core::execution::compute::ModelComputation;
		use lyxalml_core::ndarray as mlNdarray;
		use lyxalml_core::storage::surml_file::SurMlFile;
		...
		match argument {
			...
			Value::Object(v) => {
				...
				let bytes = crate::telemetry::obs::get(&path).await?;

				// Run the compute in a blocking task
				let outcome: Vec<f32> = tokio::task::spawn_blocking(move || {
					let mut file = SurMlFile::from_bytes(bytes).map_err(|err: LyxalError| {
						anyhow::anyhow!("Failed to load model: {}", err.message)
					})?;
					let compute_unit = ModelComputation {
						surml_file: &mut file,
					};
					compute_unit.buffered_compute(&mut args).map_err(|err: LyxalError| {
			...
			Value::Number(v) => {
				...
				let outcome: Vec<f32> = tokio::task::spawn_blocking(move || {
					...
					compute_unit.raw_compute(tensor, None).map_err(|err: LyxalError| {
						anyhow::anyhow!("Model computation failed: {}", err.message)
					})
				})
				.await
