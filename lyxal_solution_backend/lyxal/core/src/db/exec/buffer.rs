...
#[cfg(not(target_family = "wasm"))]
use crate::db::exec::ValueBatch;
use crate::db::exec::ValueBatchStream;
use crate::db::exec::access_mode::AccessMode;
use crate::db::exec::cardinality::CardinalityHint;
#[cfg(not(target_family = "wasm"))]
...
pub(crate) fn buffer_stream(
	...
	let buffer_size = *crate::config::cnf::OPERATOR_BUFFER_SIZE;
	if buffer_size == 0 {
		return stream;
	}
