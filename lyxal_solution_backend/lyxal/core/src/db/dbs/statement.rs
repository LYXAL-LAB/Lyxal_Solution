...
use crate::db::catalog::{Permission, TableDefinition};
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::Options;
use crate::db::doc::CursorDoc;
use crate::db::expr::cond::Cond;
use crate::db::expr::data::Data;
use crate::db::expr::fetch::Fetchs;
use crate::db::expr::field::Fields;
use crate::db::expr::group::Groups;
use crate::db::expr::limit::Limit;
use crate::db::expr::order::Ordering;
use crate::db::expr::output::Output;
use crate::db::expr::parameterize::exprs_to_fields;
use crate::db::expr::split::Splits;
use crate::db::expr::start::Start;
use crate::db::expr::statements::LiveFields;
use crate::db::expr::statements::access::AccessStatement;
use crate::db::expr::statements::create::CreateStatement;
use crate::db::expr::statements::delete::DeleteStatement;
use crate::db::expr::statements::insert::InsertStatement;
use crate::db::expr::statements::live::LiveStatement;
use crate::db::expr::statements::relate::RelateStatement;
use crate::db::expr::statements::select::SelectStatement;
use crate::db::expr::statements::show::ShowStatement;
use crate::db::expr::statements::update::UpdateStatement;
use crate::db::expr::statements::upsert::UpsertStatement;
use crate::db::expr::{Explain, Expr, FlowResultExt, Idiom, With};
use crate::db::idx::planner::QueryPlanner;
...
impl ToSql for Statement<'_> {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		match self {
			Statement::Live(v) => {
				let sql_stmt: crate::db::sql::statements::LiveStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Show(v) => {
				let sql_stmt: crate::db::sql::statements::ShowStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Select {
				stmt,
				...
				let sql_stmt: crate::db::sql::statements::SelectStatement = (*stmt).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Create(v) => {
				let sql_stmt: crate::db::sql::statements::CreateStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Upsert(v) => {
				let sql_stmt: crate::db::sql::statements::UpsertStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Update(v) => {
				let sql_stmt: crate::db::sql::statements::UpdateStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Relate(v) => {
				let sql_stmt: crate::db::sql::statements::RelateStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Delete(v) => {
				let sql_stmt: crate::db::sql::statements::DeleteStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Insert(v) => {
				let sql_stmt: crate::db::sql::statements::InsertStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
			Statement::Access(v) => {
				let sql_stmt: crate::db::sql::statements::AccessStatement = (*v).clone().into();
				sql_stmt.fmt_sql(f, fmt);
			}
		}
	}
...
impl Statement<'_> {
	...
	pub(crate) async fn from_select<'a>(
		...
		use crate::db::expr::visit::MutVisitor;
		use crate::db::idx::planner::count_exists_rewriter::CountLimitRewriter;
