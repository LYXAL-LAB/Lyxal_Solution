...
use crate::db::catalog::{DatabaseId, NamespaceId, Record};
use crate::config::cnf::NORMAL_FETCH_SIZE;
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::distinct::SyncDistinct;
use crate::db::dbs::{Iterable, Iterator, Operable, Options, Processable, Statement};
use crate::db::doc::{DocumentContext, NsDbCtx, NsDbTbCtx};
use crate::error::Error;
use crate::db::expr::dir::Dir;
use crate::db::expr::lookup::{ComputedLookupSubject, LookupKind};
use crate::db::expr::statements::relate::RelateThrough;
use crate::db::idx::planner::iterators::{IndexItemRecord, IteratorRef, RecordIterator};
use crate::db::idx::planner::{IterationStage, RecordStrategy, ScanDirection};
use crate::db::key::{graph, r#ref};
use crate::kvs::{KVKey, KVValue, Key, Transaction, Val};
use crate::db::val::{RecordId, RecordIdKey, RecordIdKeyRange, TableName, Value};
...
impl Collectable {
	...
	async fn process_range_key(doc_ctx: NsDbTbCtx, key: Key) -> Result<Processable> {
		let key = crate::db::key::record::RecordKey::decode_key(&key)?;
...
pub(super) trait Collector {
	...
	async fn collect_table(
		...
		let beg = crate::db::key::record::prefix(ns, db, table)?;
		let end = crate::db::key::record::suffix(ns, db, table)?;
	...
	async fn collect_table_count(
		...
		let ns = doc_ctx.ns.namespace_id;
		let db = doc_ctx.db.database_id;
		let beg = crate::db::key::record::prefix(ns, db, v)?;
		let end = crate::db::key::record::suffix(ns, db, v)?;
		// Create a new iterable range
	...
	async fn range_prepare(
		...
		let beg = match &r.start {
			Bound::Unbounded => crate::db::key::record::prefix(ns, db, tb)?,
			Bound::Included(v) => crate::db::key::record::new(ns, db, tb, v).encode_key()?,
			Bound::Excluded(v) => {
				let mut key = crate::db::key::record::new(ns, db, tb, v).encode_key()?;
				key.push(0x00);
				key
			}
		};
		...
		let end = match &r.end {
			Bound::Unbounded => crate::db::key::record::suffix(ns, db, tb)?,
			Bound::Excluded(v) => crate::db::key::record::new(ns, db, tb, v).encode_key()?,
			Bound::Included(v) => {
				let mut key = crate::db::key::record::new(ns, db, tb, v).encode_key()?;
				key.push(0x00);
				key
