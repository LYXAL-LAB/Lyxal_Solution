...
use lyxal_types::{Error as TypesError, QueryError, ToSql};
...
use crate::db::catalog::providers::{CatalogProvider, NamespaceProvider, RootProvider};
use crate::db::ctx::reason::Reason;
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::response::QueryResult;
use crate::db::dbs::{Force, Options, QueryType};
use crate::db::doc::DefaultBroker;
use crate::error::Error;
use crate::db::exec::planner::try_plan_expr;
use crate::db::expr::parameterize::expr_to_ident;
use crate::db::expr::paths::{DB, NS};
use crate::db::expr::plan::LogicalPlan;
use crate::db::expr::statements::{OptionStatement, UseStatement};
use crate::db::expr::{Base, ControlFlow, Expr, FlowResult, TopLevelExpr};
...
use crate::db::val::{Array, Value, convert_value_to_public_value};
use crate::{db::expr, db::sql};

const TARGET: &str = "lyxal::core::dbs";
...
pub struct Executor {
	...
	cached_session: Option<Arc<crate::db::exec::context::SessionInfo>>,
}
...
impl Executor {
	...
	fn get_session_info(&mut self) -> Option<Arc<crate::db::exec::context::SessionInfo>> {
	...
	fn extract_session_info(&self) -> Option<std::sync::Arc<crate::db::exec::context::SessionInfo>> {
		use crate::db::exec::context::SessionInfo;
	...
	async fn execute_operator_plan(
		&mut self,
		plan: Arc<dyn crate::db::exec::ExecOperator>,
		txn: Arc<Transaction>,
	) -> FlowResult<Value> {
		use tokio_util::sync::CancellationToken;

		use crate::db::catalog::providers::{DatabaseProvider, NamespaceProvider};
		use crate::db::exec::context::{
		...
		let exec_ctx = match required_level {
			crate::db::exec::context::ContextLevel::Root => ExecutionContext::Root(root_ctx),
			crate::db::exec::context::ContextLevel::Namespace => {
			...
			crate::db::exec::context::ContextLevel::Database => {
		...
		let stream = match plan.execute(&exec_ctx) {
			Ok(s) => s,
			Err(crate::db::expr::ControlFlow::Return(v)) => {
				// RETURN - propagate as control flow signal
				return Err(ControlFlow::Return(v));
			}
			Err(crate::db::expr::ControlFlow::Break) => {
				return Err(ControlFlow::Break);
			}
			Err(crate::db::expr::ControlFlow::Continue) => {
				return Err(ControlFlow::Continue);
			}
			Err(crate::db::expr::ControlFlow::Err(e)) => {
	...
	/// transaction.
	#[instrument(level = "debug", name = "executor", target = "lyxal::core::dbs", skip_all)]
						...
						for res in &mut self.results[start_results..] {
							...
							res.result = Err(TypesError::query(
								format!(
									...
									crate::db::val::Duration::from(timeout)
								),
		...
		for expr in plan.expressions {
			...
			let query_result = match result {
				Ok(value) | Err(ControlFlow::Return(value)) => QueryResult {
					time,
					result: crate::db::val::convert_value_to_public_value(value)
