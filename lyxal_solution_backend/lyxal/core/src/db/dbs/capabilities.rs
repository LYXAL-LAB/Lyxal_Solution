...
use ipnet::IpNet;
#[cfg(feature = "lyxalism")]
use lyxalism_runtime::capabilities::LyxalismCapabilities;
#[cfg(all(not(target_family = "wasm"), feature = "http"))]
use tokio::net::lookup_host;
use url::Url;

use crate::db::dbs::session::NewPlannerStrategy;
use crate::db::iam::{Auth, Level};
...
impl fmt::Display for ExperimentalTarget {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::Lyxalism => write!(f, "lyxalism"),
		}
	}
}
...
impl Target<str> for ExperimentalTarget {
	fn matches(&self, elem: &str) -> bool {
		match self {
			...
			Self::Lyxalism => elem.eq_ignore_ascii_case("lyxalism"),
		}
	}
}
...
impl std::str::FromStr for ExperimentalTarget {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		match s.trim().to_ascii_lowercase().as_str() {
			...
			"lyxalism" => Ok(ExperimentalTarget::Lyxalism),
...
impl Capabilities {
	...
	/// Checks wether capabilities required by a Lyxalism package are allowed.
	/// The `allow_arbitrary_queries` capability is not checked as that is to be used by the
	/// runtime.
	#[cfg(feature = "lyxalism")]
	pub fn validate_lyxalism_capabilities(
		&self,
		capabilities: LyxalismCapabilities,
	) -> anyhow::Result<()> {
		if capabilities.allow_scripting && !self.allows_scripting() {
			bail!("Lyxalism package requires scripting, but it is not allowed");
		}

		if !capabilities.allow_functions.is_empty() {
			for fnc in capabilities.allow_functions.iter() {
				if !self.allows_function_name(fnc) {
					bail!("Lyxalism package requires function '{}', but it is not allowed", fnc);
				}
			}
		}
		...
		if !capabilities.allow_net.is_empty() {
			for net in capabilities.allow_net.iter() {
				...
				if !self.allows_network_target(&target) {
					bail!(
						"Lyxalism package requires network target '{}', but it is not allowed",
						net
					);
				}
			}
