...
use crate::db::expr::{Expr, Fetchs, Fields};
...
impl SubscriptionDefinition {
	fn to_sql_definition(&self) -> crate::db::sql::LiveStatement {
		let fields = match &self.fields {
			SubscriptionFields::Diff => LiveFields::Diff,
			SubscriptionFields::Select(x) => LiveFields::Select(x.clone().into()),
		};

		crate::db::sql::LiveStatement {
			fields,
			what: self.what.clone().into(),
			cond: self.cond.clone().map(|c| crate::db::sql::Cond(c.into())),
...
impl InfoStructure for SubscriptionDefinition {
	fn structure(self) -> Value {
		Value::from(map! {
			"id".to_string() => crate::db::val::Uuid(self.id).into(),
			"node".to_string() => crate::db::val::Uuid(self.node).into(),
