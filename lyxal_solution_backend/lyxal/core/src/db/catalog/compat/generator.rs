...
//! ```bash
//! cargo test -p lyxal-core --lib catalog::compat::generator -- --ignored --nocapture
//! ```
//!
...
fn module_definition_fixtures() -> TypeFixtures {
	TypeFixtures {
		...
		fixtures: vec![
			Fixture {
				name: "MODULE_LYXALISM",
				description: "module with Lyxalism executable",
				value: Box::new(fix::module_lyxalism()),
			},
			Fixture {
				name: "MODULE_SILO",
...
fn generate_all_fixtures() -> String {
	...
	output.push_str("//! Catalog compatibility fixtures for Lyxal 3.0.0\n");
	output.push_str("//!\n");
	output.push_str("//! These fixtures represent the exact serialization format used in\n");
	output.push_str("//! Lyxal 3.0.0. They must NEVER be modified after being committed.\n");
...
fn generator() {
	...
	println!("Copy the following output to lyxal/core/src/catalog/compat/v3_0_0.rs");
	println!("--- EVERYTHING BELOW ---");
	println!("{}", output);
	println!("--- EVERYTHING ABOVE ---");
	println!("\n// Copy the above output to lyxal/core/src/catalog/compat/v3_0_0.rs");
