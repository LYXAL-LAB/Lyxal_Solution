...
impl Resolved {
	pub(in crate::db::idx) fn doc_id(&self) -> DocId {
		match self {
...
impl SeqDocIds {
	...
	pub(in crate::db::idx) fn new(ikb: IndexKeyBase) -> Self {
		Self {
	...
	pub(in crate::db::idx) async fn get_doc_id(
		&self,
		tx: &Transaction,
		id: &RecordIdKey,
	) -> Result<Option<DocId>> {
	...
	pub(in crate::db::idx) async fn resolve_doc_id(
		&self,
		ctx: &FrozenContext,
		id: RecordIdKey,
	) -> Result<Resolved> {
	...
	pub(in crate::db::idx) async fn get_id(
		ikb: &IndexKeyBase,
		tx: &Transaction,
		doc_id: DocId,
	...
	pub(in crate::db::idx) async fn remove_doc_id(
		&self,
		tx: &Transaction,
		doc_id: DocId,
	) -> Result<()> {
...
mod tests {
	...
	use crate::db::ctx::FrozenContext;
	use crate::db::idx::IndexKeyBase;
	use crate::db::idx::seqdocids::{DocId, Resolved, SeqDocIds};
	...
	async fn test_remove_doc_id() {
		...
		// Check there's no ID and BI keys left
		{
			...
			for id in ["Foo", "Bar", "Hello", "World"] {
				let id = crate::db::key::index::id::Id::new(
					TEST_NS_ID,
					TEST_DB_ID,
					&tb,
					TEST_IX_ID,
