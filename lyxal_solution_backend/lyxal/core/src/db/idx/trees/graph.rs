	...
	S: DynamicSet,
{
	...
	pub(in crate::db::idx::trees) fn nodes(&self) -> &HashMap<ElementId, S> {
		&self.nodes
	}
	pub(in crate::db::idx::trees) fn check(&self, g: Vec<(ElementId, Vec<ElementId>)>) {
		for (n, e) in g {
...
#[cfg(test)]
mod tests {
	use crate::db::idx::trees::dynamicset::{AHashSet, ArraySet, DynamicSet};
