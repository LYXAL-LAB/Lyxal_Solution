...
use crate::db::idx::trees::knn::Ids64;
use crate::db::idx::trees::vector::{SerializedVector, Vector};
use crate::kvs::{KVValue, Transaction};
use crate::db::val::{RecordId, RecordIdKey};
...
pub(in crate::db::idx) struct HnswDocs {
...
impl HnswDocs {
	...
	pub(in crate::db::idx) async fn new(tx: &Transaction, ikb: IndexKeyBase) -> Result<Self> {
	...
	pub(in crate::db::idx) async fn finish(&mut self, tx: &Transaction) -> Result<()> {
...
pub(in crate::db::idx) struct VecDocs {
	ikb: IndexKeyBase,
