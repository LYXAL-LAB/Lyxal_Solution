...
use crate::db::catalog::{Distance, HnswParams, TableId, VectorType};
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::Options;
use crate::error::Error;
use crate::db::expr::Cond;
use crate::db::idx::IndexKeyBase;
use crate::db::idx::planner::ScanDirection;
use crate::db::idx::planner::iterators::KnnIteratorResult;
use crate::db::idx::trees::hnsw::cache::VectorCache;
use crate::db::idx::trees::hnsw::docs::{HnswDocs, VecDocs};
use crate::db::idx::trees::hnsw::filter::HnswTruthyDocumentFilter;
use crate::db::idx::trees::hnsw::flavor::HnswFlavor;
use crate::db::idx::trees::hnsw::{ElementId, HnswSearch, VectorId, VectorPendingUpdate};
use crate::db::idx::trees::knn::KnnResultBuilder;
use crate::db::idx::trees::vector::{SerializedVector, SharedVector, Vector};
use crate::db::key::index::hp::HnswPending;
use crate::kvs::{KVValue, Key, Transaction};
use crate::db::val::{Number, RecordId, RecordIdKey, Value};
...
impl HnswIndex {
	...
	/// updates applied.
	pub(in crate::db::idx) async fn index_pendings(&self, ctx: &FrozenContext) -> Result<usize> {
		let tx = ctx.tx();
