...
use crate::db::catalog::{FullTextParams, Scoring};
...
use crate::db::expr::Idiom;
use crate::db::expr::operator::BooleanOperator;
use crate::db::idx::IndexKeyBase;
use crate::db::idx::ft::analyzer::Analyzer;
use crate::db::idx::ft::analyzer::filter::FilteringStage;
use crate::db::idx::ft::analyzer::tokenizer::Tokens;
use crate::db::idx::ft::highlighter::{HighlightParams, Highlighter, Offseter};
use crate::db::idx::ft::offset::Offset;
use crate::db::idx::ft::{DocLength, Score, TermFrequency};
use crate::db::idx::planner::iterators::MatchesHitsIterator;
use crate::db::idx::seqdocids::{DocId, SeqDocIds};
use crate::db::idx::trees::store::IndexStores;
use crate::db::key::index::tt::Tt;
...
impl QueryTerms {
	...
	pub(in crate::db::idx::ft) fn matches_or(&self, tks: &[Tokens]) -> Result<bool> {
	...
	pub(in crate::db::idx::ft) fn matches_and(&self, tks: &[Tokens]) -> Result<bool> {
		for t in self.tokens.list() {
			let t = self.tokens.get_token_string(t)?;
			let mut found = false;
			for tokens in tks {
...
pub(crate) struct Bm25Params {
	pub(in crate::db::idx) k1: f32,
	pub(in crate::db::idx) b: f32,
}
...
impl FullTextIndex {
	...
	pub(in crate::db::idx) async fn matches_value(
		&self,
		stk: &mut Stk,
		ctx: &FrozenContext,
		opt: &Options,
...
mod tests {
	...
	use crate::db::catalog::{DatabaseId, FullTextParams, IndexId, NamespaceId};
	use crate::config::cnf::dynamic::DynamicConfiguration;
	use crate::db::ctx::{Context, FrozenContext};
	use crate::db::dbs::Options;
	use crate::db::expr::statements::DefineAnalyzerStatement;
	use crate::db::idx::IndexKeyBase;
	use crate::db::idx::ft::offset::Offset;
	...
	use crate::db::sql::statements::DefineStatement;
	use crate::db::syn;
	use crate::db::val::{Array, RecordId, Value};
