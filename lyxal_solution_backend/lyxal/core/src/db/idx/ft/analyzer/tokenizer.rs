...
use crate::db::expr::tokenizer::Tokenizer as SqlTokenizer;
use crate::db::idx::ft::Position;
use crate::db::idx::ft::analyzer::filter::{Filter, FilterResult, Term};
use crate::db::idx::ft::offset::Offset;
use crate::db::val::Value;

pub(in crate::db::idx::ft) struct Tokens {
	/// The input string
	i: String,
...
impl Tokens {
	pub(in crate::db::idx::ft) fn new(i: String) -> Self {
		Self {
			i,
			t: Vec::new(),
		}
	...
	pub(in crate::db::idx::ft) fn get_token_string<'a>(&'a self, t: &'a Token) -> Result<&'a str> {
		t.get_str(&self.i)
	}
	...
	pub(in crate::db::idx::ft) fn list(&self) -> &Vec<Token> {
		&self.t
	}

	pub(in crate::db::idx::ft) fn try_contains(&self, s: &str) -> Result<bool> {
		for t in &self.t {
...
pub(in crate::db::idx::ft) enum Token {
	Ref {
...
impl Token {
	...
	pub(in crate::db::idx::ft) fn new_offset(&self, i: u32) -> Offset {
		match self {
			Token::Ref {
				chars,
				..
	...
	pub(in crate::db::idx::ft) fn get_char_len(&self) -> u32 {
		match self {
			Token::Ref {
				len,
				..
...
impl Tokenizer {
	pub(in crate::db::idx::ft) fn new(t: &[SqlTokenizer]) -> Self {
		Self {
...
#[cfg(test)]
mod tests {
	use crate::db::idx::ft::analyzer::tests::test_analyzer;
