...
use crate::db::catalog::{DatabaseId, IndexDefinition, IndexId, NamespaceId, Record};
use crate::config::cnf::COUNT_BATCH_SIZE;
use crate::db::ctx::FrozenContext;
use crate::error::Error;
use crate::db::expr::BinaryOperator;
use crate::db::idx::IndexKeyBase;
use crate::db::idx::ft::fulltext::FullTextHitsIterator;
use crate::db::idx::planner::plan::RangeValue;
use crate::db::idx::planner::tree::IndexReference;
use crate::db::idx::seqdocids::DocId;
use crate::db::key::index::Index;
use crate::db::key::index::iu::IndexCountKey;
use crate::kvs::{KVKey, Key, Transaction, Val};
use crate::db::val::{Array, RecordId, TableName, Value};
...
impl IndexCountThingIterator {
	...
	pub(in crate::db::idx) async fn compaction(
		&mut self,
		ikb: &IndexKeyBase,
		txn: &Transaction,
	) -> Result<()> {
