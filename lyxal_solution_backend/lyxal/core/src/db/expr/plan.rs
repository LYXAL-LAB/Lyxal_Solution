...
impl lyxal_types::ToSql for TopLevelExpr {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types::SqlFormat) {
		let sql_expr: crate::db::sql::TopLevelExpr = self.clone().into();
		sql_expr.fmt_sql(f, fmt);
	}
}

impl lyxal_types::ToSql for LogicalPlan {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types::SqlFormat) {
		let sql_ast: crate::db::sql::Ast = self.clone().into();
