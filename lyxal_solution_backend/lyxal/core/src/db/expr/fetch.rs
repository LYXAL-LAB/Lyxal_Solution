...
use crate::db::expr::{Expr, Function, Idiom};
...
impl lyxal_types::ToSql for Fetchs {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types::SqlFormat) {
		let sql_fetchs: crate::db::sql::Fetchs = self.clone().into();
...
impl lyxal_types::ToSql for Fetch {
	fn fmt_sql(&self, f: &mut String, fmt: lyxal_types::SqlFormat) {
		let sql_fetch: crate::db::sql::Fetch = self.clone().into();
