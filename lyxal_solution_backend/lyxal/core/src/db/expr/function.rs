...
use crate::db::expr::{Expr, Idiom, Kind, Model, ModuleExecutable, Script, Value};

use crate::db::iam::AuthLimit;
	...
	pub(crate) async fn compute(
		...
		match self {
			Function::Normal(s) => {
				...
				Ok(crate::function::run(stk, ctx, opt, doc, s, args).await?)
			}
			...
			Function::Script(s) => {
				#[cfg(feature = "scripting")]
				{
					...
					crate::function::script::run(ctx, opt, doc, &s.0, args).await.map_err(ControlFlow::Err)
				}
...
impl ToSql for FunctionCall {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let fnc: crate::db::sql::FunctionCall = self.clone().into();
		fnc.fmt_sql(f, fmt);
	}
}
