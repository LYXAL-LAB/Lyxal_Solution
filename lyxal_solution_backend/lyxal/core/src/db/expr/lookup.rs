...
use crate::db::expr::{Cond, Dir, Fields, Groups, Idiom, Limit, RecordIdKeyRangeLit, Splits};
use crate::kvs::KVKey;
use crate::db::val::{RecordId, RecordIdKey, RecordIdKeyRange, TableName};
...
impl ToSql for Lookup {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::lookup::Lookup = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
		...
		match kind {
			...
			LookupKind::Reference => match self {
				...
				Self::Table {
					...
					crate::db::key::r#ref::ftprefix(ns, db, tb, id, table)?,
					crate::db::key::r#ref::ftsuffix(ns, db, tb, id, table)?,
				)),
				...
				Self::Table {
					...
					crate::db::key::r#ref::ffprefix(ns, db, tb, id, table, field)?,
					crate::db::key::r#ref::ffsuffix(ns, db, tb, id, table, field)?,
				)),
				...
				Self::Range {
					...
					let beg = match &range.start {
						...
						Bound::Included(v) => {
							crate::db::key::r#ref::refprefix(ns, db, tb, id, table, field, v)?
						}
						Bound::Excluded(v) => {
							crate::db::key::r#ref::refsuffix(ns, db, tb, id, table, field, v)?
						}
					};
			...
			LookupKind::Graph(dir) => match self {
				...
				Self::Table {
					...
					crate::db::key::graph::ftprefix(ns, db, tb, id, dir, table)?,
					crate::db::key::graph::ftsuffix(ns, db, tb, id, dir, table)?,
				)),
				// Scan a specific range
				Self::Range {
					table,
					...
					let beg = match &range.start {
						...
						Bound::Included(v) => crate::db::key::graph::new(
							ns,
							db,
							tb,
							id,
						...
						.encode_key()?,
						Bound::Excluded(v) => crate::db::key::graph::new(
							ns,
							db,
							tb,
							id,
...
impl ToSql for LookupSubject {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::lookup::LookupSubject = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
