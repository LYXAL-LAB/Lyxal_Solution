...
use crate::db::dbs::{Iterator, Options, Statement};
use crate::db::doc::{CursorDoc, NsDbCtx};
use crate::error::Error;
use crate::db::expr::{Cond, Data, Explain, Expr, Literal, Output, With};
use crate::db::idx::planner::{QueryPlanner, RecordStrategy, StatementContext};
...
impl ToSql for UpdateStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::update::UpdateStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
