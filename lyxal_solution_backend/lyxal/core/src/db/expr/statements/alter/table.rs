...
use crate::db::catalog::{Permissions, TableType};
use crate::db::ctx::FrozenContext;
use crate::db::dbs::Options;
use crate::error::Error;
use crate::db::expr::statements::DefineTableStatement;
use crate::db::expr::{Base, ChangeFeed};
...
impl AlterTableStatement {
	...
	pub(crate) async fn compute(&self, ctx: &FrozenContext, opt: &Options) -> Result<Value> {
		...
		if self.compact {
			let key = crate::db::key::table::all::new(ns, db, &self.name);
...
impl ToSql for AlterTableStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::alter::AlterTableStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
