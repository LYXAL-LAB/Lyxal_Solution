...
use crate::db::catalog::{self, Permission, Permissions, TableDefinition};
use crate::db::ctx::FrozenContext;
use crate::db::dbs::Options;
use crate::error::Error;
use crate::db::expr::reference::Reference;
use crate::db::expr::{Base, Expr, Idiom, Kind};
...
impl AlterFieldStatement {
	...
	pub(crate) async fn compute(&self, ctx: &FrozenContext, opt: &Options) -> Result<Value> {
		...
		let key = crate::db::key::table::fd::new(ns, db, &self.what, &name);
...
impl ToSql for AlterFieldStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::alter::field::AlterFieldStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
