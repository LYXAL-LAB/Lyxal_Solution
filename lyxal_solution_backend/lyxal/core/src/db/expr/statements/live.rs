...
use crate::db::catalog::{NodeLiveQuery, SubscriptionDefinition, SubscriptionFields};
use crate::db::ctx::FrozenContext;
use crate::db::dbs::{Options, ParameterCapturePass, Variables};
use crate::db::doc::CursorDoc;
use crate::error::Error;
use crate::db::expr::visit::Visit;
use crate::db::expr::{Cond, Expr, Fetchs, Fields, FlowResultExt as _};
	...
	pub(crate) async fn compute(
			...
			.catch_return()?
		{
			Value::Table(tb) => {
				...
				let key = crate::db::key::node::lq::new(nid, live_query_id);
				txn.replace(
					&key,
					&NodeLiveQuery {
						ns,
				...
				let key = crate::db::key::table::lq::new(ns, db, &tb, live_query_id);
		...
		// Return the query id
		Ok(crate::db::val::Uuid(live_query_id).into())
	}
}
...
mod tests {
	...
	use crate::db::catalog::providers::{CatalogProvider, TableProvider};
	use crate::db::dbs::{Capabilities, Session};
