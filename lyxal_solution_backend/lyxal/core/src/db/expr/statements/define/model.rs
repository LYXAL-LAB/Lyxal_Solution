...
use crate::db::catalog::{MlModelDefinition, Permission};
...
impl DefineModelStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::db::key::database::ml::new(ns, db, &self.name, &self.version);
		txn.set(
			&key,
			&MlModelDefinition {
...
impl ToSql for DefineModelStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::define::DefineModelStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
