...
use crate::db::catalog::providers::{AuthorisationProvider, NamespaceProvider};
use crate::db::catalog::{self, AccessDefinition};
use crate::db::ctx::FrozenContext;
use crate::db::dbs::Options;
use crate::db::doc::CursorDoc;
use crate::error::Error;
use crate::db::expr::access::AccessDuration;
...
use crate::db::val::{self, Duration, Value};
	...
	pub(crate) async fn compute(
		...
		match &self.base {
			Base::Root => {
				...
				let key = crate::db::key::root::ac::new(&definition.name);
			...
			Base::Ns => {
				...
				// Process the statement
				let key = crate::db::key::namespace::ac::new(ns, &definition.name);
			...
			Base::Db => {
				...
				// Process the statement
				let key = crate::db::key::database::ac::new(ns, db, &definition.name);
				txn.set(&key, &definition, None).await?;
				// Clear the cache
				txn.clear_cache();
				// Ok all good
...
impl ToSql for DefineAccessStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::define::DefineAccessStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
