...
use crate::db::expr::filter::Filter;
use crate::db::expr::parameterize::expr_to_ident;
use crate::db::expr::tokenizer::Tokenizer;
use crate::db::expr::{Base, Expr, FlowResultExt, Idiom, Literal, Value};
...
impl DefineAnalyzerStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::db::key::database::az::new(ns, db, &definition.name);
...
impl ToSql for DefineAnalyzerStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let sql_stmt: crate::db::sql::statements::define::DefineAnalyzerStatement = self.clone().into();
