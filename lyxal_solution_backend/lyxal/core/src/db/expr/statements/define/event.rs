...
use crate::db::catalog::{EventDefinition, EventKind, TableDefinition};
...
use crate::db::expr::{Base, Expr, FlowResultExt};
...
impl DefineEventStatement {
	...
	pub(crate) async fn compute(
		...
		let key = crate::db::key::table::ev::new(ns, db, &target_table, &name);
		txn.set(
			&key,
			&EventDefinition {
				name: name.clone(),
