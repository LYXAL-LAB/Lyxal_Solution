...
use crate::db::catalog::aggregation::{
...
use crate::db::doc::{self, CursorDoc, Document, DocumentContext, NsDbTbCtx};
use crate::error::Error;
use crate::db::expr::changefeed::ChangeFeed;
...
use crate::db::val::{Array, Number, RecordId, RecordIdKey, TableName, Value};
...
impl DefineTableStatement {
	...
	pub(crate) async fn compute(
		...
		if let Some(view) = &tb_def.view {
			...
			let key = crate::db::key::table::all::new(ns.namespace_id, db.database_id, &name);
			txn.delp(&key).await?;
			...
			for ft in tables.iter() {
				...
				let key = crate::db::key::table::ft::new(ns.namespace_id, db.database_id, ft, &name);
	...
	async fn initialize_materialized_view(
		...
		for v in v {
			...
			let key = crate::db::key::record::new(ns, db, view_table_name, &id.key);
	...
	pub async fn add_in_out_fields(
		...
		if let TableType::Relation(rel) = &tb.table_type {
			...
			{
				let key = crate::db::key::table::fd::new(ns, db, &tb.name, "in");
				let val =
			...
			{
				let key = crate::db::key::table::fd::new(ns, db, &tb.name, "out");
