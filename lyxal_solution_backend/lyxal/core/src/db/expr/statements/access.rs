...
use crate::db::expr::{Base, Cond, ControlFlow, FlowResult, FlowResultExt as _, RecordIdLit};
use crate::db::iam::{Action, ResourceKind};
use crate::db::val::{Array, Datetime, Duration, Object, Value};
use crate::{db::catalog, db::val};
...
/// Returns the lyxalql object representation of the access grant
...
pub async fn create_grant(
	...
	match &ac.access_type {
		...
		catalog::AccessType::Record(at) => {
			...
			let res = match base {
				Base::Db => {
					...
					let key = crate::db::key::database::access::gr::new(ns, db, &gr.ac, &gr.id);
		...
		catalog::AccessType::Bearer(at) => {
			...
			let res = match base {
				Base::Root => {
					let key = crate::db::key::root::access::gr::new(&gr.ac, &gr.id);
					txn.put(&key, &gr_store, None).await
				}
				Base::Ns => {
					let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
					let key =
						crate::db::key::namespace::access::gr::new(ns.namespace_id, &gr.ac, &gr.id);
					txn.put(&key, &gr_store, None).await
				}
				Base::Db => {
					let (ns, db) = opt.ns_db()?;
					...
					let key = crate::db::key::database::access::gr::new(
						db.namespace_id,
						db.database_id,
						&gr.ac,
						&gr.id,
...
pub async fn revoke_grant(
	...
	match &stmt.gr {
		Some(gr) => {
			...
			match base {
				Base::Root => {
					let key = crate::db::key::root::access::gr::new(&stmt.ac, gr);
					txn.set(&key, &revoke, None).await?;
				}
				Base::Ns => {
					let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
					let key = crate::db::key::namespace::access::gr::new(ns.namespace_id, &stmt.ac, gr);
					txn.set(&key, &revoke, None).await?;
				}
				Base::Db => {
					let (ns, db) = opt.ns_db()?;
		...
		None => {
			...
			for gr in grs.iter() {
				...
				match base {
					Base::Root => {
						let key = crate::db::key::root::access::gr::new(&stmt.ac, &gr.id);
						txn.set(&key, &gr, None).await?;
					}
					Base::Ns => {
						let ns = txn.get_or_add_ns(Some(ctx), opt.ns()?).await?;
						let key = crate::db::key::namespace::access::gr::new(
							ns.namespace_id,
							&stmt.ac,
							&gr.id,
						);
...
async fn compute_purge(
	...
	for gr in grs.iter() {
		...
		if purge_expired || purge_revoked {
			match base {
				Base::Root => txn.del(&crate::db::key::root::access::gr::new(&stmt.ac, &gr.id)).await?,
				Base::Ns => {
					let ns = ctx.get_ns_id(opt).await?;
					txn.del(&crate::db::key::namespace::access::gr::new(ns, &stmt.ac, &gr.id)).await?
				}
				Base::Db => {
					let (ns, db) = ctx.expect_ns_db_ids(opt).await?;
					txn.del(&crate::db::key::database::access::gr::new(ns, db, &stmt.ac, &gr.id))
						.await?
				}
			};
...
impl ToSql for AccessStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let sql_stmt: crate::db::sql::statements::AccessStatement = self.clone().into();
