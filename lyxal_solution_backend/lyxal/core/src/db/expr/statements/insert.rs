...
use crate::db::expr::{Data, Expr, FlowResultExt as _, Output, Value};
use crate::db::idx::planner::RecordStrategy;
use crate::db::val::{Duration, RecordIdKey, TableName};
...
impl ToSql for InsertStatement {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::statements::insert::InsertStatement = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
