...
use crate::config::cnf::IDIOM_RECURSION_LIMIT;
use crate::db::ctx::FrozenContext;
use crate::db::dbs::Options;
use crate::db::doc::CursorDoc;
use crate::error::Error;
use crate::db::exe::try_join_all_buffered;
use crate::db::expr::idiom::recursion::{
	self, Recursion, clean_iteration, compute_idiom_recursion, is_final,
};
use crate::db::expr::{Expr, FlowResultExt as _, Idiom, Literal, Lookup, Value};
use crate::utils::fmt::EscapeKwFreeIdent;
use crate::db::val::{Array, RecordId};
	...
	pub(crate) fn as_old_index(&self) -> Option<usize> {
		match self {
			Part::Value(Expr::Literal(l)) => match l {
				crate::db::expr::Literal::Integer(i) => Some(*i as usize),
				crate::db::expr::Literal::Float(f) => Some(*f as usize),
				crate::db::expr::Literal::Decimal(d) => Some(usize::try_from(*d).unwrap_or_default()),
				_ => None,
			},
			_ => None,
		}
...
impl ToSql for Part {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let part: crate::db::sql::part::Part = self.clone().into();
		part.fmt_sql(f, fmt);
	}
}
...
impl ToSql for DestructurePart {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::part::DestructurePart = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
...
impl ToSql for Recurse {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let recurse: crate::db::sql::part::Recurse = self.clone().into();
...
impl ToSql for RecurseInstruction {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let stmt: crate::db::sql::part::RecurseInstruction = self.clone().into();
		stmt.fmt_sql(f, fmt);
	}
}
