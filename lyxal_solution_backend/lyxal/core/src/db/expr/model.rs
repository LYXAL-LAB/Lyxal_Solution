...
impl ToSql for Model {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let stmt: crate::db::sql::model::Model = self.clone().into();
...
impl Model {
	...
	pub(crate) async fn compute(
		...
		use crate::db::val::{CoerceError, Number};
		...
		match argument {
			...
			Value::Number(v) => {
				...
				// Get the model file as bytes
				let bytes = crate::telemetry::obs::get(&path).await.map_err(ControlFlow::from)?;
