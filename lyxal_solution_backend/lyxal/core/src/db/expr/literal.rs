...
use crate::db::expr::{Expr, FlowResult, RecordIdLit};
use crate::db::val::{
...
/// lyxalql rules regarding number equality are not observed, 1f != 1dec.
...
impl ToSql for Literal {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let lit: crate::db::sql::Literal = self.clone().into();
		lit.fmt_sql(f, fmt);
	}
}
...
impl ToSql for ObjectEntry {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let entry: crate::db::sql::literal::ObjectEntry = self.clone().into();
		entry.fmt_sql(f, fmt);
	}
}
