...
use crate::db::expr::{Expr, Literal, Part, Value};
use crate::db::val::{
...
impl HasKind for crate::db::val::Object {
	fn kind() -> Kind {
		Kind::Object
	}
}

impl HasKind for crate::db::val::Value {
	fn kind() -> Kind {
		Kind::Any
	}
}
...
impl ToSql for Kind {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let kind: crate::db::sql::Kind = self.clone().into();
		kind.fmt_sql(f, fmt);
	}
}
	...
	fn from(v: crate::types::PublicKindLiteral) -> Self {
		match v {
			...
			crate::types::PublicKindLiteral::Duration(d) => {
				KindLiteral::Duration(crate::db::val::Duration(*d))
			}
...
impl ToSql for KindLiteral {
	fn fmt_sql(&self, f: &mut String, fmt: SqlFormat) {
		let lit: crate::db::sql::kind::KindLiteral = self.clone().into();
		lit.fmt_sql(f, fmt)
	}
}
