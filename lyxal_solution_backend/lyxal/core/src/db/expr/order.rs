...
impl ToSql for Ordering {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let sql_ordering: crate::db::sql::order::Ordering = self.clone().into();
...
impl ToSql for OrderList {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let sql_order_list: crate::db::sql::order::OrderList = self.clone().into();
...
impl ToSql for Order {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let sql_order: crate::db::sql::Order = self.clone().into();
