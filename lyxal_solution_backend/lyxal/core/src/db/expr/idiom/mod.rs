...
use crate::db::expr::part::{Next, NextMethod};
use crate::db::expr::paths::{ID, IN, OUT};
use crate::db::expr::statements::info::InfoStructure;
use crate::db::expr::{FlowResult, Part, Value};
...
impl ToSql for Idiom {
	fn fmt_sql(&self, f: &mut String, sql_fmt: SqlFormat) {
		let sql_idiom: crate::db::sql::Idiom = self.clone().into();
...
impl FromStr for Idiom {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		let mut parser = crate::db::syn::parser::Parser::new_with_experimental(buf, true);
		let expr = stack
		...
		match expr {
			crate::db::sql::Expr::Idiom(idiom) => Ok(idiom.into()),
