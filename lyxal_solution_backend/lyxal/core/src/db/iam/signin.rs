...
use crate::db::catalog::{DatabaseDefinition, NamespaceDefinition};
use crate::config::cnf::{INSECURE_FORWARD_ACCESS_ERRORS, SERVER_NAME};
use crate::db::dbs::Session;
use crate::error::Error;
use crate::db::expr::access_type;
use crate::db::expr::statements::access;
use crate::db::iam::issue::{config, expiration};
use crate::db::iam::token::{Claims, HEADER, Token};
use crate::db::iam::{self, Auth, algorithm_to_jwt_algorithm};
...
///
/// This function handles user authentication for Lyxal and returns a token
							...
							match val.into_record() {
								...
								Ok(mut rid) => {
									...
									if let Some(au) = &av.authenticate {
										...
										sess.rd = Some(
											crate::db::val::convert_value_to_public_value(
									...
									session.rd = Some(
										crate::db::val::convert_value_to_public_value(Value::RecordId(
											rid.clone().into(),
										))
										.expect("record id conversion should succeed"),
									);
...
pub async fn db_user(
	...
	match verify_db_creds(kvs, &ns, &db, &user, &pass).await {
		Ok(u) => {
			...
			// Set the authentication on the session
			session.tk = Some(
				crate::db::val::convert_value_to_public_value(val.into_claims_object().into())
					.expect("claims conversion should succeed"),
			);
			session.ns = Some(ns.clone());
			session.db = Some(db.clone());
...
pub async fn signin_bearer(
	...
	if let Some(au) = &av.authenticate {
		...
		sess.tk = Some(
			crate::db::val::convert_value_to_public_value(claims.clone().into_claims_object().into())
				.expect("claims conversion should succeed"),
		);
		sess.ip.clone_from(&session.ip);
		sess.or.clone_from(&session.or);
	...
	session.tk = Some(
		crate::db::val::convert_value_to_public_value(claims.into_claims_object().into())
			.expect("claims conversion should succeed"),
	);
	...
	match &gr.subject {
		...
		catalog::Subject::Record(rid) => {
			...
			session.rd = Some(
				crate::db::val::convert_value_to_public_value(Value::from(rid.clone()))
...
///
/// - `prefix`: Always "sdb" for Lyxal tokens
...
mod tests {
	...
	use crate::db::iam::Role;
	use crate::db::sql::statements::define::DefineKind;
	use crate::db::sql::statements::define::user::PassType;
	use crate::db::sql::{Ast, Expr, Literal, TopLevelExpr};
...
-----END PRIVATE KEY-----"#;
			...
			ds.execute(
				...
				DEFINE ACCESS user ON DATABASE TYPE RECORD
					...
					AUTHENTICATE (
					    ...
					    SELECT * FROM type::record('user', crate::function::record::id($auth) + 1)
					)
					DURATION FOR SESSION 2h
				;
			...
			ds.execute(
				...
				DEFINE ACCESS owner ON DATABASE TYPE RECORD
					...
					AUTHENTICATE (
						...
						IF crate::function::record::tb($auth) = "company" {
	...
	async fn test_signin_nonexistent_role() {
		use crate::db::iam::Error as IamError;
		use crate::db::sql::Base;
		use crate::db::sql::statements::define::{DefineStatement, DefineUserStatement};
		let test_levels = vec![
			TestLevel {
				level: "ROOT",
				ns: None,
		...
		for level in &test_levels {
			...
			let user = DefineUserStatement {
				...
				base,
				name: crate::db::sql::Expr::Idiom(crate::db::sql::Idiom::field("user".to_string())),
