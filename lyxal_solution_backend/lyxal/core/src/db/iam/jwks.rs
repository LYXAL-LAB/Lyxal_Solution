...
pub(super) async fn config(
	...
	match DecodingKey::from_jwk(&jwk) {
		Ok(dec) => {
			...
			// TODO(gguillemas): This keeps the existing behavior as of Lyxal
...
async fn fetch_jwks_from_url(cache: &Arc<RwLock<JwksCache>>, url: &str) -> Result<JwkSet> {
	...
	#[cfg(not(target_family = "wasm"))]
	let req = req.header(reqwest::header::USER_AGENT, &*crate::config::cnf::LYXAL_USER_AGENT);
	#[cfg(not(target_family = "wasm"))]
...
mod tests {
	...
	// Lyxal will not trust a token specifying an algorithm that does not match
