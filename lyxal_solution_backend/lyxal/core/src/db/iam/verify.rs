...
use crate::db::iam::access::{authenticate_generic, authenticate_record};
use crate::db::iam::issue::expiration;
#[cfg(feature = "jwks")]
use crate::db::iam::jwks;
use crate::db::iam::token::Claims;
use crate::db::iam::{self, Actor, Auth, Level, Role};
use crate::kvs::Datastore;
use crate::kvs::LockType::*;
use crate::kvs::TransactionType::*;
use crate::{db::catalog, db::syn};
...
fn decode_key(alg: catalog::Algorithm, key: &[u8]) -> Result<(DecodingKey, Validation)> {
	...
	// TODO(gguillemas): This keeps the existing behavior as of Lyxal 2.0.0-alpha.9.
...
pub async fn token(kvs: &Datastore, session: &mut Session, token: &str) -> Result<()> {
	...
	// Convert the token to a LyxalQL object value
	let value = crate::db::val::Value::from(token_data.claims.clone().into_claims_object());
	...
	match &token_data.claims {
		...
		Claims {
			...
			if let Some(au) = &de.authenticate {
				...
				sess.rd = Some(
					crate::db::val::convert_value_to_public_value(crate::db::val::Value::RecordId(
						rid.clone().into(),
					))
			...
			// Set the session
			session.tk = Some(
				crate::db::val::convert_value_to_public_value(value)
...
mod tests {
	...
	async fn test_basic_nonexistent_role() {
		...
		use crate::db::sql::{Base, Expr, TopLevelExpr};
		let test_levels = vec![
			TestLevel {
				level: "ROOT",
				ns: None,
	...
	async fn test_token_record_jwks() {
		...
		use crate::db::dbs::capabilities::{Capabilities, NetTarget, Targets};
		...
		let claims = Claims {
			...
			iat: Some(Utc::now().timestamp()),
			nbf: Some(Utc::now().timestamp()),
			aud: Some(Audience::Single("lyxal-test".to_string())),
			exp: Some((Utc::now() + Duration::hours(1)).timestamp()),
			ns: Some("test".to_string()),
			db: Some("test".to_string()),
			ac: Some("token".to_string()),
	...
	async fn test_token_authenticate_clause() {
		...
		let test_cases = vec![
			TestCase {
				...
				iss_claim: Some("lyxal-test"),
				aud_claim: Some(Audience::Single("lyxal-test".to_string())),
			...
			TestCase {
				...
				aud_claim: Some(Audience::Multiple(vec![
					"invalid".to_string(),
					"lyxal-test-different".to_string(),
				])),
				error_statement: "THROW",
				...
				ds.execute(
					format!(
						...
						DEFINE ACCESS user ON {0} TYPE JWT
							...
							AUTHENTICATE {{
								IF $token.iss != "lyxal-test" {{ {2} "Invalid token issuer" }};
								IF type::is_array($token.aud) {{
									IF "lyxal-test" NOT IN $token.aud {{ {2} "Invalid token audience array" }}
								}} ELSE {{
									IF $token.aud IS NOT "lyxal-test" {{ {2} "Invalid token audience string" }}
								}};
							}}
