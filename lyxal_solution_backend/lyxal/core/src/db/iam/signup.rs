...
use crate::db::catalog::providers::{AuthorisationProvider, DatabaseProvider};
...
use crate::db::iam::token::{Claims, Token};
use crate::db::iam::{Actor, Auth, Level, Role, algorithm_to_jwt_algorithm};
...
///
/// This function handles user registration for Lyxal and returns a token
	...
	match kvs.evaluate(val, &sess, Some(vars)).await {
		...
		Ok(val) => {
			...
			if let Some(au) = &av.authenticate {
				...
				sess.rd = Some(
					crate::db::val::convert_value_to_public_value(Value::RecordId(rid.clone().into()))
...
-----END PRIVATE KEY-----"#;
			...
			assert!(!sess.au.has_role(Role::Editor), "Auth user expected to not have Editor role");
			assert!(!sess.au.has_role(Role::Owner), "Auth user expected to not have Owner role");
			// Session expiration should always be set for tokens issued by Lyxal
			let exp = sess.exp.unwrap();
			// Expiration should match the current time plus session duration with some
			// margin
			let min_sess_exp =
