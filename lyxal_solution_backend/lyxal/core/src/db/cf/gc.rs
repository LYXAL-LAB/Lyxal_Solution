...
// current time.
#[instrument(level = "trace", target = "lyxal::core::cfs", skip_all)]
...
#[instrument(level = "trace", target = "lyxal::core::cfs", skip_all, fields(ns = %ns, db = %db))]
pub async fn gc_range(
	...
	let beg = crate::db::key::change::prefix_ts(ns, db, beg_ts).encode_key()?;
	let end = crate::db::key::change::prefix_ts(ns, db, end_ts).encode_key()?;
	// Trace for debugging
	trace!(
