...
use crate::db::catalog::{FieldDefinition, TableDefinition};
use crate::db::dbs::Session;
use crate::db::expr::field::Selector;
use crate::db::expr::order::{OrderList, Ordering};
use crate::db::expr::part::Part;
use crate::db::expr::statements::SelectStatement;
use crate::db::expr::{
	self, BinaryOperator, Cond, Expr, Fields, Function, FunctionCall, Idiom, Kind, Limit, Literal,
	LogicalPlan, Start, TopLevelExpr,
};
use crate::db::gql::error::internal_error;
use crate::db::gql::schema::{geometry_gql_type_name, kind_to_type_with_enum_prefix, unwrap_type};
use crate::db::gql::utils::{GqlValueUtils, execute_plan};
use crate::kvs::Datastore;
use crate::db::val::{Array as SurArray, Datetime, Object as SurObject, RecordId, TableName, Value};
...
fn parse_version_arg(args: &IndexMap<Name, GqlValue>) -> Result<Option<Datetime>, GqlError> {
	match args.get("version") {
		Some(GqlValue::String(s)) => {
			let dt = crate::db::syn::datetime(s)
...
struct NestedSubField {
	...
	name: String,
	/// The field's Lyxal kind, if defined.
	kind: Option<Kind>,
...
fn make_table_get_field(tb: &TableDefinition, kvs: Arc<Datastore>) -> Field {
	...
	Field::new(format!("_get_{}", tb.name), TypeRef::named(&tb_name_str), move |ctx| {
		...
		FieldFuture::new(async move {
			...
			let record_id: RecordId = match crate::db::syn::record_id(&rid_str) {
				Ok(x) => x.into(),
...
fn make_generic_get_field(kvs: Arc<Datastore>) -> Field {
	Field::new("_get", TypeRef::named("record"), move |ctx| {
		...
		FieldFuture::new(async move {
			...
			let record_id: RecordId = match crate::db::syn::record_id(&id) {
				Ok(x) => x.into(),
				Err(_) => {
...
/// Convert a GraphQL filter input object into a LyxalQL `WHERE` condition.
///
...
/// Operators that map directly to Lyxal binary operators.
...
/// Operators that map to Lyxal function calls.
...
/// Convert a single field's filter object to a LyxalQL expression.
///
