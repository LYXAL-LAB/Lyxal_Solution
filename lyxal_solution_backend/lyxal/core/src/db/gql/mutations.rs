...
use crate::db::catalog::{FieldDefinition, TableDefinition, TableType};
...
use crate::db::expr::{Cond, Data, Expr, Kind, Literal, LogicalPlan, Output, TopLevelExpr};
use crate::kvs::Datastore;
use crate::db::val::{Object as SurObject, RecordId, TableName, Value};
...
fn parse_record_id(table_name: &TableName, id: &str) -> Result<RecordId, GqlError> {
	...
	match crate::db::syn::record_id(&rid_str) {
		Ok(x) => Ok(x.into()),
...
fn parse_full_record_id(id_str: &str) -> Result<RecordId, GqlError> {
	crate::db::syn::record_id(id_str)
		.map(|x| x.into())
		.map_err(|e| resolver_error(format!("Invalid record ID: {id_str}: {e}")))
}

/// Convert a GraphQL input object to a Lyxal Object for use as CONTENT/MERGE data.
///
/// Iterates over the input fields, finds each field's Kind from the field definitions,
/// and converts the GraphQL value to the corresponding Lyxal value.
