...
//! - **Type mapping** -- [`kind_to_type`] converts Lyxal `Kind` descriptors to GraphQL
//!   `TypeRef`s; [`gql_to_sql_kind`] does the reverse conversion for input values.
//! - **Value conversion** -- [`sql_value_to_gql_value`] converts Lyxal runtime values to
...
use crate::db::catalog::providers::{AuthorisationProvider, DatabaseProvider, TableProvider};
...
use crate::db::expr::kind::{GeometryKind, KindLiteral};
use crate::db::expr::{Expr, Kind, Literal};
use crate::db::gql::error::{internal_error, schema_error, type_error};
use crate::db::gql::functions::process_fns;
use crate::db::gql::mutations::process_mutations;
use crate::db::gql::relations::collect_relations;
use crate::db::gql::subscriptions::process_subscriptions;
use crate::db::gql::tables::process_tbs;
...
/// Convert a Lyxal runtime value to a GraphQL output value.
///
...
/// Convert a Lyxal runtime value to GraphQL, using field kind metadata when available.
///
...
/// Map a Lyxal [`Kind`] to a GraphQL [`TypeRef`].
///
/// This is the central type-mapping function: it translates Lyxal's type
...
/// Map a Lyxal [`Kind`] to a GraphQL [`TypeRef`], with optional enum naming scope.
///
...
fn convert_static_record_id_key(
	key: crate::db::expr::RecordIdKeyLit,
...
/// Convert a GraphQL input value to a Lyxal value, guided by the expected [`Kind`].
///
...
/// the appropriate Lyxal value type.  The `kind` parameter tells the function
...
/// duration, uuid, then parsed as LyxalQL expressions.  For `Kind::Either`,
...
/// Convert a GraphQL coordinate value (nested arrays of numbers) to a Lyxal
...
/// Convert a GraphQL geometry input Object (GeoJSON format) to a Lyxal
/// `Geometry` value.
///
...
/// Convert a Lyxal `Geometry` value to a `GqlValue::Object` in GeoJSON format.
///
