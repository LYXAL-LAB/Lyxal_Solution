...
use crate::db::catalog::FunctionDefinition;
use crate::db::dbs::Session;
use crate::db::expr::{Expr, FunctionCall, Kind, LogicalPlan, TopLevelExpr};
use crate::db::gql::schema::kind_to_type_with_enum_prefix;
...
/// - A resolver that converts GraphQL arguments to LyxalQL values, invokes the function via a
		...
		let mut field = Field::new(
			...
			move |ctx| {
				...
				FieldFuture::new(async move {
					...
					// Convert each GraphQL argument to its LyxalQL equivalent
					...
					let func_call = Expr::FunctionCall(Box::new(FunctionCall {
						receiver: crate::db::expr::Function::Custom(fnd1.name.clone()),
						arguments: args,
					}));
					...
					// Convert the LyxalQL result to a GraphQL value
