...
use crate::db::catalog::{Permission, SubscriptionDefinition, SubscriptionFields};
use crate::db::ctx::{Context, FrozenContext};
use crate::db::dbs::{MessageBroker, Options, Statement};
use crate::db::doc::{CursorDoc, Document};
use crate::error::Error;
use crate::db::expr::FlowResultExt as _;
use crate::db::expr::paths::{AC, ID, RD, TK};
...
impl Document {
	...
	async fn lq_compute(
		...
		let session_id = match sess.pick(ID.as_ref()) {
			...
			Value::String(s) => s.parse::<crate::db::val::Uuid>().ok().map(|uuid| uuid.into()),
			_ => None,
		};
	...
	async fn lq_allow(
		...
		if opt.check_perms(crate::db::iam::Action::View)? {
			// Get the table
