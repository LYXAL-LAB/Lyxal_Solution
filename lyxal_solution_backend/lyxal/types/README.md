...
The `lyxal-types` crate acts as a shared public value type system that:
...
```rust
use lyxal_types::{Array, Datetime, Number, Object, Value};
...
```rust
use lyxal_types::{LyxalValue, Value};
...
```rust
use lyxal_types::{Geometry, Value};
use geo::Point;
...
```rust
use lyxal_types::{RecordId, RecordIdKey, Value};
...
```rust
use lyxal_types::{Value, Number, Array, Object};
use std::collections::BTreeMap;
...
```rust
use lyxal_types::{Value, LyxalValue};
...
- `lyxal-types-derive`: For deriving the `LyxalValue` trait
- `lyxal-protocol`: For the Lyxal protocol
