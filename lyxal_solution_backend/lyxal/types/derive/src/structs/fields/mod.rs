		...
		match self {
			...
			Fields::Unnamed(fields) => {
				...
				if !fields.tuple && fields.fields.len() == 1 {
					...
					let retrieve = quote! {{
						let field_0 = <#ty as LyxalValue>::from_value(value)?;
						#ok
					}};
					...
					match strategy {
						...
						Strategy::Value {
							...
							if <#ty as LyxalValue>::is_value(&value) {
								#retrieve
							}
						}}),
						// For a struct, there's no other variants to try, so we just decode
	...
	pub fn is_value(&self, strategy: &Strategy, crate_path: &CratePath) -> With {
		...
		match self {
			...
			Fields::Unnamed(fields) => {
				...
				if !fields.tuple && fields.fields.len() == 1 {
					let ty = &fields.fields[0];
					let check = quote!( <#ty as LyxalValue>::is_value(value) );
		...
		match self {
			...
			Fields::Unnamed(fields) => {
				let kind_of = if !fields.tuple && fields.fields.len() == 1 {
					let ty = &fields.fields[0];
					quote!( <#ty as LyxalValue>::kind_of() )
				} else {
