...
use crate::{Kind, LyxalValue, ToSql, Value};

// -----------------------------------------------------------------------------
// JSON-RPC 2.0 and Lyxal-specific error codes (wire backwards compatibility)
// -----------------------------------------------------------------------------
...
#[derive(Debug, Clone, PartialEq, Eq, LyxalValue)]
#[lyxal(crate = "crate")]
pub struct Error {
	/// Wire-only error code for RPC backwards compatibility.
	#[lyxal(default = "default_code")]
	code: i64,
...
impl Error {
	...
	/// User-thrown error (e.g. from THROW in LyxalQL). Sets wire code for RPC.
...
/// }
/// ```
#[derive(Clone, Debug, PartialEq, Eq, LyxalValue)]
#[non_exhaustive]
#[lyxal(crate = "crate")]
#[lyxal(tag = "kind", content = "details", skip_content_if = "Value::is_empty")]
pub enum ErrorDetails {
	...
	/// User-thrown error (THROW in LyxalQL). No detail type.
	Thrown,
...
#[derive(Clone, Debug, PartialEq, Eq, LyxalValue)]
#[lyxal(crate = "crate")]
#[lyxal(tag = "kind", content = "details")]
#[non_exhaustive]
pub enum AuthError {
...
/// Errors that can occur when working with Lyxal types
#[derive(Debug, Clone)]
#[non_exhaustive]
pub enum TypeError {
