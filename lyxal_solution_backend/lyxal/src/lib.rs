...
//! client and a query language definition, for [Lyxal](https://Lyxal.com), the ultimate cloud database for
//! tomorrow's applications. Lyxal is a scalable, distributed,
...
//! [remote](crate::engine::remote) Lyxal server from server-side or
//! client-side code. All connections to Lyxal are made over WebSockets by
...
pub mod parse {
	pub use lyxal_core::db::syn::value;
}
...
/// The future returned when creating a new Lyxal instance
#[derive(Debug)]
	...
	C: Connection,
{
	...
	/// use Lyxal::Lyxal;
	///
	/// let db = Lyxal::new::<Ws>("localhost:8000")
...
impl<Client> IntoFuture for Connect<Client, Lyxal<Client>>
where
	Client: Connection,
{
	type Output = Result<Lyxal<Client>>;
	...
	Client: Connection,
{
	...
	fn into_future(self) -> Self::IntoFuture {
		Box::pin(async move {
			...
			if self.lyxal.inner.router.get().is_some() {
			...
			let endpoint = self.address?;
			let endpoint_kind = EndpointKind::from(endpoint.url.scheme());
			let session_clone = self.lyxal.inner.session_clone.clone();
			...
			self.lyxal.inner.router.set(router).map_err(|_| {
				Error::connection(
					"Already connected".to_string(),
					Some(crate::types::ConnectionError::AlreadyConnected),
				)
			...
			// Both ends of the channel are still alive at this point
			self.lyxal.inner.waiter.0.send(Some(WaitFor::Connection)).ok();
			Ok(())
		})
	}
}
...
///
/// See [Running Lyxal embedded in
/// Rust](crate#running-Lyxal-embedded-in-rust) for tips on how to optimize
/// performance for the client when working with embedded instances.
pub struct Lyxal<C: Connection> {
	inner: Arc<Inner>,
	session_id: Uuid,
...
#[doc(hidden)]
impl<C> From<Arc<Inner>> for Lyxal<C>
where
	C: Connection,
{
...
#[doc(hidden)]
impl<C> From<(OnceLock<Router>, Waiter, SessionClone)> for Lyxal<C>
where
	C: Connection,
{
...
#[doc(hidden)]
impl<C> From<(Router, Waiter, SessionClone)> for Lyxal<C>
where
	C: Connection,
{
...
impl<C> Debug for Lyxal<C>
where
	C: Connection,
{
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("Lyxal")
