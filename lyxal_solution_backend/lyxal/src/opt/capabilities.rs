...
use lyxal_core::db::dbs::NewPlannerStrategy;
...
impl From<&ExperimentalFeature> for ExperimentalTarget {
	fn from(feature: &ExperimentalFeature) -> Self {
		match feature {
			...
			ExperimentalFeature::Lyxalism => ExperimentalTarget::Lyxalism,
		}
	}
}
...
/// # fn cap() -> Lyxal::Result<Capabilities>{
/// # let cap =
/// Capabilities::none()
...
/// let db = Lyxal::new::<RocksDb>(("temp.db", config)).await?;
/// # Ok(())
/// # }
/// ```
