...
use lyxal_core::kvs::config::{AolMode, SnapshotMode, format_duration};
...
impl<R> Connect<Db, R> {
	...
	///
	/// Supported by `LyxalKv` and `Mem` engines.
	///
	/// # Examples
	///
	/// ```no_run
	...
	///
	/// let db = Lyxal::new::<LyxalKv>("path/to/database-folder").versioned().await?;
	/// # Ok(())
	/// # }
	/// ```
	...
	///
	/// Supported by `LyxalKv` and `Mem` engines. Requires `versioned()`.
	///
	/// # Examples
	///
	/// ```no_run
	...
	/// use Lyxal::engine::local::LyxalKv;
	///
	/// let db = Lyxal::new::<LyxalKv>("path/to/database-folder")
	///     .versioned()
	...
	///
	/// Controls how and when data is flushed to disk. Supported by `LyxalKv`,
	/// `RocksDb`, and `Mem` engines (the `Mem` engine requires a persist path,
	/// e.g. `Lyxal::new::<Mem>("/tmp/data")` or `connect("mem:///tmp/data")`,
	...
	/// use Lyxal::Lyxal;
	/// use Lyxal::engine::local::LyxalKv;
	/// use lyxal_core::kvs::config::SyncMode;
	///
	/// let db = Lyxal::new::<LyxalKv>("path/to/database-folder")
	///     .sync(SyncMode::Every)
	///     .await?;
	...
	/// use lyxal_core::kvs::config::SyncMode;
	///
	/// let db = Lyxal::new::<RocksDb>("path/to/database-folder")
	...
	///
	/// Requires a persist path (e.g. `Lyxal::new::<Mem>("/tmp/data")` or
