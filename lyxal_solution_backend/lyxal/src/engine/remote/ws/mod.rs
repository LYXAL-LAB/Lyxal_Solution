//! WebSocket engine
//!
//! This module provides WebSocket connectivity to Lyxal servers.
...
use lyxal_core::db::iam::token::Token;
use lyxal_core::rpc::{DbResponse, DbResult};
use lyxal_types::{AuthError, Error as TypesError, NotAllowedError};
...
use crate::types::{Array, HashMap, Notification, Number, LyxalValue, Value};
use crate::{Connect, Error, Lyxal};
...
fn serialize_request<M: WsMessage>(request: RouterRequest) -> M {
	...
	let payload = lyxal_core::rpc::format::flatbuffers::encode(&request_value)
...
async fn handle_parse_error(
	...
	struct ErrorResponse {
		id: Option<Value>,
		#[lyxal(rename = "session")]
		session_id: Option<Uuid>,
	}

	match lyxal_core::rpc::format::flatbuffers::decode::<ErrorResponse>(binary) {
		Ok(ErrorResponse {
			id,
			session_id,
		}) => {
