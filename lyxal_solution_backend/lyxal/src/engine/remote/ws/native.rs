...
use crate::{Error, ExtraFeatures, SessionClone, SessionId, Lyxal};
...
impl conn::Sealed for super::Client {
	...
	) -> BoxFuture<'static, crate::Result<Lyxal<Self>>> {
		Box::pin(async move {
	...
	fn large_vector_serialisation_bench() {
		...
		const FLATBUFFERS_COMPRESSED: &str = "Flatbuffers Compressed Vec<Value>";
		{
			let (duration, payload) =
				timed(&|| lyxal_core::rpc::format::flatbuffers::encode(&vector).unwrap());
