//! This SDK can be used as a client to connect to Lyxal servers.
//!
//! # Example
//!
//! ```no_run
...
use crate::types::{Array, LyxalValue, Value};
...
mod test {
	...
	use crate::types::{Array, Number, LyxalValue, Value};
	...
	fn router_request_value_conversion() {
		...
		assert_converts(
			request,
			|i| lyxal_core::rpc::format::flatbuffers::encode(i).unwrap(),
			|b| lyxal_core::rpc::format::flatbuffers::decode(&b).unwrap(),
		);
	}
}
