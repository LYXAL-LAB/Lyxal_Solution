//! HTTP engine for connecting to Lyxal over HTTP/HTTPS.
//!
//! This module provides HTTP-based connectivity to Lyxal servers. While HTTP is
...
//!
//! **Important:** When deploying Lyxal in a multi-node cluster behind a load balancer,
...
//!
//! Lyxal maintains session state on the server, including:
...
use lyxal_core::db::dbs::{QueryResult, QueryResultBuilder};
use lyxal_core::db::iam::Token as CoreToken;
use lyxal_core::rpc::{self, DbResponse, DbResult};
use lyxal_types::{AuthError, NotAllowedError};
#[cfg(not(target_family = "wasm"))]
...
use crate::types::{HashMap, LyxalValue, Value};
...
struct RouterState {
	...
	/// The base URL for the Lyxal server
	base_url: Url,
}
...
///
/// Use this for unencrypted connections to Lyxal servers. For production
/// deployments, consider using [`Https`] instead for encrypted connections.
///
/// # Multi-Node Deployments
///
/// When connecting to a load balancer in front of multiple Lyxal nodes,
...
///
/// Use this for TLS-encrypted connections to Lyxal servers. This is the
...
/// ```no_run
/// use Lyxal::Lyxal;
/// use Lyxal::engine::remote::http::Https;
///
/// # #[tokio::main]
/// # async fn main() -> Lyxal::Result<()> {
/// let db = Lyxal::new::<Https>("localhost:8000").await?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug)]
...
///
/// This client maintains session state and supports all Lyxal operations
...
impl Lyxal<Client> {
	...
	/// use Lyxal::Lyxal;
	/// use Lyxal::engine::remote::http::Client;
	/// use Lyxal::engine::remote::http::Http;
	///
	/// static DB: LazyLock<Lyxal<Client>> = LazyLock::new(Lyxal::init);
	///
	/// # #[tokio::main]
	/// # async fn main() -> Lyxal::Result<()> {
...
pub(crate) fn default_headers() -> HeaderMap {
	...
	headers.insert(ACCEPT, HeaderValue::from_static(lyxal_core::api::format::FLATBUFFERS));
	headers
		.insert(CONTENT_TYPE, HeaderValue::from_static(lyxal_core::api::format::FLATBUFFERS));
	headers
}
...
#[derive(Debug, Serialize, Deserialize, LyxalValue)]
...
async fn import(request: RequestBuilder, path: PathBuf) -> Result<()> {
	...
	let res = request
		.header(ACCEPT, lyxal_core::api::format::FLATBUFFERS)
		.body(file)
		.send()
		.await
		.map_err(crate::std_error_to_types_error)?;
	...
	let value: Value = lyxal_core::rpc::format::flatbuffers::decode(&bytes)
...
/// Sends an RPC request to the Lyxal server and returns the response.
///
/// # Arguments
/// * `req` - The router request containing the RPC method and parameters
/// * `base_url` - The base URL of the Lyxal server
...
async fn send_request(
	...
	let body = lyxal_core::rpc::format::flatbuffers::encode(&req_value)
	...
	let response: DbResponse = lyxal_core::rpc::format::flatbuffers::decode(&bytes)
		.map_err(|x| format!("Failed to deserialize flatbuffers payload: {x}"))
		.map_err(|e| {
			crate::Error::internal(format!("The server returned an unexpected response: {e}"))
		})?;
