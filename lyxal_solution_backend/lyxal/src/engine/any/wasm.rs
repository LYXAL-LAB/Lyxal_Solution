...
use crate::{Error, ExtraFeatures, Result, SessionClone, Lyxal, conn};
...
impl conn::Sealed for Any {
	...
	) -> BoxFuture<'static, Result<Lyxal<Self>>> {
		Box::pin(async move {
			...
			match EndpointKind::from(address.url.scheme()) {
				...
				return Err(
					Error::internal("Cannot connect to the `indxdb` storage engine as it is not enabled in this build of Lyxal".to_owned())
				);
				}
				...
				return Err(
					Error::internal("Cannot connect to the `memory` storage engine as it is not enabled in this build of Lyxal".to_owned())
				);
				}
			...
			return Err(Error::internal("Cannot connect to the `rocksdb` storage engine as it is not enabled in this build of Lyxal".to_owned()));
				}

				EndpointKind::LyxalKv => {
				...
				"Cannot connect to the `lyxalkv` storage engine as it is not enabled in this build of Lyxal".to_owned(),
			));
				}
				...
				return Err(
					Error::internal("Cannot connect to the `tikv` storage engine as it is not enabled in this build of Lyxal".to_owned())
				);
				}
				...
				return Err(Error::internal(
					"Cannot connect to the `HTTP` remote engine as it is not enabled in this build of Lyxal".to_owned(),
				));
				}
				...
				return Err(Error::internal(
					"Cannot connect to the `WebSocket` remote engine as it is not enabled in this build of Lyxal".to_owned(),
				));
				}
