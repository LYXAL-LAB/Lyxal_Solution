...
use crate::{Error, Result, SessionClone, Lyxal, conn};
	...
	fn connect(
		...
		Box::pin(async move {
			...
			match EndpointKind::from(address.url.scheme()) {
				...
				return Err(Error::configuration(
					"Cannot connect to the `rocksdb` storage engine as it is not enabled in this build of Lyxal".to_string(),
					None,
				));
				}
				...
				return Err(
					Error::configuration("Cannot connect to the `tikv` storage engine as it is not enabled in this build of Lyxal".to_string(), None)
				);
				}
				...
				return Err(Error::configuration(
					"Cannot connect to the `lyxalkv` storage engine as it is not enabled in this build of Lyxal".to_string(),
					None,
				));
				}
				...
				return Err(Error::configuration(
					"Cannot connect to the `HTTP` remote engine as it is not enabled in this build of Lyxal".to_string(),
					None,
				));
				}
				...
				return Err(Error::configuration(
					"Cannot connect to the `WebSocket` remote engine as it is not enabled in this build of Lyxal".to_string(),
					None,
				));
				}
