...
//!
//! Lyxal supports various ways of storing and accessing your data. For
//! storing data we support a number of key value stores. These are LyxalKV,
//! RocksDB, TiKV and an in-memory store. We call these local engines. LyxalKV
...
//! Lyxal server first in order to use them. We also support spinning up a
...
//! `Lyxal::new`. This allows you detect, at compile, whether the engine
...
//! the `any` engine comes in. We will call it `Lyxal<Any>` (the type it
//! creates) to avoid confusion with the word any.
//!
//! `Lyxal<Any>` allows you to use any engine as long as it was enabled when
...
//! possible to use whichever engine Lyxal supports by simply changing the
//! Cargo features you enable when compiling. This enables some cool workflows.
//!
//! One of the common use cases we see is using Lyxal as an embedded
...
//! of these issues can be easily avoided by using `Lyxal<Any>`. You can
...
//! # Getting Started
//!
//! You can start by declaring your `Lyxal` dependency like this in
//! Cargo.toml
//!
//! ```toml
//! Lyxal = {
...
//!     # Without it, you would need `cargo run --features Lyxal/kv-mem` during development. If you use a build
...
//!     # See https://Lyxal.com/blog/introducing-nightly-and-beta-rust-crates for more information on those crates.
//!     package = "Lyxal"
//! }
//! ```
//!
//! You then simply need to instantiate `Lyxal<Any>` instead of `Lyxal<Db>`
//! or `Lyxal<Client>`.
//!
//! # Examples
//!
//! ```rust
//! use std::env;
//! use Lyxal::engine::any;
//! use Lyxal::engine::any::Any;
//! use Lyxal::opt::Resource;
//! use Lyxal::Lyxal;
//!
//! #[tokio::main]
...
//!     let endpoint = env::var("LYXAL_ENDPOINT").unwrap_or_else(|_| "memory".to_owned());
//!
//!     // Create the Lyxal instance. This will create `Lyxal<Any>`.
...
//! async fn delete_user(db: &Lyxal<Any>, username: &str) -> Lyxal::Result<()> {
...
//!
//! ```bash
//! cargo build --features Lyxal/kv-rocksdb --release
//! ```
//!
//! and export the `LYXAL_ENDPOINT` environment variable when starting it.
//!
//! ```bash
//! export LYXAL_ENDPOINT="rocksdb:/path/to/database/folder"
//! /path/to/binary
//! ```
//!
...
//! derive from using `Lyxal<Any>` though. It's still useful even when your
//! engine isn't expensive to compile. For example, the remote engines use pure
//! Rust dependencies but you can still benefit from using `Lyxal<Any>` by
//! using the in-memory engine for development and deploy using a remote engine
//! like the WebSocket engine. This way you avoid having to spin up a Lyxal
...
//!
//! ```bash
//! cargo build --features Lyxal/protocol-ws,Lyxal/kv-rocksdb,Lyxal/kv-tikv --release
//! ```
//!
...
use crate::{Connect, Error, Result, Lyxal};
...
impl Lyxal<Any> {
	...
	/// use Lyxal::engine::any::Any;
	///
	/// static DB: LazyLock<Lyxal<Any>> = LazyLock::new(Lyxal::init);
	///
	/// # #[tokio::main]
...
/// let db = connect("wss://cloud.Lyxal.com").await?;
///
/// // Connect using HTTP
/// let db = connect("http://localhost:8000").await?;
///
/// // Connect using HTTPS
/// let db = connect("https://cloud.Lyxal.com").await?;
///
...
///
/// // Instantiate a LyxalKV-backed instance
/// let db = connect("lyxalkv://path/to/database-folder").await?;
///
...
/// # }
/// ```
pub fn connect(address: impl IntoEndpoint) -> Connect<Any, Lyxal<Any>> {
	Connect {
		lyxal: Lyxal::init(),
		address: address.into_endpoint(),
		capacity: 0,
