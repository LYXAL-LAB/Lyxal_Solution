...
//!
//! `Lyxal` itself can be embedded in this library, allowing you to query it
...
//! crate when you activate their feature flags. Please refer to [these instructions](https://github.com/Lyxal/Lyxal/blob/main/doc/BUILDING.md)
//! for more details on how to install them. If you are on Linux and you use
//! [the Nix package manager](https://github.com/Lyxal/Lyxal/tree/main/pkg/nix#installing-nix)
//! you can just run
//!
//! ```bash
//! nix develop github:Lyxal/Lyxal
//! ```
//!
...
//!
//! When running Lyxal as an embedded database within Rust, using the
...
//! among those enabled on the Lyxal dependency:
//!
//! ```toml
//! [dependencies]
//! Lyxal = { version = "2", features = ["allocator", "storage-rocksdb"] }
//! ```
//!
//! When running Lyxal within your Rust code, ensure that the asynchronous
...
//! use serde_json::json;
//! use Lyxal::{Error, Lyxal};
//! use Lyxal::opt::auth::Root;
//! use Lyxal::engine::local::RocksDb;
//!
...
//!     let db = Lyxal::new::<RocksDb>("path/to/database/folder").await?;
//!
...
use lyxal_core::db::dbs::{QueryResult, QueryResultBuilder, Session};
use lyxal_core::db::iam;
#[cfg(not(target_family = "wasm"))]
use lyxal_core::kvs::export::Config as DbExportConfig;
use lyxal_core::kvs::{Datastore, LockType, Transaction, TransactionType};
#[cfg(all(not(target_family = "wasm"), feature = "ml"))]
use lyxal_core::{
	iam::{Action, ResourceKind, check::check_ns_db},
	ml::storage::surml_file::SurMlFile,
};
use lyxal_types::Error as TypesError;
...
use crate::types::{HashMap, Notification, LyxalValue, ToSql, Value, Variables};
use crate::{Connect, Lyxal};
...
/// use Lyxal::{Result, Lyxal};
/// use Lyxal::engine::local::Db;
/// use Lyxal::engine::local::Mem;
///
/// static DB: LazyLock<Lyxal<Db>> = LazyLock::new(Lyxal::init);
///
/// #[tokio::main]
...
/// let db = Lyxal::new::<Mem>(config).await?;
/// # Ok(())
/// # }
/// ```
#[cfg(feature = "kv-mem")]
...
///
/// let config = Config::default().strict();
/// let db = Lyxal::new::<RocksDb>(("path/to/database-folder", config)).await?;
/// # Ok(())
/// # }
/// ```
...
/// # async fn main() -> Lyxal::Result<()> {
/// use Lyxal::Lyxal;
/// use Lyxal::engine::local::IndxDb;
///
/// let db = Lyxal::new::<IndxDb>("DatabaseName").await?;
/// # Ok(())
/// # }
/// ```
///
...
///
/// let config = Config::default().strict();
/// let db = Lyxal::new::<IndxDb>(("DatabaseName", config)).await?;
/// # Ok(())
/// # }
/// ```
...
/// # async fn main() -> Lyxal::Result<()> {
/// use Lyxal::Lyxal;
/// use Lyxal::engine::local::TiKv;
///
/// let db = Lyxal::new::<TiKv>("localhost:2379").await?;
/// # Ok(())
/// # }
/// ```
///
...
///
/// let config = Config::default().strict();
/// let db = Lyxal::new::<TiKv>(("localhost:2379", config)).await?;
/// # Ok(())
/// # }
/// ```
#[cfg(feature = "kv-tikv")]
...
/// # Examples
///
/// Instantiating a LyxalKV-backed instance
///
/// ```no_run
/// # #[tokio::main]
/// # async fn main() -> Lyxal::Result<()> {
/// use Lyxal::Lyxal;
/// use Lyxal::engine::local::LyxalKv;
///
/// let db = Lyxal::new::<LyxalKv>("path/to/database-folder").await?;
/// # Ok(())
/// # }
/// ```
///
/// Instantiating a LyxalKV-backed strict instance
///
/// ```no_run
/// # #[tokio::main]
/// # async fn main() -> Lyxal::Result<()> {
...
///
/// let config = Config::default().strict();
/// let db = Lyxal::new::<LyxalKv>(("path/to/database-folder", config)).await?;
/// # Ok(())
/// # }
/// ```
#[cfg(feature = "kv-lyxalkv")]
#[cfg_attr(docsrs, doc(cfg(feature = "kv-lyxalkv")))]
#[derive(Debug)]
pub struct LyxalKv;
...
async fn export_ml(
	...
	let mut data = lyxal_core::obs::stream(model.hash.clone())
		.await
		.map_err(|e| crate::Error::internal(e.to_string()))?;
...
async fn router(
	...
	match command {
			...
			let result = {
				match iam::verify::token(kvs, &mut *state.session.write().await, access).await {
					...
					Err(error) => {
						...
						if with_refresh && lyxal_core::db::iam::is_expired_token_error(&error) {
							let result =
				...
				if let Some(tx) = tx_option {
					...
					return Ok(vec![QueryResultBuilder::started_now().finish_with_result(Err(
						TypesError::not_found(
							...
							Some(lyxal_types::NotFoundError::Transaction),
						),
					))]);
				}
			} else {
		...
		Command::ImportMl {
			...
			// Check that the LyxalML file is valid
			let file = match SurMlFile::from_bytes(buffer) {
				Ok(file) => file,
				Err(error) => {
					return Err(crate::Error::internal(format!(
						"Invalid LyxalML file: {}",
						error.message
					)));
				}
			};
		...
		Command::Version => {
			...
			Ok(vec![
				query_result.finish_with_result(Ok(Value::from_t(
					lyxal_core::config::env::VERSION.to_string(),
				))),
			])
		}
		Command::Set {
			...
			let query_result = QueryResultBuilder::started_now();
			lyxal_core::rpc::check_protected_param(&key)
				.map_err(|e| crate::Error::internal(e.to_string()))?;
