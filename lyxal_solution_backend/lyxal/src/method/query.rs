...
use lyxal_core::db::dbs::QueryType;
...
use crate::{Connection, Error, Result, Lyxal, opt};
...
pub struct Query<'r, C: Connection> {
	...
	pub(crate) client: Cow<'r, Lyxal<C>>,
...
impl<T: LyxalValue> IntoVariables for T {
	fn into_variables(self) -> Result<Variables> {
...
/// The response type of a `Lyxal::query` request
#[derive(Debug)]
