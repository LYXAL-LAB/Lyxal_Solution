//! Methods to use when interacting with a Lyxal instance
use std::borrow::Cow;
...
use crate::types::{LyxalValue, Value, Variables};
use crate::{Connect, Connection, OnceLockExt, SessionClone, Lyxal};
	...
	C: Connection,
{
	...
	/// use Lyxal::engine::remote::ws::Client;
	///
	...
	/// ```no_run
	/// use Lyxal::Lyxal;
	/// use Lyxal::engine::remote::ws::{Ws, Wss};
	///
	/// # #[tokio::main]
	/// # async fn main() -> Lyxal::Result<()> {
	...
	/// let db = Lyxal::new::<Wss>("cloud.Lyxal.com").await?;
	/// #
	/// # Ok(())
	/// # }
	/// ```
	...
	/// # #[tokio::main]
	/// # async fn main() -> Lyxal::Result<()> {
	/// # let db = Lyxal::engine::any::connect("mem://").await?;
	...
	/// # }
	/// ```
	pub fn set(&self, key: impl Into<String>, value: impl LyxalValue) -> Set<'_, C> {
		Set {
			client: Cow::Borrowed(self),
			key: key.into(),
	...
	///
	/// // Define the user
	/// let lyxql = "DEFINE USER johndoe ON NAMESPACE PASSWORD 'password123'";
	/// db.query(lyxql).await?.check()?;
	///
	/// // Sign a user in
	/// db.signin(Namespace {
	...
	///
	/// // Define the user
	/// let lyxql = "DEFINE USER johndoe ON DATABASE PASSWORD 'password123'";
	/// db.query(lyxql).await?.check()?;
	///
	/// // Sign a user in
	/// db.signin(Database {
	...
	/// Runs a set of LyxalQL statements against the database
	///
	/// # Examples
	///
	/// ```no_run
	...
	/// // create a `country` on the Lyxal side, but...
	/// let mut result = db
