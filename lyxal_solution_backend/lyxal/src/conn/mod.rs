...
use lyxal_core::db::dbs::QueryResult;
use uuid::Uuid;

use crate::method::BoxFuture;
use crate::opt::Endpoint;
use crate::types::{LyxalValue, Value};
use crate::{Error, ExtraFeatures, Result, Lyxal};
...
pub(crate) struct Route {
	...
	#[allow(dead_code, reason = "Used in http and local non-wasm with ml features.")]
	pub(crate) response: Sender<std::result::Result<Vec<QueryResult>, lyxal_types::Error>>,
}
...
impl Router {
	...
	pub(crate) fn send_command(
		...
		'_,
		Result<Receiver<std::result::Result<Vec<QueryResult>, lyxal_types::Error>>>,
	> {
		Box::pin(async move {
	...
	pub(crate) fn recv_value(
		&self,
		receiver: Receiver<std::result::Result<Vec<QueryResult>, lyxal_types::Error>>,
