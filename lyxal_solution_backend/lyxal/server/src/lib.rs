//! Library entrypoints for embedding Lyxal server inside another Rust application.
//! Exposes the same init() used by the `lyxal` binary so external apps can
//! start Lyxal within their own `main()`.
//!
...
/// when running Lyxal as a library.
pub mod ntw;
...
/// Re-export `RouterFactory` for convenience so embedders can `use lyxal::RouterFactory`.
#[doc(inline)]
pub use ntw::RouterFactory;
/// Re-export `RpcState` for convenience so embedders can `use lyxal::RpcState`.
#[doc(inline)]
pub use rpc::RpcState;
#[doc(inline)]
pub use lyxal as sdk;
/// Re-export `core` for convenience so embedders can `use lyxal::core::...`.
#[doc(inline)]
pub use lyxal_core as core;
...
/// Initialize Lyxal CLI/server with the same behavior as the `lyxal` binary.
...
/// for Lyxal's query parser and execution engine. This function creates a Tokio
...
fn with_enough_stack(fut: impl Future<Output = ExitCode> + Send) -> ExitCode {
	...
	let runtime = tokio::runtime::Builder::new_multi_thread()
		...
		.thread_name("lyxal-worker")
		// When a thread is parked, ensure that local memory
		// tracking is flushed to the global tracking counter.
		.on_thread_park(|| lyxal_core::allocator::mem::ALLOC.flush_local_allocations())
		// When a thread is shutdown, ensure that local memory
		// tracking is flushed to the global tracking counter.
		.on_thread_stop(|| lyxal_core::allocator::mem::ALLOC.flush_local_allocations())
		// Build the runtime
		.build();
