...
use lyxal_core::map;
use crate::core::db::syn;
use lyxal_types::{Array, LyxalValue, Value, Variables, vars};
...
async fn select_all(
	...
	let vars = vars! {
		...
		"fields": Value::Array(Array::from(query.fields.unwrap_or_default().into_iter().map(LyxalValue::into_value).collect::<Vec<Value>>())),
	};
...
async fn select_one(
	...
	// Parse the Record ID as a LyxalQL value
	...
	let vars = Variables::from(map! {
		String::from("table") => Value::String(table),
		String::from("id") => rid,
		String::from("fields") => Value::Array(Array::from(query.fields.unwrap_or_default().into_iter().map(LyxalValue::into_value).collect::<Vec<Value>>())),
	});
	// Execute the query and return the result
	execute_and_return(db, sql, &session, vars, accept.as_deref(), None)
		.await
