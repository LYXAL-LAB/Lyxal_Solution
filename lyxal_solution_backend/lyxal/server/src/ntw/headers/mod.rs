//! HTTP header types and utilities for Lyxal's network layer.
//!
//! This module provides custom typed headers for Lyxal-specific HTTP headers
//! (e.g., `Lyxal-NS`, `Lyxal-DB`, `Lyxal-Auth-*`) and utilities for managing
...
pub use auth_db::LyxalAuthDatabase;
pub use auth_ns::LyxalAuthNamespace;
pub use content_type::ContentType;
pub use db::LyxalDatabase;
pub use id::LyxalId;
pub use ns::LyxalNamespace;
...
/// Creates a Tower middleware layer that adds the Lyxal version header to responses.
///
/// When enabled, adds a custom version header (e.g., `lyxal/2.0.0`) to HTTP responses.
...
///
/// When enabled, adds the standard HTTP `Server` header with Lyxal's server name.
/// This header is only added if not already present in the response.
///
/// # Parameters
...
pub fn add_server_header(
	...
	add_header(enabled, PKG_NAME.to_owned(), &SERVER)
}
