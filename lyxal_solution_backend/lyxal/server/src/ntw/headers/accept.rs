...
use crate::ntw::headers::content_type::{
	...
	HEADER_VALUE_APPLICATION_OCTET_STREAM, HEADER_VALUE_APPLICATION_LYXAL_DB_FLATBUFFERS,
...
impl std::fmt::Display for Accept {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			Accept::TextPlain => f.write_str(lyxal_core::api::format::PLAIN),
			Accept::ApplicationJson => f.write_str(lyxal_core::api::format::JSON),
			Accept::ApplicationCbor => f.write_str(lyxal_core::api::format::CBOR),
			Accept::ApplicationOctetStream => {
				f.write_str(lyxal_core::api::format::OCTET_STREAM)
			}
			Accept::ApplicationFlatbuffers => f.write_str(lyxal_core::api::format::FLATBUFFERS),
		}
	}
}
...
impl Header for Accept {
		...
		I: Iterator<Item = &'i HeaderValue>,
	{
		...
		match parts[0] {
			lyxal_core::api::format::ANY => Ok(Accept::ApplicationJson),
			lyxal_core::api::format::PLAIN => Ok(Accept::TextPlain),
			lyxal_core::api::format::JSON => Ok(Accept::ApplicationJson),
			lyxal_core::api::format::CBOR => Ok(Accept::ApplicationCbor),
			lyxal_core::api::format::OCTET_STREAM => Ok(Accept::ApplicationOctetStream),
			lyxal_core::api::format::FLATBUFFERS => Ok(Accept::ApplicationFlatbuffers),
