...
mod implementation {
	...
	use lyxal_core::db::iam::{Action, ResourceKind};
	use lyxal_core::ml::storage::surml_file::SurMlFile;
	...
	pub async fn export(
		...
		let mut data = lyxal_core::obs::stream(path)
			.await
			.context("Failed to read model file")
			.map_err(ResponseError)?;
		// Create a chunked response
		let (chn, body_stream) = lyxal::channel::bounded::<Result<Bytes, anyhow::Error>>(1);
