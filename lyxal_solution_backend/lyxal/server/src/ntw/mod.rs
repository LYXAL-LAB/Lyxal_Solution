...
use http::header;
use lyxal::headers::{AUTH_DB, AUTH_NS, DB, ID, NS};
...
/// additional middleware. The default binary uses [`LyxalComposer`].
///
/// # Examples
///
...
/// use lyxal::RouterFactory;
/// use lyxal::rpc::RpcState;
/// use lyxal::core::LyxalComposer;
///
/// struct MyComposer;
///
/// impl RouterFactory for MyComposer {
///     fn configure_router() -> Router<Arc<RpcState>> {
///         let router = LyxalComposer::configure_router();
///         router.merge(
///             Router::new()
...
/// ```
///
/// [`LyxalComposer`]: lyxal_core::LyxalComposer
...
///
/// Provides the standard set of HTTP routes shipped with the `lyxal` binary.
/// Consumers embedding Lyxal can implement `RouterFactory` on their own
/// composer to customize routes.
impl RouterFactory for LyxalComposer {
	fn configure_router() -> Router<Arc<RpcState>> {
...
pub async fn init<F: RouterFactory>(
	...
	let service = service
		...
		.layer(AsyncRequireAuthorizationLayer::new(auth::LyxalAuth))
	...
	let rpc_state = Arc::new(RpcState::new(ds.clone(), lyxal_core::db::dbs::Session::default()));
