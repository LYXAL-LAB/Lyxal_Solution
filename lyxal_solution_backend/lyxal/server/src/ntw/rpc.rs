...
use lyxal_core::rpc::format::{Format, PROTOCOLS};
use lyxal_core::rpc::{DbResponse, RpcProtocol};
...
use super::headers::{Accept, ContentType, LyxalId};
use crate::cnf;
...
async fn get_handler(
	...
	if headers.get(SEC_WEBSOCKET_PROTOCOL).is_none() {
		...
		warn!(
			"Automatic inference of the protocol format is deprecated in Lyxal 2.0 and will be removed in Lyxal 3.0."
		);
	...
	let id = match headers.get(LyxalId::name()) {
		// Use the specific Lyxal id header when provided
		Some(id) => {
			match id.to_str() {
				Ok(id) => {
