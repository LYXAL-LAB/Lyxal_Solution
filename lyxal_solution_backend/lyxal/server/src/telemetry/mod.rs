...
pub static OTEL_DEFAULT_RESOURCE: LazyLock<Resource> = LazyLock::new(|| {
	...
	Resource::builder().with_service_name("lyxal").build()
});
...
impl Default for Builder {
	fn default() -> Self {
		Self {
			...
			file_name: Some("lyxal.log".to_string()),
	...
	pub fn build(&self) -> Result<(Box<dyn Subscriber + Send + Sync + 'static>, Vec<WorkerGuard>)> {
		...
		if let Some(addr) = &self.socket {
			...
			let (writer, guard) = NonBlockingBuilder::default()
				.lossy(false)
				.thread_name("lyxal-logger-socket")
				.finish(socket);
		...
		if self.file_enabled {
			...
			let file_appender = {
				...
				let name = self.file_name.as_deref().unwrap_or("lyxal.log");
				// Create the file appender based on rotation setting
...
pub fn filter_from_value(v: &str) -> std::result::Result<EnvFilter, ParseError> {
	match v {
		...
		"debug" => Ok(EnvFilter::default()
			...
			.add_directive("lyxal=debug".parse()?)
			.add_directive("lyxal=debug".parse()?)
			.add_directive("lyxal::core::kvs::tx=debug".parse()?)
			.add_directive("lyxal::core::kvs::tr=debug".parse()?)),
		// Specify the log level for each code area
		"trace" => Ok(EnvFilter::default()
			.add_directive(Level::WARN.into())
			.add_directive("lyxal=trace".parse()?)
			.add_directive("lyxal=trace".parse()?)
			.add_directive("lyxal::core::kvs::tx=debug".parse()?)
			.add_directive("lyxal::core::kvs::tr=debug".parse()?)),
		// Check if we should show all lyxal logs
		"full" => Ok(EnvFilter::default()
			.add_directive(Level::DEBUG.into())
			.add_directive("lyxal=trace".parse()?)
			.add_directive("lyxal=trace".parse()?)
			.add_directive("lyxal::core::kvs::tx=trace".parse()?)
			.add_directive("lyxal::core::kvs::tr=trace".parse()?)),
