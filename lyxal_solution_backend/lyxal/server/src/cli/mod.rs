...
use lyxal_core::kvs::TransactionBuilderFactory;
...
const INFO: &str = "
To get started using Lyxal, and for guides on connecting to and building applications
on top of Lyxal, check out the Lyxal documentation (https://lyxal.com/docs).

If you have questions or ideas, join the Lyxal community (https://lyxal.com/community).

If you find a bug, submit an issue on GitHub (https://github.com/lyxal/lyxal/issues).

We would love it if you could star the repository (https://github.com/lyxal/lyxal).

----------
";

#[derive(Parser, Debug)]
#[command(name = "Lyxal command-line interface and server", bin_name = "lyxal")]
...
struct Cli {
	...
	#[arg(env = "LYXAL_LOG_SOCKET_FORMAT", long = "log-socket-format")]
	#[arg(global = true)]
	#[arg(default_value = "text")]
	#[arg(value_enum)]
	...
	#[arg(env = "LYXAL_ONLINE_VERSION_CHECK", long)]
...
enum Commands {
	...
	*/
	#[command(about = "Import a LyxalQL script into an existing database")]
	Import(ImportCommandArguments),
	#[command(about = "Export an existing database as a LyxalQL script")]
	...
	#[command(subcommand, about = "Manage LyxalML models within an existing database")]
	Ml(MlCommand),
	#[cfg(feature = "lyxalism")]
	#[command(subcommand, about = "Manage and execute WASM modules", name = "module")]
	Module(ModuleCommand),
	#[command(
		about = "Check if the Lyxal server is ready to accept connections",
		visible_alias = "isready"
	)]
	IsReady(IsReadyCommandArguments),
	#[command(about = "Validate LyxalQL query files")]
	...
	if args.online_version_check {
		match version_client::new(Some(Duration::from_millis(500))) {
			Ok(client) => match check_upgrade(&client, PKG_VERSION.deref()).await {
				Ok(Some(v)) => warn!("A new version of Lyxal is available: {v}"),
				Ok(None) => {}
				Err(e) => {
					error!("{}", e);
					warn!("A new version of Lyxal may be available.");
				}
			},
			Err(e) => {
				error!("{}", e);
