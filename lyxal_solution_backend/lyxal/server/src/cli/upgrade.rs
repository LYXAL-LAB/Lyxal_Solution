...
pub(crate) const ROOT: &str = "https://download.lyxal.com";
...
pub async fn init(args: UpgradeCommandArguments) -> Result<()> {
	...
	let download_filename = format!("lyxal-{new_version}.{suffix}");
	...
	if suffix.ends_with("tgz") {
		...
		tmp_path = tmp_dir.path().join("lyxal");
	}
	...
	if args.dry_run {
		...
		println!("Lyxal successfully upgraded");
	}
