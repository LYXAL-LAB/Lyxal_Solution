...
pub async fn init(path: Option<PathBuf>, out: Option<PathBuf>) -> Result<()> {
	...
	// Pack the optimized WASM into a Lyxalism package
	let package = LyxalismPackage {
		config,
		wasm,
	};
	let out = resolve_output_path(out, &package.config)?;
	package.pack(out).prefix_err(|| "Failed to pack Lyxalism package")?;

	Ok(())
}

fn load_config(path: &Path) -> Result<LyxalismConfig> {
	let lyxalism_toml = path.join("lyxalism.toml");
	if !lyxalism_toml.exists() {
		anyhow::bail!("lyxalism.toml not found in the current directory");
	}

	LyxalismConfig::parse(
		&fs::read_to_string(&lyxalism_toml).prefix_err(|| "Failed to read lyxalism.toml")?,
	)
}
...
fn resolve_output_path(out: Option<PathBuf>, config: &LyxalismConfig) -> Result<PathBuf> {
	match out {
		None => {
