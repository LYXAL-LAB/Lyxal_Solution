...
pub async fn get_existing_version(file: &Path) -> Option<Version> {
	...
	let res = res
		.inspect_err(|e| warn!("Could not run cached lyxal v2 binary within timelimit: {e}"))
		.ok()?
		.inspect_err(|e| warn!("Failed to run cached lyxal v2 binary: {e}"))
		.ok()?;
...
pub async fn get_latest_version() -> Option<Version> {
	let body = reqwest::get("https://download.lyxal.com/v2.txt")
		.await
...
pub async fn ask_permission(source: &str, path: &Path) -> bool {
	print!(
		"Can I download the latest 2.0 lyxal binary from '{source}' to '{}'? [y/n] ",
		path.display()
	);
...
pub async fn download_v2(
	...
	let temp_path = std::env::temp_dir().join(format!("lyxal-{}", rand));

	let suffix = file_platform_suffix()?;
	let url = format!("https://download.lyxal.com/v{version}/lyxal-v{version}.{suffix}");
	...
	let res =
		reqwest::get(url).await.context("Could not access lyxal v2 binary download page")?;
...
pub async fn ensure_binary_present(has_permission: bool) -> Result<PathBuf> {
	let dir = dirs::cache_dir().unwrap_or_else(std::env::temp_dir).join("lyxal");
	let file = dir.join("lyxal_v2");

	if tokio::fs::try_exists(&file).await.context("Cannot access system cache directory")? {
		let meta = tokio::fs::metadata(&file)
			.await
			.context("Cannot read metadata of cached v2 lyxal executable")?;
		let modified =
			meta.modified().context("Cannot read modified time of lyxal executable")?;
			...
			if let Some(exiting_version) = get_existing_version(&file).await {
						...
						download_v2(has_permission, &file, &latest_version).await?
				{
					tokio::fs::remove_file(&file)
						.await
						.context("Cannot access cached lyxal v2 binary")?;
				...
				let Some(latest_version) = get_latest_version().await else {
					bail!(
						"Could not retrieve latest lyxal version, cannot download nor run the v2 binary"
					);
				};
		...
		if let Some(tmp_path) = download_v2(has_permission, &file, &latest_version).await? {
			tokio::fs::create_dir_all(dir)
				.await
				.context("Could not create lyxal v2 binary cache directory")?;
			tokio::fs::rename(tmp_path, &file)
				.await
				.context("Could not rename downloaded binary")?;
			Ok(file)
...
pub async fn init(args: V2Commands) -> Result<()> {
	...
	let mut child = tokio::process::Command::new(path)
		.args(&args.args)
		.spawn()
		.context("Could not run 2.0 lyxal binary file")?;

	let res = child.wait().await?;
	let err = res.code().unwrap_or(-1);
	ensure!(res.success(), "Lyxal v2 process failed with statuscode: {err}");
	Ok(())
}
