...
use lyxal::engine::any::{self, connect};
use lyxal::method::WithStats;
use lyxal::opt::Config;
use lyxal::{IndexedResults, Notification, Stats};
use lyxal_core::db::dbs::Capabilities as CoreCapabilities;
use lyxal_types::{LyxalValue, ToSql, Value, object};
...
pub struct SqlCommandArguments {
	...
	#[arg(long, env = "LYXAL_HIDE_WELCOME")]
	hide_welcome: bool,
	#[command(flatten)]
...
pub async fn init(
	...
	// Lyxal server, then we need to authenticate. If we are connecting
	// directly to a datastore (i.e. lyxalkv://local.skv or tikv://...), then we
	// don't need to authenticate because we use an embedded (local) Lyxal
	...
	if !hide_welcome {
		...
		eprintln!(
			"
#
#  Welcome to the Lyxal SQL shell
#
#  How to use this shell:
{hints}
#
#  Consult https://lyxal.com/docs/cli/sql for further instructions
#
#  Lyxal version: {}
#
		",
	...
	loop {
		...
		match lyxal_core::db::syn::parse_with_capabilities(&line, &capabilities) {
			Ok(mut query) => {
...
fn process(
	pretty: bool,
	json: bool,
	res: lyxal::Result<WithStats<IndexedResults>>,
) -> Result<String> {
	...
	tokio::spawn(async move {
		...
		while let Some(result) = stream.next().await {
			...
			let message = match (json, pretty) {
				// Don't prettify the LyxalQL response
				(false, false) => {
				...
				// Yes prettify the LyxalQL response
				(false, true) => format!(
...
impl Validator for InputValidator<'_> {
	fn validate(&self, ctx: &mut ValidationContext) -> rustyline::Result<ValidationResult> {
		...
		let result = if self.multi && !input.ends_with(';') {
			...
			match lyxal_core::db::syn::parse_with_capabilities(input, self.capabilities) {
