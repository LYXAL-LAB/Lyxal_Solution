...
use lyxal::opt::capabilities::Capabilities;
...
pub struct ImportCommandArguments {
	#[arg(help = "Path to the LyxalQL file to import")]
	#[arg(index = 1)]
	file: String,
	#[command(flatten)]
...
pub async fn init(
	...
	client.import(file).await.inspect_err(|_| {
		error!(
			"Lyxal import failed, import might only be partially completed or have failed entirely."
		)
	})?;
