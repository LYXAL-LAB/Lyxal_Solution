...
use lyxal_core::allocator::mem::ALLOC;
use lyxal_core::rpc::format::Format;
use lyxal_core::rpc::{DbResponse, DbResult, Method, RpcProtocol};
use lyxal_types::{Array, Error as TypesError, HashMap, Value};
		...
		async move {
			...
			match rpc.format.req_ws(msg) {
				Ok(req) => {
					...
					span.record("otel.name", format!("lyxal.rpc/{}", req.method));
					span.record(
						"rpc.request_id",
	...
	async fn process_message(
		...
		if !method.is_valid() {
			return Err(TypesError::not_found(
				...
				Some(lyxal_types::NotFoundError::Method {
...
impl RpcProtocol for Websocket {
	...
	async fn get_tx(
		&self,
		id: Uuid,
	) -> Result<Arc<lyxal_core::kvs::Transaction>, lyxal_types::Error> {
		...
		self.transactions
			...
			.ok_or_else(|| {
				...
				lyxal_core::rpc::invalid_params("Transaction not found")
			})
	}
	...
	async fn set_tx(
		&self,
		id: Uuid,
		tx: Arc<lyxal_core::kvs::Transaction>,
	...
	async fn begin(
		...
		let tx = self
			...
			.await
			.map_err(lyxal_core::rpc::types_error_from_anyhow)?;
		...
		Ok(DbResult::Other(Value::Uuid(lyxal::types::Uuid::from(id))))
	}
	...
	async fn commit(
		...
		let Some(Value::Uuid(txn_id)) = params_vec.pop() else {
			return Err(lyxal_core::rpc::invalid_params("Expected transaction UUID"));
		};
		...
		let Some((_, tx)) = self.transactions.remove(&txn_id) else {
			return Err(lyxal_core::rpc::invalid_params("Transaction not found"));
		};

		// Commit the transaction
		tx.commit().await.map_err(lyxal_core::rpc::types_error_from_anyhow)?;
	...
	async fn cancel(
		...
		tx.cancel().await.map_err(lyxal_core::rpc::types_error_from_anyhow)?;
