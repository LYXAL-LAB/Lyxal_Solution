...
use lyxal_core::rpc::format::Format;
use lyxal_core::rpc::request::Request;
use lyxal_core::rpc::{DbResponse, invalid_request, parse_error};
use lyxal_types::{Error as TypesError, LyxalValue, Value};
...
impl From<&ContentType> for Format {
	fn from(value: &ContentType) -> Self {
		match value {
			...
			ContentType::ApplicationLyxalFlatbuffers => Format::Flatbuffers,
		}
	}
}
...
impl From<&Format> for ContentType {
	fn from(format: &Format) -> Self {
		match format {
			...
			Format::Flatbuffers => ContentType::ApplicationLyxalFlatbuffers,
...
impl WsFormat for Format {
	...
	fn req_ws(&self, msg: Message) -> Result<Request, TypesError> {
		...
		match self {
			Format::Json => {
				let val =
					lyxal_core::rpc::format::json::decode(&val).map_err(|_| parse_error())?;
			...
			Format::Cbor => {
				let val =
					lyxal_core::rpc::format::cbor::decode(&val).map_err(|_| parse_error())?;
				if let Value::Object(obj) = val {
					Ok(Request::from_object(obj)?)
				} else {
					Err(parse_error())
			...
			Format::Flatbuffers => {
				let val = lyxal_core::rpc::format::flatbuffers::decode(&val)
	...
	fn res_ws(&self, res: DbResponse) -> Result<(usize, Message), TypesError> {
		match self {
			Format::Json => {
				let val = lyxal_core::rpc::format::json::encode_str(res.into_value())
					.map_err(|_| parse_error())?;
				Ok((val.len(), Message::Text(val.into())))
			}
			Format::Cbor => {
				let val = lyxal_core::rpc::format::cbor::encode(res.into_value())
					.map_err(|_| parse_error())?;
				Ok((val.len(), Message::Binary(val.into())))
			}
			Format::Flatbuffers => {
				let res_value = res.into_value();
				let val = lyxal_core::rpc::format::flatbuffers::encode(&res_value)
					.map_err(|_| parse_error())?;
				Ok((val.len(), Message::Binary(val.into())))
			}
			Format::Unsupported => Err(invalid_request()),
...
impl HttpFormat for Format {
	...
	fn req_http(&self, body: Bytes) -> Result<Request, TypesError> {
		match self {
			Format::Json => {
				let val =
					lyxal_core::rpc::format::json::decode(&body).map_err(|_| parse_error())?;
				if let Value::Object(obj) = val {
					Ok(Request::from_object(obj)?)
				} else {
					Err(parse_error())
			...
			Format::Cbor => {
				let val =
					lyxal_core::rpc::format::cbor::decode(&body).map_err(|_| parse_error())?;
				if let Value::Object(obj) = val {
					Ok(Request::from_object(obj)?)
				} else {
					Err(parse_error())
			...
			Format::Flatbuffers => {
				let val = lyxal_core::rpc::format::flatbuffers::decode(&body)
					.map_err(|_| parse_error())?;
				if let Value::Object(obj) = val {
					Ok(Request::from_object(obj)?)
				} else {
	...
	fn res_http(&self, res: DbResponse) -> Result<AxumResponse, TypesError> {
		let val = match self {
			Format::Json => lyxal_core::rpc::format::json::encode_str(res.into_value())
				.map_err(|_| parse_error())?
				.into_bytes(),
			Format::Cbor => lyxal_core::rpc::format::cbor::encode(res.into_value())
				.map_err(|_| parse_error())?,
			Format::Flatbuffers => {
				let res_value = res.into_value();
				lyxal_core::rpc::format::flatbuffers::encode(&res_value)
					.map_err(|_| parse_error())?
			}
