...
use serde_json::json;
use lyxal::IndexedResults;
use lyxal::opt::auth::{Database, Namespace, Record as RecordAccess, Token};
use lyxal::opt::{Config, PatchOp, PatchOps, Resource};
use lyxal::types::{RecordId, RecordIdKey, LyxalValue, Value, array, object};
use lyxal_core::syn;
...
pub async fn select_records_fetch(new_db: impl CreateDb) {
	...
	let check_fetch = |mut response: IndexedResults, expected: &str| {
		...
		let exp = lyxal::parse::value(expected).unwrap();
		assert_eq!(val, exp);
	};
...
#[derive(Debug, LyxalValue, Eq, PartialEq, Ord, PartialOrd)]
struct Name {
	first: String,
	last: String,
}

#[derive(Debug, LyxalValue, PartialEq, PartialOrd)]
struct Person {
	id: Option<RecordId>,
	title: String,
	name: Name,
...
pub async fn patch_record_id(new_db: impl CreateDb) {
	#[derive(Debug, LyxalValue, PartialEq)]
	struct Record {
		id: RecordId,
		baz: String,
		hello: Vec<String>,
...
pub async fn client_side_transactions(new_db: impl CreateDb) {
	#[derive(Debug, Clone, LyxalValue, PartialEq)]
	struct User {
		name: String,
		email: String,
	}
