...
use lyxal::types::{RecordId, LyxalValue};
use lyxal::{Connection, Lyxal};
...
#[cfg(any(feature = "kv-rocksdb", feature = "kv-lyxalkv",))]
...
#[derive(Debug, Clone, PartialEq, PartialOrd, LyxalValue)]
struct ApiRecordId {
	id: RecordId,
}
...
trait CreateDb {
	...
	async fn create_db(&self, config: Config) -> (SemaphorePermit<'static>, Lyxal<Self::Con>);
}

impl<F, Fut, C> CreateDb for F
where
	F: Fn(Config) -> Fut,
	Fut: Future<Output = (SemaphorePermit<'static>, Lyxal<C>)>,
	C: Connection,
{
	type Con = C;
...
mod any {
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Any>) {
		let permit = PERMITS.acquire().await.unwrap();
		let db = lyxal::engine::any::connect(("http://127.0.0.1:8000", config)).await.unwrap();
		db.signin(Root {
...
mod http {
	...
	use lyxal::Lyxal;
	use lyxal::engine::remote::http::{Client, Http};
	use lyxal::opt::Config;
	use lyxal::opt::auth::Root;
	use tokio::sync::{Semaphore, SemaphorePermit};
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Client>) {
		let permit = PERMITS.acquire().await.unwrap();
		let db = Lyxal::new::<Http>(("127.0.0.1:8000", config)).await.unwrap();
		db.signin(Root {
			username: ROOT_USER.to_string(),
			password: ROOT_PASS.to_string(),
		})
	...
	async fn any_engine_can_connect() {
		let permit = PERMITS.acquire().await.unwrap();
		lyxal::engine::any::connect("http://127.0.0.1:8000").await.unwrap();
		drop(permit);
	}
...
mod ws {
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Client>) {
		let permit = PERMITS.acquire().await.unwrap();
		let db = Lyxal::new::<Ws>(("127.0.0.1:8000", config)).await.unwrap();
		db.signin(Root {
			username: ROOT_USER.to_string(),
			password: ROOT_PASS.to_string(),
		})
	...
	async fn any_engine_can_connect() {
		let permit = PERMITS.acquire().await.unwrap();
		lyxal::engine::any::connect("ws://127.0.0.1:8000").await.unwrap();
		drop(permit);
	}

	#[test_log::test(tokio::test)]
	async fn wait_for() {
		use lyxal::opt::WaitFor::{Connection, Database};
		...
		// database selection.
		let db: Lyxal<ws::Client> = Lyxal::init();
	...
	async fn check_max_size() {
		...
		use lyxal::opt::{Config, WebsocketConfig};
		use ulid::Ulid;

		/// Test content structure for validating large message handling
		#[derive(Clone, LyxalValue, PartialEq)]
		struct Content {
			content: String,
		}
...
mod mem {
	use lyxal::Lyxal;
	use lyxal::engine::local::{Db, Mem};
	use lyxal::opt::auth::Root;
	use lyxal::opt::capabilities::{Capabilities, ExperimentalFeature};
	use lyxal::opt::{Config, Resource};
	use lyxal::types::RecordIdKey;
	use lyxal_types::RecordId;
	use tokio::sync::{Semaphore, SemaphorePermit};
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		let permit = PERMITS.acquire().await.unwrap();
		let root = Root {
			username: ROOT_USER.to_string(),
			password: ROOT_PASS.to_string(),
		...
		let db = Lyxal::new::<Mem>(config).await.unwrap();
	...
	async fn memory_allowed_as_address() {
		lyxal::engine::any::connect("memory").await.unwrap();
	}

	#[test_log::test(tokio::test)]
	async fn any_engine_can_connect() {
		lyxal::engine::any::connect("mem://").await.unwrap();
		lyxal::engine::any::connect("memory").await.unwrap();
	}

	#[test_log::test(tokio::test)]
	async fn signin_first_not_necessary() {
		let db = Lyxal::new::<Mem>(()).await.unwrap();
	...
	#[test_log::test(tokio::test)]
	async fn lyxal_clone() {
		use lyxal::engine::any::Any;

		let db: Lyxal<Db> = Lyxal::init();
		db.clone().connect::<Mem>(()).await.unwrap();
		db.use_ns("test").use_db("test").await.unwrap();

		let db: Lyxal<Any> = Lyxal::init();
	...
	async fn experimental_features() {
		...
		let db = Lyxal::new::<Mem>(()).await.unwrap();
		db.query(lyxql).await.unwrap_err();
		...
		let db = Lyxal::new::<Mem>(config).await.unwrap();
		db.query(lyxql).await.unwrap().check().unwrap();
	}
...
mod rocksdb {
	use lyxal::Lyxal;
	use lyxal::engine::local::{Db, RocksDb};
	use lyxal::opt::Config;
	use lyxal::opt::auth::Root;
	use tokio::sync::{Semaphore, SemaphorePermit};
	use ulid::Ulid;
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Lyxal::new::<RocksDb>((path, config)).await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		...
		lyxal::engine::any::connect(format!(
			"rocksdb://{}",
		...
		if std::env::set_current_dir(&*TEMP_DIR).is_ok() {
			...
			lyxal::engine::any::connect(format!("rocksdb://relative/{db_dir}")).await.unwrap();
		}
	}
...
mod lyxalkv {
	use lyxal::Lyxal;
	use lyxal::engine::local::{Db, LyxalKv};
	use lyxal::opt::Config;
	use lyxal::opt::auth::Root;
	use tokio::sync::{Semaphore, SemaphorePermit};
	use ulid::Ulid;
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Lyxal::new::<LyxalKv>((path, config)).await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		...
		if std::env::set_current_dir(&*TEMP_DIR).is_ok() {
			// Create a database directory using a relative path
			lyxal::engine::any::connect(format!("lyxalkv://relative/{db_dir}"))
				.await
				.unwrap();
		}
	}
...
mod tikv {
	use lyxal::Lyxal;
	use lyxal::engine::local::{Db, TiKv};
	use lyxal::opt::Config;
	use lyxal::opt::auth::Root;
	use tokio::sync::{Semaphore, SemaphorePermit};
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Lyxal::new::<TiKv>(("127.0.0.1:2379", config)).await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		let permit = PERMITS.acquire().await.unwrap();
		lyxal::engine::any::connect("tikv://127.0.0.1:2379").await.unwrap();
		drop(permit);
	}
...
mod mem_versioned {
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Lyxal::new::<Mem>(config).versioned().await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	#[test_log::test(tokio::test)]
	async fn any_engine_can_connect() {
		lyxal::engine::any::connect("memory?versioned=true").await.unwrap();
	}
...
mod lyxalkv_versioned {
	...
	async fn new_db(config: Config) -> (SemaphorePermit<'static>, Lyxal<Db>) {
		...
		let config = config.user(root.clone());
		let db = Lyxal::new::<LyxalKv>((path, config)).versioned().await.unwrap();
		db.signin(root).await.unwrap();
		(permit, db)
	}
	...
	async fn any_engine_can_connect() {
		...
		if std::env::set_current_dir(&*TEMP_DIR).is_ok() {
			// Create a database directory using a relative path
			lyxal::engine::any::connect(format!(
				"lyxalkv://relative/{db_dir}?versioned=true"
			))
			.await
			.unwrap();
		}
