...
msg_dispatcher: StoredValue<UnboundedSender<Msg>, leptos::reactive::owner::LocalStorage>,
msgs: SmallVec<[Msg; 4]>,
...
if let Some(msg_dispatcher) = self.msg_dispatcher.try_with_value(|v: &UnboundedSender<Msg>| v.clone()) {
for cmd in std::mem::take(&mut self.cmds) {
let mut msg_dispatcher = msg_dispatcher.clone();

owner.with(|| spawn_local(async move {
...
pub struct MsgDispatcher<Msg: 'static>(StoredValue<UnboundedSender<Msg>, leptos::reactive::owner::LocalStorage>);
...
#[doc(hidden)]
pub fn new(msg_dispatcher: StoredValue<UnboundedSender<Msg>, leptos::reactive::owner::LocalStorage>) -> Self {
Self(msg_dispatcher)
}
...
if let Some(mut msg_dispatcher) = self.0.try_with_value(|v: &UnboundedSender<Msg>| v.clone()) {
spawn_local(async move {
msg_dispatcher.send(msg).await.unwrap();
});
}
...
if let Some(msg_dispatcher) = self.0.try_with_value(|v: &UnboundedSender<Msg>| v.clone()) {
