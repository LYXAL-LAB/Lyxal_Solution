...
pub fn timezone((val, _tzid): (Value, String)) -> Result<Value> {
    // TODO: Integrate chrono-tz or similar. Currently a stub returning original.
    // We strictly need chrono-tz dependency to parse TZID string.
    let dt = match val {
        Value::Datetime(d) => d.0,
        _ => return Ok(Value::None),
    };
    Ok(Value::from(dt.to_rfc3339()))
}

/// Expand RRULE into occurrences
/// Usage: ical::occurrences("FREQ=DAILY;COUNT=5", start_date, limit_date?)
pub fn occurrences((_rrule_str, _start, _limit): (String, Value, Value)) -> Result<Value> {
    // TODO: Integrate rrule crate. Currently a stub.
    // Dependency setup required adding rrule = "0.13" and chrono-tz.
    Ok(Value::Array(Array::default()))
}
