...
2. The default interpreter model is `mistral-nemo`, if you want to proceed with this, you have to pull it from ollama server by running:
   ...
   ```
3. If you want to use a different model, make sure to pull it first from the Ollama server. Then override the default interpreter model using the `LYXAL_TOOLSHIM_OLLAMA_MODEL` environment variable. For example, to use the `llama3.2` model, run:
   ...
   ```bash
   LYXAL_TOOLSHIM_OLLAMA_MODEL=llama3.2 
   ```
...
5. Enable the tool shim by setting the `LYXAL_TOOLSHIM` environment variable:

   ```bash
   LYXAL_TOOLSHIM=1 
   ```

Start a new lyxal session with your tool shim preferences:

  ```bash
  LYXAL_TOOLSHIM=1 LYXAL_TOOLSHIM_OLLAMA_MODEL=llama3.2 cargo run --bin lyxal session
  ```
