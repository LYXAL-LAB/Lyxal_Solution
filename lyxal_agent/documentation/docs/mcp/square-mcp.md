...
description: Add the Square API as a lyxal Extension
---
...
This tutorial will get you started with the [Square MCP server](https://developer.squareup.com/docs/mcp) to enable interactive and automated work for your Square seller account. The Square MCP server is an open source project that allows you to interact with the Square API using lyxal.
...
<Tabs groupId="remote-or-local">
  ...
  <Tabs groupId="interface">
    ...
    [Launch the installer](https://mcp.squareup.com/lyxal)
    </TabItem>
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
     1. [Launch the installer](https://mcp.squareup.com/lyxal)
     2. Click `OK` to confirm the installation
     3. lyxal should open a browser tab to an OAuth permissions page. Double-check which permissions you want to allow, and click `Grant Access`.
     4. It will ask you to login or reauthenticate to Square, and may ask you to confirm the permissions you want to allow.
     5. In lyxal, navigate to the chat
    ...
    <TabItem value="cli" label="lyxal CLI">
      ...
      When you start the next session, lyxal will open a browser where you can grant permissions and sign in to your Square account.
  ...
  <Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
    [Launch the installer](lyxal://extension?cmd=npx&arg=square-mcp-server&arg=start&id=mcp_square_api&name=Square%20MCP%20Server&description=Square%20API%20MCP%20Server&env=ACCESS_TOKEN%3DYour%20Access%20Token&env=SANDBOX%3Dtrue)
