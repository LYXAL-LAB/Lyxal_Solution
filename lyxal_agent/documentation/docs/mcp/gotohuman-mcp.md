...
description: Add gotoHuman MCP Server as a lyxal Extension
---
...
This tutorial covers how to add the [gotoHuman MCP Server](https://github.com/gotohuman/gotohuman-mcp-server) as a lyxal extension to bring **human-in-the-loop approvals** into your AI workflows. With gotoHuman, lyxal can pause and request a review before continuing, perfect for blog drafts, code reviews,compliance checks, etc.  


:::tip TLDR
<Tabs groupId="interface">
  <TabItem value="ui" label="lyxal Desktop" default>
  [Launch the installer](lyxal://extension?cmd=npx&arg=-y&arg=%40gotohuman%2Fmcp-server&id=gotoHuman&name=gotoHuman&description=gotoHuman%20MCP%20server%20for%20human-in-the-loop%20approvals&env=GOTOHUMAN_API_KEY%3DgotoHuman%20API%20Key)
  </TabItem>
...
In this example, lyxal sends a LinkedIn post draft to gotoHuman for approval using the `n8n news to post` template that was created.
...
### lyxal Prompt
```
Send this blog draft about lyxal to gotoHuman for review using my `n8n news to post` form. 

Include today’s date as the timestamp, these links: 
[lyxal Docs: https://block.github.io/lyxal/, gotoHuman: https://gotohuman.com/], 
summarize it as ‘Introducing lyxal’s integration with gotoHuman for human approvals,’ 
and here’s the draft:

lyxal is an open-source AI agent that runs locally on your machine, right in your terminal or IDE. 
It connects to MCP servers like gotoHuman to extend its abilities. While lyxal can automate tasks, 
...
Once approved, lyxal can continue where it left off. 
...
Perfect! I've successfully sent your lyxal blog draft to gotoHuman for review using your "n8n news to post" form. Here's what was submitted:
...
- Summary: Introducing lyxal's integration with gotoHuman for human approvals
- Links:
    - lyxal Docs
    - gotoHuman
- Draft: Your complete blog post about lyxal's integration with gotoHuman
...
Now if you open the **Review Link** from lyxal’s output, you’ll see the request waiting in your gotoHuman dashboard:  
