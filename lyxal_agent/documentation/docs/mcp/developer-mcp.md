...
description: Use Developer MCP Server as a lyxal Extension
---
...
The Developer extension allows lyxal to automate developer-centric tasks such as file editing, shell command execution, and project setup. It also provides tools for [enhanced code editing](/docs/guides/enhanced-code-editing) and [codebase analysis](/docs/guides/codebase-analysis).

This tutorial will cover enabling and using the Developer MCP Server, which is a built-in lyxal extension. 


## Configuration

:::info
The Developer extension is already enabled by default when lyxal is installed.
:::
...
In this example, I'm going to have lyxal automate setting up my JavaScript developer environment with Express, Monlyxal, Nodemon, Dotenv and initialize Git.
...
### lyxal Prompt
```
set up a new JavaScript project with Express, Monlyxal, Nodemon, and Dotenv? Fetch the latest package versions, generate a README with those versions, and initialize Git
```

After 1 minute, lyxal was done setting up my entire dev environment! 🎉
...
2. Installed Express, Monlyxal, and dotenv dependencies
...
By default, lyxal can run system commands with your user privileges and edit any accessible file **without your approval**. This is because lyxal runs in Autonomous permission mode by default and has access to the Developer extension's shell and file editing tools. While this configuration allows lyxal to work quickly and independently, there's potential for unexpected outcomes. Understanding the available access control features can help you configure lyxal to match your comfort level and specific needs.

:::tip
See the [Quick Setup Example](#quick-setup-example) below for some ways to configure more control over lyxal's behavior.
:::
...
- **[lyxal Permission Modes](/docs/guides/lyxal-permissions)** control when lyxal asks for approval:
...
- **[.lyxalignore files](/docs/guides/using-lyxalignore)** restrict which files and directories lyxal can access (`.gitignore` files are fallback)

:::tip Changing Modes In-Session
You can change lyxal permission modes during a session without restarting:
...
You might want more control over lyxal's operations when working with sensitive systems, exploring unfamiliar codebases, using untrusted models, or simply preferring to review actions before execution.

Here's an example configuration that enables oversight:

1. **Set the [permission mode](/docs/guides/lyxal-permissions)** to Smart Approval or Manual Approval:
   ```yaml
   # ~/.config/lyxal/config.yaml
   LYXAL_MODE: smart_approve  # or approve
   ```

2. **Create a [`.lyxalignore` file](/docs/guides/using-lyxalignore)** in your project to protect sensitive files:
   ```
   .env*
   secrets.*
   *.key
...
As you become more comfortable with lyxal's behavior, you can adjust these settings to reduce friction while maintaining appropriate safeguards for your environment.

:::info
Also see the [Security Guide](/docs/guides/security/) for information about using lyxal safely.
:::