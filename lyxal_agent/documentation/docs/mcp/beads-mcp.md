...
description: Add Beads MCP Server as a lyxal Extension
---
...
This tutorial covers how to add the [Beads MCP Server](https://github.com/steveyegge/beads) as a lyxal extension. Beads is a distributed, git-backed issue tracker designed for AI agents. It enables persistent task tracking with dependency management, so multiple lyxal sessions can coordinate work on complex projects.

:::tip TLDR
<Tabs groupId="interface">
  <TabItem value="ui" label="lyxal Desktop" default>
  [Launch the installer](lyxal://extension?cmd=uvx&arg=beads-mcp&id=beads&name=Beads&description=Git-backed%20issue%20tracker%20for%20AI%20agent%20task%20management)
  </TabItem>
...
In this example, we'll use Beads to coordinate building an expense tracker web app across **multiple parallel sessions**. This demonstrates how Beads enables multiple lyxal instances to work on the same project without conflicts.
...
Create a new directory and start a lyxal session.
...
Open a **new terminal** in the same directory and start a new lyxal session.
...
Open a **third terminal** in the same directory and start another lyxal session. Run this in parallel with Step 2.
