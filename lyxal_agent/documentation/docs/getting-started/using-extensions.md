...
Extensions are add-ons that provide a way to extend the functionality of lyxal by connecting with applications and tools you already use in your workflow. These extensions can be used to add new features, access data and resources, or integrate with other systems.

Extensions are based on the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol), so you can connect
lyxal to a wide ecosystem of capabilities.

lyxal automatically checks external extensions for known malware before activation. If a malicious package is detected, the [extension will be blocked](/docs/troubleshooting/known-issues#malicious-package-detected) with a clear error message.

:::tip Tutorials
Check out the [step-by-step tutorials](/docs/category/mcp-servers) for adding and using various lyxal extensions
:::

## Built-in Extensions
lyxal includes several built-in extensions you can start using out of the box:
...
- [Memory](/docs/mcp/memory-mcp): Teaches lyxal to remember your preferences as you use it.
- [Tutorial](/docs/mcp/tutorial-mcp): Provides interactive tutorials for learning about lyxal.
- [Auto Visualiser](/docs/mcp/autovisualiser-mcp): Automatically generates graphical data visualizations in conversations.

:::warning Access Control
lyxal operates autonomously by default. Combined with the Developer extension's tools, this means lyxal can execute commands and modify files without your approval. If you want more control over this behavior, you can configure the [lyxal permission mode](/docs/guides/lyxal-permissions), [tool permissions](/docs/guides/managing-tools/tool-permissions), and [.lyxalignore files](/docs/guides/using-lyxalignore). See [Configuring Access Controls](/docs/mcp/developer-mcp#configuring-access-controls) for a brief overview.
:::
...
:::info
lyxal's built-in extensions are MCP servers in their own right. If you'd like
to use the MCP servers included with lyxal with any other agent, you are free to do so.
:::

## Discovering Extensions

lyxal provides a [central directory][extensions-directory] of extensions that you can install and use. 

You can also add any other [MCP Server](#mcp-servers) as a lyxal extension, even if it's not listed in our directory.
...
You can install any MCP server as a lyxal extension. 
...
<Tabs groupId="interface">
  ...
  4. On the `Add custom extension` modal, enter the necessary details
     ...
     - The `Timeout` field lets you set how long lyxal should wait for a tool call from this extension to complete
  5. Click `Add` button
  ...
  3. Choose the type of extension you'd like to add:
      - `Built-In Extension`: Use an extension that comes pre-installed with lyxal.
...
Extensions can be installed using lyxal's deep link protocol. The URL format varies based on the extension type:

<Tabs groupId="interface">
  <TabItem value="stdio" label="StandardIO" default>
```
lyxal://extension?cmd=<command>&arg=<argument>&id=<id>&name=<name>&description=<description>
```

Required parameters:
- `cmd`: The base command to run, one of `jbang`, `npx`, `uvx`, `lyxald`, or `docker`
...
```
lyxal://extension?cmd=npx&arg=-y&arg=%40modelcontextprotocol/server-github&timeout=<timeout>&id=<id>&name=<name>&description=<description>
```
...
```
lyxal://extension?url=<remote-streamable-http-url>&type=streamable_http&id=<id>&name=<n>&description=<description>
```
...
```
lyxal://extension?url=https%3A%2F%2Fexample.com%2Fstreamable&type=streamable_http&timeout=<timeout>&id=<id>&name=<n>&description=<description>
```
...
### Config Entry
For advanced users, you can also directly edit the config file (`~/.config/lyxal/config.yaml`) to add, remove, or update an extension:
...
<Tabs groupId="interface">
  ...
  1. Run the following command to open up lyxal's configurations:
      ```sh
      lyxal configure
      ```
...
The Smart Extension Recommendation system in lyxal automatically identifies and suggests relevant extensions based on your tasks and needs. This section explains how to use this feature effectively and understand its capabilities and limitations.

When you request a task, lyxal checks its enabled extensions and their tools to determine if it can fulfill the request. If not, it suggests or enables additional extensions as needed. You can also request specific extensions by name.
...
lyxal automatically detects when an extension is needed based on your task requirements. Here's an example of how lyxal identifies and enables a needed extension during a conversation:
...
■  lyxal would like to enable the following extension, do you approve?
// highlight-start
...
lyxal responds to explicit requests for extensions, allowing users to manually enable specific tools they need. Here's an example of how lyxal handles a direct request to enable an extension:
...
lyxal relies on extension properties to determine how to handle an extension. You can edit these properties if you want to change the extension's display settings and behavior, such as the name, timeout, or environment variables.
...
You can start a tailored lyxal session with specific extensions directly from the CLI. 
...
```bash
lyxal session --with-builtin "{extension_id}"
```

For example, to enable the Developer and Computer Controller extensions and start a session, you'd run:

```bash
lyxal session --with-builtin "developer,computercontroller"
```

Or alternatively:

```bash
lyxal session --with-builtin developer --with-builtin computercontroller
```
...
```bash
lyxal session --with-extension "{extension command}" --with-extension "{another extension command}"
```
...
```bash
lyxal session --with-extension "uvx mcp-server-fetch"
```
...
```bash
lyxal session --with-extension "VAR=value command arg1 arg2"
```

For example, to start a session with the [GitHub extension](https://github.com/github/github-mcp-server), you'd run:

```bash
lyxal session --with-extension "GITHUB_PERSONAL_ACCESS_TOKEN=<YOUR_TOKEN> npx -y @modelcontextprotocol/server-github"
```
...
```bash
lyxal session --with-streamable-http-extension "{extension URL}" --with-streamable-http-extension "{another extension URL}"
```

For example, to start a session with a Streamable HTTP extension, you'd run:

```bash
lyxal session --with-streamable-http-extension "https://example.com/streamable"
```

## Developing Extensions

lyxal extensions are implemented with MCP, a standard protocol that allows AI models and agents to securely connect with local or remote resources. Learn how to build your own [extension as an MCP server](https://modelcontextprotocol.io/quickstart/server).
