...
<Tabs>
  <TabItem value="mac" label="macOS" default>
    ...
    <Tabs groupId="interface">
      <TabItem value="ui" label="lyxal Desktop" default>
        Install lyxal Desktop directly from the browser or with [Homebrew](https://brew.sh/).
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          :::tip Updating lyxal
          It's best to periodically [update lyxal](/docs/guides/updating-lyxal).
          :::
        </div>
        <h3>Option 2: Install via Homebrew</h3>
        Homebrew downloads the [same app](https://github.com/Homebrew/homebrew-cask/blob/master/Casks/b/block-lyxal.rb) but can take care of updates too.
        ```bash
          brew install --cask block-lyxal
        ```
        ---
        <div style={{ marginTop: '1rem' }}>
          :::info Permissions
          If you're on an Apple Mac M3 and the lyxal Desktop app shows no window on launch, check and update the following:

          Ensure the `~/.config` directory has read and write access.

          lyxal needs this access to create the log directory and file. Once permissions are granted, the app should load correctly. For steps on how to do this, refer to the  [Known Issues Guide](/docs/troubleshooting/known-issues#macos-permission-issues)
          :::
        </div>
      </TabItem>
      <TabItem value="cli" label="lyxal CLI">
        Install lyxal directly from the browser or with [Homebrew](https://brew.sh/).

        <h3 style={{ marginTop: '1rem' }}>Option 1: Install via Download script</h3>
        Run the following command to install the latest version of lyxal on macOS:

        ```sh
        curl -fsSL https://github.com/block/lyxal/releases/download/stable/download_cli.sh | bash
        ```
        This script will fetch the latest version of lyxal and set it up on your system.

        If you'd like to install without interactive configuration, disable `CONFIGURE`:

        ```sh
        curl -fsSL https://github.com/block/lyxal/releases/download/stable/download_cli.sh | CONFIGURE=false bash
        ```

        :::tip Updating lyxal
        It's best to keep lyxal updated. To update lyxal, run:
        ```sh
        lyxal update
        ```
        :::
        ...
        <h3>Option 2: Install via Homebrew</h3>
        Homebrew downloads the [a precompiled CLI tool](https://github.com/Homebrew/homebrew-core/blob/master/Formula/b/block-lyxal-cli.rb) and can take care of updates.
        ```bash
        brew install block-lyxal-cli
        ```
      </TabItem>
    </Tabs>
  </TabItem>
  ...
  <TabItem value="linux" label="Linux">
    ...
    <Tabs groupId="interface">
      <TabItem value="ui" label="lyxal Desktop" default>
        Install lyxal Desktop directly from the browser.
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="lyxal CLI">
        To install lyxal natively on Windows, you need one of the following environments:
        ...
        **PowerShell Installation:**
        ...
        ```powershell
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/block/lyxal/main/download_cli.ps1" -OutFile "download_cli.ps1";
        ```
        Then run the script to install lyxal:
        ```powershell
        .\download_cli.ps1
        ```

        :::info Windows PATH Setup
        If you see a warning that lyxal is not in your PATH, you need to add lyxal to your PATH:
        ...
        After updating your PATH, you can run `lyxal` commands from any directory.
        :::

        <details>
        <summary>Install via Windows Subsystem for Linux (WSL)</summary>

          We recommend running the lyxal CLI natively on Windows, but you can use WSL if you prefer a Linux-like environment.
          ...
          3. Run the lyxal installation script:
          ```bash
          curl -fsSL https://github.com/block/lyxal/releases/download/stable/download_cli.sh | bash
          ```
          :::tip
          ...
          If needed, add lyxal to your path:
...
## Set LLM Provider
lyxal works with [supported LLM providers][providers] that give lyxal the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="lyxal CLI">
    ...
    [lyxal opens the browser and prints details]
  ...
  Then run `lyxal configure` again. lyxal will detect the environment variable and display:
...
<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
        Starting a session in the lyxal Desktop is straightforward. After choosing your provider, you'll see the session interface ready for use.

        Type your questions, tasks, or instructions directly into the input field, and lyxal will get to work immediately.
    </TabItem>
    <TabItem value="cli" label="lyxal CLI">
...
You can also configure Extensions to extend lyxal's functionality, including adding new ones or toggling them on and off. For detailed instructions, visit the [Using Extensions Guide][using-extensions].
