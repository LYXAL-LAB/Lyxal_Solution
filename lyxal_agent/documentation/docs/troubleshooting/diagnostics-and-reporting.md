...
description: Use built-in diagnostics, report bugs, and request new features with lyxal's integrated support tools.
---
...
lyxal provides several built-in features to help you get support, report issues, and request new functionality. This guide covers the diagnostics system, bug reporting, and feature request tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="lyxal CLI">
    Use the session diagnostics command to generate a troubleshooting bundle. For complete details and all available options, see the [CLI Commands guide](/docs/guides/lyxal-cli-commands#session-diagnostics-options).

    ```sh
    # Generate diagnostics for a specific session
    lyxal session diagnostics --session-id <session_id>

    # Interactive selection (prompts you to choose a session)
    lyxal session diagnostics

    # Save to a custom location
    lyxal session diagnostics --session-id <session_id> --output /path/to/diagnostics.zip
    ```
...
├── logs/
│   ├── lyxal-2024-01-15.jsonl
│   ├── lyxal-2024-01-14.jsonl
│   └── ...
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="lyxal CLI">
    ...
    ```
    https://github.com/block/lyxal/issues/new?template=bug_report.md
    ```
  </TabItem>
</Tabs>
...
The feature request system helps you suggest improvements and new functionality for lyxal.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="lyxal CLI">
    ...
    ```
    https://github.com/block/lyxal/issues/new?template=feature_request.md
    ```
  </TabItem>
</Tabs>

## Error Recovery with "Ask lyxal"

When certain types of error occur in lyxal Desktop (such as failures to activate extensions), you'll see an `Ask lyxal` button in the error notification. This feature lets you quickly troubleshoot the issue with lyxal's help:

1. When the error occurs, an `Ask lyxal` button appears in the error notification
2. Click the button to send the error details to lyxal in a chat prompt
3. lyxal provides diagnostic suggestions and potential solutions
