---
title: Ralph Loop
description: Run lyxal in a loop with fresh context per iteration and cross-model review
---

Ralph Loop, based on [Geoffrey Huntley's Ralph Wiggum technique](https://ghuntley.com/ralph/), is an iterative development pattern that keeps lyxal working on a task until it's genuinely complete.
...
- [Install the lyxal CLI](/docs/getting-started/installation) because the Ralph Loop runs via the terminal.
...
```bash
mkdir -p ~/.config/lyxal/recipes

curl -sL https://raw.githubusercontent.com/block/lyxal/main/documentation/src/pages/recipes/data/recipes/ralph-loop.sh -o ~/.config/lyxal/recipes/ralph-loop.sh
curl -sL https://raw.githubusercontent.com/block/lyxal/main/documentation/src/pages/recipes/data/recipes/ralph-work.yaml -o ~/.config/lyxal/recipes/ralph-work.yaml
curl -sL https://raw.githubusercontent.com/block/lyxal/main/documentation/src/pages/recipes/data/recipes/ralph-review.yaml -o ~/.config/lyxal/recipes/ralph-review.yaml

chmod +x ~/.config/lyxal/recipes/ralph-loop.sh
```
...
```bash
~/.config/lyxal/recipes/ralph-loop.sh "Create a simple browser using Electron and React"
```
...
```bash
~/.config/lyxal/recipes/ralph-loop.sh ./prd.md
```
:::
...
|--------|-------------|
| Worker model | The model that does the actual coding work. Defaults to `LYXAL_MODEL` if set. |
| Worker provider | The provider for the worker model (e.g., `openai`, `anthropic`). Defaults to `LYXAL_PROVIDER` if set. |
...
The terminal will show lyxal moving through the worker and reviewer phases. Each iteration starts with a fresh session to keep the context clean. Here's what a successful run looks like:
...
▶ WORK PHASE
... (lyxal creates initial implementation) ...

▶ REVIEW PHASE
... (lyxal reviews the work) ...
...
Ralph Loop uses files in `.lyxal/ralph/` to persist state between iterations. This is how the worker knows what to do and the reviewer knows what was done, even though each iteration starts with fresh context.
...
#   RALPH_RECIPE_DIR      - Recipe directory (default: ~/.config/lyxal/recipes)
#

set -e

INPUT="$1"
RECIPE_DIR="${RALPH_RECIPE_DIR:-$HOME/.config/lyxal/recipes}"
...
prompt_for_settings() {
    local default_model="${LYXAL_MODEL:-}"
    local default_provider="${LYXAL_PROVIDER:-}"
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    LYXAL_PROVIDER="$WORKER_PROVIDER" LYXAL_MODEL="$WORKER_MODEL" lyxal run --recipe "$RECIPE_DIR/ralph-work.yaml" || {
    ...
    LYXAL_PROVIDER="$REVIEWER_PROVIDER" LYXAL_MODEL="$REVIEWER_MODEL" lyxal run --recipe "$RECIPE_DIR/ralph-review.yaml" || {
...
instructions: |
  ...
  1. cat .lyxal/ralph/task.md (YOUR TASK)
  2. cat .lyxal/ralph/iteration.txt 2>/dev/null || echo "1"
  3. cat .lyxal/ralph/review-feedback.txt 2>/dev/null
  ...
  - If task is complete: echo "done" > .lyxal/ralph/work-complete.txt
  - Always write a summary: echo "what I did" > .lyxal/ralph/work-summary.txt

prompt: |
  ## Ralph Work Phase
  
  Read your task from: .lyxal/ralph/task.md
  
  1. Read the task: `cat .lyxal/ralph/task.md`
  2. Check iteration: `cat .lyxal/ralph/iteration.txt 2>/dev/null || echo "1"`
  3. Check for review feedback: `cat .lyxal/ralph/review-feedback.txt 2>/dev/null`
  4. List existing files: `ls -la`
  5. Do the work (address feedback if any, otherwise make progress)
  6. Write summary: `echo "summary" > .lyxal/ralph/work-summary.txt`
  7. If complete: `echo "done" > .lyxal/ralph/work-complete.txt`
...
instructions: |
  ...
  OUTPUT:
  If approved: echo "SHIP" > .lyxal/ralph/review-result.txt
  If needs work: 
    echo "REVISE" > .lyxal/ralph/review-result.txt
    echo "specific feedback" > .lyxal/ralph/review-feedback.txt

prompt: |
  ## Ralph Review Phase
  
  1. Read the task: `cat .lyxal/ralph/task.md`
  2. Read work summary: `cat .lyxal/ralph/work-summary.txt`
  3. Check if complete: `cat .lyxal/ralph/work-complete.txt 2>/dev/null`
  4. Examine the actual files created/modified
  5. Run verification (tests, build, etc.)
  6. Decide: SHIP or REVISE
  
  If SHIP: `echo "SHIP" > .lyxal/ralph/review-result.txt`
  If REVISE: 
    `echo "REVISE" > .lyxal/ralph/review-result.txt`
    `echo "specific feedback" > .lyxal/ralph/review-feedback.txt`
