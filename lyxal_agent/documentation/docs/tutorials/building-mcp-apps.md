...
description: Create interactive UI applications that render inside lyxal Desktop
---

import { PanelLeft } from 'lucide-react';

# Building MCP Apps for lyxal

MCP Apps let MCP servers return interactive UIs that render directly inside the lyxal chat interface, rather than responding with text alone. This allows users to express intent through interaction, which is useful for workflows that require input, iteration, or visual feedback.

:::warning Experimental
MCP Apps support in lyxal is experimental and based on a draft specification. The implementation is minimal and may change, and does not yet support advanced capabilities or persistent app windows.
:::
...
- lyxal Desktop 1.19.1+ installed
:::
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "show_demo_app") {
    return {
      ...
      // This metadata tells lyxal to render the MCP App
      _meta: {
        ui: {
...
<body class="light">
  <div class="container">
    <h1>🎮 MCP App Demo</h1>
    <p class="subtitle">An interactive UI running inside lyxal</p>
    ...
    <div class="info-section">
      ...
      It communicates with lyxal via JSON-RPC messages through the sandbox bridge.
      <br><br>
      • Counter uses local state<br>
      • "Send" calls <code>ui/message</code> to append text to chat<br>
      • Theme syncs with lyxal's theme setting
    </div>
  </div>
...
1. Restart lyxal to load the new extension
2. Prompt lyxal: "Show me the demo app"
3. lyxal will call the `show_demo_app` tool
4. Your interactive app will render in the chat!

Try:
- Clicking the counter buttons
- Typing a message and clicking "Send"
- Switching lyxal between light/dark mode
...
└──────────────────┬───────────────────┘
                   │ postMessage
┌──────────────────▼───────────────────┐
│          lyxal Desktop               │  Renders UI, routes messages
└──────────────────┬───────────────────┘
                   │ MCP Protocol
┌──────────────────▼───────────────────┐
...
Your server returns a `ui://` resource URI, lyxal fetches the HTML and renders it in an iframe. The app communicates back via `postMessage`—requesting theme info, sending messages to chat, or resizing itself.
