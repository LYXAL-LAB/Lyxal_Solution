---
title: Benchmarking with lyxal
sidebar_label: Benchmark with lyxal
---

The lyxal benchmarking system allows you to evaluate lyxal performance on complex tasks with one or more system
configurations.<br></br>
This guide covers how to use the `lyxal bench` command to run benchmarks and analyze results.
...
```bash
lyxal bench selectors
```

2. Create a basic configuration file:

```bash
lyxal bench init-config -n bench-config.json
...
```bash
lyxal bench run -c bench-config.json
```
...
```bash
RUST_LOG=debug lyxal bench bench-config.json
```
...
Tool shimming allows you to use a non-tool-capable models with lyxal, provided Ollama is installed on the
system.
