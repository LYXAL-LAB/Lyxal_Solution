---
title: Using lyxal in Headless Mode for Automation
description: lyxal Headless Mode
---

# Automate Development Tasks with lyxal Headless Mode

*Run AI-powered engineering workflows in CI/CD pipelines, servers, and batch processing scenarios*

The ability to automate complex engineering tasks without human intervention has been huge, but let's take it to the next level with AI. lyxal's "headless" mode enables developers to harness the full power of AI automation in server environments, CI/CD pipelines, and batch processing scenarios where interactive sessions simply aren't feasible.

## What is Headless Mode?

Headless mode is lyxal's non-interactive execution environment, designed for automated scenarios where human intervention isn't available (or wanted).
...
lyxal run --with-builtin developer -t "Check system logs for errors in the last 24 hours, identify performance bottlenecks, and generate a maintenance report"

# Container optimization
lyxal run --no-session -t "Analyze the Dockerfile, optimize for smaller image size, and update the build process documentation"

# Cloud resource audit
lyxal run -t "Review our AWS infrastructure configuration, identify cost optimization opportunities, and create a migration plan for underutilized resources"
```
...
- name: AI-Powered Code Review
  run: |
    lyxal run --with-builtin developer \
...
lyxal run --debug -t "Examine the failing test suite, identify the root cause of failures, implement fixes, and ensure all tests pass"

# Automated documentation updates
lyxal run -t "Review code changes and update the README.md and API documentation to reflect new features and modifications"
```
...
for repo in repo1 repo2 repo3; do
  cd $repo
  lyxal run --no-session -t "Update all dependencies to latest stable versions, run tests, and create a PR if changes are needed"
  cd ..
done

# Database migration automation
lyxal run -t "Analyze the current database schema, generate migration scripts for the new requirements, and create rollback procedures"
```
...
0 2 * * * /usr/local/bin/lyxal run --no-session -t "Run comprehensive security audit, check for vulnerabilities, and email report to security team"

# Weekly dependency updates
0 9 * * 1 /usr/local/bin/lyxal run -t "Check for outdated dependencies, create update PRs for non-breaking changes, and schedule review for major updates"
```
...
lyxal run -t "Analyze the test failures in the latest CI run, identify the root cause, and create a fix with appropriate unit tests"

# Better: Even more detailed
lyxal run -t "Review the failed tests in tests/integration/, identify why the authentication middleware is failing, implement a fix that maintains backward compatibility, and add regression tests"
```
...
export LYXAL_MAX_TURNS=50
export LYXAL_MODE=auto
```

The `CONTEXT_STRATEGY` and `MAX_TURNS` settings help manage conversation limits, while `LYXAL_MODE` set to `auto` allows for non-interactive execution.
...
```bash
#!/bin/bash
if ! lyxal run --no-session -t "Run security audit and fix critical issues"; then
    echo "lyxal automation failed - manual intervention required"
    exit 1
fi
```
...
[Recipes](/docs/guides/recipes/) are lyxal's powerful way to define reusable, parameterized workflows. In headless mode, recipes become even more valuable as they enable sophisticated automation scenarios.
...
lyxal run --recipe automation-recipe.yaml
...
lyxal run --recipe main-workflow.yaml \
...
**What this means**: lyxal cannot ask for clarification, approval, or additional input during execution. If it's unsure of what to do, the prompt result will usually show you a question like "How should I proceed?".

**Impact**: If instructions are ambiguous or if unexpected situations arise, lyxal will make its best judgment based on available context, which might not always align with your intentions.
...
lyxal run -t "Fix the issues"

# Better: Specific and actionable
lyxal run -t "Fix the TypeScript compilation errors in src/components/, ensure all imports are correct, and update any deprecated API calls to use the latest syntax"
```
...
**What this means**: lyxal cannot prompt for permission to use potentially risky tools or operations.
...
```bash
export LYXAL_MODE=auto  # Automatically approve safe operations
# or configure specific tool permissions in your config
```

### 4. Context Decision Automation

**What this means**: When conversation context limits are reached, lyxal automatically applies the configured strategy without user input.
...
```bash
export LYXAL_CONTEXT_STRATEGY=summarize  # Usually the best choice for automation
export LYXAL_MAX_TURNS=100  # Prevent runaway execution
```
...
export LYXAL_MODEL=gpt-4o

# Output control
export LYXAL_CLI_MIN_PRIORITY=0.2  # Reduce verbose output
```
...
export LYXAL_LEAD_MODEL=gpt-4o  # For planning
export LYXAL_WORKER_MODEL=gpt-4o-mini  # For execution

# Security and permissions
export LYXAL_ALLOWLIST=https://company.com/allowed-extensions.json
```

## The Future of Automated Development

lyxal's headless mode represents more than just a feature -- it's a shift toward truly automated development workflows powered by AI. We can remove human intervention in routine tasks so teams can focus on high-value work while AI handles the repetitive, time-consuming operations that slow us down.

Whether you're looking to streamline your CI/CD pipelines, automate server maintenance, or handle bulk operations across multiple repositories, lyxal's headless mode provides the foundation for building sophisticated, reliable automation workflows.

**Start your automation journey today:**

1. **Install lyxal** and configure your environment variables
...
Connect with us on our [Discord community](https://discord.gg/lyxal-oss) to share your headless mode success stories, ask questions, and collaborate with other developers who want to push the boundaries of AI automation.
