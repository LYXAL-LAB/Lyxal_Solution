...
lyxal provides a command-line interface (CLI) with several commands for managing sessions, configurations and extensions. This guide covers all available CLI commands and interactive session features.

## Flag Naming Conventions

lyxal CLI follows consistent patterns for flag naming to make commands intuitive and predictable:
...
#### configure
Configure lyxal settings - providers, extensions, etc.
...
#### info [options]
Shows lyxal information, including the version, configuration file location, session storage, and logs.
...
#### version
Check the current lyxal version you have installed.
...
#### update [options]
Update the lyxal CLI to a newer version.

**Options:**
- **`--canary, -c`**: Update to the canary (development) version instead of the stable version
- **`--reconfigure, -r`**: Forces lyxal to reset configuration settings during the update process
...
lyxal update --reconfigure
```

---

#### completion
Generate shell-specific scripts to enable tab completion of lyxal commands, subcommands, and options. The script is printed to stdout, so you need to redirect it to the appropriate location for your shell and then reload or source your shell configuration.
...
lyxal completion bash
lyxal completion zsh
lyxal completion fish
```
...
```bash
lyxal completion fish > ~/.config/fish/completions/lyxal.fish
```
...
```powershell
lyxal completion powershell | Out-String | Invoke-Expression
```
...
:::tip Testing
After installing and reloading your shell, test completion by typing `lyxal ` and pressing Tab to see available commands, or `lyxal session --` and Tab to see available options.
:::
...
Starting with version 1.10.0, lyxal uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files.
Your existing sessions are automatically imported to the database. Legacy `.jsonl` files remain on disk but are no longer managed by lyxal.
:::
...
lyxal session -n my-project

# Resume a previous session
lyxal session --resume -n my-project
lyxal session --resume --session-id 20251108_2
lyxal session --resume --path ./session.json    # exported session
lyxal session --resume --path ./session.jsonl   # legacy session storage

# Fork a specific session by name
lyxal session --resume --fork --name my-project

# Fork the most recent session and show message history
lyxal session --resume --fork --history

# Start with extensions
lyxal session --with-extension "npx -y @modelcontextprotocol/server-memory"
lyxal session --with-builtin developer
lyxal session --with-streamable-http-extension "http://localhost:8080/mcp"
...
lyxal session -n my-session --debug --max-turns 25
```
...
lyxal session list --format json

# Sort sessions by date in ascending order
lyxal session list --ascending

# Filter sessions by working directory
lyxal session list -w ~/projects/myapp

# List only the 10 most recent sessions
lyxal session list --limit 10
```
...
lyxal session remove

# Remove a specific session by ID
lyxal session remove --session-id 20251108_3

# Remove a specific session by name
lyxal session remove -n my-project

# Remove all sessions starting with "project-"
lyxal session remove -r "project-.*"

# Remove all sessions containing "migration"
lyxal session remove -r ".*migration.*"
```

:::caution
Session removal is permanent and cannot be undone. lyxal will show which sessions will be removed and ask for confirmation before deleting.
:::
...
```bash
# Interactive export
lyxal session export

# Export specific session as JSON for backup
lyxal session export -n my-session --format json -o session-backup.json

# Export specific session as readable markdown
lyxal session export -n my-session -o session.md

# Export to stdout in different formats
lyxal session export --session-id 20251108_4 --format json
lyxal session export -n my-session --format yaml

# Export session by path (legacy)
lyxal session export --path ./my-session.jsonl -o exported.md
```
...
lyxal session diagnostics --session-id 20251108_5

# Generate diagnostics for a session by name
lyxal session diagnostics -n my-project-session

# Save diagnostics to a custom location
lyxal session diagnostics --session-id 20251108_5 -o /path/to/my-diagnostics.zip

# Interactive selection (prompts you to choose a session)
lyxal session diagnostics
```
...
- **`-t, --text <TEXT>`**: Input text to provide to lyxal directly
...
- **`--no-session`**: Run lyxal commands without creating or storing a session file
...
lyxal run --instructions plan.md

# Load a recipe with a prompt that lyxal executes and then exits  
lyxal run --recipe recipe.yaml

# Load a recipe and stay in an interactive session
lyxal run --recipe recipe.yaml --interactive

# Load a recipe in debug mode
lyxal run --recipe recipe.yaml --debug

# Show recipe details
lyxal run --recipe recipe.yaml --explain

# Run a recipe with parameters
lyxal run --recipe recipe.yaml --params environment=production --params region=us-west-2

# Run instructions from a file without session storage
lyxal run --no-session -i instructions.txt

# Run with a specified provider and model
lyxal run --provider anthropic --model claude-4-sonnet -t "initial prompt"

# Run with limited turns before prompting user
lyxal run --recipe recipe.yaml --max-turns 10
```
...
#### recipe
Used to validate recipe files, manage recipe sharing, list available recipes, and open recipes in lyxal desktop.
...
- **`open <RECIPE_NAME>`**: Open a recipe file directly in lyxal desktop
  - **`-p, --param <KEY=VALUE>`**: Pre-fill recipe parameter (can be specified multiple times)
...
lyxal recipe deeplink my-recipe.yaml

# Generate a deeplink and provide parameter values
lyxal recipe deeplink my-recipe.yaml -p environment=production -p region=us-west-2

# List all available recipes
lyxal recipe list

# List recipes with detailed information
lyxal recipe list --verbose

# List recipes in JSON format for automation
lyxal recipe list --format json

# Open a recipe in lyxal desktop
lyxal recipe open my-recipe.yaml

# Open a recipe by name
lyxal recipe open my-recipe

# Open a recipe and provide parameter value
lyxal recipe open my-recipe --param name=myproject

# Validate a recipe file
lyxal recipe validate my-recipe.yaml
...
**Commands:**
- `add <OPTIONS>`: Create a new scheduled job. Copies the current version of the recipe to `~/.local/share/lyxal/scheduled_recipes`
...
**Usage:**
```bash
lyxal schedule <COMMAND>

# Add a new scheduled recipe which runs every day at 9 AM
lyxal schedule add --schedule-id daily-report --cron "0 0 9 * * *" --recipe-source ./recipes/daily-report.yaml

# List all scheduled jobs
lyxal schedule list

# List the 10 most recent lyxal sessions created by a scheduled job
lyxal schedule sessions --schedule-id daily-report -l 10

# Run a recipe immediately
lyxal schedule run-now --schedule-id daily-report

# Remove a scheduled job
lyxal schedule remove --schedule-id daily-report
```
...
#### acp
Run lyxal as an Agent Client Protocol (ACP) agent server over stdio. This enables lyxal to work with ACP-compatible clients like Zed.
...
:::info
This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `lyxal acp` process. See [Using lyxal in ACP Clients](/docs/guides/acp-clients) for details.
:::
...
#### web
Start a new session in lyxal Web, a lightweight web-based interface launched via the CLI that mirrors the desktop app's chat experience.

lyxal Web is particularly useful when:
- You want to access lyxal with a graphical interface without installing the desktop app
- You need to use lyxal from different devices, including mobile
...
lyxal web --host 192.168.1.7 --port 8080

# Start web interface with authentication required
lyxal web --auth-token <TOKEN>
```
...
#### @lyxal / @g
Ask lyxal questions directly from your shell prompt, with command history included in the context. These aliases are created when you set up [terminal integration](/docs/guides/terminal-integration.md).

**Examples:**
```bash
# Ask questions with command history context
@lyxal create a python script to process these files
@lyxal create a PR description summarizing these changes
...
Once you're in an interactive session (via `lyxal session` or `lyxal run --interactive`), you can use these slash commands. All commands support tab completion. Press `/ + <Tab>` to cycle through available commands.

**Available Commands:**
- **`/?` or `/help`** - Display the help menu
- **`/builtin <names>`** - Add builtin extensions by name (comma-separated)
- **`/clear`** - Clear the current chat history
- **`/endplan`** - Exit plan mode and return to 'normal' lyxal mode
- **`/exit` or `/quit`** - Exit the session
- **`/extension <command>`** - Add a stdio extension (format: ENV1=val1 command args...)
- **`/mode <name>`** - Set the lyxal mode to use ('auto', 'approve', 'chat', 'smart_approve')
...
/clear
```
You can also create [custom slash commands for running recipes](/docs/guides/context-engineering/slash-commands) in lyxal Desktop or the CLI. 

---

### Themes

The `/t` command controls the syntax highlighting theme for markdown content in lyxal CLI responses. This affects the styles used for headers, code blocks, bold/italic text, and other markdown elements in the response output.
...
- The theme setting is saved to the [configuration file](/docs/guides/config-files) as `LYXAL_CLI_THEME` and persists between sessions
- The saved configuration can be overridden for the session using the `LYXAL_CLI_THEME` [environment variable](/docs/guides/environment-variables#session-management)
...
The lyxal CLI theme is independent from the lyxal Desktop theme.
:::
...
lyxal session --name use-custom-theme
...
- **`Ctrl+J`** - Add a newline. Can customize the character via `LYXAL_CLI_NEWLINE_KEY` in the [config file](/docs/guides/config-files) (e.g. `LYXAL_CLI_NEWLINE_KEY: n`) or as an [environment variable](/docs/guides/environment-variables#session-management). Avoid "c" and common terminal shortcuts like "r", "w", "z".
...
For composing longer prompts or working with complex code snippets, you can configure lyxal to use your preferred text editor instead of CLI input. This replaces the standard CLI input and keyboard shortcuts for the entire session.

**How it works:**
1. lyxal opens your configured editor with a template file
2. Type your prompt after the `# Your prompt:` heading (conversation history is shown below for context)
3. Save the file and close/exit the editor to send your prompt to lyxal
4. lyxal processes your prompt and reopens the editor with the response added to the conversation history
...
<Tabs>
  ...
  export LYXAL_PROMPT_EDITOR="code --wait"
  ```
  ...
  1. Navigate to the lyxal [configuration file](/docs/guides/config-files). For example, navigate to `~/.config/lyxal/config.yaml` on macOS.
  2. Add `LYXAL_PROMPT_EDITOR` and set it to your preferred editor:
  
  ```yaml
  # For terminal editors like vim or nano
  LYXAL_PROMPT_EDITOR: vim

  # Or for GUI editors like VS Code (use --wait flag)
  LYXAL_PROMPT_EDITOR: code --wait
  ```
...
GUI editors require a `--wait` or equivalent flag to ensure lyxal waits for you to finish editing before continuing. Without this flag, the editor opens but lyxal immediately proceeds as if you're done. Terminal editors like vim and nano don't need this flag.

---

### Command History Search

The `Ctrl+R` shortcut provides interactive search through your stored CLI [command history](/docs/guides/logs#command-history). This feature makes it easy to find and reuse recent commands without retyping them. When you type a search term, lyxal searches backwards through your history for matches.

**How it works:**
1. Press `Ctrl+R` in your lyxal CLI session
