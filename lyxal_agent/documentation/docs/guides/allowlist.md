---
sidebar_position: 90
title: lyxal Extension Allowlist
sidebar_label: Extension Allowlist
---

lyxal is an extensible framework that, by default, allows you to install any MCP server. However, you may want stricter controls on which MCP servers can be installed as extensions (e.g. in a corporate setting). 

This guide explains how you can create an **allowlist** of safe extensions that work with lyxal Desktop and CLI. An allowlist lets administrators control which MCP servers can be installed as lyxal extensions. When enabled, lyxal will only install extensions that are on the list, and will block installation of any others.

## How It Works

1. The allowlist is a YAML file that contains a list of allowed extension commands.
2. lyxal fetches the allowlist from a URL specified by the `LYXAL_ALLOWLIST` environment variable.
3. The allowlist is fetched when first needed and is cached. It is refetched on every restart of lyxal.
4. When a user attempts to install an extension, lyxal checks the MCP server's installation command against the allowlist.
...
Create an environment variable called `LYXAL_ALLOWLIST` and set the value to the URL of your YAML file:
...
1. Check if the `LYXAL_ALLOWLIST` environment variable is set correctly.
