...
The [Code Execution extension](/docs/mcp/code-execution-mcp) is an MCP server that uses the MCP protocol to expose three foundational meta-tools. When Code Execution is enabled, lyxal switches to Code Mode. For every request, the LLM writes JavaScript code that lyxal runs in a sandbox environment to:
...
Traditional MCP tool calling and Code Mode are two different approaches to the same goal: giving lyxal access to tools.
...
import lyxalCodeMode from '@site/blog/2025-12-15-code-mode-mcp/header-image.jpg';
  ...
  items={[
    {
      ...
      thumbnailUrl: lyxalCodeMode,
      linkUrl: '/lyxal/blog/2025/12/15/code-mode-mcp',
      date: '2025-12-15',
      duration: '5 min read'
    },
    {
      ...
      linkUrl: '/lyxal/blog/2025/12/21/code-mode-doesnt-replace-mcp',
      date: '2025-12-21',
      duration: '8 min read'
    }
  ]}
