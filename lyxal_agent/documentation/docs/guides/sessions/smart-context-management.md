...
When working with [Large Language Models (LLMs)](/docs/getting-started/providers), there are limits to how much conversation history they can process at once. lyxal provides smart context management features to help handle context and conversation limits so you can maintain productive sessions. Here are some key concepts:
...
- **Context Management**: How lyxal handles conversations approaching these limits
- **Turn**: One complete prompt-response interaction between lyxal and the LLM

## How lyxal Manages Context
lyxal uses a two-tiered approach to context management:
...
This layered approach lets lyxal handle token and context limits gracefully.

## Automatic Compaction
lyxal automatically compacts (summarizes) older parts of your conversation when approaching token limits, allowing you to maintain long-running sessions without manual intervention. 
Auto-compaction is triggered by default when you reach 80% of the token limit in lyxal Desktop and the lyxal CLI.

Control the auto-compaction behavior with the `LYXAL_AUTO_COMPACT_THRESHOLD` [environment variable](/docs/guides/environment-variables.md#session-management). 
...
When you reach the auto-compaction threshold:
  1. lyxal will automatically start compacting the conversation to make room.
  2. Once complete, you'll see a confirmation message that the conversation was compacted and summarized.
  3. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for lyxal.

:::tip Customize Compaction
You can customize how lyxal summarizes conversations during compaction by editing the `compaction.md` [prompt template](/docs/guides/prompt-templates).
:::
...
<Tabs groupId="interface">
  ...
  4. Continue the session. Your previous conversation remains visible, but only the compacted conversion is included in the active context for lyxal.
...
<TabItem value="cli" label="lyxal CLI" default>
...
When auto-compaction is disabled, or if a conversation still exceeds the context limit, lyxal offers different ways to handle it:
...
lyxal Desktop exclusively uses summarization by compacting the conversation to manage context, preserving key information while reducing size.
...
- **Headless mode** (`lyxal run`): Automatically summarizes (equivalent to `summarize`)

You can configure how lyxal handles context limits by setting the `LYXAL_CONTEXT_STRATEGY` environment variable:

```bash
# Set automatic strategy (choose one)
export LYXAL_CONTEXT_STRATEGY=summarize  # Automatically summarize (recommended)
export LYXAL_CONTEXT_STRATEGY=truncate   # Automatically remove oldest messages
export LYXAL_CONTEXT_STRATEGY=clear      # Automatically clear session
...
**With default settings (no `LYXAL_CONTEXT_STRATEGY` set)**, you'll see this prompt to choose a management option:
...
Context maxed out
--------------------------------------------------
lyxal summarized messages for you.
```

**With `LYXAL_CONTEXT_STRATEGY` configured**, lyxal will automatically apply your chosen strategy:

```sh
# Example with LYXAL_CONTEXT_STRATEGY=summarize
Context maxed out - automatically summarized messages.
--------------------------------------------------
lyxal automatically summarized messages for you.

# Example with LYXAL_CONTEXT_STRATEGY=truncate
Context maxed out - automatically truncated messages.
--------------------------------------------------
lyxal tried its best to truncate messages for you.

# Example with LYXAL_CONTEXT_STRATEGY=clear
...
## Maximum Turns
The `Max Turns` limit is the maximum number of consecutive turns that lyxal can take without user input (default: 1000). When the limit is reached, lyxal stops and prompts: "I've reached the maximum number of actions I can do without user input. Would you like me to continue?" If the user answers in the affirmative, lyxal continues until the limit is reached and then prompts again.
...
This setting is stored as the `LYXAL_MAX_TURNS` environment variable in your [config.yaml file](/docs/guides/config-files). You can configure it using the Desktop app or CLI.
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="lyxal CLI">
      ...
      │  ○ Remove Extension
      // highlight-start
      │  ● lyxal settings (Set the lyxal mode, Tool Output, Tool Permissions, Experiment, lyxal recipe github repo and more)
      // highlight-end
      └ 
      ```
      ...
      │  10
        // highlight-end
      │
      └  Set maximum turns to 10 - lyxal will ask for input after 10 consecutive actions
      ```

      :::tip
      In addition to the persistent `Max Turns` setting, you can provide a runtime override for a specific session or task via the `lyxal session --max-turns` and `lyxal run --max-turns` [CLI commands](/docs/guides/lyxal-cli-commands).
      :::
...
- **100+ turns**: More suitable for complex, multi-step automation where you trust lyxal to work independently, like "migrate this entire project from React 16 to React 18" or "implement comprehensive test coverage for this service"

Remember that even simple-seeming tasks often require multiple turns. For example, asking lyxal to "fix the failing tests" might involve analyzing test output (1 turn), identifying the root cause (1 turn), making code changes (1 turn), and verifying the fix (1 turn).

## Token Usage
After sending your first message, lyxal Desktop and lyxal CLI display token usage.
...
Context limits are automatically detected based on your model name, but lyxal provides settings to override the default limits:

| Model | Description | Best For | Setting |
|-------|-------------|----------|---------|
| **Main** | Set context limit for the main model (also serves as fallback for other models) | LiteLLM proxies, custom models with non-standard names | `LYXAL_CONTEXT_LIMIT` |
| **Lead** | Set larger context for planning in [lead/worker mode](/docs/tutorials/lead-worker) | Complex planning tasks requiring more context | `LYXAL_LEAD_CONTEXT_LIMIT` |
| **Worker** | Set smaller context for execution in lead/worker mode | Cost optimization during execution phase | `LYXAL_WORKER_CONTEXT_LIMIT` |
| **Planner** | Set context for [planner models](/docs/guides/creating-plans) | Large planning tasks requiring extensive context | `LYXAL_PLANNER_CONTEXT_LIMIT` |
...
- **LiteLLM Proxy Models**: When using LiteLLM with custom model names that don't match lyxal's patterns
...
lyxal resolves context limits with the following precedence (highest to lowest):

1. Explicit context_limit in model configuration (if set programmatically)
2. Specific environment variable (e.g., `LYXAL_LEAD_CONTEXT_LIMIT`)
3. Global environment variable (`LYXAL_CONTEXT_LIMIT`)
...
<Tabs groupId="interface">
  <TabItem value="ui" label="lyxal Desktop" default>
     ...
     Model context limit overrides are not yet available in the lyxal Desktop app.
  ...
  <TabItem value="cli" label="lyxal CLI">
    ...
    ```bash
    export LYXAL_CONTEXT_LIMIT=1000
    lyxal session
    ```
...
export LYXAL_PROVIDER="openai"
export LYXAL_MODEL="my-custom-gpt4-proxy"
export LYXAL_CONTEXT_LIMIT=200000  # Override the 32k default
```
...
export LYXAL_LEAD_MODEL="claude-opus-custom"
...
export LYXAL_PLANNER_MODEL="gpt-4-custom"
...
These costs are estimates only, and not connected to your actual provider bill. The cost shown is an approximation based on token counts and public pricing data.
    ...
    <TabItem value="cli" label="lyxal CLI">
    Show estimated cost in the lyxal CLI by setting the `LYXAL_CLI_SHOW_COST` [environment variable](/docs/guides/environment-variables.md#session-management) or including it in the [configuration file](/docs/guides/config-files.md).
