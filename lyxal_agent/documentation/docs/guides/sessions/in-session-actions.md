...
lyxal provides features you can use to manage conversations and share information during sessions.
...
- lyxal misunderstood your intent and went in the wrong direction
...
<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
        ...
        4. Click `Edit in Place` to save your changes and reprompt lyxal

        lyxal removes all conversation history after the edited message and responds contextually from that point.

        :::warning Deleted Context
        With Edit in Place, subsequent conversation history is permanently deleted from the session and removed from lyxal's context. Use this option only if you don't need lyxal to remember the context that follows the edited message.
        :::

    </TabItem>
    <TabItem value="cli" label="lyxal CLI">
        Message editing options are not available in the lyxal CLI.
    </TabItem>
</Tabs>
...
- Test how different prompts affect lyxal's responses
...
<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
        ...
        lyxal creates a new session with conversation history up to and including your edited message. The new session is named "(edited)" and the original session remains unchanged.
    ...
    <TabItem value="cli" label="lyxal CLI">
    Message editing is not available in the lyxal CLI, but you can [duplicate entire sessions](/docs/guides/sessions/session-management#duplicate-sessions) using the `--fork` flag.
    </TabItem>
</Tabs>

### Editing Scenario Tips

- **Iterative Prompt Refinement**: Start with a basic prompt, then edit and refine based on lyxal's response. This often works better than trying to craft the perfect prompt from the start.
...
Queue messages while lyxal is processing a task to manage your workflow. This is useful when:

- You want to prepare next steps while lyxal is working
...
:::tip
lyxal may perform better when complex tasks are split into subtasks, a technique called [*prompt chaining*](https://www.promptingguide.ai/techniques/prompt_chaining). This structured approach can both improve accuracy and give you more control over the process.
:::

<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
      Add a message to the queue:
      1. While lyxal is processing a response, type your next message
      2. Press `Enter` to add it to the queue (or interrupts if using [interruption keywords](#interrupt-task))
      
      Queued messages appear as numbered cards showing the queue order. The first message in the queue is automatically sent when lyxal finishes each response.
      
      :::info Related Features
      - In general, pressing `Enter` while lyxal is processing a task queues the message, but clicking `Send` sends the task immediately and [interrupts the task](#interrupt-task)
      - When you type common interrupt keywords like "stop", "wait", or "hold on" in a queued message, lyxal pauses until you enter or send the next message and then continues processing the queue
      :::

      #### Queue Management Controls
    
      Queued messages run automatically in order as lyxal finishes each task, but you can manage the queue:
      ...
      **Without queuing:**
      ...
      This approach might lead to overwhelming responses where important details get missed or tasks are handled superficially. Even sending a single prompt with clear sequential steps doesn't allow lyxal to focus on each task individually or build context progressively.

      **With queuing:**

      1. You send: "Refactor the authentication code to support OAuth 2.0"
      2. While lyxal is working, you queue the following messages:
    ...
    <TabItem value="cli" label="lyxal CLI">
        Message queuing is not available in the lyxal CLI.
    </TabItem>
</Tabs>

## Interrupt Task

Interrupt lyxal while it's processing a task to take control of the conversation. This is useful when:

- lyxal is heading in the wrong direction
...
<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
        ...
        lyxal stops processing the current task and asks for more information.
        ...
        lyxal stops processing the current task and pivots to the new request context.
        
        :::info Related features
        - Clicking `Send` while lyxal is processing a task interrupts the task but pressing `Enter` [queues the message](#queue-messages)
        - Typing a stop or pause keyword in a queued message also stops lyxal from processing the current task
    ...
    <TabItem value="cli" label="lyxal CLI">
        ...
        lyxal responds contextually to your new request.
        
    </TabItem>
</Tabs>

## Voice Dictation
Speak to lyxal directly instead of typing your prompts.
...
<Tabs groupId="interface">
    ...
    To use voice dictation:
        ...
        The first time you use voice dictation, lyxal will request access to your microphone. While recording, you'll see a live waveform of your audio in the input field, a timer, and the current size of your recording. Click the microphone button again to finish recording. 

        **If you don't see the microphone**, check the [models you have configured](/docs/getting-started/providers.md). ElevenLabs can be used as a dictation provider alongside any LLM, but OpenAI Whisper requires that you have an OpenAI model configured in lyxal, even if using another LLM provider for chat.  
    ...
    <TabItem value="cli" label="lyxal CLI">
        Voice dictation is not available in the lyxal CLI. 
    </TabItem>
</Tabs>

## Spellcheck

Spellcheck is enabled by default in the lyxal Desktop chat input field.
...
<Tabs groupId="interface">
    ...
    To disable or re-enable spellcheck:
        ...
        5. Restart lyxal for the change to take effect
        
    </TabItem>
    <TabItem value="cli" label="lyxal CLI">
        Spellcheck is not available in the lyxal CLI.
    </TabItem>
</Tabs>

## Share Files in Session

Provide lyxal with context from your codebase, documents, and other files to get more relevant and accurate assistance.

<Tabs groupId="interface">
    <TabItem value="ui" label="lyxal Desktop" default>
        Share files with lyxal in several ways:
        ...
        4. **Quick File Search**: Use the [`@` shortcut key](/docs/guides/file-management#quick-file-search-in-lyxal-desktop) to quickly find and include files
    </TabItem>
    <TabItem value="cli" label="lyxal CLI">
...
You can change some settings during a session and they will take effect immediately, rather than requiring you to start a new session. This gives you more control over context and capabilities while you're interacting with lyxal.
...
<Tabs groupId="interface">
  ...
  | [**lyxal Mode**](/docs/guides/lyxal-permissions#configuring-lyxal-mode) | <Tornado className="inline" size={16} /> mode switcher | New sessions |
  ...
  | [**lyxal Mode**](/docs/guides/lyxal-permissions#configuring-lyxal-mode) | `/mode [options]` | New sessions |

  :::info
  The CLI supports [additional slash commands](/docs/guides/lyxal-cli-commands#slash-commands) but doesn't support mid-session changes to the working directory or model.
  :::
