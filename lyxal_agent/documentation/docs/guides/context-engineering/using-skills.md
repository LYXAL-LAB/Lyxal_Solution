...
Skills are reusable sets of instructions and resources that teach lyxal how to perform specific tasks. A skill can range from a simple checklist to a detailed workflow with domain expertise, and can include supporting files like scripts or templates. Example use cases include deployment procedures, code review checklists, and API integration guides.

:::info
This functionality requires the built-in [Skills extension](/docs/mcp/skills-mcp) to be enabled (it's enabled by default).
:::

When a session starts, lyxal adds any skills that it discovers to its instructions. During the session, lyxal automatically loads a skill when:
...
You can also ask lyxal what skills are available.

:::info Claude Compatibility
lyxal skills are compatible with Claude Desktop and other [agents that support Agent Skills](https://agentskills.io/home#adoption).
:::

## Skill Locations

Skills can be stored globally and/or per-project. lyxal checks all of these directories in order and combines what it finds. If the same skill name exists in multiple directories, later directories take priority:

1. `~/.claude/skills/` — Global, shared with Claude Desktop
2. `~/.config/agents/skills/` — Global, portable across AI coding agents
3. `~/.config/lyxal/skills/` — Global, lyxal-specific
4. `./.claude/skills/` — Project-level, shared with Claude Desktop
5. `./.lyxal/skills/` — Project-level, lyxal-specific
...
When lyxal loads the skill, it sees the supporting files and can access them using the [Developer extension's](/docs/mcp/developer-mcp) file tools.
...
:::tip Other lyxal features that support reuse
- [.lyxalhints](/docs/guides/context-engineering/using-lyxalhints): Best for general preferences, project context, and repeated instructions like "Always use TypeScript"
...
- **Write for clarity** — Skills are instructions for lyxal. Use clear, direct language and numbered steps.
- **Include verification steps** — Help lyxal confirm the workflow completed successfully.
  ...
  items={[
    {
      ...
      linkUrl: '/lyxal/blog/2025/12/22/agent-skills-vs-mcp',
      date: '2025-12-22',
      duration: '4 min read'
    }
  ]}
