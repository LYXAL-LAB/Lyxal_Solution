...
This guide covers storing, organizing, and finding lyxal recipes when you need to access them again later. 

:::info Desktop UI vs CLI
- **lyxal Desktop** has a visual Recipe Library for browsing and managing saved recipes
- **lyxal CLI** stores recipes as files that you find using file paths or environment variables
:::
...
|------|----------|-------------|----------|
| **Global** | `~/.config/lyxal/recipes/` | All projects and sessions | Personal workflows, general-purpose recipes |
| **Local** | `YOUR_WORKING_DIRECTORY/.lyxal/recipes/` | Only when working in that project | Project-specific workflows, team recipes |
...
<Tabs groupId="interface">
  <TabItem value="desktop" label="lyxal Desktop" default>
...
:::
  ...
  <TabItem value="cli" label="lyxal CLI">
    ...
    * Local project recipes: `/recipe .lyxal/recipes/my-recipe.yaml`
...
<Tabs groupId="interface">
  ...
  lyxal Desktop accepts `.yaml`, `.yml`, and `.json` files, but **the CLI only supports `.yaml` and `.json`**. For full compatibility across both interfaces, avoid `.yml` extensions.
  ...
  <TabItem value="cli" label="lyxal CLI">
    Recipe import is only available in lyxal Desktop.
  </TabItem>
</Tabs>
...
Use the `lyxal recipe list` command to find all available recipes from multiple sources:
...
lyxal searches for recipes in the following locations (in order):

1. **Current directory**: `.` (looks for `*.yaml` and `*.json` files)
2. **Custom paths**: Directories specified in [`LYXAL_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) environment variable
3. **Global recipe library**: `~/.config/lyxal/recipes/` (or equivalent on your OS)
4. **Local project recipes**: `./.lyxal/recipes/`
5. **GitHub repository**: If [`LYXAL_RECIPE_GITHUB_REPO`](/docs/guides/environment-variables#recipe-configuration) environment variable is configured
...
$ lyxal recipe list
Available recipes:
lyxal-self-test - A comprehensive meta-testing recipe - local: ./lyxal-self-test.yaml
hello-world - A sample recipe demonstrating basic usage - local: ~/.config/lyxal/recipes/hello-world.yaml
job-finder - Find software engineering positions - local: ~/.config/lyxal/recipes/job-finder.yaml
```
...
Available recipes:
  lyxal-self-test - A comprehensive meta-testing recipe - local: ./lyxal-self-test.yaml
    Title: lyxal Self-Testing Integration Suite
    Path: ./lyxal-self-test.yaml
  hello-world - A sample recipe demonstrating basic usage - local: ~/.config/lyxal/recipes/hello-world.yaml
    Title: Hello World Recipe
    Path: /Users/username/.config/lyxal/recipes/hello-world.yaml
```
...
:::tip
The `lyxal recipe list` command is the recommended way to find recipes as it automatically searches all configured sources and provides consistent formatting.
:::
...
Once you've located your recipe file, [run the recipe](/docs/guides/recipes/session-recipes#run-a-recipe) or [open it in lyxal Desktop](/docs/guides/lyxal-cli-commands#recipe).

:::tip Format Compatibility
The CLI can run recipes saved from lyxal Desktop without any conversion. Both CLI-created and Desktop-saved recipes work with all recipe commands.
:::
