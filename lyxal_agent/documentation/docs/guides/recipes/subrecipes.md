...
When the main recipe is run, lyxal generates a tool for each subrecipe that:
...
**Usage:**
```bash
lyxal run --recipe code-review-pipeline.yaml --params repository_path=/path/to/repo
```
...
**Usage:**
```bash
lyxal run --recipe smart-analyzer.yaml --params repository_path=/path/to/project
```
...
**Usage:**
```bash
lyxal run --recipe travel-planner.yaml
```
