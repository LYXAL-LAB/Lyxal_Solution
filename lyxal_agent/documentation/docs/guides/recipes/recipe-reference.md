...
description: Complete technical reference for creating and customizing recipes in lyxal
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Recipes are reusable lyxal configurations that package up instructions and settings so the setup can be easily shared and launched by others.
...
:::info
`.yml` files aren't supported by lyxal CLI.
:::
...
1. Local filesystem:
   - Current directory
   - Directories specified in [`LYXAL_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) environment variable
   
2. GitHub repositories:
   - Configure using [`LYXAL_RECIPE_GITHUB_REPO`](/docs/guides/environment-variables#recipe-configuration) configuration key
...
| `prompt` | String| ✅*   | A template prompt that can include parameter substitutions. Required in [headless](/docs/tutorials/headless-lyxal) (non-interactive) mode. |
| `title` | String | ✅ | A short title describing the recipe |
| [`activities`](#activities) | Array | - | List of example prompts that can include parameter substitutions. Activities appear as clickable bubbles in lyxal Desktop. |
...
The `activities` field defines an optional message and clickable activity bubbles (buttons) that appears when a recipe is opened in lyxal Desktop.
...
| `bundled` | Boolean | (Optional) Whether the extension is bundled with lyxal |
...
- **`builtin`**: Built-in extension that is part of the bundled lyxal MCP server
...
extensions:
  - type: stdio
    ...
    timeout: 300
    bundled: true
    description: "Query https://codesearch.sqprod.co/ directly from lyxal"
...
If a recipe uses an extension that requires a secret, lyxal can prompt users to provide the secret when running the recipe:

1. When a recipe is loaded, lyxal scans all extensions (including those in subrecipes) for `env_keys` fields
2. If any required environment variables are missing from the secure keyring, lyxal prompts the user to enter them
...
The `required` and `optional` parameters work best for recipes opened in lyxal Desktop. If a value isn't provided for a `user_prompt` parameter, the parameter won't be substituted and may appear as literal `{{ parameter_name }}` text in the recipe output.
...
- `file`: The parameter value should be a file path. lyxal reads the file contents and substitutes the actual content (not the path) into the template
...
When a recipe with parameters is opened in lyxal Desktop, users are presented with a **Recipe Parameters** dialog where they can:
...
The `response` field enables recipes to enforce a final structured JSON output. When you specify a `json_schema`, lyxal will:
...
This feature is designed for **non-interactive automation** to ensure consistent, parseable output. Recipes can produce structured output when run from either the lyxal CLI or lyxal Desktop. See [use cases and ideas for automation workflows](/docs/guides/recipes/session-recipes#structured-output-for-automation).
...
- `LYXAL_RECIPE_RETRY_TIMEOUT_SECONDS`: Global timeout for success check commands
- `LYXAL_RECIPE_ON_FAILURE_TIMEOUT_SECONDS`: Global timeout for on_failure commands
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `lyxal_provider` | String | - | The AI provider to use (e.g., "anthropic", "openai") |
| `lyxal_model` | String | - | The specific model name to use |
...
settings:
  ...
  lyxal_model: "claude-sonnet-4-20250514"
  temperature: 0.7
```
...
:::note
Settings specified in a recipe will override your default lyxal configuration when that recipe is executed. If no settings are specified, lyxal will use your configured defaults.
:::
...
Recipes saved from lyxal Desktop include additional metadata fields. These fields are used by the Desktop app for organization and management but are ignored by CLI operations. 
...
Validation rules from [`validate_recipe.rs`](https://github.com/block/lyxal/blob/main/crates/lyxal/src/recipe/validate_recipe.rs) are enforced when loading recipes and used by the [`lyxal recipe validate`](/docs/guides/lyxal-cli-commands#recipe) subcommand:
...
extensions:
  - type: stdio
    ...
    timeout: 300
    bundled: true
    description: "Query codesearch directly from lyxal"
...
When these occur, lyxal will provide helpful error messages indicating what needs to be fixed.
