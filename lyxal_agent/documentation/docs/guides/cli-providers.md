...
description: Use Claude Code, Codex, Cursor Agent, or Gemini CLI subscriptions in lyxal
---

# CLI Providers

lyxal can make use of pass-through providers that integrate with existing CLI tools from Anthropic, OpenAI, Cursor, and Google. These providers allow you to use your existing Claude Code, Codex, Cursor Agent, and Google Gemini CLI subscriptions through lyxal's interface, adding session management, persistence, and workflow integration capabilities to these tools.

:::warning Limitations
These providers don’t fully support all lyxal features, may have platform or capability limitations, and can sometimes require advanced debugging if issues arise. They’re included here purely as a convenience.
:::
...
- already have a Claude Code, Codex, Cursor, or Google Gemini CLI subscription and want to use it through lyxal instead of paying per token
- need session persistence to save, resume, and export conversation history
- want to use lyxal recipes and scheduled tasks to create repeatable workflows
...
- **Recipe compatibility**: Use CLI providers in automated lyxal recipes
...
- **Unified commands**: Use the same `lyxal session` interface across all providers
- **Consistent configuration**: Manage all providers through lyxal's configuration system

:::warning Extensions
CLI providers do **not** give you access to lyxal's extension ecosystem (MCP servers, third-party integrations, etc.). They use their own built-in tools to prevent conflicts. If you need lyxal's extensions, use standard [API providers](/docs/getting-started/providers#available-providers) instead.
:::
...
- Automatic filtering of lyxal extensions from system prompts (since Claude Code has its own tool ecosystem)
...
- JSON output parsing for structured responses
- Automatic filtering of lyxal extensions from system prompts
...
3. **Configure lyxal**
   ...
   ```bash
   export LYXAL_PROVIDER=claude-code
   ```
   
   Or configure through the lyxal CLI using `lyxal configure`:
...
3. **Configure lyxal**
   ...
   ```bash
   export LYXAL_PROVIDER=cursor-agent
   ```
...
3. **Configure lyxal**
   ...
   Set the provider environment variable:
   ```bash
   export LYXAL_PROVIDER=gemini-cli
   ```
...
Once configured, you can start a lyxal session using these providers just like any others:
...
CLI providers work well in combination with other models using lyxal's [lead/worker pattern](/docs/tutorials/lead-worker):

```bash
# Use Claude Code as lead model, GPT-4o as worker
export LYXAL_LEAD_PROVIDER=claude-code
export LYXAL_PROVIDER=openai
export LYXAL_MODEL=gpt-4o
export LYXAL_LEAD_MODEL=default
...
|---------------------|-------------|---------|
| `LYXAL_PROVIDER` | Set to `claude-code` to use this provider | None |
| `LYXAL_MODEL` | Model to use (only `sonnet` or `opus` are passed to CLI) | `claude-sonnet-4-20250514` |
| `CLAUDE_CODE_COMMAND` | Path to the Claude CLI command | `claude` |

**Known Models:**

The following models are recognized and passed to the Claude CLI via the `--model` flag. If `LYXAL_MODEL` is set to a value not in this list, no model flag is passed and Claude Code uses its default:

- `sonnet`
- `opus`

**Permission Modes (`LYXAL_MODE`):**
...
| Environment Variable | Description | Default |
|---------------------|-------------|---------|
| `LYXAL_PROVIDER` | Set to `cursor-agent` to use this provider | None |
...
| Environment Variable | Description | Default |
|---------------------|-------------|---------|
| `LYXAL_PROVIDER` | Set to `codex` to use this provider | None |
| `LYXAL_MODEL` | Model to use (only known models are passed to CLI) | `gpt-5.2-codex` |
...
The following models are recognized and passed to the Codex CLI via the `-m` flag. If `LYXAL_MODEL` is set to a value not in this list, no model flag is passed and Codex uses its default:
...
| Environment Variable | Description | Default |
|---------------------|-------------|---------|
| `LYXAL_PROVIDER` | Set to `gemini-cli` to use this provider | None |
...
The CLI providers automatically filter out lyxal's extension information from system prompts since these CLI tools have their own tool ecosystems. This prevents conflicts and ensures clean interaction with the underlying CLI tools.

### Message Translation

- **Claude Code**: Converts lyxal messages to Claude's JSON message format, handling tool calls and responses appropriately
- **Codex**: Converts messages to simple text prompts with role prefixes (Human:/Assistant:), similar to Gemini CLI
- **Cursor Agent**: Converts lyxal messages to Cursor's JSON message format, handling tool calls and responses appropriately
...
CLI providers offer a way to use existing AI tool subscriptions through lyxal's interface, adding session management and workflow integration capabilities. They're particularly valuable for users with existing CLI subscriptions who want unified session management and recipe integration.
