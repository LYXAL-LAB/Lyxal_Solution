...
server.addTool({
  name: "fetchYoutube",
  description: "Fetch ALL YouTube videos from the lyxal channel.",
  parameters: z.object({}),
...
Same pattern for blogs and GitHub releases, straightforward tool functions with clear descriptions. The key is making your tool descriptions super simple and direct. lyxal needs to know exactly what each tool does.
...
Once both MCP servers were built, I plugged them into lyxal. For local servers, you just:

1. Go to Extensions in lyxal
2. Add the server with `node` command and path to your server
3. Add any environment variables
4. Toggle it on

Then I asked lyxal: "Hey, can you tell me if we have any new content?"
...
Once both MCP servers were built, I still needed something to pull them together. MCP servers do not talk to each other on their own. Without lyxal and an orchestrating recipe, they are just two separate tools waiting to be called.
...
In lyxal Desktop, open the `recipe` section, click the `calendar icon` , and choose when it should run (I set mine to 10 AM daily).

You can read more in the [Shareable Recipes Guide](https://block.github.io/lyxal/docs/guides/recipes/session-recipes#schedule-recipe).
:::
...
description: "Fetches new lyxal content or posts evergreen, generates platform-specific captions, and creates Sprout drafts."
...
instructions: |
  ...
  - contentfetcher__fetchLyxalBlog  
  ...
  **IF NO new content exists:**
  - Load the file /Users/ebonyl/.config/lyxal/evergreen.json
  ...
  **IF the item was NEW content (not evergreen):**
  ...
  - This updates ~/.config/lyxal/content-fetcher-mcp/last_seen.json
...
activities:
  - "Fetching latest lyxal content from all sources"
...
The best part? You can take this same approach for whatever automation you need. Spin up some MCP servers, create a recipe, let lyxal handle the orchestration. It's honestly so much fun watching it all come together.
...
<head>
  ...
  <meta property="og:url" content="https://block.github.io/lyxal/blog/2025/11/21/building-social-media-agent" />
  ...
  <meta property="og:image" content="https://block.github.io/lyxal/assets/images/header-image-7f5ab50f65332fb53302ca30a3f86e46.png" />
  ...
  <meta name="twitter:image" content="https://block.github.io/lyxal/assets/images/header-image-7f5ab50f65332fb53302ca30a3f86e46.png" />
</head>