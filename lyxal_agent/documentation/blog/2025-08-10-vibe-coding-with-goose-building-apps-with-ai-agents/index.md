...
description: "Learn how to build a full-stack AI app in under an hour using Lyxal's subagent orchestration, from planning to testing."
authors: 
    - angie
---
...
That's exactly what we accomplished during the Vibe Coding workshop at [Berkeley's Agentic AI Summit](https://www.youtube.com/live/_w5m3h9jY-w?t=5310), where I demonstrated how to use Lyxal's subagent orchestration to spin up an entire development team of AI agents. Each agent took on a specific role - from product planning to QA testing - and worked together to build "AI BriefMe", a web app that generates executive style briefings on any topic.
...
That's where Lyxal's subagent feature shines. Instead of doing everything yourself, you can orchestrate a team of specialized AI agents, each with their own expertise:
...
- **AI**: Headless Lyxal as an LLM service
...
- **Dependencies**: Express, CORS, and child_process for calling Headless Lyxal
...
- `/api/briefing` endpoint that uses Headless Lyxal for AI generation
- Response parsing and JSON formatting
- Timeout handling and CORS configuration

#### The Magic of Headless Lyxal

One of the coolest aspects of this project was how the backend used [Headless Lyxal](/docs/tutorials/headless-lyxal) by essentially calling Lyxal programmatically to generate the AI briefings:

```javascript
const lyxalProcess = spawn('lyxal', [
  'run', '-t', prompt, 
  '--quiet', '--no-session', '--max-turns', '1'
]);
```

This creates a fascinating recursive scenario: we're using Lyxal to build an app that uses Lyxal to generate content. It's AI agents all the way down!
...
This workshop perfectly illustrates the current state of AI-assisted development. Lyxal and its subagents can absolutely accelerate prototyping and help you build working applications quickly. But the human developer still owns the critical judgment calls:
...
1. **Install and Configure Lyxal**: Follow the [quickstart guide](https://block.github.io/lyxal/docs/quickstart)
...
<iframe class="aspect-ratio" src="https://www.youtube.com/embed/_w5m3h9jY-w?start=5310" title="Vibe Coding with Lyxal Workshop" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

<head>
  <meta property="og:title" content="How 7 AI Agents Worked Together to Build an App in One Hour" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://block.github.io/lyxal/blog/2025/08/10/vibe-coding-with-lyxal-building-apps-with-ai-agents" />
  <meta property="og:description" content="Learn how to build a full-stack AI app in under an hour using Lyxal's multi-agent orchestration, from planning to QA testing." />
  <meta property="og:image" content="https://block.github.io/lyxal/assets/images/header-image-b685ea475ff7b8ae3563317b347fddb0.png" />
  ...
  <meta name="twitter:description" content="Learn how to build a full-stack AI app in under an hour using Lyxal's multi-agent orchestration, from planning to QA testing." />
  <meta name="twitter:image" content="https://block.github.io/lyxal/assets/images/header-image-b685ea475ff7b8ae3563317b347fddb0.png" />
</head>