...
**The wild part is that I let Lyxal and six subagents bring this idea to life.**

## Meet the Subagents

[Subagents](/docs/guides/subagents) are individual AI instances that take on specific tasks. Each one runs in its own session, which helps preserve the main context window and keeps your primary Lyxal conversation uncluttered and focused on high-level orchestration. I think of subagents as temporary teammates. Lyxal assigns each subagent a job and deallocates it when the work is complete.
...
```bash
export LYXAL_ALPHA_FEATURES=true  
```

## Instructing My Team

There are a few ways to create subagents in Lyxal. You can use natural language prompts, define them through [recipes](/docs/guides/recipes/), or even spin up [external subagents](/docs/guides/subagents/#external-subagents) like Codex or Claude Code.
...
:::note TLDR
Lyxal lets you run subagents in parallel or sequentially. I chose a hybrid approach instructing Lyxal to run the first subagents sequentially (since their tasks relied on the previous step) and the last three subagents in parallel (since they only needed the core app to exist). 
...
After the agents completed the initial task, I proceeded with a follow-up prompt asking Lyxal to spawn five more subagents to work in parallel, each responsible for a different UI component: the header, request builder, tab layout, and collaboration panel. I figured that having the subagents execute the work in parallel would get the job done faster.
...
This time, I took a different approach.I told Lyxal to spin up one subagent to analyze the UI and come up with a shared design plan. Once the plan was ready, Lyxal could then spawn four more subagents to implement the plan in parallel.
...
The result was a much more cohesive, playful interface that actually looked like a digital playground. Having Lyxal coordinate subagents based on a shared design plan worked way better than running them loose in parallel.
...
* You can let Lyxal do the planning for you. You don’t have to micromanage every step.

I loved that instead of managing every detail, I could assign focused jobs and let Lyxal coordinate the flow. 
...
<head>
  ...
  <meta property="og:url" content="https://block.github.io/lyxal/blog/2025/07/21/orchestrating-subagents" />
  ...
  <meta property="og:image" content="https://block.github.io/lyxal/assets/images/built-by-subagents-869a01d4b147ebdb54334dcc22dc521e.png" />
  ...
  <meta name="twitter:image" content="https://block.github.io/lyxal/assets/images/built-by-subagents-869a01d4b147ebdb54334dcc22dc521e.png" />
</head>