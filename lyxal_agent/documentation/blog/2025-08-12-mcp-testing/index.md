---
title: "Automated MCP Testing: Using Composable Lyxal Recipes to Validate Tool Metadata"
description: "Automate MCP tool metadata validation using composable Lyxal recipes to catch regressions, optimize token usage, and ensure AI agents can reliably discover and use your tools."
authors: 
    - rarora
---
...
# Automated MCP Testing: Using Composable Lyxal Recipes to Validate Tool Metadata

When building Model Context Protocol (MCP) servers, most development focuses on tool functionality, ensuring tools execute and return expected results. But just as critical is the quality of tool metadata: descriptions, tooltips, and input schemas. These elements form the "interface language" between tools and AI agents like Lyxal.

Yet metadata often goes untested. This can break tool discovery and silently degrade agent behavior. In this post, we’ll show how to automate metadata validation using **composable Lyxal recipes**, turning manual QA into modular, repeatable workflows that:
...
## 2. System Overview: Modular and Composable Lyxal Recipes

The foundation of this framework is [Lyxal’s recipe engine](https://block.github.io/lyxal/docs/guides/recipes/). Recipes define reusable, declarative workflows for AI-assisted tasks. Each one encapsulates a step—like generating predictions or comparing results—and can be composed into larger pipelines.
...
## 3. The Core Engine: Lyxal Recipe for Tool Prediction

At the heart of the system is a Lyxal recipe that systematically transforms natural language queries into structured tool predictions. This recipe follows a clear three-step process: 
...
List the top 10 contributors to block/lyxal
Show me the closed branches in block/mcp
Show me all branches in the block/lyxal repository
```

**Step 2: Ask Lyxal to Make Predictions**
Using the developer extension, Lyxal analyzes the MCP server source code and documentation to understand available tools, their parameters, and usage patterns. It then maps each query to the most appropriate tool call.
...
instructions: |
  Generate evaluation datasets that map natural language queries to their expected tool calls with parameters. Analyze tool documentation and source code to understand available functions, their parameters, and usage patterns. Create comprehensive JSON test cases that include the original query, expected tool name, and all required/implied parameters with realistic values. The output should be a complete JSON file with test_cases array, where each case maps a natural language request to its corresponding structured tool call. Use developer tools to examine source files, read documentation, and write the final JSON dataset to disk.
...

    {
        "test_cases": [
            {
                "query": "Show me open pull requests in the block/lyxal repository",
                "expected": {
                    "tool": "tool_name",
                    "parameters": {
                        "repo_owner": "block",
...
- type: builtin
  ...
  bundled: true
settings:
  lyxal_provider: databricks
  lyxal_model: lyxal-claude-4-sonnet
  temperature: 0.0
parameters:
- key: server_input
  input_type: string
...
```bash
lyxal run --recipe generate_predictions_recipe.yaml --params output_file=my_predictions.json
```
...
```bash
lyxal run --recipe generate_predictions_recipe.yaml --params output_file=new_evaluation.json
```
...
**Step 3: Ask Lyxal to Interpret Results**
Finally, Lyxal analyzes the comparison output and highlights what's not matching, providing human-readable explanations of the differences.
...
instructions: |
  ...
  Command to generate output: lyxal run --recipe generate_predictions_recipe.yaml --params output_file={{ output_file }}
...
```bash
lyxal run --recipe evaluate_predictions.yaml --params output_file=new_evaluation.json gold_file=mcp_github_query_tool_truth.json
```
...
- **Query:** "Search for files containing console.log in block/lyxal"
- **Gold Standard:**
  ```json
  {
    "tool": "search_codebase",
...
This workflow demonstrates the true power of composable Lyxal recipes—we can orchestrate the core prediction recipe from Section 3 and the evaluation workflow from Section 4 into a continuous optimization loop that reduces token usage without breaking tool discoverability.
...
Using natural language processing, Lyxal identifies verbose tooltips, redundant documentation, and unnecessary examples, then compresses them while preserving essential information.
...
**Step 3: Fix Issues**
If evaluation tests fail, Lyxal analyzes the specific mismatches and iteratively fixes the compressed tooltips to restore functionality.
...
instructions: |
  ...
  Command to run evaluation lyxal run --recipe evaluate_predictions.yaml
...
```bash
lyxal run --recipe compress_evaluate_mcp.yaml --params target_reduction=10
```
...
Automated MCP metadata validation doesn’t have to be brittle or one-off. By using a **modular Lyxal recipe architecture**, we’ve shown how a single core prediction recipe can power multiple high-value workflows—from **catching regressions early** to **reducing tokens safely** without sacrificing discoverability.
...
<head>
  <meta property="og:title" content="Automated MCP Testing: Using Composable Lyxal Recipes to Validate Tool Metadata" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://block.github.io/lyxal/blog/2025/08/12/mcp-testing" />
  <meta property="og:description" content="Automate MCP tool metadata validation using composable Lyxal recipes to catch regressions, optimize token usage, and ensure AI agents can reliably discover and use your tools" />
  <meta property="og:image" content="https://block.github.io/lyxal/assets/images/automated_mcp_testing-296dac2cd2b1b327e58854f4bfb0c89a.jpg" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta property="twitter:domain" content="block.github.io/lyxal" />
  <meta name="twitter:title" content="Automated MCP Testing: Using Composable Lyxal Recipes to Validate Tool Metadata" />
  <meta name="twitter:description" content="Automate MCP tool metadata validation using composable Lyxal recipes to catch regressions, optimize token usage, and ensure AI agents can reliably discover and use your tools" />
  <meta name="twitter:image" content="https://block.github.io/lyxal/assets/images/automated_mcp_testing-296dac2cd2b1b327e58854f4bfb0c89a.jpg" />
</head>
