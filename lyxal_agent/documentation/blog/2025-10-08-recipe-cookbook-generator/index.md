---
title: "Build Your Own Recipe Cookbook Generator for lyxal"
description: Discover how lyxal can study your past sessions and turn your favorite workflows into reusable automation templates.
authors: 
    - ian
---
...
You've been using lyxal for weeks, maybe months. You have dozens of successful [sessions](/docs/guides/sessions/) where you asked for help with blog posts, code reviews, documentation, or data analysis. Each time you think "Didn't I already do this?" but never get around to checking. Sound familiar?

I myself had over a hundred lyxal sessions and as many megabytes of conversation data. I was sitting on a goldmine of potential automation. A coworker suggested something brilliant: "What if lyxal could analyze your sessions and build recipes automatically?" Wait, wait, wait!! Create a personalized cookbook based on my own session history? Yes, please! Let's build a "cookbook generator" recipe!
...
Let's boost our productivity by implementing some automation. We're gonna get lyxal to write a recipe that creates OTHER recipes!

## What is the Cookbook Generator Recipe?

The "cookbook generator" I'm describing here is a way to get lyxal to look at your previous sessions, analyze them for commonalities, and automatically create new recipes from common patterns. It's automation that creates automation -- meta-programming at its finest.

I'm going to share my own cookbook generator at the end.

Here's what it does:

1. **Scans your session history** -- Finds and reads all your lyxal session files
...
[I demonstrated on a livestream](https://www.youtube.com/watch?v=-_1GALH2ER0) my whole process of creating this generator using "[vibe prompting](https://www.youtube.com/watch?v=IjXmT0W4f2Q)" -- having an extended conversation with lyxal to refine the idea and answer potential questions upfront. This approach uses fewer tokens than iterative coding, resulting in higher success rates.

<iframe class="aspect-ratio" src="https://www.youtube.com/embed/-_1GALH2ER0" title="Vibe Code with lyxal: Automate my workflow" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

During my own conversation with lyxal, lyxal asked smart questions like:
...
The generator reads through your session files (typically stored in `~/.local/share/lyxal/sessions/` but this may vary on your platform, and may change in future lyxal releases) and analyzes:
- Overall outcome success
- User request patterns
- Tool usage sequences
- Common parameter types it could extract

This last one was really important for me. I ask lyxal all the time to help me build an outline for blog posts like this, or video scripts for our [YouTube channel](https://youtube.com/@lyxal-oss), or tutorial pages. They all follow a similar pattern, around a topic or subject, but the output format may vary.
...
Having lyxal prompt me for whether I know ahead of time if there are any sessions I want to exclude was helpful -- I have several REALLY large, long sessions about the [community cookbook security scanner](/blog/2025/08/25/lyxal-became-its-own-watchdog/) I built, but I didn't want to build a recipe out of all of that.

Instead, I wanted lyxal to focus on workflows that appear multiple times, and ask me to confirm any cases that it was unsure about.
...
:::tip
I wanted lyxal to write all the recipes for me, but to be as up-to-date as possible. I cloned the [lyxal repository](https://github.com/block/lyxal) and told lyxal to examine its own source code to learn how to successfully create recipes, and be sure to use proper YAML syntax.
:::

From that, I had lyxal think about the following ideas as it considered how to make my recipes:
...
Building the cookbook generator took a little over an hour of "vibe prompting" and refining the idea, and then lyxal gave me a recipe. I always verify my AI-generated work, and then spent about 15 minutes more after the livestream refining some ideas and adding more guardrails to the recipe.
...
- **Social Media Posts**: I struggle a lot with coming up with catchy social media posts, so I want to give lyxal the kind of content (blog, video, etc), point it at the content (in the case of video, I give it my narration script), and have it generate many different options that I can choose from later.
...
By running the outline generator recipe (three times), the image generator recipe, and social media recipe, lyxal took care of the following work:
...
- the script outline for a longer video going on our [YouTube channel](https://youtube.com/@lyxal-oss)
...
:::info
If you want to try some hands-free automation, check out the experimental [Perception](https://github.com/michaelneale/lyxal-perception) project from one of our teammates!
:::
...
The last thing I'd want to improve would be to have lyxal re-analyze the recipes it made with newer sessions to refine existing recipes, instead of creating new recipes each time it runs.
...
The key insight is that AI agents shouldn't just execute tasks -- they should learn from those executions and help us build better systems. This generator turns your lyxal usage history into a productivity asset, creating a personalized automation toolkit from your actual work patterns.
...
Want to contribute your own recipes or improvements to the cookbook generator? Join our [Discord community](https://discord.gg/lyxal-oss) or check out our [GitHub repository](https://github.com/block/lyxal) for more automation ideas. Join our [Hacktoberfest](https://github.com/block/lyxal/issues/4705) event going on to contribute recipes and prompt ideas to get on our leaderboard to win some great prizes!


## My Own Cookbook Generator Recipe

Here's the cookbook generator that lyxal helped me create, plus my own notes. You could try using it as-is, but I think a better approach would be to try vibe prompting with lyxal yourself to go analyze your own session history to see what kind of automation you want to set up for yourself.

```yaml
version: "1.0.0"
title: Recipe Cookbook Generator
description: |
  Analyze your lyxal session history to automatically generate recipes from your common workflows. This tool examines your past interactions with lyxal, identifies repetitive patterns, and creates reusable recipes that can automate similar tasks in the future. Perfect for capturing your personal automation patterns and building a custom recipe library.

prompt: |
  I want to analyze my lyxal session history and create recipes from common workflows I've used. I've done a variety of work, and I'd like your help finding repetitive tasks that we can turn into lyxal recipes to build my own personal 'cookbook' based on my lyxal usage patterns.
  ...
  The process should:
  1. Find and analyze my lyxal session files{% if session_storage_path %} (located at {{ session_storage_path }}){% endif %}
...
parameters:
  ...
  - key: session_storage_path
    description: Path to lyxal session storage (will auto-detect if not provided)
    input_type: string
...
instructions: |
  You are helping the user build a 'cookbook' of lyxal recipes based on their actual usage patterns.
  
  ## Step 1: Setup and Discovery
  - Determine the lyxal session storage path (try default locations like ~/.local/share/lyxal/sessions, or ask the user)
  ...
  - Each recipe includes:
    - Proper lyxal YAML format with version, title, description
...
<head>
  <meta property="og:title" content="Build Your Own Recipe Cookbook Generator for lyxal" />
  <meta property="og:type" content="article" />
  <meta property="og:url" content="https://block.github.io/lyxal/blog/2025/10/08/recipe-cookbook-generator" />
  <meta property="og:description" content="Discover how lyxal can study your past sessions and turn your favorite workflows into reusable automation templates." />
  <meta property="og:image" content="https://block.github.io/lyxal/assets/images/recipe-cookbook-generator-ba6ce396585e1fc591e339720ee80a30.png" />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="Build Your Own Recipe Cookbook Generator for lyxal" />
  <meta name="twitter:description" content="Discover how lyxal can study your past sessions and turn your favorite workflows into reusable automation templates." />
  <meta property="twitter:domain" content="block.github.io/lyxal" />
  <meta name="twitter:image" content="https://block.github.io/lyxal/assets/images/recipe-cookbook-generator-ba6ce396585e1fc591e339720ee80a30.png" />
</head>
