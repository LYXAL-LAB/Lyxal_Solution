...
Automated pipeline for detecting and documenting Recipe schema and validation rule changes between lyxal releases.

## Overview

This automation keeps the [Recipe Reference Guide](https://block.github.io/lyxal/docs/guides/recipes/recipe-reference) synchronized with code changes by:
...
cd output && lyxal run --recipe ../recipes/synthesize-validation-changes.yaml

# 5. Update recipe-reference.md
export RECIPE_REF_PATH=/path/to/recipe-reference.md
lyxal run --recipe ../recipes/update-recipe-reference.yaml
```
...
|----------|----------|---------|-------------|
| `RECIPE_REF_PATH` | No | - | Full path to `recipe-reference.md` file (overrides `LYXAL_REPO` construction) |
| `LYXAL_REPO` | No | Auto-detect | Path to lyxal repository root |

**Example (for local testing):**
```bash
export RECIPE_REF_PATH=/path/to/local/lyxal/documentation/docs/guides/recipes/recipe-reference.md
# OR
export LYXAL_REPO=/path/to/local/lyxal
```
...
{
  "validation_files": [
    "crates/lyxal/src/recipe/validate_recipe.rs",
    "crates/lyxal/src/agents/types.rs"
  ]
}
```
...
Extracts OpenAPI schema from the lyxal codebase.
...
```bash
cd output
lyxal run --recipe ../recipes/synthesize-validation-changes.yaml
```
...
- `recipe-reference.md` - Target documentation file (path from `RECIPE_REF_PATH` or `LYXAL_REPO` env var)
