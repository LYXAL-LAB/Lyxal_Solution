...
The `lyxal web` command provides a (preview) web-based chat interface for interacting with lyxal.
...
lyxal web --host 0.0.0.0 --port 8080
```

## Features

- **Real-time chat interface**: Communicate with lyxal through a clean web UI
...
### Integration with lyxal Agent

The web server creates an instance of the lyxal Agent and processes messages through the same pipeline as the CLI. However, some features like:
...
3. **Agent not configured**: Run `lyxal configure` first to set up a provider