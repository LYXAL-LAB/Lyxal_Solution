use super::completion::LyxalCompleter;
...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("LYXAL_CLI_NEWLINE_KEY")
        .ok()
...
pub fn get_input(
    editor: &mut Editor<LyxalCompleter, rustyline::history::DefaultHistory>,
    conversation_messages: Option<&Vec<String>>,
) -> Result<InputResult> {
    let config = Config::global();
    if let Ok(Some(editor_cmd)) = config.get_lyxal_prompt_editor() {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::LyxalMode(
...
/// Returns a styled prompt with the lyxal face "( O)>" followed by a space.
...
fn get_input_prompt_string() -> String {
    ...
    if cfg!(target_os = "windows") {
        ...
        format!("{} ", console::style(lyxal).cyan().bold())
    }
}
...
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, lyxal mode is set to 'auto' and returns to 'normal' lyxal mode.
                        To warm up lyxal before using '/plan', we recommend setting '/mode approve' & putting appropriate context into lyxal.
                        The model is used based on $LYXAL_PLANNER_PROVIDER and $LYXAL_PLANNER_MODEL environment variables.
...
Ctrl+{newline_key} - Add a newline (configurable via LYXAL_CLI_NEWLINE_KEY)
...
fn print_editor_help() {
    println!(
        "Editor Input:
When lyxal_prompt_editor is configured, prompts will open in your editor instead of the CLI.
Previous conversation is included as markdown headings for context.
Configure with: lyxal configure set lyxal_prompt_editor \"vim\""
    );
}
...
mod tests {
    ...
    fn test_get_input_prompt_string() {
        ...
        // Prompt should contain the lyxal face
