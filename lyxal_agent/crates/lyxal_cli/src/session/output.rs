...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("LYXAL_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("LYXAL_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("LYXAL_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("LYXAL_CLI_THEME", theme_str) {
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("LYXAL_STATUS_HOOK") {
    ...
    for content in &message.content {
        match content {
            ...
            MessageContent::Thinking(thinking) => {
                if std::env::var("LYXAL_CLI_SHOW_THINKING").is_ok()
            ...
            MessageContent::SystemNotification(notification) => {
                use lyxal::conversation::message::SystemNotificationType;
...
pub fn lyxal_mode_message(text: &str) {
    println!("\n{}", style(text).yellow(),);
}
...
pub fn display_session_info(
    ...
    provider_instance: Option<&Arc<dyn lyxal::providers::base::Provider>>,
) {
