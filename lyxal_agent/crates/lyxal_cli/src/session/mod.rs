...
use lyxal::conversation::Conversation;
use std::io::Write;
use std::str::FromStr;
...
use console::Color;
use lyxal::agents::subagent_handler::SUBAGENT_TOOL_REQUEST_TYPE;
use lyxal::agents::AgentEvent;
use lyxal::permission::permission_confirmation::PrincipalType;
use lyxal::permission::Permission;
use lyxal::permission::PermissionConfirmation;
use lyxal::providers::base::Provider;
use lyxal::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::LyxalCompleter;
use lyxal::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use lyxal::agents::types::RetryConfig;
use lyxal::agents::{Agent, SessionConfig, COMPACT_TRIGGERS};
use lyxal::config::{Config, LyxalMode};
...
use lyxal::conversation::message::{ActionRequiredData, Message, MessageContent};
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<LyxalCompleter, rustyline::history::DefaultHistory>,
    ) {
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: lyxal::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
    ...
    pub async fn add_streamable_http_extension(&mut self, extension_url: String) -> Result<()> {
        let config = Self::parse_streamable_http_extension(
            &extension_url,
            lyxal::config::DEFAULT_EXTENSION_TIMEOUT,
        );
        self.add_and_persist_extensions(vec![config]).await
    }
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<LyxalCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<LyxalCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = LyxalCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::LyxalMode(mode) => {
                history.save(editor);
                self.handle_lyxal_mode(&mode)?;
            }
    ...
    fn handle_lyxal_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match LyxalMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
        ...
        output::lyxal_mode_message(&format!("Lyxal mode set to '{:?}'", mode));
        Ok(())
    }
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set lyxal mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_lyxal_mode = config.get_lyxal_mode().unwrap_or(LyxalMode::Auto);
                    if curr_lyxal_mode != LyxalMode::Auto {
                        config.set_lyxal_mode(LyxalMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & lyxal mode
                    if curr_lyxal_mode != LyxalMode::Auto {
                        config.set_lyxal_mode(curr_lyxal_mode)?;
                    }
                } else {
    ...
    pub async fn get_session(&self) -> Result<lyxal::session::Session> {
        self.agent
            .config
            .session_manager
            .get_session(&self.session_id, false)
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let show_cost = config
            .get_param::<bool>("LYXAL_CLI_SHOW_COST")
            .unwrap_or(false);
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "Lyxal would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("LYXAL_CLI_MIN_PRIORITY")
                            .ok()
...
fn log_tool_metrics(message: &Message, messages: &Conversation) {
    for content in &message.content {
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                counter.lyxal.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<lyxal::providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                lyxal::providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
async fn get_reasoner() -> Result<Arc<dyn Provider>, anyhow::Error> {
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("LYXAL_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: LYXAL_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_lyxal_provider()
            .expect("No provider configured. Run 'lyxal configure' first")
    };

    // Try planner-specific model first, fallback to default model
    let model = if let Ok(model) = config.get_param::<String>("LYXAL_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: LYXAL_PLANNER_MODEL not found. Using default model...");
        config
            .get_lyxal_model()
            .expect("No model configured. Run 'lyxal configure' first")
    };

    let model_config =
        ModelConfig::new_with_context_env(model, Some("LYXAL_PLANNER_CONTEXT_LIMIT"))?;
