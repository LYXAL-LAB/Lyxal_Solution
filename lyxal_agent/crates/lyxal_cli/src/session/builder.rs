...
use console::style;
use lyxal::agents::{Agent, Container};
use lyxal::config::get_enabled_extensions;
use lyxal::config::resolve_extensions_for_new_session;
use lyxal::config::{get_all_extensions, Config, ExtensionConfig};
use lyxal::providers::create;
use lyxal::recipe::Recipe;
use lyxal::session::session_manager::SessionType;
use lyxal::session::{EnabledExtensionsState, ExtensionState};
...
/// Configuration for building a new Lyxal session
///
...
async fn load_extensions(
    ...
    provider_for_debug: Arc<dyn lyxal::providers::base::Provider>,
    interactive: bool,
    session_id: &str,
) -> Arc<Agent> {
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    lyxal::posthog::set_session_context("cli", session_config.resume);
    ...
    let provider_name = session_config
        .provider
        .or(saved_provider)
        .or_else(|| recipe_settings.and_then(|s| s.lyxal_provider.clone()))
        .or_else(|| config.get_lyxal_provider().ok())
        .expect("No provider configured. Run 'lyxal configure' first");

    let model_name = session_config
        .model
        .or_else(|| saved_model_config.as_ref().map(|mc| mc.model_name.clone()))
        .or_else(|| recipe_settings.and_then(|s| s.lyxal_model.clone()))
        .or_else(|| config.get_lyxal_model().ok())
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        lyxal::model::ModelConfig::new(&model_name)
            .unwrap_or_else(|e| {
    ...
    let new_provider = match create(&provider_name, model_config).await {
        ...
        Err(e) => {
            output::render_error(&format!(
                ...
                For more info, see: https://block.github.io/lyxal/docs/troubleshooting/#keychainkeyring-errors",
                e
            ));
            process::exit(1);
        }
    ...
    let debug_mode = session_config.debug || config.get_param("LYXAL_DEBUG").unwrap_or(false);
    ...
    let system_prompt_file: Option<String> = config.get_param("LYXAL_SYSTEM_PROMPT_FILE_PATH").ok();
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: lyxal::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
