...
fn create_temp_file(messages: &[&str]) -> Result<NamedTempFile> {
    let temp_file = Builder::new()
        .prefix("lyxal_prompt_")
        .suffix(".md")
        .tempfile()?;
    let mut content = String::from("# Lyxal Prompt Editor\n\n");
...
pub fn get_editor_input(editor_cmd: &str, messages: &[&str]) -> Result<(String, bool)> {
    ...
    let symlink_path = PathBuf::from(".lyxal_prompt_temp.md");
    ...
    let _original_template = {
        let mut template_content = String::from("# Lyxal Prompt Editor\n\n");
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# Lyxal Prompt Editor
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(path.exists());
        assert!(path.to_str().unwrap().contains("lyxal_prompt_"));
        assert!(path.to_str().unwrap().ends_with(".md"));

        let content = fs::read_to_string(path).unwrap();
        assert!(content.contains("# Lyxal Prompt Editor"));
    ...
    fn test_create_temp_file_with_prefix_suffix() {
        ...
        assert!(name.starts_with("lyxal_test_"));
