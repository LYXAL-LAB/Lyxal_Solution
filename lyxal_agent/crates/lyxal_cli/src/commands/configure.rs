use crate::recipes::github_recipe::LYXAL_RECIPE_GITHUB_REPO_CONFIG_KEY;
use cliclack::spinner;
use console::style;
use lyxal::agents::extension::ToolInfo;
use lyxal::agents::extension_manager::get_parameter_names;
use lyxal::agents::Agent;
use lyxal::agents::{extension::Envs, ExtensionConfig};
use lyxal::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, CreateCustomProviderParams,
};
use lyxal::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use lyxal::config::paths::Paths;
use lyxal::config::permission::PermissionLevel;
use lyxal::config::signup_tetrate::TetrateAuth;
use lyxal::config::{
    configure_tetrate, Config, ConfigError, ExperimentManager, ExtensionEntry, LyxalMode,
    PermissionManager,
};
use lyxal::model::ModelConfig;
use lyxal::posthog::{get_telemetry_choice, TELEMETRY_ENABLED_KEY};
use lyxal::providers::provider_test::test_provider_configuration;
use lyxal::providers::{create, providers, retry_operation, RetryConfig};
use lyxal::session::SessionType;
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve lyxal").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve lyxal by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how lyxal is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • lyxal version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'lyxal configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve lyxal?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve lyxal!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    println!();
    println!("{}", style("Welcome to lyxal! Let's get you set up.").dim());
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" lyxal-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "lyxal settings",
            "Set the lyxal mode, Tool Output, Tool Permissions, Experiment, lyxal recipe github repo and more",
        )
        .interact()?;
...
fn select_model_from_list(
    models: &[String],
    provider_meta: &lyxal::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with LYXAL_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_lyxal_provider().ok();
    ...
    let model: String = match models_res {
        ...
        Ok(None) => {
            let default_model =
                std::env::var("LYXAL_MODEL").unwrap_or(provider_meta.default_model.clone());
    ...
    let toolshim_enabled = std::env::var("LYXAL_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("LYXAL_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(provider_name, &model, toolshim_enabled, toolshim_model).await
    {
        Ok(()) => {
            config.set_lyxal_provider(provider_name)?;
            config.set_lyxal_model(&model)?;
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&lyxal::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with lyxal",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    let setting_type = cliclack::select("What setting would you like to configure?")
        .item("lyxal_mode", "lyxal mode", "Configure lyxal mode")
        .item(
            "telemetry",
            "Telemetry",
        ...
        .item(
            ...
            "lyxal will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_lyxal_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("LYXAL_MODE").is_ok() {
        let _ = cliclack::log::info("Notice: LYXAL_MODE environment variable is set and will override the configuration here.");
    }

    let mode = cliclack::select("Which lyxal mode would you like to configure?")
        .item(
            LyxalMode::Auto,
            "Auto Mode",
    ...
    config.set_lyxal_mode(mode)?;
    let msg = match mode {
        LyxalMode::Auto => "Set to Auto Mode - full file modification enabled",
        LyxalMode::Approve => "Set to Approve Mode - all tools and modifications require approval",
        LyxalMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        LyxalMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("LYXAL_TELEMETRY_OFF").is_ok() {
        let _ = cliclack::log::info("Notice: LYXAL_TELEMETRY_OFF environment variable is set and will override the configuration here.");
    }
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve lyxal!")?;
    } else {
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("LYXAL_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info("Notice: LYXAL_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.");
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("LYXAL_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("LYXAL_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("LYXAL_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("LYXAL_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info("Notice: LYXAL_DISABLE_KEYRING environment variable is set and will override the configuration here.");
    }

    let currently_disabled = config.get_param::<String>("LYXAL_DISABLE_KEYRING").is_ok();
    ...
    let _ = cliclack::log::warning("Note: Disabling the keyring stores secrets in a plain text file (~/.config/lyxal/secrets.yaml)");
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("LYXAL_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ =
                cliclack::log::info("You may need to restart lyxal for this change to take effect");
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param("LYXAL_DISABLE_KEYRING", Value::String("true".to_string()))?;
            cliclack::outro(
                "Secret storage set to file (~/.config/lyxal/secrets.yaml). Keep this file secure!",
            )?;
            let _ =
                cliclack::log::info("You may need to restart lyxal for this change to take effect");
        }
...
/// Configure experiment features that can be used with lyxal
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = LYXAL_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("LYXAL_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("LYXAL_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - lyxal will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use lyxal::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    use lyxal::conversation::message::Message;
    use lyxal::providers::create;
    ...
    let configured_model: String = config.get_lyxal_model()?;
    let model_config = match lyxal::model::ModelConfig::new(&configured_model) {
        Ok(config) => config,
        Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Builtin {
                                ...
                                display_name: Some(lyxal::config::DEFAULT_DISPLAY_NAME.to_string()),
                                timeout: Some(lyxal::config::DEFAULT_EXTENSION_TIMEOUT),
                                bundled: Some(true),
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use lyxal.")?;
                }
                Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use lyxal.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
...
fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = lyxal::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        lyxal::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
