...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("lyxal");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run lyxal. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("lyxal Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run lyxal
            let mut command = std::process::Command::new("lyxal");
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run lyxal with a fresh session
            let mut command = std::process::Command::new("lyxal");
            command.arg("session");
