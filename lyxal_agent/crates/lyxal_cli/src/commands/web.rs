...
use lyxal::conversation::message::Message as LyxalMessage;
...
async fn auth_middleware(
    ...
    *response.status_mut() = StatusCode::UNAUTHORIZED;
    response.headers_mut().insert(
        "WWW-Authenticate",
        "Basic realm=\"Lyxal Web Interface\"".parse().unwrap(),
    );
    Ok(response)
}
...
fn get_provider_and_model() -> (String, String) {
    ...
    let provider_name: String = match config.get_lyxal_provider() {
        Ok(p) => p,
        Err(_) => {
            eprintln!("No provider configured. Run 'lyxal configure' first");
            std::process::exit(1);
        }
    };

    let model: String = match config.get_lyxal_model() {
        Ok(m) => m,
        Err(_) => {
            eprintln!("No model configured. Run 'lyxal configure' first");
            std::process::exit(1);
        }
    };
...
async fn create_agent(provider_name: &str, model: &str) -> Result<Agent> {
    let model_config = lyxal::model::ModelConfig::new(model)?;
    ...
    let provider = lyxal::providers::create(provider_name, model_config).await?;
    agent.update_provider(provider, &init_session.id).await?;

    let enabled_configs = lyxal::config::get_enabled_extensions();
...
pub async fn handle_web(
    ...
    crate::logging::setup_logging(Some("lyxal-web"), None)?;
...
async fn serve_session(
    ...
    let html_with_session = html.replace(
        ...
        &format!(
            "<script>window.LYXAL_SESSION_NAME = '{}'; window.LYXAL_WS_TOKEN = '{}';</script>\n    <script src=\"/static/script.js\"></script>",
            session_name,
            state.ws_token
        )
    );
...
async fn process_message_streaming(
    ...
    let user_message = LyxalMessage::user().with_text(content.clone());

    let provider = agent.provider().await;
    if provider.is_err() {
        let error_msg = "I'm not properly configured yet. Please configure a provider through the CLI first using `lyxal configure`.".to_string();
        let mut sender = sender.lock().await;
        let _ = sender
            .send(Message::Text(
...
async fn process_agent_message(
    message: &LyxalMessage,
    sender: &Arc<Mutex<futures::stream::SplitSink<WebSocket, Message>>>,
    agent: &Agent,
) {
    use lyxal::conversation::message::MessageContent;
                ...
                agent
                    .handle_confirmation(
                        ...
                        lyxal::permission::PermissionConfirmation {
                            principal_type:
                                lyxal::permission::permission_confirmation::PrincipalType::Tool,
