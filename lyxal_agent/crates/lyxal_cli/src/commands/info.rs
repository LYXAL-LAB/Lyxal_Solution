...
use lyxal::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
use serde_yaml;
...
pub fn handle_info(verbose: bool) -> Result<()> {
    ...
    println!("{}", style("lyxal Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("lyxal Configuration:").cyan().bold());
        let values = config.all_values()?;
        if values.is_empty() {
            println!("  No configuration values set");
            println!(
                "  Run '{}' to configure lyxal",
