...
use chrono;
use lyxal::conversation::message::{Message, MessageContent, MessageMetadata};
use lyxal::session::{SessionManager, SessionType};
use rmcp::model::Role;
...
alias @lyxal='{lyxal_bin} term run'
alias @g='{lyxal_bin} term run'

lyxal_preexec() {
    [[ "$1" =~ ^lyxal\ term ]] && return
    [[ "$1" =~ ^(@lyxal|@g)($|[[:space:]]) ]] && return
    ('{lyxal_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$lyxal_preexec_installed" ]]; then
    lyxal_preexec_installed=1
    trap 'lyxal_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "🪿 Command '$1' not found. Asking lyxal..."
    '{lyxal_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec lyxal_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @lyxal; {lyxal_bin} term run $argv; end
function @g; {lyxal_bin} term run $argv; end

function lyxal_preexec --on-event fish_preexec
    string match -q -r '^lyxal term' -- $argv[1]; and return
    string match -q -r '^(@lyxal|@g)($|\s)' -- $argv[1]; and return
    {lyxal_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
function @lyxal {{ & '{lyxal_bin}' term run @args }}
function @g {{ & '{lyxal_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^lyxal term' -and $line -notmatch '^(@lyxal|@g)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{lyxal_bin}' term log $using:line }} | Out-Null
    }}
...
pub async fn handle_term_init(
    ...
    let lyxal_bin = std::env::current_exe()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_else(|_| "lyxal".to_string());

    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|s| s.replace("{lyxal_bin}", &lyxal_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!("AGENT_SESSION_ID not set. Run 'eval \"$(lyxal term init <shell>)\"' first.")
    })?;
...
/// Handle `lyxal term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = config
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short.strip_prefix("lyxal-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = config
        .get_lyxal_model()
        .ok()
        .and_then(|model_name| lyxal::model::ModelConfig::new(&model_name).ok())
