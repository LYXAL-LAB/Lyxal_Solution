...
use lyxal::builtin_extension::register_builtin_extensions;
use lyxal::config::Config;
use lyxal::posthog::get_telemetry_choice;
use lyxal::recipe::Recipe;
use lyxal_mcp::mcp_server_runner::{serve, McpCommand};
use lyxal_mcp::{
...
use lyxal::agents::Container;
use lyxal::session::session_manager::SessionType;
use lyxal::session::SessionManager;
use lyxal_bench::bench_config::BenchRunConfig;
use lyxal_bench::runners::bench_runner::BenchRunner;
use lyxal_bench::runners::eval_runner::EvalRunner;
use lyxal_bench::runners::metric_aggregator::MetricAggregator;
use lyxal_bench::runners::model_runner::ModelRunner;
use std::io::Read;
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, lyxal must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), lyxal::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with lyxal by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to lyxal directly",
        long_help = "Input text containing commands for lyxal. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=lyxal-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the LYXAL_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the LYXAL_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum RecipeCommand {
    ...
    #[command(about = "Open a recipe in Lyxal Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Configure lyxal settings
    #[command(about = "Configure lyxal settings")]
    Configure {},

    /// Display lyxal configuration information
    #[command(about = "Display lyxal information")]
    Info {
    ...
    #[command(about = "Run one of the mcp servers bundled with lyxal")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run lyxal as an ACP (Agent Client Protocol) agent
    #[command(about = "Run lyxal as an ACP agent server on stdio")]
    Acp {
    ...
    /// Update the lyxal CLI version
    #[command(about = "Update the lyxal CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the lyxal CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure lyxal during update
        #[arg(short, long, help = "Enforce to re-configure lyxal during update")]
        reconfigure: bool,
    },
    ...
    #[command(
        about = "Terminal-integrated lyxal session",
        long_about = "Runs a lyxal session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(lyxal term init zsh)\"  # Add to ~/.zshrc\n\n\
                      Usage:\n  \
                        lyxal term run \"list files in this directory\"\n  \
                        @lyxal \"create a python script\"  # using alias\n  \
    ...
    Completion {
        ...
        #[arg(long, default_value = "lyxal", help = "Provide a custom binary name")]
        bin_name: String,
    },
}
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent lyxal session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(lyxal term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                      With --default (anything typed that isn't a command goes to lyxal):\n  \
                        echo 'eval \"$(lyxal term init zsh --default)\"' >> ~/.zshrc"
    )]
    Init {
        ...
        /// Make lyxal the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make lyxal the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to lyxal. Only supported for zsh and bash."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        lyxal term run list files in this directory\n  \
                        @lyxal list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to lyxal (multiple words allowed without quotes)
...
async fn handle_interactive_session(
    ...
    tracing::info!(
        counter.lyxal.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
...
async fn log_session_completion(
    ...
    tracing::info!(
        counter.lyxal.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        counter.lyxal.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            counter.lyxal.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use lyxal run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    lyxal::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(lyxal_mcp::BUILTIN_EXTENSIONS.clone());
    ...
    match cli.command {
        ...
        Some(Command::Acp { builtins }) => lyxal_acp::server::run(builtins).await,
