...
use lyxal::tracing::{langfuse_layer, otlp_layer};
use lyxal_bench::bench_session::BenchAgentError;
use lyxal_bench::error_capture::ErrorCaptureLayer;
...
fn setup_logging_internal(
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = lyxal::logging::prepare_log_directory("cli", true)?;
            ...
            let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                ...
                EnvFilter::new("")
                    ...
                    // Set lyxal module to DEBUG
                    .add_directive("lyxal=debug".parse().unwrap())
                    // Set lyxal-cli to INFO
                    .add_directive("lyxal_cli=info".parse().unwrap())
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = lyxal::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        // Verify directory structure
        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "lyxal"));
