...
{
    if let Ok(only_provider) = std::env::var("LYXAL_TEST_PROVIDER") {
...
{
    use lyxal::config::ExtensionConfig;
    use tokio::sync::Mutex;

    lyxal::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let agent_config = AgentConfig::new(session_manager, permission_manager, None, LyxalMode::Auto);
    ...
    agent
        .update_provider(
            provider_arc as Arc<dyn lyxal::providers::base::Provider>,
            &session.id,
        )
        .await?;
