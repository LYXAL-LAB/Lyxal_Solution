...
use lyxal::recipe::{Recipe, SubRecipe};
...
mod tests {
    ...
    fn test_extract_recipe_info_from_cli_basic() {
        ...
        assert_eq!(settings.lyxal_provider, Some("test_provider".to_string()));
        assert_eq!(settings.lyxal_model, Some("test_model".to_string()));
...
settings:
  lyxal_provider: test_provider
  lyxal_model: test_model
  temperature: 0.7
sub_recipes:
