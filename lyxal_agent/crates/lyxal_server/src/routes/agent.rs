...
use lyxal::agents::{Container, ExtensionLoadResult};
use lyxal::lyxal_apps::{fetch_mcp_apps, LyxalApp, McpAppCache};
...
use lyxal::prompt_template::render_template;
use lyxal::providers::create;
use lyxal::recipe::Recipe;
use lyxal::recipe_deeplink;
use lyxal::session::extension_data::ExtensionState;
use lyxal::session::session_manager::SessionType;
use lyxal::session::{EnabledExtensionsState, Session};
use lyxal::{
...
async fn start_agent(
    ...
    lyxal::posthog::set_session_context("desktop", false);
    ...
    let original_recipe = if let Some(deeplink) = recipe_deeplink {
        match recipe_deeplink::decode(&deeplink) {
            ...
            Err(err) => {
                ...
                lyxal::posthog::emit_error("recipe_deeplink_decode_failed", &err.to_string());
    ...
    let mut session = manager
        ...
        .map_err(|err| {
            ...
            lyxal::posthog::emit_error("session_create_failed", &err.to_string());
            ErrorResponse {
...
async fn resume_agent(
    ...
    lyxal::posthog::set_session_context("desktop", true);
    ...
    let session = state
        ...
        .map_err(|err| {
            ...
            lyxal::posthog::emit_error("session_resume_failed", &err.to_string());
            ErrorResponse {
...
async fn get_tools(
    ...
    let lyxal_mode = config.get_lyxal_mode().unwrap_or(LyxalMode::Auto);
    ...
    let mut tools: Vec<ToolInfo> = agent
        ...
        .map(|tool| {
            let permission = permission_manager
                ...
                .or_else(|| {
                    if lyxal_mode == LyxalMode::SmartApprove {
                        permission_manager.get_smart_approve_permission(&tool.name)
                    } else if lyxal_mode == LyxalMode::Approve {
...
async fn update_agent_provider(
    ...
    let config = Config::global();
    let model = match payload.model.or_else(|| config.get_lyxal_model().ok()) {
        Some(m) => m,
        None => {
...
async fn import_app(
    ...
    let mut app = LyxalApp::from_html(&body.html).map_err(|e| ErrorResponse {
        message: format!("Invalid Lyxal App HTML: {}", e),
        status: StatusCode::BAD_REQUEST,
    })?;
