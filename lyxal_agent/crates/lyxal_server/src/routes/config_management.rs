...
use lyxal::config::declarative_providers::LoadedProvider;
use lyxal::config::paths::Paths;
use lyxal::config::ExtensionEntry;
use lyxal::config::{Config, ConfigError};
use lyxal::model::ModelConfig;
use lyxal::providers::auto_detect::detect_provider_from_api_key;
use lyxal::providers::base::{ProviderMetadata, ProviderType};
use lyxal::providers::canonical::maybe_get_canonical_model;
use lyxal::providers::create_with_default_model;
use lyxal::providers::providers as get_providers;
use lyxal::{
...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, ErrorResponse> {
    let extensions = lyxal::config::get_all_extensions();
    let warnings = lyxal::config::get_warnings();
...
pub async fn add_extension(
    ...
    let extensions = lyxal::config::get_all_extensions();
    let key = lyxal::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    lyxal::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, ErrorResponse> {
    let key = lyxal::config::extensions::name_to_key(&name);
    lyxal::config::remove_extension(&key);
...
pub async fn get_provider_models(
    ...
    let loaded_provider = lyxal::config::declarative_providers::load_provider(name.as_str()).ok();
    ...
    let provider = lyxal::providers::create(&name, model_config).await?;
...
pub async fn init_config() -> Result<Json<String>, ErrorResponse> {
    ...
    match lyxal::config::base::load_init_config_from_workspace() {
        Ok(init_values) => {
...
pub async fn upsert_permissions(
    ...
    let permission_manager = lyxal::config::PermissionManager::instance();
...
pub async fn create_custom_provider(
    ...
    let config = lyxal::config::declarative_providers::create_custom_provider(
        lyxal::config::declarative_providers::CreateCustomProviderParams {
    ...
    lyxal::providers::refresh_custom_providers().await?;
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = lyxal::config::declarative_providers::load_provider(id.as_str())
        .map_err(|e| {
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, ErrorResponse> {
    lyxal::config::declarative_providers::remove_custom_provider(&id)?;
...
pub async fn update_custom_provider(
    ...
    lyxal::config::declarative_providers::update_custom_provider(
        lyxal::config::declarative_providers::UpdateCustomProviderParams {
            id: id.clone(),
            engine: request.engine,
            display_name: request.display_name,
            api_url: request.api_url,
...
pub async fn set_config_provider(
    ...
    create_with_default_model(&provider)
        ...
        .and_then(|_| {
            let config = Config::global();
            config
                .set_lyxal_provider(provider.clone())
                .and_then(|_| config.set_lyxal_model(model.clone()))
...
pub async fn configure_provider_oauth(
    ...
    let config = lyxal::config::Config::global();
...
mod tests {
    ...
    async fn test_read_model_limits() {
        ...
        let limits: Vec<lyxal::model::ModelLimitConfig> = serde_json::from_value(response).unwrap();
