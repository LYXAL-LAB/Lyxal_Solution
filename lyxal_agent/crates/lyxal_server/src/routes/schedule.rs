...
use lyxal::scheduler::{get_default_scheduled_recipes_dir, ScheduledJob};
...
async fn create_schedule(
    ...
    scheduler
        ...
        .await
        .map_err(|e| match e {
            lyxal::scheduler::SchedulerError::CronParseError(msg) => {
                ErrorResponse::bad_request(format!("Invalid cron expression: {}", msg))
            }
            lyxal::scheduler::SchedulerError::RecipeLoadError(msg) => {
                ErrorResponse::bad_request(format!("Recipe load error: {}", msg))
            }
            lyxal::scheduler::SchedulerError::JobIdExists(msg) => ErrorResponse {
...
async fn delete_schedule(
    ...
    scheduler
        ...
        .await
        .map_err(|e| match e {
            lyxal::scheduler::SchedulerError::JobNotFound(msg) => {
...
async fn run_now_handler(
    ...
    match scheduler.run_now(&id).await {
        ...
        Err(e) => match e {
            lyxal::scheduler::SchedulerError::JobNotFound(msg) => Err(ErrorResponse::not_found(
                format!("Schedule not found: {}", msg),
            )),
            lyxal::scheduler::SchedulerError::AnyhowError(ref err) => {
...
async fn pause_schedule(
    ...
    scheduler.pause_schedule(&id).await.map_err(|e| match e {
        ...
        lyxal::scheduler::SchedulerError::AnyhowError(err) => {
