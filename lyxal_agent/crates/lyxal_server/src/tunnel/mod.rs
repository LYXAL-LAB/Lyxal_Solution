...
use lyxal::config::{paths::Paths, Config};
...
fn try_acquire_tunnel_lock() -> anyhow::Result<File> {
    ...
    file.try_lock_exclusive()
        .map_err(|_| anyhow::anyhow!("Another lyxal instance is already running the tunnel"))?;
...
impl TunnelManager {
    ...
    pub async fn check_auto_start(&self) {
        ...
        if auto_start && state == TunnelState::Idle {
            if is_tunnel_locked_by_another() {
                tracing::info!(
                    "Tunnel already running on another lyxal instance, skipping auto-start"
                );
                return;
            }
    ...
    fn is_tunnel_disabled() -> bool {
        if let Ok(val) = std::env::var("LYXAL_TUNNEL") {
    ...
    async fn start_tunnel_internal(&self) -> anyhow::Result<(TunnelInfo, mpsc::Receiver<()>)> {
        ...
        let server_secret =
            std::env::var("LYXAL_SERVER__SECRET_KEY").unwrap_or_else(|_| "test".to_string());
    ...
    pub async fn start(&self) -> anyhow::Result<TunnelInfo> {
        if Self::is_tunnel_disabled() {
            anyhow::bail!("Tunnel is disabled via LYXAL_TUNNEL environment variable");
        }
