...
pub const LYXAL_HINTS_FILENAME: &str = ".lyxalhints";
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global lyxal hints.\n");
...
mod tests {
    ...
    #[test]
    fn test_lyxalhints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(dir.path().join(LYXAL_HINTS_FILENAME), "Test hint content").unwrap();
        let gitignore = create_dummy_gitignore();
        let hints = load_hint_files(dir.path(), &[LYXAL_HINTS_FILENAME.to_string()], &gitignore);

        assert!(hints.contains("Test hint content"));
    }

    #[test]
    fn test_lyxalhints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_lyxalhints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        .unwrap();
        fs::write(
            dir.path().join(LYXAL_HINTS_FILENAME),
            "Custom hints file content from .lyxalhints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), LYXAL_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .lyxalhints"));
    }

    #[test]
    fn test_lyxalhints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".lyxalhints")); // Make sure it's not loading the default
    }

    #[test]
    fn test_nested_lyxalhints_with_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();

        fs::create_dir(project_root.join(".git")).unwrap();
        fs::write(
            project_root.join(LYXAL_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(LYXAL_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(LYXAL_HINTS_FILENAME),
        ...
        let hints = load_hint_files(
            &current_dir,
            &[LYXAL_HINTS_FILENAME.to_string()],
            &gitignore,
        );
    ...
    #[test]
    fn test_nested_lyxalhints_without_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let base_dir = temp_dir.path();

        fs::write(base_dir.join(LYXAL_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    #[test]
    fn test_nested_lyxalhints_mixed_filenames() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(LYXAL_HINTS_FILENAME),
            "Subdir .lyxalhints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .lyxalhints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(LYXAL_HINTS_FILENAME), hints_content).unwrap();
...
Root level instructions"#;
        fs::write(project_root.join(LYXAL_HINTS_FILENAME), root_hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(LYXAL_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(LYXAL_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(LYXAL_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[LYXAL_HINTS_FILENAME.to_string()], &gitignore);
