...
fn map_provider_name(provider: &str) -> &str {
    match provider {
        // Lyxal provider names that differ from models.dev names
        "xai" => "x-ai",
        "azure_openai" => "azure",
...
mod tests {
    ...
    fn test_map_to_canonical_model() {
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "lyxal-claude-4-sonnet-bedrock", r),
            Some("anthropic/claude-sonnet-4".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "klyxal-gpt-4o", r),
            Some("openai/gpt-4o".to_string())
        );
        assert_eq!(
        ...
        // === OpenAI O-series ===
        assert_eq!(
            map_to_canonical_model("databricks", "lyxal-o1", r),
            Some("openai/o1".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "klyxal-o3", r),
            Some("openai/o3".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "headless-lyxal-o3-mini", r),
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "klyxal-grok-4-fast", r),
            Some("x-ai/grok-4-fast".to_string())
        );
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "lyxal-command-r-08-2024", r),
