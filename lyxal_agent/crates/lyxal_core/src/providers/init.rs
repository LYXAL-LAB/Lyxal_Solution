...
pub async fn create(name: &str, model: ModelConfig) -> Result<Arc<dyn Provider>> {
    ...
    if let Ok(lead_model_name) = config.get_param::<String>("LYXAL_LEAD_MODEL") {
...
fn create_worker_model_config(default_model: &ModelConfig) -> Result<ModelConfig> {
    ...
    if let Ok(limit) = global_config.get_param::<usize>("LYXAL_WORKER_CONTEXT_LIMIT") {
        worker_config = worker_config.with_context_limit(Some(limit));
    } else if let Ok(limit) = global_config.get_param::<usize>("LYXAL_CONTEXT_LIMIT") {
        worker_config = worker_config.with_context_limit(Some(limit));
    }
...
mod tests {
    ...
    async fn test_create_lead_worker_provider(
        ...
        let _guard = env_lock::lock_env([
            ("LYXAL_LEAD_MODEL", Some("gpt-4o")),
            ("LYXAL_LEAD_PROVIDER", None),
            ("LYXAL_LEAD_TURNS", lead_turns),
            ("LYXAL_LEAD_FAILURE_THRESHOLD", failure_threshold),
            ("LYXAL_LEAD_FALLBACK_TURNS", fallback_turns),
    ...
    async fn test_create_regular_provider_without_lead_config() {
        let _guard = env_lock::lock_env([
            ("LYXAL_LEAD_MODEL", None),
            ("LYXAL_LEAD_PROVIDER", None),
            ("LYXAL_LEAD_TURNS", None),
            ("LYXAL_LEAD_FAILURE_THRESHOLD", None),
            ("LYXAL_LEAD_FALLBACK_TURNS", None),
            ("OPENAI_API_KEY", Some("fake-openai-no-keyring")),
            ("OPENAI_CUSTOM_HEADERS", Some("")),
        ]);
    ...
    fn test_worker_model_context_limit(
        ...
        let _guard = env_lock::lock_env([
            ("LYXAL_WORKER_CONTEXT_LIMIT", worker_limit),
            ("LYXAL_CONTEXT_LIMIT", global_limit),
        ]);
