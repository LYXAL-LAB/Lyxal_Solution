...
pub fn create_request(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since lyxal uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }

    let model_name = model_config.model_name.to_string();
    let is_o1 = model_name.starts_with("o1") || model_name.starts_with("lyxal-o1");
    let is_o3 = model_name.starts_with("o3") || model_name.starts_with("lyxal-o3");
    let is_gpt_5 = model_name.starts_with("gpt-5") || model_name.starts_with("lyxal-gpt-5");
    let is_openai_reasoning_model = is_o1 || is_o3 || is_gpt_5;
    let is_claude_sonnet =
        model_name.contains("claude-3-7-sonnet") || model_name.contains("claude-4-sonnet"); // can be lyxal- or databricks-
...
mod tests {
    ...
    fn test_is_claude_model() {
        ...
        assert!(is_claude_model("lyxal-claude-sonnet"));
