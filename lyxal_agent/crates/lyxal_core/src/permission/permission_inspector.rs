...
use crate::config::{LyxalMode, PermissionManager};
    ...
    async fn inspect(
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match lyxal_mode {
                    LyxalMode::Chat => continue,
                    LyxalMode::Auto => InspectionAction::Allow,
                    LyxalMode::Approve | LyxalMode::SmartApprove => {
