...
fn get_predefined_models() -> Vec<PredefinedModel> {
    static PREDEFINED_MODELS: Lazy<Vec<PredefinedModel>> =
        Lazy::new(|| match std::env::var("LYXAL_PREDEFINED_MODELS") {
            Ok(json_str) => serde_json::from_str(&json_str).unwrap_or_else(|e| {
                tracing::warn!("Failed to parse LYXAL_PREDEFINED_MODELS: {}", e);
                Vec::new()
            }),
            Err(_) => Vec::new(),
        });
...
impl ModelConfig {
    ...
    pub fn new_with_context_env(
        ...
        let context_limit = if let Some(ref pm) = predefined {
            ...
            } else if let Ok(val) = std::env::var("LYXAL_CONTEXT_LIMIT") {
                Some(Self::validate_context_limit(&val, "LYXAL_CONTEXT_LIMIT")?)
            } else {
                pm.context_limit
            }
        } else {
    ...
    fn parse_context_limit(
        ...
        if let Ok(val) = std::env::var("LYXAL_CONTEXT_LIMIT") {
            return Self::validate_context_limit(&val, "LYXAL_CONTEXT_LIMIT").map(Some);
        }
    ...
    fn parse_temperature() -> Result<Option<f32>, ConfigError> {
        if let Ok(val) = std::env::var("LYXAL_TEMPERATURE") {
            let temp = val.parse::<f32>().map_err(|_| {
                ConfigError::InvalidValue(
                    "LYXAL_TEMPERATURE".to_string(),
                    val.clone(),
    ...
    fn parse_max_tokens() -> Result<Option<i32>, ConfigError> {
        match crate::config::Config::global().get_param::<i32>("LYXAL_MAX_TOKENS") {
            Ok(tokens) => {
                if tokens <= 0 {
                    return Err(ConfigError::InvalidRange(
                        "lyxal_max_tokens".to_string(),
    ...
    fn parse_toolshim() -> Result<bool, ConfigError> {
        if let Ok(val) = std::env::var("LYXAL_TOOLSHIM") {
            match val.to_lowercase().as_str() {
                "1" | "true" | "yes" | "on" => Ok(true),
                "0" | "false" | "no" | "off" => Ok(false),
                _ => Err(ConfigError::InvalidValue(
                    "LYXAL_TOOLSHIM".to_string(),
                    val,
    ...
    fn parse_toolshim_model() -> Result<Option<String>, ConfigError> {
        match std::env::var("LYXAL_TOOLSHIM_OLLAMA_MODEL") {
            Ok(val) if val.trim().is_empty() => Err(ConfigError::InvalidValue(
                "LYXAL_TOOLSHIM_OLLAMA_MODEL".to_string(),
                val,
...
mod tests {
    ...
    fn test_parse_max_tokens_valid() {
        let _guard = env_lock::lock_env([("LYXAL_MAX_TOKENS", Some("4096"))]);
    ...
    fn test_parse_max_tokens_not_set() {
        let _guard = env_lock::lock_env([("LYXAL_MAX_TOKENS", None::<&str>)]);
        let result = ModelConfig::parse_max_tokens().unwrap();
    ...
    fn test_parse_max_tokens_invalid_string() {
        let _guard = env_lock::lock_env([("LYXAL_MAX_TOKENS", Some("not_a_number"))]);
    ...
    fn test_parse_max_tokens_zero() {
        let _guard = env_lock::lock_env([("LYXAL_MAX_TOKENS", Some("0"))]);
        let result = ModelConfig::parse_max_tokens();
        assert!(result.is_err());
    ...
    fn test_parse_max_tokens_negative() {
        let _guard = env_lock::lock_env([("LYXAL_MAX_TOKENS", Some("-100"))]);
        let result = ModelConfig::parse_max_tokens();
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ConfigError::InvalidRange(..)));
    }
    ...
    fn test_model_config_with_max_tokens_env() {
        let _guard = env_lock::lock_env([
            ("LYXAL_MAX_TOKENS", Some("8192")),
            ("LYXAL_TEMPERATURE", None::<&str>),
            ("LYXAL_CONTEXT_LIMIT", None::<&str>),
            ("LYXAL_TOOLSHIM", None::<&str>),
            ("LYXAL_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);
