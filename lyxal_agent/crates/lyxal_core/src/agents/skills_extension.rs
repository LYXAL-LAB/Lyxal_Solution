...
impl SkillsClient {
    ...
    fn get_default_skill_directories() -> Vec<PathBuf> {
        ...
        if let Ok(working_dir) = std::env::current_dir() {
            ...
            dirs.push(working_dir.join(".lyxal/skills"));
...
"#,
        ...
        // Simulate ~/.config/lyxal/skills (global, medium priority)
        let global_lyxal = temp_dir.path().join("global-lyxal");
        fs::create_dir(&global_lyxal).unwrap();
        let skill_global_lyxal = global_lyxal.join("my-skill");
        fs::create_dir(&skill_global_lyxal).unwrap();
        fs::write(
            skill_global_lyxal.join("SKILL.md"),
            r#"---
name: my-skill
description: From global lyxal config
---
Global lyxal config content
"#,
        )
        .unwrap();
...
"#,
        ...
        // Simulate $PWD/.lyxal/skills (working dir, highest priority)
        let working_lyxal = temp_dir.path().join("working-lyxal");
        fs::create_dir(&working_lyxal).unwrap();
        let skill_working_lyxal = working_lyxal.join("my-skill");
        fs::create_dir(&skill_working_lyxal).unwrap();
        fs::write(
            skill_working_lyxal.join("SKILL.md"),
            r#"---
name: my-skill
description: From working dir lyxal
---
Working dir lyxal content
"#,
        )
        .unwrap();

        // Test priority order: global_claude < global_lyxal < working_claude < working_lyxal
        ...
        // The last directory (working_lyxal) should win
        assert_eq!(
            skills.get("my-skill").unwrap().metadata.description,
            "From working dir lyxal"
        );
        assert!(skills
            .get("my-skill")
            .unwrap()
            .body
            .contains("Working dir lyxal content"));
    }
    ...
    fn test_builtin_skills_loaded() {
        ...
        assert!(skills.contains_key("lyxal-doc-guide"));
    }
}
