...
/// Manages lyxal extensions / MCP clients and their interactions
...
async fn child_process_client(
    ...
    // Use explicitly passed working_dir, falling back to LYXAL_WORKING_DIR env var
    let effective_working_dir = working_dir
        .map(|p| p.to_path_buf())
        .or_else(|| std::env::var("LYXAL_WORKING_DIR").ok().map(PathBuf::from));

    if let Some(ref dir) = effective_working_dir {
        if dir.exists() && dir.is_dir() {
            tracing::info!("Setting MCP process working directory: {:?}", dir);
            command.current_dir(dir);
            // Also set LYXAL_WORKING_DIR env var for the child process
            command.env("LYXAL_WORKING_DIR", dir);
        } else {
            tracing::warn!(
