...
/// MCP client implementation for Lyxal
use rmcp::{
    model::{
...
impl ClientHandler for LyxalClient {
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called lyxal");
        ...
        Ok(CreateMessageResult {
            ...
            message: SamplingMessage {
                ...
                // This doesn't mesh well with lyxal's approach which has Vec<MessageContent>
                ...
                content: if let Some(content) = response.content.first() {
                    match content {
                        ...
                        // TODO(alexhancock) - Content::Audio? lyxal's messages don't currently have it
                        _ => Content::text(""),
                    }
                } else {
                    Content::text("")
    ...
    fn get_info(&self) -> ClientInfo {
        ClientInfo {
            ...
            client_info: Implementation {
                name: "lyxal".to_string(),
                version: std::env::var("LYXAL_MCP_CLIENT_VERSION")
...
pub struct McpClient {
    client: Mutex<RunningService<RoleClient, LyxalClient>>,
...
impl McpClient {
        ...
        E: std::error::Error + From<std::io::Error> + Send + Sync + 'static,
    {
        ...
        let client = LyxalClient::new(notification_subscribers.clone(), provider);
        let client: rmcp::service::RunningService<rmcp::RoleClient, LyxalClient> =
...
mod tests {
    ...
    fn new_client() -> LyxalClient {
        LyxalClient::new(Arc::new(Mutex::new(Vec::new())), Arc::new(Mutex::new(None)))
    }
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<Meta>(json!({
                "Lyxal-Session-Id": "old-session-id",
