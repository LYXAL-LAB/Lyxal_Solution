...
pub fn manage_schedule_tool() -> Tool {
    Tool::new(
        ...
        indoc! {r#"
            Manage lyxal's internal scheduled recipe execution.

            Actions:
            - "list": List all lyxal scheduled jobs
            - "create": Create a new lyxal scheduled job from a recipe file
            - "run_now": Execute a lyxal scheduled job immediately
            - "pause": Pause a lyxal scheduled job
            - "unpause": Resume a paused lyxal scheduled job
            - "delete": Remove a lyxal scheduled job
            - "kill": Terminate a currently running lyxal scheduled job
            - "inspect": Get details about a running lyxal scheduled job
            - "sessions": List execution history for a lyxal scheduled job
