...
use crate::config::{get_enabled_extensions, Config, LyxalMode};
...
const COMPACTION_THINKING_TEXT: &str = "lyxal is compacting the conversation...";
...
impl Agent {
    pub fn new() -> Self {
        Self::with_config(AgentConfig::new(
            ...
            None,
            Config::global().get_lyxal_mode().unwrap_or(LyxalMode::Auto),
        ))
    }
    ...
    async fn prepare_reply_context(
        ...
        Ok(ReplyContext {
            ...
            toolshim_tools,
            system_prompt,
            lyxal_mode: self.config.lyxal_mode,
            tool_call_cut_off: Config::global()
                .get_param::<usize>("LYXAL_TOOL_CALL_CUTOFF")
                .unwrap_or(10),
            initial_messages,
        })
    }
    ...
    pub async fn subagents_enabled(&self, session_id: &str) -> bool {
        if self.config.lyxal_mode != LyxalMode::Auto {
            return false;
        }
        ...
        Ok(Box::pin(async_stream::try_stream! {
            let final_conversation = if !needs_auto_compact {
                ...
                let threshold = config
                    .get_param::<f64>("LYXAL_AUTO_COMPACT_THRESHOLD")
    ...
    pub async fn create_recipe(
        ...
        let settings = Settings {
            lyxal_provider: Some(provider_name.clone()),
            lyxal_model: Some(model_name.clone()),
