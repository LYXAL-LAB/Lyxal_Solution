...
use crate::lyxal_apps::McpAppResource;
use crate::lyxal_apps::{LyxalApp, WindowProps};
...
impl AppsManagerClient {
    ...
    fn load_app(&self, name: &str) -> Result<LyxalApp, String> {
        let path = self.apps_dir.join(format!("{}.html", name));

        let html =
            fs::read_to_string(&path).map_err(|e| format!("Failed to read app file: {}", e))?;

        LyxalApp::from_html(&html)
    }

    fn save_app(&self, app: &LyxalApp) -> Result<(), String> {
    ...
    fn create_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate content for a new Lyxal app. Returns the HTML code, app name, description, and window properties.".to_string(),
    ...
    fn update_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate updated content for an existing Lyxal app. Returns the improved HTML code, updated description, and optionally updated window properties.".to_string(),
...
impl McpClientTrait for AppsManagerClient {
    async fn list_tools(
        ...
        let tools = vec![
            McpTool::new(
                "list_apps".to_string(),
                "List all available Lyxal apps with their names and descriptions. Use this to see what apps exist before creating or modifying apps.".to_string(),
                schema::<ListAppsParams>(),
            ),
            McpTool::new(
                "create_app".to_string(),
                "Create a new Lyxal app based on a description or PRD. The extension will use an LLM to generate the HTML/CSS/JavaScript. Apps are sandboxed and run in standalone windows.".to_string(),
