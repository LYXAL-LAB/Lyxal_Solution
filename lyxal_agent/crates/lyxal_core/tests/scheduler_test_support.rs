...
use lyxal::providers::base::Provider as LyxalProvider;

static TEST_PROVIDER: Lazy<Mutex<Option<Arc<dyn LyxalProvider>>>> = Lazy::new(|| Mutex::new(None));
...
pub async fn set_test_provider(p: Arc<dyn LyxalProvider>) {
    let mut guard = TEST_PROVIDER.lock().await;
    *guard = Some(p);
}

pub async fn get_test_provider() -> Option<Arc<dyn LyxalProvider>> {
