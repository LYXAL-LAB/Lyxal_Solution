...
use lyxal::conversation::message::{Message, ToolRequest};
use lyxal::tool_inspection::{
...
async fn test_inspect_tools_aggregates_and_handles_errors() {
    ...
    // Act
    let results = manager
        .inspect_tools(&tool_requests, &messages, LyxalMode::Approve)
        .await
