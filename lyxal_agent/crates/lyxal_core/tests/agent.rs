...
use lyxal::agents::{Agent, AgentEvent};
use lyxal::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use lyxal::agents::platform_tools::PLATFORM_MANAGE_SCHEDULE_TOOL_NAME;
        use lyxal::agents::AgentConfig;
        use lyxal::config::permission::PermissionManager;
        use lyxal::config::LyxalMode;
        use lyxal::scheduler::{ScheduledJob, SchedulerError};
        use lyxal::scheduler_trait::SchedulerTrait;
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage lyxal's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use lyxal::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use lyxal::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use lyxal::agents::SessionConfig;
        use lyxal::conversation::message::{Message, MessageContent};
        use lyxal::model::ModelConfig;
        use lyxal::providers::base::{
            Provider, ProviderDef, ProviderMetadata, ProviderUsage, Usage,
        };
        use lyxal::providers::errors::ProviderError;
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let lyxal::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent.handle_confirmation(
                                    id.clone(),
                                    lyxal::permission::PermissionConfirmation {
                                        principal_type: lyxal::permission::permission_confirmation::PrincipalType::Tool,
                                        permission: lyxal::permission::Permission::AllowOnce,
                                    }
                                ).await;
                            }
                        }
    ...
    mod extension_manager_tests {
        use super::*;
        use lyxal::agents::extension::ExtensionConfig;
        use lyxal::agents::extension_manager_extension::{
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for lyxal so it can keep track of what it is doing"
                            .to_string(),
