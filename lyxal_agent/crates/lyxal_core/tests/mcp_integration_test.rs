...
use lyxal::agents::extension_manager::ExtensionManager;
...
use lyxal::providers::base::{Provider, ProviderDef, ProviderMetadata, ProviderUsage, Usage};
...
async fn test_replayed_session(
    ...
    std::env::set_var("LYXAL_MCP_CLIENT_VERSION", "0.0.0");

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/lyxal_test/lyxal.txt";
    if let Some(parent) = std::path::Path::new(test_file_path).parent() {
        fs::create_dir_all(parent).ok();
    }
    fs::write(test_file_path, "# lyxal\n").ok();
    ...
    let mode = if env::var("LYXAL_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let session_manager = Arc::new(lyxal::session::SessionManager::new(
