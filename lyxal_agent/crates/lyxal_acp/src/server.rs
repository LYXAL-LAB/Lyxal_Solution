use anyhow::Result;
use fs_err as fs;
use lyxal::agents::extension::{Envs, PLATFORM_EXTENSIONS};
use lyxal::agents::{Agent, AgentConfig, ExtensionConfig, SessionConfig};
use lyxal::builtin_extension::register_builtin_extensions;
use lyxal::config::base::CONFIG_YAML_NAME;
use lyxal::config::extensions::get_enabled_extensions_with_config;
use lyxal::config::paths::Paths;
use lyxal::config::permission::PermissionManager;
...
struct LyxalAcpSession {
    messages: Conversation,
    tool_requests: HashMap<String, lyxal::conversation::message::ToolRequest>,
    cancel_token: Option<CancellationToken>,
}

pub struct LyxalAcpAgent {
    sessions: Arc<Mutex<HashMap<String, LyxalAcpSession>>>,
    agent: Arc<Agent>,
    provider: Arc<dyn lyxal::providers::base::Provider>,
}

pub struct AcpServerConfig {
    pub provider: Arc<dyn lyxal::providers::base::Provider>,
    pub builtins: Vec<String>,
    pub data_dir: std::path::PathBuf,
    pub config_dir: std::path::PathBuf,
    pub lyxal_mode: lyxal::config::LyxalMode,
}
...
fn extract_tool_locations(
    tool_request: &lyxal::conversation::message::ToolRequest,
    tool_response: &lyxal::conversation::message::ToolResponse,
...
impl LyxalAcpAgent {
    ...
    pub async fn new(builtins: Vec<String>) -> Result<Self> {
        ...
        let model_config = lyxal::model::ModelConfig {
        ...
        let lyxal_mode = config
            .get_lyxal_mode()
            .unwrap_or(lyxal::config::LyxalMode::Auto);
    ...
    pub async fn with_config(config: AcpServerConfig) -> Result<Self> {
        ...
        let config_file = Config::new(&config_path, "lyxal")?;
    ...
    pub async fn create_session(&self) -> Result<String> {
        ...
        self.agent
            .update_provider(self.provider.clone(), &lyxal_session.id)
            .await?;

        let session = LyxalAcpSession {
        ...
        sessions.insert(lyxal_session.id.clone(), session);

        info!(
            session_id = %lyxal_session.id,
            session_type = "acp",
            "Session created"
        );
...
impl LyxalAcpAgent {
    ...
    async fn on_new_session(
        ...
        self.update_session_with_provider(&lyxal_session).await?;
        ...
        for mcp_server in args.mcp_servers {
            ...
            if let Err(e) = self.agent.add_extension(config, &lyxal_session.id).await {
        ...
        Ok(NewSessionResponse::new(SessionId::new(lyxal_session.id)))
    }

    async fn update_session_with_provider(
        &self,
        lyxal_session: &Session,
    ) -> Result<(), sacp::Error> {
        self.agent
            .update_provider(self.provider.clone(), &lyxal_session.id)
            .await
    ...
    async fn on_load_session(
        ...
        let manager = self.agent.config.session_manager.clone();
        let lyxal_session = manager.get_session(&session_id, true).await.map_err(|e| {
            sacp::Error::invalid_params()
                .data(format!("Failed to load session {}: {}", session_id, e))
        })?;
        self.update_session_with_provider(&lyxal_session).await?;

        let conversation = lyxal_session.conversation.ok_or_else(|| {
        ...
        let mut session = LyxalAcpSession {
    ...
    async fn on_prompt(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(lyxal::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions.get_mut(&session_id).ok_or_else(|| {
                        sacp::Error::invalid_params()
...
pub struct LyxalAcpHandler {
    pub agent: Arc<LyxalAcpAgent>,
}

impl JrMessageHandler for LyxalAcpHandler {
...
pub async fn serve<R, W>(agent: Arc<LyxalAcpAgent>, read: R, write: W) -> Result<()>
where
    R: futures::AsyncRead + Unpin + Send + 'static,
    W: futures::AsyncWrite + Unpin + Send + 'static,
{
    let handler = LyxalAcpHandler { agent };
...
pub async fn run(builtins: Vec<String>) -> Result<()> {
    ...
    let agent = Arc::new(LyxalAcpAgent::new(builtins).await?);
