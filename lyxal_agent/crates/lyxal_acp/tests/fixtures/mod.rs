...
use lyxal::config::{LyxalMode, PermissionManager};
use lyxal::model::ModelConfig;
use lyxal::providers::api_client::{ApiClient, AuthMethod};
use lyxal::providers::openai::OpenAiProvider;
use lyxal::session_context::SESSION_ID_HEADER;
use lyxal_acp::server::{serve, AcpServerConfig, LyxalAcpAgent};
...
pub async fn spawn_acp_server_in_process(
    ...
    let agent = Arc::new(LyxalAcpAgent::with_config(config).await.unwrap());
