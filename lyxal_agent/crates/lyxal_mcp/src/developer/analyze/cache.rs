...
impl AnalysisCache {
    ...
    pub fn get(
        ...
        let mut cache = lock_or_recover(&self.cache, |c: &mut LruCache<CacheKey, Arc<AnalysisResult>>| c.clear());
        let key = CacheKey {
            path: path.clone(),
            modified,
            mode: *mode,
    ...
    pub fn len(&self) -> usize {
        let cache = lock_or_recover(&self.cache, |c: &mut LruCache<CacheKey, Arc<AnalysisResult>>| c.clear());
        cache.len()
    }
