...
const DEFAULT_LYXALIGNORE_CONTENT: &str = concat!(
    "# This file is created automatically if no .lyxalignore exists.\n",
    "# Customize or uncomment the patterns below instead of deleting the file.\n",
    "# Removing it will simply cause lyxal to recreate it on the next start.\n",
    "#\n",
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        ServerInfo {
            server_info: Implementation {
                name: "lyxal-developer".to_string(),
...
impl DeveloperServer {
    ...
    pub async fn text_editor(
        ...
        if self.is_ignored(&path) {
            return Err(ErrorData::new(
                ErrorCode::INTERNAL_ERROR,
                format!(
                    "Access to '{}' is restricted by .lyxalignore",
                    path.display()
                ),
                None,
            ));
        ...
        for arg in &cmd_parts[1..] {
            ...
            if self.is_ignored(path) {
                return Err(ErrorData::new(
                    ErrorCode::INTERNAL_ERROR,
                    format!(
                        "The command attempts to access '{}' which is restricted by .lyxalignore",
                        arg
                    ),
                    None,
                ));
    ...
    fn build_ignore_patterns(cwd: &PathBuf) -> Gitignore {
        ...
        let local_ignore_path = cwd.join(".lyxalignore");

        let global_ignore_path = etcetera::choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.config_dir().join(".lyxalignore"))
            .ok();
        ...
        if !has_local_ignore && !has_global_ignore {
            for pattern in DEFAULT_LYXALIGNORE_CONTENT.lines() {
...
mod tests {
    ...
    #[tokio::test]
    #[serial]
    async fn test_lyxal_ignore_basic_patterns() {
        let temp_dir = tempfile::tempdir().unwrap();
        std::env::set_current_dir(&temp_dir).unwrap();

        // Create .lyxalignore file with patterns
        fs::write(".lyxalignore", "secret.txt\n*.env").unwrap();
    ...
    async fn test_text_editor_respects_ignore_patterns() {
        ...
        // Create .lyxalignore file
        fs::write(".lyxalignore", "secret.txt").unwrap();
    ...
    async fn test_text_editor_view_empty_directory() {
        ...
        let lyxalignore_path = temp_path.join(".lyxalignore");
        if lyxalignore_path.exists() {
            fs::remove_file(&lyxalignore_path).unwrap();
        }
    ...
    async fn test_default_patterns_when_no_ignore_files() {
        ...
        // Verify that .lyxalignore is NOT created on disk (patterns applied in memory only)
        let lyxalignore_path = temp_dir.path().join(".lyxalignore");
        assert!(
            !lyxalignore_path.exists(),
            ".lyxalignore should NOT be created on disk"
        );
