...
impl TutorialServer {
    pub fn new() -> Self {
        ...
        let instructions = formatdoc! {r#"
            Because the tutorial extension is enabled, be aware that the user may be new to using lyxal
    ...
    fn get_available_tutorials() -> String {
        ...
        for file in TUTORIALS_DIR.files() {
            ...
            let first_line = file
                .contents_utf8()
                .and_then(|s: &str| s.lines().next().map(|line| line.to_string()))
...
mod tests {
    ...
    async fn test_get_info() {
        ...
        assert_eq!(info.server_info.name, "lyxal-tutorial");
