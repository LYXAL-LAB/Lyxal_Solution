...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             3. Upon agreement:
                ...
                - Confirm the desired storage location:
                  - Local storage (.lyxal/memory) for project-specific details.
                  - Global storage (~/.config/lyxal/memory) for user-wide data.
        ...
        // Check for .lyxal/memory in current directory
        let local_memory_dir = std::env::var("LYXAL_WORKING_DIR")
            .map(PathBuf::from)
        ...
        // - macOS/Linux: ~/.config/lyxal/memory/
        // - Windows:     ~\AppData\Roaming\Block\lyxal\config\memory
        // if it fails, fall back to `.config/lyxal/memory` (relative to the current dir)
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            .map(|strategy| strategy.in_config_dir("memory"))
            .unwrap_or_else(|_| PathBuf::from(".config/lyxal/memory"));
