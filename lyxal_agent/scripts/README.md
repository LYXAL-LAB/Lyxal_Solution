# Lyxal Benchmark Scripts

This directory contains scripts for running and analyzing Lyxal benchmarks.

## run-benchmarks.sh

This script runs Lyxal benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- Lyxal CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `LYXAL_PROVIDER` and `LYXAL_MODEL` environment variables
