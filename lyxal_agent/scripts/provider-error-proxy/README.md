...
A network-level HTTP proxy for simulating provider errors when testing Lyxal's error handling and retry logic.

## Features

- **Interactive error injection**: Manually trigger different error types via stdin commands
- **Network-level interception**: No changes to Lyxal's Rust code required
...
# 2. In another terminal, configure Lyxal to use the proxy
...
# 3. Run Lyxal normally
lyxal session start "tell me a joke"
...
Then run Lyxal normally. The proxy will intercept API requests and you can manually trigger errors as needed, while authentication and metadata requests are forwarded to the real provider.
...
To use with Lyxal, set these environment variables:
  export OPENAI_HOST=http://localhost:8888
  export ANTHROPIC_HOST=http://localhost:8888
  ...
============================================================
...
2. Configure Lyxal to use the proxy (set environment variables)
3. Run Lyxal in another terminal
4. Use interactive commands to trigger different error types
5. Observe how Lyxal handles each error type
