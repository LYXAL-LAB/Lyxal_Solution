# Building lyxal Desktop on Linux

This guide covers building the lyxal Desktop application from source on various Linux distributions.
...
lyxal is not officially support termux build yet, you need some minor patch to fix build issues.
We will publish lyxal (block-lyxal) into termux-packages.
If you want to try there is a non-official build, https://github.com/shawn111/lyxal/releases/download/termux/lyxal-termux-aarch64.tar.bz2
For more details, see: https://github.com/block/lyxal/pull/3890
...
```bash
cargo build --release -p lyxal-server
```
...
mkdir -p src/bin
cp ../../target/release/lyxald src/bin/
```
...
Output: `out/make/zip/linux/x64/lyxal-linux-x64-{version}.zip`
...
Output: `out/make/deb/x64/lyxal_{version}_amd64.deb`
...
```bash
./out/lyxal-linux-x64/lyxal
```

#### Install DEB Package (if built)
```bash
sudo dpkg -i out/make/deb/x64/lyxal_*.deb
```
...
```bash
#!/bin/bash
cd /path/to/lyxal/ui/desktop/out/lyxal-linux-x64
./lyxal 2>&1 | grep -v "GLib-GObject" | grep -v "browser_main_loop"
```

#### Server Binary Not Found
If you see "Could not find lyxald binary", ensure you've:
1. Built the Rust backend: `cargo build --release -p lyxal-server`
2. Copied it to the right location: `cp ../../target/release/lyxald src/bin/`
...
1. **Backend changes**: Rebuild with `cargo build --release -p lyxal-server` and copy the binary
...
### Desktop Entry
Create `~/.local/share/applications/lyxal.desktop`:
```ini
[Desktop Entry]
Name=lyxal AI Agent
Comment=Local AI agent for development tasks
Exec=/path/to/lyxal/ui/desktop/out/lyxal-linux-x64/lyxal %U
Icon=/path/to/lyxal/ui/desktop/out/lyxal-linux-x64/resources/app.asar.unpacked/src/images/icon.png
Terminal=false
Type=Application
Categories=Development;Utility;
StartupNotify=true
MimeType=x-scheme-handler/lyxal
```

### System-wide Installation
To install system-wide:
```bash
sudo cp -r out/lyxal-linux-x64 /opt/lyxal
sudo ln -s /opt/lyxal/lyxal /usr/local/bin/lyxal-gui
```
