...
lyxal is an AI agent framework in Rust with CLI and Electron desktop interfaces.
...
cargo test -p lyxal          # specific crate
cargo test --package lyxal --test mcp_integration_test
...
crates/
├── lyxal             # core logic
├── lyxal-bench       # benchmarking
├── lyxal-cli         # CLI entry
├── lyxal-server      # backend (binary: lyxald)
├── lyxal-mcp         # MCP extensions
├── lyxal-test        # test utilities
...
Test: Prefer tests/ folder, e.g. crates/lyxal/tests/
Test: When adding features, update lyxal-self-test.yaml, rebuild, then run `lyxal run --recipe lyxal-self-test.yaml` to validate
Error: Use anyhow::Result
Provider: Implement Provider trait see providers/base.rs
MCP: Extensions in crates/lyxal-mcp/
...
## Entry Points
- CLI: crates/lyxal-cli/src/main.rs
- Server: crates/lyxal-server/src/main.rs
- UI: ui/desktop/src/main.ts
- Agent: crates/lyxal/src/agents/agent.rs
