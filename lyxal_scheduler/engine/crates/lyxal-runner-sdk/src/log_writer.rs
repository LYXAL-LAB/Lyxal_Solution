//! Streaming log writer for the Lyxal runner SDK (issue #115).
//!
...
//!
//! - **batch-at-end** (what `lyxal-shell-runner` does): collect output
...
use crate::client::{ClientError, LyxalClient, WorkEvent};
...
/// impl below wraps [`LyxalClient::push_events`]; tests substitute a
...
impl HttpPusher for LyxalClient {
    fn push_events_boxed<'a>(
        &'a self,
        execution_id: &'a str,
        events: &'a [WorkEvent],
    ) -> Pin<Box<dyn Future<Output = Result<(), ClientError>> + Send + 'a>> {
        Box::pin(LyxalClient::push_events(self, execution_id, events))
    }
}
...
impl LogWriter {
    ...
    ///   doesn't care about delivery (e.g. [`lyxal_shell_runner::exec::run`]
    ///   tests assert on exit status / tail buffer instead);
    /// - dev/CLI tools where streaming to a Lyxal server is irrelevant.
    ///
