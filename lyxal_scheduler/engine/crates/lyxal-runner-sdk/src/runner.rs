//! LyxalRunner: the main orchestrator for job execution runners.
...
use crate::client::{AckRequest, LyxalClient, PollRequest, RegisterJobRequest, RenewRequest};
...
/// the run-loop in [`LyxalRunner::start`] calls this on every poll
/// outcome.
///
...
/// Builder for constructing a LyxalRunner.
...
impl RunnerBuilder {
    ...
    pub fn build(self) -> LyxalRunner {
        let mut client = LyxalClient::new(&self.server_url);
...
pub struct LyxalRunner {
    client: Arc<LyxalClient>,
    runner_id: String,
    capabilities: Vec<String>,
    max_inflight: u32,
    tags: Vec<String>,
...
impl LyxalRunner {
    ...
    /// If the job is already managed by the Lyxalfile, the schedule is skipped
    /// (DSL has precedence).
...
mod tests {
    ...
    async fn abort_cancelled_aborts_only_the_matching_handler() {
        ...
        LyxalRunner::abort_cancelled(&aborts, &["other".to_string()]).await;
        assert!(!task.is_finished(), "non-matching cancel must not abort");

        // A cancel for the matching id aborts the handler future.
        LyxalRunner::abort_cancelled(&aborts, &["exec-1".to_string()]).await;
