//! Cron expression → Lyxal DSL converter.
//!
//! Translates standard 5-field cron expressions (`minute hour dom month dow`)
//! into Lyxal's human-readable schedule syntax.
//!
//! ## Supported patterns
//!
//! | Cron expression    | Lyxal equivalent              |
//! |--------------------|--------------------------------|
//! | `* * * * *`        | `every 1 minute`               |
//! | `*/5 * * * *`      | `every 5 minutes`              |
//! | `*/30 * * * *`     | `every 30 minutes`             |
...
//! cannot be expressed in Lyxal's DSL (which is intentional). The converter
//! will emit a best-effort translation with a warning in that case.

use crate::plural::interval_phrase;

/// Conversion result: the Lyxal schedule string plus optional warnings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConversionResult {
    /// The Lyxal schedule expression (e.g. `every 5 minutes`).
    pub schedule: String,
    /// Human-readable warnings about patterns that couldn't be fully expressed.
    pub warnings: Vec<String>,
}

/// Convert a cron expression to a Lyxal schedule string.
///
/// # Errors
///
...
pub fn convert(expr: &str) -> Result<ConversionResult, String> {
            ...
            return Ok(ConversionResult {
                ...
                warnings: vec![
                    "@yearly/@annually maps to 'every 1st of month' — \
                     Lyxal does not have a yearly-only construct; \
    ...
    if let (Some(m), Some(h)) = (fixed_minute, fixed_hour) {
        ...
        // Both dom AND dow are set — ambiguous in Lyxal
        if !is_star(dom) && !is_star(dow) {
            warnings.push(
                "Cron fires when EITHER dom OR dow matches; Lyxal cannot express this \
    ...
    Err(format!(
        "Could not convert '{expr}' to a Lyxal schedule. \
