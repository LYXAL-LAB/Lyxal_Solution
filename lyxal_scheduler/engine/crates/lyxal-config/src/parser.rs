...
/// Recursive-descent parser for Lyxalfile.
pub struct Parser {
    tokens: Vec<Token>,
    pos: usize,
}
...
/// parser, so it accepts exactly the Lyxalfile schedule grammar (same wrapping
/// trick as `calendar_config_from_definition` in lyxal-server). Used to
...
impl Parser {
    /// Parse a Lyxalfile source string into an AST.
    pub fn parse(source: &str) -> Result<Lyxalfile, ParseError> {
    ...
    fn parse_croniqfile(&mut self) -> Result<Lyxalfile, ParseError> {
...
/// The Lyxalfile DSL does not document direction, so we accept any
