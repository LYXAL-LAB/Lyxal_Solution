//! Compiles a Lyxalfile AST into a RuntimeConfig with resolved defaults and placeholders.
...
/// Lyxalfile still compiles — same shape as the previous behaviour, which
...
fn collect_vars(ast: &Lyxalfile) -> HashMap<String, String> {
...
pub struct RuntimeConfig {
    ...
    /// it from `LYXAL_OIDC_CLIENT_SECRET` and merges with this struct.
    ...
    /// `LYXAL_SMTP_USERNAME` / `LYXAL_SMTP_PASSWORD` and merges with
    ...
    /// (no rules fire). The `LYXAL_ON_FAILURE_CMD` env var continues
...
/// OIDC settings parsed from the Lyxalfile `oidc {}` block. All
/// fields are optional in the DSL; the server merges them with the
/// `LYXAL_OIDC_*` env vars at startup (DSL wins where both are set).
...
/// SMTP transport settings parsed from the Lyxalfile `smtp {}` block.
/// All fields are optional in the DSL; the server merges them with the
/// `LYXAL_SMTP_*` env vars at startup (DSL wins where both are set).
...
/// UI sign-in method gates, parsed from the Lyxalfile `auth {}` block.
...
/// did not set it — the server merges with `LYXAL_PASSWORD_LOGIN_ENABLED`
...
/// `LYXAL_REQUIRE_TOTP` at boot, defaulting to off.
#[derive(Debug, Clone, Default, Serialize)]
...
/// synthesised shell channel at boot when `LYXAL_ON_FAILURE_CMD` is
...
pub enum ChannelKind {
    ...
    /// failure context in `LYXAL_*` env vars (back-compat with the
    /// pre-#140 `LYXAL_ON_FAILURE_CMD` env-var hook).
    Shell { command: String },
    /// `webhook <url>` — POST a JSON envelope to the target URL.
    ///
    /// Signing is optional but recommended: when `signing_key` is set,
    /// the server adds an `X-Lyxal-Signature: sha256=<hex>` header
    /// whose value is HMAC-SHA256 of the raw body (issue #140 PR-2).
    /// Keep secrets out of the Lyxalfile by using a placeholder:
    ...
    /// `LYXAL_SMTP_URL` + `LYXAL_SMTP_FROM` and building
    /// lyxal-server with the `smtp` cargo feature.
...
pub struct ServerConfig {
    ...
    /// to the `LYXAL_APP_URL` env var, then to per-request host derivation.
...
/// thin runner binaries (e.g. `lyxal-shell-runner`) can decode it without
...
/// Compile a Lyxalfile AST into a RuntimeConfig.
pub fn compile(ast: &Lyxalfile) -> RuntimeConfig {
    ...
    for item in &ast.items {
        match item {
            ...
            Item::Mcp(m) => {
                ...
                // directives across a single Lyxalfile block are vanishingly
...
/// `lyxal-server::parse_duration_secs` (different crate, slightly
    ...
    fn compile_smtp_block() {
        let ast = Parser::parse(
            ...
            smtp {
                ...
                port 587
                security starttls
                from "Lyxal <noreply@example.com>"
            }
            "#,
        )
        .unwrap();
        ...
        assert_eq!(smtp.from.as_deref(), Some("Lyxal <noreply@example.com>"));
    }
