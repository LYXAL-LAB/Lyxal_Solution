//! Lyxal generic shell runner binary.
//!
//! Connects to a Lyxal server, registers itself as a runner with the
//! `shell-runner` capability, and runs jobs whose Lyxalfile carries a
//! `runner shell { ... }` or `runner exec { ... }` block.
//!
//! Environment variables:
//!   `LYXAL_SERVER_URL`       — server base URL  (default: http://localhost:4000)
//!   `LYXAL_API_KEY`          — bearer token     (recommended)
...
//!                               make it stable, e.g.
//!                                 volumes:
//!                                   - lyxal-runner-state:/var/lib/lyxal-runner
