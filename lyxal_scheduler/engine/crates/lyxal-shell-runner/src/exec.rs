...
//!
//! The actual SDK plumbing (poll/ack/lease) lives in `lyxal-runner-sdk`;
...
use lyxal_config::compile::RunnerExec;
use lyxal_runner_sdk::{HandlerError, LogWriter};
...
pub fn decode(metadata: &serde_json::Value) -> Result<RunnerExec, RunError> {
    let raw = metadata
        .get(lyxal_config::compile::RUNNER_EXEC_METADATA_KEY)
...
pub fn build_command(exec: &RunnerExec) -> Result<Command, RunError> {
    ...
    for (k, v) in std::env::vars() {
        ...
        // `PATH`, `HOME`, locale, etc. work; user-supplied env in the Lyxalfile
    ...
    R: tokio::io::AsyncRead + Unpin,
{
    ...
    while let Some(line) = lines.next_line().await? {
        ...
        //    contract documented in lyxal-runner-sdk::log_writer).
...
mod tests {
    ...
    async fn shell_command_runs_and_returns_stdout_tail() {
        let exec = RunnerExec::Shell {
            command: "echo lyxal-shell-runner".into(),
            workdir: None,
            user: None,
            env: HashMap::new(),
        };
        ...
        assert!(
            outcome
                .stdout_tail
                .iter()
                .any(|l| l.contains("lyxal-shell-runner")),
            "stdout_tail: {:?}",
            outcome.stdout_tail
        );
    }
    ...
    async fn user_supplied_env_reaches_subprocess() {
        ...
        env.insert("LYXAL_TEST_KEY".to_string(), "from-dsl".to_string());
        let exec = RunnerExec::Shell {
            command: "printf %s \"$LYXAL_TEST_KEY\"".into(),
            workdir: None,
            user: None,
            env,
        };
    ...
    fn decode_extracts_payload_from_metadata() {
        ...
        let metadata = serde_json::json!({
            lyxal_config::compile::RUNNER_EXEC_METADATA_KEY: payload.to_string(),
            "month": "2026-05",
        });
