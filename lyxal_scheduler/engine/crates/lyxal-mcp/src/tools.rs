//! MCP tool definitions for the Lyxal scheduler.
//!
...
use lyxal_runner::{AppState, RunnerStatus, WorkItem};
...
pub struct LyxalMcp {
    ...
    #[allow(dead_code)]
    tool_router: ToolRouter<LyxalMcp>,
}
...
pub struct CreateCalendarParams {
    ...
    /// Calendar rules in Lyxalfile DSL syntax — lines of `include`, `exclude`, `timezone`.
    pub rules: String,
}
...
pub struct UpdateCalendarParams {
    ...
    #[serde(default)]
    pub timezone: Option<String>,
    /// Replacement rules (Lyxalfile DSL). Omit to leave unchanged.
    #[serde(default)]
...
// strings. They mention the REST adopt endpoint and the Lyxalfile policy
...
fn dsl_calendar_refusal_msg(id_or_name: &str, action: &str) -> String {
    ...
    format!(
        "Calendar '{name}' is managed by the Lyxalfile. To take ownership and {action} it via the API, call `POST /v1/calendars/dsl:{name}/adopt` (requires `policy {{ dsl_adopt_on_mutate true }}` in the Lyxalfile)."
    )
}

fn dsl_job_refusal_msg(job_key: &str) -> String {
    format!(
        "Job '{job_key}' is managed by the Lyxalfile. To take ownership and edit it via the API, call `POST /v1/jobs/{job_key}/adopt` (requires `policy {{ dsl_adopt_on_mutate true }}` in the Lyxalfile)."
    )
}

fn dsl_schedule_refusal_msg(trigger_id: &str, job_key: &str) -> String {
    format!(
        "Schedule '{trigger_id}' belongs to DSL-managed job '{job_key}'. Adopt the job via `POST /v1/jobs/{job_key}/adopt` to edit its schedule (requires `policy {{ dsl_adopt_on_mutate true }}` in the Lyxalfile)."
    )
}
...
impl LyxalMcp {
    ...
    fn require_mutations(&self) -> Result<(), McpError> {
        if !self.mutations_enabled {
            return Err(McpError::invalid_params(
                "Mutations are disabled. Start lyxal-mcp with --mutations to enable write operations.",
                None,
            ));
        }
        Ok(())
    ...
    fn require_store(&self) -> Result<&DynStore, McpError> {
        self.store.as_ref().ok_or_else(|| {
            McpError::internal_error(
                "No persistent store available. Start lyxal-mcp with --data-dir <path> to enable store-backed operations.",
                None,
            )
        })
    }
    ...
    async fn list_executions(
        ...
        use lyxal_store::models::{ExecutionFilter, ExecutionState};
    ...
    /// `/v1/schedules`. DSL-managed jobs (defined in the Lyxalfile) are
    ...
    async fn dlq_retry(
        ...
        if !p.force
            ...
            && let Some(max_age) = lyxal_execution::retry::parse_duration(max_age_str)
        {
        ...
        store
            ...
            .map_err(|e| match e {
                lyxal_store::traits::StoreError::NotFound(_) => McpError::invalid_params(
                    format!(
    ...
    /// Create a new calendar. The `rules` body must parse as Lyxalfile DSL —
    /// validation runs the same parser as the HTTP endpoint, so syntactic
    /// errors are reported back to the caller.
    #[tool(
        description = "Create a new calendar with name, timezone, and rule lines. Validates rules with the Lyxalfile DSL parser. Requires --mutations and --data-dir."
    )]
    async fn create_calendar(
        ...
        let now = Utc::now();
        let cal = lyxal_store::models::CalendarDefinition {
    ...
    /// scheduler snapshot — needs to be embedded in `lyxal-server`; the
    ...
    async fn dashboard_forecast(
        ...
        let result = lyxal_scheduler::forecast::compute_forecast(
            &triggers,
            Utc::now(),
            p.window_minutes,
            p.bucket_minutes,
    ...
    #[tool(
        description = "Fetch one job definition by key. Falls back to the Lyxalfile DSL snapshot when the store doesn't have the row. Requires --data-dir."
    )]
    ...
    /// the Lyxalfile owns those and would just overwrite the row on reload.
    #[tool(
        description = "Create a new store-managed job. Refuses to shadow a DSL-managed (Lyxalfile) job. Pair with create_schedule to also arm a trigger. Requires --mutations and --data-dir."
    )]
    async fn create_job(
        &self,
    ...
    /// Lyxalfile are not included — read the file or use the HTTP
    /// `/v1/schedules` endpoint, which unions both sources.
    #[tool(
        description = "List store-managed schedules (trigger definitions). Optionally filter by job_key. DSL schedules from the Lyxalfile are not included. Requires --data-dir."
    )]
...
/// block and running the Lyxalfile parser. Mirrors the HTTP handler so MCP
...
fn validate_calendar_rules(rules: &str) -> Result<(), String> {
    ...
    lyxal_config::parser::Parser::parse(&source)
        .map(|_| ())
...
#[tool_handler]
impl ServerHandler for LyxalMcp {
    fn get_info(&self) -> ServerInfo {
        ...
        let instructions = format!(
            "Lyxal is a distributed job scheduler. \
...
mod tests {
    ...
    fn make_server() -> LyxalMcp {
        LyxalMcp::new(AppState::new())
    }

    fn make_server_with_mutations() -> LyxalMcp {
        use lyxal_store::sqlite::SqliteStore;
        let store: DynStore = Arc::new(SqliteStore::in_memory().unwrap());
        LyxalMcp::new_with_store(AppState::new(), store, vec![], true)
    }
    ...
    async fn job_trigger_persists_to_store() {
        use lyxal_store::models::ExecutionFilter;
    ...
    async fn dlq_retry_blocked_without_store() {
        ...
        let server = LyxalMcp {
            ...
            triggers: None,
            mutations_enabled: true,
            tool_router: LyxalMcp::tool_router(),
        };
        let err = server
            .dlq_retry(Parameters(DlqRetryParams {
                dead_letter_id: Uuid::new_v4().to_string(),
    ...
    async fn dlq_retry_re_enqueues() {
        use lyxal_store::models::{DeadLetter, DeadLetterFilter, ExecutionFilter};
