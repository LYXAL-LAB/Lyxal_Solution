...
//! and a trigger moment, it produces the types that `lyxal-runner` and
//! `lyxal-execution` need to actually execute the job.

use chrono::{DateTime, Utc};
use lyxal_config::compile::JobConfig;
use lyxal_execution::pipeline::ExecutionPolicy;
...
/// generating it (e.g. a UUID from `lyxal-store`).
///
...
mod tests {
    ...
    use lyxal_execution::{pipeline::ExecutionOutcome, retry::RetryStrategy};
    ...
    fn policy_evaluate_success_after_first_try() {
        use lyxal_execution::pipeline::ExecutionResult;
    ...
    fn dsl_to_runtime_roundtrip_fixed_strategy() {
        use lyxal_execution::retry::RetryDecision;
