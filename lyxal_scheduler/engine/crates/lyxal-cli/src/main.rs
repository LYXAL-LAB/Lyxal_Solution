...
/// `LYXAL_DEMO_MFA=1` (docker convention) work alongside the bare
...
enum Commands {
    // ── Config commands ───────────────────────────────────────────────────────
    /// Validate a Lyxalfile for errors and warnings
    Validate {
        /// Path to Lyxalfile
        #[arg(default_value = "Lyxalfile")]
        path: PathBuf,
    },
    ...
    /// Compile a Lyxalfile and print the runtime config as JSON
    Compile {
        /// Path to Lyxalfile
        #[arg(default_value = "Lyxalfile")]
        path: PathBuf,
    ...
    /// Show diff between two Lyxalfiles
    Diff {
        /// First file
        old: PathBuf,
        /// Second file
    ...
    /// Convert a standard cron expression to Lyxal DSL
    ///
    /// Examples:
    ...
    Init {
        ...
        /// "lyxal_". Without this flag no API client/key is created —
        ...
        /// `LYXAL_DEMO_MFA=1` in the docker entrypoint. Never use in
    ...
    /// Zero-to-running in one command: creates Lyxalfile, inits DB, prints next steps
    Quickstart {
        /// Data directory
        #[arg(long, default_value = "./.data")]
        data_dir: PathBuf,

        /// Lyxalfile output path
        #[arg(long, default_value = "Lyxalfile")]
        config: PathBuf,
    ...
    /// Migrate a crontab file to a Lyxalfile
    Migrate {
        /// Path to the crontab file to convert
        crontab: PathBuf,

        /// Output Lyxalfile path (prints to stdout if omitted)
        #[arg(short, long)]
