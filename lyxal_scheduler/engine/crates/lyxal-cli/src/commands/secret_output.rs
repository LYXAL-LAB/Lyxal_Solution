...
impl CredentialSink {
    ...
    fn write_file(&self, data_dir: &Path) -> Result<()> {
        ...
        body.push_str("# Lyxal initial credentials — generated once during init.\n");
...
mod tests {
    ...
    fn tempdir() -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("lyxal-secret-test-{}", Uuid::new_v4()));
    ...
    fn writes_secrets_to_private_file_when_not_revealing() {
        ...
        sink.add_with_usage(
            "API Key",
            "lyxal_secret123",
            "Authorization: ApiKey lyxal_secret123",
        );
        sink.flush(&dir).unwrap();

        let path = dir.join(CREDENTIALS_FILE);
        let body = std::fs::read_to_string(&path).unwrap();
        assert!(body.contains("lyxal_secret123"));
        assert!(body.contains("Authorization: ApiKey lyxal_secret123"));
