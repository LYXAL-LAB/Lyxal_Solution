...
use lyxal_auth::crypto::wrap_totp_secret;
use lyxal_auth::jwt_secret;
use lyxal_auth::password::hash_password;
use lyxal_auth::totp::{enroll_user, hash_recovery_code};
use lyxal_store::models::{
    ApiClient, ApiKey, PasswordCredential, RecoveryCode, Role, TotpSecret, User,
};
use lyxal_store::surreal::SurrealStore;
use lyxal_store::traits::AuthStore;
...
pub fn init(
    ...
    let seed_request: Option<(&str, Vec<String>)> = match api_key_override {
        Some(raw_key) => {
            if !raw_key.starts_with("lyxal_") {
                return Err(miette!(
                    "--api-key must start with 'lyxal_' (got prefix '{}…'); \
                     e.g. LYXAL_INIT_API_KEY=lyxal_$(openssl rand -hex 32)",
    ...
    println!("Connecting to SurrealDB at {surreal_endpoint} ({surreal_ns}/{surreal_db})");
    ...
    match seeded_key {
        ...
        None => {
            ...
            println!("  CLI → rerun `croniq init --api-key lyxal_...` for reproducible seeds");
            println!();
        }
    }
...
fn seed_demo_mfa(
    ...
    // started with `LYXAL_JWT_SECRET` set wins over the file — that
    ...
    println!("  Production seeds must never set LYXAL_DEMO_MFA=1 / --demo-mfa.");
...
mod tests {
    ...
    fn tempdir() -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("lyxal-init-test-{}", Uuid::new_v4()));
