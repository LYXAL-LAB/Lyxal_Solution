...
/// Backs the `lyxal_job_*` Prometheus metrics.
#[derive(Debug, Clone)]
...
/// `lyxal_auth::context::Role::default_scopes`. The variants are stable
...
/// `lyxal_auth::crypto::wrap_totp_secret`). `enabled` is `false`
...
/// Link between a Lyxal user and an external OIDC subject. JIT-created
...
pub struct PersonalAccessToken {
    ...
    /// SHA-256 hash of the raw token.
    pub token_hash: String,
    /// First 12 chars of the raw token for display ("lyxal_pat_…").
...
pub struct TriggerDefinition {
    ...
    pub enabled: bool,
    /// Who manages this trigger: "dsl" (Lyxalfile), "api" (REST), "runner" (self-registered).
...
pub struct CalendarDefinition {
    ...
    pub name: String,
    pub timezone: Option<String>,
    /// Calendar rules as line-separated Lyxalfile DSL text — one
    /// `include`/`exclude` directive per line, not JSON.
    pub rules: String,
    /// Who manages this calendar: "dsl" (Lyxalfile, synthesized at read time)
...
/// definition — the Lyxalfile stays canonical. One row per rule, keyed
