...
//!
//! TEMPORARILY DISABLED: SQLite has been removed. Tests will be re-enabled
//! once `SurrealStore` provides an in-memory / test backend (see `surreal.rs`).
//!
//! To re-enable: remove the `#[cfg(false)]` gate below and update the
//! `create_memory_store()` call to use `SurrealStore::in_memory()` (or
//! equivalent test helper).

#![cfg(false)] // ← remove this line to re-enable when SurrealStore is ready
