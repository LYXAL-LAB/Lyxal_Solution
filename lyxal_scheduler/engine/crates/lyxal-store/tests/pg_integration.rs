...
//!
//! It is a no-op unless `LYXAL_TEST_PG_URL` points at a reachable Postgres
...
fn pg_backend_exercises_all_traits() {
    let Ok(url) = std::env::var("LYXAL_TEST_PG_URL") else {
        eprintln!("LYXAL_TEST_PG_URL not set — skipping Postgres integration test");
        return;
    };
