...
//! lyxal-server could not use it. `PgStoreHandle` runs `PgStore` on a
...
//!
//! No-op unless `LYXAL_TEST_PG_URL` is set (same convention as
...
use lyxal_store::pg_actor::PgStoreHandle;
...
async fn pg_handle_usable_inside_tokio_runtime() {
    let Ok(url) = std::env::var("LYXAL_TEST_PG_URL") else {
        eprintln!("LYXAL_TEST_PG_URL not set — skipping PgStoreHandle runtime test");
        return;
    };
