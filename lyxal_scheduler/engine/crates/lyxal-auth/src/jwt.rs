...
/// Issuer of all JWTs minted by lyxal-server.
///
...
/// tokens lack, so the issuer moves to `"lyxal-v1"`. Old tokens become
...
pub const JWT_ISSUER: &str = "lyxal-v1";
...
impl Default for JwtConfig {
    fn default() -> Self {
        Self {
            secret: "lyxal-dev-secret-change-me".into(),
...
/// PR-A1 bumped the default issuer from `"croniq"` to `"lyxal-v1"`, so
...
mod tests {
    ...
    fn old_issuer_rejected_as_hard_cut() {
        ...
        // validating it against the default config (issuer "lyxal-v1")
        ...
        // Default config uses "lyxal-v1" — old issuer must be rejected.
