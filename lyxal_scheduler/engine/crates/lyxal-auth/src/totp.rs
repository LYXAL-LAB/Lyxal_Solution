...
/// authenticator apps display "Lyxal · alex".
const TOTP_ISSUER: &str = "Lyxal";
...
// builds it via `lyxal_auth::jwt::issue_token_pair` with a tiny TTL.)
...
mod tests {
    ...
    fn enrolment_emits_otpauth_url_with_seed() {
        ...
        assert!(e.otpauth_url.starts_with("otpauth://totp/Lyxal:"));
        assert!(e.otpauth_url.contains(&format!("secret={}", e.secret_b32)));
        assert!(e.otpauth_url.contains("issuer=Lyxal"));
    }
