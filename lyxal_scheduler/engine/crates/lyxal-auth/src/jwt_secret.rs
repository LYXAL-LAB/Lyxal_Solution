...
//!
//! Resolution order:
//!   1. `LYXAL_JWT_SECRET` environment variable
...
/// duplicated here so the CLI doesn't pull in `lyxal-server`.
...
mod tests {
    ...
    fn tempdir() -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("lyxal-jwt-secret-{}", uuid::Uuid::new_v4()));
    ...
    fn generates_then_returns_stable_value() {
        ...
        // racing with sibling tests on `LYXAL_JWT_SECRET`.
        let tmp = tempdir();
