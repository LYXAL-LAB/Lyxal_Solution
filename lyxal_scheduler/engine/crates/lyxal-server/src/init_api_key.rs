//! Boot-time reconciliation for `LYXAL_INIT_API_KEY`.
//!
//! On a fresh data dir, `croniq init` consumes `LYXAL_INIT_API_KEY` and
...
//!
//! This module fixes that: at server startup, if `LYXAL_INIT_API_KEY` is
//! set, we log whether it matches the stored key for the `default`
//! client, and — when `LYXAL_INIT_API_KEY_RECONCILE=1` is also set —
...
use chrono::Utc;
use lyxal_auth::api_key::hash_api_key;
use lyxal_store::models::ApiKey;
use lyxal_store::traits::{AuthStore, StoreError};
use uuid::Uuid;
...
impl<'a> ReconcileInputs<'a> {
    pub fn from_env_borrowed(api_key: &'a Option<String>) -> Self {
        Self {
            ...
            reconcile_enabled: env_truthy(std::env::var("LYXAL_INIT_API_KEY_RECONCILE").ok()),
        }
    }
}
...
pub fn reconcile<S: AuthStore + ?Sized>(
    ...
    let Some(default_client) = clients.iter().find(|c| c.name == DEFAULT_CLIENT_NAME) else {
        tracing::info!(
            "LYXAL_INIT_API_KEY is set but no '{DEFAULT_CLIENT_NAME}' API client exists in \
    ...
    if already_present {
        tracing::info!(
            "LYXAL_INIT_API_KEY matches an active key for client '{DEFAULT_CLIENT_NAME}' — \
             no changes."
        );
        return Ok(());
    }
    ...
    if !inputs.reconcile_enabled {
        tracing::warn!(
            "LYXAL_INIT_API_KEY differs from every active key for client \
             '{DEFAULT_CLIENT_NAME}' — env value ignored. Set \
             LYXAL_INIT_API_KEY_RECONCILE=1 to rotate (revokes existing active keys and \
    ...
    tracing::warn!(
        revoked,
        "LYXAL_INIT_API_KEY_RECONCILE=1 — installed env value as new key for client \
...
mod tests {
    use super::*;
    use lyxal_store::models::{ApiClient, ApiKey};
    

    fn tempdir() -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("lyxal-reconcile-{}", Uuid::new_v4()));
    ...
    fn unset_env_is_noop() {
        let (store, _) = store_with_default_client("lyxal_old");
        reconcile(
            &store,
            ReconcileInputs {
                api_key: None,
    ...
    fn matching_key_is_noop() {
        let (store, client_id) = store_with_default_client("lyxal_old");
        reconcile(
            &store,
            ReconcileInputs {
                api_key: Some("lyxal_old"),
    ...
    fn differing_key_with_reconcile_rotates() {
        ...
        assert_eq!(active[0].key_hash, hash_api_key("lyxal_new"));
    }
