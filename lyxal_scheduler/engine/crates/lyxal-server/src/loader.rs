//! Lyxalfile loader: parses, validates, and compiles the operator's configuration.
//!
//! Produces:
...
use lyxal_config::ast::{Lyxalfile, Item};
use lyxal_config::compile::JobConfig;
use lyxal_config::compile::{self, CatchUpPolicy, RuntimeConfig};
use lyxal_config::import::resolve_imports_with_visited;
use lyxal_config::parser::Parser;
use lyxal_scheduler::{
    calendar::Calendar,
...
fn parse_error_to_load(err: lyxal_config::parser::ParseError, source: &str) -> LoadError {
    use lyxal_config::lexer::LexError;
    use lyxal_config::parser::ParseError;
...
/// Load a Lyxalfile from a file path, resolving `import` directives recursively.
...
fn load_and_resolve(path: &Path, visited: &mut HashSet<PathBuf>) -> Result<Lyxalfile, LoadError> {
...
/// Load a Lyxalfile from a source string (no import resolution).
...
fn load_from_compiled(runtime: RuntimeConfig, ast: &Lyxalfile) -> Result<LoadedConfig, LoadError> {
    let now = Utc::now();
    ...
    let ast_jobs: Vec<&lyxal_config::ast::JobBlock> = ast
        .items
        .iter()
        .filter_map(|item| {
            if let lyxal_config::ast::Item::Job(j) = item {
                Some(j)
            } else {
                None
            }
...
pub async fn restore_queued_executions(
    ...
    jobs: &[JobConfig],
    runner_state: &lyxal_runner::AppState,
) -> usize {
    use lyxal_bridge::job_to_work_item;
    use lyxal_config::compile::ExecutionMode;
    ...
    let mut by_job: HashMap<String, Vec<&lyxal_store::models::Execution>> = HashMap::new();
...
/// line-separated Lyxalfile calendar-body DSL) into a compiled
...
/// semantic validation, so stored rows compile exactly like Lyxalfile
...
pub fn calendar_config_from_definition(
    def: &lyxal_store::models::CalendarDefinition,
) -> Result<lyxal_config::compile::CalendarConfig, String> {
    let source = format!("calendar \"__store__\" {{\n{}\n}}\n", def.rules);
    let ast = Parser::parse(&source).map_err(|e| e.to_string())?;
    let errors: Vec<String> = lyxal_config::validate::validate(&ast)
        .into_iter()
        .filter(|d| d.severity == lyxal_config::validate::Severity::Error)
        .map(|d| d.message)
        .collect();
    if !errors.is_empty() {
...
/// are compiled on top so they win name collisions (Lyxalfile precedence,
...
pub fn resolve_calendars(
    dsl: &[lyxal_config::compile::CalendarConfig],
    stored: &[lyxal_store::models::CalendarDefinition],
    strict: bool,
) -> ResolvedCalendars {
...
///
/// Used for API/runner-registered jobs that don't come from the Lyxalfile.
...
pub fn trigger_from_definition(
    def: &lyxal_store::models::TriggerDefinition,
...
pub fn job_config_from_definition(
    def: &lyxal_store::models::TriggerDefinition,
    job_def: Option<&lyxal_store::models::JobDefinition>,
) -> lyxal_config::compile::JobConfig {
    use lyxal_config::compile::*;
    use lyxal_config::schedule::CompiledSchedule;
    ...
    JobConfig {
        ...
        // API-registered jobs have no Lyxalfile `keep_last` (v1 supports
...
/// used as a fallback when an API-registered job has no Lyxalfile entry.
...
pub fn job_config_from_job_def(
    job_def: &lyxal_store::models::JobDefinition,
) -> lyxal_config::compile::JobConfig {
    use lyxal_config::compile::*;
    use lyxal_config::schedule::CompiledSchedule;
...
pub fn synth_job_def_from_dsl(
    cfg: &JobConfig,
    now: DateTime<Utc>,
) -> lyxal_store::models::JobDefinition {
    lyxal_store::models::JobDefinition {
        ...
        is_active: !matches!(
            cfg.schedule,
            lyxal_config::schedule::CompiledSchedule::Disabled
        ),
...
pub fn synth_trigger_def_from_dsl(
    cfg: &JobConfig,
    now: DateTime<Utc>,
) -> lyxal_store::models::TriggerDefinition {
    lyxal_store::models::TriggerDefinition {
...
fn schedule_from_expr(expr: &str) -> Option<Schedule> {
    ...
    let kind = lyxal_config::parser::parse_schedule_expr(expr).ok()?;
...
fn compiled_schedule_from_expr(expr: &str) -> Option<lyxal_config::schedule::CompiledSchedule> {
    use lyxal_config::schedule::CompiledSchedule;
    if let Some(seconds) = parse_interval_seconds(expr) {
...
mod tests {
    use super::*;
    use lyxal_scheduler::trigger::TriggerState;
    ...
    fn fmt_round_trip_calendar_survives_load() {
        ...
        // round-trip of a working Lyxalfile must stay loadable.
        let src = r#"
        ...
        "#;
        let ast = lyxal_config::parser::Parser::parse(src).unwrap();
        let formatted = lyxal_config::format::format(&ast);
        assert!(
    ...
    fn seed_job_state(
        store: &SqliteStore,
        key: &str,
        status: lyxal_store::models::JobStatus,
        next_fire_at: Option<chrono::DateTime<Utc>>,
        fire_count: u64,
    ) {
        store
            .upsert_job_state(&lyxal_store::models::JobState {
                job_key: key.into(),
                next_fire_at,
                last_fired_at: None,
                fire_count,
    ...
    fn restore_exhausted_disarms_once_trigger() {
        ...
        seed_job_state(
            &store,
            "migration:v2",
            lyxal_store::models::JobStatus::Exhausted,
            None,
            1,
        );
    ...
    fn restore_active_restores_next_fire_at() {
        ...
        seed_job_state(
            &store,
            "etl:sync",
            lyxal_store::models::JobStatus::Active,
            Some(stored_next),
            7,
        );
    ...
    fn restore_paused_does_not_change_trigger() {
        ...
        seed_job_state(
            &store,
            "reports:monthly",
            lyxal_store::models::JobStatus::Paused,
            None,
            0,
        );
    ...
    fn stored_state(store: &SqliteStore, key: &str) -> lyxal_store::models::JobState {
        store
            .list_job_states()
            .unwrap()
            .into_iter()
    ...
    fn restore_active_gate_disallowed_next_fire_is_healed_and_persisted() {
        ...
        assert_eq!(row.status, lyxal_store::models::JobStatus::Active);
    ...
    use lyxal_store::models::{CalendarDefinition, TriggerDefinition};
