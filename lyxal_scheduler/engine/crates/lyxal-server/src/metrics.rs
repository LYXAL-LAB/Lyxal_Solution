...
use lyxal_runner::RunnerStatus;
use lyxal_store::models::{
...
async fn handle_metrics(State(state): State<Arc<ServerState>>) -> impl IntoResponse {
    ...
    let mut body = format!(
        "# HELP lyxal_runners_total Number of known runners by status.\n\
         # TYPE lyxal_runners_total gauge\n\
         lyxal_runners_total{{status=\"online\"}} {runners_online}\n\
         lyxal_runners_total{{status=\"stale\"}} {runners_stale}\n\
         lyxal_runners_total{{status=\"dead\"}} {runners_dead}\n\
         # HELP lyxal_queue_depth Number of work items in the queue.\n\
         # TYPE lyxal_queue_depth gauge\n\
         lyxal_queue_depth {queue_depth}\n\
         # HELP lyxal_config_reload_total Config reload attempts by outcome.\n\
         # TYPE lyxal_config_reload_total counter\n\
         lyxal_config_reload_total{{result=\"success\"}} {reload_success}\n\
         lyxal_config_reload_total{{result=\"validation_error\"}} {reload_validation_err}\n\
         lyxal_config_reload_total{{result=\"apply_error\"}} {reload_apply_err}\n\
         # HELP lyxal_config_calendar_faults Jobs paused because a referenced calendar did not resolve.\n\
         # TYPE lyxal_config_calendar_faults gauge\n\
         lyxal_config_calendar_faults {calendar_faults}\n\
         # HELP lyxal_watchdog_requeued_total Executions requeued by watchdog recovery paths, by reason.\n\
         # TYPE lyxal_watchdog_requeued_total counter\n\
         lyxal_watchdog_requeued_total{{reason=\"dead_runner\"}} {wd_requeued_dead}\n\
         lyxal_watchdog_requeued_total{{reason=\"stale_claim\"}} {wd_requeued_stale}\n\
         lyxal_watchdog_requeued_total{{reason=\"reconciled\"}} {wd_requeued_reconciled}\n\
         # HELP lyxal_watchdog_cancelled_total Executions cancelled by the watchdog, by reason.\n\
         # TYPE lyxal_watchdog_cancelled_total counter\n\
         lyxal_watchdog_cancelled_total{{reason=\"queue_ttl\"}} {wd_cancelled_ttl}\n\
         lyxal_watchdog_cancelled_total{{reason=\"stranded\"}} {wd_cancelled_stranded}\n\
         # HELP lyxal_watchdog_sla_missed_total SLA-miss alerts fired by the watchdog sweep.\n\
         # TYPE lyxal_watchdog_sla_missed_total counter\n\
         lyxal_watchdog_sla_missed_total {wd_sla_missed}\n\
         # HELP lyxal_watchdog_missed_fires_total Missed-fire alerts fired by the watchdog sweep.\n\
         # TYPE lyxal_watchdog_missed_fires_total counter\n\
         lyxal_watchdog_missed_fires_total {wd_missed_fires}\n"
    );
    ...
    if let Some(hb) = &state.scheduler_heartbeat {
        body.push_str(&format!(
            "# HELP lyxal_scheduler_last_tick_timestamp Unix time (seconds) of the last successful scheduler tick.\n\
             # TYPE lyxal_scheduler_last_tick_timestamp gauge\n\
             lyxal_scheduler_last_tick_timestamp {}\n\
             # HELP lyxal_scheduler_ticks_total Successful scheduler ticks since process start.\n\
             # TYPE lyxal_scheduler_ticks_total counter\n\
             lyxal_scheduler_ticks_total {}\n",
            hb.last_tick_unix(),
            hb.ticks_total(),
        ));
    }
    ...
    if let Some(store) = &state.store {
        ...
        // scheduled — `time() - lyxal_job_last_fire_timestamp > period` or a
        // stuck `lyxal_job_overdue == 1` — even when the in-process
...
fn render_job_metrics(out: &mut String, jobs: &[JobExecutionMetrics]) {
    ...
    out.push_str(
        "# HELP lyxal_job_executions_total Terminal executions per job, by final state.\n\
         # TYPE lyxal_job_executions_total counter\n",
    );
    for j in jobs {
        let key = escape_label(&j.job_key);
        out.push_str(&format!(
            "lyxal_job_executions_total{{job_key=\"{key}\",state=\"completed\"}} {}\n",
            j.completed
        ));
        out.push_str(&format!(
            "lyxal_job_executions_total{{job_key=\"{key}\",state=\"failed\"}} {}\n",
            j.failed
        ));
        out.push_str(&format!(
            "lyxal_job_executions_total{{job_key=\"{key}\",state=\"dead\"}} {}\n",
            j.dead
        ));
        out.push_str(&format!(
            "lyxal_job_executions_total{{job_key=\"{key}\",state=\"cancelled\"}} {}\n",
            j.cancelled
        ));
    }

    out.push_str(
        "# HELP lyxal_job_duration_seconds Execution wall-clock duration per job, in seconds.\n\
         # TYPE lyxal_job_duration_seconds histogram\n",
    );
    for j in jobs {
        let key = escape_label(&j.job_key);
        for (idx, bound) in JOB_DURATION_BUCKETS_SECONDS.iter().enumerate() {
            ...
            out.push_str(&format!(
                "lyxal_job_duration_seconds_bucket{{job_key=\"{key}\",le=\"{bound}\"}} {count}\n"
            ));
        }
        out.push_str(&format!(
            "lyxal_job_duration_seconds_bucket{{job_key=\"{key}\",le=\"+Inf\"}} {}\n",
            j.duration_count
        ));
        out.push_str(&format!(
            "lyxal_job_duration_seconds_sum{{job_key=\"{key}\"}} {}\n",
            j.duration_sum_ms as f64 / 1000.0
        ));
        out.push_str(&format!(
            "lyxal_job_duration_seconds_count{{job_key=\"{key}\"}} {}\n",
            j.duration_count
        ));
    }

    out.push_str(
        "# HELP lyxal_job_last_run_timestamp Unix time (seconds) of the most recent finished execution per job.\n\
         # TYPE lyxal_job_last_run_timestamp gauge\n",
    );
    for j in jobs {
        if let Some(ts) = j.last_run_at {
            let key = escape_label(&j.job_key);
            out.push_str(&format!(
                "lyxal_job_last_run_timestamp{{job_key=\"{key}\"}} {}\n",
                ts.timestamp()
            ));
        }
    }
...
fn render_job_state_metrics(
    ...
    out.push_str(
        "# HELP lyxal_job_last_fire_timestamp Unix time (seconds) of the last scheduled fire per job.\n\
         # TYPE lyxal_job_last_fire_timestamp gauge\n",
    );
    for s in states {
        if let Some(ts) = s.last_fired_at {
            let key = escape_label(&s.job_key);
            out.push_str(&format!(
                "lyxal_job_last_fire_timestamp{{job_key=\"{key}\"}} {}\n",
                ts.timestamp()
            ));
        }
    }
    ...
    out.push_str(
        "# HELP lyxal_job_next_fire_timestamp Unix time (seconds) of the next scheduled fire per job.\n\
         # TYPE lyxal_job_next_fire_timestamp gauge\n",
    );
    for s in states {
        if let Some(ts) = s.next_fire_at {
            let key = escape_label(&s.job_key);
            out.push_str(&format!(
                "lyxal_job_next_fire_timestamp{{job_key=\"{key}\"}} {}\n",
                ts.timestamp()
            ));
        }
    }
    ...
    out.push_str(
        "# HELP lyxal_job_overdue Whether an active job's next scheduled fire is in the past (1) or not (0). A stuck 1 signals a stalled scheduler.\n\
         # TYPE lyxal_job_overdue gauge\n",
    );
    for s in states {
        ...
        if let Some(ts) = s.next_fire_at {
            ...
            out.push_str(&format!(
                "lyxal_job_overdue{{job_key=\"{key}\"}} {overdue}\n"
            ));
        }
    }
}
...
mod tests {
    ...
    async fn metrics_returns_prometheus_format() {
        ...
        assert!(body.contains("lyxal_runners_total"));
        assert!(body.contains("lyxal_queue_depth"));

        // Watchdog counters are always emitted, starting at zero.
        assert!(body.contains("# TYPE lyxal_watchdog_requeued_total counter"));
        assert!(body.contains("lyxal_watchdog_requeued_total{reason=\"dead_runner\"} 0"));
        assert!(body.contains("lyxal_watchdog_cancelled_total{reason=\"queue_ttl\"} 0"));
        assert!(body.contains("lyxal_watchdog_sla_missed_total 0"));
        assert!(body.contains("lyxal_watchdog_missed_fires_total 0"));
    }

    #[tokio::test]
    async fn metrics_reports_watchdog_counters_after_reaping_sweep() {
        use crate::watchdog::WatchdogLoop;
        use lyxal_store::models::{Execution, ExecutionState};

        let store =
            unimplemented!("SurrealStore test backend not yet available");
        ...
        assert!(body.contains("lyxal_watchdog_requeued_total{reason=\"stale_claim\"} 1"));
        assert!(body.contains("lyxal_watchdog_requeued_total{reason=\"dead_runner\"} 0"));
        ...
        .unwrap();
        assert!(body.contains("lyxal_watchdog_requeued_total{reason=\"dead_runner\"} 2"));
    }
    ...
    async fn metrics_includes_scheduler_heartbeat_when_present() {
        ...
        assert!(body.contains("# TYPE lyxal_scheduler_last_tick_timestamp gauge"));
        assert!(body.contains("lyxal_scheduler_last_tick_timestamp 1700000000"));
        assert!(body.contains("lyxal_scheduler_ticks_total 1"));
    }
    ...
    async fn metrics_omits_scheduler_heartbeat_when_absent() {
        ...
        assert!(!body.contains("lyxal_scheduler_last_tick_timestamp"));
    }
    ...
    fn render_job_metrics_emits_counter_histogram_and_gauge() {
        ...
        assert!(out.contains("# TYPE lyxal_job_executions_total counter"));
        assert!(out.contains(
            "lyxal_job_executions_total{job_key=\"billing:invoice\",state=\"completed\"} 7"
        ));
        assert!(
            out.contains(
                "lyxal_job_executions_total{job_key=\"billing:invoice\",state=\"dead\"} 2"
            )
        );

        assert!(out.contains("# TYPE lyxal_job_duration_seconds histogram"));
        // The +Inf bucket equals the histogram _count.
        assert!(out.contains(
            "lyxal_job_duration_seconds_bucket{job_key=\"billing:invoice\",le=\"+Inf\"} 3"
        ));
        assert!(out.contains("lyxal_job_duration_seconds_sum{job_key=\"billing:invoice\"} 124.7"));
        assert!(out.contains("lyxal_job_duration_seconds_count{job_key=\"billing:invoice\"} 3"));

        assert!(
            out.contains("lyxal_job_last_run_timestamp{job_key=\"billing:invoice\"} 1700000000")
        );
    }
    ...
    fn render_job_metrics_escapes_label_and_skips_missing_last_run() {
        ...
        assert!(!out.contains("lyxal_job_last_run_timestamp{"));
    }
    ...
    fn render_job_state_metrics_emits_fire_timestamps_and_overdue() {
        ...
        assert!(out.contains("# TYPE lyxal_job_last_fire_timestamp gauge"));
        assert!(
            out.contains("lyxal_job_last_fire_timestamp{job_key=\"billing:backup\"} 1699913600")
        );
        // etl:sync never fired → no last-fire sample.
        assert!(!out.contains("lyxal_job_last_fire_timestamp{job_key=\"etl:sync\"}"));

        assert!(out.contains("lyxal_job_next_fire_timestamp{job_key=\"billing:backup\"}"));

        // Overdue: the backup's next fire is in the past, etl:sync's is not.
        assert!(out.contains("lyxal_job_overdue{job_key=\"billing:backup\"} 1"));
        assert!(out.contains("lyxal_job_overdue{job_key=\"etl:sync\"} 0"));
    }
