//! lyxal-server binary: the main server process.
//!
//! Usage:
//! ```sh
//! lyxal-server --config Lyxalfile --listen :4000 --data-dir ./.data
//! ```
...
#[command(
    name = "lyxal-server",
    about = "Lyxal distributed job scheduler",
    long_about = "Lyxal distributed job scheduler.\n\
\n\
...
\n\
    ...
    RUST_LOG=info,lyxal_scheduler=debug  # mixed: info globally, debug for scheduler\n\
    RUST_LOG=warn                       # production-quiet\n\
\n\
Unset RUST_LOG behaves as `info`."
)]
struct Cli {
    /// Path to the Lyxalfile configuration.
    #[arg(short, long, default_value = "Lyxalfile")]
    config: PathBuf,
    ...
    /// Defaults to `$LYXAL_DATA_DIR` when set, otherwise `./.data`. The
    ...
    #[arg(short, long, default_value = "./.data", env = "LYXAL_DATA_DIR")]
    data_dir: PathBuf,
    ...
    /// Watch the Lyxalfile for changes and hot-reload on modification.
    #[arg(long)]
    watch: bool,
...
/// Optional subcommands. With none, `lyxal-server` runs the server — the
...
async fn main() -> Result<()> {
    ...
    tracing::info!(config = %cli.config.display(), "loading Lyxalfile");
    ...
    // `lyxal-server doctor`: report config health and exit without binding
    ...
    if !password_login_enabled
        && lyxal_server::oidc::config_from_dsl_and_env(loaded.runtime.oidc.as_ref()).is_err()
    {
        anyhow::bail!(
            "refusing to start: password login is disabled but no working OIDC config was found.\n\
             Either re-enable password login in the Lyxalfile (auth {{ password {{ enabled true }} }})\n\
             or configure OIDC via LYXAL_OIDC_ISSUER / LYXAL_OIDC_CLIENT_ID / \
             LYXAL_OIDC_CLIENT_SECRET / LYXAL_OIDC_REDIRECT_URL."
        );
    }
    if !password_login_enabled {
    ...
    // Enforced 2FA (auth { totp { required true } } / LYXAL_REQUIRE_TOTP).
    ...
    if require_totp {
        tracing::warn!(
            "enforced 2FA is ON — every password login must present a TOTP or recovery \
             ...
             LYXAL_REQUIRE_TOTP=false), re-enrol, then re-enable."
        );
    }

    // Open the persistence store connected directly to SurrealDB
    ...
    tracing::info!(
        ...
        ns = %surreal_ns,
        db = %surreal_db,
        "connecting Lyxal engine to SurrealDB"
    );

    let store: DynStore = lyxal_server::store::dyn_store(
        ...
        .await
        .context("Failed to connect Lyxal engine to SurrealDB")?,
    );


    // Reconcile LYXAL_INIT_API_KEY against the stored 'default' client.
    ...
    // and rotate when LYXAL_INIT_API_KEY_RECONCILE=1 is set. See #217.
    {
        let init_api_key = lyxal_server::env_secret::env_or_file("LYXAL_INIT_API_KEY");
        let inputs = lyxal_server::init_api_key::ReconcileInputs::from_env_borrowed(&init_api_key);
        lyxal_server::init_api_key::reconcile(&*store, inputs)
            .context("failed to reconcile LYXAL_INIT_API_KEY against stored API client")?;
    }
    ...
    let trigger_dedup_window_secs = match loaded.runtime.pull_api.as_ref() {
        ...
        None => lyxal_server::api::DEFAULT_TRIGGER_DEDUP_WINDOW_SECS,
    };
    ...
    // JWT secret resolution: LYXAL_JWT_SECRET env > $DATA_DIR/jwt.secret
    // (auto-created on first boot). Shared with the CLI via
    // `lyxal_auth::jwt_secret::ensure` so CLI-side TOTP encryption and
    // server-side decryption always agree on the same secret.
    let jwt_secret = lyxal_auth::jwt_secret::ensure(&cli.data_dir).unwrap_or_else(|e| {
        tracing::error!(error = %e, "could not resolve JWT secret");
        std::process::exit(1);
    });
    let jwt_config = Some(lyxal_auth::jwt::JwtConfig {
        secret: jwt_secret,
        ..Default::default()
    });
    // Scheduler command channel for live job registration via API
    let (scheduler_cmd_tx, mut scheduler_cmd_rx) =
        mpsc::unbounded_channel::<lyxal_server::scheduler::SchedulerCommand>();
    ...
    // scheduler task on Lyxalfile reload so the REST API can union DSL
    ...
    let scheduler_heartbeat = Arc::new(lyxal_server::scheduler::SchedulerHeartbeat::default());
    ...
    // — Arc::get_mut requires a unique strong-ref.
    {
        ...
        *s.config_faults.write().unwrap() = std::mem::take(&mut loaded.calendar_faults);
        ...
        s.email_sender = lyxal_server::email::build_from_dsl_and_env(loaded.runtime.smtp.as_ref());
        // Issue #140 PR-5: surface the effective alerts config
        // (after LYXAL_ON_FAILURE_CMD synthesis) so the read-only
        // `GET /v1/alerts/config` endpoint can serve it.
        s.alerts = lyxal_server::alerts::merge_legacy_env_hook(loaded.runtime.alerts.clone());
    ...
    if let Some(url) = &server_state.app_base_url {
        tracing::info!(app_base_url = %url, "public base URL pinned (server.app_url / LYXAL_APP_URL)");
    }
    // Surface configuration health at boot (missing SMTP, unpinned URL, …).
    // The same checks back `lyxal-server doctor` and GET /v1/system/diagnostics.
    {
        use lyxal_server::diagnostics::{DiagnosticsInput, Severity, run_diagnostics};
    ...
    if cli.watch {
        match lyxal_server::watcher::watch_config(&config_path_abs) {
            Ok(raw_rx) => {
                let debounce_tx = reload_tx.clone();
                tokio::spawn(lyxal_server::watcher::debounced_reload_loop(
                    raw_rx,
                    std::time::Duration::from_millis(500),
                    debounce_tx,
                ));
                tracing::info!(path = %config_path_abs.display(), "watching Lyxalfile for changes");
            }
            Err(e) => {
    ...
    // SIGHUP → re-read Lyxalfile (unix only). Matches the long-standing
    ...
    // Reconcile API/runner-registered jobs from DB (not in Lyxalfile)
    {
        use lyxal_server::loader::{
        ...
        loop {
            tokio::select! {
                _ = interval.tick() => {
                    ...
                    match tokio::time::timeout(TICK_TIMEOUT, scheduler_loop.tick(now)).await {
                        ...
                        Err(_elapsed) => {
                            ...
                            // lyxal_scheduler_last_tick_timestamp instead of
                            // looking healthy.
                            tracing::error!(
                ...
                Some(path) = reload_rx.recv() => {
                    tracing::info!(path = %path.display(), "Lyxalfile reload requested");
    ...
    // back-compat synthesis from `LYXAL_ON_FAILURE_CMD` for installs
    ...
    let alert_throttle = lyxal_server::alerts::load_throttle_state(&proc_store, &alerts_cfg);
    ...
    let watchdog = WatchdogLoop::with_alerts(
        ...
        lyxal_server::watchdog::empty_sla_fired_set(),
        Arc::clone(&server_state.email_sender),
    );
    ...
    let execution_retention: Option<chrono::Duration> = loaded
        ...
        .execution_retention
        .as_deref()
        .and_then(|raw| match lyxal_execution::retry::parse_duration(raw) {
    ...
    let _watchdog_task = tokio::spawn(async move {
        ...
        loop {
            ...
            // Fold the sweep into the cumulative `lyxal_watchdog_*` counters
    ...
    // ── Metrics server (from CLI --metrics or observability.metrics in Lyxalfile)
    ...
    if let Some(ref metrics_listen) = metrics_listen {
        ...
        let metrics_app = lyxal_server::metrics::metrics_router(Arc::clone(&server_state));
    ...
    if mcp_enabled {
        ...
        let mcp_router = lyxal_server::mcp::mcp_router(
            mcp_state,
            mcp_runner,
            Some(mcp_store),
            mcp_jobs,
        ...
        tracing::info!("MCP HTTP transport disabled by Lyxalfile");
    }
    ...
    tracing::info!(address = %addr, "lyxal-server listening");
    let listener = tokio::net::TcpListener::bind(addr).await?;
    axum::serve(listener, app)
        .with_graceful_shutdown(shutdown_signal())
        .await?;
    tracing::info!("lyxal-server stopped gracefully");
...
/// env `LYXAL_PASSWORD_LOGIN_ENABLED` → default `true`.
///
...
/// out because of `LYXAL_PASSWORD_LOGIN_ENABLED=disable` would be a bad
/// surprise.
fn resolve_password_login_enabled(rt: &lyxal_config::compile::RuntimeConfig) -> bool {
    if let Some(v) = rt.auth.password.enabled {
        return v;
    }
    match std::env::var("LYXAL_PASSWORD_LOGIN_ENABLED").ok() {
        Some(s) => !matches!(
...
/// `LYXAL_APP_URL` → `None`. `None` means "not configured", and the link base
/// is then derived per-request from the forwarded / `Host` headers (see
/// `lyxal_server::api::resolve_link_base`). Blank values are ignored and
...
fn resolve_app_base_url(dsl_app_url: Option<&str>) -> Option<String> {
    ...
    match std::env::var("LYXAL_APP_URL") {
        Ok(s) if !s.trim().is_empty() => Some(s.trim().to_string()),
        _ => None,
    }
}

/// `lyxal-server doctor` — print a configuration health report and exit.
///
/// Offline preflight: runs the same checks surfaced at boot and via
/// `GET /v1/system/diagnostics`, but from the loaded Lyxalfile + env only
/// (no server, DB, or ports). Exits non-zero on any `Critical` finding so it
/// can gate a deploy.
fn run_doctor(rt: &lyxal_config::compile::RuntimeConfig) -> Result<()> {
    use lyxal_server::diagnostics::{DiagnosticsInput, Severity, run_diagnostics};

    let smtp_feature = lyxal_server::email::smtp_feature_compiled();
    let smtp_configured = lyxal_server::email::smtp_configured(rt.smtp.as_ref());
    let input = DiagnosticsInput::from_runtime(
        ...
        // + from) from the Lyxalfile smtp{} block or LYXAL_SMTP_* env.
...
/// `LYXAL_REQUIRE_TOTP` → default `false`.
///
...
fn resolve_require_totp(rt: &lyxal_config::compile::RuntimeConfig) -> bool {
    if let Some(v) = rt.auth.totp.required {
        return v;
    }
    match std::env::var("LYXAL_REQUIRE_TOTP").ok() {
        Some(s) => matches!(
            s.trim().to_ascii_lowercase().as_str(),
            "true" | "yes" | "on" | "1"
        ),
...
// TODO: The open_store / DbBackend / open_postgres_store functions have been
// removed together with the SQLite and PostgreSQL backends. Once SurrealStore
// is implemented, add a `connect_surreal_store(dsn: &str) -> Result<DynStore>`
// function here and wire it into main() above.
...
mod app_base_url_tests {
    ...
    /// `LYXAL_APP_URL` is process-global, so concurrent set/remove would
    ...
    fn none_when_unset() {
        let _g = env_guard();
        unsafe { std::env::remove_var("LYXAL_APP_URL") };
    ...
    fn reads_env_override() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_APP_URL", "https://croniq.example.com") };
    ...
    fn blank_env_is_none() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_APP_URL", "   ") };
        let url = resolve_app_base_url(None);
        unsafe { std::env::remove_var("LYXAL_APP_URL") };
        assert_eq!(url, None);
    }
    ...
    fn trims_surrounding_whitespace() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_APP_URL", "  https://app.example  ") };
        let url = resolve_app_base_url(None);
        unsafe { std::env::remove_var("LYXAL_APP_URL") };
    ...
    fn dsl_app_url_beats_env() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_APP_URL", "https://env.example") };
...
mod db_backend_tests {
    ...
    /// Serialise env mutation: `LYXAL_DB` is process-global and cargo runs
    ...
    fn defaults_to_sqlite_when_blank() {
        let _g = env_guard();
        unsafe { std::env::remove_var("LYXAL_DB") };
    ...
    fn env_overrides_dsl() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_DB", "postgres://h/db") };
    ...
    fn blank_env_falls_through_to_dsl() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_DB", "   ") };
