...
use lyxal_config::compile::SmtpDslConfig;
use std::sync::Arc;
...
/// setting `LYXAL_SMTP_*` has no effect — diagnostics flag that case.
pub fn smtp_feature_compiled() -> bool {
    cfg!(feature = "smtp")
}

/// Whether an SMTP transport can be assembled from the given DSL block +
/// the current `LYXAL_SMTP_*` environment, ignoring the `smtp` cargo
...
pub fn smtp_configured(dsl: Option<&SmtpDslConfig>) -> bool {
    let has_target = dsl.is_some_and(|d| d.host.is_some())
        || env_present("LYXAL_SMTP_HOST")
        || env_present("LYXAL_SMTP_URL");
    let has_from = dsl.is_some_and(|d| d.from.is_some()) || env_present("LYXAL_SMTP_FROM");
    has_target && has_from
}
...
/// Build the production sender at startup, merging the Lyxalfile
/// `smtp {}` block with the `LYXAL_SMTP_*` env vars. With the `smtp`
/// cargo feature disabled, always returns [`NoopSender`].
///
/// Precedence:
///   1. `LYXAL_SMTP_URL` (+ `_FILE`) — legacy composite, wins when set.
...
///      any field the DSL leaves unset falls back to `LYXAL_SMTP_HOST/
///      PORT/SECURITY/FROM`. Credentials (`LYXAL_SMTP_USERNAME/PASSWORD`)
///      are ENV-only.
///
...
/// Convenience wrapper for callers without a Lyxalfile `smtp {}` block.
...
mod smtp {
    ...
    /// Assemble a sender from the `smtp {}` block ⊕ `LYXAL_SMTP_*` env.
    ...
    pub fn build(dsl: Option<&SmtpDslConfig>) -> Result<Option<SmtpSender>, String> {
        let from_raw = dsl
            ...
            .or_else(|| env_or_file("LYXAL_SMTP_FROM"));

        // 1. Legacy composite URL wins when present.
        if let Some(url) = env_or_file("LYXAL_SMTP_URL") {
            let from = from_raw.ok_or(
                "LYXAL_SMTP_URL is set but no from address (LYXAL_SMTP_FROM / smtp { from })",
            )?;
            let transport = SmtpTransport::from_url(&url)
                .map_err(|e| format!("invalid LYXAL_SMTP_URL: {e}"))?
                .build();
            let from = parse_from(&from)?;
            tracing::info!(target: "croniq::email", "SMTP sender configured (LYXAL_SMTP_URL)");
        ...
        let Some(host) = dsl
            ...
            .or_else(|| env_or_file("LYXAL_SMTP_HOST"))
        else {
            return Ok(None);
        };
        let from = from_raw
            .ok_or("smtp host configured but no from address (LYXAL_SMTP_FROM / smtp { from })")?;

        let port: u16 = dsl
            .and_then(|d| d.port)
            .or_else(|| env_or_file("LYXAL_SMTP_PORT").and_then(|s| s.trim().parse().ok()))
            .unwrap_or(587);
        let security = dsl
            .and_then(|d| d.security.clone())
            .or_else(|| env_or_file("LYXAL_SMTP_SECURITY"))
        ...
        if let Some(user) = env_or_file("LYXAL_SMTP_USERNAME") {
            let pass = env_or_file("LYXAL_SMTP_PASSWORD").unwrap_or_default();
...
mod tests {
    ...
    fn noop_sender_returns_ok_and_does_not_leak_body() {
        ...
        let body = "Click https://croniq.test/invitations/accept?token=lyxal_inv_secret123";
    ...
    fn smtp_build_decomposed_from_dsl_only() {
        ...
        let dsl = SmtpDslConfig {
            ...
            from: Some("Lyxal <noreply@example.com>".into()),
        };
    ...
    fn smtp_build_rejects_unknown_security() {
        ...
        if crate::env_secret::env_or_file("LYXAL_SMTP_URL").is_none() {
    ...
    fn smtp_build_none_when_nothing_configured() {
        // No DSL, and (in a clean env) no LYXAL_SMTP_* ⇒ Ok(None) so the
        // caller falls back to NoopSender.
        if std::env::var_os("LYXAL_SMTP_HOST").is_none()
            && std::env::var_os("LYXAL_SMTP_URL").is_none()
            && std::env::var_os("LYXAL_SMTP_HOST_FILE").is_none()
            && std::env::var_os("LYXAL_SMTP_URL_FILE").is_none()
        {
