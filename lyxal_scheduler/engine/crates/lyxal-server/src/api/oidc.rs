...
use lyxal_auth::jwt::issue_token_pair;
use lyxal_auth::{AuthMethod, CallerType, default_scopes_for_role};
use lyxal_store::models::{OidcIdentity, OidcPendingLogin, RefreshToken, Role, User};
...
fn oidc_config(state: &ServerState, headers: &HeaderMap) -> OidcConfigResponse {
    if let Some(provider) = &state.oidc {
        ...
        // the request Host (when LYXAL_APP_URL is unset) points back at it.
