//! Jobs CRUD endpoints.
//!
//! DSL-defined jobs (from the Lyxalfile) live in `state.dsl_jobs`, not in the
//! persistent store. Read endpoints union the two sources; mutation endpoints
//! refuse to touch DSL-managed entries (the Lyxalfile owns them and would
...
use lyxal_store::models::{DslAdoption, JobDefinition, JobStatus, TriggerDefinition};
        ...
        match self {
            ...
            Self::DslManaged { job_key } => (
                ...
                Json(serde_json::json!({
                    "error": "dsl_managed",
                    "message": format!(
                        "Job '{job_key}' is managed by the Lyxalfile. Edit the file and reload, or call POST /v1/jobs/{job_key}/adopt to take ownership (requires `policy {{ dsl_adopt_on_mutate true }}`)."
                    ),
                })),
            )
                .into_response(),
...
/// Check whether `job_key` is DSL-managed. Returns `true` if the Lyxalfile
...
///
/// Collision policy:
/// - DSL-managed (Lyxalfile) → skip (Lyxalfile has precedence)
...
pub async fn handle_register(
    ...
    // DSL precedence — check the in-memory Lyxalfile map, not the store,
...
///
/// Requires `policy { dsl_adopt_on_mutate true }` in the Lyxalfile. Mirrors
        ...
        .load(std::sync::atomic::Ordering::Relaxed)
    {
        return Err((
            ...
            Json(serde_json::json!({
                ...
                "message": "DSL adoption is disabled — set `policy { dsl_adopt_on_mutate true }` in the Lyxalfile to enable",
            })),
        ));
    }
...
mod tests {
    ...
    async fn job_states_surfaces_config_error() {
        use lyxal_store::models::{JobState, JobStatus};
        let store = make_store();
        store
    ...
    fn gated_trigger_map() -> HashMap<String, lyxal_scheduler::trigger::Trigger> {
    ...
    fn make_state_with_triggers(
        store: DynStore,
        triggers: HashMap<String, lyxal_scheduler::trigger::Trigger>,
    ) -> Arc<ServerState> {
