...
use chrono::Utc;
use lyxal_auth::api_key::{generate_api_key, hash_api_key};
use lyxal_auth::context::Scope;
use lyxal_auth::crypto::unwrap_totp_secret;
use lyxal_auth::jwt::{
    issue_mfa_token, issue_token_pair, issue_totp_enroll_token, validate_mfa_token,
    validate_totp_enroll_token,
};
use lyxal_auth::password::verify_password;
use lyxal_auth::totp::{hash_recovery_code, verify_code};
use lyxal_auth::{AuthMethod, CallerContext, CallerType, default_scopes_for_role};
use lyxal_store::models::{ApiClient, ApiKey, RefreshToken};
...
/// path can call it.
fn mint_user_tokens(
    jwt_config: &lyxal_auth::jwt::JwtConfig,
    user: &lyxal_store::models::User,
...
fn verify_second_factor(
    ...
    store: &crate::store::DynStore,
    user_id: &str,
    secret_row: &lyxal_store::models::TotpSecret,
    code: &Option<String>,
