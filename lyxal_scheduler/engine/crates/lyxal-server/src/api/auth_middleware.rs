...
use lyxal_auth::api_key::{hash_api_key, hash_token};
use lyxal_auth::context::Scope;
use lyxal_auth::jwt::validate_token;
use lyxal_auth::{AuthMethod, CallerContext, CallerType};
...
pub async fn require_auth(
    ...
    let ctx = if let Some(token) = header.strip_prefix("Bearer ") {
        // PAT short-circuit: a "Bearer lyxal_pat_…" header is a PAT,
        ...
        if token.starts_with("lyxal_pat_") {
        ...
        // Explicit `Authorization: PAT lyxal_pat_…` form for clients
...
/// Resolve an `Authorization: ... lyxal_pat_…` header into a
