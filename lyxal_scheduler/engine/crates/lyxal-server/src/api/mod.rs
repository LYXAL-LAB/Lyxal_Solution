...
use lyxal_auth::CallerContext;
use lyxal_auth::context::Scope;
use lyxal_auth::jwt::JwtConfig;
use lyxal_runner::{
...
use lyxal_config::compile::{CalendarConfig, JobConfig};
use lyxal_store::models::{Execution, ExecutionFilter, ExecutionState, MaintenanceState};
...
/// #279): 10 minutes. Overridable via the Lyxalfile
...
pub struct ServerState {
    ...
    /// DSL-defined jobs (from the Lyxalfile). Shared with the scheduler task,
    /// which replaces its contents on Lyxalfile hot-reload. The REST API
    ...
    /// DSL-defined calendars (from the Lyxalfile). Same hot-reload semantics
    ...
    /// Server-wide policy flag from the Lyxalfile `policy { dsl_adopt_on_mutate ... }`
    ...
    /// `policy { strict_calendars }` from the Lyxalfile (default `true`).
    ...
    /// Path to the Lyxalfile, needed by the admin reload endpoint.
    pub config_path: Option<std::path::PathBuf>,
    /// Counters for `lyxal_config_reload_total`, incremented by both the
    ...
    /// (`lyxal_watchdog_*` metrics), incremented by the watchdog sweep task
    ...
    /// OIDC login links, from `LYXAL_APP_URL`. `None` when unset — the base
    ...
    /// `LYXAL_PASSWORD_LOGIN_ENABLED`; defaults to `true`. When `false`,
    ...
    /// `auth { totp { required bool } }` + env `LYXAL_REQUIRE_TOTP`;
    ...
    pub alerts: lyxal_config::compile::AlertsConfig,
    ...
    /// it as `lyxal_scheduler_last_tick_timestamp` so external monitoring can
    ...
    /// Resolved at boot from the Lyxalfile `pull_api {
    ...
    /// `GET /v1/jobs/states` and counted by `lyxal_config_calendar_faults`.
...
impl ServerState {
    pub fn new(
        ...
        Arc::new(Self {
            ...
            require_totp: false,
            alerts: lyxal_config::compile::AlertsConfig::default(),
            console_hub: None,
...
///
/// Precedence:
/// 1. `configured` — the operator's `LYXAL_APP_URL`. Authoritative, returned
...
pub fn server_router(state: Arc<ServerState>) -> Router {
    ...
    let authenticated = Router::new()
        ...
        // Per-job scheduling liveness (issue #250).
    ...
    if let Some(ref store) = state.store {
        ...
        items.retain(|item| {
            ...
            match store.claim_execution(id, runner_id, now) {
                ...
                Err(lyxal_store::traits::StoreError::Conflict(_))
                | Err(lyxal_store::traits::StoreError::NotFound(_)) => {
                    tracing::warn!(
...
fn concurrency_limit(item: &WorkItem) -> Option<u64> {
    item.metadata
        .get(lyxal_config::compile::MAX_CONCURRENT_METADATA_KEY)?
        .as_str()?
        .parse::<u64>()
        .ok()
        .filter(|n| *n > 0)
...
async fn handle_list_executions(
    ...
    let filter = ExecutionFilter {
        ...
        state: params.get("state").and_then(|s| match s.as_str() {
            "queued" => Some(lyxal_store::models::ExecutionState::Queued),
            "claimed" => Some(lyxal_store::models::ExecutionState::Claimed),
            "completed" => Some(lyxal_store::models::ExecutionState::Completed),
            "failed" => Some(lyxal_store::models::ExecutionState::Failed),
            "dead" => Some(lyxal_store::models::ExecutionState::Dead),
            "cancelled" => Some(lyxal_store::models::ExecutionState::Cancelled),
            _ => None,
        }),
...
/// label (`production`, `staging`, `dev`, …) read from `LYXAL_ENV`.
...
async fn handle_version() -> Json<VersionResponse> {
    ...
    let env = std::env::var("LYXAL_ENV").unwrap_or_else(|_| "unknown".into());
...
mod tests {
    ...
    async fn version_returns_build_metadata() {
        ...
        // env is read from LYXAL_ENV at request time. The test process may or
    ...
    async fn version_reads_env_var() {
        ...
        // and don't race on env vars within a single test. LYXAL_ENV is only
        ...
        // test and no other code path reads LYXAL_ENV concurrently.
        unsafe {
            std::env::set_var("LYXAL_ENV", "staging");
        }
        let (state, _rx) = make_state();
        let app = server_router(Arc::clone(&state));
    ...
    async fn auth_accepts_valid_jwt() {
        ...
        let pair = lyxal_auth::jwt::issue_token_pair(
            jwt_config,
            "test-user",
            "test-client",
            lyxal_auth::CallerType::User,
            Some("test-user"),
            Some(lyxal_auth::Role::Admin),
            lyxal_auth::AuthMethod::Password,
            &["admin".into()],
        )
        .unwrap();
    ...
    async fn trigger_inherits_dsl_runner_exec_metadata() {
        ...
        use lyxal_config::compile::RUNNER_EXEC_METADATA_KEY;
    ...
    fn make_guard_state() -> (
        ...
        let store: DynStore = crate::store::unimplemented!("SurrealStore test backend not yet available");
        let runner = AppState::new();
        let (tx, rx) = mpsc::unbounded_channel();
    ...
    async fn seed_guarded_execution(
        ...
        use lyxal_config::compile::MAX_CONCURRENT_METADATA_KEY;
    ...
    fn audit_actions(
        store: &crate::store::DynStore,
        action: &str,
    ) -> Vec<lyxal_store::models::AuditEvent> {
        store
            .audit_list(&lyxal_store::models::AuditFilter {
