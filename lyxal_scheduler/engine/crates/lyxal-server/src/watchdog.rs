...
use lyxal_config::compile::{AlertsConfig, JobConfig, RuleTrigger};
use lyxal_runner::{AppState, RunnerStatus, WorkItem};
use lyxal_store::models::{ExecutionState, JobStatus};
    ...
    async fn sweep_sla_missed(&self, now: DateTime<Utc>, result: &mut WatchdogResult) {
        ...
        for execution in &claimed {
            ...
            for rule in &self.alerts.rules {
                ...
                // channels (LYXAL_REASON=sla_miss vs dead_letter).
                let ctx = crate::alerts::FailureContext {
                    job_key: execution.job_key.clone(),
    ...
    async fn sweep_stale_claims(&self, now: DateTime<Utc>, result: &mut WatchdogResult) {
        ...
        for execution in &claimed {
            ...
            let timeout_secs = self
                ...
                .and_then(|t| lyxal_execution::retry::parse_duration(&t))
    ...
    async fn expire_queued_by_ttl(&self, now: DateTime<Utc>, result: &mut WatchdogResult) {
        ...
        let ttls: HashMap<&str, chrono::Duration> = self
            ...
            .filter_map(|(key, job)| {
                ...
                let std_dur = lyxal_execution::retry::parse_duration(ttl_str)?;
...
mod tests {
    ...
    use lyxal_config::compile::{ChannelConfig, ChannelKind, RuleConfig, RuleTrigger};
    ...
    use lyxal_store::traits::JobStore;
