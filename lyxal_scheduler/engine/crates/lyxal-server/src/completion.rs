...
use chrono::Utc;
use lyxal_bridge::job_to_execution_policy;
use lyxal_config::compile::JobConfig;
use lyxal_execution::pipeline::{ExecutionOutcome, ExecutionResult};
use lyxal_runner::{AppState, CompletionStatus};
use lyxal_store::models::{DeadLetter, Execution, ExecutionState};
use uuid::Uuid;
...
pub struct CompletionProcessor {
    ...
    /// `alerts {}` block. The `LYXAL_ON_FAILURE_CMD` back-compat path
    /// is layered in at boot via [`crate::alerts::merge_legacy_env_hook`].
    alerts: lyxal_config::compile::AlertsConfig,
    ...
    /// `LYXAL_SMTP_URL` + `LYXAL_SMTP_FROM` and the `smtp` cargo
    /// feature.
...
impl CompletionProcessor {
    pub fn new(jobs: Vec<JobConfig>, store: DynStore, runner: Arc<AppState>) -> Self {
        Self::with_alerts(
            ...
            store,
            runner,
            lyxal_config::compile::AlertsConfig::default(),
    ...
    /// sender.
    ///
    /// Used by `main.rs` to wire the Lyxalfile `alerts {}` block (plus
            ...
            ExecutionOutcome::Retry {
                ...
                match self.store.create_execution(&retry_execution) {
                    Ok(()) => {
                        ...
                        let item = lyxal_runner::WorkItem {
...
mod tests {
    ...
    use lyxal_config::compile::{DeadLetterConfig, RetryConfig, RunnerConfig};
    ...
    fn make_job(key: &str, max_attempts: u32) -> JobConfig {
        JobConfig {
            ...
            variant: None,
            description: None,
            schedule: lyxal_config::schedule::CompiledSchedule::Disabled,
            ...
            execution_mode: lyxal_config::compile::ExecutionMode::default(),
            catch_up: lyxal_config::compile::CatchUpPolicy::default(),
            queue_ttl: None,
    ...
    mod failing_store {
        ...
        impl lyxal_store::traits::Store for FailingCreateStore {}
    }
    ...
    async fn retry_persist_failure_skips_enqueue_and_dead_letters() {
        use lyxal_store::models::DeadLetterFilter;
    ...
    async fn api_job_without_dsl_entry_uses_store_fallback() {
        ...
        store
            .create_job_definition(&lyxal_store::models::JobDefinition {
                job_key: job_key.into(),
                description: None,
    ...
    async fn dead_letter_dispatches_configured_alert_rule() {
        ...
        use lyxal_store::models::{AlertDeliveryFilter, AlertDeliveryState};
    ...
    async fn dead_letter_with_empty_alerts_records_no_deliveries() {
        ...
        // `alerts {}` block and no `LYXAL_ON_FAILURE_CMD` env var is
