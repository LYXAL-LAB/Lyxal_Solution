...
/// Build a reload plan from a Lyxalfile path.
///
...
/// Atomic counters for `lyxal_config_reload_total`.
...
mod tests {
    ...
    async fn adopted_dsl_calendar_is_skipped_in_plan() {
        ...
        // the DSL output even though the Lyxalfile still defines it.
        store
            .insert_adoption(&lyxal_store::models::DslAdoption {
    ...
    async fn reload_with_broken_calendar_pauses_job_not_ungated() {
        ...
        assert_ne!(
            ...
            lyxal_scheduler::trigger::TriggerState::Paused
        );
    ...
    fn seed_store_calendar(store: &DynStore, name: &str, rules: &str) {
        store
            .create_calendar(&lyxal_store::models::CalendarDefinition {
