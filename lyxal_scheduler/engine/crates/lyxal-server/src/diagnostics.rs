...
//! `lyxal-server doctor` subcommand, and through the admin
...
pub struct DiagnosticsInput {
    /// A public base URL is explicitly pinned — via the Lyxalfile
    /// `server { app_url "…" }` directive or the `LYXAL_APP_URL` env var.
    pub app_base_url_configured: bool,
    /// A real email transport is active (i.e. not the `NoopSender`).
    pub email_delivery_active: bool,
    /// Both `LYXAL_SMTP_URL` and `LYXAL_SMTP_FROM` are present in the env.
    pub smtp_env_present: bool,
    /// This `lyxal-server` build was compiled with the `smtp` cargo feature.
...
impl DiagnosticsInput {
    ...
    /// (`LYXAL_SMTP_URL` + `LYXAL_SMTP_FROM`) and the compiled `smtp` feature
    ...
    pub fn from_runtime(
        ...
        Self {
            ...
            smtp_env_present: std::env::var("LYXAL_SMTP_URL").is_ok()
                && std::env::var("LYXAL_SMTP_FROM").is_ok(),
    ...
    if !input.email_delivery_active {
        if input.smtp_env_present && !input.smtp_feature_compiled {
            ...
            out.push(Diagnostic {
                ...
                detail: "LYXAL_SMTP_URL / LYXAL_SMTP_FROM are set, but lyxal-server was \
            ...
            out.push(Diagnostic {
                ...
                detail: "Invitation and password-reset emails are not sent. The accept / reset \
                    ...
                    .into(),
                remedy: Some(
                    "Configure SMTP via LYXAL_SMTP_URL + LYXAL_SMTP_FROM (requires a build \
    ...
    if !input.app_base_url_configured {
        out.push(Diagnostic {
            ...
            detail: "Invitation, password-reset, and OIDC login links are derived per-request \
                ...
                .into(),
            remedy: Some(
                "Pin the URL with server { app_url \"https://your.host\" } in the Lyxalfile \
                 or the LYXAL_APP_URL env var."
                    .into(),
            ),
        });
    }
        ...
        out.push(Diagnostic {
            ...
            remedy: Some(
                "Have them enrol TOTP, or temporarily set auth { totp { required false } } \
                 (LYXAL_REQUIRE_TOTP=false) so they can sign in and enrol."
                    .into(),
            ),
        });
    }
