...
//! the Lyxalfile `alerts { … }` block, and this module decides which
...
//!   - Back-compat: at boot, if `LYXAL_ON_FAILURE_CMD` is set, the
...
use lyxal_config::compile::{AlertsConfig, ChannelConfig, ChannelKind, RuleConfig, RuleTrigger};
use lyxal_store::models::{AlertDelivery, AlertDeliveryState};
use hmac::{Hmac, Mac};
use sha2::Sha256;
use uuid::Uuid;
...
/// Sentinel rule name used by the `LYXAL_ON_FAILURE_CMD` back-compat
...
pub struct FailureContext {
    ...
    /// `LYXAL_REASON` to shell channels.
    pub reason: String,
}
...
pub fn load_throttle_state(store: &DynStore, alerts: &AlertsConfig) -> ThrottleMap {
    ...
    for rule in &alerts.rules {
        ...
        let filter = lyxal_store::models::AlertDeliveryFilter {
...
fn deliver_shell(
    ...
    let result = Command::new("sh")
        .arg("-c")
        .arg(command)
        .env("LYXAL_JOB_KEY", &ctx.job_key)
        .env("LYXAL_EXECUTION_ID", &ctx.execution_id)
        .env("LYXAL_ERROR", &ctx.error)
        .env("LYXAL_ATTEMPT", ctx.attempt.to_string())
        .env("LYXAL_REASON", &ctx.reason)
        .env("LYXAL_RULE", rule_name)
        .env("LYXAL_CHANNEL", &channel.name)
        .output();
...
async fn deliver_webhook(
    ...
    let payload = WebhookPayload {
        ...
        lyxal_version: env!("CARGO_PKG_VERSION"),
    };
    ...
    let send_once = |client: reqwest::Client, body: Vec<u8>, signature: Option<String>| {
        ...
        async move {
            let mut req = client
                ...
                .header("X-Lyxal-Event", "alerts.fired")
                .header("X-Lyxal-Delivery-Id", delivery_id_hdr);
            if let Some(sig) = signature {
                req = req.header("X-Lyxal-Signature", format!("sha256={sig}"));
            }
...
fn compose_email(rule_name: &str, ctx: &FailureContext, now: DateTime<Utc>) -> (String, String) {
    let subject = format!("[Lyxal] {} failed (rule: {})", ctx.job_key, rule_name);
    let body = format!(
        "Lyxal detected a permanent job failure.\n\
         \n\
         Job:           {job_key}\n\
         Rule:          {rule_name}\n\
         Reason:        {reason}\n\
...
/// `LYXAL_ON_FAILURE_CMD` env var, if set. Logs a one-shot
...
pub fn merge_legacy_env_hook(base: AlertsConfig) -> AlertsConfig {
    let env_cmd = std::env::var("LYXAL_ON_FAILURE_CMD")
        .ok()
        .filter(|s| !s.is_empty());
    ...
    if !base.rules.is_empty() || !base.channels.is_empty() {
        tracing::warn!(
            "LYXAL_ON_FAILURE_CMD is set but the Lyxalfile has its own \
    ...
    tracing::info!(
        "LYXAL_ON_FAILURE_CMD is set without an `alerts {{ … }}` block — \
...
mod tests {
    ...
    #[cfg(unix)]
    use lyxal_store::models::AlertDeliveryFilter;
    ...
    async fn override_disable_suppresses_fire() {
        ...
        let store = make_store();
        store
            .upsert_alert_rule_override(&lyxal_store::models::AlertRuleOverride {
    ...
    /// `LYXAL_ON_FAILURE_CMD`. Without serialisation cargo's default
    ...
    fn merge_legacy_env_hook_synthesises_rule_when_env_set_and_dsl_empty() {
        let _g = env_guard();
        unsafe { std::env::set_var("LYXAL_ON_FAILURE_CMD", "true") };
        let merged = merge_legacy_env_hook(AlertsConfig::default());
        unsafe { std::env::remove_var("LYXAL_ON_FAILURE_CMD") };
    ...
    fn merge_legacy_env_hook_yields_when_dsl_block_present() {
        ...
        unsafe { std::env::set_var("LYXAL_ON_FAILURE_CMD", "echo ignored") };
    ...
    async fn webhook_delivery_sends_envelope_with_signature() {
        ...
        assert_eq!(
            cap.headers.get("x-lyxal-event").map(String::as_str),
            Some("alerts.fired")
        );
        assert!(cap.headers.contains_key("x-lyxal-delivery-id"));

        // Signature: sha256=<hex(hmac(secret, raw_body))>
        let sig_header = cap.headers.get("x-lyxal-signature").expect("signature");
        ...
        assert!(payload["lyxal_version"].is_string());
    }
    ...
    async fn webhook_unsigned_omits_signature_header() {
        ...
        assert!(
            !caps[0].headers.contains_key("x-lyxal-signature"),
            "unsigned webhook must NOT send X-Lyxal-Signature"
        );
    }
    ...
    async fn webhook_retries_once_on_5xx() {
        ...
        let id_a = caps[0].headers.get("x-lyxal-delivery-id").cloned();
        let id_b = caps[1].headers.get("x-lyxal-delivery-id").cloned();
        assert_eq!(
            id_a, id_b,
            "retry must reuse the original X-Lyxal-Delivery-Id"
        );
    }
    ...
    fn compose_email_subject_and_body_contain_key_fields() {
        ...
        assert!(subject.starts_with("[Lyxal]"));
