...
//! limited to the parts Lyxal needs:
//!   1. Discovery: GET `<issuer>/.well-known/openid-configuration`
//!   2. Authorize URL build (no PKCE — Lyxal is a confidential client
...
//! add a Lyxalfile `oidc { … }` block for IaC. Env vars:
//!   LYXAL_OIDC_ISSUER           required to enable
//!   LYXAL_OIDC_CLIENT_ID        required
//!   LYXAL_OIDC_CLIENT_SECRET    required
//!   LYXAL_OIDC_REDIRECT_URL     required
//!   LYXAL_OIDC_DEFAULT_ROLE     optional, default "viewer"
//!   LYXAL_OIDC_PROVIDER_NAME    optional, default "oidc"
//!   LYXAL_OIDC_POST_LOGIN       optional, default "/"
...
/// Build the final config by merging an optional Lyxalfile `oidc {}`
...
pub fn config_from_dsl_and_env(
    dsl: Option<&lyxal_config::compile::OidcDslConfig>,
    ...
    let issuer = dsl
        ...
        .or_else(|| std::env::var("LYXAL_OIDC_ISSUER").ok())
        .ok_or(OidcError::NotConfigured)?;
    let client_id = dsl
        .and_then(|d| d.client_id.clone())
        .or_else(|| std::env::var("LYXAL_OIDC_CLIENT_ID").ok())
        .ok_or(OidcError::NotConfigured)?;
    let client_secret =
        std::env::var("LYXAL_OIDC_CLIENT_SECRET").map_err(|_| OidcError::NotConfigured)?;
    let redirect_url = dsl
        .and_then(|d| d.redirect_url.clone())
        .or_else(|| std::env::var("LYXAL_OIDC_REDIRECT_URL").ok())
        .ok_or(OidcError::NotConfigured)?;

    let default_role_str = dsl_or_env(
        dsl.and_then(|d| d.default_role.as_ref()),
        "LYXAL_OIDC_DEFAULT_ROLE",
    );
...
pub fn config_from_env() -> Result<OidcConfig, OidcError> {
    let issuer = std::env::var("LYXAL_OIDC_ISSUER").map_err(|_| OidcError::NotConfigured)?;
    let client_id = std::env::var("LYXAL_OIDC_CLIENT_ID").map_err(|_| OidcError::NotConfigured)?;
    let client_secret =
        std::env::var("LYXAL_OIDC_CLIENT_SECRET").map_err(|_| OidcError::NotConfigured)?;
    let redirect_url =
        std::env::var("LYXAL_OIDC_REDIRECT_URL").map_err(|_| OidcError::NotConfigured)?;
    let default_role = std::env::var("LYXAL_OIDC_DEFAULT_ROLE")
        .ok()
        .as_deref()
        .and_then(|s| s.parse::<Role>().ok())
        .unwrap_or(Role::Viewer);
    let provider_name =
        std::env::var("LYXAL_OIDC_PROVIDER_NAME").unwrap_or_else(|_| "oidc".into());
    let post_login_redirect =
        std::env::var("LYXAL_OIDC_POST_LOGIN").unwrap_or_else(|_| "/".into());
    Ok(OidcConfig {
        issuer,
        client_id,
        client_secret,
