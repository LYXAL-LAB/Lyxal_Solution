//! TEMPORARILY DISABLED: SQLite has been removed.
//! This test file used SqliteStore::in_memory() / sqlite_store().
//! Re-enable once SurrealStore provides a test backend.

#![cfg(false)]

//! End-to-end integration tests for lyxal-server.
//!
//! These tests exercise the complete pipeline:
//!
//! ```text
//! Lyxalfile ──load──► SchedulerLoop::tick()
//!                           │
//!                           ▼ enqueue WorkItem
//!                      POST /v1/poll  ←── runner
//!                           │
...
use lyxal_store::{models::ExecutionState, sqlite::SqliteStore};
...
/// A fully wired in-memory Lyxal server for integration testing.
struct TestServer {
    pub state: Arc<ServerState>,
    pub scheduler: SchedulerLoop,
    pub processor: Arc<CompletionProcessor>,
    pub store: DynStore,
    completion_rx: mpsc::UnboundedReceiver<lyxal_server::CompletionEvent>,
}

impl TestServer {
    /// Create a test server from a Lyxalfile source string.
    ///
...
async fn exhausted_retries_dead_lettered() {
    ...
    let dls = srv
        .store
        .list_dead_letters(&lyxal_store::models::DeadLetterFilter {
...
async fn bulk_delete_all_clears_dead_letter_queue() {
    ...
    for _ in 0..2 {
        store
            .add_dead_letter(&lyxal_store::models::DeadLetter {
                id: uuid::Uuid::new_v4(),
    ...
    assert!(
        store
            .list_dead_letters(&lyxal_store::models::DeadLetterFilter::default())
            .unwrap()
            .is_empty(),
