...
use chrono::Utc;
use lyxal_auth::CallerType;
use lyxal_auth::jwt::{JwtConfig, issue_token_pair};
...
struct Harness {
    ...
    dsl_jobs_shared: Arc<RwLock<Vec<lyxal_config::compile::JobConfig>>>,
    #[allow(dead_code)] // exposed for test harness symmetry; new tests will exercise it
    dsl_calendars_shared: Arc<RwLock<Vec<lyxal_config::compile::CalendarConfig>>>,
    _tmp: TempDir,
...
impl Harness {
    async fn new(initial_config: &str) -> Self {
        ...
        let config_path = tmp.path().join("Lyxalfile");
...
async fn reload_preserves_api_registered_triggers() {
    ...
    h.store
        .create_trigger(&lyxal_store::models::TriggerDefinition {
