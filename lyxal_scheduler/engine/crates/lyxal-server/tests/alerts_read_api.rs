...
use lyxal_config::compile::{AlertsConfig, ChannelConfig, ChannelKind};
use lyxal_runner::AppState;
use lyxal_server::api::{ServerState, server_router};
...
fn make_state(alerts: AlertsConfig, seeded: &[AlertDelivery]) -> Arc<ServerState> {
    let store = lyxal_server::store::sqlite_store(SqliteStore::in_memory().unwrap());
    for d in seeded {
