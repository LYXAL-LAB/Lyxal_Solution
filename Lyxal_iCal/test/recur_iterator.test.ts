...
import ICAL, { Time, Recur } from '../src/index';

// Shim for legacy assertions
const assert = {
  throws: (fn: Function, regex?: RegExp) => expect(fn).toThrow(regex),
  equal: (a: any, b: any, msg?: string) => expect(a).toEqual(b),
  deepEqual: (a: any, b: any, msg?: string) => expect(a).toEqual(b),
  isTrue: (a: any, msg?: string) => expect(a).toBe(true),
  ok: (a: any, msg?: string) => expect(a).toBeTruthy()
};

describe('recur_iterator', function () {
  ...
  describe('#toJSON', function () {
    ...
    it('completed', function () {
      ...
      assert.isTrue(newIter.completed, 'new iter completed');
    });

    it('INTERVAL: mid iteration (two iterations)', function () {
      iterator.next();
      iterator.next();
    ...
    it('from the begining of iteration', function () {
      let expected = {
  ...
  describe('#normalizeByMonthDayRules', function () {
    let recur: any, iterator: any;
    ...
    it('negative and positive days', function () {
      let result = iterator.normalizeByMonthDayRules(
        2012, 2, [1, -1]
      );
  ...
  function testRRULE(ruleString: string, options: any) {
    let runner = options.only ? it.only : it;
  ...
  // @ts-ignore
  testRRULE.only = function (ruleString: string, options: any) {
    options.only = true;
    testRRULE(ruleString, options);
  };

  describe('SECONDLY/MINUTELY/HOURLY', function () {
    // Simple secondly
  ...
  describe('YEARLY', function () {
    ...
    */
    ...
    testRRULE('FREQ=YEARLY;BYYEARDAY=-275,-276,-277,-278,-279,-280,-281;BYDAY=MO', {
      dtStart: '2014-03-31T03:00:00',
      dates: [
        '2014-03-31T03:00:00',
        '2015-03-30T03:00:00',
        '2016-03-28T03:00:00'
