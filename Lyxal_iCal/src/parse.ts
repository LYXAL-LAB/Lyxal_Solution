...
const RFC6868_REPLACE_MAP: Record<string, string> = { "^'": '"', "^n": "\n", "^^": "^" };
 ...
 */
export default function parse(input: string): any | any[] {
 ...
 */
parse._handleContentLine = function (line: string, state: ParserState): void {
    ...
    if (paramPos !== -1) {
        ...
        params = parsedParams[0] as Record<string, any>;
        ...
        if (typeof parsedParams[1] === 'string') {
            ...
            parsedParamLength = (parsedParams[1] as string[]).reduce((accumulator, currentValue) => {
                return accumulator + currentValue.length;
            }, 0);
        }
        lastParamIndex = parsedParamLength + (parsedParams[2] as number) + paramPos;
        if ((lastValuePos =
        ...
        if (name === 'begin') {
            let newComponent: any[] = [value.toLowerCase(), [], []];
            if (state.stack.length === 1) {
    ...
    if (ungroupedName in state.designSet!.property) {
        ...
        if ('multiValue' in propertyDetails) {
            multiValue = propertyDetails.multiValue || false;
        }

        if ('structuredValue' in propertyDetails) {
            structuredValue = propertyDetails.structuredValue || false;
        }

        if (value && propertyDetails.detectType) {
    ...
    if (multiValue && structuredValue) {
        value = parse._parseMultiValue(value!, structuredValue as string, valueType!, [], multiValue as string, state.designSet!, structuredValue as string);
        result = [ungroupedName, params, valueType, value];
    } else if (multiValue) {
        result = [ungroupedName, params, valueType];
        parse._parseMultiValue(value!, multiValue as string, valueType!, result, null, state.designSet!, false);
    } else if (structuredValue) {
        value = parse._parseMultiValue(value!, structuredValue as string, valueType!, [], null, state.designSet!, structuredValue as string);
        result = [ungroupedName, params, valueType, value];
    } else {
 ...
 */
parse._parseValue = function (value: string, type: string, designSet: designSet, structuredValue?: string | boolean): any {
    if (type in designSet.value && 'fromICAL' in designSet.value[type]) {
        return designSet.value[type].fromICAL!(value, typeof structuredValue === 'string' ? structuredValue : undefined);
    }
    return value;
};
 ...
 * @return                    Array containing key/valye pairs of parsed parameters, the
 *                                      parsed value, and the position of the last parameter found
 */
parse._parseParameters = function (line: string, start: number, designSet: designSet): [Record<string, any>, string | string[], number] {
    let lastParam = start;
    let pos: number | false = 0;
    let delim = PARAM_NAME_DELIMITER;
    let result: Record<string, any> = {};
    let name, lcname;
    let value: string, valuePos = -1;
    let type: string, multiValue: string | boolean, mvdelim: string | boolean;
    ...
    while ((pos !== false) &&
        ...
        if (lcname in designSet.param) {
            multiValue = designSet.param[lcname].multiValue || false;
 ...
 */
parse._parseMultiValue = function (buffer: string, delim: string, type: string, result: any[], innerMulti: string | null, designSet: designSet, structuredValue?: string | boolean): any | any[] {
    let pos = 0;
    let lastPos = 0;
    let value;
    if (delim.length === 0) {
 ...
 * @param callback    The callback for each line
 */
parse._eachLine = function (buffer: string, callback: (err: any, line: string) => void): void {
