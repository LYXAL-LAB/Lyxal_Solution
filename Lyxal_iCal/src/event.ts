...
export class Event {
    ...
    rangeExceptions: [number, string][]; // Stores [unixTime, recurrenceIdString]
     ...
     */
    constructor(component?: Component | EventOptions | null, options?: EventOptions) {
        if (!(component instanceof Component)) {
            options = component as EventOptions;
            component = null;
        }
    ...
    getRecurrenceTypes(): Record<string, boolean> {
        ...
        for (; i < len; i++) {
            let value = rules[i].getFirstValue() as any;
     ...
     */
    _setTime(propName: string, time: Time): void {
...
function compareRangeException(a: [number, string], b: [number, string]): number {
    if (a[0] > b[0]) return 1;
    if (b[0] > a[0]) return -1;
    return 0;
}
