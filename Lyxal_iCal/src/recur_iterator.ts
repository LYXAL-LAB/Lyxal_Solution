...
import type { byParts, weekDay } from "./types";
...
export class RecurIterator {
    static _indexMap: Record<string, number> = {
        "BYSECOND": 0,
        "BYMINUTE": 1,
        "BYHOUR": 2,
        "BYDAY": 3,
    ...
    static _expandMap: Record<string, number[]> = {
     ...
     */
    by_data: any; // Using any for flexibility as it mirrors rule.parts but can be modified
     ...
     */
    constructor(options: RecurIteratorOptions) {
        // Initialize properties to satisfy TypeScript before calling fromData
        // These will be overwritten by fromData
        this.rule = options.rule as Recur;
     ...
     * @param options                The iterator options
     */
    fromData(options: RecurIteratorOptions): void {
        ...
        if (this.rule.freq == "MONTHLY") {
            if (this.has_by_data("BYDAY")) {
                ...
                for (let bydow of this.by_data.BYDAY) {
                    ...
                    let dayOfMonth = this.last.nthWeekDay(dow as weekDay, pos);
                    ...
                    if (dayOfMonth > daysInMonth || dayOfMonth <= 0) {
                        ...
                        while (dayOfMonth > daysInMonth || dayOfMonth <= 0) {
                            ...
                            dayOfMonth = this.last.nthWeekDay(dow as weekDay, pos);
                        }
                    }
     ...
     * @return {Time}
     */
    next(again: boolean = false): Time | null {
    ...
    increment_second(inc: number): void {
     ...
     */
    normalizeByMonthDayRules(year: number, month: number, rules: number[]): number[] {
        ...
        for (; ruleIdx < len; ruleIdx++) {
            rule = rules[ruleIdx]; // Assuming rules are already numbers, if string parse them
            // In TS conversion, we assume rules are numbers as per types.
            // But original code had parseInt.
            // If rules can be strings, we should parse.
            // Based on types.ts, BYMONTHDAY is number[].
     ...
     */
    _byDayAndMonthDay(isInit?: boolean): number {
        let byMonthDay: number[]; // setup in initMonth
        let byDay = this.by_data.BYDAY;

        let date: number;
        let dateIdx = 0;
        let dateLen: number; // setup in initMonth
        ...
        while (!dataIsValid && monthsCounter) {
            ...
            for (let dayIdx = 0; dayIdx < dayLen; dayIdx++) {
                ...
                if (this.last.isNthWeekDay(dow as weekDay, pos)) {
    ...
    next_month(): number {
        ...
        if (this.has_by_data("BYDAY") && this.has_by_data("BYMONTHDAY")) {
            ...
            this.increment_month();
            let daysInMonth = Time.daysInMonth(this.last.month, this.last.year);
            if (this.by_data.BYMONTHDAY && this.by_data.BYMONTHDAY[0] > daysInMonth) {
                data_valid = 0;
            } else if (this.by_data.BYMONTHDAY) {
     ...
     */
    ruleDayOfWeek(dow: string, aWeekStart?: weekDay): [number, number] {
    ...
    next_generic(aRuleType: string, aInterval: string, aDateAttr: string, aFollowingAttr: string, aPreviousIncr?: string): number {
        let has_by_rule = (aRuleType in this.by_data);
        let this_freq = (this.rule.freq == aInterval);
        let end_of_data = 0;

        if (aPreviousIncr && (this as any)[aPreviousIncr]() == 0) {
            return end_of_data;
        }
        ...
        if (has_by_rule) {
            ...
            (this.last as any)[aDateAttr] = dta[this.by_indices[aRuleType]];
        } else if (this_freq) {
            (this as any)["increment_" + aDateAttr](this.rule.interval);
        }

        if (has_by_rule && end_of_data && this_freq) {
            (this as any)["increment_" + aFollowingAttr](1);
        }

        return end_of_data;
    }

    increment_monthday(inc: number): void {
        for (let i = 0; i < inc; i++) {
            let daysInMonth = Time.daysInMonth(this.last.month, this.last.year);
            this.last.day++;
    ...
    increment_generic(inc: number, aDateAttr: string, aFactor: number, aNextIncrement: string): void {
        (this.last as any)[aDateAttr] += inc;
        let nextunit = trunc((this.last as any)[aDateAttr] / aFactor);
        (this.last as any)[aDateAttr] %= aFactor;
        if (nextunit != 0) {
            (this as any)["increment_" + aNextIncrement](nextunit);
        }
    }

    has_by_data(aRuleType: string): boolean {
        return (aRuleType in this.rule.parts);
    }

    expand_year_days(aYear: number): void {
        let t = new Time();
        this.days = [];
        ...
        for (let part of rules) {
            if (part in this.rule.parts) {
                parts[part] = (this.rule.parts as any)[part];
            }
        }

        if ("BYMONTH" in parts && "BYWEEKNO" in parts) {
            let valid = 1;
            let validWeeks: Record<number, number> = {};
            t.year = aYear;
            t.isDate = true;
    ...
    expand_by_day(aYear: number): number[] {
    ...
    is_day_in_byday(tt: Time): number {
        if (this.by_data.BYDAY) {
            for (let day of this.by_data.BYDAY) {
                ...
                if ((pos == 0 && dow == this_dow) ||
                    (tt.nthWeekDay(dow as weekDay, pos) == tt.day)) {
                    return 1;
                }
            }
        }
     ...
     *
     * @private
     * @param aPos position to check for.
     * @return false unless BYSETPOS rules exist
    ...
    sort_byday_rules(aRules: string[]): void {
    ...
    check_contract_restriction(aRuleType: string, v: number | string): boolean {
    ...
    setup_defaults(aRuleType: string, req: string, deftime: number): number {
        let indexMapValue = RecurIterator._indexMap[aRuleType];
        let ruleMapValue = RecurIterator._expandMap[this.rule.freq!][indexMapValue];
     ...
     * @return {Object}
     */
    toJSON(): RecurIteratorOptions {
        let result: RecurIteratorOptions = Object.create(null);
...
export interface RecurIteratorOptions {
    rule: Recur | any;
    dtstart: Time | any;
    initialized?: boolean;
    by_data?: any;
