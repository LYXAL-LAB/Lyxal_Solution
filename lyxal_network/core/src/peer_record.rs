use lyxal_network_identity::{Keypair, PeerId, SigningError};
use quick_protobuf::{BytesReader, Writer};
use web_time::SystemTime;

use crate::{proto, signed_envelope, signed_envelope::SignedEnvelope, DecodeError, Multiaddr};

// Legacy constants for backward compatibility with existing Rust lyxal_network deployments.
const LEGACY_PAYLOAD_TYPE: &str = "/lyxal_network/routing-state-record";
const LEGACY_DOMAIN_SEP: &str = "lyxal_network-routing-state";

// Standard constants for cross-implementation compatibility with Go/JS lyxal_network.
// Defined in https://github.com/multiformats/multicodec/blob/master/table.csv
// and https://github.com/lyxal_network/specs/blob/master/RFC/0002-signed-envelopes.md.
const STANDARD_PAYLOAD_TYPE: &[u8] = &[0x03, 0x01];
const STANDARD_DOMAIN_SEP: &str = "lyxal_network-peer-record";
...
/// envelope. For more information see RFC0003 of the lyxal_network specifications: <https://github.com/lyxal_network/specs/blob/master/RFC/0003-routing-records.md>
///
/// ## Cross-Implementation Compatibility
///
/// This implementation provides two formats:
/// - **Legacy format** (default methods): Compatible with existing Rust lyxal_network deployments.
...
/// non-Rust lyxal_network implementations.
...
impl PeerRecord {
    ...
    /// Rust lyxal_network deployments.
    ///
    ...
    ///
    /// For cross-implementation compatibility with Go/JS lyxal_network, use
    ...
    /// format.
    ///
    /// Uses the standard lyxal_network-peer-record format for cross-implementation compatibility
    /// with Go and JavaScript lyxal_network implementations.
    ///
    /// If this function succeeds, the [`SignedEnvelope`] contained a peer record with a valid
    /// signature and can hence be considered authenticated.
    ...
    /// Rust lyxal_network deployments.
    ///
    /// This is the same key that is used for authenticating every lyxal_network connection of your
    /// application, i.e. what you use when setting up your [`crate::transport::Transport`].
    ///
    /// For cross-implementation compatibility with Go/JS lyxal_network, use [`Self::new_interop`].
...
pub enum FromEnvelopeError {
    ...
    InvalidPeerId(#[from] lyxal_network_identity::ParseError),
