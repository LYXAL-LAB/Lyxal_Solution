...
use lyxal_network_swarm::{ConnectionHandler, NetworkBehaviour, StreamProtocol};
use web_time::Duration;

macro_rules! gen_behaviour {
    ($($name:ident {$($field:ident),*};)*) => {$(
        #[derive(lyxal_network_swarm::NetworkBehaviour, Default)]
...
impl<T: BigBehaviour + NetworkBehaviour> BigBehaviour for lyxal_network_swarm::Swarm<T> {
    fn behaviours(&mut self) -> &mut [SpinningBehaviour] {
        self.behaviour_mut().behaviours()
    }
}

fn new_swarm<T: NetworkBehaviour>(beh: T) -> lyxal_network_swarm::Swarm<T> {
    let keypair = lyxal_network_identity::Keypair::generate_ed25519();
    lyxal_network_swarm::Swarm::new(
        MemoryTransport::default()
            .upgrade(multistream_select::Version::V1)
            .authenticate(lyxal_network_plaintext::Config::new(&keypair))
            .multiplex(lyxal_network_yamux::Config::default())
            .boxed(),
        beh,
        keypair.public().to_peer_id(),
        lyxal_network_swarm::Config::without_executor().with_idle_connection_timeout(Duration::MAX),
    )
}
...
impl NetworkBehaviour for SpinningBehaviour {
    ...
    fn handle_established_inbound_connection(
        ...
        _peer: lyxal_network_identity::PeerId,
    ...
    fn on_swarm_event(&mut self, _: lyxal_network_swarm::FromSwarm) {}

    fn on_connection_handler_event(
        &mut self,
        _peer_id: lyxal_network_identity::PeerId,
        _connection_id: lyxal_network_swarm::ConnectionId,
    ...
    ) -> std::task::Poll<lyxal_network_swarm::ToSwarm<Self::ToSwarm, lyxal_network_swarm::THandlerInEvent<Self>>>
    {
        if self.finished && !self.emitted {
            self.emitted = true;
            std::task::Poll::Ready(lyxal_network_swarm::ToSwarm::GenerateEvent(FinishedSpinning))
        } else {
...
impl ConnectionHandler for SpinningHandler {
    ...
    fn listen_protocol(&self) -> lyxal_network_swarm::SubstreamProtocol<Self::InboundProtocol> {
        lyxal_network_swarm::SubstreamProtocol::new(Upgrade(self.protocols), ())
    }
    ...
    fn poll(
        ...
        lyxal_network_swarm::ConnectionHandlerEvent<Self::OutboundProtocol, (), Self::ToBehaviour>,
    > {
        ...
        std::task::Poll::Ready(lyxal_network_swarm::ConnectionHandlerEvent::NotifyBehaviour(
            FinishedSpinning,
        ))
    }
    ...
    fn on_connection_event(
        &mut self,
        _event: lyxal_network_swarm::handler::ConnectionEvent<
...
impl OutboundUpgrade<lyxal_network_swarm::Stream> for Upgrade {
    type Output = lyxal_network_swarm::Stream;
    type Error = Infallible;
    type Future = futures::future::Ready<Result<Self::Output, Self::Error>>;

    fn upgrade_outbound(self, s: lyxal_network_swarm::Stream, _: Self::Info) -> Self::Future {
        futures::future::ready(Ok(s))
    }
}

impl InboundUpgrade<lyxal_network_swarm::Stream> for Upgrade {
    type Output = lyxal_network_swarm::Stream;
    type Error = Infallible;
    type Future = futures::future::Ready<Result<Self::Output, Self::Error>>;

    fn upgrade_inbound(self, s: lyxal_network_swarm::Stream, _: Self::Info) -> Self::Future {
        futures::future::ready(Ok(s))
    }
}
