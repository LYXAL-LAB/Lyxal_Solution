...
use build::*;
use dns::*;
use lyxal_network_core::{muxing::StreamMuxerBox, Transport};
...
pub trait AuthenticatedMultiplexedTransport:
    Transport<
        ...
        Output = (lyxal_network_identity::PeerId, StreamMuxerBox),
    > + Send
    + Unpin
    + 'static
{
...
where
    T: Transport<Output = (lyxal_network_identity::PeerId, StreamMuxerBox)> + Send + Unpin + 'static,
