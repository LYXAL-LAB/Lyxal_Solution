    ...
    > {
        Ok(SwarmBuilder {
            ...
            phase: WebsocketPhase {
                transport: lyxal_network_dns::tokio::Transport::system(self.phase.transport)?,
            },
        })
    }
}
...
impl<T: AuthenticatedMultiplexedTransport> SwarmBuilder<super::provider::Tokio, DnsPhase<T>> {
    ...
    ) -> SwarmBuilder<super::provider::Tokio, WebsocketPhase<impl AuthenticatedMultiplexedTransport>>
    {
        SwarmBuilder {
            ...
            phantom: PhantomData,
            phase: WebsocketPhase {
                transport: lyxal_network_dns::tokio::Transport::custom(self.phase.transport, cfg, opts),
            },
        }
    }
}
