...
pub struct Stream {
    ...
    inner: SendWrapper<lyxal_network_webrtc_utils::Stream<PollDataChannel>>,
}

pub(crate) type DropListener = SendWrapper<lyxal_network_webrtc_utils::DropListener<PollDataChannel>>;

impl Stream {
    pub(crate) fn new(data_channel: RtcDataChannel) -> (Self, DropListener) {
        let (inner, drop_listener) =
            lyxal_network_webrtc_utils::Stream::new(PollDataChannel::new(data_channel));
