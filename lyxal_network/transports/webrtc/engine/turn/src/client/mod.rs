...
impl Client {
    ...
    pub async fn allocate(&self) -> Result<Arc<dyn Conn + Send + Sync + 'static>> {
        let config = {
            let mut ci = self.client_internal.lock().await;
            ci.allocate().await? // This should return RelayConnConfig
        };
        let relay_conn_instance = RelayConn::new(Arc::clone(&self.client_internal), config).await;
        Ok(Arc::new(relay_conn_instance))
    }
