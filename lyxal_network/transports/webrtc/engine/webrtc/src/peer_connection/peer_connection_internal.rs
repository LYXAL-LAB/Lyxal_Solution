...
use crate::rtp_transceiver::{PayloadType, create_stream_info};
...
impl PeerConnectionInternal {
    ...
    pub(crate) fn make_negotiation_needed_trigger(
        &self,
    ) -> impl Fn() -> Pin<Box<dyn Future<Output = ()> + Send + Sync>> + Send + Sync + 'static {
        let params = self.create_negotiation_needed_params();
        move || {
            // Cloning params to ensure it's owned and 'static
            let params_owned = params.clone();
            Box::pin(async move {
                RTCPeerConnection::do_negotiation_needed(params_owned).await;
            })
        }
    }
