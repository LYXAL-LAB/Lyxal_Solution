...
impl Endpoint {
    ...
    pub(crate) fn url(&self) -> String {
        ...
        if self.is_ipv6 {
            format!("https://[{host}]:{port}/.well-known/lyxal_network-webtransport?type=noise")
        } else {
            format!("https://{host}:{port}/.well-known/lyxal_network-webtransport?type=noise")
        }
    }
...
mod tests {
    ...
    fn valid_webtransport_multiaddr() {
        ...
        assert_eq!(
            endpoint.url(),
            "https://127.0.0.1:44874/.well-known/lyxal_network-webtransport?type=noise"
        );
    }
    ...
    fn ipv6_webtransport() {
        ...
        assert_eq!(
            endpoint.url(),
            "https://[::1]:44874/.well-known/lyxal_network-webtransport?type=noise"
        );
    }

    #[test]
    fn dns_webtransport() {
        let addr = Multiaddr::from_str("/dns/lyxal_network.io/udp/44874/quic-v1/webtransport/certhash/uEiCaDd1Ca1A8IVJ3hsIxIyi11cwxaDKqzVrBkGJbKZU5ng/certhash/uEiDv-VGW8oXxui_G_Kqp-87YjvET-Hr2qYAMYPePJDcsjQ/p2p/12D3KooWR7EfNv5SLtgjMRjUwR8AvNu3hP4fLrtSa9fmHHXKYWNG").unwrap();
        let endpoint = Endpoint::from_multiaddr(&addr).unwrap();

        assert_eq!(endpoint.host, "lyxal_network.io");
        assert_eq!(endpoint.port, 44874);
        assert_eq!(
            endpoint.url(),
            "https://lyxal_network.io:44874/.well-known/lyxal_network-webtransport?type=noise"
        );
    }
}
