...
unsafe fn v4_socket_from_adapter(unicast_addr: &IpAdapterUnicastAddress) -> SocketAddrV4 {
    ...
    let v4_addr = Ipv4Addr::new(
        unsafe { *sin_addr.S_addr() as u8 },
        unsafe { (*sin_addr.S_addr() >> 8) as u8 },
        unsafe { (*sin_addr.S_addr() >> 16) as u8 },
        unsafe { (*sin_addr.S_addr() >> 24) as u8 },
    );
...
unsafe fn v6_socket_from_adapter(unicast_addr: &IpAdapterUnicastAddress) -> SocketAddrV6 {
    ...
    let in6_addr: SOCKADDR_IN6 = unsafe { *sock_addr6 };

    let v6_addr = unsafe { (*in6_addr.sin6_addr.u.Word()).into() };

    SocketAddrV6::new(v6_addr, 0, in6_addr.sin6_flowinfo, unsafe {
...
unsafe fn local_ifaces_with_buffer(buffer: &mut Vec<u8>) -> io::Result<()> {
    ...
    match ret_code {
        ...
        ERROR_BUFFER_OVERFLOW => {
            ...
            unsafe { local_ifaces_with_buffer(buffer) }
        }
    ...
    while !adapter_addr.is_null() {
        ...
        while !unicast_addr.is_null() {
            ...
            if curr_unicast_addr.dad_state != IpDadState::IpDadStateDeprecated {
                if unsafe { is_ipv4_enabled(curr_unicast_addr) } {
                    adapter_addresses.push(Interface {
                        name: "".to_string(),
                        kind: Kind::Ipv4,
                        addr: Some(SocketAddr::V4(unsafe {
                            v4_socket_from_adapter(curr_unicast_addr)
                        })),
                        mask: None,
                        hop: None,
                    });
                } else if unsafe { is_ipv6_enabled(curr_unicast_addr) } {
                    let mut v6_sock = unsafe { v6_socket_from_adapter(curr_unicast_addr) };
...
pub fn ifaces() -> Result<Vec<Interface>, ::std::io::Error> {
    ...
    unsafe {
        unsafe { local_ifaces_with_buffer(&mut adapters_list)? };

        Ok(unsafe { map_adapter_addresses(adapters_list.as_ptr() as *const IpAdapterAddresses) })
    }
}
...
unsafe fn is_ipv4_enabled(unicast_addr: &IpAdapterUnicastAddress) -> bool {
    if unicast_addr.length != 0 {
        let socket_addr = &unicast_addr.address;
        let sa_family = unsafe { (*socket_addr.lpSockaddr).sa_family };
