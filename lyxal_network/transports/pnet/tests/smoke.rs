...
use lyxal_network_pnet::{PnetConfig, PreSharedKey};
use lyxal_network_swarm::{dummy, Config, NetworkBehaviour, Swarm, SwarmEvent};
...
async fn can_establish_connection_tcp() {
    can_establish_connection_inner_with_timeout(
        lyxal_network_tcp::tokio::Transport::default,
...
async fn can_establish_connection_websocket() {
    can_establish_connection_inner_with_timeout(
        || lyxal_network_websocket::Config::new(lyxal_network_tcp::tokio::Transport::default()),
...
async fn can_establish_connection_inner_with_timeout<F, T>(
    ...
    <T as lyxal_network_core::Transport>::Error: Send + Sync + 'static,
    <T as lyxal_network_core::Transport>::Output: AsyncRead + AsyncWrite + Send + Unpin,
    <T as lyxal_network_core::Transport>::ListenerUpgrade: Send,
    <T as lyxal_network_core::Transport>::Dial: Send,
{
...
where
    ...
    <T as lyxal_network_core::Transport>::Dial: Send,
{
    let identity = lyxal_network_identity::Keypair::generate_ed25519();
    let transport = transport
        .and_then(move |socket, _| pnet.handshake(socket))
        .upgrade(Version::V1)
        .authenticate(lyxal_network_noise::Config::new(&identity).unwrap())
