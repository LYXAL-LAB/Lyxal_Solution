...
#[derive(Debug, Parser)]
#[command(name = "lyxal_network server", about = "A lyxal_network server binary.")]
struct Opts {
    ...
    /// Whether to run the lyxal_network Kademlia protocol and join the IPFS DHT.
    #[arg(long)]
    enable_kademlia: bool,

    /// Whether to run the lyxal_network Autonat protocol.
    #[arg(long)]
    enable_autonat: bool,
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut swarm = lyxal_network::SwarmBuilder::with_existing_identity(local_keypair)
        .with_tokio()
        .with_tcp(
    ...
    for address in &config.addresses.swarm {
        match swarm.listen_on(address.clone()) {
            Ok(_) => {}
            Err(e @ lyxal_network::TransportError::MultiaddrNotSupported(_)) => {
