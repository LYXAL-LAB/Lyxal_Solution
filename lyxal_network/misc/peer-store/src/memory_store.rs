...
use lyxal_network_swarm::{behaviour::ConnectionEstablished, DialError, FromSwarm};

use super::Store;

/// Event emitted from the [`MemoryStore`] to the [`Swarm`](lyxal_network_swarm::Swarm).
#[derive(Debug, Clone)]
pub enum Event {
...
pub struct MemoryStore<T = ()> {
    ...
    /// Events to emit to [`Behaviour`](crate::Behaviour) and [`Swarm`](lyxal_network_swarm::Swarm).
...
impl<T> MemoryStore<T> {
    ...
    /// [`Swarm::add_peer_address`](lyxal_network_swarm::Swarm::add_peer_address).
    ///
...
mod test {
    ...
    use lyxal_network::identify;
    use lyxal_network_core::{multiaddr::Protocol, Multiaddr, PeerId};
    use lyxal_network_swarm::{NetworkBehaviour, Swarm, SwarmEvent};
    ...
    fn update_address_on_connect() {
        ...
        rt.block_on(async {
            ...
            swarm2
                .dial(
                    lyxal_network_swarm::dial_opts::DialOpts::peer_id(swarm1_peer_id)
                        .condition(lyxal_network_swarm::dial_opts::PeerCondition::Always)
