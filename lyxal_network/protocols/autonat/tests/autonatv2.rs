...
use lyxal_network_autonat::v2::{
    client::{self, Config},
    server,
};
use lyxal_network_core::{multiaddr::Protocol, transport::TransportError, Multiaddr};
...
#[tokio::test]
async fn dial_back_to_non_lyxal_network() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env())
        .try_init();
    let (mut alice, mut bob) = bootstrap().await;
...
async fn new_server() -> Swarm<CombinedServer> {
    let mut node = Swarm::new_ephemeral_tokio(|identity| CombinedServer {
        autonat: lyxal_network_autonat::v2::server::Behaviour::default(),
        identify: lyxal_network_identify::Behaviour::new(lyxal_network_identify::Config::new(
            "/lyxal_network-test/1.0.0".into(),
...
async fn new_client() -> Swarm<CombinedClient> {
    let mut node = Swarm::new_ephemeral_tokio(|identity| CombinedClient {
        autonat: lyxal_network_autonat::v2::client::Behaviour::new(
            OsRng,
...
struct CombinedServer {
    autonat: lyxal_network_autonat::v2::server::Behaviour,
    identify: lyxal_network_identify::Behaviour,
}

#[derive(lyxal_network_swarm::NetworkBehaviour)]
#[behaviour(prelude = "lyxal_network_swarm::derive_prelude")]
struct CombinedClient {
    autonat: lyxal_network_autonat::v2::client::Behaviour,
    identify: lyxal_network_identify::Behaviour,
}

async fn new_dummy() -> Swarm<lyxal_network_identify::Behaviour> {
    let mut node = Swarm::new_ephemeral_tokio(|identity| {
        lyxal_network_identify::Behaviour::new(lyxal_network_identify::Config::new(
            "/lyxal_network-test/1.0.0".into(),
            identity.public().clone(),
        ))
    });
