...
This module provides a generic [`NetworkBehaviour`](lyxal_network_swarm::NetworkBehaviour) for stream-oriented protocols.
Streams are the fundamental primitive of lyxal_network and all other protocols are implemented using streams.
In contrast to other [`NetworkBehaviour`](lyxal_network_swarm::NetworkBehaviour)s, this module takes a different design approach.
...
To accept streams for a particular [`StreamProtocol`](lyxal_network_swarm::StreamProtocol) using this module, use [`Control::accept`]:

### Example

```rust,no_run
# fn main() {
# use lyxal_network_swarm::{Swarm, StreamProtocol};
# use lyxal_network_stream as stream;
