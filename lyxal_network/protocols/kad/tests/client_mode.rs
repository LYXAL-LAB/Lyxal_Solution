...
use lyxal_network_kad::{store::MemoryStore, Behaviour, Config, Event, Mode};
use lyxal_network_swarm::{Swarm, SwarmEvent};
...
async fn two_servers_add_each_other_to_routing_table() {
    ...
    match lyxal_network_swarm_test::drive(&mut server1, &mut server2).await {
        (
...
async fn adding_an_external_addresses_activates_server_mode_on_existing_connections() {
    ...
    match lyxal_network_swarm_test::drive(&mut client, &mut server).await {
...
async fn set_client_to_server_mode() {
    ...
    assert!(info
        .protocols
        .iter()
        .all(|proto| lyxal_network_kad::PROTOCOL_NAME.ne(proto)));
    ...
    assert!(info
        .protocols
        .iter()
        .any(|proto| lyxal_network_kad::PROTOCOL_NAME.eq(proto)));
}
    ...
    fn new(k: identity::Keypair) -> Self {
        ...
        Self {
            ...
            kad: Behaviour::with_config(
                ...
                Config::new(lyxal_network_kad::PROTOCOL_NAME),
            ),
        }
    }
}
