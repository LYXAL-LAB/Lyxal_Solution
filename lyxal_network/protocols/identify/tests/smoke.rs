...
async fn periodic_identify() {
    ...
    match lyxal_network_swarm_test::drive(&mut swarm1, &mut swarm2).await {
        (
...
async fn identify_push() {
    ...
    let ([e1, e2], [e3, e4]) = lyxal_network_swarm_test::drive(&mut swarm1, &mut swarm2).await;
    ...
    let swarm1_received_info = match lyxal_network_swarm_test::drive(&mut swarm1, &mut swarm2).await {
