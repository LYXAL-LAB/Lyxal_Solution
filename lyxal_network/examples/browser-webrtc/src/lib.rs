...
use js_sys::Date;
use lyxal_network::{core::Multiaddr, ping, swarm::SwarmEvent};
use lyxal_network_webrtc_websys as webrtc_websys;
use wasm_bindgen::prelude::*;
use web_sys::{Document, HtmlElement};

#[wasm_bindgen]
pub async fn run(lyxal_network_endpoint: String) -> Result<(), JsError> {
    ...
    body.append_p(&format!(
        "Let's ping the lyxal_network server over WebRTC for {:?}:",
        ping_duration
    ))?;
    ...
    let addr = lyxal_network_endpoint.parse::<Multiaddr>()?;
    ...
    loop {
        match swarm.next().await.unwrap() {
            ...
            SwarmEvent::ConnectionClosed {
                ...
                if let lyxal_network::swarm::ConnectionError::KeepAliveTimeout = cause {
