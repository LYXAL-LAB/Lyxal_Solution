...
async fn main() -> anyhow::Result<()> {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("browser_webrtc_example=debug,lyxal_network_webrtc=info,lyxal_network_ping=debug")
        .try_init();
...
pub(crate) async fn serve(lyxal_network_transport: Multiaddr) {
    let Some(Protocol::Ip4(listen_addr)) = lyxal_network_transport.iter().next() else {
    ...
    let server = Router::new()
        ...
        .with_state(Libp2pEndpoint(lyxal_network_transport))
        .layer(
            // allow cors
            CorsLayer::new()
                .allow_origin(Any)
...
async fn get_index(
    State(Libp2pEndpoint(lyxal_network_endpoint)): State<Libp2pEndpoint>,
    ...
    let html = std::str::from_utf8(&content)
        ...
        .replace("__LIBP2P_ENDPOINT__", &lyxal_network_endpoint.to_string());
