...
The example showcases how to run a p2p network with **lyxal_network** and collect metrics using `lyxal_network-metrics` as well as span data via `opentelemetry`.
It sets up multiple nodes in the network and measures various metrics, such as `lyxal_network_ping`, to evaluate the network's performance.
...
3. Open a third terminal and run the following command to retrieve the metrics from either the first or second node:
   ...
   Make sure to check the `lyxal_network_ping` metrics, which should have a value greater than zero (`>0`).
...
`lyxal_network` consistently applies spans to the following functions:
...
If you just wanted to see the spans of the `lyxal_network_ping` crate, you can filter like this:

```
RUST_LOG=lyxal_network_ping[ConnectionHandler::poll]=trace
```

## Conclusion

This example demonstrates how to utilize the `lyxal_network-metrics` crate to collect and analyze metrics in a lyxal_network network.
