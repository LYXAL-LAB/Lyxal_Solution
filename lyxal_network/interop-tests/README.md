...
To run this test against all released lyxal_network versions you'll need to have the
[lyxal_network/test-plans](https://github.com/lyxal_network/test-plans) checked out. Then do
the following (from the root directory of this repository):

1. Build the image: `docker build -t rust-lyxal_network-head . -f interop-tests/Dockerfile`.
2. Build the images for all released versions in `lyxal_network/test-plans`: `(cd <path to >/lyxal_network/test-plans/multidim-interop/ && make)`.
3. Run the test:
```
RUST_LIBP2P="$PWD"; (cd <path to >/lyxal_network/test-plans/multidim-interop/ && npm run test -- --extra-version=$RUST_LIBP2P/interop-tests/ping-version.json --name-filter="rust-lyxal_network-head")
```
