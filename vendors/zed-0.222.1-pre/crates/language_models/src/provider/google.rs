...
use google_ai::{
    FunctionDeclaration, GenerateContentResponse, GoogleAiError, GoogleModelMode, Part,
    SystemInstruction, ThinkingConfig, UsageMetadata,
};
...
impl GoogleLanguageModelProvider {
    ...
    fn create_language_model(&self, model: google_ai::Model) -> Arc<dyn LanguageModel> {
        Arc::new(GoogleLanguageModel {
            ...
            request_limiter: RateLimiter::new_with_throttle(4, Duration::from_secs(1)),
        })
    }
        ...
        models
            ...
            .map(|model| {
                Arc::new(GoogleLanguageModel {
                    ...
                    state: self.state.clone(),
                    http_client: self.http_client.clone(),
                    request_limiter: RateLimiter::new_with_throttle(
                        4,
...
impl LanguageModel for GoogleLanguageModel {
    ...
    fn count_tokens(
        ...
        async move {
            ...
            .await
            .map_err(|err| {
                if let Some(google_ai_error) = err.downcast_ref::<GoogleAiError>() {
                    let retry_after = google_ai_error.retry_delay();
                    return LanguageModelCompletionError::from_http_status(
                        PROVIDER_NAME,
                        google_ai_error.status,
                        google_ai_error.body.clone(),
                        retry_after,
                    );
                }
        ...
        content
            ...
            .flat_map(|content| match content {
                ...
                language_model::MessageContent::Thinking { text, signature } => {
                    if !text.is_empty() || signature.as_ref().is_some_and(|s| !s.is_empty()) {
...
impl GoogleEventMapper {
    ...
    pub fn map_event(
        ...
        if let Some(error) = event.error {
            ...
            events.push(Err(LanguageModelCompletionError::from_http_status(
                PROVIDER_NAME,
                http_client::http::StatusCode::from_u16(error.code as u16)
                    .unwrap_or(http_client::http::StatusCode::INTERNAL_SERVER_ERROR),
                error.message,
                retry_delay,
            )));
            return events;
        ...
        if let Some(candidates) = event.candidates {
            for candidate in candidates {
                if let Some(finish_reason) = candidate.finish_reason.as_deref() {
                    let new_reason = match finish_reason {
                        "STOP" => StopReason::EndTurn,
                        "MAX_TOKENS" => StopReason::MaxTokens,
                        "SAFETY" | "RECITATION" | "PROHIBITED_CONTENT" | "SPII" => {
                            StopReason::Refusal
                        }
                        "OTHER" => StopReason::EndTurn,
                        "BLOCKLIST" => StopReason::Refusal,
                        "MALFORMED_FUNCTION_CALL" => StopReason::EndTurn,
                        _ => {
                    ...
                    // If we already detected a ToolUse, don't let a "STOP" override it.
                    // Google often sends "STOP" as the finish reason even when it called tools.
                    if self.stop_reason != StopReason::ToolUse || new_reason != StopReason::EndTurn {
                ...
                candidate
                    ...
                    .for_each(|part| match part {
                        Part::TextPart(text_part) => {
                            if let Some(signature) = text_part.thought_signature {
