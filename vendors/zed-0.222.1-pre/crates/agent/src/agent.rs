...
impl NativeAgent {
    ...
    fn register_session(
        ...
        let (session_id, title, project, action_log, prompt_capabilities_rx, thread_model) = {
    ...
    fn handle_thread_token_usage_updated(
        ...
        if let Some(usage) = usage.0.as_ref() {
            if usage.max_tokens > 0 {
                let usage_ratio = usage.used_tokens as f64 / usage.max_tokens as f64;
                if usage_ratio > 0.95 {
                    log::info!(
                        "Thread {} reached 95% token usage, triggering immediate summarization",
                        thread.read(cx).id()
                    );
                    ...
                    if !thread.read(cx).is_busy() {
                        log::info!(
                            "Thread {} reached 80% token usage, triggering background summarization",
                            thread.read(cx).id()
                        );
                        thread.update(cx, |thread, cx| {
                            let _ = thread.summary(cx);
    ...
    fn handle_models_updated_event(
        ...
        for session in self.sessions.values_mut() {
            session.thread.update(cx, |thread, cx| {
                ...
                let summary_model = summarization_model
                    .clone()
                    .or_else(|| thread.model().cloned());
                thread.set_summarization_model(summary_model, cx);
            });
        }
    }
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                ...
                cx.new(|cx| {
                    ...
                    let summary_model = summarization_model.or_else(|| thread.model().cloned());
                    thread.set_summarization_model(summary_model, cx);
                    thread
                })
            })
