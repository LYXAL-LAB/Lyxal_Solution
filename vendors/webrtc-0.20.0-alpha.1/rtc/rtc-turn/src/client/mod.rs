...
impl ClientConfig {
    pub fn new(local_addr: SocketAddr, turn_serv_addr: SocketAddr, transport_protocol: TransportProtocol) -> Self {
        Self {
...
impl Client {
    /// new returns a new Client instance. listeningAddress is the address and port to listen on, default "0.0.0.0:0"
        /// local_addr returns the local address of the client
    pub fn local_addr(&self) -> SocketAddr {
        self.local_addr
    }

    /// peer_addr returns the TURN server address if set
    pub fn peer_addr(&self) -> SocketAddr {
        self.turn_serv_addr.unwrap_or_else(|| "0.0.0.0:0".parse().unwrap())
    }
