    ...
    pub(crate) async fn event_loop(
        ...
        let create_socket_recv_future = |idx: usize,
            ...
            match socket.recv_from(&mut buf).await {
                Ok((n, peer_addr)) => Ok::<_, (std::io::Error, usize)>((n, local_addr, peer_addr, idx, buf)),
                Err(e) => Err((e, idx)),
            }
        };
                ...
                result = socket_recv_futures.next().fuse() => {
                    match result {
                        ...
                        Some(Err((err, idx))) => {
                            ...
                            let (local_addr, socket) = &socket_list[idx];
                            let new_buf = vec![0u8; 2000];
                            socket_recv_futures.push(
                                create_socket_recv_future(idx, *local_addr, socket.clone(), new_buf).boxed()
                            );
                        }
                        None => {
