...
pub fn parse(ics: &str) -> Result<IcalObject, IcalError> {
    ...
    for (idx, line) in unfolded.iter().enumerate() {
        ...
        if let Some(rest) = trimmed.strip_prefix("BEGIN:") {
            let name = rest.trim();
            stack.push(PartialComponent::new(name.to_string()));
            continue;
        }
        if let Some(rest) = trimmed.strip_prefix("END:") {
            let end_name = rest.trim().to_string();
            ...
            } else if matches!(component, Component::VEvent{..}|Component::VTodo{..}|Component::VJournal{..}|Component::VFreebusy{..}|Component::VTimezone{..}|Component::Other{..}) {
                // Must be VCALENDAR at root
                if let Component::Other { name, .. } = &component {
                    if name.to_ascii_uppercase() == "VCALENDAR" {
                        if let Component::Other { props, subcomponents, .. } = component {
    ...
    if !stack.is_empty() {
        return Err(IcalError::UnexpectedEnd(stack.last().unwrap().name.clone()));
    }
...
fn parse_property(line: &str, line_no: usize) -> Result<Property, IcalError> {
    ...
    let value = parts.next().ok_or(IcalError::ParseError { line: line_no, reason: "Missing ':'".into() })?;
