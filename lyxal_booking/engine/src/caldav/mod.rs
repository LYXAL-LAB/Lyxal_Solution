...
impl CaldavClient {
    ...
    pub fn with_bearer(base_url: &str, access_token: &str) -> Result<Self> {
        Self::build(
            base_url,
            CaldavAuth::Bearer {
    ...
    fn build(base_url: &str, auth: CaldavAuth) -> Result<Self> {
        ...
        if !matches!(parsed.scheme(), "http" | "https") {
            bail!("Unsupported CalDAV URL scheme: {}", parsed.scheme());
        }

        let host = parsed
            .host_str()
            .ok_or_else(|| anyhow::anyhow!("CalDAV URL has no host"))?;

        if !parsed.username().is_empty() || parsed.password().is_some() {
            bail!("Credentials are not allowed in CalDAV URL");
        }
    ...
    /// Resolve a href enforcing same-origin policy to prevent cross-origin credential leaks.
    fn resolve_url(&self, href: &str) -> Result<String> {
        let resolved = if href.starts_with("http://") || href.starts_with("https://") {
            reqwest::Url::parse(href)?
        } else {
            reqwest::Url::parse(&self.origin)?.join(href)?
        };

        let expected = reqwest::Url::parse(&self.origin)?;

        if resolved.scheme() != expected.scheme()
            || resolved.host_str() != expected.host_str()
            || resolved.port_or_known_default() != expected.port_or_known_default()
        {
            bail!("CalDAV href points to a different origin (cross-origin leak prevented): {}", href);
        }
...
<d:propfind xmlns:d="DAV:"><d:prop><d:getetag/></d:prop></d:propfind>"#;
    ...
    pub async fn fetch_events(&self, calendar_href: &str) -> Result<Vec<RawEvent>> {
        ...
        if !status.is_success() && status.as_u16() != 207 {
            let body = resp.text().await.unwrap_or_default();
            let snippet: String = body.chars().take(500).collect();
            bail!("REPORT {} returned {}: {}", url, status, snippet);
        }
...
<D:multistatus xmlns:D="DAV:" xmlns:b="http://calendarserver.org/ns/" xmlns:a="http://apple.com/ns/ical/"><D:response><D:href>/SOGo/dav/user@example.com/Calendar/</D:href><D:propstat><D:status>HTTP/1.1 200 OK</D:status><D:prop><D:resourcetype><D:collection/></D:resourcetype><D:displayname>Calendar</D:displayname></D:prop></D:propstat></D:response><D:response><D:href>/SOGo/dav/user@example.com/Calendar/personal/</D:href><D:propstat><D:status>HTTP/1.1 200 OK</D:status><D:prop><D:resourcetype><D:collection/><calendar xmlns="urn:ietf:params:xml:ns:caldav"/><vevent-collection xmlns="http://groupdav.org/"/><vtodo-collection xmlns="http://groupdav.org/"/><schedule-outbox xmlns="urn:ietf:params:xml:ns:caldav"/></D:resourcetype><D:displayname>Personal Calendar</D:displayname><a:calendar-color>#AAAAAAFF</a:calendar-color><b:getctag>-1</b:getctag><D:sync-token>-1</D:sync-token></D:prop></D:propstat></D:response></D:multistatus>"#;
    ...
    fn resolve_absolute_path() {
        ...
        .unwrap();
        assert_eq!(
            client.resolve_url("/principals/users/alice/").unwrap(),
    ...
    fn resolve_with_port() {
        let client = CaldavClient::new("https://cal.example.com:8443/dav", "user", "pass").unwrap();
        assert_eq!(
            client.resolve_url("/calendars/alice/").unwrap(),
    ...
    fn resolve_with_port_nextcloud_principal() {
        let client = CaldavClient::new("https://my-nextcloud:8080/remote.php/dav/", "user", "pass").unwrap();
        assert_eq!(
            client.resolve_url("/remote.php/dav/principals/users/alice/").unwrap(),
    ...
    fn resolve_without_port_https_default() {
        let client = CaldavClient::new("https://cal.example.com/dav", "user", "pass").unwrap();
        assert_eq!(
            client.resolve_url("/calendars/alice/").unwrap(),
    ...
    fn resolve_with_explicit_default_port_stripped() {
        let client = CaldavClient::new("https://cal.example.com:443/dav", "user", "pass").unwrap();
        assert_eq!(
            client.resolve_url("/calendars/alice/").unwrap(),
            "https://cal.example.com/calendars/alice/"
        );
...
<d:multistatus>
  <d:response>
    ...
    // --- CaldavClient::new origin parsing & Same-Origin checks ---

    #[test]
    fn client_origin_parsing() {
        let client = CaldavClient::new("https://cloud.example.com/remote.php/dav", "u", "p").unwrap();
    ...
    fn client_trims_trailing_slash() {
        let client = CaldavClient::new("https://cloud.example.com/dav/", "u", "p").unwrap();
        assert_eq!(client.base_url, "https://cloud.example.com/dav");
    }

    #[test]
    fn reject_cross_origin_absolute_url() {
        let client = CaldavClient::new("https://nextcloud.example.com/dav", "user", "pass").unwrap();
        assert!(client.resolve_url("https://attacker.example/collect").is_err());
        assert_eq!(
            client.resolve_url("/principals/users/alice/").unwrap(),
            "https://nextcloud.example.com/principals/users/alice/"
        );
    ...
    async fn google_discover_principal_short_circuits() {
        ...
        let client = CaldavClient::with_bearer(url, "fake-token").unwrap();
