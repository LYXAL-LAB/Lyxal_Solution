...
#[tokio::test]
async fn validate_surreal_schema_and_functions_import() -> anyhow::Result<()> {
    ...
    let base_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();

    // Ordre des 14 etapes impose par la Charte d'Architecture Lyxal OS
    let steps: Vec<(&str, Vec<&str>)> = vec![
        (
            "01_options_and_context",
            vec![], // Contexte namespace/database selectionne par SurrealBookingStore
        ),
        (
            "02_tables",
            vec![
        ...
        (
            ...
            vec!["schema/account_indexes.surql"],
        ),
        (
            "04_relations",
            vec![
    ...
    println!("=== DEMARRAGE VALIDATION IMPORT SURREALQL (MEMOIRE) ===");

    let mut total_success = 0;
    let mut total_failed = 0;

    for (step_name, rel_paths) in steps {
        println!("--- ETAPE : {} ---", step_name);
        for rel_path in rel_paths {
            let full_path = base_dir.join(rel_path);
            let content = match std::fs::read_to_string(&full_path) {
                Ok(c) => c,
                Err(err) => {
                    println!("[ERREUR LECTURE] {}: {}", rel_path, err);
                    panic!("Fichier introuvable: {}", rel_path);
                }
            };

            let mut response = store.client().query(&content).await?;
            let errors = response.take_errors();
            if !errors.is_empty() {
                println!("[ERREUR SURREALQL] {}: {:?}", rel_path, errors);
                total_failed += 1;
                panic!("Erreur d'execution SurrealQL dans {}", rel_path);
            } else {
                println!("[OK IMPORT] {}", rel_path);
                total_success += 1;
            }
        }
    }

    println!("=== BILAN : {} FICHIERS SURREALQL IMPORTES AVEC SUCCES (0 ERREURS) ===", total_success);
    Ok(())
}
