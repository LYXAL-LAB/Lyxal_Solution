...
impl<T: Revisioned + SerializeRevisioned> SerializeRevisioned for AlterKind<T> {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		...
		SerializeRevisioned::serialize_lyxal_revisioned(&Self::revision(), w)?;
		match self {
			AlterKind::None => SerializeRevisioned::serialize_lyxal_revisioned(&0u32, w)?,
			AlterKind::Set(x) => {
				SerializeRevisioned::serialize_lyxal_revisioned(&1u32, w)?;
				SerializeRevisioned::serialize_lyxal_revisioned(x, w)?;
			}
			AlterKind::Drop => {
				SerializeRevisioned::serialize_lyxal_revisioned(&2u32, w)?;
			}
		}
		Ok(())
	}
