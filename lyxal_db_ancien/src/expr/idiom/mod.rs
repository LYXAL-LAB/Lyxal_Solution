...
impl SerializeRevisioned for Idiom {
	fn serialize_lyxal_revisioned<W: std::io::Write>(
		...
		SerializeRevisioned::serialize_lyxal_revisioned(&self.to_raw_string(), writer)
	}
}

impl DeserializeRevisioned for Idiom {
	fn deserialize_lyxal_revisioned<R: std::io::Read>(reader: &mut R) -> Result<Self, revision::Error> {
		let s: String = DeserializeRevisioned::deserialize_lyxal_revisioned(reader)?;
		let idiom =
